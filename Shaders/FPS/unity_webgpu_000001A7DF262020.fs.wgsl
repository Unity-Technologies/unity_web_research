...
fn main_1() {
  ...
  u_xlat1 = ((vec3<f32>(x_1306.z, x_1306.x, x_1306.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_1314 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_1314);
  let x_1316 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_1316 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_1323 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_1323, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_1327 : vec3<f32> = u_xlat1;
  let x_1330 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (x_1327 * vec3<f32>(x_1330.z, x_1330.z, x_1330.z));
  let x_1334 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_1334);
  let x_1338 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1340 : vec2<f32> = (vec2<f32>(x_1338.x, x_1338.y) * vec2<f32>(0.5f, 0.5f));
  let x_1341 : vec3<f32> = u_xlat2;
  u_xlat2 = vec3<f32>(x_1340.x, x_1340.y, x_1341.z);
  let x_1343 : vec3<f32> = u_xlat11;
  let x_1346 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1349 : vec3<f32> = u_xlat2;
  let x_1351 : vec2<f32> = ((vec2<f32>(x_1343.y, x_1343.z) * vec2<f32>(x_1346.x, x_1346.y)) + vec2<f32>(x_1349.x, x_1349.y));
  let x_1352 : vec3<f32> = u_xlat2;
  u_xlat2 = vec3<f32>(x_1352.x, x_1351.x, x_1351.y);
  let x_1355 : f32 = u_xlat11.x;
  let x_1357 : f32 = x_26.x_Lut2D_Params.y;
  let x_1360 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_1355 * x_1357) + x_1360);
  let x_1368 : vec3<f32> = u_xlat2;
  let x_1370 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_1368.x, x_1368.z));
  let x_1371 : vec3<f32> = vec3<f32>(x_1370.x, x_1370.y, x_1370.z);
  let x_1372 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1371.x, x_1371.y, x_1371.z, x_1372.w);
  let x_1375 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat4.x = x_1375;
  u_xlat4.y = 0.0f;
  let x_1378 : vec3<f32> = u_xlat2;
  let x_1380 : vec4<f32> = u_xlat4;
  u_xlat21 = (vec2<f32>(x_1378.x, x_1378.z) + vec2<f32>(x_1380.x, x_1380.y));
  let x_1386 : vec2<f32> = u_xlat21;
  let x_1387 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_1386);
  u_xlat2 = vec3<f32>(x_1387.x, x_1387.y, x_1387.z);
  let x_1390 : f32 = u_xlat1.x;
  let x_1392 : f32 = x_26.x_Lut2D_Params.z;
  let x_1395 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_1390 * x_1392) + -(x_1395));
  let x_1399 : vec4<f32> = u_xlat3;
  let x_1402 : vec3<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_1399.x, x_1399.y, x_1399.z)) + x_1402);
  let x_1404 : vec3<f32> = u_xlat1;
  let x_1406 : vec3<f32> = u_xlat11;
  let x_1408 : vec4<f32> = u_xlat3;
  let x_1410 : vec3<f32> = ((vec3<f32>(x_1404.x, x_1404.x, x_1404.x) * x_1406) + vec3<f32>(x_1408.x, x_1408.y, x_1408.z));
  let x_1411 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1410.x, x_1410.y, x_1410.z, x_1411.w);
  let x_1415 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1415);
  let x_1417 : bool = u_xlatb1;
  if (x_1417) {
    let x_1420 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_1420.x, x_1420.y, x_1420.z);
    let x_1422 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_1422, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1426 : vec3<f32> = u_xlat1;
    u_xlat0.w = dot(x_1426, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_356 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_355.x, x_355.y, x_355.z, x_356.w);
  let x_359 : vec4<f32> = u_xlat1;
  let x_363 : vec3<f32> = ((vec3<f32>(x_359.x, x_359.y, x_359.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_364 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_363.x, x_363.y, x_363.z, x_364.w);
  let x_367 : vec4<f32> = u_xlat0;
  SV_Target0 = x_367;
  let x_370 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_370;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

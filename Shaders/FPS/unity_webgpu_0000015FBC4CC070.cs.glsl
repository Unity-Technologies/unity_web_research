...
layout(set = 0, binding = 1) uniform highp texture2D Depth;
writeonly layout(set = 0, binding = 2, rg32f) highp uniform image2D LinearZ_origX0X;
writeonly layout(set = 0, binding = 3, rg32f) highp uniform image2D DS2x_origX1X;
writeonly layout(set = 0, binding = 4, rg32f) highp uniform image2DArray DS2xAtlas_origX2X;
writeonly layout(set = 0, binding = 5, rg32f) highp uniform image2D DS4x_origX3X;
writeonly layout(set = 0, binding = 6, rg32f) highp uniform image2DArray DS4xAtlas_origX4X;
vec2 u_xlat0;
ivec2 u_xlati0;
uvec4 u_xlatu0;
bvec2 u_xlatb0;
...
void main()
{
    ...
    u_xlati8.x = int(int_bitfieldInsert(int(gl_GlobalInvocationID.x), int(gl_GlobalInvocationID.y), 2 & 0x1F, 30));
    ...
    if(u_xlati8.x == 0) {
        ...
        u_xlati1 = int(int_bitfieldInsert(int(u_xlatu8.x), int(u_xlatu8.y), 2 & 0x1F, 30));

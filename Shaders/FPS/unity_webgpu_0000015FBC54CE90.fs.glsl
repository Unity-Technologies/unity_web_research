...
layout(set = 0, binding = 1) uniform highp texture2D _DepthTex;
...
layout(set = 0, binding = 4) uniform highp  sampler sampler_DepthTex;
void main()
{
    u_xlat0 = texture(sampler2D(_MainTex, sampler_MainTex), vs_TEXCOORD0.xy);
    SV_Target0 = u_xlat0 * _Color;
    u_xlat0.x = texture(sampler2D(_DepthTex, sampler_DepthTex), vs_TEXCOORD0.xy).x;

...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_745.w, x_745.x, x_745.y, x_745.z) * vec4<f32>(x_756.x, x_756.y, x_756.z, x_756.w));
  let x_763 : vec4<f32> = u_xlat0;
  let x_770 : vec3<f32> = ((vec3<f32>(x_763.y, x_763.z, x_763.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_800 : vec2<f32> = x_31.x_Lut3D_Params;
  ...
  let x_807 : f32 = x_31.x_Lut3D_Params.x;
  u_xlat19 = (x_807 * 0.5f);
  let x_809 : vec4<f32> = u_xlat1;
  let x_812 : vec2<f32> = x_31.x_Lut3D_Params;
  let x_815 : f32 = u_xlat19;
  let x_817 : vec3<f32> = ((vec3<f32>(x_809.x, x_809.y, x_809.z) * vec3<f32>(x_812.x, x_812.x, x_812.x)) + vec3<f32>(x_815, x_815, x_815));
  ...
  u_xlat0 = vec4<f32>(x_832.x, x_831.x, x_831.y, x_831.z);
  let x_837 : f32 = x_31.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_837);
  let x_839 : bool = u_xlatb1;
  if (x_839) {
    let x_842 : vec4<f32> = u_xlat0;
    let x_843 : vec3<f32> = vec3<f32>(x_842.y, x_842.z, x_842.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_853.x, x_853.y, x_853.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_863 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_863.y, x_863.z, x_863.w, x_863.x);
  return;
}

...
fn main_1() {
  ...
  let x_268 : f32 = x_31.x_Metallic;
  let x_270 : f32 = x_31.x_Metallic;
  let x_271 : vec3<f32> = vec3<f32>(x_266, x_268, x_270);
  let x_276 : vec3<f32> = u_xlat0;
  let x_280 : vec3<f32> = ((vec3<f32>(x_271.x, x_271.y, x_271.z) * x_276) + vec3<f32>(0.039999999f, 0.039999999f, 0.039999999f));
  let x_281 : vec4<f32> = SV_Target1;
  SV_Target1 = vec4<f32>(x_280.x, x_280.y, x_280.z, x_281.w);
  let x_284 : f32 = x_31.x_Metallic;
  u_xlat0.x = ((-(x_284) * 0.959999979f) + 0.959999979f);
  let x_290 : vec3<f32> = u_xlat0;
  let x_292 : vec3<f32> = u_xlat1;
  u_xlat0 = (vec3<f32>(x_290.x, x_290.x, x_290.x) * x_292);
  let x_294 : vec4<f32> = u_xlat2;
  let x_296 : vec3<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_294.x, x_294.y, x_294.z) * x_296);
  let x_299 : vec3<f32> = u_xlat1;
  let x_301 : vec3<f32> = exp2(-(x_299));
  let x_302 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_301.x, x_301.y, x_301.z, x_302.w);
  let x_305 : vec3<f32> = u_xlat0;
  let x_306 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_305.x, x_305.y, x_305.z, x_306.w);
  SV_Target0.w = 1.0f;
  let x_312 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_312;
  let x_315 : vec3<f32> = vs_TEXCOORD1;
  let x_318 : vec3<f32> = ((x_315 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_319 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_318.x, x_318.y, x_318.z, x_319.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}
...
@fragment
fn main(@location(0) vs_TEXCOORD0_param : vec2<f32>, @location(3) vs_COLOR0_param : vec4<f32>, @location(2) vs_TEXCOORD2_param : vec3<f32>, @location(1) vs_TEXCOORD1_param : vec3<f32>, @location(4) vs_TEXCOORD5_param : vec3<f32>, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

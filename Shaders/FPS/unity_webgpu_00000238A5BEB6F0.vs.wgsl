...
fn main_1() {
  ...
  let x_444 : vec4<f32> = x_433.unity_Builtins2Array[(x_441 / 7i)].unity_SHBgArray;
  let x_445 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_444, x_445);
  let x_448 : i32 = u_xlati0;
  let x_452 : vec4<f32> = x_433.unity_Builtins2Array[(x_448 / 7i)].unity_SHBbArray;
  let x_453 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_452, x_453);
  let x_458 : i32 = u_xlati0;
  let x_462 : vec4<f32> = x_433.unity_Builtins2Array[(x_458 / 7i)].unity_SHCArray;
  let x_464 : vec3<f32> = u_xlat5;
  let x_467 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_462.x, x_462.y, x_462.z) * vec3<f32>(x_464.x, x_464.x, x_464.x)) + vec3<f32>(x_467.x, x_467.y, x_467.z));
...
fn main(@builtin(instance_index) gl_InstanceIndex_param : u32, @location(0) in_POSITION0_param : vec4<f32>, @location(4) in_TANGENT0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_COLOR0, vs_TEXCOORD3, vs_TEXCOORD7, vs_SV_InstanceID0);
}

#line 1 "Particles/Standard Unlit"
...
v2f_img o ;
o = ( v2f_img ) 0 ; ;
{ UnitySetupInstanceID ( v . instanceID ) ; vertInstancingSetup ( ) ; UnitySetupCompoundMatrices ( ) ; } ;
;
...
#line 14 "F:/src/unity_webgpu/Shaders/Includes/UnityStandardParticleInstancing.cginc"
...
{
DefaultParticleInstanceData data = unity_ParticleInstanceData [ unity_InstanceID ] ;
#line 30
...
DefaultParticleInstanceData data = unity_ParticleInstanceData [ unity_InstanceID ] ;
color = lerp ( float4 ( 1.0f , 1.0f , 1.0f , 1.0f ) , color , unity_ParticleUseMeshColors ) ;
...
vertInstancingUVs ( v . texcoords . xy , o . texcoord ) ; o . texcoord = ( o . texcoord . xy * _MainTex_ST . xy + _MainTex_ST . zw ) ; ;
...
albedo *= IN . color ; ;
float softParticlesFade = 1.0f ; if ( _SoftParticleFadeParams . x > 0.0 || _SoftParticleFadeParams . y > 0.0 ) { float sceneZ = LinearEyeDepth ( ( tex2Dproj ( _CameraDepthTexture , IN . projectedPosition ) . r ) ) ; softParticlesFade = saturate ( _SoftParticleFadeParams . y * ( ( sceneZ - _SoftParticleFadeParams . x ) - IN . projectedPosition . z ) ) ; albedo . a *= softParticlesFade ; } ;
float cameraFade = saturate ( ( IN . projectedPosition . z - _CameraFadeParams . x ) * _CameraFadeParams . y ) ; albedo . a *= cameraFade ; ;
#line 309
...
float3 emission = 0 ;
#line 324
float4 grabPosUV = IN . grabPassPosition ; grabPosUV . xy += normal . xy * _DistortionStrengthScaled * albedo . a ; float3 grabPass = tex2Dproj ( _GrabTexture , grabPosUV ) . rgb ; albedo . rgb = lerp ( grabPass , albedo . rgb , saturate ( albedo . a - _DistortionBlend ) ) ; ;

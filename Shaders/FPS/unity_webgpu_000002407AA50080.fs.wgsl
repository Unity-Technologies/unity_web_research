...
fn main_1() {
  ...
  let x_543 : vec3<f32> = ((vec3<f32>(x_536.z, x_536.x, x_536.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_572 : vec3<f32> = x_49.x_Lut2D_Params;
  u_xlat6 = (vec3<f32>(x_568.x, x_568.y, x_568.z) * vec3<f32>(x_572.z, x_572.z, x_572.z));
  let x_576 : f32 = u_xlat6.x;
  u_xlat6.x = floor(x_576);
  let x_580 : vec3<f32> = x_49.x_Lut2D_Params;
  let x_583 : vec2<f32> = (vec2<f32>(x_580.x, x_580.y) * vec2<f32>(0.5f, 0.5f));
  let x_584 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_583.x, x_583.y, x_584.z, x_584.w);
  let x_586 : vec3<f32> = u_xlat6;
  let x_589 : vec3<f32> = x_49.x_Lut2D_Params;
  ...
  u_xlat11 = (vec2<f32>(x_623.x, x_623.z) + vec2<f32>(x_625.x, x_625.y));
  let x_631 : vec2<f32> = u_xlat11;
  let x_632 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_631);
  ...
  if (x_665) {
    ...
    let x_676 : vec3<f32> = clamp(vec3<f32>(x_672.x, x_672.y, x_672.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_677 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_676.x, x_676.y, x_676.z, x_677.w);
    let x_679 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_679.x, x_679.y, x_679.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

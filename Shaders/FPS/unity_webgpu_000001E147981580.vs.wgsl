...
@group(0) @binding(5) var<uniform> x_38 : UnityInstancing_PerDraw0;

var<private> u_xlatb3 : bool;

@group(0) @binding(3) var<uniform> x_80 : VGlobals;
...
fn main_1() {
  ...
  u_xlat1.x = min(x_213, x_215);
  let x_219 : f32 = u_xlat0.z;
  let x_222 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_219) + x_222);
  let x_230 : f32 = x_80.unity_LightShadowBias.y;
  let x_232 : f32 = u_xlat1.x;
  let x_235 : f32 = u_xlat0.z;
  gl_Position.z = ((x_230 * x_232) + x_235);
  let x_244 : vec2<f32> = in_TEXCOORD0;
  let x_246 : vec4<f32> = x_80.x_MainTex_ST;
  let x_250 : vec4<f32> = x_80.x_MainTex_ST;
  vs_TEXCOORD1 = ((x_244 * vec2<f32>(x_246.x, x_246.y)) + vec2<f32>(x_250.z, x_250.w));
  let x_256 : vec4<f32> = in_COLOR0;
  vs_TEXCOORD3 = x_256;
  let x_257 : vec4<f32> = u_xlat0;
  let x_258 : vec3<f32> = vec3<f32>(x_257.x, x_257.y, x_257.w);
  let x_260 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_258.x, x_258.y, x_260.z, x_258.z);
  return;
}
...
@vertex
fn main(@builtin(instance_index) gl_InstanceIndex_param : u32, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>, @location(2) in_COLOR0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_TEXCOORD1, vs_TEXCOORD3);
}

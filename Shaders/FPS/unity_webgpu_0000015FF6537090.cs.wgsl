...
fn main_1() {
  ...
  if (x_51) {
    ...
    let x_590 : vec3<f32> = (vec3<f32>(x_586.x, x_586.y, x_586.z) * vec3<f32>(1.313386083f, 1.313386083f, 1.313386083f));
    ...
    let x_610 : vec3<f32> = ((vec3<f32>(x_603.x, x_603.y, x_603.z) * vec3<f32>(x_605.x, x_605.y, x_605.z)) + vec3<f32>(0.00544f, 0.00544f, 0.00544f));
    let x_611 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_610.x, x_610.y, x_610.z, x_611.w);
    let x_613 : vec4<f32> = u_xlat0;
    let x_618 : vec3<f32> = ((vec3<f32>(x_613.x, x_613.y, x_613.z) * vec3<f32>(0.262677222f, 0.262677222f, 0.262677222f)) + vec3<f32>(0.289999992f, 0.289999992f, 0.289999992f));
    ...
    let x_628 : vec3<f32> = ((vec3<f32>(x_621.x, x_621.y, x_621.z) * vec3<f32>(x_623.x, x_623.y, x_623.z)) + vec3<f32>(0.08160001f, 0.08160001f, 0.08160001f));
    ...
    let x_642 : vec3<f32> = (vec3<f32>(x_638.x, x_638.y, x_638.z) + vec3<f32>(-0.066666663f, -0.066666663f, -0.066666663f));
    let x_643 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_642.x, x_642.y, x_642.z, x_643.w);
    let x_645 : vec4<f32> = u_xlat0;
    let x_647 : vec3<f32> = (vec3<f32>(x_645.x, x_645.y, x_645.z) * vec3<f32>(1.313386083f, 1.313386083f, 1.313386083f));
    ...
    u_xlat0.w = 1.0f;
    let x_660 : vec3<u32> = gl_GlobalInvocationID;
    let x_663 : vec4<f32> = u_xlat0;
    textureStore(x_Output_origX0X, bitcast<vec3<i32>>(x_660), x_663);
  }
  return;
}

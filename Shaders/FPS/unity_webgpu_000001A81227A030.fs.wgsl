...
fn main_1() {
  ...
  let x_876 : f32 = x_55.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_876);
  let x_878 : bool = u_xlatb0;
  if (x_878) {
    let x_881 : vec4<f32> = u_xlat1;
    u_xlat0 = vec3<f32>(x_881.x, x_881.y, x_881.z);
    let x_883 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_883, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_887 : vec3<f32> = u_xlat0;
    u_xlat1.w = dot(x_887, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_818 : f32 = x_26.x_PostExposure;
  let x_820 : f32 = x_26.x_PostExposure;
  let x_822 : f32 = x_26.x_PostExposure;
  let x_824 : f32 = x_26.x_PostExposure;
  let x_825 : vec4<f32> = vec4<f32>(x_818, x_820, x_822, x_824);
  u_xlat0 = (x_815 * vec4<f32>(x_825.x, x_825.y, x_825.z, x_825.w));
  let x_832 : vec4<f32> = u_xlat0;
  u_xlat1 = ((vec3<f32>(x_832.z, x_832.x, x_832.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_840 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_840);
  let x_842 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_842 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_849 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_849, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_853 : vec3<f32> = u_xlat1;
  let x_856 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (x_853 * vec3<f32>(x_856.z, x_856.z, x_856.z));
  let x_860 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_860);
  let x_864 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_866 : vec2<f32> = (vec2<f32>(x_864.x, x_864.y) * vec2<f32>(0.5f, 0.5f));
  let x_867 : vec3<f32> = u_xlat2;
  u_xlat2 = vec3<f32>(x_866.x, x_866.y, x_867.z);
  let x_869 : vec3<f32> = u_xlat11;
  let x_872 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_875 : vec3<f32> = u_xlat2;
  let x_877 : vec2<f32> = ((vec2<f32>(x_869.y, x_869.z) * vec2<f32>(x_872.x, x_872.y)) + vec2<f32>(x_875.x, x_875.y));
  let x_878 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_878.x, x_877.x, x_877.y, x_878.w);
  let x_881 : f32 = u_xlat11.x;
  let x_883 : f32 = x_26.x_Lut2D_Params.y;
  let x_886 : f32 = u_xlat3.y;
  u_xlat3.x = ((x_881 * x_883) + x_886);
  let x_894 : vec4<f32> = u_xlat3;
  let x_896 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_894.x, x_894.z));
  let x_897 : vec3<f32> = vec3<f32>(x_896.x, x_896.y, x_896.z);
  let x_898 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_897.x, x_897.y, x_897.z, x_898.w);
  let x_901 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat22.x = x_901;
  let x_903 : vec2<f32> = u_xlat22;
  let x_904 : vec4<f32> = u_xlat3;
  u_xlat21 = (x_903 + vec2<f32>(x_904.x, x_904.z));
  let x_910 : vec2<f32> = u_xlat21;
  let x_911 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_910);
  u_xlat2 = vec3<f32>(x_911.x, x_911.y, x_911.z);
  let x_914 : f32 = u_xlat1.x;
  let x_916 : f32 = x_26.x_Lut2D_Params.z;
  let x_919 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_914 * x_916) + -(x_919));
  let x_923 : vec4<f32> = u_xlat4;
  let x_926 : vec3<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_923.x, x_923.y, x_923.z)) + x_926);
  ...
  let x_939 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_939);
  let x_941 : bool = u_xlatb1;
  if (x_941) {
    let x_944 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_944.x, x_944.y, x_944.z);
    let x_946 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_946, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_950 : vec3<f32> = u_xlat1;
    u_xlat0.w = dot(x_950, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

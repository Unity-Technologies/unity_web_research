...
fn main_1() {
  ...
  u_xlat0 = ((x_1064 * x_1065) + x_1067);
  let x_1074 : vec4<f32> = vs_TEXCOORD0;
  let x_1076 : vec4<f32> = textureSample(x_EmissionMap, sampler_EmissionMap, vec2<f32>(x_1074.x, x_1074.y));
  u_xlat1 = vec3<f32>(x_1076.x, x_1076.y, x_1076.z);
  let x_1080 : vec3<f32> = u_xlat1;
  let x_1083 : vec4<f32> = x_33.x_EmissionColor;
  let x_1086 : vec3<f32> = u_xlat0;
  let x_1087 : vec3<f32> = ((x_1080 * vec3<f32>(x_1083.x, x_1083.y, x_1083.z)) + x_1086);
  let x_1088 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1087.x, x_1087.y, x_1087.z, x_1088.w);
  SV_Target0.w = 1.0f;
  return;
}

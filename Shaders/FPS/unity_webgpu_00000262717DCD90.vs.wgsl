...
@group(0) @binding(15) var<uniform> x_18 : VGlobals;
...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_378.y, x_378.y, x_378.y, x_378.y) * x_382);
  let x_385 : vec4<f32> = x_18.unity_WorldToLight[0i];
  let x_386 : vec4<f32> = u_xlat0;
  let x_389 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_385 * vec4<f32>(x_386.x, x_386.x, x_386.x, x_386.x)) + x_389);
  let x_392 : vec4<f32> = x_18.unity_WorldToLight[2i];
  let x_393 : vec4<f32> = u_xlat0;
  let x_396 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_392 * vec4<f32>(x_393.z, x_393.z, x_393.z, x_393.z)) + x_396);
  let x_400 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_401 : vec4<f32> = u_xlat0;
  let x_404 : vec4<f32> = u_xlat1;
  vs_TEXCOORD6 = ((x_400 * vec4<f32>(x_401.w, x_401.w, x_401.w, x_401.w)) + x_404);

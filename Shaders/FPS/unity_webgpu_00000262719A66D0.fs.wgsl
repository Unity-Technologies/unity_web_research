...
fn main_1() {
  ...
  let x_384 : vec3<f32> = exp2(-(vec3<f32>(x_381.x, x_381.y, x_381.z)));
  let x_385 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_384.x, x_384.y, x_384.z, x_385.w);
  let x_388 : vec4<f32> = u_xlat1;
  let x_392 : vec3<f32> = ((vec3<f32>(x_388.x, x_388.y, x_388.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_393 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_392.x, x_392.y, x_392.z, x_393.w);
  let x_396 : vec4<f32> = u_xlat0;
  SV_Target0 = x_396;
  let x_399 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_399;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

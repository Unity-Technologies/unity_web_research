...
fn main_1() {
  ...
  let x_160 : f32 = u_xlat1.w;
  vs_TEXCOORD3.w = x_160;
  let x_162 : vec4<f32> = u_xlat0;
  let x_164 : vec4<f32> = u_xlat0;
  let x_166 : vec2<f32> = (vec2<f32>(x_162.z, x_162.z) + vec2<f32>(x_164.x, x_164.w));
  let x_167 : vec4<f32> = vs_TEXCOORD3;
  vs_TEXCOORD3 = vec4<f32>(x_166.x, x_166.y, x_167.z, x_167.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(1) in_COLOR0_param : vec4<f32>, @location(2) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_COLOR0, vs_TEXCOORD1, vs_TEXCOORD3);
}

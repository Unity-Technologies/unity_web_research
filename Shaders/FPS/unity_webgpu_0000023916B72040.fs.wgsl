...
fn main_1() {
  ...
  let x_905 : f32 = x_31.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_905);
  ...
  if (x_907) {
    ...
    let x_918 : vec3<f32> = clamp(vec3<f32>(x_914.x, x_914.y, x_914.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_919 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_918.x, x_918.y, x_918.z, x_919.w);
    let x_921 : vec4<f32> = u_xlat0;
    u_xlat3.w = dot(vec3<f32>(x_921.x, x_921.y, x_921.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

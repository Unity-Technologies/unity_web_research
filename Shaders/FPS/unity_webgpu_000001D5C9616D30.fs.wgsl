...
fn main_1() {
  ...
  u_xlatb3 = vec3<bool>(x_478.x, x_478.y, x_478.z);
  let x_482 : vec3<f32> = u_xlat0;
  hlslcc_movcTemp = x_482;
  let x_484 : bool = u_xlatb3.x;
  if (x_484) {
  ...
  let x_688 : f32 = x_680;
  u_xlat2.z = x_688;
  let x_692 : f32 = x_55.x_LumaInAlpha;
  u_xlatb0.x = (0.5f < x_692);
  let x_696 : bool = u_xlatb0.x;
  if (x_696) {
    let x_699 : vec4<f32> = u_xlat2;
    u_xlat0 = vec3<f32>(x_699.x, x_699.y, x_699.z);
    let x_701 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_701, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_705 : vec3<f32> = u_xlat0;
    u_xlat2.w = dot(x_705, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

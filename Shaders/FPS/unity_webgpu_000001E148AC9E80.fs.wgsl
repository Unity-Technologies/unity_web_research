...
fn main_1() {
  ...
  let x_455 : vec3<f32> = ((vec3<f32>(x_448.x, x_448.y, x_448.z) * vec3<f32>(x_450.x, x_450.y, x_450.z)) + vec3<f32>(x_453.x, x_453.y, x_453.z));
  let x_456 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_455.x, x_455.y, x_455.z, x_456.w);
  let x_459 : vec4<f32> = u_xlat1;
  let x_463 : vec3<f32> = ((vec3<f32>(x_459.x, x_459.y, x_459.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_464 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_463.x, x_463.y, x_463.z, x_464.w);

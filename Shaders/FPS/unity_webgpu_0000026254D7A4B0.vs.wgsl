...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_456.y, x_456.w, x_456.z, x_456.x) * x_458);
  let x_461 : vec4<f32> = x_43.unity_SHBr;
  let x_462 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_461, x_462);
  let x_466 : vec4<f32> = x_43.unity_SHBg;
  let x_467 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_466, x_467);
  let x_471 : vec4<f32> = x_43.unity_SHBb;
  let x_472 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_471, x_472);
  let x_479 : vec4<f32> = x_43.unity_SHC;
  let x_481 : vec4<f32> = u_xlat0;
  let x_484 : vec4<f32> = u_xlat2;
  vs_TEXCOORD6 = ((vec3<f32>(x_479.x, x_479.y, x_479.z) * vec3<f32>(x_481.x, x_481.x, x_481.x)) + vec3<f32>(x_484.x, x_484.y, x_484.z));
  return;
}

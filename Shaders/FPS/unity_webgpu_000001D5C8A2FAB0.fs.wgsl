...
fn main_1() {
  ...
  u_xlat25 = dot(vec3<f32>(x_826.x, x_826.y, x_826.z), vec3<f32>(x_828.x, x_828.y, x_828.z));
  let x_836 : f32 = u_xlat25;
  let x_838 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_836, x_836));
  u_xlat25 = x_838.x;
  let x_848 : vec4<f32> = u_xlat4;
  let x_850 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, vec3<f32>(x_848.x, x_848.y, x_848.z));
  u_xlat4.x = x_850.w;
  let x_854 : f32 = u_xlat4.x;
  let x_855 : f32 = u_xlat25;
  u_xlat4.x = (x_854 * x_855);
  let x_858 : f32 = u_xlat24;
  let x_860 : f32 = u_xlat4.x;
  u_xlat24 = (x_858 * x_860);
  let x_863 : vec3<f32> = vs_TEXCOORD2;
  let x_864 : vec4<f32> = u_xlat3;
  u_xlat4.x = dot(x_863, -(vec3<f32>(x_864.x, x_864.y, x_864.z)));
  let x_870 : vec3<f32> = vs_TEXCOORD3;
  let x_871 : vec4<f32> = u_xlat3;
  u_xlat4.y = dot(x_870, -(vec3<f32>(x_871.x, x_871.y, x_871.z)));
  let x_877 : vec3<f32> = vs_TEXCOORD4;
  let x_878 : vec4<f32> = u_xlat3;
  u_xlat4.z = dot(x_877, -(vec3<f32>(x_878.x, x_878.y, x_878.z)));
  let x_883 : vec4<f32> = u_xlat4;
  let x_885 : vec4<f32> = u_xlat4;
  u_xlat3.x = dot(vec3<f32>(x_883.x, x_883.y, x_883.z), vec3<f32>(x_885.x, x_885.y, x_885.z));
  let x_890 : f32 = u_xlat3.x;
  u_xlat3.x = inverseSqrt(x_890);
  let x_893 : vec4<f32> = u_xlat3;
  let x_895 : vec4<f32> = u_xlat4;
  let x_897 : vec3<f32> = (vec3<f32>(x_893.x, x_893.x, x_893.x) * vec3<f32>(x_895.x, x_895.y, x_895.z));
  let x_898 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_897.x, x_897.y, x_897.z, x_898.w);
  let x_900 : f32 = u_xlat24;
  let x_904 : vec4<f32> = x_18.x_LightColor0;
  let x_906 : vec3<f32> = (vec3<f32>(x_900, x_900, x_900) * vec3<f32>(x_904.x, x_904.y, x_904.z));
  let x_907 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_906.x, x_906.y, x_906.z, x_907.w);
  let x_909 : vec3<f32> = u_xlat1;
  let x_910 : f32 = u_xlat21;
  let x_913 : vec3<f32> = u_xlat0;
  u_xlat1 = ((x_909 * vec3<f32>(x_910, x_910, x_910)) + x_913);
  let x_915 : vec3<f32> = u_xlat1;
  let x_916 : vec3<f32> = u_xlat1;
  u_xlat21 = dot(x_915, x_916);
  let x_918 : f32 = u_xlat21;
  u_xlat21 = inverseSqrt(x_918);
  let x_920 : f32 = u_xlat21;
  let x_922 : vec3<f32> = u_xlat1;
  u_xlat1 = (vec3<f32>(x_920, x_920, x_920) * x_922);
  let x_924 : vec4<f32> = u_xlat3;
  let x_926 : vec3<f32> = u_xlat0;
  u_xlat0.x = dot(vec3<f32>(x_924.x, x_924.y, x_924.z), x_926);
  let x_929 : vec4<f32> = u_xlat3;
  let x_931 : vec3<f32> = u_xlat1;
  u_xlat0.y = dot(vec3<f32>(x_929.x, x_929.y, x_929.z), x_931);
  let x_934 : vec3<f32> = u_xlat0;
  let x_937 : vec2<f32> = max(vec2<f32>(x_934.x, x_934.y), vec2<f32>(0.0f, 0.0f));
  let x_938 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_937.x, x_937.y, x_938.z);
  let x_941 : f32 = u_xlat22;
  u_xlat14 = (x_941 * 128.0f);
  let x_946 : f32 = u_xlat0.y;
  u_xlat7.x = log2(x_946);
  let x_950 : f32 = u_xlat7.x;
  let x_951 : f32 = u_xlat14;
  u_xlat7.x = (x_950 * x_951);
  let x_955 : f32 = u_xlat7.x;
  u_xlat7.x = exp2(x_955);
  let x_958 : vec4<f32> = u_xlat2;
  let x_960 : vec4<f32> = u_xlat4;
  u_xlat1 = (vec3<f32>(x_958.x, x_958.y, x_958.z) * vec3<f32>(x_960.x, x_960.y, x_960.z));
  let x_963 : vec4<f32> = u_xlat4;
  let x_967 : vec4<f32> = x_18.x_SpecColor;
  let x_969 : vec3<f32> = (vec3<f32>(x_963.x, x_963.y, x_963.z) * vec3<f32>(x_967.x, x_967.y, x_967.z));
  ...
  u_xlat7 = (vec3<f32>(x_972.x, x_972.x, x_972.x) * vec3<f32>(x_974.x, x_974.y, x_974.z));
  let x_979 : vec3<f32> = u_xlat1;
  let x_980 : vec3<f32> = u_xlat0;
  let x_983 : vec3<f32> = u_xlat7;
  let x_984 : vec3<f32> = ((x_979 * vec3<f32>(x_980.x, x_980.x, x_980.x)) + x_983);
  let x_985 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_984.x, x_984.y, x_984.z, x_985.w);
  let x_988 : f32 = u_xlat2.w;
  SV_Target0.w = x_988;
  return;
}

...
@group(0) @binding(13) var<uniform> x_19 : VGlobals;
...
fn main_1() {
  ...
  let x_612 : vec2<f32> = (vec2<f32>(x_606.y, x_606.y) * vec2<f32>(x_610.x, x_610.y));
  let x_613 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_612.x, x_612.y, x_613.z);
  let x_616 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_618 : vec4<f32> = u_xlat1;
  let x_621 : vec3<f32> = u_xlat0;
  let x_623 : vec2<f32> = ((vec2<f32>(x_616.x, x_616.y) * vec2<f32>(x_618.x, x_618.x)) + vec2<f32>(x_621.x, x_621.y));
  let x_624 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_623.x, x_623.y, x_624.z);
  let x_627 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_629 : vec4<f32> = u_xlat1;
  let x_632 : vec3<f32> = u_xlat0;
  let x_634 : vec2<f32> = ((vec2<f32>(x_627.x, x_627.y) * vec2<f32>(x_629.z, x_629.z)) + vec2<f32>(x_632.x, x_632.y));
  let x_635 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_634.x, x_634.y, x_635.z);
  let x_640 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_642 : vec4<f32> = u_xlat1;
  let x_645 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec2<f32>(x_640.x, x_640.y) * vec2<f32>(x_642.w, x_642.w)) + vec2<f32>(x_645.x, x_645.y));
  return;
}

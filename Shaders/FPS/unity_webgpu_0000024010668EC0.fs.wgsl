...
@group(0) @binding(0) var<uniform> x_38 : PGlobals;
...
fn main_1() {
  ...
  txVec0 = vec3<f32>(x_415.x, x_415.y, x_416);
  ...
  u_xlat0.x = x_430;
  let x_434 : f32 = x_38.x_LightShadowData.x;
  u_xlat4 = (-(x_434) + 1.0f);
  let x_439 : vec4<f32> = u_xlat0;
  let x_441 : f32 = u_xlat4;
  let x_445 : vec4<f32> = x_38.x_LightShadowData;
  SV_Target0 = ((vec4<f32>(x_439.x, x_439.x, x_439.x, x_439.x) * vec4<f32>(x_441, x_441, x_441, x_441)) + vec4<f32>(x_445.x, x_445.x, x_445.x, x_445.x));
  return;
}

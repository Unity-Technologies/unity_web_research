...
fn main_1() {
  ...
  let x_595 : f32 = x_51.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_595);
  let x_597 : bool = u_xlatb0;
  if (x_597) {
    ...
    let x_608 : vec3<f32> = clamp(vec3<f32>(x_604.x, x_604.y, x_604.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_609 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_608.x, x_608.y, x_608.z, x_609.w);
    let x_611 : vec4<f32> = u_xlat0;
    u_xlat2.w = dot(vec3<f32>(x_611.x, x_611.y, x_611.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

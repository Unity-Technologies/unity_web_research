...
fn main_1() {
  ...
  let x_210 : vec3<f32> = ((vec3<f32>(x_204.x, x_204.y, x_204.z) * vec3<f32>(x_206, x_206, x_206)) + x_209);
  let x_211 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_210.x, x_210.y, x_210.z, x_211.w);
  SV_Target0.w = 1.0f;
  return;
}
...
@fragment
fn main(@location(0) vs_TEXCOORD0_param : vec2<f32>, @location(2) vs_TEXCOORD2_param : vec3<f32>, @location(1) vs_TEXCOORD1_param : vec3<f32>, @location(3) vs_TEXCOORD3_param : vec3<f32>, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

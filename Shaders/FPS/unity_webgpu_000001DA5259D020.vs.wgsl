...
fn main_1() {
  ...
  let x_147 : vec4<f32> = in_COLOR0;
  vs_COLOR0 = x_147;
  let x_149 : f32 = u_xlat0.y;
  let x_152 : f32 = x_17.x_ProjectionParams.x;
  u_xlat0.y = (x_149 * x_152);
  let x_155 : vec4<f32> = u_xlat0;
  let x_159 : vec3<f32> = (vec3<f32>(x_155.x, x_155.w, x_155.y) * vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_160 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_159.x, x_160.y, x_159.y, x_159.z);
  let x_163 : vec4<f32> = u_xlat0;
  let x_164 : vec2<f32> = vec2<f32>(x_163.z, x_163.w);
  let x_165 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_165.x, x_165.y, x_164.x, x_164.y);
  let x_167 : vec4<f32> = u_xlat1;
  let x_169 : vec4<f32> = u_xlat1;
  let x_171 : vec2<f32> = (vec2<f32>(x_167.z, x_167.z) + vec2<f32>(x_169.x, x_169.w));
  let x_172 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_171.x, x_171.y, x_172.z, x_172.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(2) in_TEXCOORD0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(3) in_COLOR0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD2, gl_Position, vs_TEXCOORD0, vs_TEXCOORD1, vs_COLOR0, vs_TEXCOORD4);
}

...
fn main_1() {
  ...
  let x_468 : f32 = x_55.x_PostExposure;
  let x_470 : f32 = x_55.x_PostExposure;
  let x_472 : f32 = x_55.x_PostExposure;
  let x_474 : f32 = x_55.x_PostExposure;
  let x_475 : vec4<f32> = vec4<f32>(x_468, x_470, x_472, x_474);
  u_xlat0 = (x_465 * vec4<f32>(x_475.x, x_475.y, x_475.z, x_475.w));
  let x_482 : vec4<f32> = u_xlat0;
  let x_489 : vec3<f32> = ((vec3<f32>(x_482.z, x_482.x, x_482.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_504 : vec3<f32> = ((vec3<f32>(x_497.x, x_497.y, x_497.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_505 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_504.x, x_504.y, x_504.z, x_505.w);
  let x_507 : vec4<f32> = u_xlat1;
  let x_511 : vec3<f32> = clamp(vec3<f32>(x_507.x, x_507.y, x_507.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_512 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_511.x, x_511.y, x_511.z, x_512.w);
  let x_516 : vec4<f32> = u_xlat1;
  let x_520 : vec3<f32> = x_55.x_Lut2D_Params;
  u_xlat8 = (vec3<f32>(x_516.x, x_516.y, x_516.z) * vec3<f32>(x_520.z, x_520.z, x_520.z));
  let x_524 : f32 = u_xlat8.x;
  u_xlat8.x = floor(x_524);
  let x_528 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_531 : vec2<f32> = (vec2<f32>(x_528.x, x_528.y) * vec2<f32>(0.5f, 0.5f));
  let x_532 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_531.x, x_531.y, x_532.z, x_532.w);
  let x_534 : vec3<f32> = u_xlat8;
  let x_537 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_540 : vec4<f32> = u_xlat2;
  let x_542 : vec2<f32> = ((vec2<f32>(x_534.y, x_534.z) * vec2<f32>(x_537.x, x_537.y)) + vec2<f32>(x_540.x, x_540.y));
  let x_543 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_543.x, x_542.x, x_542.y, x_543.w);
  let x_546 : f32 = u_xlat8.x;
  let x_549 : f32 = x_55.x_Lut2D_Params.y;
  let x_552 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_546 * x_549) + x_552);
  ...
  u_xlat15 = (vec2<f32>(x_571.x, x_571.z) + vec2<f32>(x_573.x, x_573.y));
  let x_579 : vec2<f32> = u_xlat15;
  let x_580 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_579);
  ...
  u_xlat1.x = ((x_585 * x_588) + -(x_591));
  let x_595 : vec4<f32> = u_xlat3;
  let x_598 : vec4<f32> = u_xlat2;
  u_xlat8 = (-(vec3<f32>(x_595.x, x_595.y, x_595.z)) + vec3<f32>(x_598.x, x_598.y, x_598.z));
  let x_601 : vec4<f32> = u_xlat1;
  let x_603 : vec3<f32> = u_xlat8;
  let x_605 : vec4<f32> = u_xlat3;
  let x_607 : vec3<f32> = ((vec3<f32>(x_601.x, x_601.x, x_601.x) * x_603) + vec3<f32>(x_605.x, x_605.y, x_605.z));
  let x_608 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_607.x, x_607.y, x_607.z, x_608.w);
  let x_615 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_615);
  ...
  if (x_617) {
    ...
    u_xlat0.w = dot(vec3<f32>(x_631.x, x_631.y, x_631.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

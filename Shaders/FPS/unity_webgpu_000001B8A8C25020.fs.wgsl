...
fn main_1() {
  ...
  let x_1512 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_1512);
  let x_1514 : bool = u_xlatb0;
  if (x_1514) {
    let x_1517 : vec4<f32> = u_xlat2;
    let x_1518 : vec3<f32> = vec3<f32>(x_1517.x, x_1517.y, x_1517.z);
    let x_1519 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1518.x, x_1518.y, x_1518.z, x_1519.w);
    let x_1521 : vec4<f32> = u_xlat0;
    let x_1525 : vec3<f32> = clamp(vec3<f32>(x_1521.x, x_1521.y, x_1521.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1526 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1525.x, x_1525.y, x_1525.z, x_1526.w);
    let x_1528 : vec4<f32> = u_xlat0;
    u_xlat2.w = dot(vec3<f32>(x_1528.x, x_1528.y, x_1528.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

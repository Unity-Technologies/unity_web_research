...
fn main_1() {
  ...
  let x_245 : vec3<f32> = vs_TEXCOORD6;
  ...
  let x_290 : vec3<f32> = (vec3<f32>(x_287.x, x_287.y, x_287.z) * x_289);
  let x_291 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_290.x, x_290.y, x_290.z, x_291.w);
  let x_294 : vec3<f32> = u_xlat0;
  let x_295 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_294.x, x_294.y, x_294.z, x_295.w);
  SV_Target0.w = 1.0f;
  let x_301 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_301;
  let x_304 : vec3<f32> = vs_TEXCOORD0;
  let x_307 : vec3<f32> = ((x_304 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_308 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_307.x, x_307.y, x_307.z, x_308.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}
...
@fragment
fn main(@location(3) vs_TEXCOORD2_param : vec2<f32>, @location(2) vs_COLOR0_param : vec4<f32>, @location(1) vs_TEXCOORD1_param : vec3<f32>, @location(0) vs_TEXCOORD0_param : vec3<f32>, @location(4) vs_TEXCOORD6_param : vec3<f32>, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

...
fn main_1() {
  ...
  u_xlat1 = ((vec3<f32>(x_1526.z, x_1526.x, x_1526.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_1534 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_1534);
  let x_1536 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_1536 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_1543 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_1543, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_1547 : vec3<f32> = u_xlat1;
  let x_1550 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (x_1547 * vec3<f32>(x_1550.z, x_1550.z, x_1550.z));
  let x_1554 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_1554);
  let x_1558 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1560 : vec2<f32> = (vec2<f32>(x_1558.x, x_1558.y) * vec2<f32>(0.5f, 0.5f));
  let x_1561 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1560.x, x_1560.y, x_1561.z, x_1561.w);
  let x_1563 : vec3<f32> = u_xlat11;
  let x_1566 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1569 : vec4<f32> = u_xlat2;
  let x_1571 : vec2<f32> = ((vec2<f32>(x_1563.y, x_1563.z) * vec2<f32>(x_1566.x, x_1566.y)) + vec2<f32>(x_1569.x, x_1569.y));
  let x_1572 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1572.x, x_1571.x, x_1571.y, x_1572.w);
  let x_1575 : f32 = u_xlat11.x;
  let x_1577 : f32 = x_26.x_Lut2D_Params.y;
  let x_1580 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_1575 * x_1577) + x_1580);
  let x_1588 : vec4<f32> = u_xlat2;
  let x_1590 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_1588.x, x_1588.z));
  let x_1591 : vec3<f32> = vec3<f32>(x_1590.x, x_1590.y, x_1590.z);
  let x_1592 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1591.x, x_1591.y, x_1591.z, x_1592.w);
  let x_1595 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat4.x = x_1595;
  u_xlat4.y = 0.0f;
  let x_1598 : vec4<f32> = u_xlat2;
  let x_1600 : vec4<f32> = u_xlat4;
  u_xlat21 = (vec2<f32>(x_1598.x, x_1598.z) + vec2<f32>(x_1600.x, x_1600.y));
  let x_1606 : vec2<f32> = u_xlat21;
  let x_1607 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_1606);
  let x_1608 : vec3<f32> = vec3<f32>(x_1607.x, x_1607.y, x_1607.z);
  let x_1609 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1608.x, x_1608.y, x_1608.z, x_1609.w);
  let x_1612 : f32 = u_xlat1.x;
  let x_1614 : f32 = x_26.x_Lut2D_Params.z;
  let x_1617 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_1612 * x_1614) + -(x_1617));
  let x_1621 : vec4<f32> = u_xlat3;
  let x_1624 : vec4<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_1621.x, x_1621.y, x_1621.z)) + vec3<f32>(x_1624.x, x_1624.y, x_1624.z));
  let x_1627 : vec3<f32> = u_xlat1;
  let x_1629 : vec3<f32> = u_xlat11;
  let x_1631 : vec4<f32> = u_xlat3;
  let x_1633 : vec3<f32> = ((vec3<f32>(x_1627.x, x_1627.x, x_1627.x) * x_1629) + vec3<f32>(x_1631.x, x_1631.y, x_1631.z));
  let x_1634 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1633.x, x_1633.y, x_1633.z, x_1634.w);
  let x_1638 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1638);
  let x_1640 : bool = u_xlatb1;
  if (x_1640) {
    let x_1643 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_1643.x, x_1643.y, x_1643.z);
    let x_1645 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_1645, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1649 : vec3<f32> = u_xlat1;
    u_xlat0.w = dot(x_1649, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

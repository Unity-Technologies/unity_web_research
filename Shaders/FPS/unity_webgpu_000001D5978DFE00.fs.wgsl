...
fn main_1() {
  ...
  u_xlat0 = ((vec3<f32>(x_1205.x, x_1205.y, x_1205.z) * x_1207) + x_1209);
  let x_1216 : vec4<f32> = vs_TEXCOORD0;
  let x_1218 : vec4<f32> = textureSample(x_EmissionMap, sampler_EmissionMap, vec2<f32>(x_1216.x, x_1216.y));
  u_xlat1 = vec3<f32>(x_1218.x, x_1218.y, x_1218.z);
  let x_1222 : vec3<f32> = u_xlat1;
  let x_1225 : vec4<f32> = x_33.x_EmissionColor;
  let x_1228 : vec3<f32> = u_xlat0;
  let x_1229 : vec3<f32> = ((x_1222 * vec3<f32>(x_1225.x, x_1225.y, x_1225.z)) + x_1228);
  let x_1230 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1229.x, x_1229.y, x_1229.z, x_1230.w);
  SV_Target0.w = 1.0f;
  return;
}

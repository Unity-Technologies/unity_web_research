...
writeonly layout(set = 0, binding = 2, r32f) highp uniform image2D LinearZ_origX0X;
writeonly layout(set = 0, binding = 3, r32f) highp uniform image2D DS2x_origX1X;
writeonly layout(set = 0, binding = 4, r32f) highp uniform image2DArray DS2xAtlas_origX2X;
writeonly layout(set = 0, binding = 5, r32f) highp uniform image2D DS4x_origX3X;
writeonly layout(set = 0, binding = 6, r32f) highp uniform image2DArray DS4xAtlas_origX4X;
float u_xlat0;
int u_xlati0;
uvec4 u_xlatu0;
float u_xlat1;
...
void main()
{
    ...
    u_xlati4 = int(int_bitfieldInsert(int(gl_GlobalInvocationID.x), int(gl_GlobalInvocationID.y), 2 & 0x1F, 30));
    ...
    if(u_xlati4 == 0) {
        ...
        u_xlati12 = int(int_bitfieldInsert(int(u_xlatu4.x), int(u_xlatu4.y), 2 & 0x1F, 30));

...
fn main_1() {
  ...
  let x_195 : f32 = x_17.unity_LightShadowBias.x;
  let x_198 : f32 = u_xlat0.w;
  u_xlat1.x = (x_195 / x_198);
  let x_202 : f32 = u_xlat1.x;
  u_xlat1.x = min(x_202, 0.0f);
  let x_206 : f32 = u_xlat1.x;
  u_xlat1.x = max(x_206, -1.0f);
  let x_212 : f32 = u_xlat0.z;
  let x_214 : f32 = u_xlat1.x;
  u_xlat6 = (x_212 + x_214);
  let x_217 : f32 = u_xlat0.w;
  let x_218 : f32 = u_xlat6;
  u_xlat1.x = min(x_217, x_218);
  let x_225 : vec4<f32> = u_xlat0;
  let x_226 : vec3<f32> = vec3<f32>(x_225.x, x_225.y, x_225.w);
  let x_229 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_226.x, x_226.y, x_229.z, x_226.z);
  let x_231 : f32 = u_xlat6;
  let x_234 : f32 = u_xlat1.x;
  u_xlat0.x = (-(x_231) + x_234);
  let x_238 : f32 = x_17.unity_LightShadowBias.y;
  let x_240 : f32 = u_xlat0.x;
  let x_242 : f32 = u_xlat6;
  gl_Position.z = ((x_238 * x_240) + x_242);
  let x_246 : vec4<f32> = in_POSITION0;
  let x_249 : vec4<f32> = x_17.unity_ObjectToWorld[1i];
  ...
  let x_255 : vec4<f32> = x_17.unity_ObjectToWorld[0i];
  ...
  let x_266 : vec4<f32> = x_17.unity_ObjectToWorld[2i];
  ...
  let x_279 : vec4<f32> = x_17.unity_ObjectToWorld[3i];
  let x_281 : vec4<f32> = in_POSITION0;
  let x_284 : vec4<f32> = u_xlat0;
  vs_TEXCOORD1 = ((vec3<f32>(x_279.x, x_279.y, x_279.z) * vec3<f32>(x_281.w, x_281.w, x_281.w)) + vec3<f32>(x_284.x, x_284.y, x_284.z));
  return;
}

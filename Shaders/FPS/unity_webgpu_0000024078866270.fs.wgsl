...
fn main_1() {
  ...
  u_xlatb2 = vec3<bool>(x_492.x, x_492.y, x_492.z);
  let x_496 : vec4<f32> = u_xlat0;
  hlslcc_movcTemp = x_496;
  let x_498 : bool = u_xlatb2.x;
  if (x_498) {
  ...
  let x_708 : f32 = x_700;
  u_xlat0.z = x_708;
  let x_710 : vec2<f32> = vs_TEXCOORD0;
  let x_713 : vec4<f32> = x_49.x_Dithering_Coords;
  let x_717 : vec4<f32> = x_49.x_Dithering_Coords;
  let x_719 : vec2<f32> = ((x_710 * vec2<f32>(x_713.x, x_713.y)) + vec2<f32>(x_717.z, x_717.w));
  let x_720 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_719.x, x_719.y, x_720.z, x_720.w);
  let x_728 : vec4<f32> = u_xlat1;
  let x_730 : vec4<f32> = textureSample(x_DitheringTex, sampler_DitheringTex, vec2<f32>(x_728.x, x_728.y));
  u_xlat15 = x_730.w;
  let x_732 : f32 = u_xlat15;
  u_xlat15 = ((x_732 * 2.0f) + -1.0f);
  let x_736 : f32 = u_xlat15;
  u_xlat1.x = ((x_736 * 340282346638528859811704183484516925440.0f) + 0.5f);
  let x_742 : f32 = u_xlat1.x;
  u_xlat1.x = clamp(x_742, 0.0f, 1.0f);
  let x_746 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_746 * 2.0f) + -1.0f);
  ...
  let x_765 : vec3<f32> = (vec3<f32>(x_763.x, x_763.y, x_763.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_781 : vec3<f32> = (vec3<f32>(x_779.x, x_779.y, x_779.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_792 : vec3<f32> = ((vec3<f32>(x_789.x, x_789.y, x_789.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_793 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_792.x, x_792.y, x_792.z, x_793.w);
  let x_795 : vec4<f32> = u_xlat0;
  let x_797 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_795.x, x_795.y, x_795.z, x_795.x));
  u_xlatb0 = vec3<bool>(x_797.x, x_797.y, x_797.z);
  let x_800 : bool = u_xlatb0.x;
  if (x_800) {
  ...
  let x_842 : vec3<f32> = ((vec3<f32>(x_835, x_835, x_835) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_840.x, x_840.y, x_840.z));
  let x_843 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_842.x, x_842.y, x_842.z, x_843.w);
  let x_845 : vec4<f32> = u_xlat0;
  let x_847 : vec3<f32> = (vec3<f32>(x_845.x, x_845.y, x_845.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_848 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_847.x, x_847.y, x_847.z, x_848.w);
  let x_850 : vec4<f32> = u_xlat0;
  let x_852 : vec3<f32> = (vec3<f32>(x_850.x, x_850.y, x_850.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_853 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_852.x, x_852.y, x_852.z, x_853.w);
  let x_855 : vec4<f32> = u_xlat2;
  let x_857 : vec3<f32> = (vec3<f32>(x_855.x, x_855.y, x_855.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_858 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_857.x, x_857.y, x_857.z, x_858.w);
  let x_860 : vec4<f32> = u_xlat2;
  let x_863 : vec3<f32> = max(abs(vec3<f32>(x_860.x, x_860.y, x_860.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  let x_864 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_863.x, x_863.y, x_863.z, x_864.w);
  let x_866 : vec4<f32> = u_xlat2;
  let x_868 : vec3<f32> = log2(vec3<f32>(x_866.x, x_866.y, x_866.z));
  let x_869 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_868.x, x_868.y, x_868.z, x_869.w);
  let x_871 : vec4<f32> = u_xlat2;
  let x_873 : vec3<f32> = (vec3<f32>(x_871.x, x_871.y, x_871.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_874 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_873.x, x_873.y, x_873.z, x_874.w);
  let x_876 : vec4<f32> = u_xlat2;
  let x_878 : vec3<f32> = exp2(vec3<f32>(x_876.x, x_876.y, x_876.z));
  let x_879 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_878.x, x_878.y, x_878.z, x_879.w);
  let x_881 : vec4<f32> = u_xlat0;
  let x_883 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_881.x, x_881.y, x_881.z, x_881.x));
  u_xlatb0 = vec3<bool>(x_883.x, x_883.y, x_883.z);

...
fn main_1() {
  ...
  let x_556 : vec3<f32> = ((x_551 * vec3<f32>(x_552.x, x_552.z, x_552.w)) + x_555);
  let x_557 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_556.x, x_556.y, x_556.z, x_557.w);
  let x_560 : vec4<f32> = u_xlat0;
  let x_563 : vec3<f32> = exp2(-(vec3<f32>(x_560.x, x_560.y, x_560.z)));
  let x_564 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_563.x, x_563.y, x_563.z, x_564.w);
  let x_567 : f32 = u_xlat3;
  SV_Target0.w = x_567;
  let x_569 : vec3<f32> = u_xlat1;
  let x_570 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_569.x, x_569.y, x_569.z, x_570.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}

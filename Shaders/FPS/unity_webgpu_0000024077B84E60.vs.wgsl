...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_288.y, x_288.w, x_288.z, x_288.x) * x_290);
  let x_293 : vec4<f32> = x_17.unity_SHBr;
  let x_294 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_293, x_294);
  let x_298 : vec4<f32> = x_17.unity_SHBg;
  let x_299 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_298, x_299);
  let x_303 : vec4<f32> = x_17.unity_SHBb;
  let x_304 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_303, x_304);
  let x_310 : vec4<f32> = x_17.unity_SHC;
  let x_312 : vec4<f32> = u_xlat0;
  let x_315 : vec4<f32> = u_xlat2;
  vs_TEXCOORD6 = ((vec3<f32>(x_310.x, x_310.y, x_310.z) * vec3<f32>(x_312.x, x_312.x, x_312.x)) + vec3<f32>(x_315.x, x_315.y, x_315.z));
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(4) in_TANGENT0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_COLOR0, vs_TEXCOORD3, vs_TEXCOORD6);
}

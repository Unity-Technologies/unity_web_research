...
fn main_1() {
  ...
  let x_824 : f32 = x_31.x_PostExposure;
  let x_826 : f32 = x_31.x_PostExposure;
  let x_828 : f32 = x_31.x_PostExposure;
  let x_830 : f32 = x_31.x_PostExposure;
  let x_831 : vec4<f32> = vec4<f32>(x_824, x_826, x_828, x_830);
  u_xlat0 = (vec4<f32>(x_820.w, x_820.x, x_820.y, x_820.z) * vec4<f32>(x_831.x, x_831.y, x_831.z, x_831.w));
  let x_838 : vec4<f32> = u_xlat0;
  let x_845 : vec3<f32> = ((vec3<f32>(x_838.y, x_838.z, x_838.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_846 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_845.x, x_845.y, x_845.z, x_846.w);
  let x_848 : vec4<f32> = u_xlat1;
  let x_850 : vec3<f32> = log2(vec3<f32>(x_848.x, x_848.y, x_848.z));
  let x_851 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_850.x, x_850.y, x_850.z, x_851.w);
  let x_853 : vec4<f32> = u_xlat1;
  let x_860 : vec3<f32> = ((vec3<f32>(x_853.x, x_853.y, x_853.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  ...
  let x_875 : vec2<f32> = x_31.x_Lut3D_Params;
  let x_877 : vec3<f32> = (vec3<f32>(x_870.x, x_870.y, x_870.z) * vec3<f32>(x_875.y, x_875.y, x_875.y));
  let x_878 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_877.x, x_877.y, x_877.z, x_878.w);
  let x_881 : f32 = x_31.x_Lut3D_Params.x;
  u_xlat19 = (x_881 * 0.5f);
  let x_883 : vec4<f32> = u_xlat1;
  let x_886 : vec2<f32> = x_31.x_Lut3D_Params;
  let x_889 : f32 = u_xlat19;
  let x_891 : vec3<f32> = ((vec3<f32>(x_883.x, x_883.y, x_883.z) * vec3<f32>(x_886.x, x_886.x, x_886.x)) + vec3<f32>(x_889, x_889, x_889));
  let x_892 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_891.x, x_891.y, x_891.z, x_892.w);
  let x_902 : vec4<f32> = u_xlat1;
  let x_904 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_902.x, x_902.y, x_902.z));
  let x_905 : vec3<f32> = vec3<f32>(x_904.x, x_904.y, x_904.z);
  let x_906 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_906.x, x_905.x, x_905.y, x_905.z);
  let x_911 : f32 = x_31.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_911);
  let x_913 : bool = u_xlatb1;
  if (x_913) {
    let x_916 : vec4<f32> = u_xlat0;
    let x_917 : vec3<f32> = vec3<f32>(x_916.y, x_916.z, x_916.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_927.x, x_927.y, x_927.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_933 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_933.y, x_933.z, x_933.w, x_933.x);
  return;
}

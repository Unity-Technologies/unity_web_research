...
o . texcoord1 = ( UnpackUV ( i . texcoord1 ) . xy * _FaceTex_ST . xy + _FaceTex_ST . zw ) ;
float2 pixelSize = vPosition . w ;
pixelSize /= abs ( float2 ( _ScreenParams . x * glstate_matrix_projection [ 0 ] [ 0 ] , _ScreenParams . y * glstate_matrix_projection [ 1 ] [ 1 ] ) ) ;
#line 113
float4 clampedRect = clamp ( _ClipRect , - 2e10 , 2e10 ) ;
o . mask = float4 ( vert . xy * 2 - clampedRect . xy - clampedRect . zw , 0.25 / ( 0.25 * float2 ( _MaskSoftnessX , _MaskSoftnessY ) + pixelSize . xy ) ) ;
...
c = float4 ( tex2D ( _FaceTex , i . texcoord1 ) . rgb * i . color . rgb , i . color . a * c . a ) ;
#line 136
return c ;
}

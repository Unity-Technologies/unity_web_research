...
#line 5 "F:/Unity/test_webgpu_fps/Library/PackageCache/com.unity.probuilder@5.0.4/Content/Shader/ProBuilderCG.cginc"
...
{
v . z -= ORTHO_CAM_OFFSET * ( 1 - glstate_matrix_projection [ 3 ] [ 3 ] ) ;
v . xy /= _ScreenParams . xy ;
v . xy = ( v . xy - .5 ) / .5 ;
v . xy *= v . w ;
return v ;
...
return mul ( glstate_matrix_projection , ret ) ;
}

inline float4 UnityObjectToClipPosWithOffsetMetal ( float3 pos )
{
float4 ret = float4 ( UnityObjectToViewPos ( pos ) , 1 ) ;
ret *= lerp ( .99 , .95 , ( 1 - glstate_matrix_projection [ 3 ] [ 3 ] ) ) ;
return mul ( glstate_matrix_projection , ret ) ;
}
...
clip . z -= .0001 * ( 1 - glstate_matrix_projection [ 3 ] [ 3 ] ) ;
...
o . pos . xyz *= lerp ( .99 , .95 , ( 1 - glstate_matrix_projection [ 3 ] [ 3 ] ) ) ;
o . pos = mul ( glstate_matrix_projection , o . pos ) ;

...
fn main_1() {
  ...
  let x_821 : vec3<f32> = (vec3<f32>(x_817.x, x_817.y, x_817.z) * vec3<f32>(8000.0f, 8000.0f, 8000.0f));
  let x_822 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_821.x, x_821.y, x_821.z, x_822.w);
  let x_824 : vec4<f32> = u_xlat0;
  let x_828 : vec3<f32> = clamp(vec3<f32>(x_824.x, x_824.y, x_824.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_829 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_828.x, x_828.y, x_828.z, x_829.w);
  let x_831 : vec4<f32> = u_xlat0;
  let x_834 : vec4<f32> = x_18.x_LightColor0;
  let x_836 : vec3<f32> = (vec3<f32>(x_831.x, x_831.y, x_831.z) * vec3<f32>(x_834.x, x_834.y, x_834.z));
  let x_837 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_836.x, x_836.y, x_836.z, x_837.w);
  let x_839 : vec4<f32> = u_xlat0;
  let x_843 : vec3<f32> = (vec3<f32>(x_839.x, x_839.y, x_839.z) * vec3<f32>(27.0f, 27.0f, 27.0f));
  ...
  vs_TEXCOORD3 = (vec3<f32>(x_847.x, x_847.y, x_847.z) / vec3<f32>(x_849, x_849, x_849));

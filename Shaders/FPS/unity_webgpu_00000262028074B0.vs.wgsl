...
@group(0) @binding(14) var x_Gradient : texture_2d<f32>;

@group(0) @binding(15) var sampler_Gradient : sampler;

var<private> in_TEXCOORD0 : vec4<f32>;

@group(0) @binding(13) var<uniform> x_43 : VGlobals;
...
fn main_1() {
  ...
  let x_435 : vec2<f32> = (vec2<f32>(x_429.y, x_429.y) * vec2<f32>(x_433.x, x_433.y));
  ...
  let x_446 : vec2<f32> = ((vec2<f32>(x_439.x, x_439.y) * vec2<f32>(x_441.x, x_441.x)) + vec2<f32>(x_444.x, x_444.y));
  ...
  let x_457 : vec2<f32> = ((vec2<f32>(x_450.x, x_450.y) * vec2<f32>(x_452.z, x_452.z)) + vec2<f32>(x_455.x, x_455.y));
  let x_458 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_457.x, x_457.y, x_458.z, x_458.w);
  let x_463 : vec4<f32> = x_43.unity_WorldToLight[3i];
  let x_465 : vec4<f32> = u_xlat0;
  let x_468 : vec4<f32> = u_xlat0;
  vs_TEXCOORD5 = ((vec2<f32>(x_463.x, x_463.y) * vec2<f32>(x_465.w, x_465.w)) + vec2<f32>(x_468.x, x_468.y));

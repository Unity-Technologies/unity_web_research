...
fn main_1() {
  ...
  let x_315 : vec3<f32> = (vec3<f32>(x_312.x, x_312.y, x_312.z) * x_314);
  let x_316 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_315.x, x_315.y, x_315.z, x_316.w);
  let x_319 : vec3<f32> = u_xlat0;
  let x_320 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_319.x, x_319.y, x_319.z, x_320.w);
  SV_Target0.w = 1.0f;
  let x_326 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_326;
  let x_329 : vec3<f32> = vs_TEXCOORD0;
  let x_332 : vec3<f32> = ((x_329 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_333 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_332.x, x_332.y, x_332.z, x_333.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}

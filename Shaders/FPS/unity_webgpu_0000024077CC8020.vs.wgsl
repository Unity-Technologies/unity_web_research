...
@group(0) @binding(0) var<uniform> x_18 : VGlobals;
...
fn main_1() {
  ...
  vs_TEXCOORD0 = (x_757 * -50.0f);
  let x_762 : f32 = x_18.x_GroundColor.x;
  let x_764 : f32 = x_18.x_GroundColor.y;
  let x_766 : f32 = x_18.x_GroundColor.z;
  let x_768 : vec4<f32> = u_xlat0;
  let x_771 : vec3<f32> = u_xlat4;
  let x_772 : vec3<f32> = ((vec3<f32>(x_762, x_764, x_766) * vec3<f32>(x_768.x, x_768.y, x_768.z)) + x_771);
  let x_773 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_772.x, x_772.y, x_772.z, x_773.w);
  let x_777 : vec4<f32> = u_xlat0;
  let x_780 : f32 = x_18.x_Exposure;
  vs_TEXCOORD1 = (vec3<f32>(x_777.x, x_777.y, x_777.z) * vec3<f32>(x_780, x_780, x_780));
  let x_784 : vec4<f32> = x_18.x_WorldSpaceLightPos0;
  let x_786 : vec3<f32> = u_xlat3;
  u_xlat0.x = dot(vec3<f32>(x_784.x, x_784.y, x_784.z), -(x_786));
  let x_791 : f32 = u_xlat0.x;
  let x_793 : f32 = u_xlat0.x;
  u_xlat0.x = (x_791 * x_793);
  let x_797 : f32 = u_xlat0.x;
  u_xlat0.x = ((x_797 * 0.75f) + 0.75f);
  let x_802 : vec4<f32> = u_xlat0;
  let x_804 : vec3<f32> = u_xlat4;
  let x_805 : vec3<f32> = (vec3<f32>(x_802.x, x_802.x, x_802.x) * x_804);
  let x_806 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_805.x, x_805.y, x_805.z, x_806.w);
  let x_809 : vec4<f32> = u_xlat0;
  let x_812 : f32 = x_18.x_Exposure;
  vs_TEXCOORD2 = (vec3<f32>(x_809.x, x_809.y, x_809.z) * vec3<f32>(x_812, x_812, x_812));
  return;
}

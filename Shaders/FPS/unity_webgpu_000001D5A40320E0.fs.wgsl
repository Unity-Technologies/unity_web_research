...
fn main_1() {
  ...
  let x_882 : f32 = x_26.x_PostExposure;
  let x_883 : vec4<f32> = vec4<f32>(x_876, x_878, x_880, x_882);
  u_xlat0 = (vec4<f32>(x_872.w, x_872.x, x_872.y, x_872.z) * vec4<f32>(x_883.x, x_883.y, x_883.z, x_883.w));
  let x_890 : vec4<f32> = u_xlat0;
  let x_897 : vec3<f32> = ((vec3<f32>(x_890.y, x_890.z, x_890.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_898 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_897.x, x_897.y, x_897.z, x_898.w);
  let x_900 : vec4<f32> = u_xlat1;
  let x_902 : vec3<f32> = log2(vec3<f32>(x_900.x, x_900.y, x_900.z));
  let x_903 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_902.x, x_902.y, x_902.z, x_903.w);
  let x_905 : vec4<f32> = u_xlat1;
  let x_912 : vec3<f32> = ((vec3<f32>(x_905.x, x_905.y, x_905.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_913 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_912.x, x_912.y, x_912.z, x_913.w);
  let x_915 : vec4<f32> = u_xlat1;
  let x_919 : vec3<f32> = clamp(vec3<f32>(x_915.x, x_915.y, x_915.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_920 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_919.x, x_919.y, x_919.z, x_920.w);
  let x_922 : vec4<f32> = u_xlat1;
  let x_927 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_929 : vec3<f32> = (vec3<f32>(x_922.x, x_922.y, x_922.z) * vec3<f32>(x_927.y, x_927.y, x_927.y));
  let x_930 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_929.x, x_929.y, x_929.z, x_930.w);
  let x_933 : f32 = x_26.x_Lut3D_Params.x;
  u_xlat31 = (x_933 * 0.5f);
  let x_935 : vec4<f32> = u_xlat1;
  let x_938 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_941 : f32 = u_xlat31;
  let x_943 : vec3<f32> = ((vec3<f32>(x_935.x, x_935.y, x_935.z) * vec3<f32>(x_938.x, x_938.x, x_938.x)) + vec3<f32>(x_941, x_941, x_941));
  let x_944 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_943.x, x_943.y, x_943.z, x_944.w);
  let x_954 : vec4<f32> = u_xlat1;
  let x_956 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_954.x, x_954.y, x_954.z));
  let x_957 : vec3<f32> = vec3<f32>(x_956.x, x_956.y, x_956.z);
  let x_958 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_958.x, x_957.x, x_957.y, x_957.z);
  let x_962 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_962);
  let x_964 : bool = u_xlatb1;
  if (x_964) {
    let x_967 : vec4<f32> = u_xlat0;
    let x_968 : vec3<f32> = vec3<f32>(x_967.y, x_967.z, x_967.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_978.x, x_978.y, x_978.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_988 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_988.y, x_988.z, x_988.w, x_988.x);
  return;
}

...
fn main_1() {
  ...
  let x_796 : f32 = x_26.x_PostExposure;
  let x_798 : f32 = x_26.x_PostExposure;
  let x_800 : f32 = x_26.x_PostExposure;
  let x_802 : f32 = x_26.x_PostExposure;
  let x_803 : vec4<f32> = vec4<f32>(x_796, x_798, x_800, x_802);
  u_xlat0 = (vec4<f32>(x_792.w, x_792.x, x_792.y, x_792.z) * vec4<f32>(x_803.x, x_803.y, x_803.z, x_803.w));
  let x_810 : vec4<f32> = u_xlat0;
  u_xlat1 = ((vec3<f32>(x_810.y, x_810.z, x_810.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_818 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_818);
  let x_820 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_820 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_827 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_827, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_831 : vec3<f32> = u_xlat1;
  let x_834 : vec2<f32> = x_26.x_Lut3D_Params;
  u_xlat1 = (x_831 * vec3<f32>(x_834.y, x_834.y, x_834.y));
  let x_838 : f32 = x_26.x_Lut3D_Params.x;
  u_xlat31 = (x_838 * 0.5f);
  let x_840 : vec3<f32> = u_xlat1;
  let x_842 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_845 : f32 = u_xlat31;
  u_xlat1 = ((x_840 * vec3<f32>(x_842.x, x_842.x, x_842.x)) + vec3<f32>(x_845, x_845, x_845));
  let x_856 : vec3<f32> = u_xlat1;
  let x_857 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, x_856);
  let x_858 : vec3<f32> = vec3<f32>(x_857.x, x_857.y, x_857.z);
  let x_859 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_859.x, x_858.x, x_858.y, x_858.z);
  let x_863 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_863);
  let x_865 : bool = u_xlatb1;
  if (x_865) {
    let x_868 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_868.y, x_868.z, x_868.w);
    let x_870 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_870, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_874 : vec3<f32> = u_xlat1;
    u_xlat0.x = dot(x_874, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_883 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_883.y, x_883.z, x_883.w, x_883.x);
  return;
}

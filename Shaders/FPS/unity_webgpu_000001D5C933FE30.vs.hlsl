...
static const int _ALPHABLEND_ON = 0 ;
...
#line 96
TextureCube_float _ShadowMapTexture ; SamplerComparisonState sampler_ShadowMapTexture ;
#line 106
...
float4 shadowVals ;
#line 128
shadowVals . x = _ShadowMapTexture . SampleCmp ( sampler_ShadowMapTexture , ( float4 ( vec + float3 ( z , z , z ) , mydist ) ) . xyz , ( float4 ( vec + float3 ( z , z , z ) , mydist ) ) . w ) ;
shadowVals . y = _ShadowMapTexture . SampleCmp ( sampler_ShadowMapTexture , ( float4 ( vec + float3 ( - z , - z , z ) , mydist ) ) . xyz , ( float4 ( vec + float3 ( - z , - z , z ) , mydist ) ) . w ) ;
shadowVals . z = _ShadowMapTexture . SampleCmp ( sampler_ShadowMapTexture , ( float4 ( vec + float3 ( - z , z , - z ) , mydist ) ) . xyz , ( float4 ( vec + float3 ( - z , z , - z ) , mydist ) ) . w ) ;
shadowVals . w = _ShadowMapTexture . SampleCmp ( sampler_ShadowMapTexture , ( float4 ( vec + float3 ( z , - z , - z ) , mydist ) ) . xyz , ( float4 ( vec + float3 ( z , - z , - z ) , mydist ) ) . w ) ;
...
#line 20 "F:/src/unity_webgpu/Shaders/Includes/UnityStandardInput.cginc"
float4 _Color ;
float _Cutoff ;
...
{
float4 texcoord ;
texcoord . xy = ( v . uv0 . xy * _MainTex_ST . xy + _MainTex_ST . zw ) ;
texcoord . zw = ( ( ( _UVSec == 0 ) ? v . uv0 : v . uv1 ) . xy * _DetailAlbedoMap_ST . xy + _DetailAlbedoMap_ST . zw ) ;
return texcoord ;
}

float DetailMask ( float2 uv )
{
return tex2D ( _DetailMask , uv ) . a ;
}

float3 Albedo ( float4 texcoords )
{
float3 albedo = _Color . rgb * tex2D ( _MainTex , texcoords . xy ) . rgb ;
#line 105
return albedo ;
}
...
{
#line 124
float occ = tex2D ( _OcclusionMap , uv ) . g ;
return LerpOneTo ( occ , _OcclusionStrength ) ;

}

float4 SpecularGloss ( float2 uv )
{
float4 sg ;
#line 141
...
#line 201
float3 NormalInTangentSpace ( float4 texcoords )
{
float3 normalTangent = UnpackScaleNormal ( tex2D ( _BumpMap , texcoords . xy ) , _BumpScale ) ;
#line 221
return normalTangent ;
}
#line 225
float4 Parallax ( float4 texcoords , float3 viewDir )
{
#line 229
return texcoords ;
#line 236
...
#line 17 "F:/src/unity_webgpu/Shaders/Includes/UnityStandardCore.cginc"
float3 NormalizePerVertexNormal ( float3 n )
{
#line 22
return n ;
...
{
float4 specGloss = SpecularGloss ( i_tex . xy ) ;
float3 specColor = specGloss . rgb ;
float smoothness = specGloss . a ;

float oneMinusReflectivity ;
float3 diffColor = EnergyConservationBetweenDiffuseAndSpecular ( Albedo ( i_tex ) , specColor , oneMinusReflectivity ) ;
...
{
float2 metallicGloss = MetallicRough ( i_tex . xy ) ;
float metallic = metallicGloss . x ;
float smoothness = metallicGloss . y ;

float oneMinusReflectivity ;
float3 specColor ;
float3 diffColor = DiffuseAndSpecularFromMetallic ( Albedo ( i_tex ) , metallic , specColor , oneMinusReflectivity ) ;
...
#line 245
inline FragmentCommonData FragmentSetup ( inout float4 i_tex , float3 i_eyeVec , float3 i_viewDirForParallax , float4 tangentToWorld [ 3 ] , float3 i_posWorld )
{
i_tex = Parallax ( i_tex , i_viewDirForParallax ) ;

float alpha = Alpha ( i_tex . xy ) ;
#line 254
...
float3 _LightCoord : TEXCOORD6 ; float4 _ShadowCoord : TEXCOORD7 ;
#line 371
} ;
...
FragmentCommonData s = FragmentSetup ( i . tex , i . eyeVec . xyz , float3 ( 0 , 0 , 0 ) , i . tangentToWorldAndPackedData , float3 ( i . tangentToWorldAndPackedData [ 0 ] . w , i . tangentToWorldAndPackedData [ 1 ] . w , i . tangentToWorldAndPackedData [ 2 ] . w ) ) ;

;
;

UnityLight mainLight = MainLight ( ) ;
float3 lightCoord = mul ( unity_WorldToLight , float4 ( s . posWorld , 1 ) ) . xyz ; float shadow = UnityComputeForwardShadows ( 0 , s . posWorld , 0 ) ; float atten = tex2D ( _LightTextureB0 , dot ( lightCoord , lightCoord ) . rr ) . r * texCUBE ( _LightTexture0 , lightCoord ) . w * shadow ; ;

float occlusion = Occlusion ( i . tex . xy ) ;
UnityGI gi = FragmentGI ( s , occlusion , i . ambientOrLightmapUV , atten , mainLight ) ;

float4 c = BRDF1_Unity_PBS ( s . diffColor , s . specColor , s . oneMinusReflectivity , s . smoothness , s . normalWorld , - s . eyeVec , gi . light , gi . indirect ) ;
...
float4 fragForwardBase ( VertexOutputForwardBase i ) : COLOR
{
...
FragmentCommonData s = FragmentSetup ( i . tex , i . eyeVec . xyz , float3 ( 0 , 0 , 0 ) , i . tangentToWorldAndLightDir , i . posWorld ) ;

float3 lightCoord = mul ( unity_WorldToLight , float4 ( s . posWorld , 1 ) ) . xyz ; float shadow = UnityComputeForwardShadows ( 0 , s . posWorld , 0 ) ; float atten = tex2D ( _LightTextureB0 , dot ( lightCoord , lightCoord ) . rr ) . r * texCUBE ( _LightTexture0 , lightCoord ) . w * shadow ;
UnityLight light = AdditiveLight ( float3 ( i . tangentToWorldAndLightDir [ 0 ] . w , i . tangentToWorldAndLightDir [ 1 ] . w , i . tangentToWorldAndLightDir [ 2 ] . w ) , atten ) ;
UnityIndirect noIndirect = ZeroIndirect ( ) ;

float4 c = BRDF1_Unity_PBS ( s . diffColor , s . specColor , s . oneMinusReflectivity , s . smoothness , s . normalWorld , - s . eyeVec , light , noIndirect ) ;
...
float4 fragForwardAdd ( VertexOutputForwardAdd i ) : COLOR
{
...
float3 emissiveColor = BRDF1_Unity_PBS ( s . diffColor , s . specColor , s . oneMinusReflectivity , s . smoothness , s . normalWorld , - s . eyeVec , gi . light , gi . indirect ) . rgb ;
#line 669
...
float3 posWorld ,
float occlusion , float4 i_ambientOrLightmapUV , float atten , float smoothness , float3 normalWorld , float3 eyeVec ,
UnityLight light ,
bool reflections )
{
...
#line 18 "F:/src/unity_webgpu/Shaders/Includes/UnityStandardCoreForward.cginc"
VertexOutputForwardBase vertBase ( VertexInput v ) { return vertForwardBase ( v ) ; }
VertexOutputForwardAdd vertAdd ( VertexInput v ) { return vertForwardAdd ( v ) ; }
float4 fragBase ( VertexOutputForwardBase i ) : COLOR { return fragForwardBaseInternal ( i ) ; }
float4 fragAdd ( VertexOutputForwardAdd i ) : COLOR { return fragForwardAddInternal ( i ) ; }

...
#line 52
float _Glossiness , _Metallic , _Cutoff , _Cutoff2 , _EdgeSizeBot , _EdgeSizeTop , _NoiseStrength , _BoundsY ;
float4 _Color , _EdgeColor1 , _EdgeColor2 ;
float4 _bounds ;
#line 57
...
#line 80
float4 MetallicSmooth = tex2D ( _MetallicSmooth , IN . uv_MainTex ) ;
...
float a = tex2D ( _Noise , noiseUV1 ) . x ;
float b = tex2D ( _Noise , noiseUV2 ) . x ;

float topNoise = a * b * 2 ;
float botNoise = tex2D ( _Noise , float2 ( pos . y * 6 , 0 ) ) . x ;

float cutoff = _Cutoff * yBound - ( yBound / 2 ) ;
#line 101
float Edge = step ( cutoff , pos . y ) ;
float EdgeBot = smoothstep ( cutoff - _EdgeSizeBot , cutoff , pos . y ) ;
float EdgeTop = smoothstep ( cutoff + _EdgeSizeTop , cutoff , pos . y ) ;

float3 glowBot = _EdgeColor1 * EdgeBot * ( 1 - Edge ) ;
float3 glowTop = lerp ( _EdgeColor2 , _EdgeColor1 , EdgeTop ) * Edge ;
...
float3 _LightCoord : TEXCOORD5 ; float4 _ShadowCoord : TEXCOORD6 ;
#line 130
} ;
float4 _MainTex_ST ;
#line 134
...
#line 204
surf ( surfIN , o ) ;
float3 lightCoord = mul ( unity_WorldToLight , float4 ( worldPos , 1 ) ) . xyz ; float shadow = UnityComputeForwardShadows ( 0 , worldPos , 0 ) ; float atten = tex2D ( _LightTextureB0 , dot ( lightCoord , lightCoord ) . rr ) . r * texCUBE ( _LightTexture0 , lightCoord ) . w * shadow ;
float4 c = 0 ;
float3 worldN ;

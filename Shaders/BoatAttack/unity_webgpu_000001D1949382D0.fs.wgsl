...
fn main_1() {
  ...
  u_xlat1 = log2(abs(x_332));
  let x_335 : vec3<f32> = u_xlat1;
  u_xlat1 = (x_335 * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_337 : vec3<f32> = u_xlat1;
  u_xlat1 = exp2(x_337);
  let x_339 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_339 * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_342 : vec3<f32> = u_xlat0;
  u_xlat2 = (x_342 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_344 : vec3<f32> = u_xlat0;
  let x_346 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_344.x, x_344.y, x_344.z, x_344.x));
  u_xlatb0 = vec3<bool>(x_346.x, x_346.y, x_346.z);
  ...
  if (x_373) {
    ...
    let x_381 : f32 = u_xlat1.z;
    x_374 = x_381;
  }
  let x_382 : f32 = x_374;
  u_xlat0.z = x_382;
  let x_384 : vec2<f32> = vs_TEXCOORD0;
  let x_387 : vec4<f32> = x_27.x_Dithering_Params;
  let x_391 : vec4<f32> = x_27.x_Dithering_Params;
  let x_393 : vec2<f32> = ((x_384 * vec2<f32>(x_387.x, x_387.y)) + vec2<f32>(x_391.z, x_391.w));
  let x_394 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_393.x, x_393.y, x_394.z);
  let x_404 : vec3<f32> = u_xlat1;
  let x_408 : f32 = x_27.x_GlobalMipBias.x;
  let x_409 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_404.x, x_404.y), x_408);
  u_xlat27 = x_409.w;
  let x_411 : f32 = u_xlat27;
  u_xlat27 = ((x_411 * 2.0f) + -1.0f);
  let x_415 : f32 = u_xlat27;
  u_xlat1.x = (-(abs(x_415)) + 1.0f);
  let x_421 : f32 = u_xlat27;
  u_xlatb27 = (x_421 >= 0.0f);
  let x_423 : bool = u_xlatb27;
  u_xlat27 = select(-1.0f, 1.0f, x_423);
  let x_426 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_426);
  let x_430 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_430) + 1.0f);
  let x_434 : f32 = u_xlat27;
  let x_436 : f32 = u_xlat1.x;
  u_xlat27 = (x_434 * x_436);
  let x_438 : f32 = u_xlat27;
  let x_443 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_438, x_438, x_438) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_443);
  let x_445 : vec3<f32> = u_xlat0;
  u_xlat1 = (x_445 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_449 : vec3<f32> = u_xlat1;
  u_xlat1 = (x_449 * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_453 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(abs(x_453));
  let x_456 : vec3<f32> = u_xlat1;
  u_xlat1 = (x_456 * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_460 : vec3<f32> = u_xlat1;
  u_xlat1 = exp2(x_460);
  let x_462 : vec3<f32> = u_xlat0;
  u_xlat2 = (x_462 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));

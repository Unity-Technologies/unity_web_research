...
fn main_1() {
  ...
  u_xlat9 = max(x_87, 1.17549435e-37f);
  ...
  let x_361 : vec4<f32> = x_17.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_358.y, x_358.y, x_358.y, x_358.y) * x_361);
  let x_364 : vec4<f32> = x_17.unity_MatrixVP[0i];
  let x_365 : vec4<f32> = u_xlat0;
  let x_368 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_364 * vec4<f32>(x_365.x, x_365.x, x_365.x, x_365.x)) + x_368);
  let x_371 : vec4<f32> = x_17.unity_MatrixVP[2i];
  let x_372 : vec4<f32> = u_xlat0;
  let x_375 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_371 * vec4<f32>(x_372.z, x_372.z, x_372.z, x_372.z)) + x_375);
  let x_381 : vec4<f32> = u_xlat0;
  let x_383 : vec4<f32> = x_17.unity_MatrixVP[3i];
  gl_Position = (x_381 + x_383);
  return;
}

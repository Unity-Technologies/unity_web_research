...
fn main_1() {
  ...
  let x_185 : vec4<f32> = phase0_Output0_5;
  vs_INTERP4 = vec2<f32>(x_185.x, x_185.y);
  let x_188 : vec4<f32> = phase0_Output0_5;
  vs_INTERP5 = vec2<f32>(x_188.z, x_188.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP0, gl_Position, vs_INTERP1, vs_INTERP2, vs_INTERP3);
}

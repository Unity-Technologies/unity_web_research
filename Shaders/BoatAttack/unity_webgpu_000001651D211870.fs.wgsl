...
fn main_1() {
  ...
  let x_815 : vec4<f32> = x_26.x_Dithering_Params;
  let x_819 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_832 : f32 = x_26.x_GlobalMipBias.x;
  let x_833 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_829.x, x_829.y), x_832);
  u_xlat30 = x_833.w;
  let x_835 : f32 = u_xlat30;
  u_xlat30 = ((x_835 * 2.0f) + -1.0f);
  let x_838 : f32 = u_xlat30;
  u_xlat1.x = (-(abs(x_838)) + 1.0f);
  let x_844 : f32 = u_xlat30;
  u_xlatb30 = (x_844 >= 0.0f);
  let x_846 : bool = u_xlatb30;
  u_xlat30 = select(-1.0f, 1.0f, x_846);
  ...
  let x_871 : vec3<f32> = (x_868 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_872 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_871.x, x_871.y, x_871.z, x_872.w);
  let x_874 : vec4<f32> = u_xlat1;
  let x_878 : vec3<f32> = (vec3<f32>(x_874.x, x_874.y, x_874.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_891 : vec3<f32> = (vec3<f32>(x_887.x, x_887.y, x_887.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_892 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_891.x, x_891.y, x_891.z, x_892.w);
  let x_894 : vec4<f32> = u_xlat1;
  let x_896 : vec3<f32> = exp2(vec3<f32>(x_894.x, x_894.y, x_894.z));
  let x_897 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_896.x, x_896.y, x_896.z, x_897.w);
  let x_899 : vec3<f32> = u_xlat0;
  let x_902 : vec3<f32> = (x_899 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_903 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_902.x, x_902.y, x_902.z, x_903.w);
  let x_907 : vec3<f32> = u_xlat0;
  let x_909 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_907.x, x_907.y, x_907.z, x_907.x));
  u_xlatb0 = vec3<bool>(x_909.x, x_909.y, x_909.z);
  let x_914 : bool = u_xlatb0.x;
  if (x_914) {

...
fn main_1() {
  ...
  let x_701 : vec3<f32> = sqrt(vec3<f32>(x_699.x, x_699.y, x_699.z));
  let x_702 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_701.x, x_701.y, x_701.z, x_702.w);
  let x_704 : vec2<f32> = vs_TEXCOORD0;
  let x_707 : vec4<f32> = x_25.x_Dithering_Params;
  let x_711 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_724 : f32 = x_25.x_GlobalMipBias.x;
  let x_725 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_721.x, x_721.y), x_724);
  u_xlat15 = x_725.w;
  let x_727 : f32 = u_xlat15;
  u_xlat15 = ((x_727 * 2.0f) + -1.0f);
  let x_731 : f32 = u_xlat15;
  u_xlatb1.x = (x_731 >= 0.0f);
  let x_735 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_735);
  let x_738 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_738)) + 1.0f);
  ...
  u_xlat15 = (x_747 * x_749);
  let x_751 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_751.x, x_751.y, x_751.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_761 : vec3<f32> = (vec3<f32>(x_759.x, x_759.y, x_759.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_762 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_761.x, x_761.y, x_761.z, x_762.w);
  let x_764 : vec4<f32> = u_xlat2;
  let x_766 : vec3<f32> = exp2(vec3<f32>(x_764.x, x_764.y, x_764.z));
  let x_767 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_766.x, x_766.y, x_766.z, x_767.w);
  let x_769 : vec4<f32> = u_xlat2;
  let x_772 : vec3<f32> = ((vec3<f32>(x_769.x, x_769.y, x_769.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_773 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_772.x, x_772.y, x_772.z, x_773.w);
  let x_776 : vec4<f32> = u_xlat0;
  let x_778 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_776.x, x_776.y, x_776.z, x_776.x));
  ...
  let x_823 : vec3<f32> = ((vec3<f32>(x_816, x_816, x_816) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_821.x, x_821.y, x_821.z));
  let x_824 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_823.x, x_823.y, x_823.z, x_824.w);
  let x_826 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_826.x, x_826.y, x_826.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_829 : vec4<f32> = u_xlat0;
  let x_831 : vec3<f32> = (vec3<f32>(x_829.x, x_829.y, x_829.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_832 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_831.x, x_831.y, x_831.z, x_832.w);
  let x_834 : vec4<f32> = u_xlat2;
  let x_836 : vec3<f32> = (vec3<f32>(x_834.x, x_834.y, x_834.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_847 : vec3<f32> = (vec3<f32>(x_845.x, x_845.y, x_845.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_900 : vec3<f32> = max(vec3<f32>(x_897.x, x_897.y, x_897.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_901 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_900.x, x_900.y, x_900.z, x_901.w);
  SV_Target0.w = 1.0f;
  return;
}

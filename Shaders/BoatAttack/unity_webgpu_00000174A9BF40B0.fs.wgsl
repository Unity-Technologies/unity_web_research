...
fn main_1() {
  ...
  let x_731 : vec2<f32> = vs_TEXCOORD0;
  let x_734 : vec4<f32> = x_26.x_Dithering_Params;
  let x_738 : vec4<f32> = x_26.x_Dithering_Params;
  let x_740 : vec2<f32> = ((x_731 * vec2<f32>(x_734.x, x_734.y)) + vec2<f32>(x_738.z, x_738.w));
  let x_741 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_740.x, x_740.y, x_741.z, x_741.w);
  let x_748 : vec4<f32> = u_xlat1;
  let x_751 : f32 = x_26.x_GlobalMipBias.x;
  let x_752 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_748.x, x_748.y), x_751);
  u_xlat15 = x_752.w;
  let x_754 : f32 = u_xlat15;
  u_xlat15 = ((x_754 * 2.0f) + -1.0f);
  let x_759 : f32 = u_xlat15;
  u_xlatb1.x = (x_759 >= 0.0f);
  let x_763 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_763);
  let x_766 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_766)) + 1.0f);
  ...
  u_xlat15 = (x_775 * x_777);
  let x_779 : vec4<f32> = u_xlat0;
  let x_781 : vec3<f32> = (vec3<f32>(x_779.x, x_779.y, x_779.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_782 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_781.x, x_781.y, x_781.z, x_782.w);
  let x_784 : vec4<f32> = u_xlat0;
  let x_787 : vec3<f32> = log2(abs(vec3<f32>(x_784.x, x_784.y, x_784.z)));
  let x_788 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_787.x, x_787.y, x_787.z, x_788.w);
  let x_790 : vec4<f32> = u_xlat2;
  let x_792 : vec3<f32> = (vec3<f32>(x_790.x, x_790.y, x_790.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_803 : vec3<f32> = ((vec3<f32>(x_800.x, x_800.y, x_800.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_804 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_803.x, x_803.y, x_803.z, x_804.w);
  let x_807 : vec4<f32> = u_xlat0;
  let x_809 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_807.x, x_807.y, x_807.z, x_807.x));
  u_xlatb0 = vec3<bool>(x_809.x, x_809.y, x_809.z);
  ...
  let x_821 : f32 = x_813;
  u_xlat0.x = x_821;
  let x_824 : bool = u_xlatb0.y;
  if (x_824) {
    let x_829 : f32 = u_xlat1.y;
    x_825 = x_829;
  } else {
    let x_832 : f32 = u_xlat2.y;
    x_825 = x_832;
  }
  let x_833 : f32 = x_825;
  u_xlat0.y = x_833;
  let x_836 : bool = u_xlatb0.z;
  if (x_836) {
    let x_841 : f32 = u_xlat1.z;
    x_837 = x_841;
  } else {
    let x_844 : f32 = u_xlat2.z;
    x_837 = x_844;
  }
  let x_845 : f32 = x_837;
  u_xlat0.z = x_845;
  ...
  let x_854 : vec3<f32> = ((vec3<f32>(x_847, x_847, x_847) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_852.x, x_852.y, x_852.z));
  let x_855 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_854.x, x_854.y, x_854.z, x_855.w);
  let x_857 : vec4<f32> = u_xlat0;
  let x_859 : vec3<f32> = (vec3<f32>(x_857.x, x_857.y, x_857.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_860 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_859.x, x_859.y, x_859.z, x_860.w);
  let x_862 : vec4<f32> = u_xlat0;
  let x_864 : vec3<f32> = (vec3<f32>(x_862.x, x_862.y, x_862.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_865 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_864.x, x_864.y, x_864.z, x_865.w);
  let x_867 : vec4<f32> = u_xlat2;
  let x_869 : vec3<f32> = (vec3<f32>(x_867.x, x_867.y, x_867.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_870 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_869.x, x_869.y, x_869.z, x_870.w);
  let x_872 : vec4<f32> = u_xlat2;
  let x_875 : vec3<f32> = log2(abs(vec3<f32>(x_872.x, x_872.y, x_872.z)));
  let x_876 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_875.x, x_875.y, x_875.z, x_876.w);
  let x_878 : vec4<f32> = u_xlat2;
  let x_880 : vec3<f32> = (vec3<f32>(x_878.x, x_878.y, x_878.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_881 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_880.x, x_880.y, x_880.z, x_881.w);
  let x_883 : vec4<f32> = u_xlat2;
  let x_885 : vec3<f32> = exp2(vec3<f32>(x_883.x, x_883.y, x_883.z));
  ...
  let x_902 : f32 = x_894;
  u_xlat0.x = x_902;
  let x_905 : bool = u_xlatb0.y;
  if (x_905) {
    let x_910 : f32 = u_xlat1.y;
    x_906 = x_910;
  } else {
    let x_913 : f32 = u_xlat2.y;
    x_906 = x_913;
  }
  let x_914 : f32 = x_906;
  u_xlat0.y = x_914;
  let x_917 : bool = u_xlatb0.z;
  if (x_917) {
    let x_922 : f32 = u_xlat1.z;
    x_918 = x_922;
  } else {
  ...
  let x_933 : vec3<f32> = max(vec3<f32>(x_930.x, x_930.y, x_930.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
@group(1) @binding(6) var<uniform> x_354 : VGlobals;
...
fn main_1() {
  ...
  let x_356 : vec4<f32> = x_354.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_350.y, x_350.y, x_350.y, x_350.y) * x_356);
  let x_359 : vec4<f32> = x_354.unity_MatrixVP[0i];
  let x_360 : vec4<f32> = u_xlat0;
  let x_363 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_359 * vec4<f32>(x_360.x, x_360.x, x_360.x, x_360.x)) + x_363);
  let x_366 : vec4<f32> = x_354.unity_MatrixVP[2i];
  let x_367 : vec4<f32> = u_xlat0;
  let x_370 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_366 * vec4<f32>(x_367.z, x_367.z, x_367.z, x_367.z)) + x_370);
  let x_376 : vec4<f32> = u_xlat0;
  let x_378 : vec4<f32> = x_354.unity_MatrixVP[3i];
  gl_Position = (x_376 + x_378);
  return;
}

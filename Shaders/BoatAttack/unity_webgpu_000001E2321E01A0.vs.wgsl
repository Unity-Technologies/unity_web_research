...
@group(1) @binding(4) var<uniform> x_382 : MainLightShadows;
...
fn main_1() {
  ...
  let x_384 : vec4<f32> = x_382.x_MainLightWorldToShadow[0i][1i];
  let x_386 : vec3<f32> = (vec3<f32>(x_376.y, x_376.y, x_376.y) * vec3<f32>(x_384.x, x_384.y, x_384.z));
  let x_387 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_386.x, x_386.y, x_386.z, x_387.w);
  let x_390 : vec4<f32> = x_382.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_401 : vec4<f32> = x_382.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_415 : vec4<f32> = x_382.x_MainLightWorldToShadow[0i][3i];
  let x_417 : vec3<f32> = (vec3<f32>(x_412.x, x_412.y, x_412.z) + vec3<f32>(x_415.x, x_415.y, x_415.z));
  let x_418 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_417.x, x_417.y, x_417.z, x_418.w);
  vs_TEXCOORD6.w = 0.0f;
  let x_422 : vec4<f32> = x_40.x_WavingTint;
  let x_426 : vec3<f32> = (vec3<f32>(x_422.x, x_422.y, x_422.z) + vec3<f32>(-0.5f, -0.5f, -0.5f));
  let x_427 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_426.x, x_426.y, x_426.z, x_427.w);
  let x_429 : vec4<f32> = u_xlat0;
  let x_431 : vec4<f32> = u_xlat1;
  let x_436 : vec3<f32> = ((vec3<f32>(x_429.x, x_429.x, x_429.x) * vec3<f32>(x_431.x, x_431.y, x_431.z)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_437 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_436.x, x_436.y, x_436.z, x_437.w);
  let x_439 : vec4<f32> = u_xlat1;
  let x_442 : vec4<f32> = in_COLOR0;
  ...
  let x_452 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_451.x, x_451.y, x_451.z, x_452.w);
  let x_454 : vec3<f32> = u_xlat5;
  let x_457 : vec4<f32> = x_351.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_454.y, x_454.y, x_454.y, x_454.y) * x_457);
  let x_460 : vec4<f32> = x_351.unity_MatrixVP[0i];
  let x_461 : vec3<f32> = u_xlat5;
  let x_464 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_460 * vec4<f32>(x_461.x, x_461.x, x_461.x, x_461.x)) + x_464);
  let x_467 : vec4<f32> = x_351.unity_MatrixVP[2i];
  let x_468 : vec3<f32> = u_xlat5;
  let x_471 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_467 * vec4<f32>(x_468.z, x_468.z, x_468.z, x_468.z)) + x_471);
  let x_477 : vec4<f32> = u_xlat0;
  let x_479 : vec4<f32> = x_351.unity_MatrixVP[3i];
  gl_Position = (x_477 + x_479);
  return;
}
...
fn main(@location(5) in_TEXCOORD1_param : vec2<f32>, @location(4) in_TEXCOORD0_param : vec2<f32>, @location(0) in_POSITION0_param : vec4<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(3) in_COLOR0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, vs_TEXCOORD0, vs_TEXCOORD7, vs_TEXCOORD2, vs_TEXCOORD3, vs_TEXCOORD6, gl_Position);
}

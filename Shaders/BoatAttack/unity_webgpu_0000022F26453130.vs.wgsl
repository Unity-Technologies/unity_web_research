...
@group(1) @binding(7) var<uniform> x_143 : VGlobals;
...
fn main_1() {
  ...
  let x_267 : vec2<f32> = in_TEXCOORD1;
  let x_270 : vec4<f32> = x_44.unity_LightmapST;
  let x_274 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_267 * vec2<f32>(x_270.x, x_270.y)) + vec2<f32>(x_274.z, x_274.w));
  let x_277 : vec4<f32> = u_xlat0;
  let x_280 : vec4<f32> = x_143.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_277.y, x_277.y, x_277.y, x_277.y) * x_280);
  let x_283 : vec4<f32> = x_143.unity_MatrixVP[0i];
  let x_284 : vec4<f32> = u_xlat0;
  let x_287 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_283 * vec4<f32>(x_284.x, x_284.x, x_284.x, x_284.x)) + x_287);
  let x_290 : vec4<f32> = x_143.unity_MatrixVP[2i];
  ...
  let x_302 : vec4<f32> = x_143.unity_MatrixVP[3i];
  gl_Position = (x_300 + x_302);
  return;
}
...
fn main(@location(3) in_TEXCOORD0_param : vec2<f32>, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(4) in_TEXCOORD1_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_TEXCOORD3, vs_TEXCOORD4, vs_TEXCOORD7, gl_Position);
}

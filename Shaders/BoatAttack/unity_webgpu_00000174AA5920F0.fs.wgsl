...
fn main_1() {
  ...
  let x_739 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_737.x, x_737.y, x_737.z, x_737.x));
  ...
  u_xlat0 = ((vec3<f32>(x_777, x_777, x_777) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_782);
  let x_784 : vec3<f32> = u_xlat0;
  let x_785 : vec3<f32> = (x_784 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_786 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_785.x, x_785.y, x_785.z, x_786.w);
  let x_788 : vec3<f32> = u_xlat0;
  let x_789 : vec3<f32> = (x_788 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_790 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_789.x, x_789.y, x_789.z, x_790.w);
  let x_792 : vec4<f32> = u_xlat2;
  let x_794 : vec3<f32> = (vec3<f32>(x_792.x, x_792.y, x_792.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_795 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_794.x, x_794.y, x_794.z, x_795.w);
  let x_797 : vec4<f32> = u_xlat2;
  let x_800 : vec3<f32> = log2(abs(vec3<f32>(x_797.x, x_797.y, x_797.z)));
  let x_801 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_800.x, x_800.y, x_800.z, x_801.w);
  let x_803 : vec4<f32> = u_xlat2;
  let x_805 : vec3<f32> = (vec3<f32>(x_803.x, x_803.y, x_803.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_806 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_805.x, x_805.y, x_805.z, x_806.w);
  let x_808 : vec4<f32> = u_xlat2;
  let x_810 : vec3<f32> = exp2(vec3<f32>(x_808.x, x_808.y, x_808.z));
  ...
  let x_856 : vec3<f32> = max(x_855, vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_857 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_856.x, x_856.y, x_856.z, x_857.w);
  SV_Target0.w = 1.0f;
  return;
}

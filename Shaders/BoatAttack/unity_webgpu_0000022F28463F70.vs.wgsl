...
@group(1) @binding(3) var<uniform> x_293 : MainLightShadows;
...
fn main_1() {
  ...
  let x_295 : vec4<f32> = x_293.x_MainLightWorldToShadow[0i][1i];
  let x_297 : vec3<f32> = (vec3<f32>(x_287.y, x_287.y, x_287.y) * vec3<f32>(x_295.x, x_295.y, x_295.z));
  let x_298 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_297.x, x_297.y, x_297.z, x_298.w);
  let x_301 : vec4<f32> = x_293.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_312 : vec4<f32> = x_293.x_MainLightWorldToShadow[0i][2i];
  let x_314 : vec3<f32> = u_xlat1;
  let x_317 : vec4<f32> = u_xlat0;
  let x_319 : vec3<f32> = ((vec3<f32>(x_312.x, x_312.y, x_312.z) * vec3<f32>(x_314.z, x_314.z, x_314.z)) + vec3<f32>(x_317.x, x_317.y, x_317.z));
  let x_320 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_319.x, x_319.y, x_319.z, x_320.w);
  let x_323 : vec4<f32> = u_xlat0;
  let x_326 : vec4<f32> = x_293.x_MainLightWorldToShadow[0i][3i];
  let x_328 : vec3<f32> = (vec3<f32>(x_323.x, x_323.y, x_323.z) + vec3<f32>(x_326.x, x_326.y, x_326.z));
  let x_329 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_328.x, x_328.y, x_328.z, x_329.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_332 : vec3<f32> = u_xlat1;
  let x_335 : vec4<f32> = x_136.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_332.y, x_332.y, x_332.y, x_332.y) * x_335);
  let x_338 : vec4<f32> = x_136.unity_MatrixVP[0i];
  let x_339 : vec3<f32> = u_xlat1;
  let x_342 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_338 * vec4<f32>(x_339.x, x_339.x, x_339.x, x_339.x)) + x_342);
  let x_345 : vec4<f32> = x_136.unity_MatrixVP[2i];
  let x_346 : vec3<f32> = u_xlat1;
  let x_349 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_345 * vec4<f32>(x_346.z, x_346.z, x_346.z, x_346.z)) + x_349);
  let x_355 : vec4<f32> = u_xlat0;
  let x_357 : vec4<f32> = x_136.unity_MatrixVP[3i];
  gl_Position = (x_355 + x_357);
  return;
}

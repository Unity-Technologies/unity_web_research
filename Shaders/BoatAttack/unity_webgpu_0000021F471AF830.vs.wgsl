...
fn main_1() {
  ...
  let x_98 : f32 = x_69.x_ProjectionParams.x;
  u_xlat1.x = (x_94 * x_98);
  let x_102 : f32 = u_xlat1.x;
  u_xlat1.w = (x_102 * 0.5f);
  let x_108 : vec4<f32> = u_xlat0;
  let x_111 : vec2<f32> = (vec2<f32>(x_108.x, x_108.w) * vec2<f32>(0.5f, 0.5f));
  let x_112 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_111.x, x_112.y, x_111.y, x_112.w);
  let x_116 : vec4<f32> = u_xlat1;
  let x_118 : vec4<f32> = u_xlat1;
  let x_120 : vec2<f32> = (vec2<f32>(x_116.z, x_116.z) + vec2<f32>(x_118.x, x_118.w));
  let x_121 : vec4<f32> = vs_TEXCOORD0;
  vs_TEXCOORD0 = vec4<f32>(x_120.x, x_120.y, x_121.z, x_121.w);
  let x_123 : vec4<f32> = u_xlat0;
  let x_124 : vec2<f32> = vec2<f32>(x_123.z, x_123.w);

...
fn main_1() {
  ...
  let x_942 : vec2<f32> = vs_TEXCOORD0;
  let x_945 : vec4<f32> = x_25.x_Dithering_Params;
  let x_949 : vec4<f32> = x_25.x_Dithering_Params;
  let x_951 : vec2<f32> = ((x_942 * vec2<f32>(x_945.x, x_945.y)) + vec2<f32>(x_949.z, x_949.w));
  let x_952 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_951.x, x_951.y, x_952.z, x_952.w);
  let x_959 : vec4<f32> = u_xlat1;
  let x_962 : f32 = x_25.x_GlobalMipBias.x;
  let x_963 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_959.x, x_959.y), x_962);
  u_xlat15 = x_963.w;
  let x_965 : f32 = u_xlat15;
  u_xlat15 = ((x_965 * 2.0f) + -1.0f);
  let x_969 : f32 = u_xlat15;
  u_xlatb1.x = (x_969 >= 0.0f);
  let x_973 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_973);
  let x_976 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_976)) + 1.0f);
  let x_980 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_980);
  ...
  let x_991 : vec3<f32> = (vec3<f32>(x_989.x, x_989.y, x_989.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_1002 : vec3<f32> = (vec3<f32>(x_1000.x, x_1000.y, x_1000.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_1013 : vec3<f32> = ((vec3<f32>(x_1010.x, x_1010.y, x_1010.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  ...
  let x_1085 : vec3<f32> = log2(abs(vec3<f32>(x_1082.x, x_1082.y, x_1082.z)));
  let x_1086 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1085.x, x_1085.y, x_1085.z, x_1086.w);
  let x_1088 : vec4<f32> = u_xlat2;
  let x_1090 : vec3<f32> = (vec3<f32>(x_1088.x, x_1088.y, x_1088.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_1091 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1090.x, x_1090.y, x_1090.z, x_1091.w);
  let x_1093 : vec4<f32> = u_xlat2;
  let x_1095 : vec3<f32> = exp2(vec3<f32>(x_1093.x, x_1093.y, x_1093.z));
  let x_1096 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1095.x, x_1095.y, x_1095.z, x_1096.w);
  let x_1098 : vec4<f32> = u_xlat0;
  let x_1100 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1098.x, x_1098.y, x_1098.z, x_1098.x));
  u_xlatb0 = vec3<bool>(x_1100.x, x_1100.y, x_1100.z);
  ...
  u_xlat0.y = x_1124;
  let x_1127 : bool = u_xlatb0.z;
  if (x_1127) {
    let x_1132 : f32 = u_xlat1.z;
    x_1128 = x_1132;
  } else {
  ...
  let x_1143 : vec3<f32> = max(vec3<f32>(x_1140.x, x_1140.y, x_1140.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  let x_860 : f32 = x_852;
  u_xlat0.x = x_860;
  let x_863 : bool = u_xlatb0.y;
  if (x_863) {
    let x_868 : f32 = u_xlat1.y;
    x_864 = x_868;
  } else {
    let x_871 : f32 = u_xlat2.y;
    x_864 = x_871;
  }
  let x_872 : f32 = x_864;
  u_xlat0.y = x_872;
  let x_875 : bool = u_xlatb0.z;
  if (x_875) {
  ...
  let x_889 : vec4<f32> = x_26.x_Dithering_Params;
  let x_893 : vec4<f32> = x_26.x_Dithering_Params;
  let x_895 : vec2<f32> = ((x_886 * vec2<f32>(x_889.x, x_889.y)) + vec2<f32>(x_893.z, x_893.w));
  let x_896 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_895.x, x_895.y, x_896.z);
  let x_903 : vec3<f32> = u_xlat1;
  let x_906 : f32 = x_26.x_GlobalMipBias.x;
  ...
  u_xlat1 = (x_933 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_935 : vec3<f32> = u_xlat0;
  let x_937 : vec3<f32> = log2(abs(x_935));
  let x_938 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_937.x, x_937.y, x_937.z, x_938.w);
  let x_940 : vec4<f32> = u_xlat2;
  let x_942 : vec3<f32> = (vec3<f32>(x_940.x, x_940.y, x_940.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_953 : vec3<f32> = ((vec3<f32>(x_950.x, x_950.y, x_950.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_954 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_953.x, x_953.y, x_953.z, x_954.w);
  let x_956 : vec3<f32> = u_xlat0;
  let x_958 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_956.x, x_956.y, x_956.z, x_956.x));
  ...
  let x_994 : f32 = x_986;
  u_xlat0.z = x_994;
  let x_996 : f32 = u_xlat15;
  let x_1001 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_996, x_996, x_996) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_1001);
  let x_1003 : vec3<f32> = u_xlat0;
  u_xlat1 = (x_1003 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1005 : vec3<f32> = u_xlat0;
  let x_1006 : vec3<f32> = (x_1005 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1007 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1006.x, x_1006.y, x_1006.z, x_1007.w);
  let x_1009 : vec4<f32> = u_xlat2;
  let x_1011 : vec3<f32> = (vec3<f32>(x_1009.x, x_1009.y, x_1009.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_1022 : vec3<f32> = (vec3<f32>(x_1020.x, x_1020.y, x_1020.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_1023 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1022.x, x_1022.y, x_1022.z, x_1023.w);
  let x_1025 : vec4<f32> = u_xlat2;
  let x_1027 : vec3<f32> = exp2(vec3<f32>(x_1025.x, x_1025.y, x_1025.z));
  ...
  let x_1073 : vec3<f32> = max(x_1072, vec3<f32>(0.0f, 0.0f, 0.0f));

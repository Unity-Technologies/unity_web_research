...
fn main_1() {
  ...
  let x_733 : vec3<f32> = sqrt(vec3<f32>(x_731.x, x_731.y, x_731.z));
  let x_734 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_733.x, x_733.y, x_733.z, x_734.w);
  let x_736 : vec2<f32> = vs_TEXCOORD0;
  let x_739 : vec4<f32> = x_26.x_Dithering_Params;
  let x_743 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_756 : f32 = x_26.x_GlobalMipBias.x;
  let x_757 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_753.x, x_753.y), x_756);
  u_xlat15 = x_757.w;
  let x_759 : f32 = u_xlat15;
  u_xlat15 = ((x_759 * 2.0f) + -1.0f);
  let x_764 : f32 = u_xlat15;
  u_xlatb1.x = (x_764 >= 0.0f);
  let x_768 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_768);
  let x_771 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_771)) + 1.0f);
  ...
  u_xlat15 = (x_780 * x_782);
  let x_784 : vec4<f32> = u_xlat0;
  let x_786 : vec3<f32> = (vec3<f32>(x_784.x, x_784.y, x_784.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_813 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_811.x, x_811.y, x_811.z, x_811.x));
  ...
  let x_858 : vec3<f32> = ((vec3<f32>(x_851, x_851, x_851) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_856.x, x_856.y, x_856.z));
  let x_859 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_858.x, x_858.y, x_858.z, x_859.w);
  let x_861 : vec4<f32> = u_xlat0;
  let x_863 : vec3<f32> = (vec3<f32>(x_861.x, x_861.y, x_861.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_864 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_863.x, x_863.y, x_863.z, x_864.w);
  let x_866 : vec4<f32> = u_xlat0;
  let x_868 : vec3<f32> = (vec3<f32>(x_866.x, x_866.y, x_866.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_869 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_868.x, x_868.y, x_868.z, x_869.w);
  let x_871 : vec4<f32> = u_xlat2;
  let x_873 : vec3<f32> = (vec3<f32>(x_871.x, x_871.y, x_871.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_874 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_873.x, x_873.y, x_873.z, x_874.w);
  let x_876 : vec4<f32> = u_xlat2;
  let x_879 : vec3<f32> = log2(abs(vec3<f32>(x_876.x, x_876.y, x_876.z)));
  let x_880 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_879.x, x_879.y, x_879.z, x_880.w);
  let x_882 : vec4<f32> = u_xlat2;
  let x_884 : vec3<f32> = (vec3<f32>(x_882.x, x_882.y, x_882.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_937 : vec3<f32> = max(vec3<f32>(x_934.x, x_934.y, x_934.z), vec3<f32>(0.0f, 0.0f, 0.0f));

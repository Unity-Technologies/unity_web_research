...
fn main_1() {
  ...
  let x_615 : vec3<f32> = ((x_609 * vec3<f32>(x_610.x, x_610.x, x_610.x)) + vec3<f32>(x_613.x, x_613.y, x_613.z));
  let x_616 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_615.x, x_615.y, x_615.z, x_616.w);
  let x_618 : vec2<f32> = vs_TEXCOORD0;
  let x_621 : vec4<f32> = x_26.x_Dithering_Params;
  let x_625 : vec4<f32> = x_26.x_Dithering_Params;
  let x_627 : vec2<f32> = ((x_618 * vec2<f32>(x_621.x, x_621.y)) + vec2<f32>(x_625.z, x_625.w));
  let x_628 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_627.x, x_627.y, x_628.z);
  let x_635 : vec3<f32> = u_xlat1;
  let x_638 : f32 = x_26.x_GlobalMipBias.x;
  let x_639 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_635.x, x_635.y), x_638);
  u_xlat15 = x_639.w;
  let x_641 : f32 = u_xlat15;
  u_xlat15 = ((x_641 * 2.0f) + -1.0f);
  let x_646 : f32 = u_xlat15;
  u_xlatb1.x = (x_646 >= 0.0f);
  let x_650 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_650);
  let x_653 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_653)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_666.x, x_666.y, x_666.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_669 : vec4<f32> = u_xlat0;
  let x_672 : vec3<f32> = log2(abs(vec3<f32>(x_669.x, x_669.y, x_669.z)));
  let x_673 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_672.x, x_672.y, x_672.z, x_673.w);
  let x_675 : vec4<f32> = u_xlat2;
  let x_677 : vec3<f32> = (vec3<f32>(x_675.x, x_675.y, x_675.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_688 : vec3<f32> = ((vec3<f32>(x_685.x, x_685.y, x_685.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_689 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_688.x, x_688.y, x_688.z, x_689.w);
  let x_692 : vec4<f32> = u_xlat0;
  let x_694 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_692.x, x_692.y, x_692.z, x_692.x));
  u_xlatb0 = vec3<bool>(x_694.x, x_694.y, x_694.z);
  let x_697 : bool = u_xlatb0.x;
  if (x_697) {
  ...
  let x_718 : f32 = x_710;
  u_xlat0.y = x_718;
  let x_721 : bool = u_xlatb0.z;
  if (x_721) {
    let x_726 : f32 = u_xlat1.z;
    x_722 = x_726;
  } else {
  ...
  let x_739 : vec3<f32> = ((vec3<f32>(x_732, x_732, x_732) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_737.x, x_737.y, x_737.z));
  let x_740 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_739.x, x_739.y, x_739.z, x_740.w);
  let x_742 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_742.x, x_742.y, x_742.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_745 : vec4<f32> = u_xlat0;
  let x_747 : vec3<f32> = (vec3<f32>(x_745.x, x_745.y, x_745.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_748 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_747.x, x_747.y, x_747.z, x_748.w);
  let x_750 : vec4<f32> = u_xlat2;
  let x_752 : vec3<f32> = (vec3<f32>(x_750.x, x_750.y, x_750.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_753 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_752.x, x_752.y, x_752.z, x_753.w);
  let x_755 : vec4<f32> = u_xlat2;
  let x_758 : vec3<f32> = log2(abs(vec3<f32>(x_755.x, x_755.y, x_755.z)));
  let x_759 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_758.x, x_758.y, x_758.z, x_759.w);
  let x_761 : vec4<f32> = u_xlat2;
  let x_763 : vec3<f32> = (vec3<f32>(x_761.x, x_761.y, x_761.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_816 : vec3<f32> = max(vec3<f32>(x_813.x, x_813.y, x_813.z), vec3<f32>(0.0f, 0.0f, 0.0f));

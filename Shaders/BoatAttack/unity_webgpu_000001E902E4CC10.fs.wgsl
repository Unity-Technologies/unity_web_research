...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_627.x, x_627.y, x_627.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_630 : vec4<f32> = u_xlat0;
  let x_633 : vec3<f32> = log2(abs(vec3<f32>(x_630.x, x_630.y, x_630.z)));
  let x_634 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_633.x, x_633.y, x_633.z, x_634.w);
  let x_636 : vec4<f32> = u_xlat2;
  let x_638 : vec3<f32> = (vec3<f32>(x_636.x, x_636.y, x_636.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_649 : vec3<f32> = ((vec3<f32>(x_646.x, x_646.y, x_646.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_650 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_649.x, x_649.y, x_649.z, x_650.w);
  let x_653 : vec4<f32> = u_xlat0;
  let x_655 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_653.x, x_653.y, x_653.z, x_653.x));
  ...
  let x_696 : vec4<f32> = x_40.x_Dithering_Params;
  let x_700 : vec4<f32> = x_40.x_Dithering_Params;
  let x_702 : vec2<f32> = ((x_693 * vec2<f32>(x_696.x, x_696.y)) + vec2<f32>(x_700.z, x_700.w));
  let x_703 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_702.x, x_702.y, x_703.z);
  let x_710 : vec3<f32> = u_xlat1;
  let x_713 : f32 = x_40.x_GlobalMipBias.x;
  ...
  u_xlatb1.x = (x_719 >= 0.0f);
  let x_723 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_723);
  let x_726 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_726)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_814.x, x_814.y, x_814.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_817 : vec4<f32> = u_xlat0;
  let x_819 : vec3<f32> = (vec3<f32>(x_817.x, x_817.y, x_817.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_820 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_819.x, x_819.y, x_819.z, x_820.w);
  let x_822 : vec4<f32> = u_xlat2;
  let x_824 : vec3<f32> = (vec3<f32>(x_822.x, x_822.y, x_822.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_825 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_824.x, x_824.y, x_824.z, x_825.w);
  let x_827 : vec4<f32> = u_xlat2;
  let x_830 : vec3<f32> = log2(abs(vec3<f32>(x_827.x, x_827.y, x_827.z)));
  let x_831 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_830.x, x_830.y, x_830.z, x_831.w);
  let x_833 : vec4<f32> = u_xlat2;
  let x_835 : vec3<f32> = (vec3<f32>(x_833.x, x_833.y, x_833.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  if (x_872) {
    ...
    let x_880 : f32 = u_xlat2.z;
    x_873 = x_880;
  }
  let x_881 : f32 = x_873;
  u_xlat0.z = x_881;
  let x_885 : vec4<f32> = u_xlat0;
  let x_888 : vec3<f32> = max(vec3<f32>(x_885.x, x_885.y, x_885.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_889 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_888.x, x_888.y, x_888.z, x_889.w);
  SV_Target0.w = 1.0f;
  return;
}

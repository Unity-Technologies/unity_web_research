...
fn main_1() {
  ...
  let x_887 : f32 = x_879;
  u_xlat0.z = x_887;
  let x_889 : vec2<f32> = vs_TEXCOORD0;
  let x_892 : vec4<f32> = x_25.x_Dithering_Params;
  let x_896 : vec4<f32> = x_25.x_Dithering_Params;
  let x_898 : vec2<f32> = ((x_889 * vec2<f32>(x_892.x, x_892.y)) + vec2<f32>(x_896.z, x_896.w));
  let x_899 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_898.x, x_898.y, x_899.z);
  let x_906 : vec3<f32> = u_xlat1;
  let x_909 : f32 = x_25.x_GlobalMipBias.x;
  let x_910 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_906.x, x_906.y), x_909);
  u_xlat15 = x_910.w;
  let x_912 : f32 = u_xlat15;
  u_xlat15 = ((x_912 * 2.0f) + -1.0f);
  let x_916 : f32 = u_xlat15;
  u_xlatb1.x = (x_916 >= 0.0f);
  let x_920 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_920);
  let x_923 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_923)) + 1.0f);
  ...
  u_xlat15 = (x_932 * x_934);
  let x_936 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_936.x, x_936.y, x_936.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_947 : vec3<f32> = (vec3<f32>(x_945.x, x_945.y, x_945.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_958 : vec3<f32> = ((vec3<f32>(x_955.x, x_955.y, x_955.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_959 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_958.x, x_958.y, x_958.z, x_959.w);
  let x_961 : vec4<f32> = u_xlat0;
  let x_963 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_961.x, x_961.y, x_961.z, x_961.x));
  ...
  let x_1008 : vec3<f32> = ((vec3<f32>(x_1001, x_1001, x_1001) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_1006.x, x_1006.y, x_1006.z));
  let x_1009 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1008.x, x_1008.y, x_1008.z, x_1009.w);
  let x_1011 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_1011.x, x_1011.y, x_1011.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1014 : vec4<f32> = u_xlat0;
  let x_1016 : vec3<f32> = (vec3<f32>(x_1014.x, x_1014.y, x_1014.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1017 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1016.x, x_1016.y, x_1016.z, x_1017.w);
  let x_1019 : vec4<f32> = u_xlat2;
  let x_1021 : vec3<f32> = (vec3<f32>(x_1019.x, x_1019.y, x_1019.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1022 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1021.x, x_1021.y, x_1021.z, x_1022.w);
  let x_1024 : vec4<f32> = u_xlat2;
  let x_1027 : vec3<f32> = log2(abs(vec3<f32>(x_1024.x, x_1024.y, x_1024.z)));
  let x_1028 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1027.x, x_1027.y, x_1027.z, x_1028.w);
  let x_1030 : vec4<f32> = u_xlat2;
  let x_1032 : vec3<f32> = (vec3<f32>(x_1030.x, x_1030.y, x_1030.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  if (x_1057) {
    ...
    let x_1065 : f32 = u_xlat2.y;
    x_1058 = x_1065;
  }
  let x_1066 : f32 = x_1058;
  u_xlat0.y = x_1066;
  let x_1069 : bool = u_xlatb0.z;
  if (x_1069) {
    let x_1074 : f32 = u_xlat1.z;
    x_1070 = x_1074;
  } else {
  ...
  let x_1085 : vec3<f32> = max(vec3<f32>(x_1082.x, x_1082.y, x_1082.z), vec3<f32>(0.0f, 0.0f, 0.0f));

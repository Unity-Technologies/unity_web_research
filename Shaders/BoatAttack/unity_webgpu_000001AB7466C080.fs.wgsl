...
fn main_1() {
  ...
  let x_590 : vec4<f32> = x_40.x_Dithering_Params;
  let x_594 : vec4<f32> = x_40.x_Dithering_Params;
  ...
  let x_607 : f32 = x_40.x_GlobalMipBias.x;
  let x_608 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_604.x, x_604.y), x_607);
  u_xlat15 = x_608.w;
  ...
  u_xlat1 = (vec3<f32>(x_633.x, x_633.y, x_633.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_636 : vec4<f32> = u_xlat0;
  let x_639 : vec3<f32> = log2(abs(vec3<f32>(x_636.x, x_636.y, x_636.z)));
  let x_640 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_639.x, x_639.y, x_639.z, x_640.w);
  let x_642 : vec4<f32> = u_xlat2;
  let x_644 : vec3<f32> = (vec3<f32>(x_642.x, x_642.y, x_642.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_655 : vec3<f32> = ((vec3<f32>(x_652.x, x_652.y, x_652.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_656 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_655.x, x_655.y, x_655.z, x_656.w);
  let x_659 : vec4<f32> = u_xlat0;
  let x_661 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_659.x, x_659.y, x_659.z, x_659.x));
  u_xlatb0 = vec3<bool>(x_661.x, x_661.y, x_661.z);
  ...
  let x_673 : f32 = x_665;
  u_xlat0.x = x_673;
  let x_676 : bool = u_xlatb0.y;
  if (x_676) {
    let x_681 : f32 = u_xlat1.y;
    x_677 = x_681;
  } else {
  ...
  let x_685 : f32 = x_677;
  u_xlat0.y = x_685;
  let x_688 : bool = u_xlatb0.z;
  if (x_688) {
    let x_693 : f32 = u_xlat1.z;
    x_689 = x_693;
  } else {
    let x_696 : f32 = u_xlat2.z;
    x_689 = x_696;
  }
  let x_697 : f32 = x_689;
  u_xlat0.z = x_697;
  ...
  let x_706 : vec3<f32> = ((vec3<f32>(x_699, x_699, x_699) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_704.x, x_704.y, x_704.z));
  let x_707 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_706.x, x_706.y, x_706.z, x_707.w);
  let x_709 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_709.x, x_709.y, x_709.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_712 : vec4<f32> = u_xlat0;
  let x_714 : vec3<f32> = (vec3<f32>(x_712.x, x_712.y, x_712.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_715 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_714.x, x_714.y, x_714.z, x_715.w);
  let x_717 : vec4<f32> = u_xlat2;
  let x_719 : vec3<f32> = (vec3<f32>(x_717.x, x_717.y, x_717.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_740 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_738.x, x_738.y, x_738.z, x_738.x));
  u_xlatb0 = vec3<bool>(x_740.x, x_740.y, x_740.z);
  let x_743 : bool = u_xlatb0.x;
  if (x_743) {
  ...
  let x_752 : f32 = x_744;
  u_xlat0.x = x_752;
  let x_755 : bool = u_xlatb0.y;
  if (x_755) {
    let x_760 : f32 = u_xlat1.y;
    x_756 = x_760;
  } else {
  ...
  let x_764 : f32 = x_756;
  u_xlat0.y = x_764;
  let x_767 : bool = u_xlatb0.z;
  if (x_767) {
    let x_772 : f32 = u_xlat1.z;
    x_768 = x_772;
  } else {
  ...
  let x_783 : vec3<f32> = max(vec3<f32>(x_780.x, x_780.y, x_780.z), vec3<f32>(0.0f, 0.0f, 0.0f));

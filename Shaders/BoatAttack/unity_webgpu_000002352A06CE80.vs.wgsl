...
@group(1) @binding(3) var<uniform> x_215 : MainLightShadows;
...
fn main_1() {
  ...
  let x_217 : vec4<f32> = x_215.x_MainLightWorldToShadow[0i][1i];
  let x_219 : vec3<f32> = (vec3<f32>(x_209.y, x_209.y, x_209.y) * vec3<f32>(x_217.x, x_217.y, x_217.z));
  let x_220 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_219.x, x_219.y, x_219.z, x_220.w);
  let x_223 : vec4<f32> = x_215.x_MainLightWorldToShadow[0i][0i];
  ...
  u_xlat0 = vec4<f32>(x_230.x, x_230.y, x_231.z, x_230.z);
  let x_234 : vec4<f32> = x_215.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_248 : vec4<f32> = x_215.x_MainLightWorldToShadow[0i][3i];
  let x_250 : vec3<f32> = (vec3<f32>(x_245.x, x_245.y, x_245.z) + vec3<f32>(x_248.x, x_248.y, x_248.z));
  let x_251 : vec4<f32> = vs_INTERP9;
  vs_INTERP9 = vec4<f32>(x_250.x, x_250.y, x_250.z, x_251.w);
  vs_INTERP9.w = 0.0f;
  let x_257 : vec4<f32> = phase0_Output0_6;
  vs_INTERP5 = vec2<f32>(x_257.x, x_257.y);
  let x_260 : vec4<f32> = phase0_Output0_6;
  vs_INTERP6 = vec2<f32>(x_260.z, x_260.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>, @location(4) in_COLOR0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_INTERP0, vs_INTERP1, vs_INTERP2, vs_INTERP3, vs_INTERP4, vs_INTERP9);
}

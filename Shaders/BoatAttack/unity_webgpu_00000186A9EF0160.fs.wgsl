...
fn main_1() {
  ...
  let x_825 : vec4<f32> = x_26.x_Dithering_Params;
  let x_829 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_842 : f32 = x_26.x_GlobalMipBias.x;
  let x_843 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_839.x, x_839.y), x_842);
  u_xlat15 = x_843.w;
  let x_845 : f32 = u_xlat15;
  u_xlat15 = ((x_845 * 2.0f) + -1.0f);
  let x_849 : f32 = u_xlat15;
  u_xlatb1.x = (x_849 >= 0.0f);
  let x_853 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_853);
  let x_856 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_856)) + 1.0f);
  ...
  u_xlat15 = (x_865 * x_867);
  let x_869 : vec3<f32> = u_xlat0;
  u_xlat1 = (x_869 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_871 : vec3<f32> = u_xlat0;
  let x_873 : vec3<f32> = log2(abs(x_871));
  let x_874 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_873.x, x_873.y, x_873.z, x_874.w);
  let x_876 : vec4<f32> = u_xlat2;
  let x_878 : vec3<f32> = (vec3<f32>(x_876.x, x_876.y, x_876.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_889 : vec3<f32> = ((vec3<f32>(x_886.x, x_886.y, x_886.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_890 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_889.x, x_889.y, x_889.z, x_890.w);
  let x_893 : vec3<f32> = u_xlat0;
  let x_895 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_893.x, x_893.y, x_893.z, x_893.x));
  ...
  u_xlat1 = (x_940 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_942 : vec3<f32> = u_xlat0;
  let x_943 : vec3<f32> = (x_942 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_944 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_943.x, x_943.y, x_943.z, x_944.w);
  let x_946 : vec4<f32> = u_xlat2;
  let x_948 : vec3<f32> = (vec3<f32>(x_946.x, x_946.y, x_946.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_959 : vec3<f32> = (vec3<f32>(x_957.x, x_957.y, x_957.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_969 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_967.x, x_967.y, x_967.z, x_967.x));
  u_xlatb0 = vec3<bool>(x_969.x, x_969.y, x_969.z);
  let x_972 : bool = u_xlatb0.x;
  if (x_972) {
  ...
  let x_981 : f32 = x_973;
  u_xlat0.x = x_981;
  let x_984 : bool = u_xlatb0.y;
  if (x_984) {
    let x_989 : f32 = u_xlat1.y;
    x_985 = x_989;
  } else {
    let x_992 : f32 = u_xlat2.y;
    x_985 = x_992;
  }
  let x_993 : f32 = x_985;
  u_xlat0.y = x_993;
  let x_996 : bool = u_xlatb0.z;
  if (x_996) {
    let x_1001 : f32 = u_xlat1.z;
    x_997 = x_1001;
  } else {
  ...
  u_xlat0.z = x_1005;
  let x_1009 : vec3<f32> = u_xlat0;
  let x_1010 : vec3<f32> = max(x_1009, vec3<f32>(0.0f, 0.0f, 0.0f));

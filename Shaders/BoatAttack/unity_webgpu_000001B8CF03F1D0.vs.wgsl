...
fn main_1() {
  ...
  let x_161 : vec2<f32> = u_xlat6;
  let x_162 : vec2<f32> = (vec2<f32>(0.25f, 0.25f) / x_161);
  let x_163 : vec4<f32> = vs_TEXCOORD2;
  vs_TEXCOORD2 = vec4<f32>(x_163.x, x_163.y, x_162.x, x_162.y);
  let x_166 : vec4<f32> = in_COLOR0;
  let x_168 : vec4<f32> = x_28.x_Color;
  u_xlat1 = (x_166 * x_168);
  let x_172 : vec4<f32> = u_xlat1;
  let x_176 : f32 = x_28.x_DiffusePower;
  let x_178 : vec3<f32> = (vec3<f32>(x_172.x, x_172.y, x_172.z) * vec3<f32>(x_176, x_176, x_176));
  let x_179 : vec4<f32> = vs_COLOR0;
  vs_COLOR0 = vec4<f32>(x_178.x, x_178.y, x_178.z, x_179.w);
  let x_182 : f32 = u_xlat1.w;
  vs_COLOR0.w = x_182;
  let x_189 : vec2<f32> = in_TEXCOORD0;
  vs_TEXCOORD0 = x_189;
  let x_192 : vec4<f32> = x_28.x_ClipRect;
  u_xlat1 = max(x_192, vec4<f32>(-20000000000.0f, -20000000000.0f, -20000000000.0f, -20000000000.0f));
  let x_196 : vec4<f32> = u_xlat1;
  u_xlat1 = min(x_196, vec4<f32>(20000000000.0f, 20000000000.0f, 20000000000.0f, 20000000000.0f));
  let x_200 : vec2<f32> = u_xlat0;
  let x_204 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_200 * vec2<f32>(2.0f, 2.0f)) + -(vec2<f32>(x_204.x, x_204.y)));
  let x_208 : vec4<f32> = u_xlat1;
  let x_211 : vec2<f32> = u_xlat0;
  let x_212 : vec2<f32> = (-(vec2<f32>(x_208.z, x_208.w)) + x_211);
  let x_213 : vec4<f32> = vs_TEXCOORD2;
  vs_TEXCOORD2 = vec4<f32>(x_212.x, x_212.y, x_213.z, x_213.w);
  return;
}

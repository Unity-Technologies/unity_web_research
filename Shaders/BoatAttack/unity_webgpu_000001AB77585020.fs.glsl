...
layout(set = 0, binding = 20) uniform mediump  sampler sampler_MainLightCookieTexture;
layout(set = 0, binding = 21) uniform mediump  sampler sampler_AdditionalLightsCookieAtlasTexture;
layout(set = 0, binding = 22) uniform mediump  sampler sampler_DitheringTexture;
void main()
{
vec4 u_xlat0;
bool u_xlatb0;
...
vec4 hlslcc_FragCoord = vec4(gl_FragCoord.xyz, 1.0/gl_FragCoord.w);
    ...
    u_xlat16.xy = hlslcc_FragCoord.xy * vec2(_DitheringTextureInvSize);
    u_xlat16.x = texture(sampler2D(_DitheringTexture, sampler_DitheringTexture), u_xlat16.xy, _GlobalMipBias.x).w;
    u_xlatb29 = unity_LODFade.x>=0.0;
    u_xlat16.x = (u_xlatb29) ? abs(u_xlat16.x) : -abs(u_xlat16.x);

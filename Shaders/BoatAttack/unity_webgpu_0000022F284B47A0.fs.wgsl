...
fn main_1() {
  ...
  let x_561 : vec4<f32> = x_27.x_Grain_TilingParams;
  let x_565 : vec4<f32> = x_27.x_Grain_TilingParams;
  ...
  let x_578 : f32 = x_27.x_GlobalMipBias.x;
  let x_579 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_575.x, x_575.y), x_578);
  u_xlat15 = x_579.w;
  let x_581 : f32 = u_xlat15;
  u_xlat15 = (x_581 + -0.5f);
  let x_584 : f32 = u_xlat15;
  let x_585 : f32 = u_xlat15;
  u_xlat15 = (x_584 + x_585);
  let x_587 : vec3<f32> = u_xlat0;
  u_xlat1.x = dot(x_587, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_595 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_595);
  let x_600 : f32 = x_27.x_Grain_Params.y;
  let x_602 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_600 * -(x_602)) + 1.0f);
  let x_608 : f32 = u_xlat15;
  let x_610 : vec3<f32> = u_xlat0;
  u_xlat6 = (vec3<f32>(x_608, x_608, x_608) * x_610);
  let x_612 : vec3<f32> = u_xlat6;
  let x_615 : vec2<f32> = x_27.x_Grain_Params;
  u_xlat6 = (x_612 * vec3<f32>(x_615.x, x_615.x, x_615.x));
  let x_618 : vec3<f32> = u_xlat6;
  let x_619 : vec4<f32> = u_xlat1;
  let x_622 : vec3<f32> = u_xlat0;
  u_xlat0 = ((x_618 * vec3<f32>(x_619.x, x_619.x, x_619.x)) + x_622);
  let x_624 : vec2<f32> = vs_TEXCOORD0;
  let x_627 : vec4<f32> = x_27.x_Dithering_Params;
  let x_631 : vec4<f32> = x_27.x_Dithering_Params;
  ...
  let x_644 : f32 = x_27.x_GlobalMipBias.x;
  ...
  let x_673 : vec3<f32> = (x_672 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_674 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_673.x, x_673.y, x_673.z, x_674.w);
  let x_676 : vec3<f32> = u_xlat0;
  let x_678 : vec3<f32> = log2(abs(x_676));
  ...
  u_xlat0 = ((vec3<f32>(x_738, x_738, x_738) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_743);
  let x_745 : vec3<f32> = u_xlat0;
  let x_746 : vec3<f32> = (x_745 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_747 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_746.x, x_746.y, x_746.z, x_747.w);
  let x_749 : vec3<f32> = u_xlat0;
  let x_750 : vec3<f32> = (x_749 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_751 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_750.x, x_750.y, x_750.z, x_751.w);
  let x_753 : vec4<f32> = u_xlat2;
  let x_755 : vec3<f32> = (vec3<f32>(x_753.x, x_753.y, x_753.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_766 : vec3<f32> = (vec3<f32>(x_764.x, x_764.y, x_764.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_776 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_774.x, x_774.y, x_774.z, x_774.x));
  ...
  let x_817 : vec3<f32> = max(x_816, vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  let x_747 : vec3<f32> = sqrt(vec3<f32>(x_745.x, x_745.y, x_745.z));
  let x_748 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_747.x, x_747.y, x_747.z, x_748.w);
  let x_750 : vec2<f32> = vs_TEXCOORD0;
  let x_753 : vec4<f32> = x_25.x_Dithering_Params;
  let x_757 : vec4<f32> = x_25.x_Dithering_Params;
  let x_759 : vec2<f32> = ((x_750 * vec2<f32>(x_753.x, x_753.y)) + vec2<f32>(x_757.z, x_757.w));
  let x_760 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_759.x, x_759.y, x_760.z);
  let x_767 : vec3<f32> = u_xlat1;
  let x_770 : f32 = x_25.x_GlobalMipBias.x;
  let x_771 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_767.x, x_767.y), x_770);
  u_xlat15 = x_771.w;
  let x_773 : f32 = u_xlat15;
  u_xlat15 = ((x_773 * 2.0f) + -1.0f);
  let x_777 : f32 = u_xlat15;
  u_xlatb1.x = (x_777 >= 0.0f);
  let x_781 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_781);
  let x_784 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_784)) + 1.0f);
  ...
  u_xlat15 = (x_793 * x_795);
  let x_797 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_797.x, x_797.y, x_797.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_800 : vec4<f32> = u_xlat0;
  let x_802 : vec3<f32> = log2(vec3<f32>(x_800.x, x_800.y, x_800.z));
  let x_803 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_802.x, x_802.y, x_802.z, x_803.w);
  let x_805 : vec4<f32> = u_xlat2;
  let x_807 : vec3<f32> = (vec3<f32>(x_805.x, x_805.y, x_805.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_818 : vec3<f32> = ((vec3<f32>(x_815.x, x_815.y, x_815.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_819 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_818.x, x_818.y, x_818.z, x_819.w);
  let x_822 : vec4<f32> = u_xlat0;
  let x_824 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_822.x, x_822.y, x_822.z, x_822.x));
  ...
  let x_869 : vec3<f32> = ((vec3<f32>(x_862, x_862, x_862) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_867.x, x_867.y, x_867.z));
  let x_870 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_869.x, x_869.y, x_869.z, x_870.w);
  let x_872 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_872.x, x_872.y, x_872.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_875 : vec4<f32> = u_xlat0;
  let x_877 : vec3<f32> = (vec3<f32>(x_875.x, x_875.y, x_875.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_878 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_877.x, x_877.y, x_877.z, x_878.w);
  let x_880 : vec4<f32> = u_xlat2;
  let x_882 : vec3<f32> = (vec3<f32>(x_880.x, x_880.y, x_880.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_883 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_882.x, x_882.y, x_882.z, x_883.w);
  let x_885 : vec4<f32> = u_xlat2;
  let x_888 : vec3<f32> = log2(abs(vec3<f32>(x_885.x, x_885.y, x_885.z)));
  let x_889 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_888.x, x_888.y, x_888.z, x_889.w);
  let x_891 : vec4<f32> = u_xlat2;
  let x_893 : vec3<f32> = (vec3<f32>(x_891.x, x_891.y, x_891.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_894 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_893.x, x_893.y, x_893.z, x_894.w);
  let x_896 : vec4<f32> = u_xlat2;
  let x_898 : vec3<f32> = exp2(vec3<f32>(x_896.x, x_896.y, x_896.z));
  let x_899 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_898.x, x_898.y, x_898.z, x_899.w);
  let x_901 : vec4<f32> = u_xlat0;
  let x_903 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_901.x, x_901.y, x_901.z, x_901.x));

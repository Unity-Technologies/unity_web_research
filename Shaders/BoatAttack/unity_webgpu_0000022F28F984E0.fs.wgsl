...
fn main_1() {
  ...
  let x_489 : vec3<f32> = sqrt(vec3<f32>(x_487.x, x_487.y, x_487.z));
  let x_490 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_489.x, x_489.y, x_489.z, x_490.w);
  let x_492 : vec2<f32> = vs_TEXCOORD0;
  let x_495 : vec4<f32> = x_26.x_Dithering_Params;
  let x_499 : vec4<f32> = x_26.x_Dithering_Params;
  let x_501 : vec2<f32> = ((x_492 * vec2<f32>(x_495.x, x_495.y)) + vec2<f32>(x_499.z, x_499.w));
  let x_502 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_501.x, x_501.y, x_502.z);
  let x_509 : vec3<f32> = u_xlat1;
  let x_512 : f32 = x_26.x_GlobalMipBias.x;
  let x_513 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_509.x, x_509.y), x_512);
  u_xlat15 = x_513.w;
  let x_515 : f32 = u_xlat15;
  u_xlat15 = ((x_515 * 2.0f) + -1.0f);
  let x_520 : f32 = u_xlat15;
  u_xlatb1.x = (x_520 >= 0.0f);
  let x_524 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_524);
  let x_527 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_527)) + 1.0f);
  ...
  u_xlat15 = (x_536 * x_538);
  let x_540 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_540.x, x_540.y, x_540.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_543 : vec4<f32> = u_xlat0;
  let x_545 : vec3<f32> = log2(vec3<f32>(x_543.x, x_543.y, x_543.z));
  let x_546 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_545.x, x_545.y, x_545.z, x_546.w);
  let x_548 : vec4<f32> = u_xlat2;
  let x_550 : vec3<f32> = (vec3<f32>(x_548.x, x_548.y, x_548.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_561 : vec3<f32> = ((vec3<f32>(x_558.x, x_558.y, x_558.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_562 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_561.x, x_561.y, x_561.z, x_562.w);
  let x_565 : vec4<f32> = u_xlat0;
  let x_567 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_565.x, x_565.y, x_565.z, x_565.x));
  ...
  let x_579 : f32 = x_571;
  u_xlat0.x = x_579;
  let x_582 : bool = u_xlatb0.y;
  if (x_582) {
    let x_587 : f32 = u_xlat1.y;
    x_583 = x_587;
  } else {
  ...
  let x_612 : vec3<f32> = ((vec3<f32>(x_605, x_605, x_605) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_610.x, x_610.y, x_610.z));
  let x_613 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_612.x, x_612.y, x_612.z, x_613.w);
  let x_615 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_615.x, x_615.y, x_615.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_618 : vec4<f32> = u_xlat0;
  let x_620 : vec3<f32> = (vec3<f32>(x_618.x, x_618.y, x_618.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_621 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_620.x, x_620.y, x_620.z, x_621.w);
  let x_623 : vec4<f32> = u_xlat2;
  let x_625 : vec3<f32> = (vec3<f32>(x_623.x, x_623.y, x_623.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_626 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_625.x, x_625.y, x_625.z, x_626.w);
  let x_628 : vec4<f32> = u_xlat2;
  let x_631 : vec3<f32> = log2(abs(vec3<f32>(x_628.x, x_628.y, x_628.z)));
  let x_632 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_631.x, x_631.y, x_631.z, x_632.w);
  let x_634 : vec4<f32> = u_xlat2;
  let x_636 : vec3<f32> = (vec3<f32>(x_634.x, x_634.y, x_634.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_690 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_689.x, x_689.y, x_689.z, x_690.w);
  SV_Target0.w = 1.0f;
  return;
}

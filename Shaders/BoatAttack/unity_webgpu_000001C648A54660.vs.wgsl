...
@group(1) @binding(5) var<uniform> x_250 : VGlobals;
...
fn main_1() {
  ...
  let x_252 : vec4<f32> = x_250.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_246.y, x_246.y, x_246.y, x_246.y) * x_252);
  let x_255 : vec4<f32> = x_250.unity_MatrixVP[0i];
  let x_256 : vec4<f32> = u_xlat0;
  let x_259 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_255 * vec4<f32>(x_256.x, x_256.x, x_256.x, x_256.x)) + x_259);
  let x_262 : vec4<f32> = x_250.unity_MatrixVP[2i];
  let x_263 : vec4<f32> = u_xlat0;
  let x_266 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_262 * vec4<f32>(x_263.z, x_263.z, x_263.z, x_263.z)) + x_266);
  let x_272 : vec4<f32> = u_xlat0;
  let x_274 : vec4<f32> = x_250.unity_MatrixVP[3i];
  gl_Position = (x_272 + x_274);
  return;
}

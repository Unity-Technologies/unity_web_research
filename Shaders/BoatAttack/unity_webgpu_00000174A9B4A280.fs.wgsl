...
fn main_1() {
  ...
  let x_669 : vec3<f32> = sqrt(vec3<f32>(x_667.x, x_667.y, x_667.z));
  let x_670 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_669.x, x_669.y, x_669.z, x_670.w);
  let x_672 : vec2<f32> = vs_TEXCOORD0;
  let x_675 : vec4<f32> = x_26.x_Dithering_Params;
  let x_679 : vec4<f32> = x_26.x_Dithering_Params;
  let x_681 : vec2<f32> = ((x_672 * vec2<f32>(x_675.x, x_675.y)) + vec2<f32>(x_679.z, x_679.w));
  let x_682 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_681.x, x_681.y, x_682.z, x_682.w);
  let x_689 : vec4<f32> = u_xlat1;
  let x_692 : f32 = x_26.x_GlobalMipBias.x;
  let x_693 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_689.x, x_689.y), x_692);
  u_xlat15 = x_693.w;
  let x_695 : f32 = u_xlat15;
  u_xlat15 = ((x_695 * 2.0f) + -1.0f);
  let x_700 : f32 = u_xlat15;
  u_xlatb1.x = (x_700 >= 0.0f);
  let x_704 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_704);
  let x_707 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_707)) + 1.0f);
  ...
  let x_722 : vec3<f32> = (vec3<f32>(x_720.x, x_720.y, x_720.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_723 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_722.x, x_722.y, x_722.z, x_723.w);
  let x_725 : vec4<f32> = u_xlat0;
  let x_727 : vec3<f32> = log2(vec3<f32>(x_725.x, x_725.y, x_725.z));
  let x_728 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_727.x, x_727.y, x_727.z, x_728.w);
  let x_730 : vec4<f32> = u_xlat2;
  let x_732 : vec3<f32> = (vec3<f32>(x_730.x, x_730.y, x_730.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_743 : vec3<f32> = ((vec3<f32>(x_740.x, x_740.y, x_740.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  ...
  let x_804 : vec3<f32> = (vec3<f32>(x_802.x, x_802.y, x_802.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_805 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_804.x, x_804.y, x_804.z, x_805.w);
  let x_807 : vec4<f32> = u_xlat2;
  let x_809 : vec3<f32> = (vec3<f32>(x_807.x, x_807.y, x_807.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_820 : vec3<f32> = (vec3<f32>(x_818.x, x_818.y, x_818.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_821 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_820.x, x_820.y, x_820.z, x_821.w);
  let x_823 : vec4<f32> = u_xlat2;
  let x_825 : vec3<f32> = exp2(vec3<f32>(x_823.x, x_823.y, x_823.z));
  ...
  u_xlatb0 = vec3<bool>(x_830.x, x_830.y, x_830.z);
  let x_833 : bool = u_xlatb0.x;
  if (x_833) {
  ...
  let x_842 : f32 = x_834;
  u_xlat0.x = x_842;
  let x_845 : bool = u_xlatb0.y;
  if (x_845) {
    let x_850 : f32 = u_xlat1.y;
    x_846 = x_850;
  } else {
    let x_853 : f32 = u_xlat2.y;
    x_846 = x_853;
  }
  let x_854 : f32 = x_846;
  u_xlat0.y = x_854;
  let x_857 : bool = u_xlatb0.z;
  if (x_857) {
    let x_862 : f32 = u_xlat1.z;
    x_858 = x_862;
  } else {
    let x_865 : f32 = u_xlat2.z;
    x_858 = x_865;
  }
  let x_866 : f32 = x_858;
  u_xlat0.z = x_866;
  let x_870 : vec4<f32> = u_xlat0;
  let x_873 : vec3<f32> = max(vec3<f32>(x_870.x, x_870.y, x_870.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
;
    ...
    u_xlat47.x = vs_INTERP0.y * unity_MatrixV[1].z;
    u_xlat47.x = unity_MatrixV[0].z * vs_INTERP0.x + u_xlat47.x;
    u_xlat47.x = unity_MatrixV[2].z * vs_INTERP0.z + u_xlat47.x;
    u_xlat47.x = u_xlat47.x + unity_MatrixV[3].z;
    u_xlat47.x = (-u_xlat47.x) + (-_ProjectionParams.y);
    u_xlat47.x = max(u_xlat47.x, 0.0);
    u_xlat47.x = u_xlat47.x * unity_FogParams.x;
    ...
    u_xlat8.xyz = u_xlat22.xyz * (-vec3(u_xlat68)) + (-u_xlat4.xyz);
    u_xlat68 = dot(u_xlat22.xyz, u_xlat4.xyz);
    u_xlat68 = clamp(u_xlat68, 0.0, 1.0);
    u_xlat68 = (-u_xlat68) + 1.0;
    u_xlat68 = u_xlat68 * u_xlat68;
    u_xlat68 = u_xlat68 * u_xlat68;
    u_xlat3.x = (-u_xlat69) * 0.699999988 + 1.70000005;
    ...
    u_xlat3.xw = vec2(u_xlat70) * vec2(u_xlat70) + vec2(-1.0, 1.0);
    u_xlat69 = float(1.0) / u_xlat3.w;
    u_xlat70 = u_xlat28 + -0.0399999991;
    u_xlat68 = u_xlat68 * u_xlat70 + 0.0399999991;
    ...
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu0 ; u_xlatu_loop_1++)
    {
        ...
        u_xlat28 = inversesqrt(u_xlat70);
        u_xlat11.xyz = vec3(u_xlat28) * u_xlat10.xyz;
        ...
        u_xlat73 = dot(_AdditionalLightsSpotDir[int(u_xlatu69)].xyz, u_xlat11.xyz);
        u_xlat73 = u_xlat73 * _AdditionalLightsAttenuation[int(u_xlatu69)].z + _AdditionalLightsAttenuation[int(u_xlatu69)].w;
        u_xlat73 = clamp(u_xlat73, 0.0, 1.0);
        u_xlat73 = u_xlat73 * u_xlat73;
        u_xlat70 = u_xlat70 * u_xlat73;
        u_xlatu73 = u_xlatu69 >> (5u & uint(0x1F));
        u_xlati52 = 1 << (int(u_xlatu69) & int(0x1F));
        u_xlati73 = int(uint(u_xlati52) & floatBitsToUint(_AdditionalLightsCookieEnableBits[int(u_xlatu73)]));
        if(u_xlati73 != 0) {
            u_xlati73 = int(_AdditionalLightsLightTypes[int(u_xlatu69)]);
            u_xlati52 = (u_xlati73 != 0) ? 0 : 1;
            u_xlati74 = int(u_xlatu69) << (2 & int(0x1F));
            if(u_xlati52 != 0) {
                ...
                if(u_xlati73 != 0) {
                    u_xlat56.xy = vs_INTERP0.yy * _AdditionalLightsWorldToLights[(u_xlati74 + 1) / 4][(u_xlati74 + 1) % 4].xy;
                    u_xlat56.xy = _AdditionalLightsWorldToLights[u_xlati74 / 4][u_xlati74 % 4].xy * vs_INTERP0.xx + u_xlat56.xy;
                    u_xlat56.xy = _AdditionalLightsWorldToLights[(u_xlati74 + 2) / 4][(u_xlati74 + 2) % 4].xy * vs_INTERP0.zz + u_xlat56.xy;
                    u_xlat56.xy = u_xlat56.xy + _AdditionalLightsWorldToLights[(u_xlati74 + 3) / 4][(u_xlati74 + 3) % 4].xy;
                    ...
                    u_xlat13.xyz = vec3(u_xlat73) * u_xlat13.xyz;
                    u_xlat73 = dot(abs(u_xlat13.xyz), vec3(1.0, 1.0, 1.0));
                    u_xlat73 = max(u_xlat73, 9.99999997e-07);
                    u_xlat73 = float(1.0) / float(u_xlat73);
                    u_xlat14.xyz = vec3(u_xlat73) * u_xlat13.zxy;
                    ...
                    u_xlat52.xy = u_xlat13.xy * vec2(u_xlat73) + u_xlat52.xy;
            ...
            u_xlat12.xyz = (u_xlatb8.x) ? u_xlat12.xyz : vec3(u_xlat73);
        } else {

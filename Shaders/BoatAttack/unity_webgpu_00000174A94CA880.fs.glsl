...
;
    ...
    u_xlat1.x = u_xlat6.y * _ScaleBiasRt.x + _ScaleBiasRt.y;
    u_xlat6.z = (-u_xlat1.x) + 1.0;
    u_xlat1.x = texture(sampler2D(_ScreenSpaceOcclusionTexture, sampler_ScreenSpaceOcclusionTexture), u_xlat6.xz, _GlobalMipBias.x).x;
    ...
    if(u_xlatb4.x){
        ...
        u_xlat40 = (u_xlatb6.y) ? u_xlat4.w : u_xlat4.x;
        u_xlat4.xyz = (u_xlatb6.x) ? u_xlat4.xyz : vec3(u_xlat40);
    } else {
        u_xlat4.x = float(1.0);
        u_xlat4.y = float(1.0);
        u_xlat4.z = float(1.0);

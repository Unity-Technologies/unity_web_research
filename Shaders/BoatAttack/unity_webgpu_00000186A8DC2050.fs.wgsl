...
fn main_1() {
  ...
  let x_796 : vec3<f32> = ((vec3<f32>(x_790.x, x_790.x, x_790.x) * x_792) + vec3<f32>(x_794.x, x_794.y, x_794.z));
  let x_797 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_796.x, x_796.y, x_796.z, x_797.w);
  let x_799 : vec4<f32> = u_xlat0;
  let x_801 : vec3<f32> = sqrt(vec3<f32>(x_799.x, x_799.y, x_799.z));
  let x_802 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_801.x, x_801.y, x_801.z, x_802.w);
  let x_804 : vec2<f32> = vs_TEXCOORD0;
  let x_807 : vec4<f32> = x_25.x_Dithering_Params;
  let x_811 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_824 : f32 = x_25.x_GlobalMipBias.x;
  let x_825 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_821.x, x_821.y), x_824);
  u_xlat15 = x_825.w;
  let x_827 : f32 = u_xlat15;
  u_xlat15 = ((x_827 * 2.0f) + -1.0f);
  ...
  let x_853 : vec3<f32> = (vec3<f32>(x_851.x, x_851.y, x_851.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_863 : vec3<f32> = (vec3<f32>(x_861.x, x_861.y, x_861.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_874 : vec3<f32> = ((vec3<f32>(x_871.x, x_871.y, x_871.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_875 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_874.x, x_874.y, x_874.z, x_875.w);
  let x_878 : vec4<f32> = u_xlat0;
  let x_880 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_878.x, x_878.y, x_878.z, x_878.x));
  u_xlatb0 = vec3<bool>(x_880.x, x_880.y, x_880.z);
  let x_883 : bool = u_xlatb0.x;
  if (x_883) {
  ...
  let x_892 : f32 = x_884;
  u_xlat0.x = x_892;
  let x_895 : bool = u_xlatb0.y;
  if (x_895) {
    let x_900 : f32 = u_xlat1.y;
    x_896 = x_900;
  } else {
    let x_903 : f32 = u_xlat2.y;
    x_896 = x_903;
  }
  let x_904 : f32 = x_896;
  u_xlat0.y = x_904;
  let x_907 : bool = u_xlatb0.z;
  if (x_907) {
    let x_912 : f32 = u_xlat1.z;
    x_908 = x_912;
  } else {
    let x_915 : f32 = u_xlat2.z;
    x_908 = x_915;
  }
  let x_916 : f32 = x_908;
  u_xlat0.z = x_916;
  ...
  let x_925 : vec3<f32> = ((vec3<f32>(x_918, x_918, x_918) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_923.x, x_923.y, x_923.z));
  let x_926 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_925.x, x_925.y, x_925.z, x_926.w);
  let x_928 : vec4<f32> = u_xlat0;
  let x_930 : vec3<f32> = (vec3<f32>(x_928.x, x_928.y, x_928.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_931 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_930.x, x_930.y, x_930.z, x_931.w);
  let x_933 : vec4<f32> = u_xlat0;
  let x_935 : vec3<f32> = (vec3<f32>(x_933.x, x_933.y, x_933.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_936 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_935.x, x_935.y, x_935.z, x_936.w);
  let x_938 : vec4<f32> = u_xlat2;
  let x_940 : vec3<f32> = (vec3<f32>(x_938.x, x_938.y, x_938.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_941 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_940.x, x_940.y, x_940.z, x_941.w);
  let x_943 : vec4<f32> = u_xlat2;
  let x_946 : vec3<f32> = log2(abs(vec3<f32>(x_943.x, x_943.y, x_943.z)));
  let x_947 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_946.x, x_946.y, x_946.z, x_947.w);
  let x_949 : vec4<f32> = u_xlat2;
  let x_951 : vec3<f32> = (vec3<f32>(x_949.x, x_949.y, x_949.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_952 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_951.x, x_951.y, x_951.z, x_952.w);
  let x_954 : vec4<f32> = u_xlat2;
  let x_956 : vec3<f32> = exp2(vec3<f32>(x_954.x, x_954.y, x_954.z));
  let x_957 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_956.x, x_956.y, x_956.z, x_957.w);
  let x_959 : vec4<f32> = u_xlat0;
  let x_961 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_959.x, x_959.y, x_959.z, x_959.x));
  u_xlatb0 = vec3<bool>(x_961.x, x_961.y, x_961.z);
  ...
  let x_973 : f32 = x_965;
  u_xlat0.x = x_973;
  let x_976 : bool = u_xlatb0.y;
  if (x_976) {
    let x_981 : f32 = u_xlat1.y;
    x_977 = x_981;
  } else {
  ...
  let x_985 : f32 = x_977;
  u_xlat0.y = x_985;
  let x_988 : bool = u_xlatb0.z;
  if (x_988) {
    let x_993 : f32 = u_xlat1.z;
    x_989 = x_993;
  } else {
  ...
  let x_1005 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1004.x, x_1004.y, x_1004.z, x_1005.w);
  SV_Target0.w = 1.0f;
  return;
}

...
vec4 hlslcc_FragCoord = vec4(gl_FragCoord.xyz, 1.0/gl_FragCoord.w);
    ...
    if(u_xlatb4){
        ...
        if(u_xlatb4){
            ...
            u_xlatb46.x = _MainLightShadowParams.y==2.0;
            if(u_xlatb46.x){
    ...
    u_xlatb46.x = 0.0>=vs_TEXCOORD8.z;
    u_xlatb67 = vs_TEXCOORD8.z>=1.0;
    u_xlatb46.x = u_xlatb67 || u_xlatb46.x;
    u_xlat4.x = (u_xlatb46.x) ? 1.0 : u_xlat4.x;
    ...
    u_xlatb46.x = _MainLightCookieTextureFormat!=-1.0;
    if(u_xlatb46.x){
        u_xlat46.xy = vs_TEXCOORD7.yy * _MainLightWorldToLight[1].xy;
        u_xlat46.xy = _MainLightWorldToLight[0].xy * vs_TEXCOORD7.xx + u_xlat46.xy;
        u_xlat46.xy = _MainLightWorldToLight[2].xy * vs_TEXCOORD7.zz + u_xlat46.xy;
        u_xlat46.xy = u_xlat46.xy + _MainLightWorldToLight[3].xy;
        u_xlat46.xy = u_xlat46.xy * vec2(0.5, 0.5) + vec2(0.5, 0.5);
        u_xlat7 = texture(sampler2D(_MainLightCookieTexture, sampler_MainLightCookieTexture), u_xlat46.xy, _GlobalMipBias.x);
        u_xlatb46.xy = equal(vec4(vec4(_MainLightCookieTextureFormat, _MainLightCookieTextureFormat, _MainLightCookieTextureFormat, _MainLightCookieTextureFormat)), vec4(0.0, 1.0, 0.0, 1.0)).xy;
        u_xlat67 = (u_xlatb46.y) ? u_xlat7.w : u_xlat7.x;
        u_xlat7.xyz = (u_xlatb46.x) ? u_xlat7.xyz : vec3(u_xlat67);
    } else {
        u_xlat7.x = float(1.0);
        u_xlat7.y = float(1.0);
        u_xlat7.z = float(1.0);

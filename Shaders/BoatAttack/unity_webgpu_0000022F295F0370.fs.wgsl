...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_618.x, x_618.y, x_618.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_621 : vec4<f32> = u_xlat0;
  let x_624 : vec3<f32> = log2(abs(vec3<f32>(x_621.x, x_621.y, x_621.z)));
  let x_625 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_624.x, x_624.y, x_624.z, x_625.w);
  let x_627 : vec4<f32> = u_xlat2;
  let x_629 : vec3<f32> = (vec3<f32>(x_627.x, x_627.y, x_627.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_640 : vec3<f32> = ((vec3<f32>(x_637.x, x_637.y, x_637.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));

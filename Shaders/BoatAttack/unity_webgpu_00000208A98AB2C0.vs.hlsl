#line 1 "Hidden/TerrainEngine/CameraFacingBillboardTree"
...
float3 zaxis = vertViewVector / distance ;
...
float3 CalcTreeLighting ( float3 viewDir , float3 lightColor , float3 lightDir , float3 albedo , float3 normal , float backContribScale )
{
float backContrib = saturate ( dot ( viewDir , - lightDir ) ) ;
float ndotl = dot ( lightDir , normal ) ;
backContrib = lerp ( saturate ( - ndotl ) , backContrib , _TranslucencyViewDependency ) * backContribScale ;
float3 translucencyColor = backContrib * _TranslucencyColor ;
const float diffuseWrap = 0.8 ;
...
col . rgb = lerp ( ( unity_FogColor ) . rgb , ( col ) . rgb , saturate ( ( input . fogCoord ) . x ) ) ;
return col ;
}

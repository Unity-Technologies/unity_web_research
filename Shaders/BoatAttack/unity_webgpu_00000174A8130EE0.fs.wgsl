...
fn main_1() {
  ...
  let x_907 : vec3<f32> = ((x_901 * vec3<f32>(x_902.x, x_902.x, x_902.x)) + vec3<f32>(x_905.x, x_905.y, x_905.z));
  let x_908 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_907.x, x_907.y, x_907.z, x_908.w);
  let x_910 : vec2<f32> = vs_TEXCOORD0;
  let x_913 : vec4<f32> = x_26.x_Dithering_Params;
  let x_917 : vec4<f32> = x_26.x_Dithering_Params;
  let x_919 : vec2<f32> = ((x_910 * vec2<f32>(x_913.x, x_913.y)) + vec2<f32>(x_917.z, x_917.w));
  let x_920 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_919.x, x_919.y, x_920.z, x_920.w);
  let x_927 : vec4<f32> = u_xlat1;
  let x_930 : f32 = x_26.x_GlobalMipBias.x;
  let x_931 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_927.x, x_927.y), x_930);
  u_xlat18 = x_931.w;
  let x_933 : f32 = u_xlat18;
  u_xlat18 = ((x_933 * 2.0f) + -1.0f);
  let x_937 : f32 = u_xlat18;
  u_xlatb1.x = (x_937 >= 0.0f);
  let x_941 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_941);
  let x_944 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_944)) + 1.0f);
  ...
  u_xlat18 = (x_953 * x_955);
  let x_957 : vec4<f32> = u_xlat0;
  let x_959 : vec3<f32> = (vec3<f32>(x_957.x, x_957.y, x_957.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_970 : vec3<f32> = (vec3<f32>(x_968.x, x_968.y, x_968.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_981 : vec3<f32> = ((vec3<f32>(x_978.x, x_978.y, x_978.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_982 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_981.x, x_981.y, x_981.z, x_982.w);
  let x_985 : vec4<f32> = u_xlat0;
  let x_987 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_985.x, x_985.y, x_985.z, x_985.x));
  ...
  let x_1111 : vec3<f32> = max(vec3<f32>(x_1108.x, x_1108.y, x_1108.z), vec3<f32>(0.0f, 0.0f, 0.0f));

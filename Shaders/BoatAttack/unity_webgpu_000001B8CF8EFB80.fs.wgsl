...
fn main_1() {
  ...
  let x_759 : vec4<f32> = x_22.unity_WorldToLight[2i];
  let x_761 : vec3<f32> = vs_TEXCOORD5;
  let x_764 : vec2<f32> = u_xlat8;
  u_xlat8 = ((vec2<f32>(x_759.x, x_759.y) * vec2<f32>(x_761.z, x_761.z)) + x_764);
  let x_766 : vec2<f32> = u_xlat8;
  let x_768 : vec4<f32> = x_22.unity_WorldToLight[3i];
  u_xlat8 = (x_766 + vec2<f32>(x_768.x, x_768.y));
  ...
  let x_784 : vec4<f32> = x_22.x_LightColor0;
  let x_786 : vec3<f32> = (vec3<f32>(x_780.x, x_780.x, x_780.x) * vec3<f32>(x_784.x, x_784.y, x_784.z));
  let x_787 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_786.x, x_786.y, x_786.z, x_787.w);
  let x_789 : vec4<f32> = u_xlat2;
  let x_793 : vec4<f32> = x_22.x_SpecColor;
  let x_795 : vec3<f32> = (vec3<f32>(x_789.x, x_789.y, x_789.z) * vec3<f32>(x_793.x, x_793.y, x_793.z));
  ...
  u_xlat4 = (vec3<f32>(x_798.x, x_798.x, x_798.x) * vec3<f32>(x_800.x, x_800.y, x_800.z));
  let x_804 : f32 = u_xlat1.w;
  u_xlat14 = max(x_804, 0.0001f);
  let x_807 : vec4<f32> = u_xlat1;
  let x_809 : f32 = u_xlat14;
  let x_811 : vec3<f32> = (vec3<f32>(x_807.x, x_807.y, x_807.z) / vec3<f32>(x_809, x_809, x_809));
  let x_812 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_811.x, x_811.y, x_811.z, x_812.w);
  let x_817 : f32 = u_xlat1.w;
  SV_Target0.w = x_817;
  let x_820 : vec4<f32> = u_xlat2;
  let x_822 : vec4<f32> = u_xlat1;
  let x_824 : vec3<f32> = (vec3<f32>(x_820.x, x_820.y, x_820.z) * vec3<f32>(x_822.x, x_822.y, x_822.z));
  ...
  let x_833 : vec3<f32> = ((vec3<f32>(x_827.x, x_827.y, x_827.z) * vec3<f32>(x_829.x, x_829.x, x_829.x)) + x_832);
  let x_834 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_833.x, x_833.y, x_833.z, x_834.w);
  let x_837 : f32 = vs_TEXCOORD9;
  let x_839 : f32 = x_22.x_ProjectionParams.y;
  u_xlat12 = (x_837 / x_839);
  let x_841 : f32 = u_xlat12;
  u_xlat12 = (-(x_841) + 1.0f);
  let x_844 : f32 = u_xlat12;
  let x_846 : f32 = x_22.x_ProjectionParams.z;
  u_xlat12 = (x_844 * x_846);
  let x_848 : f32 = u_xlat12;
  u_xlat12 = max(x_848, 0.0f);
  let x_850 : f32 = u_xlat12;
  let x_853 : f32 = x_22.unity_FogParams.x;
  u_xlat12 = (x_850 * x_853);
  let x_855 : f32 = u_xlat12;
  let x_856 : f32 = u_xlat12;
  u_xlat12 = (x_855 * -(x_856));
  let x_859 : f32 = u_xlat12;
  u_xlat12 = exp2(x_859);
  let x_861 : vec4<f32> = u_xlat0;
  let x_863 : f32 = u_xlat12;
  let x_865 : vec3<f32> = (vec3<f32>(x_861.x, x_861.y, x_861.z) * vec3<f32>(x_863, x_863, x_863));

...
@group(1) @binding(2) var<uniform> x_66 : VGlobals;
...
fn main_1() {
  ...
  vs_INTERP1 = vec3<f32>(x_137.x, x_137.y, x_137.z);
  let x_139 : vec3<f32> = u_xlat1;
  let x_142 : vec4<f32> = x_66.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_139.y, x_139.y, x_139.y, x_139.y) * x_142);
  let x_145 : vec4<f32> = x_66.unity_MatrixVP[0i];
  let x_146 : vec3<f32> = u_xlat1;
  let x_149 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_145 * vec4<f32>(x_146.x, x_146.x, x_146.x, x_146.x)) + x_149);
  let x_152 : vec4<f32> = x_66.unity_MatrixVP[2i];
  let x_153 : vec3<f32> = u_xlat1;
  let x_156 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_152 * vec4<f32>(x_153.z, x_153.z, x_153.z, x_153.z)) + x_156);
  let x_158 : vec4<f32> = u_xlat0;
  let x_160 : vec4<f32> = x_66.unity_MatrixVP[3i];
  u_xlat0 = (x_158 + x_160);
  let x_167 : f32 = u_xlat0.z;
  gl_Position.z = min(x_167, 1.0f);
  let x_171 : vec4<f32> = u_xlat0;
  let x_172 : vec3<f32> = vec3<f32>(x_171.x, x_171.y, x_171.w);
  let x_175 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_172.x, x_172.y, x_175.z, x_172.z);

...
fn main_1() {
  ...
  let x_690 : vec3<f32> = (vec3<f32>(x_688.x, x_688.y, x_688.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_691 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_690.x, x_690.y, x_690.z, x_691.w);
  let x_693 : vec4<f32> = u_xlat0;
  let x_696 : vec3<f32> = log2(abs(vec3<f32>(x_693.x, x_693.y, x_693.z)));
  let x_697 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_696.x, x_696.y, x_696.z, x_697.w);
  let x_699 : vec4<f32> = u_xlat2;
  let x_701 : vec3<f32> = (vec3<f32>(x_699.x, x_699.y, x_699.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_712 : vec3<f32> = ((vec3<f32>(x_709.x, x_709.y, x_709.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_713 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_712.x, x_712.y, x_712.z, x_713.w);
  let x_716 : vec4<f32> = u_xlat0;
  let x_718 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_716.x, x_716.y, x_716.z, x_716.x));
  u_xlatb0 = vec3<bool>(x_718.x, x_718.y, x_718.z);

...
fn main_1() {
  ...
  u_xlat1 = vec3<f32>(x_567.x, x_567.y, x_568.z);
  ...
  u_xlat1 = (vec3<f32>(x_606.x, x_606.y, x_606.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_609 : vec4<f32> = u_xlat0;
  let x_612 : vec3<f32> = log2(abs(vec3<f32>(x_609.x, x_609.y, x_609.z)));
  let x_613 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_612.x, x_612.y, x_612.z, x_613.w);
  let x_615 : vec4<f32> = u_xlat2;
  let x_617 : vec3<f32> = (vec3<f32>(x_615.x, x_615.y, x_615.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_618 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_617.x, x_617.y, x_617.z, x_618.w);
  let x_620 : vec4<f32> = u_xlat2;
  let x_622 : vec3<f32> = exp2(vec3<f32>(x_620.x, x_620.y, x_620.z));
  let x_623 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_622.x, x_622.y, x_622.z, x_623.w);
  let x_625 : vec4<f32> = u_xlat2;
  let x_628 : vec3<f32> = ((vec3<f32>(x_625.x, x_625.y, x_625.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_629 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_628.x, x_628.y, x_628.z, x_629.w);
  let x_632 : vec4<f32> = u_xlat0;
  let x_634 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_632.x, x_632.y, x_632.z, x_632.x));
  ...
  let x_679 : vec3<f32> = ((vec3<f32>(x_672, x_672, x_672) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_677.x, x_677.y, x_677.z));
  let x_680 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_679.x, x_679.y, x_679.z, x_680.w);
  let x_682 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_682.x, x_682.y, x_682.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_685 : vec4<f32> = u_xlat0;
  let x_687 : vec3<f32> = (vec3<f32>(x_685.x, x_685.y, x_685.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_688 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_687.x, x_687.y, x_687.z, x_688.w);
  let x_690 : vec4<f32> = u_xlat2;
  let x_692 : vec3<f32> = (vec3<f32>(x_690.x, x_690.y, x_690.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_693 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_692.x, x_692.y, x_692.z, x_693.w);
  let x_695 : vec4<f32> = u_xlat2;
  let x_698 : vec3<f32> = log2(abs(vec3<f32>(x_695.x, x_695.y, x_695.z)));
  let x_699 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_698.x, x_698.y, x_698.z, x_699.w);
  let x_701 : vec4<f32> = u_xlat2;
  let x_703 : vec3<f32> = (vec3<f32>(x_701.x, x_701.y, x_701.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_756 : vec3<f32> = max(vec3<f32>(x_753.x, x_753.y, x_753.z), vec3<f32>(0.0f, 0.0f, 0.0f));

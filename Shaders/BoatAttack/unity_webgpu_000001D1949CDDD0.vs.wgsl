...
fn main_1() {
  ...
  vs_TEXCOORD6 = ((vec3<f32>(x_621.x, x_621.y, x_621.z) * vec3<f32>(x_623.z, x_623.z, x_623.z)) + vec3<f32>(x_626.x, x_626.y, x_626.w));
  let x_630 : f32 = u_xlat3.y;
  let x_632 : f32 = u_xlat3.y;
  u_xlat0.x = (x_630 * x_632);
  ...
  u_xlat1 = (vec4<f32>(x_645.y, x_645.w, x_645.z, x_645.x) * x_647);
  let x_650 : vec4<f32> = x_19.unity_SHBr;
  let x_651 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_650, x_651);
  let x_655 : vec4<f32> = x_19.unity_SHBg;
  let x_656 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_655, x_656);
  let x_661 : vec4<f32> = x_19.unity_SHBb;
  let x_662 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_661, x_662);
  let x_669 : vec4<f32> = x_19.unity_SHC;
  let x_671 : vec4<f32> = u_xlat0;
  let x_674 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_669.x, x_669.y, x_669.z) * vec3<f32>(x_671.x, x_671.x, x_671.x)) + vec3<f32>(x_674.x, x_674.y, x_674.z));
  return;
}

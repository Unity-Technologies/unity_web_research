...
fn main_1() {
  ...
  let x_621 : vec4<f32> = x_19.x_EnvMatrix[2i];
  let x_623 : vec4<f32> = u_xlat3;
  let x_626 : vec3<f32> = u_xlat0;
  vs_TEXCOORD7 = ((vec3<f32>(x_621.x, x_621.y, x_621.z) * vec3<f32>(x_623.z, x_623.z, x_623.z)) + x_626);
  let x_628 : vec4<f32> = u_xlat1;
  let x_632 : vec4<f32> = x_19.unity_WorldToLight[1i];
  let x_634 : vec2<f32> = (vec2<f32>(x_628.y, x_628.y) * vec2<f32>(x_632.x, x_632.y));
  let x_635 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_634.x, x_634.y, x_635.z);
  let x_638 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_640 : vec4<f32> = u_xlat1;
  let x_643 : vec3<f32> = u_xlat0;
  let x_645 : vec2<f32> = ((vec2<f32>(x_638.x, x_638.y) * vec2<f32>(x_640.x, x_640.x)) + vec2<f32>(x_643.x, x_643.y));
  let x_646 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_645.x, x_645.y, x_646.z);
  let x_649 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_651 : vec4<f32> = u_xlat1;
  let x_654 : vec3<f32> = u_xlat0;
  let x_656 : vec2<f32> = ((vec2<f32>(x_649.x, x_649.y) * vec2<f32>(x_651.z, x_651.z)) + vec2<f32>(x_654.x, x_654.y));
  let x_657 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_656.x, x_656.y, x_657.z);
  let x_662 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_664 : vec4<f32> = u_xlat1;
  let x_667 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec2<f32>(x_662.x, x_662.y) * vec2<f32>(x_664.w, x_664.w)) + vec2<f32>(x_667.x, x_667.y));
  return;
}

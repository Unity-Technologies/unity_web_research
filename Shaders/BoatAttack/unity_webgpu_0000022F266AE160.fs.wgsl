...
fn main_1() {
  ...
  let x_843 : vec3<f32> = ((x_837 * vec3<f32>(x_838.x, x_838.x, x_838.x)) + vec3<f32>(x_841.x, x_841.y, x_841.z));
  let x_844 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_843.x, x_843.y, x_843.z, x_844.w);
  let x_846 : vec4<f32> = u_xlat0;
  let x_848 : vec3<f32> = (vec3<f32>(x_846.x, x_846.y, x_846.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_849 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_848.x, x_848.y, x_848.z, x_849.w);
  let x_851 : vec4<f32> = u_xlat0;
  let x_854 : vec3<f32> = log2(abs(vec3<f32>(x_851.x, x_851.y, x_851.z)));
  ...
  let x_917 : vec4<f32> = x_26.x_Dithering_Params;
  let x_921 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_934 : f32 = x_26.x_GlobalMipBias.x;
  let x_935 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_931.x, x_931.y), x_934);
  u_xlat18 = x_935.w;
  let x_937 : f32 = u_xlat18;
  u_xlat18 = ((x_937 * 2.0f) + -1.0f);
  let x_941 : f32 = u_xlat18;
  u_xlatb1.x = (x_941 >= 0.0f);
  let x_945 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_945);
  let x_948 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_948)) + 1.0f);
  let x_952 : f32 = u_xlat18;
  u_xlat18 = sqrt(x_952);
  let x_954 : f32 = u_xlat18;
  u_xlat18 = (-(x_954) + 1.0f);
  let x_957 : f32 = u_xlat18;
  let x_959 : f32 = u_xlat1.x;
  u_xlat18 = (x_957 * x_959);
  let x_961 : vec4<f32> = u_xlat0;
  let x_963 : vec3<f32> = (vec3<f32>(x_961.x, x_961.y, x_961.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_974 : vec3<f32> = (vec3<f32>(x_972.x, x_972.y, x_972.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_985 : vec3<f32> = ((vec3<f32>(x_982.x, x_982.y, x_982.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_986 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_985.x, x_985.y, x_985.z, x_986.w);
  let x_988 : vec4<f32> = u_xlat0;
  let x_990 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_988.x, x_988.y, x_988.z, x_988.x));
  u_xlatb0 = vec3<bool>(x_990.x, x_990.y, x_990.z);
  let x_993 : bool = u_xlatb0.x;
  if (x_993) {
  ...
  let x_1035 : vec3<f32> = ((vec3<f32>(x_1028, x_1028, x_1028) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_1033.x, x_1033.y, x_1033.z));
  let x_1036 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1035.x, x_1035.y, x_1035.z, x_1036.w);
  let x_1038 : vec4<f32> = u_xlat0;
  let x_1040 : vec3<f32> = (vec3<f32>(x_1038.x, x_1038.y, x_1038.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1041 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1040.x, x_1040.y, x_1040.z, x_1041.w);
  let x_1043 : vec4<f32> = u_xlat0;
  let x_1045 : vec3<f32> = (vec3<f32>(x_1043.x, x_1043.y, x_1043.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1046 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1045.x, x_1045.y, x_1045.z, x_1046.w);
  let x_1048 : vec4<f32> = u_xlat2;
  let x_1050 : vec3<f32> = (vec3<f32>(x_1048.x, x_1048.y, x_1048.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_1061 : vec3<f32> = (vec3<f32>(x_1059.x, x_1059.y, x_1059.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_1071 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1069.x, x_1069.y, x_1069.z, x_1069.x));
  ...
  let x_1114 : vec3<f32> = max(vec3<f32>(x_1111.x, x_1111.y, x_1111.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1115 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1114.x, x_1114.y, x_1114.z, x_1115.w);
  SV_Target0.w = 1.0f;
  return;
}

...
@group(1) @binding(1) var<uniform> x_62 : VGlobals;
...
fn main_1() {
  ...
  let x_747 : f32 = x_62.x_GroundColor.x;
  let x_749 : f32 = x_62.x_GroundColor.y;
  let x_751 : f32 = x_62.x_GroundColor.z;
  let x_753 : vec3<f32> = u_xlat0;
  let x_755 : vec3<f32> = u_xlat4;
  let x_756 : vec3<f32> = ((vec3<f32>(x_747, x_749, x_751) * x_753) + x_755);
  let x_757 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_756.x, x_756.y, x_756.z, x_757.w);
  let x_761 : vec4<f32> = u_xlat1;
  let x_764 : f32 = x_62.x_Exposure;
  vs_TEXCOORD1 = (vec3<f32>(x_761.x, x_761.y, x_761.z) * vec3<f32>(x_764, x_764, x_764));
  let x_768 : vec4<f32> = x_62.x_MainLightPosition;
  let x_770 : vec3<f32> = u_xlat3;
  u_xlat21 = dot(vec3<f32>(x_768.x, x_768.y, x_768.z), -(x_770));
  let x_773 : f32 = u_xlat21;
  let x_774 : f32 = u_xlat21;
  u_xlat21 = (x_773 * x_774);
  let x_776 : f32 = u_xlat21;
  u_xlat21 = ((x_776 * 0.75f) + 0.75f);
  let x_780 : f32 = u_xlat21;
  let x_782 : vec3<f32> = u_xlat4;
  let x_783 : vec3<f32> = (vec3<f32>(x_780, x_780, x_780) * x_782);
  let x_784 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_783.x, x_783.y, x_783.z, x_784.w);
  let x_787 : vec4<f32> = u_xlat1;
  let x_790 : f32 = x_62.x_Exposure;
  vs_TEXCOORD2 = (vec3<f32>(x_787.x, x_787.y, x_787.z) * vec3<f32>(x_790, x_790, x_790));
  let x_794 : vec4<f32> = x_62.x_MainLightColor;
  let x_797 : vec4<f32> = x_62.x_MainLightColor;
  u_xlat21 = dot(vec3<f32>(x_794.x, x_794.y, x_794.z), vec3<f32>(x_797.x, x_797.y, x_797.z));
  let x_800 : f32 = u_xlat21;
  u_xlat21 = sqrt(x_800);
  let x_802 : f32 = u_xlat21;
  u_xlat21 = max(x_802, 0.25f);
  let x_804 : f32 = u_xlat21;
  u_xlat21 = min(x_804, 1.0f);
  let x_806 : vec3<f32> = u_xlat0;
  u_xlat0 = (x_806 * vec3<f32>(8000.0f, 8000.0f, 8000.0f));
  let x_810 : vec3<f32> = u_xlat0;
  u_xlat0 = clamp(x_810, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_814 : vec3<f32> = u_xlat0;
  let x_816 : vec4<f32> = x_62.x_MainLightColor;
  u_xlat0 = (x_814 * vec3<f32>(x_816.x, x_816.y, x_816.z));
  let x_819 : vec3<f32> = u_xlat0;
  u_xlat0 = (x_819 * vec3<f32>(27.0f, 27.0f, 27.0f));
  let x_824 : vec3<f32> = u_xlat0;
  let x_825 : f32 = u_xlat21;
  vs_TEXCOORD3 = (x_824 / vec3<f32>(x_825, x_825, x_825));
  let x_829 : vec3<f32> = u_xlat3;
  vs_TEXCOORD0 = -(x_829);
  return;
}

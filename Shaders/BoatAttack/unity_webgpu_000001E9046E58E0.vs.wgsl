...
fn main_1() {
  ...
  let x_467 : vec4<f32> = x_394.unity_MatrixVP[3i];
  u_xlat0 = (x_465 + x_467);
  let x_474 : f32 = u_xlat0.z;
  gl_Position.z = min(x_474, 1.0f);
  let x_478 : vec4<f32> = u_xlat0;
  let x_479 : vec3<f32> = vec3<f32>(x_478.x, x_478.y, x_478.w);
  let x_482 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_479.x, x_479.y, x_482.z, x_479.z);
  let x_486 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP2 = x_486;
  return;
}

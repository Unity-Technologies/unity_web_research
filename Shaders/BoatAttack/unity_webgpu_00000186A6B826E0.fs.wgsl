...
fn main_1() {
  ...
  let x_823 : vec2<f32> = vs_TEXCOORD0;
  let x_826 : vec4<f32> = x_25.x_Dithering_Params;
  let x_830 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_843 : f32 = x_25.x_GlobalMipBias.x;
  let x_844 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_840.x, x_840.y), x_843);
  u_xlat15 = x_844.w;
  let x_846 : f32 = u_xlat15;
  u_xlat15 = ((x_846 * 2.0f) + -1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_870.x, x_870.y, x_870.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_881 : vec3<f32> = (vec3<f32>(x_879.x, x_879.y, x_879.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_892 : vec3<f32> = ((vec3<f32>(x_889.x, x_889.y, x_889.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_893 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_892.x, x_892.y, x_892.z, x_893.w);
  let x_896 : vec4<f32> = u_xlat0;
  let x_898 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_896.x, x_896.y, x_896.z, x_896.x));
  u_xlatb0 = vec3<bool>(x_898.x, x_898.y, x_898.z);
  let x_901 : bool = u_xlatb0.x;
  if (x_901) {
  ...
  let x_943 : vec3<f32> = ((vec3<f32>(x_936, x_936, x_936) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_941.x, x_941.y, x_941.z));
  let x_944 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_943.x, x_943.y, x_943.z, x_944.w);
  let x_946 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_946.x, x_946.y, x_946.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_949 : vec4<f32> = u_xlat0;
  let x_951 : vec3<f32> = (vec3<f32>(x_949.x, x_949.y, x_949.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_952 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_951.x, x_951.y, x_951.z, x_952.w);
  let x_954 : vec4<f32> = u_xlat2;
  let x_956 : vec3<f32> = (vec3<f32>(x_954.x, x_954.y, x_954.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_957 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_956.x, x_956.y, x_956.z, x_957.w);
  let x_959 : vec4<f32> = u_xlat2;
  let x_962 : vec3<f32> = log2(abs(vec3<f32>(x_959.x, x_959.y, x_959.z)));
  let x_963 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_962.x, x_962.y, x_962.z, x_963.w);
  let x_965 : vec4<f32> = u_xlat2;
  let x_967 : vec3<f32> = (vec3<f32>(x_965.x, x_965.y, x_965.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_968 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_967.x, x_967.y, x_967.z, x_968.w);
  let x_970 : vec4<f32> = u_xlat2;
  let x_972 : vec3<f32> = exp2(vec3<f32>(x_970.x, x_970.y, x_970.z));
  let x_973 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_972.x, x_972.y, x_972.z, x_973.w);
  let x_975 : vec4<f32> = u_xlat0;
  let x_977 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_975.x, x_975.y, x_975.z, x_975.x));
  u_xlatb0 = vec3<bool>(x_977.x, x_977.y, x_977.z);
  ...
  let x_1020 : vec3<f32> = max(vec3<f32>(x_1017.x, x_1017.y, x_1017.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1021 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1020.x, x_1020.y, x_1020.z, x_1021.w);
  SV_Target0.w = 1.0f;
  return;
}

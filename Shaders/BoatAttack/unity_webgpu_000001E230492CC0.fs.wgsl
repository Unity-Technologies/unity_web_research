...
fn main_1() {
  ...
  let x_698 : vec2<f32> = vs_TEXCOORD0;
  let x_701 : vec4<f32> = x_40.x_Dithering_Params;
  let x_705 : vec4<f32> = x_40.x_Dithering_Params;
  let x_707 : vec2<f32> = ((x_698 * vec2<f32>(x_701.x, x_701.y)) + vec2<f32>(x_705.z, x_705.w));
  let x_708 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_707.x, x_707.y, x_708.z);
  let x_715 : vec3<f32> = u_xlat1;
  let x_718 : f32 = x_40.x_GlobalMipBias.x;
  let x_719 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_715.x, x_715.y), x_718);
  u_xlat15 = x_719.w;
  let x_721 : f32 = u_xlat15;
  u_xlat15 = ((x_721 * 2.0f) + -1.0f);
  let x_724 : f32 = u_xlat15;
  u_xlatb1.x = (x_724 >= 0.0f);
  let x_728 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_728);
  let x_731 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_731)) + 1.0f);
  ...
  u_xlat15 = (x_740 * x_742);
  let x_744 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_744.x, x_744.y, x_744.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_747 : vec4<f32> = u_xlat0;
  let x_750 : vec3<f32> = log2(abs(vec3<f32>(x_747.x, x_747.y, x_747.z)));
  let x_751 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_750.x, x_750.y, x_750.z, x_751.w);
  let x_753 : vec4<f32> = u_xlat2;
  let x_755 : vec3<f32> = (vec3<f32>(x_753.x, x_753.y, x_753.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_766 : vec3<f32> = ((vec3<f32>(x_763.x, x_763.y, x_763.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_767 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_766.x, x_766.y, x_766.z, x_767.w);
  let x_770 : vec4<f32> = u_xlat0;
  let x_772 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_770.x, x_770.y, x_770.z, x_770.x));
  ...
  let x_817 : vec3<f32> = ((vec3<f32>(x_810, x_810, x_810) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_815.x, x_815.y, x_815.z));
  let x_818 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_817.x, x_817.y, x_817.z, x_818.w);
  let x_820 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_820.x, x_820.y, x_820.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));

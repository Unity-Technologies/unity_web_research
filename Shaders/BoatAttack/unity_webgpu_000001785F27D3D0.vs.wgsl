...
fn main_1() {
  ...
  vs_INTERP0 = vec3<f32>(x_428.x, x_428.y, x_428.z);
  let x_434 : vec4<f32> = u_xlat1;
  let x_436 : vec4<f32> = x_44.unity_MatrixVP[3i];
  gl_Position = (x_434 + x_436);
  let x_441 : vec4<f32> = in_TANGENT0;
  let x_444 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_446 : vec3<f32> = (vec3<f32>(x_441.y, x_441.y, x_441.y) * vec3<f32>(x_444.x, x_444.y, x_444.z));
  let x_447 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_446.x, x_446.y, x_446.z, x_447.w);
  let x_450 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_452 : vec4<f32> = in_TANGENT0;
  ...
  let x_461 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_463 : vec4<f32> = in_TANGENT0;
  let x_466 : vec4<f32> = u_xlat0;
  let x_468 : vec3<f32> = ((vec3<f32>(x_461.x, x_461.y, x_461.z) * vec3<f32>(x_463.z, x_463.z, x_463.z)) + vec3<f32>(x_466.x, x_466.y, x_466.z));
  ...
  u_xlat15 = dot(vec3<f32>(x_471.x, x_471.y, x_471.z), vec3<f32>(x_473.x, x_473.y, x_473.z));
  let x_476 : f32 = u_xlat15;
  u_xlat15 = max(x_476, 1.17549435e-37f);
  let x_478 : f32 = u_xlat15;
  u_xlat15 = inverseSqrt(x_478);
  let x_481 : f32 = u_xlat15;
  let x_483 : vec4<f32> = u_xlat0;
  let x_485 : vec3<f32> = (vec3<f32>(x_481, x_481, x_481) * vec3<f32>(x_483.x, x_483.y, x_483.z));
  let x_486 : vec4<f32> = vs_INTERP2;
  vs_INTERP2 = vec4<f32>(x_485.x, x_485.y, x_485.z, x_486.w);
  let x_489 : f32 = in_TANGENT0.w;
  vs_INTERP2.w = x_489;
  let x_494 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP3 = x_494;
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(4) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP1, gl_Position, vs_INTERP2, vs_INTERP3);
}

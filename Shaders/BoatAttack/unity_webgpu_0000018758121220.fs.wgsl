...
fn main_1() {
  ...
  let x_905 : vec4<f32> = x_25.x_Dithering_Params;
  let x_909 : vec4<f32> = x_25.x_Dithering_Params;
  let x_911 : vec2<f32> = ((x_902 * vec2<f32>(x_905.x, x_905.y)) + vec2<f32>(x_909.z, x_909.w));
  let x_912 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_911.x, x_911.y, x_912.z);
  let x_919 : vec3<f32> = u_xlat1;
  let x_922 : f32 = x_25.x_GlobalMipBias.x;
  let x_923 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_919.x, x_919.y), x_922);
  u_xlat15 = x_923.w;
  let x_925 : f32 = u_xlat15;
  u_xlat15 = ((x_925 * 2.0f) + -1.0f);
  let x_929 : f32 = u_xlat15;
  u_xlatb1.x = (x_929 >= 0.0f);
  let x_933 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_933);
  let x_936 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_936)) + 1.0f);
  let x_940 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_940);
  let x_942 : f32 = u_xlat15;
  u_xlat15 = (-(x_942) + 1.0f);
  let x_945 : f32 = u_xlat15;
  let x_947 : f32 = u_xlat1.x;
  u_xlat15 = (x_945 * x_947);
  let x_949 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_949.x, x_949.y, x_949.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_952 : vec4<f32> = u_xlat0;
  let x_955 : vec3<f32> = log2(abs(vec3<f32>(x_952.x, x_952.y, x_952.z)));
  let x_956 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_955.x, x_955.y, x_955.z, x_956.w);
  let x_958 : vec4<f32> = u_xlat2;
  let x_960 : vec3<f32> = (vec3<f32>(x_958.x, x_958.y, x_958.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_961 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_960.x, x_960.y, x_960.z, x_961.w);
  let x_963 : vec4<f32> = u_xlat2;
  let x_965 : vec3<f32> = exp2(vec3<f32>(x_963.x, x_963.y, x_963.z));
  let x_966 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_965.x, x_965.y, x_965.z, x_966.w);
  let x_968 : vec4<f32> = u_xlat2;
  let x_971 : vec3<f32> = ((vec3<f32>(x_968.x, x_968.y, x_968.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_972 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_971.x, x_971.y, x_971.z, x_972.w);
  let x_974 : vec4<f32> = u_xlat0;
  let x_976 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_974.x, x_974.y, x_974.z, x_974.x));
  u_xlatb0 = vec3<bool>(x_976.x, x_976.y, x_976.z);
  let x_979 : bool = u_xlatb0.x;
  if (x_979) {
    let x_984 : f32 = u_xlat1.x;
    x_980 = x_984;
  } else {
    let x_987 : f32 = u_xlat2.x;
    x_980 = x_987;
  }
  let x_988 : f32 = x_980;
  u_xlat0.x = x_988;
  let x_991 : bool = u_xlatb0.y;
  if (x_991) {
    let x_996 : f32 = u_xlat1.y;
    x_992 = x_996;
  } else {
    let x_999 : f32 = u_xlat2.y;
    x_992 = x_999;
  }
  let x_1000 : f32 = x_992;
  u_xlat0.y = x_1000;
  let x_1003 : bool = u_xlatb0.z;
  if (x_1003) {
    let x_1008 : f32 = u_xlat1.z;
    x_1004 = x_1008;
  } else {
  ...
  let x_1021 : vec3<f32> = ((vec3<f32>(x_1014, x_1014, x_1014) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_1019.x, x_1019.y, x_1019.z));
  let x_1022 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1021.x, x_1021.y, x_1021.z, x_1022.w);
  let x_1024 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_1024.x, x_1024.y, x_1024.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1027 : vec4<f32> = u_xlat0;
  let x_1029 : vec3<f32> = (vec3<f32>(x_1027.x, x_1027.y, x_1027.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1030 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1029.x, x_1029.y, x_1029.z, x_1030.w);
  let x_1032 : vec4<f32> = u_xlat2;
  let x_1034 : vec3<f32> = (vec3<f32>(x_1032.x, x_1032.y, x_1032.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_1045 : vec3<f32> = (vec3<f32>(x_1043.x, x_1043.y, x_1043.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_1098 : vec3<f32> = max(vec3<f32>(x_1095.x, x_1095.y, x_1095.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1099 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1098.x, x_1098.y, x_1098.z, x_1099.w);
  SV_Target0.w = 1.0f;
  return;
}

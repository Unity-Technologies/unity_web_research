...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_805.x, x_805.y, x_805.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_816 : vec3<f32> = (vec3<f32>(x_814.x, x_814.y, x_814.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_827 : vec3<f32> = ((vec3<f32>(x_824.x, x_824.y, x_824.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_828 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_827.x, x_827.y, x_827.z, x_828.w);
  let x_831 : vec4<f32> = u_xlat0;
  let x_833 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_831.x, x_831.y, x_831.z, x_831.x));
  u_xlatb0 = vec3<bool>(x_833.x, x_833.y, x_833.z);
  let x_836 : bool = u_xlatb0.x;
  if (x_836) {
  ...
  let x_845 : f32 = x_837;
  u_xlat0.x = x_845;
  let x_848 : bool = u_xlatb0.y;
  if (x_848) {
    let x_853 : f32 = u_xlat1.y;
    x_849 = x_853;
  } else {
    let x_856 : f32 = u_xlat2.y;
    x_849 = x_856;
  }
  let x_857 : f32 = x_849;
  u_xlat0.y = x_857;
  let x_860 : bool = u_xlatb0.z;
  if (x_860) {
    let x_865 : f32 = u_xlat1.z;
    x_861 = x_865;
  } else {
  ...
  let x_874 : vec4<f32> = x_25.x_Dithering_Params;
  let x_878 : vec4<f32> = x_25.x_Dithering_Params;
  let x_880 : vec2<f32> = ((x_871 * vec2<f32>(x_874.x, x_874.y)) + vec2<f32>(x_878.z, x_878.w));
  let x_881 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_880.x, x_880.y, x_881.z);
  let x_888 : vec3<f32> = u_xlat1;
  let x_891 : f32 = x_25.x_GlobalMipBias.x;
  let x_892 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_888.x, x_888.y), x_891);
  u_xlat15 = x_892.w;
  let x_894 : f32 = u_xlat15;
  u_xlat15 = ((x_894 * 2.0f) + -1.0f);
  let x_898 : f32 = u_xlat15;
  u_xlatb1.x = (x_898 >= 0.0f);
  let x_902 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_902);
  let x_905 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_905)) + 1.0f);
  ...
  u_xlat15 = (x_914 * x_916);
  let x_918 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_918.x, x_918.y, x_918.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_921 : vec4<f32> = u_xlat0;
  let x_924 : vec3<f32> = log2(abs(vec3<f32>(x_921.x, x_921.y, x_921.z)));
  let x_925 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_924.x, x_924.y, x_924.z, x_925.w);
  let x_927 : vec4<f32> = u_xlat2;
  let x_929 : vec3<f32> = (vec3<f32>(x_927.x, x_927.y, x_927.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_940 : vec3<f32> = ((vec3<f32>(x_937.x, x_937.y, x_937.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_941 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_940.x, x_940.y, x_940.z, x_941.w);
  let x_943 : vec4<f32> = u_xlat0;
  let x_945 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_943.x, x_943.y, x_943.z, x_943.x));
  ...
  let x_990 : vec3<f32> = ((vec3<f32>(x_983, x_983, x_983) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_988.x, x_988.y, x_988.z));
  let x_991 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_990.x, x_990.y, x_990.z, x_991.w);
  let x_993 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_993.x, x_993.y, x_993.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_996 : vec4<f32> = u_xlat0;
  let x_998 : vec3<f32> = (vec3<f32>(x_996.x, x_996.y, x_996.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_999 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_998.x, x_998.y, x_998.z, x_999.w);
  let x_1001 : vec4<f32> = u_xlat2;
  let x_1003 : vec3<f32> = (vec3<f32>(x_1001.x, x_1001.y, x_1001.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1004 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1003.x, x_1003.y, x_1003.z, x_1004.w);
  let x_1006 : vec4<f32> = u_xlat2;
  let x_1009 : vec3<f32> = log2(abs(vec3<f32>(x_1006.x, x_1006.y, x_1006.z)));
  let x_1010 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1009.x, x_1009.y, x_1009.z, x_1010.w);
  let x_1012 : vec4<f32> = u_xlat2;
  let x_1014 : vec3<f32> = (vec3<f32>(x_1012.x, x_1012.y, x_1012.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_1024 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1022.x, x_1022.y, x_1022.z, x_1022.x));
  u_xlatb0 = vec3<bool>(x_1024.x, x_1024.y, x_1024.z);
  ...
  u_xlat0.x = x_1036;
  let x_1039 : bool = u_xlatb0.y;
  if (x_1039) {
    let x_1044 : f32 = u_xlat1.y;
    x_1040 = x_1044;
  } else {
    let x_1047 : f32 = u_xlat2.y;
    x_1040 = x_1047;
  }
  let x_1048 : f32 = x_1040;
  u_xlat0.y = x_1048;
  let x_1051 : bool = u_xlatb0.z;
  if (x_1051) {
    let x_1056 : f32 = u_xlat1.z;
    x_1052 = x_1056;
  } else {
    let x_1059 : f32 = u_xlat2.z;
    x_1052 = x_1059;
  }
  let x_1060 : f32 = x_1052;
  u_xlat0.z = x_1060;
  let x_1064 : vec4<f32> = u_xlat0;
  let x_1067 : vec3<f32> = max(vec3<f32>(x_1064.x, x_1064.y, x_1064.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  u_xlat0 = (x_583 * vec4<f32>(x_593.x, x_593.y, x_593.z, x_593.w));
  let x_600 : vec4<f32> = u_xlat0;
  let x_607 : vec3<f32> = ((vec3<f32>(x_600.x, x_600.y, x_600.z) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  u_xlat1.x = (x_642 * 0.5f);
  let x_645 : vec4<f32> = u_xlat0;
  let x_648 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_651 : vec4<f32> = u_xlat1;
  let x_653 : vec3<f32> = ((vec3<f32>(x_645.x, x_645.y, x_645.z) * vec3<f32>(x_648.x, x_648.x, x_648.x)) + vec3<f32>(x_651.x, x_651.x, x_651.x));
  let x_654 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_653.x, x_653.y, x_653.z, x_654.w);
  let x_664 : vec4<f32> = u_xlat0;
  let x_666 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_664.x, x_664.y, x_664.z));
  ...
  let x_677 : vec4<f32> = x_55.x_Dithering_Coords;
  ...
  u_xlat1.x = ((x_693 * 2.0f) + -1.0f);
  let x_698 : f32 = u_xlat1.x;
  u_xlat8.x = ((x_698 * 340282346638528859811704183484516925440.0f) + 0.5f);
  let x_704 : f32 = u_xlat8.x;
  u_xlat8.x = clamp(x_704, 0.0f, 1.0f);
  let x_708 : f32 = u_xlat8.x;
  u_xlat8.x = ((x_708 * 2.0f) + -1.0f);
  let x_713 : f32 = u_xlat1.x;
  u_xlat1.x = (-(abs(x_713)) + 1.0f);
  let x_719 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_719);
  let x_723 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_723) + 1.0f);
  let x_728 : f32 = u_xlat1.x;
  let x_730 : f32 = u_xlat8.x;
  u_xlat1.x = (x_728 * x_730);
  let x_733 : vec4<f32> = u_xlat0;
  u_xlat8 = (vec3<f32>(x_733.x, x_733.y, x_733.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_738 : vec4<f32> = u_xlat0;
  let x_742 : vec3<f32> = max(abs(vec3<f32>(x_738.x, x_738.y, x_738.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  ...
  let x_780 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_777.x, x_777.y, x_777.z, x_777.x));
  ...
  let x_825 : vec3<f32> = ((vec3<f32>(x_818.x, x_818.x, x_818.x) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_823.x, x_823.y, x_823.z));
  let x_826 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_825.x, x_825.y, x_825.z, x_826.w);
  let x_828 : vec4<f32> = u_xlat0;
  let x_831 : vec3<f32> = (vec3<f32>(x_828.x, x_828.y, x_828.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_832 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_831.x, x_831.y, x_831.z, x_832.w);
  let x_834 : vec4<f32> = u_xlat0;
  let x_837 : vec3<f32> = (vec3<f32>(x_834.x, x_834.y, x_834.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_838 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_837.x, x_837.y, x_837.z, x_838.w);
  let x_840 : vec4<f32> = u_xlat2;
  let x_843 : vec3<f32> = (vec3<f32>(x_840.x, x_840.y, x_840.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_844 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_843.x, x_843.y, x_843.z, x_844.w);
  let x_846 : vec4<f32> = u_xlat2;
  let x_849 : vec3<f32> = max(abs(vec3<f32>(x_846.x, x_846.y, x_846.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  ...
  let x_860 : vec3<f32> = (vec3<f32>(x_857.x, x_857.y, x_857.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));

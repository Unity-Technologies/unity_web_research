...
fn main_1() {
  ...
  let x_430 : f32 = x_55.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_430);
  let x_432 : bool = u_xlatb0;
  if (x_432) {
    let x_435 : vec4<f32> = u_xlat2;
    u_xlat0 = vec3<f32>(x_435.x, x_435.y, x_435.z);
    let x_437 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_437, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_441 : vec3<f32> = u_xlat0;
    u_xlat2.w = dot(x_441, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

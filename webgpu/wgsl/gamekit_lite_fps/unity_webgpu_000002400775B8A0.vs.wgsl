...
fn main_1() {
  ...
  let x_231 : vec4<f32> = x_17.unity_ObjectToWorld[1i];
  let x_233 : vec3<f32> = (vec3<f32>(x_228.y, x_228.y, x_228.y) * vec3<f32>(x_231.x, x_231.y, x_231.z));
  let x_234 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_233.x, x_233.y, x_233.z, x_234.w);
  let x_237 : vec4<f32> = x_17.unity_ObjectToWorld[0i];
  let x_239 : vec4<f32> = in_POSITION0;
  ...
  let x_248 : vec4<f32> = x_17.unity_ObjectToWorld[2i];
  ...
  let x_261 : vec4<f32> = x_17.unity_ObjectToWorld[3i];
  let x_263 : vec4<f32> = in_POSITION0;
  let x_266 : vec4<f32> = u_xlat0;
  vs_TEXCOORD1 = ((vec3<f32>(x_261.x, x_261.y, x_261.z) * vec3<f32>(x_263.w, x_263.w, x_263.w)) + vec3<f32>(x_266.x, x_266.y, x_266.z));
  return;
}

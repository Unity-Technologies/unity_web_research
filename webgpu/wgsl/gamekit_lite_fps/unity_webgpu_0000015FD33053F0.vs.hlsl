...
float4 FragMotionVectors ( MotionVectorData i ) : COLOR
{
...
return lerp ( float4 ( uvDiff , 0 , 1 ) , 0 , ( float ) _ForceNoMotion ) ;
}
#line 81
...
inline float2 CalculateMotion ( float rawDepth , float2 inUV , float3 inRay )
{
...
float4 FragMotionVectorsCamera ( CamMotionVectors i ) : COLOR
{
float depth = ( tex2D ( _CameraDepthTexture , i . uv ) . r ) ;
return float4 ( CalculateMotion ( depth , i . uv , i . ray ) , 0 , 1 ) ;
}

float4 FragMotionVectorsCameraWithDepth ( CamMotionVectors i , out float outDepth : DEPTH ) : COLOR
{
float depth = ( tex2D ( _CameraDepthTexture , i . uv ) . r ) ;
outDepth = depth ;
return float4 ( CalculateMotion ( depth , i . uv , i . ray ) , 0 , 1 ) ;

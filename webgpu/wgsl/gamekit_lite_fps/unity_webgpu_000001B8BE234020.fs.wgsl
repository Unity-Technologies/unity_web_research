...
fn main_1() {
  ...
  let x_1587 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_1587);
  let x_1589 : bool = u_xlatb0;
  if (x_1589) {
    let x_1592 : vec4<f32> = u_xlat3;
    let x_1593 : vec3<f32> = vec3<f32>(x_1592.x, x_1592.y, x_1592.z);
    let x_1594 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1593.x, x_1593.y, x_1593.z, x_1594.w);
    let x_1596 : vec4<f32> = u_xlat0;
    let x_1600 : vec3<f32> = clamp(vec3<f32>(x_1596.x, x_1596.y, x_1596.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1601 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1600.x, x_1600.y, x_1600.z, x_1601.w);
    let x_1603 : vec4<f32> = u_xlat0;
    u_xlat3.w = dot(vec3<f32>(x_1603.x, x_1603.y, x_1603.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

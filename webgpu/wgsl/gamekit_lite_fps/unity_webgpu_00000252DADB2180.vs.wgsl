...
@group(0) @binding(12) var<uniform> x_18 : VGlobals;
...
fn main_1() {
  ...
  let x_241 : f32 = x_18.x_ProjectionParams.x;
  u_xlat0.y = (x_239 * x_241);
  let x_244 : vec4<f32> = u_xlat0;
  let x_248 : vec3<f32> = (vec3<f32>(x_244.x, x_244.w, x_244.y) * vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_249 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_248.x, x_249.y, x_248.y, x_248.z);
  let x_252 : vec4<f32> = u_xlat0;
  let x_253 : vec2<f32> = vec2<f32>(x_252.z, x_252.w);
  let x_254 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_254.x, x_254.y, x_253.x, x_253.y);
  let x_256 : vec4<f32> = u_xlat1;
  let x_258 : vec4<f32> = u_xlat1;
  let x_260 : vec2<f32> = (vec2<f32>(x_256.z, x_256.z) + vec2<f32>(x_258.x, x_258.w));
  let x_261 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_260.x, x_260.y, x_261.z, x_261.w);
  return;
}

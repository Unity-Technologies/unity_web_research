...
float _Scale ;
#line 41
float3 fold ( float3 st , float3 face )
{
float3 c = min ( max ( st , - float3 ( 1. , 1. , 1. ) ) , float3 ( 1. , 1. , 1. ) ) ;
float3 f = abs ( st - c ) ;
float m = max ( max ( f . x , f . y ) , f . z ) ;
return c - m * face ;
}

float3 gauss ( float d )
{
#line 53
float3 v = float3 ( d , 3. * d , 5. * d ) * _Scale ;
return exp ( - v * v ) ;
}

float4 frag_default ( v2f i )
{
float3 st ;

float3 face = lerp ( float3 ( 0. , 0. , 0. ) , i . uvw . xyz , abs ( i . uvw . xyz ) == float3 ( 1. , 1. , 1. ) ) ;
float3 u = face . zxy * _Texel ;
float3 v = face . yzx * _Texel ;
float4 s = float4 ( i . uvw . xyz * ( float3 ( 1. , 1. , 1. ) - abs ( face ) ) , 0. ) ;
#line 67
float w = 1. / sqrt ( 1. + dot ( s . xyz , s . xyz ) ) ;
float3 C = gauss ( w * w * w ) ;

float4 s1 , s2 , s3 ;
float3 c ;

float3 up1 = fold ( i . uvw . xyz + 1.5 * u , face ) ;
float3 um1 = fold ( i . uvw . xyz - 1.5 * u , face ) ;
float3 up2 = fold ( i . uvw . xyz + 2.5 * u , face ) ;
float3 um2 = fold ( i . uvw . xyz - 2.5 * u , face ) ;

float3 vp1 = fold ( i . uvw . xyz + 1.5 * v , face ) ;
float3 vm1 = fold ( i . uvw . xyz - 1.5 * v , face ) ;
float3 vp2 = fold ( i . uvw . xyz + 2.5 * v , face ) ;
float3 vm2 = fold ( i . uvw . xyz - 2.5 * v , face ) ;
...
st = fold ( st , face ) ;
s3 = max ( float4 ( 0.0 , 0.0 , 0.0 , 0.0 ) , _MainTex . SampleLevel ( sampler_MainTex , st , _Level ) ) ;

st = i . uvw . xyz - 1.5 * u - 2.5 * v ;
st = fold ( st , face ) ;
s2 = max ( float4 ( 0.0 , 0.0 , 0.0 , 0.0 ) , _MainTex . SampleLevel ( sampler_MainTex , st , _Level ) ) ;

st = vm2 - .5 * u ;
s1 = max ( float4 ( 0.0 , 0.0 , 0.0 , 0.0 ) , _MainTex . SampleLevel ( sampler_MainTex , st , _Level ) ) ;
...
float4 frag_loop ( v2f i )
{
float3 face = lerp ( float3 ( 0. , 0. , 0. ) , i . uvw . xyz , abs ( i . uvw . xyz ) == float3 ( 1. , 1. , 1. ) ) ;
float3 u = face . zxy * _Texel ;
float3 v = face . yzx * _Texel ;
float3 s = i . uvw . xyz * ( float3 ( 1. , 1. , 1. ) - abs ( face ) ) ;
#line 275
float w = 1. / sqrt ( 1. + dot ( s , s ) ) ;
float3 C = gauss ( w * w * w ) ;

float3 accuColor = float3 ( 0.0 , 0.0 , 0.0 ) ;
float accuWeight = 0.0 ;
...
{
float xSign = ix * 2 - 1 ;
float fx = xSign * ( float ( x ) + 0.5h ) ;

float3 uvw = i . uvw . xyz + fx * u + fy * v ;
uvw = fold ( uvw , face ) ;
rgb += _MainTex . SampleLevel ( sampler_MainTex , uvw , _Level ) . rgb ;
}

float weight = C [ x ] * C [ y ] ;
...
float4 result = float4 ( accuColor / accuWeight , 1.0 ) ;

return result ;
}

float4 frag_noblur ( v2f i )
{
return _MainTex . SampleLevel ( sampler_MainTex , i . uvw . xyz , _Level ) ;
}

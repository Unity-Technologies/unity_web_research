...
fn main_1() {
  ...
  let x_685 : f32 = x_31.x_PostExposure;
  let x_687 : f32 = x_31.x_PostExposure;
  let x_689 : f32 = x_31.x_PostExposure;
  let x_691 : f32 = x_31.x_PostExposure;
  let x_692 : vec4<f32> = vec4<f32>(x_685, x_687, x_689, x_691);
  u_xlat0 = (x_682 * vec4<f32>(x_692.x, x_692.y, x_692.z, x_692.w));
  let x_699 : vec4<f32> = u_xlat0;
  let x_706 : vec3<f32> = ((vec3<f32>(x_699.z, x_699.x, x_699.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_721 : vec3<f32> = ((vec3<f32>(x_714.x, x_714.y, x_714.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_722 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_721.x, x_721.y, x_721.z, x_722.w);
  let x_724 : vec4<f32> = u_xlat1;
  let x_728 : vec3<f32> = clamp(vec3<f32>(x_724.x, x_724.y, x_724.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_729 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_728.x, x_728.y, x_728.z, x_729.w);
  let x_733 : vec4<f32> = u_xlat1;
  let x_737 : vec3<f32> = x_31.x_Lut2D_Params;
  u_xlat6 = (vec3<f32>(x_733.x, x_733.y, x_733.z) * vec3<f32>(x_737.z, x_737.z, x_737.z));
  let x_741 : f32 = u_xlat6.x;
  u_xlat6.x = floor(x_741);
  let x_745 : vec3<f32> = x_31.x_Lut2D_Params;
  let x_748 : vec2<f32> = (vec2<f32>(x_745.x, x_745.y) * vec2<f32>(0.5f, 0.5f));
  let x_749 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_748.x, x_748.y, x_749.z, x_749.w);
  let x_751 : vec3<f32> = u_xlat6;
  let x_754 : vec3<f32> = x_31.x_Lut2D_Params;
  let x_757 : vec4<f32> = u_xlat2;
  let x_759 : vec2<f32> = ((vec2<f32>(x_751.y, x_751.z) * vec2<f32>(x_754.x, x_754.y)) + vec2<f32>(x_757.x, x_757.y));
  let x_760 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_760.x, x_759.x, x_759.y, x_760.w);
  let x_763 : f32 = u_xlat6.x;
  let x_765 : f32 = x_31.x_Lut2D_Params.y;
  let x_768 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_763 * x_765) + x_768);
  let x_776 : vec4<f32> = u_xlat2;
  let x_778 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_776.x, x_776.z));
  ...
  let x_783 : f32 = x_31.x_Lut2D_Params.y;
  u_xlat4.x = x_783;
  u_xlat4.y = 0.0f;
  let x_787 : vec4<f32> = u_xlat2;
  let x_789 : vec4<f32> = u_xlat4;
  u_xlat11 = (vec2<f32>(x_787.x, x_787.z) + vec2<f32>(x_789.x, x_789.y));
  let x_795 : vec2<f32> = u_xlat11;
  let x_796 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_795);
  ...
  if (x_831) {
    ...
    u_xlat0.w = dot(vec3<f32>(x_845.x, x_845.y, x_845.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

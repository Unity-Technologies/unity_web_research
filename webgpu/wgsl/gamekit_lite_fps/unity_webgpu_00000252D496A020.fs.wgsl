...
fn main_1() {
  ...
  u_xlat1 = ((vec3<f32>(x_1463.z, x_1463.x, x_1463.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_1471 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_1471);
  let x_1473 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_1473 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_1480 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_1480, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_1484 : vec3<f32> = u_xlat1;
  let x_1487 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (x_1484 * vec3<f32>(x_1487.z, x_1487.z, x_1487.z));
  let x_1491 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_1491);
  let x_1495 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1497 : vec2<f32> = (vec2<f32>(x_1495.x, x_1495.y) * vec2<f32>(0.5f, 0.5f));
  let x_1498 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1497.x, x_1497.y, x_1498.z, x_1498.w);
  let x_1500 : vec3<f32> = u_xlat11;
  let x_1503 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1506 : vec4<f32> = u_xlat2;
  let x_1508 : vec2<f32> = ((vec2<f32>(x_1500.y, x_1500.z) * vec2<f32>(x_1503.x, x_1503.y)) + vec2<f32>(x_1506.x, x_1506.y));
  let x_1509 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1509.x, x_1508.x, x_1508.y, x_1509.w);
  let x_1512 : f32 = u_xlat11.x;
  let x_1514 : f32 = x_26.x_Lut2D_Params.y;
  let x_1517 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_1512 * x_1514) + x_1517);
  let x_1525 : vec4<f32> = u_xlat2;
  let x_1527 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_1525.x, x_1525.z));
  let x_1528 : vec3<f32> = vec3<f32>(x_1527.x, x_1527.y, x_1527.z);
  let x_1529 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1528.x, x_1528.y, x_1528.z, x_1529.w);
  let x_1532 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat4.x = x_1532;
  u_xlat4.y = 0.0f;
  let x_1535 : vec4<f32> = u_xlat2;
  let x_1537 : vec4<f32> = u_xlat4;
  u_xlat21 = (vec2<f32>(x_1535.x, x_1535.z) + vec2<f32>(x_1537.x, x_1537.y));
  let x_1543 : vec2<f32> = u_xlat21;
  let x_1544 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_1543);
  let x_1545 : vec3<f32> = vec3<f32>(x_1544.x, x_1544.y, x_1544.z);
  let x_1546 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1545.x, x_1545.y, x_1545.z, x_1546.w);
  let x_1549 : f32 = u_xlat1.x;
  let x_1551 : f32 = x_26.x_Lut2D_Params.z;
  let x_1554 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_1549 * x_1551) + -(x_1554));
  let x_1558 : vec4<f32> = u_xlat3;
  let x_1561 : vec4<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_1558.x, x_1558.y, x_1558.z)) + vec3<f32>(x_1561.x, x_1561.y, x_1561.z));
  let x_1564 : vec3<f32> = u_xlat1;
  let x_1566 : vec3<f32> = u_xlat11;
  let x_1568 : vec4<f32> = u_xlat3;
  let x_1570 : vec3<f32> = ((vec3<f32>(x_1564.x, x_1564.x, x_1564.x) * x_1566) + vec3<f32>(x_1568.x, x_1568.y, x_1568.z));
  let x_1571 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1570.x, x_1570.y, x_1570.z, x_1571.w);
  let x_1575 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1575);
  let x_1577 : bool = u_xlatb1;
  if (x_1577) {
    let x_1580 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_1580.x, x_1580.y, x_1580.z);
    let x_1582 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_1582, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1586 : vec3<f32> = u_xlat1;
    u_xlat0.w = dot(x_1586, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

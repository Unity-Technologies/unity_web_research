...
@group(0) @binding(18) var<uniform> x_18 : VGlobals;
...
fn main_1() {
  ...
  let x_311 : f32 = x_18.x_ProjectionParams.x;
  u_xlat0.y = (x_309 * x_311);
  let x_314 : vec4<f32> = u_xlat0;
  let x_318 : vec3<f32> = (vec3<f32>(x_314.x, x_314.w, x_314.y) * vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_319 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_318.x, x_319.y, x_318.y, x_318.z);
  let x_322 : vec4<f32> = u_xlat0;
  let x_323 : vec2<f32> = vec2<f32>(x_322.z, x_322.w);
  let x_324 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_324.x, x_324.y, x_323.x, x_323.y);
  ...
  vs_TEXCOORD7 = vec4<f32>(x_330.x, x_330.y, x_331.z, x_331.w);
  return;
}

...
layout(set = 0, binding = 1) uniform mediump texture3D _Lut3D;
 struct _HistogramBuffer_type {
	uint[1] value;
};

layout(set = 0, binding = 2, std430) readonly buffer _HistogramBuffer {
...
vec3 u_xlat9;
float u_xlat10;
layout(set = 0, binding = 3) uniform mediump  sampler sampler_Lut3D;
void main()
{

...
fn main_1() {
  ...
  let x_443 : f32 = x_51.x_PostExposure;
  let x_445 : f32 = x_51.x_PostExposure;
  let x_447 : f32 = x_51.x_PostExposure;
  let x_449 : f32 = x_51.x_PostExposure;
  ...
  let x_494 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_496 : vec3<f32> = (vec3<f32>(x_489.x, x_489.y, x_489.z) * vec3<f32>(x_494.y, x_494.y, x_494.y));
  let x_497 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_496.x, x_496.y, x_496.z, x_497.w);
  let x_500 : f32 = x_51.x_Lut3D_Params.x;
  u_xlat22 = (x_500 * 0.5f);
  let x_502 : vec4<f32> = u_xlat1;
  let x_505 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_508 : f32 = u_xlat22;
  let x_510 : vec3<f32> = ((vec3<f32>(x_502.x, x_502.y, x_502.z) * vec3<f32>(x_505.x, x_505.x, x_505.x)) + vec3<f32>(x_508, x_508, x_508));
  let x_511 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_510.x, x_510.y, x_510.z, x_511.w);
  let x_521 : vec4<f32> = u_xlat1;
  let x_523 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_521.x, x_521.y, x_521.z));
  let x_524 : vec3<f32> = vec3<f32>(x_523.x, x_523.y, x_523.z);
  let x_525 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_525.x, x_524.x, x_524.y, x_524.z);
  let x_531 : f32 = x_51.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_531);
  let x_533 : bool = u_xlatb1;
  if (x_533) {
    let x_536 : vec4<f32> = u_xlat0;
    let x_537 : vec3<f32> = vec3<f32>(x_536.y, x_536.z, x_536.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_547.x, x_547.y, x_547.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_553 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_553.y, x_553.z, x_553.w, x_553.x);
  return;
}

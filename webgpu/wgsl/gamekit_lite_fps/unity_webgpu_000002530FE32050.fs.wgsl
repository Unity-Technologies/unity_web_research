...
fn main_1() {
  ...
  let x_831 : vec4<f32> = x_31.x_Dithering_Coords;
  let x_835 : vec4<f32> = x_31.x_Dithering_Coords;
  let x_837 : vec2<f32> = ((vec2<f32>(x_827.x, x_827.y) * vec2<f32>(x_831.x, x_831.y)) + vec2<f32>(x_835.z, x_835.w));
  let x_838 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_837.x, x_837.y, x_838.z, x_838.w);
  let x_845 : vec4<f32> = u_xlat1;
  let x_847 : vec4<f32> = textureSample(x_DitheringTex, sampler_DitheringTex, vec2<f32>(x_845.x, x_845.y));
  u_xlat1.x = x_847.w;
  let x_851 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_851 * 2.0f) + -1.0f);
  let x_858 : f32 = u_xlat1.x;
  u_xlat7.x = ((x_858 * 340282346638528859811704183484516925440.0f) + 0.5f);
  let x_864 : f32 = u_xlat7.x;
  u_xlat7.x = clamp(x_864, 0.0f, 1.0f);
  let x_868 : f32 = u_xlat7.x;
  u_xlat7.x = ((x_868 * 2.0f) + -1.0f);
  let x_873 : f32 = u_xlat1.x;
  u_xlat1.x = (-(abs(x_873)) + 1.0f);
  let x_879 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_879);
  let x_883 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_883) + 1.0f);
  let x_888 : f32 = u_xlat1.x;
  let x_890 : f32 = u_xlat7.x;
  u_xlat1.x = (x_888 * x_890);
  let x_893 : vec4<f32> = u_xlat0;
  u_xlat7 = (vec3<f32>(x_893.x, x_893.y, x_893.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_898 : vec4<f32> = u_xlat0;
  let x_903 : vec3<f32> = max(abs(vec3<f32>(x_898.x, x_898.y, x_898.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  ...
  let x_915 : vec3<f32> = (vec3<f32>(x_911.x, x_911.y, x_911.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_930 : vec3<f32> = ((vec3<f32>(x_923.x, x_923.y, x_923.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_931 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_930.x, x_930.y, x_930.z, x_931.w);
  let x_935 : vec4<f32> = u_xlat0;
  let x_938 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_935.x, x_935.y, x_935.z, x_935.x));
  u_xlatb0 = vec3<bool>(x_938.x, x_938.y, x_938.z);
  let x_941 : bool = u_xlatb0.x;
  if (x_941) {
  ...
  let x_950 : f32 = x_942;
  u_xlat0.x = x_950;
  let x_953 : bool = u_xlatb0.y;
  if (x_953) {
    let x_958 : f32 = u_xlat7.y;
    x_954 = x_958;
  } else {
    let x_961 : f32 = u_xlat2.y;
    x_954 = x_961;
  }
  let x_962 : f32 = x_954;
  u_xlat0.y = x_962;
  ...
  let x_991 : vec3<f32> = (vec3<f32>(x_987.x, x_987.y, x_987.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_992 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_991.x, x_991.y, x_991.z, x_992.w);
  let x_994 : vec4<f32> = u_xlat0;
  let x_998 : vec3<f32> = (vec3<f32>(x_994.x, x_994.y, x_994.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_999 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_998.x, x_998.y, x_998.z, x_999.w);
  let x_1001 : vec4<f32> = u_xlat2;
  let x_1005 : vec3<f32> = (vec3<f32>(x_1001.x, x_1001.y, x_1001.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1006 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1005.x, x_1005.y, x_1005.z, x_1006.w);
  let x_1008 : vec4<f32> = u_xlat2;
  let x_1011 : vec3<f32> = max(abs(vec3<f32>(x_1008.x, x_1008.y, x_1008.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  let x_1012 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1011.x, x_1011.y, x_1011.z, x_1012.w);
  let x_1014 : vec4<f32> = u_xlat2;
  let x_1016 : vec3<f32> = log2(vec3<f32>(x_1014.x, x_1014.y, x_1014.z));
  let x_1017 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1016.x, x_1016.y, x_1016.z, x_1017.w);
  let x_1019 : vec4<f32> = u_xlat2;
  let x_1023 : vec3<f32> = (vec3<f32>(x_1019.x, x_1019.y, x_1019.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_1024 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1023.x, x_1023.y, x_1023.z, x_1024.w);
  let x_1026 : vec4<f32> = u_xlat2;
  let x_1028 : vec3<f32> = exp2(vec3<f32>(x_1026.x, x_1026.y, x_1026.z));
  let x_1029 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1028.x, x_1028.y, x_1028.z, x_1029.w);
  let x_1033 : vec4<f32> = u_xlat0;
  let x_1035 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1033.x, x_1033.y, x_1033.z, x_1033.x));
  u_xlatb0 = vec3<bool>(x_1035.x, x_1035.y, x_1035.z);
  let x_1040 : bool = u_xlatb0.x;
  if (x_1040) {
  ...
  SV_Target0.x = x_1049;
  let x_1053 : bool = u_xlatb0.y;
  if (x_1053) {
    let x_1058 : f32 = u_xlat1.y;
    x_1054 = x_1058;
  } else {
    let x_1061 : f32 = u_xlat2.y;
    x_1054 = x_1061;
  }
  let x_1062 : f32 = x_1054;
  SV_Target0.y = x_1062;
  let x_1065 : bool = u_xlatb0.z;
  if (x_1065) {
    let x_1070 : f32 = u_xlat1.z;
    x_1066 = x_1070;
  } else {
    let x_1073 : f32 = u_xlat2.z;
    x_1066 = x_1073;
  }

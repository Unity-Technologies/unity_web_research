...
fn main_1() {
  ...
  let x_298 : vec3<f32> = (vec3<f32>(x_295.x, x_295.y, x_295.z) * x_297);
  let x_299 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_298.x, x_298.y, x_298.z, x_299.w);
  let x_302 : vec3<f32> = u_xlat0;
  let x_303 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_302.x, x_302.y, x_302.z, x_303.w);
  SV_Target0.w = 1.0f;
  let x_309 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_309;
  let x_312 : vec3<f32> = vs_TEXCOORD1;
  let x_315 : vec3<f32> = ((x_312 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_316 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_315.x, x_315.y, x_315.z, x_316.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}

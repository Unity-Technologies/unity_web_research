...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_494.w, x_494.x, x_494.y, x_494.z) + vec4<f32>(x_496.w, x_496.x, x_496.y, x_496.z));
  let x_503 : f32 = x_51.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_503);
  let x_505 : bool = u_xlatb1;
  if (x_505) {
    let x_508 : vec4<f32> = u_xlat0;
    let x_509 : vec3<f32> = vec3<f32>(x_508.y, x_508.z, x_508.w);
    let x_510 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_509.x, x_509.y, x_509.z, x_510.w);
    let x_512 : vec4<f32> = u_xlat1;
    let x_516 : vec3<f32> = clamp(vec3<f32>(x_512.x, x_512.y, x_512.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_517 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_516.x, x_516.y, x_516.z, x_517.w);
    let x_519 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_519.x, x_519.y, x_519.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_529 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_529.y, x_529.z, x_529.w, x_529.x);
  return;
}

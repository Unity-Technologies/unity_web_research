...
fn main_1() {
  ...
  let x_1374 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_1374);
  let x_1376 : bool = u_xlatb0;
  if (x_1376) {
    let x_1379 : vec4<f32> = u_xlat2;
    let x_1380 : vec3<f32> = vec3<f32>(x_1379.x, x_1379.y, x_1379.z);
    let x_1381 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1380.x, x_1380.y, x_1380.z, x_1381.w);
    let x_1383 : vec4<f32> = u_xlat0;
    let x_1387 : vec3<f32> = clamp(vec3<f32>(x_1383.x, x_1383.y, x_1383.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1388 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1387.x, x_1387.y, x_1387.z, x_1388.w);
    let x_1390 : vec4<f32> = u_xlat0;
    u_xlat2.w = dot(vec3<f32>(x_1390.x, x_1390.y, x_1390.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

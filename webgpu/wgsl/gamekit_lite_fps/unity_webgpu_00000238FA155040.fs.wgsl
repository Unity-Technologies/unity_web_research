...
fn main_1() {
  ...
  u_xlat4.z = x_1098;
  let x_1102 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0.x = (0.5f < x_1102);
  let x_1106 : bool = u_xlatb0.x;
  if (x_1106) {
    let x_1109 : vec4<f32> = u_xlat4;
    u_xlat0 = vec3<f32>(x_1109.x, x_1109.y, x_1109.z);
    let x_1111 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_1111, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1115 : vec3<f32> = u_xlat0;
    u_xlat4.w = dot(x_1115, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

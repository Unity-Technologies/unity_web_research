...
fn main_1() {
  ...
  u_xlat0 = (x_742 + x_743);
  let x_746 : vec2<f32> = vs_TEXCOORD1;
  let x_749 : vec4<f32> = x_26.x_Grain_Params2;
  ...
  u_xlat31 = dot(vec3<f32>(x_780.x, x_780.y, x_780.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_787 : f32 = u_xlat31;
  u_xlat31 = sqrt(x_787);
  let x_791 : f32 = x_26.x_Grain_Params1.x;
  let x_792 : f32 = u_xlat31;
  u_xlat31 = ((x_791 * -(x_792)) + 1.0f);
  let x_796 : vec4<f32> = u_xlat0;
  let x_798 : vec4<f32> = u_xlat1;
  let x_800 : vec3<f32> = (vec3<f32>(x_796.x, x_796.y, x_796.z) * vec3<f32>(x_798.x, x_798.y, x_798.z));
  ...
  let x_812 : vec3<f32> = (vec3<f32>(x_803.x, x_803.y, x_803.z) * vec3<f32>(x_806, x_808, x_810));
  ...
  let x_822 : vec3<f32> = ((vec3<f32>(x_815.x, x_815.y, x_815.z) * vec3<f32>(x_817, x_817, x_817)) + vec3<f32>(x_820.x, x_820.y, x_820.z));
  ...
  let x_834 : f32 = x_26.x_PostExposure;
  let x_835 : vec4<f32> = vec4<f32>(x_828, x_830, x_832, x_834);
  u_xlat0 = (x_825 * vec4<f32>(x_835.x, x_835.y, x_835.z, x_835.w));
  let x_842 : vec4<f32> = u_xlat0;
  let x_849 : vec3<f32> = ((vec3<f32>(x_842.z, x_842.x, x_842.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_850 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_849.x, x_849.y, x_849.z, x_850.w);
  let x_852 : vec4<f32> = u_xlat1;
  let x_854 : vec3<f32> = log2(vec3<f32>(x_852.x, x_852.y, x_852.z));
  let x_855 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_854.x, x_854.y, x_854.z, x_855.w);
  let x_857 : vec4<f32> = u_xlat1;
  let x_864 : vec3<f32> = ((vec3<f32>(x_857.x, x_857.y, x_857.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_865 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_864.x, x_864.y, x_864.z, x_865.w);
  let x_867 : vec4<f32> = u_xlat1;
  let x_871 : vec3<f32> = clamp(vec3<f32>(x_867.x, x_867.y, x_867.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  ...
  u_xlat11 = (vec3<f32>(x_875.x, x_875.y, x_875.z) * vec3<f32>(x_879.z, x_879.z, x_879.z));
  let x_883 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_883);
  ...
  u_xlat2 = vec3<f32>(x_889.x, x_889.y, x_890.z);
  ...
  u_xlat3 = vec4<f32>(x_901.x, x_900.x, x_900.y, x_901.w);
  let x_904 : f32 = u_xlat11.x;
  let x_906 : f32 = x_26.x_Lut2D_Params.y;
  let x_909 : f32 = u_xlat3.y;
  u_xlat3.x = ((x_904 * x_906) + x_909);
  ...
  u_xlat21 = (x_926 + vec2<f32>(x_927.x, x_927.z));
  let x_933 : vec2<f32> = u_xlat21;
  let x_934 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_933);
  u_xlat2 = vec3<f32>(x_934.x, x_934.y, x_934.z);
  let x_937 : f32 = u_xlat1.x;
  let x_939 : f32 = x_26.x_Lut2D_Params.z;
  let x_942 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_937 * x_939) + -(x_942));
  let x_946 : vec4<f32> = u_xlat4;
  let x_949 : vec3<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_946.x, x_946.y, x_946.z)) + x_949);
  let x_951 : vec4<f32> = u_xlat1;
  let x_953 : vec3<f32> = u_xlat11;
  let x_955 : vec4<f32> = u_xlat4;
  let x_957 : vec3<f32> = ((vec3<f32>(x_951.x, x_951.x, x_951.x) * x_953) + vec3<f32>(x_955.x, x_955.y, x_955.z));
  let x_958 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_957.x, x_957.y, x_957.z, x_958.w);
  let x_962 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_962);
  let x_964 : bool = u_xlatb1;
  if (x_964) {
    let x_967 : vec4<f32> = u_xlat0;
    let x_968 : vec3<f32> = vec3<f32>(x_967.x, x_967.y, x_967.z);
    let x_969 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_968.x, x_968.y, x_968.z, x_969.w);
    let x_971 : vec4<f32> = u_xlat1;
    let x_975 : vec3<f32> = clamp(vec3<f32>(x_971.x, x_971.y, x_971.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_976 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_975.x, x_975.y, x_975.z, x_976.w);
    let x_978 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_978.x, x_978.y, x_978.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

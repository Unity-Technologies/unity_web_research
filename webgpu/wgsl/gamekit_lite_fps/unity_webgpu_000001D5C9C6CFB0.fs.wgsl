...
fn main_1() {
  ...
  let x_551 : f32 = x_55.x_PostExposure;
  let x_553 : f32 = x_55.x_PostExposure;
  let x_555 : f32 = x_55.x_PostExposure;
  let x_557 : f32 = x_55.x_PostExposure;
  let x_558 : vec4<f32> = vec4<f32>(x_551, x_553, x_555, x_557);
  u_xlat0 = (vec4<f32>(x_547.w, x_547.x, x_547.y, x_547.z) * vec4<f32>(x_558.x, x_558.y, x_558.z, x_558.w));
  let x_565 : vec4<f32> = u_xlat0;
  let x_572 : vec3<f32> = ((vec3<f32>(x_565.y, x_565.z, x_565.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_587 : vec3<f32> = ((vec3<f32>(x_580.x, x_580.y, x_580.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_588 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_587.x, x_587.y, x_587.z, x_588.w);
  let x_590 : vec4<f32> = u_xlat1;
  let x_594 : vec3<f32> = clamp(vec3<f32>(x_590.x, x_590.y, x_590.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_595 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_594.x, x_594.y, x_594.z, x_595.w);
  let x_597 : vec4<f32> = u_xlat1;
  let x_602 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_604 : vec3<f32> = (vec3<f32>(x_597.x, x_597.y, x_597.z) * vec3<f32>(x_602.y, x_602.y, x_602.y));
  let x_605 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_604.x, x_604.y, x_604.z, x_605.w);
  let x_608 : f32 = x_55.x_Lut3D_Params.x;
  u_xlat22 = (x_608 * 0.5f);
  let x_610 : vec4<f32> = u_xlat1;
  let x_613 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_616 : f32 = u_xlat22;
  let x_618 : vec3<f32> = ((vec3<f32>(x_610.x, x_610.y, x_610.z) * vec3<f32>(x_613.x, x_613.x, x_613.x)) + vec3<f32>(x_616, x_616, x_616));
  let x_619 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_618.x, x_618.y, x_618.z, x_619.w);
  let x_629 : vec4<f32> = u_xlat1;
  let x_631 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_629.x, x_629.y, x_629.z));
  let x_632 : vec3<f32> = vec3<f32>(x_631.x, x_631.y, x_631.z);
  let x_633 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_633.x, x_632.x, x_632.y, x_632.z);
  let x_640 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_640);
  let x_642 : bool = u_xlatb1;
  if (x_642) {
    let x_645 : vec4<f32> = u_xlat0;
    let x_646 : vec3<f32> = vec3<f32>(x_645.y, x_645.z, x_645.w);
    let x_647 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_646.x, x_646.y, x_646.z, x_647.w);
    let x_649 : vec4<f32> = u_xlat1;
    let x_653 : vec3<f32> = clamp(vec3<f32>(x_649.x, x_649.y, x_649.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_654 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_653.x, x_653.y, x_653.z, x_654.w);
    let x_656 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_656.x, x_656.y, x_656.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_662 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_662.y, x_662.z, x_662.w, x_662.x);
  return;
}

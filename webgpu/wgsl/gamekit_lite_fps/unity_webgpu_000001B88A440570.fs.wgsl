...
fn main_1() {
  ...
  let x_357 : vec4<f32> = textureSample(x_EmissionMap, sampler_EmissionMap, vec2<f32>(x_355.x, x_355.y));
  ...
  let x_365 : vec4<f32> = x_36.x_EmissionColor;
  let x_367 : vec3<f32> = (vec3<f32>(x_361.x, x_361.y, x_361.z) * vec3<f32>(x_365.x, x_365.y, x_365.z));
  ...
  let x_378 : vec3<f32> = ((vec3<f32>(x_371.x, x_371.y, x_371.z) * vec3<f32>(x_373.x, x_373.y, x_373.z)) + vec3<f32>(x_376.x, x_376.y, x_376.z));
  let x_379 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_378.x, x_378.y, x_378.z, x_379.w);
  let x_382 : vec4<f32> = u_xlat1;
  let x_386 : vec3<f32> = ((vec3<f32>(x_382.x, x_382.y, x_382.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_387 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_386.x, x_386.y, x_386.z, x_387.w);
  let x_390 : vec4<f32> = u_xlat0;
  SV_Target0 = x_390;
  let x_393 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_393;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

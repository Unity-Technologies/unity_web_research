...
void main()
{
    ...
    if(u_xlatb12){
        ...
        u_xlatb2.xyz = lessThan(u_xlat1.xyzx, _CustomToneCurve.yyyy).xyz;
        if(u_xlatb2.x){
            u_xlat2.xw = _ToeSegmentA.zw;
            u_xlat12 = _ToeSegmentA.x;
            u_xlat13 = _ToeSegmentA.y;
            u_xlat3.xy = _ToeSegmentB.xy;
        } else {
            u_xlatb1.x = u_xlat1.x<_CustomToneCurve.z;
            if(u_xlatb1.x){
                u_xlat2.xw = _MidSegmentA.zw;
                u_xlat12 = _MidSegmentA.x;
                u_xlat13 = _MidSegmentA.y;
                u_xlat3.xy = _MidSegmentB.xy;
            } else {
                u_xlat2.xw = _ShoSegmentA.zw;
                u_xlat12 = _ShoSegmentA.x;
                u_xlat13 = _ShoSegmentA.y;
                u_xlat3.xy = _ShoSegmentB.xy;
            }
        }
        u_xlat0.x = u_xlat0.x * _CustomToneCurve.x + (-u_xlat12);
        u_xlat0.x = u_xlat2.x * u_xlat0.x;
        u_xlatb12 = 0.0<u_xlat0.x;
        u_xlat0.x = log2(u_xlat0.x);
        u_xlat0.x = u_xlat0.x * u_xlat3.y;
        u_xlat0.x = u_xlat0.x * 0.693147182 + u_xlat3.x;
        u_xlat0.x = u_xlat0.x * 1.44269502;
        u_xlat0.x = exp2(u_xlat0.x);
        u_xlat0.x = u_xlatb12 ? u_xlat0.x : float(0.0);
        u_xlat3.x = u_xlat0.x * u_xlat2.w + u_xlat13;
        if(u_xlatb2.y){
            u_xlat0.xw = _ToeSegmentA.zw;
            u_xlat1.xw = _ToeSegmentA.xy;
            u_xlat2.xy = _ToeSegmentB.xy;
        } else {
            u_xlatb5 = u_xlat1.y<_CustomToneCurve.z;
            if(u_xlatb5){
                u_xlat0.xw = _MidSegmentA.zw;
                u_xlat1.xw = _MidSegmentA.xy;
                u_xlat2.xy = _MidSegmentB.xy;
            } else {
                u_xlat0.xw = _ShoSegmentA.zw;
                u_xlat1.xw = _ShoSegmentA.xy;
                u_xlat2.xy = _ShoSegmentB.xy;
            }
        }
        u_xlat4.x = u_xlat0.y * _CustomToneCurve.x + (-u_xlat1.x);
        u_xlat0.x = u_xlat0.x * u_xlat4.x;
        u_xlatb4 = 0.0<u_xlat0.x;
        u_xlat0.x = log2(u_xlat0.x);
        u_xlat0.x = u_xlat0.x * u_xlat2.y;
        u_xlat0.x = u_xlat0.x * 0.693147182 + u_xlat2.x;
        u_xlat0.x = u_xlat0.x * 1.44269502;
        u_xlat0.x = exp2(u_xlat0.x);
        u_xlat0.x = u_xlatb4 ? u_xlat0.x : float(0.0);
        u_xlat3.y = u_xlat0.x * u_xlat0.w + u_xlat1.w;
        if(u_xlatb2.z){
            u_xlat0.xyw = _ToeSegmentA.zwx;
            u_xlat1.x = _ToeSegmentA.y;
            u_xlat5.xz = _ToeSegmentB.xy;
        } else {
            u_xlatb9 = u_xlat1.z<_CustomToneCurve.z;
            if(u_xlatb9){
                u_xlat0.xyw = _MidSegmentA.zwx;
                u_xlat1.x = _MidSegmentA.y;
                u_xlat5.xz = _MidSegmentB.xy;
            } else {
                u_xlat0.xyw = _ShoSegmentA.zwx;
                u_xlat1.x = _ShoSegmentA.y;
                u_xlat5.xz = _ShoSegmentB.xy;
            }
        }
        u_xlat8 = u_xlat0.z * _CustomToneCurve.x + (-u_xlat0.w);
        u_xlat0.x = u_xlat0.x * u_xlat8;
        u_xlatb8 = 0.0<u_xlat0.x;
        u_xlat0.x = log2(u_xlat0.x);
        u_xlat0.x = u_xlat0.x * u_xlat5.z;
        u_xlat0.x = u_xlat0.x * 0.693147182 + u_xlat5.x;
        u_xlat0.x = u_xlat0.x * 1.44269502;
        u_xlat0.x = exp2(u_xlat0.x);
        u_xlat0.x = u_xlatb8 ? u_xlat0.x : float(0.0);
        u_xlat3.z = u_xlat0.x * u_xlat0.y + u_xlat1.x;

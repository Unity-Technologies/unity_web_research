...
fn main_1() {
  ...
  let x_210 : f32 = x_55.x_PostExposure;
  let x_212 : f32 = x_55.x_PostExposure;
  let x_214 : f32 = x_55.x_PostExposure;
  let x_216 : f32 = x_55.x_PostExposure;
  let x_217 : vec4<f32> = vec4<f32>(x_210, x_212, x_214, x_216);
  u_xlat0 = (vec4<f32>(x_206.w, x_206.x, x_206.y, x_206.z) * vec4<f32>(x_217.x, x_217.y, x_217.z, x_217.w));
  let x_224 : vec4<f32> = u_xlat0;
  let x_231 : vec3<f32> = ((vec3<f32>(x_224.y, x_224.z, x_224.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_261 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_263 : vec3<f32> = (vec3<f32>(x_256.x, x_256.y, x_256.z) * vec3<f32>(x_261.y, x_261.y, x_261.y));
  let x_264 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_263.x, x_263.y, x_263.z, x_264.w);
  let x_268 : f32 = x_55.x_Lut3D_Params.x;
  u_xlat22 = (x_268 * 0.5f);
  let x_270 : vec4<f32> = u_xlat1;
  let x_273 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_276 : f32 = u_xlat22;
  let x_278 : vec3<f32> = ((vec3<f32>(x_270.x, x_270.y, x_270.z) * vec3<f32>(x_273.x, x_273.x, x_273.x)) + vec3<f32>(x_276, x_276, x_276));
  let x_279 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_278.x, x_278.y, x_278.z, x_279.w);
  let x_289 : vec4<f32> = u_xlat1;
  let x_291 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_289.x, x_289.y, x_289.z));
  let x_292 : vec3<f32> = vec3<f32>(x_291.x, x_291.y, x_291.z);
  let x_293 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_293.x, x_292.x, x_292.y, x_292.z);
  let x_300 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_300);
  let x_302 : bool = u_xlatb1;
  if (x_302) {
    let x_305 : vec4<f32> = u_xlat0;
    let x_306 : vec3<f32> = vec3<f32>(x_305.y, x_305.z, x_305.w);
    let x_307 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_306.x, x_306.y, x_306.z, x_307.w);
    let x_309 : vec4<f32> = u_xlat1;
    let x_313 : vec3<f32> = clamp(vec3<f32>(x_309.x, x_309.y, x_309.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_314 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_313.x, x_313.y, x_313.z, x_314.w);
    let x_316 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_316.x, x_316.y, x_316.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_326 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_326.y, x_326.z, x_326.w, x_326.x);
  return;
}

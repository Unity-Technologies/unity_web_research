...
fn main_1() {
  ...
  let x_262 : vec2<f32> = in_TEXCOORD0;
  let x_264 : vec4<f32> = x_80.x_MainTex_ST;
  let x_268 : vec4<f32> = x_80.x_MainTex_ST;
  vs_TEXCOORD1 = ((x_262 * vec2<f32>(x_264.x, x_264.y)) + vec2<f32>(x_268.z, x_268.w));
  let x_274 : vec4<f32> = in_COLOR0;
  vs_TEXCOORD3 = x_274;
  let x_275 : vec4<f32> = u_xlat0;
  let x_276 : vec3<f32> = vec3<f32>(x_275.x, x_275.y, x_275.w);
  let x_278 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_276.x, x_276.y, x_278.z, x_276.z);
  return;
}

...
fn main_1() {
  ...
  let x_912 : vec3<f32> = ((vec3<f32>(x_905.x, x_905.y, x_905.z) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_942 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_944 : vec3<f32> = (vec3<f32>(x_937.x, x_937.y, x_937.z) * vec3<f32>(x_942.y, x_942.y, x_942.y));
  let x_945 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_944.x, x_944.y, x_944.z, x_945.w);
  let x_948 : f32 = x_55.x_Lut3D_Params.x;
  u_xlat1.x = (x_948 * 0.5f);
  let x_951 : vec4<f32> = u_xlat0;
  let x_954 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_957 : vec4<f32> = u_xlat1;
  let x_959 : vec3<f32> = ((vec3<f32>(x_951.x, x_951.y, x_951.z) * vec3<f32>(x_954.x, x_954.x, x_954.x)) + vec3<f32>(x_957.x, x_957.x, x_957.x));
  let x_960 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_959.x, x_959.y, x_959.z, x_960.w);
  let x_970 : vec4<f32> = u_xlat0;
  let x_972 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_970.x, x_970.y, x_970.z));
  ...
  let x_976 : vec2<f32> = vs_TEXCOORD0;
  let x_979 : vec4<f32> = x_55.x_Dithering_Coords;
  let x_983 : vec4<f32> = x_55.x_Dithering_Coords;
  let x_985 : vec2<f32> = ((x_976 * vec2<f32>(x_979.x, x_979.y)) + vec2<f32>(x_983.z, x_983.w));
  ...
  u_xlat10.x = ((x_1004 * 340282346638528859811704183484516925440.0f) + 0.5f);
  let x_1010 : f32 = u_xlat10.x;
  u_xlat10.x = clamp(x_1010, 0.0f, 1.0f);
  let x_1014 : f32 = u_xlat10.x;
  u_xlat10.x = ((x_1014 * 2.0f) + -1.0f);
  ...
  u_xlat10 = (vec3<f32>(x_1039.x, x_1039.y, x_1039.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_1087 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_1084.x, x_1084.y, x_1084.z, x_1084.x));
  u_xlatb0 = vec3<bool>(x_1087.x, x_1087.y, x_1087.z);
  ...
  let x_1133 : vec3<f32> = ((vec3<f32>(x_1126.x, x_1126.x, x_1126.x) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_1131.x, x_1131.y, x_1131.z));
  let x_1134 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1133.x, x_1133.y, x_1133.z, x_1134.w);
  let x_1136 : vec4<f32> = u_xlat0;
  let x_1140 : vec3<f32> = (vec3<f32>(x_1136.x, x_1136.y, x_1136.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1141 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1140.x, x_1140.y, x_1140.z, x_1141.w);
  let x_1143 : vec4<f32> = u_xlat0;
  let x_1147 : vec3<f32> = (vec3<f32>(x_1143.x, x_1143.y, x_1143.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1148 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1147.x, x_1147.y, x_1147.z, x_1148.w);
  let x_1150 : vec4<f32> = u_xlat2;
  let x_1154 : vec3<f32> = (vec3<f32>(x_1150.x, x_1150.y, x_1150.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1155 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1154.x, x_1154.y, x_1154.z, x_1155.w);
  let x_1157 : vec4<f32> = u_xlat2;
  let x_1160 : vec3<f32> = max(abs(vec3<f32>(x_1157.x, x_1157.y, x_1157.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  let x_1161 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1160.x, x_1160.y, x_1160.z, x_1161.w);
  let x_1163 : vec4<f32> = u_xlat2;
  let x_1165 : vec3<f32> = log2(vec3<f32>(x_1163.x, x_1163.y, x_1163.z));
  let x_1166 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1165.x, x_1165.y, x_1165.z, x_1166.w);
  let x_1168 : vec4<f32> = u_xlat2;
  let x_1172 : vec3<f32> = (vec3<f32>(x_1168.x, x_1168.y, x_1168.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_1173 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1172.x, x_1172.y, x_1172.z, x_1173.w);
  let x_1175 : vec4<f32> = u_xlat2;
  let x_1177 : vec3<f32> = exp2(vec3<f32>(x_1175.x, x_1175.y, x_1175.z));
  let x_1178 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1177.x, x_1177.y, x_1177.z, x_1178.w);
  let x_1182 : vec4<f32> = u_xlat0;
  let x_1184 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1182.x, x_1182.y, x_1182.z, x_1182.x));
  u_xlatb0 = vec3<bool>(x_1184.x, x_1184.y, x_1184.z);
  let x_1189 : bool = u_xlatb0.x;
  if (x_1189) {

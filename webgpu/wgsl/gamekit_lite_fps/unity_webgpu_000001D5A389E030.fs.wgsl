...
fn main_1() {
  ...
  if (x_878) {
    ...
    u_xlat2.w = ((x_1110 * x_1112) + 1.0f);
  }
  let x_1119 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_1119);
  let x_1121 : bool = u_xlatb0;
  if (x_1121) {
    let x_1124 : vec4<f32> = u_xlat2;
    let x_1125 : vec3<f32> = vec3<f32>(x_1124.x, x_1124.y, x_1124.z);
    let x_1126 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1125.x, x_1125.y, x_1125.z, x_1126.w);
    let x_1128 : vec4<f32> = u_xlat0;
    let x_1132 : vec3<f32> = clamp(vec3<f32>(x_1128.x, x_1128.y, x_1128.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1133 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1132.x, x_1132.y, x_1132.z, x_1133.w);
    let x_1135 : vec4<f32> = u_xlat0;
    u_xlat2.w = dot(vec3<f32>(x_1135.x, x_1135.y, x_1135.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_230 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_228, x_228));
  u_xlat13 = x_230.x;
  let x_240 : vec3<f32> = u_xlat2;
  let x_241 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, x_240);
  u_xlat2.x = x_241.w;
  let x_244 : f32 = u_xlat13;
  let x_246 : f32 = u_xlat2.x;
  u_xlat13 = (x_244 * x_246);
  let x_248 : f32 = u_xlat12;
  let x_249 : f32 = u_xlat13;
  u_xlat12 = (x_248 * x_249);
  let x_251 : f32 = u_xlat12;
  let x_255 : vec4<f32> = x_18.x_LightColor0;
  u_xlat2 = (vec3<f32>(x_251, x_251, x_251) * vec3<f32>(x_255.x, x_255.y, x_255.z));
  let x_259 : vec3<f32> = vs_TEXCOORD1;
  let x_260 : vec3<f32> = u_xlat0;
  u_xlat0.x = dot(x_259, x_260);
  let x_264 : f32 = u_xlat0.x;
  u_xlat0.x = max(x_264, 0.0f);
  let x_268 : vec3<f32> = u_xlat1;
  let x_269 : vec3<f32> = u_xlat2;
  u_xlat4 = (x_268 * x_269);
  let x_273 : vec3<f32> = u_xlat0;
  let x_275 : vec3<f32> = u_xlat4;
  let x_276 : vec3<f32> = (vec3<f32>(x_273.x, x_273.x, x_273.x) * x_275);
  let x_277 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_276.x, x_276.y, x_276.z, x_277.w);
  SV_Target0.w = 1.0f;
  return;
}

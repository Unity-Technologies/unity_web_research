#line 1 "Hidden/PostProcessing/Debug/Overlays"
static const int SOURCE_GBUFFER = 0 ;
...
{
float d = _CameraDepthTexture . SampleLevel ( sampler_CameraDepthTexture , i . texcoordStereo , 0 ) . r ;
...
{
#line 46
float4 cdn = _CameraDepthNormalsTexture . Sample ( sampler_CameraDepthNormalsTexture , i . texcoordStereo ) ;
...
{
float2 mv = _CameraMotionVectorsTexture . Sample ( sampler_CameraMotionVectorsTexture , UnityStereoTransformScreenSpaceTex ( coords ) ) . xy ;
mv . y *= - 1.0 ;
return mv ;
}
...
float3 src = _MainTex . Sample ( sampler_MainTex , i . texcoordStereo ) . rgb ;
float2 mv = SampleMotionVectors ( i . texcoord ) ;
#line 112
const float kIntensity = _Params . x ;
#line 115
float phi = atan2 ( mv . x , mv . y ) ;
float hue = ( phi / 3.14159265359 + 1.0 ) * 0.5 ;
...
{
float3 color = _MainTex . Sample ( sampler_MainTex , i . texcoordStereo ) . rgb ;

...
@group(0) @binding(18) var x_Gradient : texture_2d<f32>;

@group(0) @binding(19) var sampler_Gradient : sampler;
...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_429.y, x_429.y, x_429.y, x_429.y) * x_433);
  let x_436 : vec4<f32> = x_43.unity_WorldToLight[0i];
  let x_437 : vec4<f32> = u_xlat0;
  let x_440 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_436 * vec4<f32>(x_437.x, x_437.x, x_437.x, x_437.x)) + x_440);
  let x_443 : vec4<f32> = x_43.unity_WorldToLight[2i];
  let x_444 : vec4<f32> = u_xlat0;
  let x_447 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_443 * vec4<f32>(x_444.z, x_444.z, x_444.z, x_444.z)) + x_447);
  let x_451 : vec4<f32> = x_43.unity_WorldToLight[3i];
  let x_452 : vec4<f32> = u_xlat0;
  let x_455 : vec4<f32> = u_xlat1;
  vs_TEXCOORD5 = ((x_451 * vec4<f32>(x_452.w, x_452.w, x_452.w, x_452.w)) + x_455);

...
fn main_1() {
  ...
  let x_951 : f32 = x_31.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_951);
  let x_953 : bool = u_xlatb0;
  if (x_953) {
    ...
    let x_964 : vec3<f32> = clamp(vec3<f32>(x_960.x, x_960.y, x_960.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_965 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_964.x, x_964.y, x_964.z, x_965.w);
    let x_967 : vec4<f32> = u_xlat0;
    u_xlat3.w = dot(vec3<f32>(x_967.x, x_967.y, x_967.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

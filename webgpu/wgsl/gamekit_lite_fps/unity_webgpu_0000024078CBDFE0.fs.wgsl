...
fn main_1() {
  ...
  let x_670 : f32 = x_49.x_PostExposure;
  let x_672 : f32 = x_49.x_PostExposure;
  let x_674 : f32 = x_49.x_PostExposure;
  let x_676 : f32 = x_49.x_PostExposure;
  let x_677 : vec4<f32> = vec4<f32>(x_670, x_672, x_674, x_676);
  u_xlat0 = (x_667 * vec4<f32>(x_677.x, x_677.y, x_677.z, x_677.w));
  let x_684 : vec4<f32> = u_xlat0;
  let x_691 : vec3<f32> = ((vec3<f32>(x_684.z, x_684.x, x_684.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_692 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_691.x, x_691.y, x_691.z, x_692.w);
  let x_694 : vec4<f32> = u_xlat1;
  let x_696 : vec3<f32> = log2(vec3<f32>(x_694.x, x_694.y, x_694.z));
  let x_697 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_696.x, x_696.y, x_696.z, x_697.w);
  let x_699 : vec4<f32> = u_xlat1;
  let x_706 : vec3<f32> = ((vec3<f32>(x_699.x, x_699.y, x_699.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_707 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_706.x, x_706.y, x_706.z, x_707.w);
  let x_709 : vec4<f32> = u_xlat1;
  let x_713 : vec3<f32> = clamp(vec3<f32>(x_709.x, x_709.y, x_709.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_714 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_713.x, x_713.y, x_713.z, x_714.w);
  let x_716 : vec4<f32> = u_xlat1;
  let x_720 : vec3<f32> = x_49.x_Lut2D_Params;
  u_xlat6 = (vec3<f32>(x_716.x, x_716.y, x_716.z) * vec3<f32>(x_720.z, x_720.z, x_720.z));
  let x_724 : f32 = u_xlat6.x;
  u_xlat6.x = floor(x_724);
  let x_728 : vec3<f32> = x_49.x_Lut2D_Params;
  let x_731 : vec2<f32> = (vec2<f32>(x_728.x, x_728.y) * vec2<f32>(0.5f, 0.5f));
  let x_732 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_731.x, x_731.y, x_732.z, x_732.w);
  let x_734 : vec3<f32> = u_xlat6;
  let x_737 : vec3<f32> = x_49.x_Lut2D_Params;
  let x_740 : vec4<f32> = u_xlat2;
  let x_742 : vec2<f32> = ((vec2<f32>(x_734.y, x_734.z) * vec2<f32>(x_737.x, x_737.y)) + vec2<f32>(x_740.x, x_740.y));
  let x_743 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_743.x, x_742.x, x_742.y, x_743.w);
  let x_746 : f32 = u_xlat6.x;
  let x_748 : f32 = x_49.x_Lut2D_Params.y;
  let x_751 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_746 * x_748) + x_751);
  ...
  u_xlat11 = (vec2<f32>(x_771.x, x_771.z) + vec2<f32>(x_773.x, x_773.y));
  let x_779 : vec2<f32> = u_xlat11;
  let x_780 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_779);
  ...
  let x_787 : f32 = x_49.x_Lut2D_Params.z;
  let x_790 : f32 = u_xlat6.x;
  u_xlat1.x = ((x_785 * x_787) + -(x_790));
  let x_794 : vec4<f32> = u_xlat3;
  let x_797 : vec4<f32> = u_xlat2;
  u_xlat6 = (-(vec3<f32>(x_794.x, x_794.y, x_794.z)) + vec3<f32>(x_797.x, x_797.y, x_797.z));
  ...
  let x_811 : f32 = x_49.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_811);
  let x_813 : bool = u_xlatb1;
  if (x_813) {
    ...
    u_xlat0.w = dot(vec3<f32>(x_827.x, x_827.y, x_827.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

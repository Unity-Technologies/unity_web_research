...
fn main_1() {
  ...
  let x_458 : f32 = u_xlat0.z;
  u_xlat1.x = min(x_456, x_458);
  let x_462 : f32 = u_xlat0.z;
  let x_465 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_462) + x_465);
  let x_473 : f32 = x_52.unity_LightShadowBias.y;
  let x_475 : f32 = u_xlat1.x;
  let x_478 : f32 = u_xlat0.z;
  gl_Position.z = ((x_473 * x_475) + x_478);
  let x_482 : vec4<f32> = u_xlat0;
  let x_483 : vec3<f32> = vec3<f32>(x_482.x, x_482.y, x_482.w);
  let x_485 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_483.x, x_483.y, x_485.z, x_483.z);
  return;
}

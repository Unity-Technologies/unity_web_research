...
{
#line 76
float shadow = _ShadowMapTexture . SampleCmpLevelZero ( sampler_ShadowMapTexture , ( shadowCoord ) . xy / ( shadowCoord ) . w , ( shadowCoord ) . z / ( shadowCoord ) . w ) ;
...
#line 92
float4 spotShadowCoord = mul ( unity_WorldToShadow [ 0 ] , float4 ( worldPos , 1 ) ) ;
#line 96
...
float4 _LightCoord : TEXCOORD6 ; float4 _ShadowCoord : TEXCOORD7 ;
#line 371
} ;
...
float4 lightCoord = mul ( unity_WorldToLight , float4 ( s . posWorld , 1 ) ) ; float shadow = UnityComputeForwardShadows ( 0 , s . posWorld , 0 ) ; float atten = ( lightCoord . z > 0 ) * UnitySpotCookie ( lightCoord ) * UnitySpotAttenuate ( lightCoord . xyz ) * shadow ; ;

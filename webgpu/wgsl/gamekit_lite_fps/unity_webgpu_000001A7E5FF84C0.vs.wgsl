...
@group(0) @binding(2) var<uniform> x_326 : UnityInstancing_PerDraw2;
...
fn main_1() {
  ...
  u_xlat2 = (vec4<f32>(x_316.y, x_316.z, x_316.z, x_316.x) * vec4<f32>(x_318.x, x_318.y, x_318.z, x_318.z));
  let x_327 : i32 = u_xlati0;
  let x_330 : vec4<f32> = x_326.unity_Builtins2Array[(x_327 / 7i)].unity_SHBrArray;
  let x_331 : vec4<f32> = u_xlat2;
  u_xlat3.x = dot(x_330, x_331);
  let x_334 : i32 = u_xlati0;
  let x_338 : vec4<f32> = x_326.unity_Builtins2Array[(x_334 / 7i)].unity_SHBgArray;
  let x_339 : vec4<f32> = u_xlat2;
  u_xlat3.y = dot(x_338, x_339);
  let x_342 : i32 = u_xlati0;
  let x_346 : vec4<f32> = x_326.unity_Builtins2Array[(x_342 / 7i)].unity_SHBbArray;
  let x_347 : vec4<f32> = u_xlat2;
  u_xlat3.z = dot(x_346, x_347);
  let x_351 : i32 = u_xlati0;
  let x_355 : vec4<f32> = x_326.unity_Builtins2Array[(x_351 / 7i)].unity_SHCArray;
  let x_357 : vec4<f32> = u_xlat1;
  let x_360 : vec3<f32> = u_xlat3;
  vs_TEXCOORD4 = ((vec3<f32>(x_355.x, x_355.y, x_355.z) * vec3<f32>(x_357.x, x_357.x, x_357.x)) + x_360);
...
fn main(@builtin(instance_index) gl_InstanceIndex_param : u32, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, gl_Position, vs_TEXCOORD0, vs_COLOR0, vs_TEXCOORD2, vs_TEXCOORD4, vs_SV_InstanceID0);
}

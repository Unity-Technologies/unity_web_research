...
fn main_1() {
  ...
  let x_523 : f32 = x_55.x_PostExposure;
  let x_524 : vec4<f32> = vec4<f32>(x_517, x_519, x_521, x_523);
  u_xlat0 = (vec4<f32>(x_513.w, x_513.x, x_513.y, x_513.z) * vec4<f32>(x_524.x, x_524.y, x_524.z, x_524.w));
  let x_531 : vec4<f32> = u_xlat0;
  let x_538 : vec3<f32> = ((vec3<f32>(x_531.y, x_531.z, x_531.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_567 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_569 : vec3<f32> = (vec3<f32>(x_563.x, x_563.y, x_563.z) * vec3<f32>(x_567.y, x_567.y, x_567.y));
  let x_570 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_569.x, x_569.y, x_569.z, x_570.w);
  let x_574 : f32 = x_55.x_Lut3D_Params.x;
  u_xlat22 = (x_574 * 0.5f);
  let x_576 : vec4<f32> = u_xlat1;
  let x_579 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_582 : f32 = u_xlat22;
  let x_584 : vec3<f32> = ((vec3<f32>(x_576.x, x_576.y, x_576.z) * vec3<f32>(x_579.x, x_579.x, x_579.x)) + vec3<f32>(x_582, x_582, x_582));
  let x_585 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_584.x, x_584.y, x_584.z, x_585.w);
  let x_595 : vec4<f32> = u_xlat1;
  let x_597 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_595.x, x_595.y, x_595.z));
  let x_598 : vec3<f32> = vec3<f32>(x_597.x, x_597.y, x_597.z);
  let x_599 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_599.x, x_598.x, x_598.y, x_598.z);
  let x_604 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_604);
  let x_606 : bool = u_xlatb1;
  if (x_606) {
    let x_609 : vec4<f32> = u_xlat0;
    let x_610 : vec3<f32> = vec3<f32>(x_609.y, x_609.z, x_609.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_620.x, x_620.y, x_620.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_626 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_626.y, x_626.z, x_626.w, x_626.x);
  return;
}

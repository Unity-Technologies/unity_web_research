...
fn main_1() {
  ...
  let x_799 : vec2<f32> = vs_TEXCOORD1;
  let x_802 : vec4<f32> = x_55.x_Grain_Params2;
  let x_806 : vec4<f32> = x_55.x_Grain_Params2;
  ...
  let x_818 : vec4<f32> = textureSample(x_GrainTex, sampler_GrainTex, vec2<f32>(x_816.x, x_816.y));
  ...
  u_xlat27 = dot(vec3<f32>(x_833.x, x_833.y, x_833.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_840 : f32 = u_xlat27;
  u_xlat27 = sqrt(x_840);
  let x_844 : f32 = x_55.x_Grain_Params1.x;
  let x_845 : f32 = u_xlat27;
  u_xlat27 = ((x_844 * -(x_845)) + 1.0f);
  ...
  let x_861 : f32 = x_55.x_Grain_Params1.y;
  let x_863 : f32 = x_55.x_Grain_Params1.y;
  ...
  let x_882 : f32 = x_55.x_PostExposure;
  let x_884 : f32 = x_55.x_PostExposure;
  let x_886 : f32 = x_55.x_PostExposure;
  let x_888 : f32 = x_55.x_PostExposure;
  let x_889 : vec4<f32> = vec4<f32>(x_882, x_884, x_886, x_888);
  u_xlat0 = (vec4<f32>(x_878.w, x_878.x, x_878.y, x_878.z) * vec4<f32>(x_889.x, x_889.y, x_889.z, x_889.w));
  let x_896 : vec4<f32> = u_xlat0;
  let x_903 : vec3<f32> = ((vec3<f32>(x_896.y, x_896.z, x_896.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_918 : vec3<f32> = ((vec3<f32>(x_911.x, x_911.y, x_911.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_919 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_918.x, x_918.y, x_918.z, x_919.w);
  let x_921 : vec4<f32> = u_xlat1;
  let x_925 : vec3<f32> = clamp(vec3<f32>(x_921.x, x_921.y, x_921.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_926 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_925.x, x_925.y, x_925.z, x_926.w);
  let x_928 : vec4<f32> = u_xlat1;
  let x_933 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_935 : vec3<f32> = (vec3<f32>(x_928.x, x_928.y, x_928.z) * vec3<f32>(x_933.y, x_933.y, x_933.y));
  let x_936 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_935.x, x_935.y, x_935.z, x_936.w);
  let x_940 : f32 = x_55.x_Lut3D_Params.x;
  u_xlat28 = (x_940 * 0.5f);
  let x_942 : vec4<f32> = u_xlat1;
  let x_945 : vec2<f32> = x_55.x_Lut3D_Params;
  let x_948 : f32 = u_xlat28;
  let x_950 : vec3<f32> = ((vec3<f32>(x_942.x, x_942.y, x_942.z) * vec3<f32>(x_945.x, x_945.x, x_945.x)) + vec3<f32>(x_948, x_948, x_948));
  let x_951 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_950.x, x_950.y, x_950.z, x_951.w);
  let x_961 : vec4<f32> = u_xlat1;
  let x_963 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_961.x, x_961.y, x_961.z));
  let x_964 : vec3<f32> = vec3<f32>(x_963.x, x_963.y, x_963.z);
  let x_965 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_965.x, x_964.x, x_964.y, x_964.z);
  let x_970 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_970);
  let x_972 : bool = u_xlatb1;
  if (x_972) {
    let x_975 : vec4<f32> = u_xlat0;
    let x_976 : vec3<f32> = vec3<f32>(x_975.y, x_975.z, x_975.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_986.x, x_986.y, x_986.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_992 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_992.y, x_992.z, x_992.w, x_992.x);
  return;
}

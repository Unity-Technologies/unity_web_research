...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_355.w, x_355.x, x_355.y, x_355.z) + vec4<f32>(x_357.w, x_357.x, x_357.y, x_357.z));
  let x_364 : f32 = x_51.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_364);
  let x_366 : bool = u_xlatb1;
  if (x_366) {
    let x_369 : vec4<f32> = u_xlat0;
    let x_370 : vec3<f32> = vec3<f32>(x_369.y, x_369.z, x_369.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_380.x, x_380.y, x_380.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_390 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_390.y, x_390.z, x_390.w, x_390.x);
  return;
}

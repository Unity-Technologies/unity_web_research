...
fn main_1() {
  ...
  let x_954 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_954);
  let x_956 : bool = u_xlatb1;
  if (x_956) {
    ...
    let x_967 : vec3<f32> = clamp(vec3<f32>(x_963.x, x_963.y, x_963.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_968 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_967.x, x_967.y, x_967.z, x_968.w);
    let x_970 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_970.x, x_970.y, x_970.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_835 : f32 = x_31.x_PostExposure;
  let x_837 : f32 = x_31.x_PostExposure;
  let x_838 : vec4<f32> = vec4<f32>(x_831, x_833, x_835, x_837);
  u_xlat0 = (vec4<f32>(x_827.w, x_827.x, x_827.y, x_827.z) * vec4<f32>(x_838.x, x_838.y, x_838.z, x_838.w));
  let x_845 : vec4<f32> = u_xlat0;
  let x_852 : vec3<f32> = ((vec3<f32>(x_845.y, x_845.z, x_845.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_853 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_852.x, x_852.y, x_852.z, x_853.w);
  let x_855 : vec4<f32> = u_xlat1;
  let x_857 : vec3<f32> = log2(vec3<f32>(x_855.x, x_855.y, x_855.z));
  let x_858 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_857.x, x_857.y, x_857.z, x_858.w);
  let x_860 : vec4<f32> = u_xlat1;
  let x_867 : vec3<f32> = ((vec3<f32>(x_860.x, x_860.y, x_860.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_868 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_867.x, x_867.y, x_867.z, x_868.w);
  let x_870 : vec4<f32> = u_xlat1;
  let x_874 : vec3<f32> = clamp(vec3<f32>(x_870.x, x_870.y, x_870.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  ...
  let x_883 : vec3<f32> = (vec3<f32>(x_877.x, x_877.y, x_877.z) * vec3<f32>(x_881.y, x_881.y, x_881.y));
  ...
  let x_892 : vec2<f32> = x_31.x_Lut3D_Params;
  let x_895 : f32 = u_xlat19;
  let x_897 : vec3<f32> = ((vec3<f32>(x_889.x, x_889.y, x_889.z) * vec3<f32>(x_892.x, x_892.x, x_892.x)) + vec3<f32>(x_895, x_895, x_895));
  let x_898 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_897.x, x_897.y, x_897.z, x_898.w);
  let x_908 : vec4<f32> = u_xlat1;
  let x_910 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_908.x, x_908.y, x_908.z));
  let x_911 : vec3<f32> = vec3<f32>(x_910.x, x_910.y, x_910.z);
  let x_912 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_912.x, x_911.x, x_911.y, x_911.z);
  let x_916 : f32 = x_31.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_916);
  let x_918 : bool = u_xlatb1;
  if (x_918) {
    let x_921 : vec4<f32> = u_xlat0;
    let x_922 : vec3<f32> = vec3<f32>(x_921.y, x_921.z, x_921.w);
    let x_923 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_922.x, x_922.y, x_922.z, x_923.w);
    let x_925 : vec4<f32> = u_xlat1;
    let x_929 : vec3<f32> = clamp(vec3<f32>(x_925.x, x_925.y, x_925.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_930 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_929.x, x_929.y, x_929.z, x_930.w);
    let x_932 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_932.x, x_932.y, x_932.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_942 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_942.y, x_942.z, x_942.w, x_942.x);
  return;
}

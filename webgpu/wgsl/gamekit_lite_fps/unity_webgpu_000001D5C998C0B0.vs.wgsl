...
fn main_1() {
  ...
  let x_351 : vec2<f32> = (vec2<f32>(x_345.y, x_345.y) * vec2<f32>(x_349.x, x_349.y));
  ...
  let x_362 : vec2<f32> = ((vec2<f32>(x_355.x, x_355.y) * vec2<f32>(x_357.x, x_357.x)) + vec2<f32>(x_360.x, x_360.y));
  ...
  let x_373 : vec2<f32> = ((vec2<f32>(x_366.x, x_366.y) * vec2<f32>(x_368.z, x_368.z)) + vec2<f32>(x_371.x, x_371.y));
  let x_374 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_373.x, x_373.y, x_374.z, x_374.w);
  let x_379 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_381 : vec4<f32> = u_xlat0;
  let x_384 : vec4<f32> = u_xlat0;
  vs_TEXCOORD6 = ((vec2<f32>(x_379.x, x_379.y) * vec2<f32>(x_381.w, x_381.w)) + vec2<f32>(x_384.x, x_384.y));

...
#line 26 "F:/src/Graphics/com.unity.postprocessing/PostProcessing/Shaders/Builtins/ScreenSpaceReflections.hlsl"
struct Ray
{
float3 origin ;
float3 direction ;
...
float result = offset / ( ( 1.0 - 0.275 ) * .25 + 0.275 ) ;
...
{
float2 k = abs ( uv - 0.5 ) * _Params . x ;
k . x *= _MainTex_TexelSize . y * _MainTex_TexelSize . z ;
return pow ( saturate ( 1.0 - dot ( k , k ) ) , 5. ) ;
}
...
float end = ray . origin . z + ray . direction . z * _Params . z ;
float magnitude = _Params . z ;
...
float2 uv = input . texcoord * _Params2 . y ;
uv . y *= _Params2 . x ;
...
#line 258
float4 FragTest ( VaryingsDefault i ) : COLOR
{
...
float confidence = ( float ) result . iterationCount / ( float ) _Params2 . w ;
return float4 ( result . uv , confidence , ( float ) result . isHit ) ;
}

float4 FragResolve ( VaryingsDefault i ) : COLOR
{
...
float4 FragReproject ( VaryingsDefault i ) : COLOR
{
...
const float2 k = 1.0 * _MainTex_TexelSize . xy ;
...
float weight = clamp ( lerp ( 0.95 , 0.7 ,
history . a * 100.0 ) , 0.7 , 0.95 ) ;

color . a *= 0.85 ;
return lerp ( color , history , weight ) ;
}

float4 FragComposite ( VaryingsDefault i ) : COLOR
{
...
#line 369
float4 resolve = _Resolve . SampleLevel ( sampler_Resolve , i . texcoordStereo , SmoothnessToRoughness ( gbuffer1 . a ) * ( _Params . w - 1.0 ) + 1.0 ) ;
#line 372
...
resolve . rgb = BRDF1_Unity_PBS ( gbuffer0 . rgb , gbuffer1 . rgb , oneMinusReflectivity , gbuffer1 . a , normal , - eye , light , indirect ) . rgb ;
...
resolve . a *= 2. * resolve . a ;
float fade = 1.0 - saturate ( 1.5 * resolve . a * smoothstep ( 0.5 , 1.0 , 1.5 * resolve . a ) * _Params . y ) ;

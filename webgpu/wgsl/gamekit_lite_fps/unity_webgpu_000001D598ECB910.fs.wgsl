...
fn main_1() {
  ...
  let x_753 : vec4<f32> = x_26.x_Grain_Params2;
  let x_757 : vec4<f32> = x_26.x_Grain_Params2;
  ...
  let x_781 : vec3<f32> = clamp(vec3<f32>(x_777.x, x_777.y, x_777.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_782 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_781.x, x_781.y, x_781.z, x_782.w);
  let x_784 : vec4<f32> = u_xlat3;
  u_xlat31 = dot(vec3<f32>(x_784.x, x_784.y, x_784.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_791 : f32 = u_xlat31;
  u_xlat31 = sqrt(x_791);
  let x_795 : f32 = x_26.x_Grain_Params1.x;
  let x_796 : f32 = u_xlat31;
  u_xlat31 = ((x_795 * -(x_796)) + 1.0f);
  ...
  let x_810 : f32 = x_26.x_Grain_Params1.y;
  let x_812 : f32 = x_26.x_Grain_Params1.y;
  let x_814 : f32 = x_26.x_Grain_Params1.y;
  let x_816 : vec3<f32> = (vec3<f32>(x_807.x, x_807.y, x_807.z) * vec3<f32>(x_810, x_812, x_814));
  let x_817 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_816.x, x_816.y, x_816.z, x_817.w);
  let x_819 : vec4<f32> = u_xlat1;
  let x_821 : f32 = u_xlat31;
  let x_824 : vec4<f32> = u_xlat0;
  let x_826 : vec3<f32> = ((vec3<f32>(x_819.x, x_819.y, x_819.z) * vec3<f32>(x_821, x_821, x_821)) + vec3<f32>(x_824.x, x_824.y, x_824.z));
  ...
  let x_837 : f32 = u_xlat0.w;
  u_xlat0.w = x_837;
  let x_840 : f32 = u_xlat0.w;
  u_xlat0.w = clamp(x_840, 0.0f, 1.0f);
  let x_843 : vec4<f32> = u_xlat1;
  let x_847 : vec3<f32> = (vec3<f32>(x_843.z, x_843.x, x_843.y) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_848 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_847.x, x_847.y, x_847.z, x_848.w);
  let x_850 : vec4<f32> = u_xlat1;
  let x_854 : vec3<f32> = max(vec3<f32>(x_850.z, x_850.x, x_850.y), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  let x_855 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_854.x, x_854.y, x_854.z, x_855.w);
  let x_857 : vec4<f32> = u_xlat4;
  let x_859 : vec3<f32> = log2(vec3<f32>(x_857.x, x_857.y, x_857.z));
  let x_860 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_859.x, x_859.y, x_859.z, x_860.w);
  let x_862 : vec4<f32> = u_xlat4;
  let x_866 : vec3<f32> = (vec3<f32>(x_862.x, x_862.y, x_862.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_881 : vec3<f32> = ((vec3<f32>(x_874.x, x_874.y, x_874.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  ...
  u_xlatb1 = vec3<bool>(x_889.x, x_889.y, x_889.z);
  let x_892 : bool = u_xlatb1.x;
  if (x_892) {
  ...
  let x_901 : f32 = x_893;
  u_xlat1.x = x_901;
  let x_904 : bool = u_xlatb1.y;
  if (x_904) {
    let x_909 : f32 = u_xlat3.y;
    x_905 = x_909;
  } else {
    let x_912 : f32 = u_xlat4.y;
    x_905 = x_912;
  }
  let x_913 : f32 = x_905;
  u_xlat1.y = x_913;
  let x_916 : bool = u_xlatb1.z;
  if (x_916) {
  ...
  let x_932 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (vec3<f32>(x_928.x, x_928.y, x_928.z) * vec3<f32>(x_932.z, x_932.z, x_932.z));
  let x_936 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_936);
  let x_940 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_942 : vec2<f32> = (vec2<f32>(x_940.x, x_940.y) * vec2<f32>(0.5f, 0.5f));
  let x_943 : vec3<f32> = u_xlat2;
  u_xlat2 = vec3<f32>(x_942.x, x_942.y, x_943.z);
  let x_945 : vec3<f32> = u_xlat11;
  let x_948 : vec3<f32> = x_26.x_Lut2D_Params;
  ...
  u_xlat3 = vec4<f32>(x_954.x, x_953.x, x_953.y, x_954.w);
  let x_957 : f32 = u_xlat11.x;
  let x_959 : f32 = x_26.x_Lut2D_Params.y;
  let x_962 : f32 = u_xlat3.y;
  u_xlat3.x = ((x_957 * x_959) + x_962);
  ...
  let x_977 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat22.x = x_977;
  let x_979 : vec2<f32> = u_xlat22;
  let x_980 : vec4<f32> = u_xlat3;
  u_xlat21 = (x_979 + vec2<f32>(x_980.x, x_980.z));
  let x_986 : vec2<f32> = u_xlat21;
  let x_987 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_986);
  u_xlat2 = vec3<f32>(x_987.x, x_987.y, x_987.z);
  let x_990 : f32 = u_xlat1.x;
  let x_992 : f32 = x_26.x_Lut2D_Params.z;
  let x_995 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_990 * x_992) + -(x_995));
  let x_999 : vec4<f32> = u_xlat4;
  let x_1002 : vec3<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_999.x, x_999.y, x_999.z)) + x_1002);
  let x_1004 : vec4<f32> = u_xlat1;
  let x_1006 : vec3<f32> = u_xlat11;
  let x_1008 : vec4<f32> = u_xlat4;
  let x_1010 : vec3<f32> = ((vec3<f32>(x_1004.x, x_1004.x, x_1004.x) * x_1006) + vec3<f32>(x_1008.x, x_1008.y, x_1008.z));
  let x_1011 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1010.x, x_1010.y, x_1010.z, x_1011.w);
  let x_1013 : vec4<f32> = u_xlat1;
  u_xlat2 = (vec3<f32>(x_1013.x, x_1013.y, x_1013.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1018 : vec4<f32> = u_xlat1;
  let x_1022 : vec3<f32> = (vec3<f32>(x_1018.x, x_1018.y, x_1018.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1023 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1022.x, x_1022.y, x_1022.z, x_1023.w);
  let x_1025 : vec4<f32> = u_xlat3;
  let x_1029 : vec3<f32> = (vec3<f32>(x_1025.x, x_1025.y, x_1025.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1030 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1029.x, x_1029.y, x_1029.z, x_1030.w);
  let x_1032 : vec4<f32> = u_xlat3;
  let x_1035 : vec3<f32> = max(abs(vec3<f32>(x_1032.x, x_1032.y, x_1032.z)), vec3<f32>(1.1920929e-07f, 1.1920929e-07f, 1.1920929e-07f));
  ...
  u_xlat0.z = x_1095;
  let x_1099 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1.x = (0.5f < x_1099);
  let x_1103 : bool = u_xlatb1.x;
  if (x_1103) {
    ...
    let x_1114 : vec3<f32> = clamp(vec3<f32>(x_1110.x, x_1110.y, x_1110.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1115 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_1114.x, x_1114.y, x_1114.z, x_1115.w);
    let x_1117 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_1117.x, x_1117.y, x_1117.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

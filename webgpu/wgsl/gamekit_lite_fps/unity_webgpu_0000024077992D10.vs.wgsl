...
@group(0) @binding(4) var x_Gradient : texture_2d<f32>;

@group(0) @binding(5) var sampler_Gradient : sampler;

var<private> in_TEXCOORD0 : vec4<f32>;

@group(0) @binding(3) var<uniform> x_43 : VGlobals;
...
fn main_1() {
  ...
  let x_380 : vec4<f32> = in_TEXCOORD0;
  let x_384 : vec4<f32> = x_43.x_Noise_ST;
  let x_388 : vec4<f32> = x_43.x_Noise_ST;
  vs_TEXCOORD1 = ((vec2<f32>(x_380.x, x_380.y) * vec2<f32>(x_384.x, x_384.y)) + vec2<f32>(x_388.z, x_388.w));
  let x_391 : vec4<f32> = u_xlat0;
  let x_394 : vec4<f32> = x_43.unity_ObjectToWorld[1i];
  let x_396 : vec3<f32> = (vec3<f32>(x_391.y, x_391.y, x_391.y) * vec3<f32>(x_394.x, x_394.y, x_394.z));
  let x_397 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_396.x, x_396.y, x_396.z, x_397.w);
  let x_400 : vec4<f32> = x_43.unity_ObjectToWorld[0i];
  ...
  u_xlat0 = vec4<f32>(x_407.x, x_407.y, x_408.z, x_407.z);
  let x_411 : vec4<f32> = x_43.unity_ObjectToWorld[2i];
  let x_413 : vec4<f32> = u_xlat0;
  let x_416 : vec4<f32> = u_xlat0;
  let x_418 : vec3<f32> = ((vec3<f32>(x_411.x, x_411.y, x_411.z) * vec3<f32>(x_413.z, x_413.z, x_413.z)) + vec3<f32>(x_416.x, x_416.y, x_416.w));
  let x_419 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_418.x, x_418.y, x_418.z, x_419.w);
  let x_423 : vec4<f32> = x_43.unity_ObjectToWorld[3i];
  let x_425 : vec4<f32> = in_POSITION0;
  let x_428 : vec4<f32> = u_xlat0;
  vs_TEXCOORD2 = ((vec3<f32>(x_423.x, x_423.y, x_423.z) * vec3<f32>(x_425.w, x_425.w, x_425.w)) + vec3<f32>(x_428.x, x_428.y, x_428.z));
  return;
}

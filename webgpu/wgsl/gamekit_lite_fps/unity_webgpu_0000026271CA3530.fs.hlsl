...
#line 50 "Legacy Shaders/VertexLit"
float3 computeLighting ( int idx , float3 dirToLight , float3 eyeNormal , float3 viewDir , float4 diffuseColor , float shininess , float atten , inout float3 specColor ) {
float NdotL = max ( dot ( eyeNormal , dirToLight ) , 0.0 ) ;

float3 color = NdotL * diffuseColor . rgb * unity_LightColor [ idx ] . rgb ;

if ( NdotL > 0.0 ) {
float3 h = normalize ( dirToLight + viewDir ) ;
float HdotN = max ( dot ( eyeNormal , h ) , 0.0 ) ;
float sp = saturate ( pow ( HdotN , shininess ) ) ;
specColor += ( atten * sp ) * unity_LightColor [ idx ] . rgb ;
}
return color * atten ;
}
#line 65
float3 computeOneLight ( int idx , float3 eyePosition , float3 eyeNormal , float3 viewDir , float4 diffuseColor , float shininess , inout float3 specColor ) {
...
#line 78
float rho = max ( dot ( dirToLight , unity_SpotDirection [ idx ] . xyz ) , 0.0 ) ;
float spotAtt = ( rho - unity_LightAtten [ idx ] . x ) * unity_LightAtten [ idx ] . y ;
...
float3 eyePos = mul ( unity_MatrixMV , float4 ( IN . pos , 1 ) ) . xyz ;
float3 eyeNormal = normalize ( mul ( ( float3x3 ) unity_MatrixITMV , IN . normal ) . xyz ) ;
float3 viewDir = 0.0 ;
viewDir = - normalize ( eyePos ) ;

float3 lcolor = _Emission . rgb + _Color . rgb * glstate_lightmodel_ambient . rgb ;
float3 specColor = 0.0 ;
float shininess = _Shininess * 128.0 ;
for ( int il = 0 ; il < unity_VertexLightParams . x ; ++ il ) {

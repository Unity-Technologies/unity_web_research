...
fn main_1() {
  ...
  let x_789 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_789);
  let x_791 : bool = u_xlatb0;
  if (x_791) {
    let x_794 : vec4<f32> = u_xlat3;
    u_xlat0 = vec3<f32>(x_794.x, x_794.y, x_794.z);
    let x_796 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_796, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_800 : vec3<f32> = u_xlat0;
    u_xlat3.w = dot(x_800, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

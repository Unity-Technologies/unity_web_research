...
fn main_1() {
  ...
  u_xlat0 = (x_869 + x_870);
  let x_873 : vec2<f32> = vs_TEXCOORD1;
  let x_876 : vec4<f32> = x_26.x_Grain_Params2;
  let x_880 : vec4<f32> = x_26.x_Grain_Params2;
  ...
  let x_892 : vec4<f32> = textureSample(x_GrainTex, sampler_GrainTex, vec2<f32>(x_890.x, x_890.y));
  ...
  u_xlat31 = dot(vec3<f32>(x_907.x, x_907.y, x_907.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_914 : f32 = u_xlat31;
  u_xlat31 = sqrt(x_914);
  let x_918 : f32 = x_26.x_Grain_Params1.x;
  let x_919 : f32 = u_xlat31;
  u_xlat31 = ((x_918 * -(x_919)) + 1.0f);
  let x_923 : vec4<f32> = u_xlat0;
  let x_925 : vec4<f32> = u_xlat1;
  let x_927 : vec3<f32> = (vec3<f32>(x_923.x, x_923.y, x_923.z) * vec3<f32>(x_925.x, x_925.y, x_925.z));
  ...
  let x_937 : f32 = x_26.x_Grain_Params1.y;
  let x_939 : vec3<f32> = (vec3<f32>(x_930.x, x_930.y, x_930.z) * vec3<f32>(x_933, x_935, x_937));
  ...
  let x_949 : vec3<f32> = ((vec3<f32>(x_942.x, x_942.y, x_942.z) * vec3<f32>(x_944, x_944, x_944)) + vec3<f32>(x_947.x, x_947.y, x_947.z));
  let x_950 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_949.x, x_949.y, x_949.z, x_950.w);
  let x_952 : vec4<f32> = u_xlat0;
  let x_956 : f32 = x_26.x_PostExposure;
  let x_958 : f32 = x_26.x_PostExposure;
  let x_960 : f32 = x_26.x_PostExposure;
  let x_962 : f32 = x_26.x_PostExposure;
  let x_963 : vec4<f32> = vec4<f32>(x_956, x_958, x_960, x_962);
  u_xlat0 = (vec4<f32>(x_952.w, x_952.x, x_952.y, x_952.z) * vec4<f32>(x_963.x, x_963.y, x_963.z, x_963.w));
  let x_970 : vec4<f32> = u_xlat0;
  let x_977 : vec3<f32> = ((vec3<f32>(x_970.y, x_970.z, x_970.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_978 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_977.x, x_977.y, x_977.z, x_978.w);
  let x_980 : vec4<f32> = u_xlat1;
  let x_982 : vec3<f32> = log2(vec3<f32>(x_980.x, x_980.y, x_980.z));
  let x_983 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_982.x, x_982.y, x_982.z, x_983.w);
  let x_985 : vec4<f32> = u_xlat1;
  let x_992 : vec3<f32> = ((vec3<f32>(x_985.x, x_985.y, x_985.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_993 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_992.x, x_992.y, x_992.z, x_993.w);
  let x_995 : vec4<f32> = u_xlat1;
  let x_999 : vec3<f32> = clamp(vec3<f32>(x_995.x, x_995.y, x_995.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_1000 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_999.x, x_999.y, x_999.z, x_1000.w);
  let x_1002 : vec4<f32> = u_xlat1;
  let x_1007 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_1009 : vec3<f32> = (vec3<f32>(x_1002.x, x_1002.y, x_1002.z) * vec3<f32>(x_1007.y, x_1007.y, x_1007.y));
  let x_1010 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1009.x, x_1009.y, x_1009.z, x_1010.w);
  let x_1013 : f32 = x_26.x_Lut3D_Params.x;
  u_xlat31 = (x_1013 * 0.5f);
  let x_1015 : vec4<f32> = u_xlat1;
  let x_1018 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_1021 : f32 = u_xlat31;
  let x_1023 : vec3<f32> = ((vec3<f32>(x_1015.x, x_1015.y, x_1015.z) * vec3<f32>(x_1018.x, x_1018.x, x_1018.x)) + vec3<f32>(x_1021, x_1021, x_1021));
  let x_1024 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1023.x, x_1023.y, x_1023.z, x_1024.w);
  let x_1034 : vec4<f32> = u_xlat1;
  let x_1036 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_1034.x, x_1034.y, x_1034.z));
  let x_1037 : vec3<f32> = vec3<f32>(x_1036.x, x_1036.y, x_1036.z);
  let x_1038 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1038.x, x_1037.x, x_1037.y, x_1037.z);
  let x_1042 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1042);
  let x_1044 : bool = u_xlatb1;
  if (x_1044) {
    ...
    let x_1055 : vec3<f32> = clamp(vec3<f32>(x_1051.x, x_1051.y, x_1051.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1056 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_1055.x, x_1055.y, x_1055.z, x_1056.w);
    let x_1058 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_1058.x, x_1058.y, x_1058.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
#line 16 ""
Texture2D _MainTex ; SamplerState sampler_LinearClamp ;
float4 _MainTex_TexelSize ;

Texture2D _HistoryTex ; SamplerState sampler_HistoryTex ;
...
_CameraDepthTexture . Sample ( sampler_CameraDepthTexture , UnityStereoClamp ( uv - k ) ) . r ,
_CameraDepthTexture . Sample ( sampler_CameraDepthTexture , UnityStereoClamp ( uv + float2 ( k . x , - k . y ) ) ) . r ,
_CameraDepthTexture . Sample ( sampler_CameraDepthTexture , UnityStereoClamp ( uv + float2 ( - k . x , k . y ) ) ) . r ,
_CameraDepthTexture . Sample ( sampler_CameraDepthTexture , UnityStereoClamp ( uv + k ) ) . r
) ;
#line 47
float3 result = float3 ( 0.0 , 0.0 , _CameraDepthTexture . Sample ( sampler_CameraDepthTexture , uv ) . r ) ;
result = lerp ( result , float3 ( - 1.0 , - 1.0 , neighborhood . x ) , step ( result . z , neighborhood . x ) ) ;
result = lerp ( result , float3 ( 1.0 , - 1.0 , neighborhood . y ) , step ( result . z , neighborhood . y ) ) ;
result = lerp ( result , float3 ( - 1.0 , 1.0 , neighborhood . z ) , step ( result . z , neighborhood . z ) ) ;
result = lerp ( result , float3 ( 1.0 , 1.0 , neighborhood . w ) , step ( result . z , neighborhood . w ) ) ;
...
float4 topLeft = _MainTex . Sample ( sampler_LinearClamp , UnityStereoClamp ( uv - k * 0.5 ) ) ;
float4 bottomRight = _MainTex . Sample ( sampler_LinearClamp , UnityStereoClamp ( uv + k * 0.5 ) ) ;

float4 corners = 4.0 * ( topLeft + bottomRight ) - 2.0 * color ;
#line 90
color += ( color - ( corners * 0.166667 ) ) * 2.718282 * _Sharpness ;
color = clamp ( color , 0.0 , 65472.0 ) ;
#line 94
float4 average = ( corners + color ) * 0.142857 ;

float4 history = _HistoryTex . Sample ( sampler_HistoryTex , UnityStereoClamp ( texcoord - motion ) ) ;
...
float2 motion = _CameraMotionVectorsTexture . Sample ( sampler_CameraMotionVectorsTexture , closest ) . xy ;
...
float2 motion = _CameraMotionVectorsTexture . Sample ( sampler_CameraMotionVectorsTexture , i . texcoordStereo ) . xy ;
return Solve ( motion , i . texcoordStereo ) ;
}

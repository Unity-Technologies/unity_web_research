...
layout(set = 0, binding = 10) uniform mediump  sampler sampler_PointRepeat;
...
vec4 hlslcc_FragCoord = vec4(gl_FragCoord.xyz, 1.0/gl_FragCoord.w);
    ...
    u_xlatb1 = u_xlatb1 || u_xlatb32;
    u_xlat30 = (u_xlatb1) ? 1.0 : u_xlat30;
    u_xlat5.xyz = vs_TEXCOORD1.xyz + (-_WorldSpaceCameraPos.xyz);
    u_xlat1.x = dot(u_xlat5.xyz, u_xlat5.xyz);
    u_xlat32 = u_xlat1.x * _MainLightShadowParams.z + _MainLightShadowParams.w;
    ...
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu32 ; u_xlatu_loop_1++)
    {
        ...
        u_xlatb37 = u_xlati36>=0;
        if(u_xlatb37){
            u_xlatb37 = vec4(0.0, 0.0, 0.0, 0.0)!=vec4(_AdditionalShadowParams[u_xlati34].z);
            if(u_xlatb37){
                ...
                u_xlat8.x = trunc(_AdditionalShadowParams[u_xlati34].w);
                u_xlat37 = u_xlat37 + u_xlat8.x;
                u_xlati36 = int(u_xlat37);
            }
            u_xlati36 = u_xlati36 << (2 & int(0x1F));
            u_xlat8 = vs_TEXCOORD1.yyyy * _AdditionalLightsWorldToShadow[(u_xlati36 + 1) / 4][(u_xlati36 + 1) % 4];
            u_xlat8 = _AdditionalLightsWorldToShadow[u_xlati36 / 4][u_xlati36 % 4] * vs_TEXCOORD1.xxxx + u_xlat8;
            u_xlat8 = _AdditionalLightsWorldToShadow[(u_xlati36 + 2) / 4][(u_xlati36 + 2) % 4] * vs_TEXCOORD1.zzzz + u_xlat8;
            u_xlat8 = u_xlat8 + _AdditionalLightsWorldToShadow[(u_xlati36 + 3) / 4][(u_xlati36 + 3) % 4];
            ...
            u_xlat37 = 1.0 + (-_AdditionalShadowParams[u_xlati34].x);
            u_xlat36 = u_xlat36 * _AdditionalShadowParams[u_xlati34].x + u_xlat37;

...
fn main_1() {
  ...
  let x_758 : vec3<f32> = (vec3<f32>(x_756.x, x_756.y, x_756.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_759 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_758.x, x_758.y, x_758.z, x_759.w);
  let x_761 : vec4<f32> = u_xlat0;
  let x_764 : vec3<f32> = log2(abs(vec3<f32>(x_761.x, x_761.y, x_761.z)));
  let x_765 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_764.x, x_764.y, x_764.z, x_765.w);
  let x_767 : vec4<f32> = u_xlat2;
  let x_769 : vec3<f32> = (vec3<f32>(x_767.x, x_767.y, x_767.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_770 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_769.x, x_769.y, x_769.z, x_770.w);
  let x_772 : vec4<f32> = u_xlat2;
  let x_774 : vec3<f32> = exp2(vec3<f32>(x_772.x, x_772.y, x_772.z));
  let x_775 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_774.x, x_774.y, x_774.z, x_775.w);
  let x_777 : vec4<f32> = u_xlat2;
  let x_780 : vec3<f32> = ((vec3<f32>(x_777.x, x_777.y, x_777.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_781 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_780.x, x_780.y, x_780.z, x_781.w);
  let x_784 : vec4<f32> = u_xlat0;
  let x_786 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_784.x, x_784.y, x_784.z, x_784.x));
  ...
  let x_827 : vec4<f32> = x_38.x_Dithering_Params;
  let x_831 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_844 : f32 = x_38.x_GlobalMipBias.x;
  let x_845 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_841.x, x_841.y), x_844);
  u_xlat15 = x_845.w;
  let x_847 : f32 = u_xlat15;
  u_xlat15 = ((x_847 * 2.0f) + -1.0f);
  let x_850 : f32 = u_xlat15;
  u_xlatb1.x = (x_850 >= 0.0f);
  let x_854 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_854);
  let x_857 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_857)) + 1.0f);
  ...
  let x_899 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_897.x, x_897.y, x_897.z, x_897.x));
  ...
  let x_944 : vec3<f32> = ((vec3<f32>(x_937, x_937, x_937) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_942.x, x_942.y, x_942.z));
  let x_945 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_944.x, x_944.y, x_944.z, x_945.w);
  let x_947 : vec4<f32> = u_xlat0;
  let x_949 : vec3<f32> = (vec3<f32>(x_947.x, x_947.y, x_947.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_950 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_949.x, x_949.y, x_949.z, x_950.w);
  let x_952 : vec4<f32> = u_xlat0;
  let x_954 : vec3<f32> = (vec3<f32>(x_952.x, x_952.y, x_952.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_955 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_954.x, x_954.y, x_954.z, x_955.w);
  let x_957 : vec4<f32> = u_xlat2;
  let x_959 : vec3<f32> = (vec3<f32>(x_957.x, x_957.y, x_957.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_960 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_959.x, x_959.y, x_959.z, x_960.w);
  let x_962 : vec4<f32> = u_xlat2;
  let x_965 : vec3<f32> = log2(abs(vec3<f32>(x_962.x, x_962.y, x_962.z)));
  let x_966 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_965.x, x_965.y, x_965.z, x_966.w);
  let x_968 : vec4<f32> = u_xlat2;
  let x_970 : vec3<f32> = (vec3<f32>(x_968.x, x_968.y, x_968.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_971 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_970.x, x_970.y, x_970.z, x_971.w);
  let x_973 : vec4<f32> = u_xlat2;
  let x_975 : vec3<f32> = exp2(vec3<f32>(x_973.x, x_973.y, x_973.z));
  let x_976 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_975.x, x_975.y, x_975.z, x_976.w);
  let x_978 : vec4<f32> = u_xlat0;
  let x_980 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_978.x, x_978.y, x_978.z, x_978.x));
  u_xlatb0 = vec3<bool>(x_980.x, x_980.y, x_980.z);
  let x_983 : bool = u_xlatb0.x;
  if (x_983) {
  ...
  let x_1004 : f32 = x_996;
  u_xlat0.y = x_1004;
  let x_1007 : bool = u_xlatb0.z;
  if (x_1007) {
  ...
  let x_1023 : vec3<f32> = max(vec3<f32>(x_1020.x, x_1020.y, x_1020.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1024 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1023.x, x_1023.y, x_1023.z, x_1024.w);
  SV_Target0.w = 1.0f;
  return;
}

#version 440
#extension GL_ARB_explicit_attrib_location : require
#extension GL_ARB_shading_language_420pack : require

layout(origin_upper_left) in vec4 gl_FragCoord;
#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
precise vec4 u_xlat_precise_vec4;
precise ivec4 u_xlat_precise_ivec4;
precise bvec4 u_xlat_precise_bvec4;
precise uvec4 u_xlat_precise_uvec4;
layout(set = 1, binding = 0, std140) uniform PGlobals {
	vec2 _GlobalMipBias;
	vec4 _SourceSize;
};
layout(set = 0, binding = 0) uniform mediump texture2D _BlitTexture;
layout(set = 0, binding = 1) uniform mediump texture2D _FullCoCTexture;
layout(location = 0) in highp  vec2 vs_TEXCOORD0;
layout(location = 0) out highp float SV_Target0;
layout(location = 1) out highp vec4 SV_Target1;
vec3 u_xlat0;
vec4 u_xlat1;
vec4 u_xlat2;
float u_xlat3;
vec3 u_xlat4;
vec3 u_xlat5;
float u_xlat6;
float u_xlat9;
float u_xlat12;
float u_xlat18;
layout(set = 0, binding = 2) uniform mediump  sampler sampler_LinearClamp;
void main()
{
    u_xlat0.x = texture(sampler2D(_FullCoCTexture, sampler_LinearClamp), vs_TEXCOORD0.xy, _GlobalMipBias.x).x;
    u_xlat1 = _SourceSize.zwzw * vec4(0.899999976, -0.400000006, -0.899999976, 0.400000006) + vs_TEXCOORD0.xyxy;
    u_xlat6 = texture(sampler2D(_FullCoCTexture, sampler_LinearClamp), u_xlat1.xy, _GlobalMipBias.x).x;
    u_xlat12 = u_xlat6 + u_xlat0.x;
    u_xlat18 = texture(sampler2D(_FullCoCTexture, sampler_LinearClamp), u_xlat1.zw, _GlobalMipBias.x).x;
    u_xlat12 = u_xlat18 + u_xlat12;
    u_xlat2 = _SourceSize.zwzw * vec4(0.400000006, 0.899999976, -0.400000006, -0.899999976) + vs_TEXCOORD0.xyxy;
    u_xlat3 = texture(sampler2D(_FullCoCTexture, sampler_LinearClamp), u_xlat2.xy, _GlobalMipBias.x).x;
    u_xlat12 = u_xlat12 + u_xlat3;
    u_xlat9 = texture(sampler2D(_FullCoCTexture, sampler_LinearClamp), u_xlat2.zw, _GlobalMipBias.x).x;
    u_xlat12 = u_xlat12 + u_xlat9;
    SV_Target0 = u_xlat12 * 0.200000003;
    u_xlat4.xyz = texture(sampler2D(_BlitTexture, sampler_LinearClamp), u_xlat1.xy, _GlobalMipBias.x).xyz;
    u_xlat1.xyz = texture(sampler2D(_BlitTexture, sampler_LinearClamp), u_xlat1.zw, _GlobalMipBias.x).xyz;
    u_xlat4.xyz = vec3(u_xlat6) * u_xlat4.xyz;
    u_xlat5.xyz = texture(sampler2D(_BlitTexture, sampler_LinearClamp), vs_TEXCOORD0.xy, _GlobalMipBias.x).xyz;
    u_xlat0.xyz = u_xlat5.xyz * u_xlat0.xxx + u_xlat4.xyz;
    u_xlat0.xyz = u_xlat1.xyz * vec3(u_xlat18) + u_xlat0.xyz;
    u_xlat1.xyz = texture(sampler2D(_BlitTexture, sampler_LinearClamp), u_xlat2.xy, _GlobalMipBias.x).xyz;
    u_xlat2.xyz = texture(sampler2D(_BlitTexture, sampler_LinearClamp), u_xlat2.zw, _GlobalMipBias.x).xyz;
    u_xlat0.xyz = u_xlat1.xyz * vec3(u_xlat3) + u_xlat0.xyz;
    u_xlat0.xyz = u_xlat2.xyz * vec3(u_xlat9) + u_xlat0.xyz;
    SV_Target1.xyz = u_xlat0.xyz * vec3(0.200000003, 0.200000003, 0.200000003);
    SV_Target1.w = 0.0;
    return;
}


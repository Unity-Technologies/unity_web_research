...
fn main_1() {
  ...
  u_xlat4.x = (-(x_733) + x_737);
  let x_740 : f32 = u_xlat11;
  let x_742 : f32 = u_xlat4.x;
  let x_745 : f32 = x_22.x_FaceShininess;
  u_xlat4.x = ((x_740 * x_742) + x_745);
  let x_749 : f32 = u_xlat4.x;
  u_xlat4.x = (x_749 * 128.0f);
  let x_754 : f32 = u_xlat0.x;
  let x_756 : f32 = u_xlat4.x;
  u_xlat0.x = (x_754 * x_756);
  let x_760 : f32 = u_xlat0.x;
  u_xlat0.x = exp2(x_760);
  let x_763 : vec3<f32> = vs_TEXCOORD5;
  let x_767 : vec4<f32> = x_22.unity_WorldToLight[1i];
  u_xlat2 = (vec4<f32>(x_763.y, x_763.y, x_763.y, x_763.y) * x_767);
  let x_770 : vec4<f32> = x_22.unity_WorldToLight[0i];
  let x_771 : vec3<f32> = vs_TEXCOORD5;
  let x_774 : vec4<f32> = u_xlat2;
  u_xlat2 = ((x_770 * vec4<f32>(x_771.x, x_771.x, x_771.x, x_771.x)) + x_774);
  let x_778 : vec4<f32> = x_22.unity_WorldToLight[2i];
  let x_779 : vec3<f32> = vs_TEXCOORD5;
  let x_782 : vec4<f32> = u_xlat2;
  u_xlat2 = ((x_778 * vec4<f32>(x_779.z, x_779.z, x_779.z, x_779.z)) + x_782);
  let x_784 : vec4<f32> = u_xlat2;
  let x_786 : vec4<f32> = x_22.unity_WorldToLight[3i];
  u_xlat2 = (x_784 + x_786);
  let x_788 : vec4<f32> = u_xlat2;
  let x_790 : vec4<f32> = u_xlat2;
  u_xlat4 = (vec2<f32>(x_788.x, x_788.y) / vec2<f32>(x_790.w, x_790.w));
  let x_793 : vec2<f32> = u_xlat4;
  u_xlat4 = (x_793 + vec2<f32>(0.5f, 0.5f));
  let x_801 : vec2<f32> = u_xlat4;
  let x_802 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, x_801);
  u_xlat4.x = x_802.w;
  let x_807 : f32 = u_xlat2.z;
  u_xlatb8 = (0.0f < x_807);
  let x_809 : vec4<f32> = u_xlat2;
  let x_811 : vec4<f32> = u_xlat2;
  u_xlat2.x = dot(vec3<f32>(x_809.x, x_809.y, x_809.z), vec3<f32>(x_811.x, x_811.y, x_811.z));
  let x_820 : vec4<f32> = u_xlat2;
  let x_822 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_820.x, x_820.x));
  u_xlat2.x = x_822.x;
  let x_826 : bool = u_xlatb8;
  u_xlat8 = select(0.0f, 1.0f, x_826);
  let x_829 : f32 = u_xlat4.x;
  let x_830 : f32 = u_xlat8;
  u_xlat4.x = (x_829 * x_830);
  let x_834 : f32 = u_xlat2.x;
  let x_836 : f32 = u_xlat4.x;
  u_xlat4.x = (x_834 * x_836);
  let x_839 : vec2<f32> = u_xlat4;
  let x_843 : vec4<f32> = x_22.x_LightColor0;
  let x_845 : vec3<f32> = (vec3<f32>(x_839.x, x_839.x, x_839.x) * vec3<f32>(x_843.x, x_843.y, x_843.z));
  let x_846 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_845.x, x_845.y, x_845.z, x_846.w);
  let x_848 : vec4<f32> = u_xlat2;
  let x_852 : vec4<f32> = x_22.x_SpecColor;
  let x_854 : vec3<f32> = (vec3<f32>(x_848.x, x_848.y, x_848.z) * vec3<f32>(x_852.x, x_852.y, x_852.z));
  ...
  let x_895 : vec3<f32> = ((vec3<f32>(x_888.x, x_888.y, x_888.z) * vec3<f32>(x_890.w, x_890.w, x_890.w)) + vec3<f32>(x_893.x, x_893.y, x_893.z));
  let x_896 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_895.x, x_895.y, x_895.z, x_896.w);
  let x_899 : f32 = vs_TEXCOORD9;
  let x_901 : f32 = x_22.x_ProjectionParams.y;
  u_xlat12 = (x_899 / x_901);
  let x_903 : f32 = u_xlat12;
  u_xlat12 = (-(x_903) + 1.0f);
  let x_906 : f32 = u_xlat12;
  let x_908 : f32 = x_22.x_ProjectionParams.z;
  u_xlat12 = (x_906 * x_908);
  let x_910 : f32 = u_xlat12;
  u_xlat12 = max(x_910, 0.0f);
  let x_912 : f32 = u_xlat12;
  let x_915 : f32 = x_22.unity_FogParams.x;
  u_xlat12 = (x_912 * x_915);
  let x_917 : f32 = u_xlat12;
  let x_918 : f32 = u_xlat12;
  u_xlat12 = (x_917 * -(x_918));
  let x_921 : f32 = u_xlat12;
  u_xlat12 = exp2(x_921);
  let x_923 : vec4<f32> = u_xlat0;
  let x_925 : f32 = u_xlat12;

...
fn main_1() {
  ...
  let x_556 : vec4<f32> = x_26.x_Grain_TilingParams;
  let x_560 : vec4<f32> = x_26.x_Grain_TilingParams;
  ...
  let x_574 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_570.x, x_570.y), x_573);
  u_xlat15 = x_574.w;
  let x_576 : f32 = u_xlat15;
  u_xlat15 = (x_576 + -0.5f);
  let x_579 : f32 = u_xlat15;
  let x_580 : f32 = u_xlat15;
  u_xlat15 = (x_579 + x_580);
  let x_582 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_582.x, x_582.y, x_582.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_591 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_591);
  let x_596 : f32 = x_26.x_Grain_Params.y;
  let x_598 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_596 * -(x_598)) + 1.0f);
  let x_604 : f32 = u_xlat15;
  let x_606 : vec4<f32> = u_xlat0;
  u_xlat6 = (vec3<f32>(x_604, x_604, x_604) * vec3<f32>(x_606.x, x_606.y, x_606.z));
  let x_609 : vec3<f32> = u_xlat6;
  let x_612 : vec2<f32> = x_26.x_Grain_Params;
  u_xlat6 = (x_609 * vec3<f32>(x_612.x, x_612.x, x_612.x));
  let x_615 : vec3<f32> = u_xlat6;
  let x_616 : vec4<f32> = u_xlat1;
  let x_619 : vec4<f32> = u_xlat0;
  let x_621 : vec3<f32> = ((x_615 * vec3<f32>(x_616.x, x_616.x, x_616.x)) + vec3<f32>(x_619.x, x_619.y, x_619.z));
  let x_622 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_621.x, x_621.y, x_621.z, x_622.w);
  let x_624 : vec4<f32> = u_xlat0;
  let x_626 : vec3<f32> = (vec3<f32>(x_624.x, x_624.y, x_624.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_627 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_626.x, x_626.y, x_626.z, x_627.w);
  let x_629 : vec4<f32> = u_xlat0;
  let x_632 : vec3<f32> = log2(abs(vec3<f32>(x_629.x, x_629.y, x_629.z)));
  let x_633 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_632.x, x_632.y, x_632.z, x_633.w);
  let x_635 : vec4<f32> = u_xlat2;
  let x_637 : vec3<f32> = (vec3<f32>(x_635.x, x_635.y, x_635.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_638 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_637.x, x_637.y, x_637.z, x_638.w);
  let x_640 : vec4<f32> = u_xlat2;
  let x_642 : vec3<f32> = exp2(vec3<f32>(x_640.x, x_640.y, x_640.z));
  let x_643 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_642.x, x_642.y, x_642.z, x_643.w);
  let x_645 : vec4<f32> = u_xlat2;
  let x_648 : vec3<f32> = ((vec3<f32>(x_645.x, x_645.y, x_645.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_649 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_648.x, x_648.y, x_648.z, x_649.w);
  let x_652 : vec4<f32> = u_xlat0;
  let x_654 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_652.x, x_652.y, x_652.z, x_652.x));
  u_xlatb0 = vec3<bool>(x_654.x, x_654.y, x_654.z);

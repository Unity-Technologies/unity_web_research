#version 440
#extension GL_ARB_explicit_attrib_location : require
#extension GL_ARB_shading_language_420pack : require

layout(origin_upper_left) in vec4 gl_FragCoord;
#define HLSLCC_ENABLE_UNIFORM_BUFFERS 1
#if HLSLCC_ENABLE_UNIFORM_BUFFERS
#define UNITY_UNIFORM
#else
#define UNITY_UNIFORM uniform
#endif
#define UNITY_SUPPORTS_UNIFORM_LOCATION 1
#if UNITY_SUPPORTS_UNIFORM_LOCATION
#define UNITY_LOCATION(x) layout(location = x)
#define UNITY_BINDING(x) layout(binding = x, std140)
#else
#define UNITY_LOCATION(x)
#define UNITY_BINDING(x) layout(std140)
#endif
precise vec4 u_xlat_precise_vec4;
precise ivec4 u_xlat_precise_ivec4;
precise bvec4 u_xlat_precise_bvec4;
precise uvec4 u_xlat_precise_uvec4;
const uvec4 ImmCB_0[] = uvec4[4] (
	uvec4(0x3F800000, 0x0, 0x0, 0x0),
	uvec4(0x0, 0x3F800000, 0x0, 0x0),
	uvec4(0x0, 0x0, 0x3F800000, 0x0),
	uvec4(0x0, 0x0, 0x0, 0x3F800000));
layout(set = 1, binding = 0, std140) uniform PGlobals {
	vec2 _GlobalMipBias;
	vec4 _MainLightPosition;
	vec4 _MainLightColor;
	vec4 _AdditionalLightsCount;
	vec3 _WorldSpaceCameraPos;
	vec4 _ProjectionParams;
	vec4 unity_OrthoParams;
	vec4 unity_FogParams;
	vec4 unity_FogColor;
	mat4x4 unity_MatrixV;
};
layout(set = 1, binding = 1, std140) uniform AdditionalLights {
	vec4 _AdditionalLightsPosition[32];
	vec4 _AdditionalLightsColor[32];
	vec4 _AdditionalLightsAttenuation[32];
	vec4 _AdditionalLightsSpotDir[32];
	vec4 _AdditionalLightsOcclusionProbes[32];
	float _AdditionalLightsLayerMasks[32];
};
layout(set = 1, binding = 2, std140) uniform UnityPerDraw {
	mat4x4 unity_ObjectToWorld;
	mat4x4 unity_WorldToObject;
	vec4 unity_LODFade;
	vec4 unity_WorldTransformParams;
	vec4 unity_RenderingLayer;
	vec4 unity_LightData;
	vec4 unity_LightIndices[2];
	vec4 unity_ProbesOcclusion;
	vec4 unity_SpecCube0_HDR;
	vec4 unity_SpecCube1_HDR;
	vec4 unity_SpecCube0_BoxMax;
	vec4 unity_SpecCube0_BoxMin;
	vec4 unity_SpecCube0_ProbePosition;
	vec4 unity_SpecCube1_BoxMax;
	vec4 unity_SpecCube1_BoxMin;
	vec4 unity_SpecCube1_ProbePosition;
	vec4 unity_LightmapST;
	vec4 unity_DynamicLightmapST;
	vec4 unity_SHAr;
	vec4 unity_SHAg;
	vec4 unity_SHAb;
	vec4 unity_SHBr;
	vec4 unity_SHBg;
	vec4 unity_SHBb;
	vec4 unity_SHC;
	vec4 unity_RendererBounds_Min;
	vec4 unity_RendererBounds_Max;
	mat4x4 unity_MatrixPreviousM;
	mat4x4 unity_MatrixPreviousMI;
	vec4 unity_MotionVectorsParams;
	vec4 unity_SpriteColor;
	vec4 unity_SpriteProps;
};
layout(set = 1, binding = 3, std140) uniform LightShadows {
	mat4x4 _MainLightWorldToShadow[5];
	vec4 _CascadeShadowSplitSpheres0;
	vec4 _CascadeShadowSplitSpheres1;
	vec4 _CascadeShadowSplitSpheres2;
	vec4 _CascadeShadowSplitSpheres3;
	vec4 _CascadeShadowSplitSphereRadii;
	vec4 _MainLightShadowOffset0;
	vec4 _MainLightShadowOffset1;
	vec4 _MainLightShadowParams;
	vec4 _MainLightShadowmapSize;
	vec4 _AdditionalShadowOffset0;
	vec4 _AdditionalShadowOffset1;
	vec4 _AdditionalShadowFadeParams;
	vec4 _AdditionalShadowmapSize;
	vec4 _AdditionalShadowParams[32];
	mat4x4 _AdditionalLightsWorldToShadow[32];
};
layout(set = 1, binding = 4, std140) uniform UnityPerMaterial {
	vec4 _SampleTexture2D_a490b3241735d68e9ee2d1f1bbfed8dc_Texture_1_Texture2D_TexelSize;
	vec4 _MainTex_TexelSize;
	vec4 Texture2D_DE8BF47E_TexelSize;
	float Vector1_6E11FCEA;
	float Vector1_A492C01C;
	float Vector1_8B35DE98;
	vec4 Texture2D_C005B064_TexelSize;
	vec4 Texture2D_D7D66558_TexelSize;
	float Vector1_90E376AD;
};
layout(set = 0, binding = 0) uniform mediump textureCube unity_SpecCube0;
layout(set = 0, binding = 1) uniform mediump texture2D _MainLightShadowmapTexture;
layout(set = 0, binding = 2) uniform mediump texture2D _AdditionalLightsShadowmapTexture;
layout(set = 0, binding = 3) uniform mediump texture2D _SampleTexture2D_a490b3241735d68e9ee2d1f1bbfed8dc_Texture_1_Texture2D;
layout(set = 0, binding = 4) uniform mediump texture2D _MainTex;
layout(set = 0, binding = 5) uniform mediump texture2D Texture2D_DE8BF47E;
layout(set = 0, binding = 6) uniform mediump texture2D Texture2D_C005B064;
layout(set = 0, binding = 7) uniform mediump texture2D Texture2D_D7D66558;
layout(location = 0) in highp  vec4 vs_INTERP4;
layout(location = 1) in highp  vec4 vs_INTERP5;
layout(location = 2) in highp  vec3 vs_INTERP7;
layout(location = 3) in highp  vec3 vs_INTERP8;
layout(location = 0) out highp vec4 SV_Target0;
vec4 u_xlat0;
uint u_xlatu0;
vec4 u_xlat1;
int u_xlati1;
uint u_xlatu1;
vec3 u_xlat2;
vec3 u_xlat3;
vec4 u_xlat4;
vec3 u_xlat5;
bvec4 u_xlatb5;
vec4 u_xlat6;
vec4 u_xlat7;
vec4 u_xlat8;
vec3 u_xlat9;
vec4 u_xlat10;
bvec4 u_xlatb10;
vec3 u_xlat11;
bvec2 u_xlatb11;
vec2 u_xlat12;
bool u_xlatb12;
vec3 u_xlat15;
vec3 u_xlat17;
vec3 u_xlat18;
int u_xlati18;
uint u_xlatu18;
bool u_xlatb18;
float u_xlat21;
float u_xlat22;
float u_xlat23;
bool u_xlatb23;
float u_xlat29;
int u_xlati29;
float u_xlat33;
float u_xlat34;
float u_xlat35;
float u_xlat36;
uint u_xlatu36;
float u_xlat37;
float u_xlat38;
float u_xlat40;
float u_xlat41;
int u_xlati41;
float u_xlat42;
bool u_xlatb42;
float u_xlat43;
layout(set = 0, binding = 8) uniform mediump  sampler samplerunity_SpecCube0;
layout(set = 0, binding = 9) uniform mediump  samplerShadow sampler_LinearClampCompare;
layout(set = 0, binding = 10) uniform mediump  sampler sampler_SampleTexture2D_a490b3241735d68e9ee2d1f1bbfed8dc_Texture_1_Texture2D;
layout(set = 0, binding = 11) uniform mediump  sampler sampler_MainTex;
layout(set = 0, binding = 12) uniform mediump  sampler samplerTexture2D_DE8BF47E;
layout(set = 0, binding = 13) uniform mediump  sampler samplerTexture2D_C005B064;
layout(set = 0, binding = 14) uniform mediump  sampler samplerTexture2D_D7D66558;
void main()
{
    u_xlat0.x = dot(vs_INTERP8.xyz, vs_INTERP8.xyz);
    u_xlat0.x = sqrt(u_xlat0.x);
    u_xlat0.x = float(1.0) / u_xlat0.x;
    u_xlatb11.x = 0.0<vs_INTERP4.w;
    u_xlatb11.y = unity_WorldTransformParams.w>=0.0;
    u_xlat11.x = (u_xlatb11.x) ? float(1.0) : float(-1.0);
    u_xlat11.y = (u_xlatb11.y) ? float(1.0) : float(-1.0);
    u_xlat11.x = u_xlat11.y * u_xlat11.x;
    u_xlat1.xyz = vs_INTERP4.yzx * vs_INTERP8.zxy;
    u_xlat1.xyz = vs_INTERP8.yzx * vs_INTERP4.zxy + (-u_xlat1.xyz);
    u_xlat0.yzw = u_xlat11.xxx * u_xlat1.xyz;
    u_xlat1.xyz = u_xlat0.xxx * vs_INTERP8.xyz;
    u_xlat2.xyz = u_xlat0.xxx * vs_INTERP4.xyz;
    u_xlat3.xyz = u_xlat0.yzw * u_xlat0.xxx;
    u_xlat4 = texture(sampler2D(_MainTex, sampler_MainTex), vs_INTERP5.xy, _GlobalMipBias.x);
    u_xlat0.x = u_xlat4.w + -0.5;
    u_xlat34 = u_xlat0.x * 4.0 + vs_INTERP7.y;
    u_xlat34 = u_xlat34 * 0.330000013;
    u_xlat35 = vs_INTERP7.y + 1.0;
    u_xlat35 = u_xlat35 * -1.33333337 + 1.0;
    u_xlat34 = max(u_xlat34, u_xlat35);
    u_xlat1.w = max(u_xlat34, 0.100000001);
    u_xlat5.xy = vs_INTERP5.xy * vec2(4.0, 4.0);
    u_xlat6.xyz = texture(sampler2D(Texture2D_D7D66558, samplerTexture2D_D7D66558), u_xlat5.xy, _GlobalMipBias.x).xyz;
    u_xlat35 = (-Vector1_8B35DE98) * 0.0111109996 + 0.949999988;
    u_xlat36 = Vector1_A492C01C + -10.0;
    u_xlat36 = (-u_xlat36) + vs_INTERP7.y;
    u_xlat36 = u_xlat36 * 0.0500000007;
    u_xlat36 = clamp(u_xlat36, 0.0, 1.0);
    u_xlat37 = u_xlat36 * -2.0 + 3.0;
    u_xlat36 = u_xlat36 * u_xlat36;
    u_xlat36 = u_xlat36 * u_xlat37;
    u_xlat7 = texture(sampler2D(Texture2D_DE8BF47E, samplerTexture2D_DE8BF47E), vs_INTERP5.xy, _GlobalMipBias.x);
    u_xlat18.xyz = u_xlat7.wyz * vec3(2.0, 2.0, 2.0) + vec3(-1.0, -1.0, -1.0);
    u_xlat3.xyz = u_xlat3.xyz * u_xlat18.yyy;
    u_xlat2.xyz = u_xlat18.xxx * u_xlat2.xyz + u_xlat3.xyz;
    u_xlat1.xyz = u_xlat18.zzz * u_xlat1.xyz + u_xlat2.xyz;
    u_xlat1.x = dot(u_xlat1.xyz, u_xlat1.xyz);
    u_xlat1.x = max(u_xlat1.x, 1.17549435e-38);
    u_xlat1.x = inversesqrt(u_xlat1.x);
    u_xlat1.x = u_xlat1.x * u_xlat1.y;
    u_xlat1.x = u_xlat36 * u_xlat1.x + (-u_xlat35);
    u_xlat1.x = u_xlat1.x * 10.0;
    u_xlat1.x = clamp(u_xlat1.x, 0.0, 1.0);
    u_xlat12.x = u_xlat1.x * -2.0 + 3.0;
    u_xlat1.x = u_xlat1.x * u_xlat1.x;
    u_xlat1.x = u_xlat1.x * u_xlat12.x;
    u_xlat1.xw = min(u_xlat1.xw, vec2(1.0, 1.0));
    u_xlat1.x = (-u_xlat1.x) + 1.0;
    u_xlat2.xyz = u_xlat4.xyz + (-u_xlat6.xyz);
    u_xlat2.xyz = u_xlat1.xxx * u_xlat2.xyz + u_xlat6.xyz;
    u_xlat3.xyz = u_xlat1.www * u_xlat2.xyz;
    u_xlat15.xyz = texture(sampler2D(Texture2D_C005B064, samplerTexture2D_C005B064), u_xlat5.xy, _GlobalMipBias.x).xyw;
    u_xlat15.x = u_xlat15.x * u_xlat15.z;
    u_xlat5.xy = u_xlat15.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat12.x = dot(u_xlat5.xy, u_xlat5.xy);
    u_xlat12.x = min(u_xlat12.x, 1.0);
    u_xlat12.x = (-u_xlat12.x) + 1.0;
    u_xlat12.x = sqrt(u_xlat12.x);
    u_xlat5.z = max(u_xlat12.x, 1.00000002e-16);
    u_xlat12.x = dot(unity_ObjectToWorld[0].xyz, unity_ObjectToWorld[0].xyz);
    u_xlat6.x = sqrt(u_xlat12.x);
    u_xlat12.x = dot(unity_ObjectToWorld[1].xyz, unity_ObjectToWorld[1].xyz);
    u_xlat6.y = sqrt(u_xlat12.x);
    u_xlat12.x = dot(unity_ObjectToWorld[2].xyz, unity_ObjectToWorld[2].xyz);
    u_xlat6.z = sqrt(u_xlat12.x);
    u_xlat12.x = dot(u_xlat6.xyz, u_xlat6.xyz);
    u_xlat12.x = sqrt(u_xlat12.x);
    u_xlat12.x = u_xlat12.x * Vector1_90E376AD;
    u_xlat12.x = u_xlat12.x * 30.0;
    u_xlat12.xy = u_xlat12.xx * vs_INTERP5.xy;
    u_xlat15.xyz = texture(sampler2D(_SampleTexture2D_a490b3241735d68e9ee2d1f1bbfed8dc_Texture_1_Texture2D, sampler_SampleTexture2D_a490b3241735d68e9ee2d1f1bbfed8dc_Texture_1_Texture2D), u_xlat12.xy, _GlobalMipBias.x).xyw;
    u_xlat15.x = u_xlat15.x * u_xlat15.z;
    u_xlat12.xy = u_xlat15.xy * vec2(2.0, 2.0) + vec2(-1.0, -1.0);
    u_xlat35 = dot(u_xlat12.xy, u_xlat12.xy);
    u_xlat35 = min(u_xlat35, 1.0);
    u_xlat35 = (-u_xlat35) + 1.0;
    u_xlat35 = sqrt(u_xlat35);
    u_xlat35 = max(u_xlat35, 1.00000002e-16);
    u_xlat6.xy = u_xlat12.xy + u_xlat18.xy;
    u_xlat6.z = u_xlat35 * u_xlat18.z;
    u_xlat12.x = dot(u_xlat6.xyz, u_xlat6.xyz);
    u_xlat12.x = max(u_xlat12.x, 6.10351562e-05);
    u_xlat12.x = inversesqrt(u_xlat12.x);
    u_xlat15.xyz = u_xlat6.xyz * u_xlat12.xxx + (-u_xlat5.xyz);
    u_xlat4.yzw = u_xlat1.xxx * u_xlat15.xyz + u_xlat5.xyz;
    u_xlat0.x = -abs(u_xlat0.x) + 1.0;
    u_xlat0 = u_xlat0 * u_xlat4.xzzz;
    u_xlat0.x = u_xlat0.x * Vector1_6E11FCEA;
    u_xlat0.x = u_xlat1.x * u_xlat0.x + -0.850000024;
    u_xlat0.x = u_xlat1.w * u_xlat0.x + 0.850000024;
    u_xlat0.x = clamp(u_xlat0.x, 0.0, 1.0);
    u_xlat11.xyz = u_xlat4.yyy * vs_INTERP4.xyz + u_xlat0.yzw;
    u_xlat11.xyz = u_xlat4.www * vs_INTERP8.xyz + u_xlat11.xyz;
    u_xlat1.x = dot(u_xlat11.xyz, u_xlat11.xyz);
    u_xlat1.x = inversesqrt(u_xlat1.x);
    u_xlat4.xyz = u_xlat11.xyz * u_xlat1.xxx;
    u_xlatb11.x = unity_OrthoParams.w==0.0;
    u_xlat1.xyz = (-vs_INTERP7.xyz) + _WorldSpaceCameraPos.xyz;
    u_xlat22 = dot(u_xlat1.xyz, u_xlat1.xyz);
    u_xlat22 = inversesqrt(u_xlat22);
    u_xlat1.xyz = vec3(u_xlat22) * u_xlat1.xyz;
    u_xlat5.x = unity_MatrixV[0].z;
    u_xlat5.y = unity_MatrixV[1].z;
    u_xlat5.z = unity_MatrixV[2].z;
    u_xlat11.xyz = (u_xlatb11.x) ? u_xlat1.xyz : u_xlat5.xyz;
    u_xlat1.xyz = vs_INTERP7.xyz + (-_CascadeShadowSplitSpheres0.xyz);
    u_xlat5.xyz = vs_INTERP7.xyz + (-_CascadeShadowSplitSpheres1.xyz);
    u_xlat6.xyz = vs_INTERP7.xyz + (-_CascadeShadowSplitSpheres2.xyz);
    u_xlat18.xyz = vs_INTERP7.xyz + (-_CascadeShadowSplitSpheres3.xyz);
    u_xlat8.x = dot(u_xlat1.xyz, u_xlat1.xyz);
    u_xlat8.y = dot(u_xlat5.xyz, u_xlat5.xyz);
    u_xlat8.z = dot(u_xlat6.xyz, u_xlat6.xyz);
    u_xlat8.w = dot(u_xlat18.xyz, u_xlat18.xyz);
    u_xlatb5 = lessThan(u_xlat8, _CascadeShadowSplitSphereRadii);
    u_xlat6.x = u_xlatb5.x ? float(1.0) : 0.0;
    u_xlat6.y = u_xlatb5.y ? float(1.0) : 0.0;
    u_xlat6.z = u_xlatb5.z ? float(1.0) : 0.0;
    u_xlat6.w = u_xlatb5.w ? float(1.0) : 0.0;
;
    u_xlat1.x = (u_xlatb5.x) ? float(-1.0) : float(-0.0);
    u_xlat1.y = (u_xlatb5.y) ? float(-1.0) : float(-0.0);
    u_xlat1.z = (u_xlatb5.z) ? float(-1.0) : float(-0.0);
    u_xlat1.xyz = u_xlat1.xyz + u_xlat6.yzw;
    u_xlat6.yzw = max(u_xlat1.xyz, vec3(0.0, 0.0, 0.0));
    u_xlat1.x = dot(u_xlat6, vec4(4.0, 3.0, 2.0, 1.0));
    u_xlat1.x = (-u_xlat1.x) + 4.0;
    u_xlatu1 = uint(u_xlat1.x);
    u_xlati1 = int(u_xlatu1) << (2 & int(0x1F));
    u_xlat5.xyz = vs_INTERP7.yyy * _MainLightWorldToShadow[(u_xlati1 + 1) / 4][(u_xlati1 + 1) % 4].xyz;
    u_xlat5.xyz = _MainLightWorldToShadow[u_xlati1 / 4][u_xlati1 % 4].xyz * vs_INTERP7.xxx + u_xlat5.xyz;
    u_xlat5.xyz = _MainLightWorldToShadow[(u_xlati1 + 2) / 4][(u_xlati1 + 2) % 4].xyz * vs_INTERP7.zzz + u_xlat5.xyz;
    u_xlat1.xyz = u_xlat5.xyz + _MainLightWorldToShadow[(u_xlati1 + 3) / 4][(u_xlati1 + 3) % 4].xyz;
    u_xlat35 = vs_INTERP7.y * unity_MatrixV[1].z;
    u_xlat35 = unity_MatrixV[0].z * vs_INTERP7.x + u_xlat35;
    u_xlat35 = unity_MatrixV[2].z * vs_INTERP7.z + u_xlat35;
    u_xlat35 = u_xlat35 + unity_MatrixV[3].z;
    u_xlat35 = (-u_xlat35) + (-_ProjectionParams.y);
    u_xlat35 = max(u_xlat35, 0.0);
    u_xlat35 = u_xlat35 * unity_FogParams.x;
    u_xlat4.w = 1.0;
    u_xlat5.x = dot(unity_SHAr, u_xlat4);
    u_xlat5.y = dot(unity_SHAg, u_xlat4);
    u_xlat5.z = dot(unity_SHAb, u_xlat4);
    u_xlat6 = u_xlat4.yzzx * u_xlat4.xyzz;
    u_xlat8.x = dot(unity_SHBr, u_xlat6);
    u_xlat8.y = dot(unity_SHBg, u_xlat6);
    u_xlat8.z = dot(unity_SHBb, u_xlat6);
    u_xlat36 = u_xlat4.y * u_xlat4.y;
    u_xlat36 = u_xlat4.x * u_xlat4.x + (-u_xlat36);
    u_xlat6.xyz = unity_SHC.xyz * vec3(u_xlat36) + u_xlat8.xyz;
    u_xlat5.xyz = u_xlat5.xyz + u_xlat6.xyz;
    u_xlat5.xyz = max(u_xlat5.xyz, vec3(0.0, 0.0, 0.0));
    u_xlat3.xyz = u_xlat3.xyz * vec3(0.863999963, 0.863999963, 0.863999963);
    u_xlat2.xyz = u_xlat1.www * u_xlat2.xyz + vec3(-0.0399999991, -0.0399999991, -0.0399999991);
    u_xlat2.xyz = u_xlat2.xyz * vec3(0.100000001, 0.100000001, 0.100000001) + vec3(0.0399999991, 0.0399999991, 0.0399999991);
    u_xlat34 = (-u_xlat0.x) + 1.0;
    u_xlat36 = u_xlat34 * u_xlat34;
    u_xlat36 = max(u_xlat36, 0.0078125);
    u_xlat37 = u_xlat36 * u_xlat36;
    u_xlat0.x = u_xlat0.x + 0.136000037;
    u_xlat0.x = min(u_xlat0.x, 1.0);
    u_xlat38 = u_xlat36 * 4.0 + 2.0;
    u_xlat6.x = min(u_xlat7.x, 1.0);
    vec3 txVec0 = vec3(u_xlat1.xy,u_xlat1.z);
    u_xlat1.x = textureLod(sampler2DShadow(_MainLightShadowmapTexture, sampler_LinearClampCompare), txVec0, 0.0);
    u_xlat12.x = (-_MainLightShadowParams.x) + 1.0;
    u_xlat1.x = u_xlat1.x * _MainLightShadowParams.x + u_xlat12.x;
    u_xlatb12 = 0.0>=u_xlat1.z;
    u_xlatb23 = u_xlat1.z>=1.0;
    u_xlatb12 = u_xlatb23 || u_xlatb12;
    u_xlat1.x = (u_xlatb12) ? 1.0 : u_xlat1.x;
    u_xlat17.xyz = vs_INTERP7.xyz + (-_WorldSpaceCameraPos.xyz);
    u_xlat12.x = dot(u_xlat17.xyz, u_xlat17.xyz);
    u_xlat23 = u_xlat12.x * _MainLightShadowParams.z + _MainLightShadowParams.w;
    u_xlat23 = clamp(u_xlat23, 0.0, 1.0);
    u_xlat17.x = (-u_xlat1.x) + 1.0;
    u_xlat1.x = u_xlat23 * u_xlat17.x + u_xlat1.x;
    u_xlat23 = dot((-u_xlat11.xyz), u_xlat4.xyz);
    u_xlat23 = u_xlat23 + u_xlat23;
    u_xlat17.xyz = u_xlat4.xyz * (-vec3(u_xlat23)) + (-u_xlat11.xyz);
    u_xlat23 = dot(u_xlat4.xyz, u_xlat11.xyz);
    u_xlat23 = clamp(u_xlat23, 0.0, 1.0);
    u_xlat23 = (-u_xlat23) + 1.0;
    u_xlat23 = u_xlat23 * u_xlat23;
    u_xlat23 = u_xlat23 * u_xlat23;
    u_xlat7.x = (-u_xlat34) * 0.699999988 + 1.70000005;
    u_xlat34 = u_xlat34 * u_xlat7.x;
    u_xlat34 = u_xlat34 * 6.0;
    u_xlat7 = textureLod(samplerCube(unity_SpecCube0, samplerunity_SpecCube0), u_xlat17.xyz, u_xlat34);
    u_xlat34 = u_xlat7.w + -1.0;
    u_xlat34 = unity_SpecCube0_HDR.w * u_xlat34 + 1.0;
    u_xlat34 = max(u_xlat34, 0.0);
    u_xlat34 = log2(u_xlat34);
    u_xlat34 = u_xlat34 * unity_SpecCube0_HDR.y;
    u_xlat34 = exp2(u_xlat34);
    u_xlat34 = u_xlat34 * unity_SpecCube0_HDR.x;
    u_xlat17.xyz = u_xlat7.xyz * vec3(u_xlat34);
    u_xlat7.xy = vec2(u_xlat36) * vec2(u_xlat36) + vec2(-1.0, 1.0);
    u_xlat34 = float(1.0) / u_xlat7.y;
    u_xlat18.xyz = (-u_xlat2.xyz) + u_xlat0.xxx;
    u_xlat18.xyz = vec3(u_xlat23) * u_xlat18.xyz + u_xlat2.xyz;
    u_xlat18.xyz = vec3(u_xlat34) * u_xlat18.xyz;
    u_xlat17.xyz = u_xlat17.xyz * u_xlat18.xyz;
    u_xlat5.xyz = u_xlat5.xyz * u_xlat3.xyz + u_xlat17.xyz;
    u_xlat0.x = u_xlat1.x * unity_LightData.z;
    u_xlat1.x = dot(u_xlat4.xyz, _MainLightPosition.xyz);
    u_xlat1.x = clamp(u_xlat1.x, 0.0, 1.0);
    u_xlat0.x = u_xlat0.x * u_xlat1.x;
    u_xlat1.xzw = u_xlat0.xxx * _MainLightColor.xyz;
    u_xlat17.xyz = u_xlat11.xyz + _MainLightPosition.xyz;
    u_xlat0.x = dot(u_xlat17.xyz, u_xlat17.xyz);
    u_xlat0.x = max(u_xlat0.x, 1.17549435e-38);
    u_xlat0.x = inversesqrt(u_xlat0.x);
    u_xlat17.xyz = u_xlat0.xxx * u_xlat17.xyz;
    u_xlat0.x = dot(u_xlat4.xyz, u_xlat17.xyz);
    u_xlat0.x = clamp(u_xlat0.x, 0.0, 1.0);
    u_xlat36 = dot(_MainLightPosition.xyz, u_xlat17.xyz);
    u_xlat36 = clamp(u_xlat36, 0.0, 1.0);
    u_xlat0.x = u_xlat0.x * u_xlat0.x;
    u_xlat0.x = u_xlat0.x * u_xlat7.x + 1.00001001;
    u_xlat36 = u_xlat36 * u_xlat36;
    u_xlat0.x = u_xlat0.x * u_xlat0.x;
    u_xlat36 = max(u_xlat36, 0.100000001);
    u_xlat0.x = u_xlat0.x * u_xlat36;
    u_xlat0.x = u_xlat38 * u_xlat0.x;
    u_xlat0.x = u_xlat37 / u_xlat0.x;
    u_xlat17.xyz = u_xlat2.xyz * u_xlat0.xxx + u_xlat3.xyz;
    u_xlat1.xzw = u_xlat1.xzw * u_xlat17.xyz;
    u_xlat0.x = min(_AdditionalLightsCount.x, unity_LightData.y);
    u_xlatu0 =  uint(int(u_xlat0.x));
    u_xlat12.x = u_xlat12.x * _AdditionalShadowFadeParams.x + _AdditionalShadowFadeParams.y;
    u_xlat12.x = clamp(u_xlat12.x, 0.0, 1.0);
    u_xlat17.x = float(0.0);
    u_xlat17.y = float(0.0);
    u_xlat17.z = float(0.0);
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu0 ; u_xlatu_loop_1++)
    {
        u_xlatu18 = u_xlatu_loop_1 >> (2u & uint(0x1F));
        u_xlati29 = int(u_xlatu_loop_1 & 3u);
        u_xlat18.x = dot(unity_LightIndices[int(u_xlatu18)], uintBitsToFloat(ImmCB_0[u_xlati29]));
        u_xlati18 = int(u_xlat18.x);
        u_xlat8.xyz = (-vs_INTERP7.xyz) * _AdditionalLightsPosition[u_xlati18].www + _AdditionalLightsPosition[u_xlati18].xyz;
        u_xlat29 = dot(u_xlat8.xyz, u_xlat8.xyz);
        u_xlat29 = max(u_xlat29, 6.10351562e-05);
        u_xlat40 = inversesqrt(u_xlat29);
        u_xlat9.xyz = vec3(u_xlat40) * u_xlat8.xyz;
        u_xlat41 = float(1.0) / float(u_xlat29);
        u_xlat29 = u_xlat29 * _AdditionalLightsAttenuation[u_xlati18].x;
        u_xlat29 = (-u_xlat29) * u_xlat29 + 1.0;
        u_xlat29 = max(u_xlat29, 0.0);
        u_xlat29 = u_xlat29 * u_xlat29;
        u_xlat29 = u_xlat29 * u_xlat41;
        u_xlat41 = dot(_AdditionalLightsSpotDir[u_xlati18].xyz, u_xlat9.xyz);
        u_xlat41 = u_xlat41 * _AdditionalLightsAttenuation[u_xlati18].z + _AdditionalLightsAttenuation[u_xlati18].w;
        u_xlat41 = clamp(u_xlat41, 0.0, 1.0);
        u_xlat41 = u_xlat41 * u_xlat41;
        u_xlat29 = u_xlat29 * u_xlat41;
        u_xlati41 = int(_AdditionalShadowParams[u_xlati18].w);
        u_xlatb42 = u_xlati41>=0;
        if(u_xlatb42){
            u_xlatb42 = vec4(0.0, 0.0, 0.0, 0.0)!=vec4(_AdditionalShadowParams[u_xlati18].z);
            if(u_xlatb42){
                u_xlatb10.xyz = greaterThanEqual(abs(u_xlat9.zzyz), abs(u_xlat9.xyxx)).xyz;
                u_xlatb42 = u_xlatb10.y && u_xlatb10.x;
                u_xlatb10.xyw = lessThan((-u_xlat9.zyzx), vec4(0.0, 0.0, 0.0, 0.0)).xyw;
                u_xlat10.x = (u_xlatb10.x) ? float(5.0) : float(4.0);
                u_xlat10.y = (u_xlatb10.y) ? float(3.0) : float(2.0);
                u_xlat43 = u_xlatb10.w ? 1.0 : float(0.0);
                u_xlat21 = (u_xlatb10.z) ? u_xlat10.y : u_xlat43;
                u_xlat42 = (u_xlatb42) ? u_xlat10.x : u_xlat21;
                u_xlat10.x = trunc(_AdditionalShadowParams[u_xlati18].w);
                u_xlat42 = u_xlat42 + u_xlat10.x;
                u_xlati41 = int(u_xlat42);
            }
            u_xlati41 = u_xlati41 << (2 & int(0x1F));
            u_xlat10 = vs_INTERP7.yyyy * _AdditionalLightsWorldToShadow[(u_xlati41 + 1) / 4][(u_xlati41 + 1) % 4];
            u_xlat10 = _AdditionalLightsWorldToShadow[u_xlati41 / 4][u_xlati41 % 4] * vs_INTERP7.xxxx + u_xlat10;
            u_xlat10 = _AdditionalLightsWorldToShadow[(u_xlati41 + 2) / 4][(u_xlati41 + 2) % 4] * vs_INTERP7.zzzz + u_xlat10;
            u_xlat10 = u_xlat10 + _AdditionalLightsWorldToShadow[(u_xlati41 + 3) / 4][(u_xlati41 + 3) % 4];
            u_xlat10.xyz = u_xlat10.xyz / u_xlat10.www;
            vec3 txVec1 = vec3(u_xlat10.xy,u_xlat10.z);
            u_xlat41 = textureLod(sampler2DShadow(_AdditionalLightsShadowmapTexture, sampler_LinearClampCompare), txVec1, 0.0);
            u_xlat42 = 1.0 + (-_AdditionalShadowParams[u_xlati18].x);
            u_xlat41 = u_xlat41 * _AdditionalShadowParams[u_xlati18].x + u_xlat42;
            u_xlatb42 = 0.0>=u_xlat10.z;
            u_xlatb10.x = u_xlat10.z>=1.0;
            u_xlatb42 = u_xlatb42 || u_xlatb10.x;
            u_xlat41 = (u_xlatb42) ? 1.0 : u_xlat41;
        } else {
            u_xlat41 = 1.0;
        }
        u_xlat42 = (-u_xlat41) + 1.0;
        u_xlat41 = u_xlat12.x * u_xlat42 + u_xlat41;
        u_xlat29 = u_xlat29 * u_xlat41;
        u_xlat41 = dot(u_xlat4.xyz, u_xlat9.xyz);
        u_xlat41 = clamp(u_xlat41, 0.0, 1.0);
        u_xlat29 = u_xlat29 * u_xlat41;
        u_xlat10.xyz = vec3(u_xlat29) * _AdditionalLightsColor[u_xlati18].xyz;
        u_xlat18.xyz = u_xlat8.xyz * vec3(u_xlat40) + u_xlat11.xyz;
        u_xlat8.x = dot(u_xlat18.xyz, u_xlat18.xyz);
        u_xlat8.x = max(u_xlat8.x, 1.17549435e-38);
        u_xlat8.x = inversesqrt(u_xlat8.x);
        u_xlat18.xyz = u_xlat18.xyz * u_xlat8.xxx;
        u_xlat8.x = dot(u_xlat4.xyz, u_xlat18.xyz);
        u_xlat8.x = clamp(u_xlat8.x, 0.0, 1.0);
        u_xlat18.x = dot(u_xlat9.xyz, u_xlat18.xyz);
        u_xlat18.x = clamp(u_xlat18.x, 0.0, 1.0);
        u_xlat29 = u_xlat8.x * u_xlat8.x;
        u_xlat29 = u_xlat29 * u_xlat7.x + 1.00001001;
        u_xlat18.x = u_xlat18.x * u_xlat18.x;
        u_xlat29 = u_xlat29 * u_xlat29;
        u_xlat18.x = max(u_xlat18.x, 0.100000001);
        u_xlat18.x = u_xlat18.x * u_xlat29;
        u_xlat18.x = u_xlat38 * u_xlat18.x;
        u_xlat18.x = u_xlat37 / u_xlat18.x;
        u_xlat18.xyz = u_xlat2.xyz * u_xlat18.xxx + u_xlat3.xyz;
        u_xlat17.xyz = u_xlat18.xyz * u_xlat10.xyz + u_xlat17.xyz;
    }
    u_xlat0.xyz = u_xlat5.xyz * u_xlat6.xxx + u_xlat1.xzw;
    u_xlat0.xyz = u_xlat17.xyz + u_xlat0.xyz;
    u_xlat33 = u_xlat35 * (-u_xlat35);
    u_xlat33 = exp2(u_xlat33);
    u_xlat0.xyz = u_xlat0.xyz + (-unity_FogColor.xyz);
    SV_Target0.xyz = vec3(u_xlat33) * u_xlat0.xyz + unity_FogColor.xyz;
    SV_Target0.w = 1.0;
    return;
}


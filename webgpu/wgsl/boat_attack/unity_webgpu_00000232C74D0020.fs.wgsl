...
fn main_1() {
  ...
  let x_829 : vec3<f32> = sqrt(vec3<f32>(x_827.x, x_827.y, x_827.z));
  let x_830 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_829.x, x_829.y, x_829.z, x_830.w);
  let x_832 : vec2<f32> = vs_TEXCOORD0;
  let x_835 : vec4<f32> = x_38.x_Dithering_Params;
  let x_839 : vec4<f32> = x_38.x_Dithering_Params;
  let x_841 : vec2<f32> = ((x_832 * vec2<f32>(x_835.x, x_835.y)) + vec2<f32>(x_839.z, x_839.w));
  let x_842 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_841.x, x_841.y, x_842.z, x_842.w);
  let x_849 : vec4<f32> = u_xlat1;
  let x_852 : f32 = x_38.x_GlobalMipBias.x;
  let x_853 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_849.x, x_849.y), x_852);
  u_xlat15 = x_853.w;
  let x_855 : f32 = u_xlat15;
  u_xlat15 = ((x_855 * 2.0f) + -1.0f);
  let x_858 : f32 = u_xlat15;
  u_xlatb1.x = (x_858 >= 0.0f);
  let x_862 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_862);
  let x_865 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_865)) + 1.0f);
  ...
  u_xlat15 = (x_874 * x_876);
  let x_878 : vec4<f32> = u_xlat0;
  let x_880 : vec3<f32> = (vec3<f32>(x_878.x, x_878.y, x_878.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_907 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_905.x, x_905.y, x_905.z, x_905.x));
  ...
  let x_952 : vec3<f32> = ((vec3<f32>(x_945, x_945, x_945) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_950.x, x_950.y, x_950.z));
  let x_953 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_952.x, x_952.y, x_952.z, x_953.w);
  let x_955 : vec4<f32> = u_xlat0;
  let x_957 : vec3<f32> = (vec3<f32>(x_955.x, x_955.y, x_955.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_958 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_957.x, x_957.y, x_957.z, x_958.w);
  let x_960 : vec4<f32> = u_xlat0;
  let x_962 : vec3<f32> = (vec3<f32>(x_960.x, x_960.y, x_960.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_963 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_962.x, x_962.y, x_962.z, x_963.w);
  let x_965 : vec4<f32> = u_xlat2;
  let x_967 : vec3<f32> = (vec3<f32>(x_965.x, x_965.y, x_965.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_968 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_967.x, x_967.y, x_967.z, x_968.w);
  let x_970 : vec4<f32> = u_xlat2;
  let x_973 : vec3<f32> = log2(abs(vec3<f32>(x_970.x, x_970.y, x_970.z)));
  let x_974 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_973.x, x_973.y, x_973.z, x_974.w);
  let x_976 : vec4<f32> = u_xlat2;
  let x_978 : vec3<f32> = (vec3<f32>(x_976.x, x_976.y, x_976.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_979 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_978.x, x_978.y, x_978.z, x_979.w);
  let x_981 : vec4<f32> = u_xlat2;
  let x_983 : vec3<f32> = exp2(vec3<f32>(x_981.x, x_981.y, x_981.z));
  let x_984 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_983.x, x_983.y, x_983.z, x_984.w);
  let x_986 : vec4<f32> = u_xlat0;
  let x_988 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_986.x, x_986.y, x_986.z, x_986.x));
  ...
  let x_1031 : vec3<f32> = max(vec3<f32>(x_1028.x, x_1028.y, x_1028.z), vec3<f32>(0.0f, 0.0f, 0.0f));

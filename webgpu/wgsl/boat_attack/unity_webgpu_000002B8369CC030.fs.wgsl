...
fn main_1() {
  ...
  let x_720 : vec2<f32> = vs_TEXCOORD0;
  let x_723 : vec4<f32> = x_38.x_Dithering_Params;
  let x_727 : vec4<f32> = x_38.x_Dithering_Params;
  let x_729 : vec2<f32> = ((x_720 * vec2<f32>(x_723.x, x_723.y)) + vec2<f32>(x_727.z, x_727.w));
  let x_730 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_729.x, x_729.y, x_730.z, x_730.w);
  let x_737 : vec4<f32> = u_xlat1;
  let x_740 : f32 = x_38.x_GlobalMipBias.x;
  let x_741 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_737.x, x_737.y), x_740);
  u_xlat15 = x_741.w;
  let x_743 : f32 = u_xlat15;
  u_xlat15 = ((x_743 * 2.0f) + -1.0f);
  let x_746 : f32 = u_xlat15;
  u_xlatb1.x = (x_746 >= 0.0f);
  let x_750 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_750);
  let x_753 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_753)) + 1.0f);
  ...
  u_xlat15 = (x_762 * x_764);
  let x_766 : vec4<f32> = u_xlat0;
  let x_768 : vec3<f32> = (vec3<f32>(x_766.x, x_766.y, x_766.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));

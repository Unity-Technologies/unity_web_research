...
fn main_1() {
  ...
  let x_512 : vec4<f32> = x_13.x_Dithering_Params;
  let x_516 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_530 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_526.x, x_526.y), x_529);
  u_xlat15 = x_530.w;
  let x_532 : f32 = u_xlat15;
  u_xlat15 = ((x_532 * 2.0f) + -1.0f);
  let x_537 : f32 = u_xlat15;
  u_xlatb1.x = (x_537 >= 0.0f);
  let x_541 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_541);
  let x_544 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_544)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_557.x, x_557.y, x_557.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_560 : vec4<f32> = u_xlat0;
  let x_563 : vec3<f32> = log2(abs(vec3<f32>(x_560.x, x_560.y, x_560.z)));
  let x_564 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_563.x, x_563.y, x_563.z, x_564.w);
  let x_566 : vec4<f32> = u_xlat2;
  let x_568 : vec3<f32> = (vec3<f32>(x_566.x, x_566.y, x_566.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_569 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_568.x, x_568.y, x_568.z, x_569.w);
  let x_571 : vec4<f32> = u_xlat2;
  let x_573 : vec3<f32> = exp2(vec3<f32>(x_571.x, x_571.y, x_571.z));
  let x_574 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_573.x, x_573.y, x_573.z, x_574.w);
  let x_576 : vec4<f32> = u_xlat2;
  let x_579 : vec3<f32> = ((vec3<f32>(x_576.x, x_576.y, x_576.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_580 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_579.x, x_579.y, x_579.z, x_580.w);
  let x_583 : vec4<f32> = u_xlat0;
  let x_585 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_583.x, x_583.y, x_583.z, x_583.x));
  ...
  let x_630 : vec3<f32> = ((vec3<f32>(x_623, x_623, x_623) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_628.x, x_628.y, x_628.z));
  let x_631 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_630.x, x_630.y, x_630.z, x_631.w);
  let x_633 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_633.x, x_633.y, x_633.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_636 : vec4<f32> = u_xlat0;
  let x_638 : vec3<f32> = (vec3<f32>(x_636.x, x_636.y, x_636.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_639 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_638.x, x_638.y, x_638.z, x_639.w);
  let x_641 : vec4<f32> = u_xlat2;
  let x_643 : vec3<f32> = (vec3<f32>(x_641.x, x_641.y, x_641.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_644 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_643.x, x_643.y, x_643.z, x_644.w);
  let x_646 : vec4<f32> = u_xlat2;
  let x_649 : vec3<f32> = log2(abs(vec3<f32>(x_646.x, x_646.y, x_646.z)));
  let x_650 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_649.x, x_649.y, x_649.z, x_650.w);
  let x_652 : vec4<f32> = u_xlat2;
  let x_654 : vec3<f32> = (vec3<f32>(x_652.x, x_652.y, x_652.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_664 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_662.x, x_662.y, x_662.z, x_662.x));
  u_xlatb0 = vec3<bool>(x_664.x, x_664.y, x_664.z);
  ...
  let x_676 : f32 = x_668;
  u_xlat0.x = x_676;
  let x_679 : bool = u_xlatb0.y;
  if (x_679) {
    let x_684 : f32 = u_xlat1.y;
    x_680 = x_684;
  } else {
  ...
  let x_688 : f32 = x_680;
  u_xlat0.y = x_688;
  let x_691 : bool = u_xlatb0.z;
  if (x_691) {
    let x_696 : f32 = u_xlat1.z;
    x_692 = x_696;
  } else {
    let x_699 : f32 = u_xlat2.z;
    x_692 = x_699;
  }
  let x_700 : f32 = x_692;
  u_xlat0.z = x_700;

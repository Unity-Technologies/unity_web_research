...
#line 205
PixShader ( surfIN , o ) ;
float4 lightCoord = mul ( unity_WorldToLight , float4 ( worldPos , 1 ) ) ; float shadow = UnityComputeForwardShadows ( 0 , 0 , 0 ) ; float atten = ( lightCoord . z > 0 ) * UnitySpotCookie ( lightCoord ) * UnitySpotAttenuate ( lightCoord . xyz ) * shadow ;
float4 c = 0 ;
float3 worldN ;

...
fn main_1() {
  ...
  let x_763 : vec2<f32> = vs_TEXCOORD0;
  let x_766 : vec4<f32> = x_38.x_Dithering_Params;
  let x_770 : vec4<f32> = x_38.x_Dithering_Params;
  let x_772 : vec2<f32> = ((x_763 * vec2<f32>(x_766.x, x_766.y)) + vec2<f32>(x_770.z, x_770.w));
  let x_773 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_772.x, x_772.y, x_773.z, x_773.w);
  let x_780 : vec4<f32> = u_xlat1;
  let x_783 : f32 = x_38.x_GlobalMipBias.x;
  let x_784 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_780.x, x_780.y), x_783);
  u_xlat15 = x_784.w;
  let x_786 : f32 = u_xlat15;
  u_xlat15 = ((x_786 * 2.0f) + -1.0f);
  let x_789 : f32 = u_xlat15;
  u_xlatb1.x = (x_789 >= 0.0f);
  let x_793 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_793);
  let x_796 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_796)) + 1.0f);
  ...
  u_xlat15 = (x_805 * x_807);
  let x_809 : vec4<f32> = u_xlat0;
  let x_811 : vec3<f32> = (vec3<f32>(x_809.x, x_809.y, x_809.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_812 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_811.x, x_811.y, x_811.z, x_812.w);
  let x_814 : vec4<f32> = u_xlat0;
  let x_817 : vec3<f32> = log2(abs(vec3<f32>(x_814.x, x_814.y, x_814.z)));
  let x_818 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_817.x, x_817.y, x_817.z, x_818.w);
  let x_820 : vec4<f32> = u_xlat2;
  let x_822 : vec3<f32> = (vec3<f32>(x_820.x, x_820.y, x_820.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_833 : vec3<f32> = ((vec3<f32>(x_830.x, x_830.y, x_830.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_834 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_833.x, x_833.y, x_833.z, x_834.w);
  let x_837 : vec4<f32> = u_xlat0;
  let x_839 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_837.x, x_837.y, x_837.z, x_837.x));
  ...
  let x_884 : vec3<f32> = ((vec3<f32>(x_877, x_877, x_877) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_882.x, x_882.y, x_882.z));
  let x_885 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_884.x, x_884.y, x_884.z, x_885.w);
  let x_887 : vec4<f32> = u_xlat0;
  let x_889 : vec3<f32> = (vec3<f32>(x_887.x, x_887.y, x_887.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_890 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_889.x, x_889.y, x_889.z, x_890.w);
  let x_892 : vec4<f32> = u_xlat0;
  let x_894 : vec3<f32> = (vec3<f32>(x_892.x, x_892.y, x_892.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_895 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_894.x, x_894.y, x_894.z, x_895.w);
  let x_897 : vec4<f32> = u_xlat2;
  let x_899 : vec3<f32> = (vec3<f32>(x_897.x, x_897.y, x_897.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_900 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_899.x, x_899.y, x_899.z, x_900.w);
  let x_902 : vec4<f32> = u_xlat2;
  let x_905 : vec3<f32> = log2(abs(vec3<f32>(x_902.x, x_902.y, x_902.z)));
  let x_906 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_905.x, x_905.y, x_905.z, x_906.w);
  let x_908 : vec4<f32> = u_xlat2;
  let x_910 : vec3<f32> = (vec3<f32>(x_908.x, x_908.y, x_908.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_911 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_910.x, x_910.y, x_910.z, x_911.w);
  let x_913 : vec4<f32> = u_xlat2;
  let x_915 : vec3<f32> = exp2(vec3<f32>(x_913.x, x_913.y, x_913.z));
  let x_916 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_915.x, x_915.y, x_915.z, x_916.w);
  let x_918 : vec4<f32> = u_xlat0;
  let x_920 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_918.x, x_918.y, x_918.z, x_918.x));
  ...
  let x_963 : vec3<f32> = max(vec3<f32>(x_960.x, x_960.y, x_960.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_745.x, x_745.y, x_745.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_748 : vec4<f32> = u_xlat0;
  let x_751 : vec3<f32> = log2(abs(vec3<f32>(x_748.x, x_748.y, x_748.z)));
  let x_752 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_751.x, x_751.y, x_751.z, x_752.w);
  let x_754 : vec4<f32> = u_xlat2;
  let x_756 : vec3<f32> = (vec3<f32>(x_754.x, x_754.y, x_754.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_757 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_756.x, x_756.y, x_756.z, x_757.w);
  let x_759 : vec4<f32> = u_xlat2;
  let x_761 : vec3<f32> = exp2(vec3<f32>(x_759.x, x_759.y, x_759.z));
  let x_762 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_761.x, x_761.y, x_761.z, x_762.w);
  let x_764 : vec4<f32> = u_xlat2;
  let x_767 : vec3<f32> = ((vec3<f32>(x_764.x, x_764.y, x_764.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  ...
  let x_809 : f32 = x_801;
  u_xlat0.z = x_809;
  let x_811 : vec2<f32> = vs_TEXCOORD0;
  let x_814 : vec4<f32> = x_25.x_Dithering_Params;
  let x_818 : vec4<f32> = x_25.x_Dithering_Params;
  let x_820 : vec2<f32> = ((x_811 * vec2<f32>(x_814.x, x_814.y)) + vec2<f32>(x_818.z, x_818.w));
  let x_821 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_820.x, x_820.y, x_821.z);
  let x_828 : vec3<f32> = u_xlat1;
  let x_831 : f32 = x_25.x_GlobalMipBias.x;
  let x_832 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_828.x, x_828.y), x_831);
  u_xlat15 = x_832.w;
  let x_834 : f32 = u_xlat15;
  u_xlat15 = ((x_834 * 2.0f) + -1.0f);
  let x_838 : f32 = u_xlat15;
  u_xlatb1.x = (x_838 >= 0.0f);
  let x_842 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_842);
  let x_845 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_845)) + 1.0f);
  ...
  u_xlat15 = (x_854 * x_856);
  let x_858 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_858.x, x_858.y, x_858.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_869 : vec3<f32> = (vec3<f32>(x_867.x, x_867.y, x_867.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_880 : vec3<f32> = ((vec3<f32>(x_877.x, x_877.y, x_877.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_881 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_880.x, x_880.y, x_880.z, x_881.w);
  let x_883 : vec4<f32> = u_xlat0;
  let x_885 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_883.x, x_883.y, x_883.z, x_883.x));
  ...
  let x_921 : f32 = x_913;
  u_xlat0.z = x_921;
  let x_923 : f32 = u_xlat15;
  let x_928 : vec4<f32> = u_xlat0;
  let x_930 : vec3<f32> = ((vec3<f32>(x_923, x_923, x_923) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_928.x, x_928.y, x_928.z));
  let x_931 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_930.x, x_930.y, x_930.z, x_931.w);
  let x_933 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_933.x, x_933.y, x_933.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_936 : vec4<f32> = u_xlat0;
  let x_938 : vec3<f32> = (vec3<f32>(x_936.x, x_936.y, x_936.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_939 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_938.x, x_938.y, x_938.z, x_939.w);
  let x_941 : vec4<f32> = u_xlat2;
  let x_943 : vec3<f32> = (vec3<f32>(x_941.x, x_941.y, x_941.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_954 : vec3<f32> = (vec3<f32>(x_952.x, x_952.y, x_952.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_1007 : vec3<f32> = max(vec3<f32>(x_1004.x, x_1004.y, x_1004.z), vec3<f32>(0.0f, 0.0f, 0.0f));

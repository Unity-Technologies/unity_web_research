#line 1 "Hidden/VideoDecode"
static const int ADJUST_TO_LINEARSPACE = 0 ;
...
inline float4 AdjustForColorSpace ( float4 color )
{
...
o . texcoord = ( o . texcoord . xy . xy * _MainTex_ST . xy + _MainTex_ST . zw ) ;
return o ;
}

float4 fragmentRGBOne ( v2f i ) : COLOR
{
float4 y = tex2D ( _MainTex , i . texcoord ) . a ;
float u = tex2D ( _SecondTex , i . texcoord ) . a ;
float v = tex2D ( _ThirdTex , i . texcoord ) . a ;
float y1 = 1.15625 * y . a ;
y . r = y1 + 1.59375 * v - 0.87254 ;
y . g = y1 - 0.390625 * u - 0.8125 * v + 0.53137 ;
y . b = y1 + 1.984375 * u - 1.06862 ;

return AdjustForColorSpace ( float4 ( y . rgb , 1.0 ) ) ;
}

float4 fragmentNV12RGBOne ( v2f i ) : COLOR
{
float3 yCbCr = float3 ( tex2D ( _MainTex , i . texcoord ) . a - 0.0625 ,
tex2D ( _SecondTex , i . texcoord ) . r - 0.5 ,
tex2D ( _SecondTex , i . texcoord ) . g - 0.5 ) ;
float4 result = float4 ( dot ( float3 ( _MatrixColorConversion [ 0 ] [ 0 ] , _MatrixColorConversion [ 0 ] [ 1 ] , _MatrixColorConversion [ 0 ] [ 2 ] ) , yCbCr ) ,
...
float4 fragmentNV12RGBA ( v2f i ) : COLOR
{
float ty = 0.5f * i . texcoord . x ;
float ta = ty + 0.5f * _MainTex_ST . x ;
float tuv = ty ;

float y = tex2D ( _MainTex , float2 ( ty , i . texcoord . y ) ) . a ;
float a = tex2D ( _MainTex , float2 ( ta , i . texcoord . y ) ) . a ;
float2 uv = tex2D ( _SecondTex , float2 ( tuv , i . texcoord . y ) ) . rg ;
float u = uv . r ;
float v = uv . g ;

float y1 = 1.15625 * y ;
float4 result = float4 ( y1 + 1.59375 * v - 0.87254 ,
...
float4 fragmentRGB_FullAlpha ( v2f i ) : COLOR
{
float2 tc = float2 ( 0.5f * i . texcoord . x , i . texcoord . y ) ;
float4 y = tex2D ( _MainTex , tc ) . a ;
float u = tex2D ( _SecondTex , tc ) . a ;
float v = tex2D ( _ThirdTex , tc ) . a ;
float a = tex2D ( _MainTex , float2 ( tc . x + 0.5f , tc . y ) ) . a ;
float y1 = 1.15625 * y . a ;
y . r = y1 + 1.59375 * v - 0.87254 ;
y . g = y1 - 0.390625 * u - 0.8125 * v + 0.53137 ;
y . b = y1 + 1.984375 * u - 1.06862 ;
return AdjustForColorSpace ( float4 ( y . rgb , a ) ) ;
}

float4 fragmentRGBA ( v2f i ) : COLOR
{
float2 tc = float2 ( 0.5f * i . texcoord . x , i . texcoord . y ) ;
float4 y = tex2D ( _MainTex , tc ) . a ;
float u = tex2D ( _SecondTex , tc ) . a ;
...
y . g = y1 - 0.390625 * u - 0.8125 * v + 0.53137 ;
y . b = y1 + 1.984375 * u - 1.06862 ;
return AdjustForColorSpace ( float4 ( y . rgb , 1.15625 * ( a - 0.062745 ) ) ) ;
}

float4 fragmentRGBASplit ( v2f i ) : COLOR
{
float2 tc = float2 ( 0.5f * i . texcoord . x , i . texcoord . y ) ;
float4 col = tex2D ( _MainTex , tc ) ;
float a = tex2D ( _MainTex , float2 ( tc . x + 0.5f , tc . y ) ) . g ;
return AdjustForColorSpace ( float4 ( col . rgb , a ) ) ;
}

float4 fragmentRGBANormal ( v2f i ) : COLOR
{

...
fn main_1() {
  ...
  u_xlat1 = vec4<f32>(x_872.x, x_872.y, x_873.z, x_873.w);
  ...
  let x_912 : vec3<f32> = (vec3<f32>(x_910.x, x_910.y, x_910.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_913 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_912.x, x_912.y, x_912.z, x_913.w);
  let x_915 : vec4<f32> = u_xlat0;
  let x_918 : vec3<f32> = log2(abs(vec3<f32>(x_915.x, x_915.y, x_915.z)));
  ...
  let x_985 : vec3<f32> = ((vec3<f32>(x_978, x_978, x_978) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_983.x, x_983.y, x_983.z));
  let x_986 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_985.x, x_985.y, x_985.z, x_986.w);
  let x_988 : vec4<f32> = u_xlat0;
  let x_990 : vec3<f32> = (vec3<f32>(x_988.x, x_988.y, x_988.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_991 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_990.x, x_990.y, x_990.z, x_991.w);
  let x_993 : vec4<f32> = u_xlat0;
  let x_995 : vec3<f32> = (vec3<f32>(x_993.x, x_993.y, x_993.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_996 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_995.x, x_995.y, x_995.z, x_996.w);
  let x_998 : vec4<f32> = u_xlat2;
  let x_1000 : vec3<f32> = (vec3<f32>(x_998.x, x_998.y, x_998.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_1011 : vec3<f32> = (vec3<f32>(x_1009.x, x_1009.y, x_1009.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_1021 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1019.x, x_1019.y, x_1019.z, x_1019.x));
  u_xlatb0 = vec3<bool>(x_1021.x, x_1021.y, x_1021.z);
  ...
  u_xlat0.x = x_1033;
  let x_1036 : bool = u_xlatb0.y;
  if (x_1036) {
    let x_1041 : f32 = u_xlat1.y;
    x_1037 = x_1041;
  } else {
    let x_1044 : f32 = u_xlat2.y;
    x_1037 = x_1044;
  }
  let x_1045 : f32 = x_1037;
  u_xlat0.y = x_1045;
  let x_1048 : bool = u_xlatb0.z;
  if (x_1048) {
    let x_1053 : f32 = u_xlat1.z;
    x_1049 = x_1053;
  } else {
    let x_1056 : f32 = u_xlat2.z;
    x_1049 = x_1056;
  }
  let x_1057 : f32 = x_1049;
  u_xlat0.z = x_1057;
  let x_1061 : vec4<f32> = u_xlat0;
  let x_1064 : vec3<f32> = max(vec3<f32>(x_1061.x, x_1061.y, x_1061.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1065 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1064.x, x_1064.y, x_1064.z, x_1065.w);
  SV_Target0.w = 1.0f;
  return;
}

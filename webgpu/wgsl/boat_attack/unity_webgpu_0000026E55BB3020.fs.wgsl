...
fn main_1() {
  ...
  let x_722 : vec3<f32> = (vec3<f32>(x_720.x, x_720.y, x_720.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_733 : vec3<f32> = (vec3<f32>(x_731.x, x_731.y, x_731.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_744 : vec3<f32> = ((vec3<f32>(x_741.x, x_741.y, x_741.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_745 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_744.x, x_744.y, x_744.z, x_745.w);
  let x_748 : vec4<f32> = u_xlat0;
  let x_750 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_748.x, x_748.y, x_748.z, x_748.x));
  ...
  let x_791 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_808 : f32 = x_38.x_GlobalMipBias.x;
  let x_809 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_805.x, x_805.y), x_808);
  u_xlat15 = x_809.w;
  let x_811 : f32 = u_xlat15;
  u_xlat15 = ((x_811 * 2.0f) + -1.0f);
  ...
  let x_836 : vec3<f32> = (vec3<f32>(x_834.x, x_834.y, x_834.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_863 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_861.x, x_861.y, x_861.z, x_861.x));
  ...
  let x_908 : vec3<f32> = ((vec3<f32>(x_901, x_901, x_901) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_906.x, x_906.y, x_906.z));
  let x_909 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_908.x, x_908.y, x_908.z, x_909.w);
  let x_911 : vec4<f32> = u_xlat0;
  let x_913 : vec3<f32> = (vec3<f32>(x_911.x, x_911.y, x_911.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_914 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_913.x, x_913.y, x_913.z, x_914.w);
  let x_916 : vec4<f32> = u_xlat0;
  let x_918 : vec3<f32> = (vec3<f32>(x_916.x, x_916.y, x_916.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_919 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_918.x, x_918.y, x_918.z, x_919.w);
  let x_921 : vec4<f32> = u_xlat2;
  let x_923 : vec3<f32> = (vec3<f32>(x_921.x, x_921.y, x_921.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_934 : vec3<f32> = (vec3<f32>(x_932.x, x_932.y, x_932.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_935 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_934.x, x_934.y, x_934.z, x_935.w);
  let x_937 : vec4<f32> = u_xlat2;
  let x_939 : vec3<f32> = exp2(vec3<f32>(x_937.x, x_937.y, x_937.z));
  let x_940 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_939.x, x_939.y, x_939.z, x_940.w);
  let x_942 : vec4<f32> = u_xlat0;
  let x_944 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_942.x, x_942.y, x_942.z, x_942.x));
  ...
  let x_987 : vec3<f32> = max(vec3<f32>(x_984.x, x_984.y, x_984.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_988 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_987.x, x_987.y, x_987.z, x_988.w);
  SV_Target0.w = 1.0f;
  return;
}

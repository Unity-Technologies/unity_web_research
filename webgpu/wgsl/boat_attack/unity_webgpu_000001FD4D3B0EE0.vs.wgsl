...
@group(1) @binding(6) var<uniform> x_203 : VGlobals;
...
fn main_1() {
  ...
  let x_205 : vec4<f32> = x_203.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_199.y, x_199.y, x_199.y, x_199.y) * x_205);
  let x_208 : vec4<f32> = x_203.unity_MatrixVP[0i];
  let x_209 : vec4<f32> = u_xlat0;
  let x_212 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_208 * vec4<f32>(x_209.x, x_209.x, x_209.x, x_209.x)) + x_212);
  let x_215 : vec4<f32> = x_203.unity_MatrixVP[2i];
  let x_216 : vec4<f32> = u_xlat0;
  let x_219 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_215 * vec4<f32>(x_216.z, x_216.z, x_216.z, x_216.z)) + x_219);
  let x_225 : vec4<f32> = u_xlat0;
  let x_227 : vec4<f32> = x_203.unity_MatrixVP[3i];
  gl_Position = (x_225 + x_227);
  return;
}
...
fn main(@location(3) in_TEXCOORD0_param : vec2<f32>, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_TEXCOORD3, gl_Position);
}

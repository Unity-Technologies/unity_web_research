...
fn main_1() {
  ...
  let x_549 : vec3<f32> = sqrt(vec3<f32>(x_547.x, x_547.y, x_547.z));
  let x_550 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_549.x, x_549.y, x_549.z, x_550.w);
  let x_552 : vec2<f32> = vs_TEXCOORD0;
  let x_555 : vec4<f32> = x_26.x_Dithering_Params;
  let x_559 : vec4<f32> = x_26.x_Dithering_Params;
  let x_561 : vec2<f32> = ((x_552 * vec2<f32>(x_555.x, x_555.y)) + vec2<f32>(x_559.z, x_559.w));
  let x_562 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_561.x, x_561.y, x_562.z);
  let x_569 : vec3<f32> = u_xlat1;
  let x_572 : f32 = x_26.x_GlobalMipBias.x;
  let x_573 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_569.x, x_569.y), x_572);
  u_xlat15 = x_573.w;
  let x_575 : f32 = u_xlat15;
  u_xlat15 = ((x_575 * 2.0f) + -1.0f);
  let x_580 : f32 = u_xlat15;
  u_xlatb1.x = (x_580 >= 0.0f);
  let x_584 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_584);
  let x_587 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_587)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_600.x, x_600.y, x_600.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_603 : vec4<f32> = u_xlat0;
  let x_605 : vec3<f32> = log2(vec3<f32>(x_603.x, x_603.y, x_603.z));
  let x_606 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_605.x, x_605.y, x_605.z, x_606.w);
  let x_608 : vec4<f32> = u_xlat2;
  let x_610 : vec3<f32> = (vec3<f32>(x_608.x, x_608.y, x_608.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_621 : vec3<f32> = ((vec3<f32>(x_618.x, x_618.y, x_618.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_622 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_621.x, x_621.y, x_621.z, x_622.w);
  let x_625 : vec4<f32> = u_xlat0;
  let x_627 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_625.x, x_625.y, x_625.z, x_625.x));
  ...
  let x_672 : vec3<f32> = ((vec3<f32>(x_665, x_665, x_665) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_670.x, x_670.y, x_670.z));
  let x_673 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_672.x, x_672.y, x_672.z, x_673.w);
  let x_675 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_675.x, x_675.y, x_675.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_678 : vec4<f32> = u_xlat0;
  let x_680 : vec3<f32> = (vec3<f32>(x_678.x, x_678.y, x_678.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_681 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_680.x, x_680.y, x_680.z, x_681.w);
  let x_683 : vec4<f32> = u_xlat2;
  let x_685 : vec3<f32> = (vec3<f32>(x_683.x, x_683.y, x_683.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_686 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_685.x, x_685.y, x_685.z, x_686.w);
  let x_688 : vec4<f32> = u_xlat2;
  let x_691 : vec3<f32> = log2(abs(vec3<f32>(x_688.x, x_688.y, x_688.z)));
  let x_692 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_691.x, x_691.y, x_691.z, x_692.w);
  let x_694 : vec4<f32> = u_xlat2;
  let x_696 : vec3<f32> = (vec3<f32>(x_694.x, x_694.y, x_694.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_697 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_696.x, x_696.y, x_696.z, x_697.w);
  let x_699 : vec4<f32> = u_xlat2;
  let x_701 : vec3<f32> = exp2(vec3<f32>(x_699.x, x_699.y, x_699.z));
  ...
  let x_749 : vec3<f32> = max(vec3<f32>(x_746.x, x_746.y, x_746.z), vec3<f32>(0.0f, 0.0f, 0.0f));

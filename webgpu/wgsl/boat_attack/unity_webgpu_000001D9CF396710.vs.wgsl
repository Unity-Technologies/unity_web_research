...
struct VGlobals {
  ...
  /* @offset(128) */
  x_TexCoordOffsetScale : vec4<f32>,
}
...
fn main_1() {
  ...
  let x_85 : vec4<f32> = x_17.x_TexCoordOffsetScale;
  let x_87 : vec2<f32> = (x_83 + vec2<f32>(x_85.x, x_85.y));
  let x_88 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_87.x, x_87.y, x_88.z, x_88.w);
  let x_91 : vec4<f32> = u_xlat0;
  let x_94 : vec4<f32> = x_17.x_TexCoordOffsetScale;
  let x_96 : vec2<f32> = (vec2<f32>(x_91.x, x_91.y) * vec2<f32>(x_94.z, x_94.w));
  let x_97 : vec4<f32> = vs_TEXCOORD0;
  vs_TEXCOORD0 = vec4<f32>(x_97.x, x_97.y, x_96.x, x_96.y);
  let x_99 : vec2<f32> = in_TEXCOORD0;
  let x_100 : vec4<f32> = vs_TEXCOORD0;
  vs_TEXCOORD0 = vec4<f32>(x_99.x, x_99.y, x_100.z, x_100.w);
  return;
}

...
fn main_1() {
  ...
  let x_872 : vec3<f32> = (vec3<f32>(x_870.x, x_870.y, x_870.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_873 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_872.x, x_872.y, x_872.z, x_873.w);
  let x_875 : vec4<f32> = u_xlat0;
  let x_878 : vec3<f32> = log2(abs(vec3<f32>(x_875.x, x_875.y, x_875.z)));
  let x_879 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_878.x, x_878.y, x_878.z, x_879.w);
  let x_881 : vec4<f32> = u_xlat2;
  let x_883 : vec3<f32> = (vec3<f32>(x_881.x, x_881.y, x_881.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_894 : vec3<f32> = ((vec3<f32>(x_891.x, x_891.y, x_891.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_895 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_894.x, x_894.y, x_894.z, x_895.w);
  let x_898 : vec4<f32> = u_xlat0;
  let x_900 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_898.x, x_898.y, x_898.z, x_898.x));
  ...
  if (x_927) {
    ...
    let x_935 : f32 = u_xlat2.z;
    x_928 = x_935;
  }
  let x_936 : f32 = x_928;
  u_xlat0.z = x_936;
  let x_938 : vec2<f32> = vs_TEXCOORD0;
  let x_941 : vec4<f32> = x_38.x_Dithering_Params;
  let x_945 : vec4<f32> = x_38.x_Dithering_Params;
  let x_947 : vec2<f32> = ((x_938 * vec2<f32>(x_941.x, x_941.y)) + vec2<f32>(x_945.z, x_945.w));
  let x_948 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_947.x, x_947.y, x_948.z, x_948.w);
  let x_955 : vec4<f32> = u_xlat1;
  let x_958 : f32 = x_38.x_GlobalMipBias.x;
  let x_959 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_955.x, x_955.y), x_958);
  u_xlat18 = x_959.w;
  let x_961 : f32 = u_xlat18;
  u_xlat18 = ((x_961 * 2.0f) + -1.0f);
  let x_964 : f32 = u_xlat18;
  u_xlatb1.x = (x_964 >= 0.0f);
  let x_968 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_968);
  let x_971 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_971)) + 1.0f);
  ...
  u_xlat18 = (x_980 * x_982);
  let x_984 : vec4<f32> = u_xlat0;
  let x_986 : vec3<f32> = (vec3<f32>(x_984.x, x_984.y, x_984.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_987 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_986.x, x_986.y, x_986.z, x_987.w);
  let x_989 : vec4<f32> = u_xlat0;
  let x_992 : vec3<f32> = log2(abs(vec3<f32>(x_989.x, x_989.y, x_989.z)));
  let x_993 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_992.x, x_992.y, x_992.z, x_993.w);
  let x_995 : vec4<f32> = u_xlat2;
  let x_997 : vec3<f32> = (vec3<f32>(x_995.x, x_995.y, x_995.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_1008 : vec3<f32> = ((vec3<f32>(x_1005.x, x_1005.y, x_1005.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  ...
  u_xlat0.x = x_1025;
  let x_1028 : bool = u_xlatb0.y;
  if (x_1028) {
    let x_1033 : f32 = u_xlat1.y;
    x_1029 = x_1033;
  } else {
    let x_1036 : f32 = u_xlat2.y;
    x_1029 = x_1036;
  }
  let x_1037 : f32 = x_1029;
  u_xlat0.y = x_1037;
  let x_1040 : bool = u_xlatb0.z;
  if (x_1040) {
  ...
  let x_1058 : vec3<f32> = ((vec3<f32>(x_1051, x_1051, x_1051) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1056.x, x_1056.y, x_1056.z));
  let x_1059 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1058.x, x_1058.y, x_1058.z, x_1059.w);
  let x_1061 : vec4<f32> = u_xlat0;
  let x_1063 : vec3<f32> = (vec3<f32>(x_1061.x, x_1061.y, x_1061.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1064 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1063.x, x_1063.y, x_1063.z, x_1064.w);
  let x_1066 : vec4<f32> = u_xlat0;
  let x_1068 : vec3<f32> = (vec3<f32>(x_1066.x, x_1066.y, x_1066.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1069 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1068.x, x_1068.y, x_1068.z, x_1069.w);
  let x_1071 : vec4<f32> = u_xlat2;
  let x_1073 : vec3<f32> = (vec3<f32>(x_1071.x, x_1071.y, x_1071.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_1084 : vec3<f32> = (vec3<f32>(x_1082.x, x_1082.y, x_1082.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1094 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1092.x, x_1092.y, x_1092.z, x_1092.x));
  u_xlatb0 = vec3<bool>(x_1094.x, x_1094.y, x_1094.z);
  let x_1097 : bool = u_xlatb0.x;
  if (x_1097) {
  ...
  u_xlat0.x = x_1106;
  let x_1109 : bool = u_xlatb0.y;
  if (x_1109) {
    let x_1114 : f32 = u_xlat1.y;
    x_1110 = x_1114;
  } else {
    let x_1117 : f32 = u_xlat2.y;
    x_1110 = x_1117;
  }
  let x_1118 : f32 = x_1110;
  u_xlat0.y = x_1118;
  let x_1121 : bool = u_xlatb0.z;
  if (x_1121) {
  ...
  let x_1137 : vec3<f32> = max(vec3<f32>(x_1134.x, x_1134.y, x_1134.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  u_xlat0 = (vec3<f32>(x_628.y, x_628.y, x_628.y) * vec3<f32>(x_632.x, x_632.y, x_632.z));
  let x_636 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_638 : vec4<f32> = u_xlat1;
  let x_641 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_636.x, x_636.y, x_636.z) * vec3<f32>(x_638.x, x_638.x, x_638.x)) + x_641);
  let x_644 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_646 : vec4<f32> = u_xlat1;
  let x_649 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_644.x, x_644.y, x_644.z) * vec3<f32>(x_646.z, x_646.z, x_646.z)) + x_649);
  let x_653 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_655 : vec4<f32> = u_xlat1;
  let x_658 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec3<f32>(x_653.x, x_653.y, x_653.z) * vec3<f32>(x_655.w, x_655.w, x_655.w)) + x_658);
  return;
}

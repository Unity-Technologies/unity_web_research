...
fn main_1() {
  ...
  let x_820 : vec4<f32> = x_13.x_Dithering_Params;
  let x_824 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_838 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_834.x, x_834.y), x_837);
  u_xlat18 = x_838.w;
  let x_840 : f32 = u_xlat18;
  u_xlat18 = ((x_840 * 2.0f) + -1.0f);
  let x_844 : f32 = u_xlat18;
  u_xlatb1.x = (x_844 >= 0.0f);
  let x_848 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_848);
  let x_851 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_851)) + 1.0f);
  ...
  u_xlat18 = (x_860 * x_862);
  let x_864 : vec4<f32> = u_xlat0;
  let x_866 : vec3<f32> = (vec3<f32>(x_864.x, x_864.y, x_864.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_867 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_866.x, x_866.y, x_866.z, x_867.w);
  let x_869 : vec4<f32> = u_xlat0;
  let x_872 : vec3<f32> = log2(abs(vec3<f32>(x_869.x, x_869.y, x_869.z)));
  let x_873 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_872.x, x_872.y, x_872.z, x_873.w);
  let x_875 : vec4<f32> = u_xlat2;
  let x_877 : vec3<f32> = (vec3<f32>(x_875.x, x_875.y, x_875.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_878 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_877.x, x_877.y, x_877.z, x_878.w);
  let x_880 : vec4<f32> = u_xlat2;
  let x_882 : vec3<f32> = exp2(vec3<f32>(x_880.x, x_880.y, x_880.z));
  let x_883 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_882.x, x_882.y, x_882.z, x_883.w);
  let x_885 : vec4<f32> = u_xlat2;
  let x_888 : vec3<f32> = ((vec3<f32>(x_885.x, x_885.y, x_885.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_889 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_888.x, x_888.y, x_888.z, x_889.w);
  let x_892 : vec4<f32> = u_xlat0;
  let x_894 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_892.x, x_892.y, x_892.z, x_892.x));
  ...
  let x_939 : vec3<f32> = ((vec3<f32>(x_932, x_932, x_932) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_937.x, x_937.y, x_937.z));
  let x_940 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_939.x, x_939.y, x_939.z, x_940.w);
  let x_942 : vec4<f32> = u_xlat0;
  let x_944 : vec3<f32> = (vec3<f32>(x_942.x, x_942.y, x_942.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_945 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_944.x, x_944.y, x_944.z, x_945.w);
  let x_947 : vec4<f32> = u_xlat0;
  let x_949 : vec3<f32> = (vec3<f32>(x_947.x, x_947.y, x_947.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_950 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_949.x, x_949.y, x_949.z, x_950.w);
  let x_952 : vec4<f32> = u_xlat2;
  let x_954 : vec3<f32> = (vec3<f32>(x_952.x, x_952.y, x_952.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_955 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_954.x, x_954.y, x_954.z, x_955.w);
  let x_957 : vec4<f32> = u_xlat2;
  let x_960 : vec3<f32> = log2(abs(vec3<f32>(x_957.x, x_957.y, x_957.z)));
  let x_961 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_960.x, x_960.y, x_960.z, x_961.w);
  let x_963 : vec4<f32> = u_xlat2;
  let x_965 : vec3<f32> = (vec3<f32>(x_963.x, x_963.y, x_963.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_966 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_965.x, x_965.y, x_965.z, x_966.w);
  let x_968 : vec4<f32> = u_xlat2;
  let x_970 : vec3<f32> = exp2(vec3<f32>(x_968.x, x_968.y, x_968.z));
  let x_971 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_970.x, x_970.y, x_970.z, x_971.w);
  let x_973 : vec4<f32> = u_xlat0;
  let x_975 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_973.x, x_973.y, x_973.z, x_973.x));
  ...
  let x_1018 : vec3<f32> = max(vec3<f32>(x_1015.x, x_1015.y, x_1015.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  let x_198 : vec2<f32> = in_TEXCOORD1;
  let x_201 : vec4<f32> = x_44.unity_LightmapST;
  let x_205 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD8 = ((x_198 * vec2<f32>(x_201.x, x_201.y)) + vec2<f32>(x_205.z, x_205.w));
  let x_208 : vec4<f32> = u_xlat0;
  let x_214 : vec4<f32> = x_212.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_208.y, x_208.y, x_208.y, x_208.y) * x_214);
  let x_217 : vec4<f32> = x_212.unity_MatrixVP[0i];
  let x_218 : vec4<f32> = u_xlat0;
  let x_221 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_217 * vec4<f32>(x_218.x, x_218.x, x_218.x, x_218.x)) + x_221);
  let x_224 : vec4<f32> = x_212.unity_MatrixVP[2i];
  ...
  let x_236 : vec4<f32> = x_212.unity_MatrixVP[3i];

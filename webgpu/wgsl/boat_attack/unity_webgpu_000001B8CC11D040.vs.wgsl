...
fn main_1() {
  ...
  let x_638 : vec2<f32> = (vec2<f32>(x_632.y, x_632.y) * vec2<f32>(x_636.x, x_636.y));
  let x_639 : vec3<f32> = u_xlat0;
  u_xlat0 = vec3<f32>(x_638.x, x_638.y, x_639.z);
  let x_642 : vec4<f32> = x_19.unity_WorldToLight[0i];
  ...
  u_xlat0 = vec3<f32>(x_649.x, x_649.y, x_650.z);
  let x_653 : vec4<f32> = x_19.unity_WorldToLight[2i];
  ...
  u_xlat0 = vec3<f32>(x_660.x, x_660.y, x_661.z);
  let x_666 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_668 : vec4<f32> = u_xlat1;
  let x_671 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec2<f32>(x_666.x, x_666.y) * vec2<f32>(x_668.w, x_668.w)) + vec2<f32>(x_671.x, x_671.y));
  return;
}

...
fn main_1() {
  ...
  let x_887 : vec2<f32> = vs_TEXCOORD0;
  let x_890 : vec4<f32> = x_13.x_Dithering_Params;
  let x_894 : vec4<f32> = x_13.x_Dithering_Params;
  let x_896 : vec2<f32> = ((x_887 * vec2<f32>(x_890.x, x_890.y)) + vec2<f32>(x_894.z, x_894.w));
  let x_897 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_896.x, x_896.y, x_897.z, x_897.w);
  let x_904 : vec4<f32> = u_xlat1;
  let x_907 : f32 = x_13.x_GlobalMipBias.x;
  let x_908 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_904.x, x_904.y), x_907);
  u_xlat18 = x_908.w;
  let x_910 : f32 = u_xlat18;
  u_xlat18 = ((x_910 * 2.0f) + -1.0f);
  let x_914 : f32 = u_xlat18;
  u_xlatb1.x = (x_914 >= 0.0f);
  let x_918 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_918);
  let x_921 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_921)) + 1.0f);
  let x_925 : f32 = u_xlat18;
  u_xlat18 = sqrt(x_925);
  ...
  let x_1009 : vec3<f32> = ((vec3<f32>(x_1002, x_1002, x_1002) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1007.x, x_1007.y, x_1007.z));
  let x_1010 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1009.x, x_1009.y, x_1009.z, x_1010.w);
  let x_1012 : vec4<f32> = u_xlat0;
  let x_1014 : vec3<f32> = (vec3<f32>(x_1012.x, x_1012.y, x_1012.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1015 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1014.x, x_1014.y, x_1014.z, x_1015.w);
  let x_1017 : vec4<f32> = u_xlat0;
  let x_1019 : vec3<f32> = (vec3<f32>(x_1017.x, x_1017.y, x_1017.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1020 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1019.x, x_1019.y, x_1019.z, x_1020.w);
  let x_1022 : vec4<f32> = u_xlat2;
  let x_1024 : vec3<f32> = (vec3<f32>(x_1022.x, x_1022.y, x_1022.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1025 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1024.x, x_1024.y, x_1024.z, x_1025.w);
  let x_1027 : vec4<f32> = u_xlat2;
  let x_1030 : vec3<f32> = log2(abs(vec3<f32>(x_1027.x, x_1027.y, x_1027.z)));
  let x_1031 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1030.x, x_1030.y, x_1030.z, x_1031.w);
  let x_1033 : vec4<f32> = u_xlat2;
  let x_1035 : vec3<f32> = (vec3<f32>(x_1033.x, x_1033.y, x_1033.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1045 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1043.x, x_1043.y, x_1043.z, x_1043.x));
  u_xlatb0 = vec3<bool>(x_1045.x, x_1045.y, x_1045.z);
  let x_1048 : bool = u_xlatb0.x;
  if (x_1048) {
  ...
  let x_1088 : vec3<f32> = max(vec3<f32>(x_1085.x, x_1085.y, x_1085.z), vec3<f32>(0.0f, 0.0f, 0.0f));

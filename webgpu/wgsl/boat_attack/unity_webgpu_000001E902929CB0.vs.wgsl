...
@group(1) @binding(5) var<uniform> x_150 : VGlobals;
...
fn main_1() {
  ...
  let x_152 : vec4<f32> = x_150.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_146.y, x_146.y, x_146.y, x_146.y) * x_152);
  let x_155 : vec4<f32> = x_150.unity_MatrixVP[0i];
  let x_156 : vec4<f32> = u_xlat0;
  let x_159 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_155 * vec4<f32>(x_156.x, x_156.x, x_156.x, x_156.x)) + x_159);
  let x_162 : vec4<f32> = x_150.unity_MatrixVP[2i];
  let x_163 : vec4<f32> = u_xlat0;
  let x_166 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_162 * vec4<f32>(x_163.z, x_163.z, x_163.z, x_163.z)) + x_166);
  let x_172 : vec4<f32> = u_xlat0;
  let x_174 : vec4<f32> = x_150.unity_MatrixVP[3i];
  gl_Position = (x_172 + x_174);
  return;
}

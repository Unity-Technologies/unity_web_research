...
fn main_1() {
  ...
  let x_596 : vec3<f32> = (vec3<f32>(x_594.x, x_594.y, x_594.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_597 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_596.x, x_596.y, x_596.z, x_597.w);
  let x_599 : vec4<f32> = u_xlat0;
  let x_602 : vec3<f32> = log2(abs(vec3<f32>(x_599.x, x_599.y, x_599.z)));
  let x_603 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_602.x, x_602.y, x_602.z, x_603.w);
  let x_605 : vec4<f32> = u_xlat2;
  let x_607 : vec3<f32> = (vec3<f32>(x_605.x, x_605.y, x_605.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_608 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_607.x, x_607.y, x_607.z, x_608.w);
  let x_610 : vec4<f32> = u_xlat2;
  let x_612 : vec3<f32> = exp2(vec3<f32>(x_610.x, x_610.y, x_610.z));
  let x_613 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_612.x, x_612.y, x_612.z, x_613.w);
  let x_615 : vec4<f32> = u_xlat2;
  let x_618 : vec3<f32> = ((vec3<f32>(x_615.x, x_615.y, x_615.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_619 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_618.x, x_618.y, x_618.z, x_619.w);
  let x_622 : vec4<f32> = u_xlat0;
  let x_624 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_622.x, x_622.y, x_622.z, x_622.x));
  ...
  let x_665 : vec4<f32> = x_13.x_Dithering_Params;
  let x_669 : vec4<f32> = x_13.x_Dithering_Params;
  let x_671 : vec2<f32> = ((x_662 * vec2<f32>(x_665.x, x_665.y)) + vec2<f32>(x_669.z, x_669.w));
  let x_672 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_671.x, x_671.y, x_672.z, x_672.w);
  let x_679 : vec4<f32> = u_xlat1;
  let x_682 : f32 = x_13.x_GlobalMipBias.x;
  let x_683 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_679.x, x_679.y), x_682);
  u_xlat15 = x_683.w;
  let x_685 : f32 = u_xlat15;
  u_xlat15 = ((x_685 * 2.0f) + -1.0f);
  let x_690 : f32 = u_xlat15;
  u_xlatb1.x = (x_690 >= 0.0f);
  let x_694 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_694);
  let x_697 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_697)) + 1.0f);
  ...
  u_xlat15 = (x_706 * x_708);
  let x_710 : vec4<f32> = u_xlat0;
  let x_712 : vec3<f32> = (vec3<f32>(x_710.x, x_710.y, x_710.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_713 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_712.x, x_712.y, x_712.z, x_713.w);
  let x_715 : vec4<f32> = u_xlat0;
  let x_718 : vec3<f32> = log2(abs(vec3<f32>(x_715.x, x_715.y, x_715.z)));
  let x_719 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_718.x, x_718.y, x_718.z, x_719.w);
  let x_721 : vec4<f32> = u_xlat2;
  let x_723 : vec3<f32> = (vec3<f32>(x_721.x, x_721.y, x_721.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_734 : vec3<f32> = ((vec3<f32>(x_731.x, x_731.y, x_731.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_735 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_734.x, x_734.y, x_734.z, x_735.w);
  let x_737 : vec4<f32> = u_xlat0;
  let x_739 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_737.x, x_737.y, x_737.z, x_737.x));
  ...
  let x_784 : vec3<f32> = ((vec3<f32>(x_777, x_777, x_777) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_782.x, x_782.y, x_782.z));
  let x_785 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_784.x, x_784.y, x_784.z, x_785.w);
  let x_787 : vec4<f32> = u_xlat0;
  let x_789 : vec3<f32> = (vec3<f32>(x_787.x, x_787.y, x_787.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_790 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_789.x, x_789.y, x_789.z, x_790.w);
  let x_792 : vec4<f32> = u_xlat0;
  let x_794 : vec3<f32> = (vec3<f32>(x_792.x, x_792.y, x_792.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_795 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_794.x, x_794.y, x_794.z, x_795.w);
  let x_797 : vec4<f32> = u_xlat2;
  let x_799 : vec3<f32> = (vec3<f32>(x_797.x, x_797.y, x_797.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_810 : vec3<f32> = (vec3<f32>(x_808.x, x_808.y, x_808.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_820 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_818.x, x_818.y, x_818.z, x_818.x));
  ...
  let x_863 : vec3<f32> = max(vec3<f32>(x_860.x, x_860.y, x_860.z), vec3<f32>(0.0f, 0.0f, 0.0f));

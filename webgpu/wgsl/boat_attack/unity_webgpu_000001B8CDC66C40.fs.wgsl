...
fn main_1() {
  ...
  let x_761 : vec3<f32> = sqrt(vec3<f32>(x_759.x, x_759.y, x_759.z));
  let x_762 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_761.x, x_761.y, x_761.z, x_762.w);
  let x_764 : vec2<f32> = vs_TEXCOORD0;
  let x_767 : vec4<f32> = x_25.x_Dithering_Params;
  let x_771 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_784 : f32 = x_25.x_GlobalMipBias.x;
  ...
  u_xlatb1.x = (x_791 >= 0.0f);
  let x_795 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_795);
  let x_798 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_798)) + 1.0f);
  ...
  let x_816 : vec3<f32> = log2(vec3<f32>(x_814.x, x_814.y, x_814.z));
  ...
  let x_838 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_836.x, x_836.y, x_836.z, x_836.x));
  ...
  let x_883 : vec3<f32> = ((vec3<f32>(x_876, x_876, x_876) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_881.x, x_881.y, x_881.z));
  let x_884 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_883.x, x_883.y, x_883.z, x_884.w);
  let x_886 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_886.x, x_886.y, x_886.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_889 : vec4<f32> = u_xlat0;
  let x_891 : vec3<f32> = (vec3<f32>(x_889.x, x_889.y, x_889.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_892 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_891.x, x_891.y, x_891.z, x_892.w);
  let x_894 : vec4<f32> = u_xlat2;
  let x_896 : vec3<f32> = (vec3<f32>(x_894.x, x_894.y, x_894.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_897 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_896.x, x_896.y, x_896.z, x_897.w);
  let x_899 : vec4<f32> = u_xlat2;
  let x_902 : vec3<f32> = log2(abs(vec3<f32>(x_899.x, x_899.y, x_899.z)));
  let x_903 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_902.x, x_902.y, x_902.z, x_903.w);
  let x_905 : vec4<f32> = u_xlat2;
  let x_907 : vec3<f32> = (vec3<f32>(x_905.x, x_905.y, x_905.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_960 : vec3<f32> = max(vec3<f32>(x_957.x, x_957.y, x_957.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_961 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_960.x, x_960.y, x_960.z, x_961.w);
  SV_Target0.w = 1.0f;
  return;
}

...
#line 205
PixShader ( surfIN , o ) ;
float3 lightCoord = mul ( unity_WorldToLight , float4 ( worldPos , 1 ) ) . xyz ; float shadow = UnityComputeForwardShadows ( 0 , 0 , 0 ) ; float atten = tex2D ( _LightTexture0 , dot ( lightCoord , lightCoord ) . rr ) . r * shadow ;
float4 c = 0 ;
float3 worldN ;

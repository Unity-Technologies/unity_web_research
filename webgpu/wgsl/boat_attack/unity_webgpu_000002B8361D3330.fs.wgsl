...
fn main_1() {
  ...
  let x_511 : vec3<f32> = sqrt(vec3<f32>(x_509.x, x_509.y, x_509.z));
  let x_512 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_511.x, x_511.y, x_511.z, x_512.w);
  let x_514 : vec2<f32> = vs_TEXCOORD0;
  let x_517 : vec4<f32> = x_13.x_Dithering_Params;
  let x_521 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_534 : f32 = x_13.x_GlobalMipBias.x;
  let x_535 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_531.x, x_531.y), x_534);
  u_xlat15 = x_535.w;
  let x_537 : f32 = u_xlat15;
  u_xlat15 = ((x_537 * 2.0f) + -1.0f);
  let x_542 : f32 = u_xlat15;
  u_xlatb1.x = (x_542 >= 0.0f);
  let x_546 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_546);
  let x_549 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_549)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_562.x, x_562.y, x_562.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_572 : vec3<f32> = (vec3<f32>(x_570.x, x_570.y, x_570.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_573 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_572.x, x_572.y, x_572.z, x_573.w);
  let x_575 : vec4<f32> = u_xlat2;
  let x_577 : vec3<f32> = exp2(vec3<f32>(x_575.x, x_575.y, x_575.z));
  let x_578 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_577.x, x_577.y, x_577.z, x_578.w);
  let x_580 : vec4<f32> = u_xlat2;
  let x_583 : vec3<f32> = ((vec3<f32>(x_580.x, x_580.y, x_580.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_584 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_583.x, x_583.y, x_583.z, x_584.w);
  let x_587 : vec4<f32> = u_xlat0;
  let x_589 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_587.x, x_587.y, x_587.z, x_587.x));
  ...
  let x_634 : vec3<f32> = ((vec3<f32>(x_627, x_627, x_627) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_632.x, x_632.y, x_632.z));
  let x_635 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_634.x, x_634.y, x_634.z, x_635.w);
  let x_637 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_637.x, x_637.y, x_637.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_640 : vec4<f32> = u_xlat0;
  let x_642 : vec3<f32> = (vec3<f32>(x_640.x, x_640.y, x_640.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_643 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_642.x, x_642.y, x_642.z, x_643.w);
  let x_645 : vec4<f32> = u_xlat2;
  let x_647 : vec3<f32> = (vec3<f32>(x_645.x, x_645.y, x_645.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_648 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_647.x, x_647.y, x_647.z, x_648.w);
  let x_650 : vec4<f32> = u_xlat2;
  let x_653 : vec3<f32> = log2(abs(vec3<f32>(x_650.x, x_650.y, x_650.z)));
  let x_654 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_653.x, x_653.y, x_653.z, x_654.w);
  let x_656 : vec4<f32> = u_xlat2;
  let x_658 : vec3<f32> = (vec3<f32>(x_656.x, x_656.y, x_656.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_668 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_666.x, x_666.y, x_666.z, x_666.x));
  u_xlatb0 = vec3<bool>(x_668.x, x_668.y, x_668.z);
  let x_671 : bool = u_xlatb0.x;
  if (x_671) {
  ...
  let x_680 : f32 = x_672;
  u_xlat0.x = x_680;
  let x_683 : bool = u_xlatb0.y;
  if (x_683) {
    let x_688 : f32 = u_xlat1.y;
    x_684 = x_688;
  } else {
    let x_691 : f32 = u_xlat2.y;
    x_684 = x_691;
  }
  let x_692 : f32 = x_684;
  u_xlat0.y = x_692;
  let x_695 : bool = u_xlatb0.z;
  if (x_695) {
    let x_700 : f32 = u_xlat1.z;
    x_696 = x_700;
  } else {
    let x_703 : f32 = u_xlat2.z;
    x_696 = x_703;
  }
  let x_704 : f32 = x_696;
  u_xlat0.z = x_704;
  let x_708 : vec4<f32> = u_xlat0;
  let x_711 : vec3<f32> = max(vec3<f32>(x_708.x, x_708.y, x_708.z), vec3<f32>(0.0f, 0.0f, 0.0f));

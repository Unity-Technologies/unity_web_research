...
fn main_1() {
  ...
  let x_255 : vec4<f32> = x_68.x_ClipRect;
  let x_259 : vec4<f32> = x_68.x_ClipRect;
  let x_261 : vec2<f32> = (-(vec2<f32>(x_255.x, x_255.y)) + vec2<f32>(x_259.z, x_259.w));
  let x_262 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_261.x, x_261.y, x_262.z, x_262.w);
  let x_264 : vec4<f32> = u_xlat1;
  let x_267 : vec4<f32> = vs_TEXCOORD2;
  let x_271 : vec2<f32> = (vec2<f32>(x_264.x, x_264.y) + -(abs(vec2<f32>(x_267.x, x_267.y))));
  let x_272 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_271.x, x_271.y, x_272.z, x_272.w);
  let x_274 : vec4<f32> = u_xlat1;
  let x_276 : vec4<f32> = vs_TEXCOORD2;
  let x_278 : vec2<f32> = (vec2<f32>(x_274.x, x_274.y) * vec2<f32>(x_276.z, x_276.w));
  let x_279 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_278.x, x_278.y, x_279.z, x_279.w);
  let x_281 : vec4<f32> = u_xlat1;
  let x_285 : vec2<f32> = clamp(vec2<f32>(x_281.x, x_281.y), vec2<f32>(0.0f, 0.0f), vec2<f32>(1.0f, 1.0f));
  let x_286 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_285.x, x_285.y, x_286.z, x_286.w);
  let x_289 : f32 = u_xlat1.y;
  let x_291 : f32 = u_xlat1.x;
  u_xlat1.x = (x_289 * x_291);
  let x_296 : f32 = u_xlat0.w;
  let x_298 : f32 = u_xlat1.x;
  u_xlat5 = ((x_296 * x_298) + -0.00100000004749745131f);
  let x_302 : vec4<f32> = u_xlat0;
  let x_303 : vec4<f32> = u_xlat1;
  u_xlat0 = (x_302 * vec4<f32>(x_303.x, x_303.x, x_303.x, x_303.x));
  let x_308 : vec4<f32> = u_xlat0;
  let x_309 : vec4<f32> = vs_COLOR0;
  SV_Target0 = (x_308 * vec4<f32>(x_309.w, x_309.w, x_309.w, x_309.w));
  let x_313 : f32 = u_xlat5;
  u_xlatb0 = (x_313 < 0.0f);
  let x_315 : bool = u_xlatb0;
  if (((select(0i, 1i, x_315) * -1i) != 0i)) {
    discard;
  }
  return;
}
...
@fragment
fn main(@location(1) vs_TEXCOORD0_param : vec2<f32>, @location(2) vs_TEXCOORD1_param : vec4<f32>, @location(4) vs_TEXCOORD5_param : vec4<f32>, @location(0) vs_COLOR0_param : vec4<f32>, @location(3) vs_TEXCOORD2_param : vec4<f32>, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

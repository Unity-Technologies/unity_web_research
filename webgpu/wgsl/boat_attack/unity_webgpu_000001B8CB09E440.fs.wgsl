...
fn main_1() {
  ...
  txVec0 = vec3<f32>(x_388.x, x_388.y, x_389);
  let x_401 : vec3<f32> = txVec0;
  let x_403 : f32 = textureSampleCompareLevel(x_ShadowMapTexture, sampler_ShadowMapTexture, x_401.xy, x_401.z);
  u_xlat0.x = x_403;
  let x_407 : f32 = x_37.x_LightShadowData.x;
  u_xlat4 = (-(x_407) + 1.0f);
  let x_412 : vec4<f32> = u_xlat0;
  let x_414 : f32 = u_xlat4;
  let x_418 : vec4<f32> = x_37.x_LightShadowData;
  SV_Target0 = ((vec4<f32>(x_412.x, x_412.x, x_412.x, x_412.x) * vec4<f32>(x_414, x_414, x_414, x_414)) + vec4<f32>(x_418.x, x_418.x, x_418.x, x_418.x));
  return;
}

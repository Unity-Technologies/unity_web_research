...
fn main_1() {
  ...
  let x_590 : vec4<f32> = x_40.x_Grain_TilingParams;
  ...
  let x_608 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_604.x, x_604.y), x_607);
  u_xlat15 = x_608.w;
  let x_610 : f32 = u_xlat15;
  u_xlat15 = (x_610 + -0.5f);
  let x_613 : f32 = u_xlat15;
  let x_614 : f32 = u_xlat15;
  u_xlat15 = (x_613 + x_614);
  let x_616 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_616.x, x_616.y, x_616.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_625 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_625);
  let x_630 : f32 = x_40.x_Grain_Params.y;
  let x_632 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_630 * -(x_632)) + 1.0f);
  let x_638 : f32 = u_xlat15;
  let x_640 : vec4<f32> = u_xlat0;
  u_xlat6 = (vec3<f32>(x_638, x_638, x_638) * vec3<f32>(x_640.x, x_640.y, x_640.z));
  let x_643 : vec3<f32> = u_xlat6;
  let x_646 : vec2<f32> = x_40.x_Grain_Params;
  u_xlat6 = (x_643 * vec3<f32>(x_646.x, x_646.x, x_646.x));
  let x_649 : vec3<f32> = u_xlat6;
  let x_650 : vec3<f32> = u_xlat1;
  let x_653 : vec4<f32> = u_xlat0;
  let x_655 : vec3<f32> = ((x_649 * vec3<f32>(x_650.x, x_650.x, x_650.x)) + vec3<f32>(x_653.x, x_653.y, x_653.z));
  let x_656 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_655.x, x_655.y, x_655.z, x_656.w);
  let x_658 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_658.x, x_658.y, x_658.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_661 : vec4<f32> = u_xlat0;
  let x_664 : vec3<f32> = log2(abs(vec3<f32>(x_661.x, x_661.y, x_661.z)));
  let x_665 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_664.x, x_664.y, x_664.z, x_665.w);
  let x_667 : vec4<f32> = u_xlat2;
  let x_669 : vec3<f32> = (vec3<f32>(x_667.x, x_667.y, x_667.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_680 : vec3<f32> = ((vec3<f32>(x_677.x, x_677.y, x_677.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_681 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_680.x, x_680.y, x_680.z, x_681.w);
  let x_684 : vec4<f32> = u_xlat0;
  let x_686 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_684.x, x_684.y, x_684.z, x_684.x));
  u_xlatb0 = vec3<bool>(x_686.x, x_686.y, x_686.z);
  ...
  let x_700 : f32 = x_692;
  SV_Target0.x = x_700;
  let x_704 : bool = u_xlatb0.y;
  if (x_704) {
    let x_709 : f32 = u_xlat1.y;
    x_705 = x_709;
  } else {
    let x_712 : f32 = u_xlat2.y;
    x_705 = x_712;
  }
  let x_713 : f32 = x_705;
  SV_Target0.y = x_713;
  let x_716 : bool = u_xlatb0.z;
  if (x_716) {
    let x_721 : f32 = u_xlat1.z;
    x_717 = x_721;
  } else {
    let x_724 : f32 = u_xlat2.z;
    x_717 = x_724;
  }
  let x_725 : f32 = x_717;
  SV_Target0.z = x_725;

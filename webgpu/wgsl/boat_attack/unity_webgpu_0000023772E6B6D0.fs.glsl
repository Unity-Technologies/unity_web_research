...
layout(set = 0, binding = 7) uniform mediump  sampler sampler_LinearRepeat;
void main()
{
    ...
    if(u_xlatb6){
        ...
        u_xlat11.x = float(1.0) / float(u_xlat6.y);
    ...
    u_xlat2 = texture(sampler2D(_Bloom_Texture, sampler_LinearClamp), u_xlat1.xw, _GlobalMipBias.x);
    ...
    u_xlat1.xw = u_xlat6.xy * _LensDirt_Params.xy + _LensDirt_Params.zw;
    u_xlat3.xyz = texture(sampler2D(_LensDirt_Texture, sampler_LinearClamp), u_xlat1.xw, _GlobalMipBias.x).xyz;
    ...
    SV_Target0.xyz = u_xlat6.xyz * u_xlat1.xxx + u_xlat0.xyz;
    SV_Target0.w = 1.0;
    return;
}

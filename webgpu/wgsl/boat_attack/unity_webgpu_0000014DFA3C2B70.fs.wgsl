...
fn main_1() {
  ...
  let x_774 : vec3<f32> = (vec3<f32>(x_772.x, x_772.y, x_772.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_775 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_774.x, x_774.y, x_774.z, x_775.w);
  let x_777 : vec4<f32> = u_xlat0;
  let x_780 : vec3<f32> = log2(abs(vec3<f32>(x_777.x, x_777.y, x_777.z)));
  let x_781 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_780.x, x_780.y, x_780.z, x_781.w);
  let x_783 : vec4<f32> = u_xlat2;
  let x_785 : vec3<f32> = (vec3<f32>(x_783.x, x_783.y, x_783.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_786 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_785.x, x_785.y, x_785.z, x_786.w);
  let x_788 : vec4<f32> = u_xlat2;
  let x_790 : vec3<f32> = exp2(vec3<f32>(x_788.x, x_788.y, x_788.z));
  let x_791 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_790.x, x_790.y, x_790.z, x_791.w);
  let x_793 : vec4<f32> = u_xlat2;
  let x_796 : vec3<f32> = ((vec3<f32>(x_793.x, x_793.y, x_793.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_797 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_796.x, x_796.y, x_796.z, x_797.w);
  let x_800 : vec4<f32> = u_xlat0;
  let x_802 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_800.x, x_800.y, x_800.z, x_800.x));
  ...
  let x_843 : vec4<f32> = x_13.x_Dithering_Params;
  let x_847 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_860 : f32 = x_13.x_GlobalMipBias.x;
  let x_861 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_857.x, x_857.y), x_860);
  u_xlat15 = x_861.w;
  let x_863 : f32 = u_xlat15;
  u_xlat15 = ((x_863 * 2.0f) + -1.0f);
  let x_868 : f32 = u_xlat15;
  u_xlatb1.x = (x_868 >= 0.0f);
  let x_872 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_872);
  let x_875 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_875)) + 1.0f);
  ...
  u_xlat15 = (x_884 * x_886);
  let x_888 : vec4<f32> = u_xlat0;
  let x_890 : vec3<f32> = (vec3<f32>(x_888.x, x_888.y, x_888.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_891 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_890.x, x_890.y, x_890.z, x_891.w);
  let x_893 : vec4<f32> = u_xlat0;
  let x_896 : vec3<f32> = log2(abs(vec3<f32>(x_893.x, x_893.y, x_893.z)));
  let x_897 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_896.x, x_896.y, x_896.z, x_897.w);
  let x_899 : vec4<f32> = u_xlat2;
  let x_901 : vec3<f32> = (vec3<f32>(x_899.x, x_899.y, x_899.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_902 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_901.x, x_901.y, x_901.z, x_902.w);
  let x_904 : vec4<f32> = u_xlat2;
  let x_906 : vec3<f32> = exp2(vec3<f32>(x_904.x, x_904.y, x_904.z));
  let x_907 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_906.x, x_906.y, x_906.z, x_907.w);
  let x_909 : vec4<f32> = u_xlat2;
  let x_912 : vec3<f32> = ((vec3<f32>(x_909.x, x_909.y, x_909.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  ...
  let x_962 : vec3<f32> = ((vec3<f32>(x_955, x_955, x_955) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_960.x, x_960.y, x_960.z));
  let x_963 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_962.x, x_962.y, x_962.z, x_963.w);
  let x_965 : vec4<f32> = u_xlat0;
  let x_967 : vec3<f32> = (vec3<f32>(x_965.x, x_965.y, x_965.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_968 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_967.x, x_967.y, x_967.z, x_968.w);
  let x_970 : vec4<f32> = u_xlat0;
  let x_972 : vec3<f32> = (vec3<f32>(x_970.x, x_970.y, x_970.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_973 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_972.x, x_972.y, x_972.z, x_973.w);
  let x_975 : vec4<f32> = u_xlat2;
  let x_977 : vec3<f32> = (vec3<f32>(x_975.x, x_975.y, x_975.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_978 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_977.x, x_977.y, x_977.z, x_978.w);
  let x_980 : vec4<f32> = u_xlat2;
  let x_983 : vec3<f32> = log2(abs(vec3<f32>(x_980.x, x_980.y, x_980.z)));
  let x_984 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_983.x, x_983.y, x_983.z, x_984.w);
  let x_986 : vec4<f32> = u_xlat2;
  let x_988 : vec3<f32> = (vec3<f32>(x_986.x, x_986.y, x_986.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_998 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_996.x, x_996.y, x_996.z, x_996.x));
  ...
  let x_1042 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1041.x, x_1041.y, x_1041.z, x_1042.w);
  SV_Target0.w = 1.0f;
  return;
}

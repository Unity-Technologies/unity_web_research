...
fn main_1() {
  ...
  let x_695 : vec4<f32> = x_38.x_Dithering_Params;
  let x_699 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_713 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_709.x, x_709.y), x_712);
  u_xlat15 = x_713.w;
  let x_715 : f32 = u_xlat15;
  u_xlat15 = ((x_715 * 2.0f) + -1.0f);
  let x_718 : f32 = u_xlat15;
  u_xlatb1.x = (x_718 >= 0.0f);
  let x_722 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_722);
  let x_725 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_725)) + 1.0f);
  ...
  u_xlat15 = (x_734 * x_736);
  let x_738 : vec4<f32> = u_xlat0;
  let x_740 : vec3<f32> = (vec3<f32>(x_738.x, x_738.y, x_738.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_741 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_740.x, x_740.y, x_740.z, x_741.w);
  let x_743 : vec4<f32> = u_xlat0;
  let x_746 : vec3<f32> = log2(abs(vec3<f32>(x_743.x, x_743.y, x_743.z)));
  let x_747 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_746.x, x_746.y, x_746.z, x_747.w);
  let x_749 : vec4<f32> = u_xlat2;
  let x_751 : vec3<f32> = (vec3<f32>(x_749.x, x_749.y, x_749.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_752 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_751.x, x_751.y, x_751.z, x_752.w);
  let x_754 : vec4<f32> = u_xlat2;
  let x_756 : vec3<f32> = exp2(vec3<f32>(x_754.x, x_754.y, x_754.z));
  let x_757 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_756.x, x_756.y, x_756.z, x_757.w);
  let x_759 : vec4<f32> = u_xlat2;
  let x_762 : vec3<f32> = ((vec3<f32>(x_759.x, x_759.y, x_759.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_763 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_762.x, x_762.y, x_762.z, x_763.w);
  let x_766 : vec4<f32> = u_xlat0;
  let x_768 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_766.x, x_766.y, x_766.z, x_766.x));
  u_xlatb0 = vec3<bool>(x_768.x, x_768.y, x_768.z);
  let x_771 : bool = u_xlatb0.x;
  if (x_771) {
  ...
  let x_813 : vec3<f32> = ((vec3<f32>(x_806, x_806, x_806) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_811.x, x_811.y, x_811.z));
  let x_814 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_813.x, x_813.y, x_813.z, x_814.w);
  let x_816 : vec4<f32> = u_xlat0;
  let x_818 : vec3<f32> = (vec3<f32>(x_816.x, x_816.y, x_816.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_819 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_818.x, x_818.y, x_818.z, x_819.w);
  let x_821 : vec4<f32> = u_xlat0;
  let x_823 : vec3<f32> = (vec3<f32>(x_821.x, x_821.y, x_821.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_824 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_823.x, x_823.y, x_823.z, x_824.w);
  let x_826 : vec4<f32> = u_xlat2;
  let x_828 : vec3<f32> = (vec3<f32>(x_826.x, x_826.y, x_826.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_839 : vec3<f32> = (vec3<f32>(x_837.x, x_837.y, x_837.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_849 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_847.x, x_847.y, x_847.z, x_847.x));
  u_xlatb0 = vec3<bool>(x_849.x, x_849.y, x_849.z);
  let x_852 : bool = u_xlatb0.x;
  if (x_852) {
  ...
  let x_861 : f32 = x_853;
  u_xlat0.x = x_861;
  let x_864 : bool = u_xlatb0.y;
  if (x_864) {
    let x_869 : f32 = u_xlat1.y;
    x_865 = x_869;
  } else {
    let x_872 : f32 = u_xlat2.y;
    x_865 = x_872;
  }
  let x_873 : f32 = x_865;
  u_xlat0.y = x_873;
  let x_876 : bool = u_xlatb0.z;
  if (x_876) {
    let x_881 : f32 = u_xlat1.z;
    x_877 = x_881;
  } else {
  ...
  let x_892 : vec3<f32> = max(vec3<f32>(x_889.x, x_889.y, x_889.z), vec3<f32>(0.0f, 0.0f, 0.0f));

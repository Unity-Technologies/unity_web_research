...
fn main_1() {
  ...
  let x_152 : vec4<f32> = x_21.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_150, vec3<f32>(x_152.x, x_152.y, x_152.z));
  let x_157 : vec3<f32> = in_NORMAL0;
  let x_159 : vec4<f32> = x_21.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_157, vec3<f32>(x_159.x, x_159.y, x_159.z));
  let x_163 : vec3<f32> = in_NORMAL0;
  let x_165 : vec4<f32> = x_21.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_163, vec3<f32>(x_165.x, x_165.y, x_165.z));
  let x_169 : vec3<f32> = u_xlat0;
  let x_170 : vec3<f32> = u_xlat0;
  u_xlat6 = dot(x_169, x_170);
  let x_172 : f32 = u_xlat6;
  u_xlat6 = max(x_172, 1.17549435e-38f);
  let x_174 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_174);
  let x_177 : f32 = u_xlat6;
  let x_179 : vec3<f32> = u_xlat0;
  vs_INTERP9 = (vec3<f32>(x_177, x_177, x_177) * x_179);
  return;
}
...
@vertex
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>, @location(4) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP8, gl_Position, vs_INTERP4, vs_INTERP5, vs_INTERP6, vs_INTERP9);
}

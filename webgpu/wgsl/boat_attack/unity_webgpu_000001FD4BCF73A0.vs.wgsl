...
@group(0) @binding(12) var x_TerrainNormalmapTexture : texture_2d<f32>;

@group(0) @binding(11) var x_TerrainHeightmapTexture : texture_2d<f32>;
...
@group(1) @binding(3) var<uniform> x_270 : LightShadows;

var<private> vs_TEXCOORD8 : vec4<f32>;

@group(1) @binding(4) var<uniform> x_314 : VGlobals;
...
fn main_1() {
  ...
  let x_272 : vec4<f32> = x_270.x_MainLightWorldToShadow[0i][1i];
  let x_274 : vec3<f32> = (vec3<f32>(x_261.y, x_261.y, x_261.y) * vec3<f32>(x_272.x, x_272.y, x_272.z));
  let x_275 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_274.x, x_274.y, x_274.z, x_275.w);
  let x_278 : vec4<f32> = x_270.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_289 : vec4<f32> = x_270.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_303 : vec4<f32> = x_270.x_MainLightWorldToShadow[0i][3i];
  let x_305 : vec3<f32> = (vec3<f32>(x_300.x, x_300.y, x_300.z) + vec3<f32>(x_303.x, x_303.y, x_303.z));
  let x_306 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_305.x, x_305.y, x_305.z, x_306.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_310 : vec4<f32> = u_xlat0;
  let x_316 : vec4<f32> = x_314.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_310.y, x_310.y, x_310.y, x_310.y) * x_316);
  let x_319 : vec4<f32> = x_314.unity_MatrixVP[0i];
  let x_320 : vec4<f32> = u_xlat0;
  let x_323 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_319 * vec4<f32>(x_320.x, x_320.x, x_320.x, x_320.x)) + x_323);
  let x_326 : vec4<f32> = x_314.unity_MatrixVP[2i];
  let x_327 : vec4<f32> = u_xlat0;
  let x_330 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_326 * vec4<f32>(x_327.z, x_327.z, x_327.z, x_327.z)) + x_330);
  let x_336 : vec4<f32> = u_xlat0;
  let x_338 : vec4<f32> = x_314.unity_MatrixVP[3i];
  gl_Position = (x_336 + x_338);
  return;
}

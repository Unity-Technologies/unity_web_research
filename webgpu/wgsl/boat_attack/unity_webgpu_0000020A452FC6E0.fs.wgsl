...
fn main_1() {
  ...
  let x_947 : vec3<f32> = sqrt(vec3<f32>(x_945.x, x_945.y, x_945.z));
  let x_948 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_947.x, x_947.y, x_947.z, x_948.w);
  let x_950 : vec2<f32> = vs_TEXCOORD0;
  let x_953 : vec4<f32> = x_25.x_Dithering_Params;
  let x_957 : vec4<f32> = x_25.x_Dithering_Params;
  let x_959 : vec2<f32> = ((x_950 * vec2<f32>(x_953.x, x_953.y)) + vec2<f32>(x_957.z, x_957.w));
  let x_960 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_959.x, x_959.y, x_960.z, x_960.w);
  let x_967 : vec4<f32> = u_xlat1;
  let x_970 : f32 = x_25.x_GlobalMipBias.x;
  let x_971 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_967.x, x_967.y), x_970);
  u_xlat15 = x_971.w;
  let x_973 : f32 = u_xlat15;
  u_xlat15 = ((x_973 * 2.0f) + -1.0f);
  let x_977 : f32 = u_xlat15;
  u_xlatb1.x = (x_977 >= 0.0f);
  let x_981 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_981);
  let x_984 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_984)) + 1.0f);
  ...
  u_xlat15 = (x_993 * x_995);
  let x_997 : vec4<f32> = u_xlat0;
  let x_999 : vec3<f32> = (vec3<f32>(x_997.x, x_997.y, x_997.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_1000 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_999.x, x_999.y, x_999.z, x_1000.w);
  let x_1002 : vec4<f32> = u_xlat0;
  let x_1004 : vec3<f32> = log2(vec3<f32>(x_1002.x, x_1002.y, x_1002.z));
  let x_1005 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1004.x, x_1004.y, x_1004.z, x_1005.w);
  let x_1007 : vec4<f32> = u_xlat2;
  let x_1009 : vec3<f32> = (vec3<f32>(x_1007.x, x_1007.y, x_1007.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_1010 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1009.x, x_1009.y, x_1009.z, x_1010.w);
  let x_1012 : vec4<f32> = u_xlat2;
  let x_1014 : vec3<f32> = exp2(vec3<f32>(x_1012.x, x_1012.y, x_1012.z));
  let x_1015 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1014.x, x_1014.y, x_1014.z, x_1015.w);
  let x_1017 : vec4<f32> = u_xlat2;
  let x_1020 : vec3<f32> = ((vec3<f32>(x_1017.x, x_1017.y, x_1017.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_1021 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1020.x, x_1020.y, x_1020.z, x_1021.w);
  let x_1024 : vec4<f32> = u_xlat0;
  let x_1026 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_1024.x, x_1024.y, x_1024.z, x_1024.x));
  ...
  let x_1071 : vec3<f32> = ((vec3<f32>(x_1064, x_1064, x_1064) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1069.x, x_1069.y, x_1069.z));
  let x_1072 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1071.x, x_1071.y, x_1071.z, x_1072.w);
  let x_1074 : vec4<f32> = u_xlat0;
  let x_1076 : vec3<f32> = (vec3<f32>(x_1074.x, x_1074.y, x_1074.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1077 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1076.x, x_1076.y, x_1076.z, x_1077.w);
  let x_1079 : vec4<f32> = u_xlat0;
  let x_1081 : vec3<f32> = (vec3<f32>(x_1079.x, x_1079.y, x_1079.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1082 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1081.x, x_1081.y, x_1081.z, x_1082.w);
  let x_1084 : vec4<f32> = u_xlat2;
  let x_1086 : vec3<f32> = (vec3<f32>(x_1084.x, x_1084.y, x_1084.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1087 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1086.x, x_1086.y, x_1086.z, x_1087.w);
  let x_1089 : vec4<f32> = u_xlat2;
  let x_1092 : vec3<f32> = log2(abs(vec3<f32>(x_1089.x, x_1089.y, x_1089.z)));
  let x_1093 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1092.x, x_1092.y, x_1092.z, x_1093.w);
  let x_1095 : vec4<f32> = u_xlat2;
  let x_1097 : vec3<f32> = (vec3<f32>(x_1095.x, x_1095.y, x_1095.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_1098 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1097.x, x_1097.y, x_1097.z, x_1098.w);
  let x_1100 : vec4<f32> = u_xlat2;
  let x_1102 : vec3<f32> = exp2(vec3<f32>(x_1100.x, x_1100.y, x_1100.z));
  let x_1103 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1102.x, x_1102.y, x_1102.z, x_1103.w);
  let x_1105 : vec4<f32> = u_xlat0;
  let x_1107 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1105.x, x_1105.y, x_1105.z, x_1105.x));
  ...
  let x_1150 : vec3<f32> = max(vec3<f32>(x_1147.x, x_1147.y, x_1147.z), vec3<f32>(0.0f, 0.0f, 0.0f));

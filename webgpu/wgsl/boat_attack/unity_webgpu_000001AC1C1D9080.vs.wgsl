...
@group(1) @binding(6) var<uniform> x_149 : VGlobals;
...
fn main_1() {
  ...
  let x_151 : vec4<f32> = x_149.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_145.y, x_145.y, x_145.y, x_145.y) * x_151);
  let x_154 : vec4<f32> = x_149.unity_MatrixVP[0i];
  let x_155 : vec4<f32> = u_xlat0;
  let x_158 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_154 * vec4<f32>(x_155.x, x_155.x, x_155.x, x_155.x)) + x_158);
  let x_161 : vec4<f32> = x_149.unity_MatrixVP[2i];
  let x_162 : vec4<f32> = u_xlat0;
  let x_165 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_161 * vec4<f32>(x_162.z, x_162.z, x_162.z, x_162.z)) + x_165);
  let x_171 : vec4<f32> = u_xlat0;
  let x_173 : vec4<f32> = x_149.unity_MatrixVP[3i];

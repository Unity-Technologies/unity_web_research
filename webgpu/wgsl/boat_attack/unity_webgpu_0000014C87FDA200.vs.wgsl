...
@group(1) @binding(5) var<uniform> x_136 : VGlobals;
...
@group(1) @binding(3) var<uniform> x_296 : LightShadows;
...
fn main_1() {
  ...
  let x_298 : vec4<f32> = x_296.x_MainLightWorldToShadow[0i][1i];
  let x_300 : vec3<f32> = (vec3<f32>(x_287.y, x_287.y, x_287.y) * vec3<f32>(x_298.x, x_298.y, x_298.z));
  let x_301 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_300.x, x_300.y, x_300.z, x_301.w);
  let x_304 : vec4<f32> = x_296.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_315 : vec4<f32> = x_296.x_MainLightWorldToShadow[0i][2i];
  let x_317 : vec3<f32> = u_xlat1;
  let x_320 : vec4<f32> = u_xlat0;
  let x_322 : vec3<f32> = ((vec3<f32>(x_315.x, x_315.y, x_315.z) * vec3<f32>(x_317.z, x_317.z, x_317.z)) + vec3<f32>(x_320.x, x_320.y, x_320.z));
  let x_323 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_322.x, x_322.y, x_322.z, x_323.w);
  let x_326 : vec4<f32> = u_xlat0;
  let x_329 : vec4<f32> = x_296.x_MainLightWorldToShadow[0i][3i];
  let x_331 : vec3<f32> = (vec3<f32>(x_326.x, x_326.y, x_326.z) + vec3<f32>(x_329.x, x_329.y, x_329.z));
  let x_332 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_331.x, x_331.y, x_331.z, x_332.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_335 : vec3<f32> = u_xlat1;
  let x_338 : vec4<f32> = x_136.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_335.y, x_335.y, x_335.y, x_335.y) * x_338);
  let x_341 : vec4<f32> = x_136.unity_MatrixVP[0i];
  let x_342 : vec3<f32> = u_xlat1;
  let x_345 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_341 * vec4<f32>(x_342.x, x_342.x, x_342.x, x_342.x)) + x_345);
  let x_348 : vec4<f32> = x_136.unity_MatrixVP[2i];
  let x_349 : vec3<f32> = u_xlat1;
  let x_352 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_348 * vec4<f32>(x_349.z, x_349.z, x_349.z, x_349.z)) + x_352);
  let x_358 : vec4<f32> = u_xlat0;
  let x_360 : vec4<f32> = x_136.unity_MatrixVP[3i];
  gl_Position = (x_358 + x_360);
  return;
}

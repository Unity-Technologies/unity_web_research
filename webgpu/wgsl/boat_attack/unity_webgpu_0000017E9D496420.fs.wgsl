...
fn main_1() {
  ...
  let x_734 : f32 = x_22.x_FaceShininess;
  let x_738 : f32 = x_22.x_OutlineShininess;
  u_xlat4.x = (-(x_734) + x_738);
  let x_741 : f32 = u_xlat11;
  let x_743 : f32 = u_xlat4.x;
  let x_746 : f32 = x_22.x_FaceShininess;
  u_xlat4.x = ((x_741 * x_743) + x_746);
  let x_750 : f32 = u_xlat4.x;
  u_xlat4.x = (x_750 * 128.0f);
  let x_755 : f32 = u_xlat0.x;
  let x_757 : f32 = u_xlat4.x;
  u_xlat0.x = (x_755 * x_757);
  let x_761 : f32 = u_xlat0.x;
  u_xlat0.x = exp2(x_761);
  let x_764 : vec3<f32> = vs_TEXCOORD5;
  let x_768 : vec4<f32> = x_22.unity_WorldToLight[1i];
  u_xlat2 = (vec4<f32>(x_764.y, x_764.y, x_764.y, x_764.y) * x_768);
  let x_771 : vec4<f32> = x_22.unity_WorldToLight[0i];
  let x_772 : vec3<f32> = vs_TEXCOORD5;
  let x_775 : vec4<f32> = u_xlat2;
  u_xlat2 = ((x_771 * vec4<f32>(x_772.x, x_772.x, x_772.x, x_772.x)) + x_775);
  let x_779 : vec4<f32> = x_22.unity_WorldToLight[2i];
  let x_780 : vec3<f32> = vs_TEXCOORD5;
  let x_783 : vec4<f32> = u_xlat2;
  u_xlat2 = ((x_779 * vec4<f32>(x_780.z, x_780.z, x_780.z, x_780.z)) + x_783);
  let x_785 : vec4<f32> = u_xlat2;
  let x_787 : vec4<f32> = x_22.unity_WorldToLight[3i];
  u_xlat2 = (x_785 + x_787);
  let x_789 : vec4<f32> = u_xlat2;
  let x_791 : vec4<f32> = u_xlat2;
  u_xlat4 = (vec2<f32>(x_789.x, x_789.y) / vec2<f32>(x_791.w, x_791.w));
  let x_794 : vec2<f32> = u_xlat4;
  u_xlat4 = (x_794 + vec2<f32>(0.5f, 0.5f));
  let x_802 : vec2<f32> = u_xlat4;
  let x_803 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, x_802);
  u_xlat4.x = x_803.w;
  let x_808 : f32 = u_xlat2.z;
  u_xlatb8 = (0.0f < x_808);
  let x_810 : vec4<f32> = u_xlat2;
  let x_812 : vec4<f32> = u_xlat2;
  u_xlat2.x = dot(vec3<f32>(x_810.x, x_810.y, x_810.z), vec3<f32>(x_812.x, x_812.y, x_812.z));
  let x_821 : vec4<f32> = u_xlat2;
  let x_823 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_821.x, x_821.x));
  u_xlat2.x = x_823.x;
  let x_827 : bool = u_xlatb8;
  u_xlat8 = select(0.0f, 1.0f, x_827);
  let x_830 : f32 = u_xlat4.x;
  let x_831 : f32 = u_xlat8;
  u_xlat4.x = (x_830 * x_831);
  let x_835 : f32 = u_xlat2.x;
  let x_837 : f32 = u_xlat4.x;
  u_xlat4.x = (x_835 * x_837);
  let x_840 : vec2<f32> = u_xlat4;
  let x_844 : vec4<f32> = x_22.x_LightColor0;
  let x_846 : vec3<f32> = (vec3<f32>(x_840.x, x_840.x, x_840.x) * vec3<f32>(x_844.x, x_844.y, x_844.z));
  let x_847 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_846.x, x_846.y, x_846.z, x_847.w);
  let x_849 : vec4<f32> = u_xlat2;
  let x_853 : vec4<f32> = x_22.x_SpecColor;
  let x_855 : vec3<f32> = (vec3<f32>(x_849.x, x_849.y, x_849.z) * vec3<f32>(x_853.x, x_853.y, x_853.z));
  ...
  let x_862 : vec3<f32> = (vec3<f32>(x_858.x, x_858.x, x_858.x) * vec3<f32>(x_860.x, x_860.y, x_860.z));
  let x_863 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_862.x, x_862.y, x_862.z, x_863.w);
  let x_866 : f32 = u_xlat1.w;
  u_xlat14 = max(x_866, 0.00009999999747378752f);
  ...
  let x_879 : f32 = u_xlat1.w;
  SV_Target0.w = x_879;
  let x_882 : vec4<f32> = u_xlat2;
  let x_884 : vec4<f32> = u_xlat1;
  let x_886 : vec3<f32> = (vec3<f32>(x_882.x, x_882.y, x_882.z) * vec3<f32>(x_884.x, x_884.y, x_884.z));
  ...
  let x_896 : vec3<f32> = ((vec3<f32>(x_889.x, x_889.y, x_889.z) * vec3<f32>(x_891.w, x_891.w, x_891.w)) + vec3<f32>(x_894.x, x_894.y, x_894.z));
  let x_897 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_896.x, x_896.y, x_896.z, x_897.w);
  let x_900 : f32 = vs_TEXCOORD9;
  let x_902 : f32 = x_22.x_ProjectionParams.y;
  u_xlat12 = (x_900 / x_902);
  let x_904 : f32 = u_xlat12;
  u_xlat12 = (-(x_904) + 1.0f);
  let x_907 : f32 = u_xlat12;
  let x_909 : f32 = x_22.x_ProjectionParams.z;
  u_xlat12 = (x_907 * x_909);
  let x_911 : f32 = u_xlat12;
  u_xlat12 = max(x_911, 0.0f);
  let x_913 : f32 = u_xlat12;
  let x_916 : f32 = x_22.unity_FogParams.x;
  u_xlat12 = (x_913 * x_916);
  let x_918 : f32 = u_xlat12;
  let x_919 : f32 = u_xlat12;
  u_xlat12 = (x_918 * -(x_919));
  let x_922 : f32 = u_xlat12;
  u_xlat12 = exp2(x_922);
  let x_924 : vec4<f32> = u_xlat0;
  let x_926 : f32 = u_xlat12;
  let x_928 : vec3<f32> = (vec3<f32>(x_924.x, x_924.y, x_924.z) * vec3<f32>(x_926, x_926, x_926));
  let x_929 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_928.x, x_928.y, x_928.z, x_929.w);
  return;
}
...
@fragment
fn main(@location(2) vs_TEXCOORD6_param : vec2<f32>, @location(0) vs_TEXCOORD0_param : vec4<f32>, @location(1) vs_TEXCOORD1_param : vec2<f32>, @location(8) vs_COLOR0_param : vec4<f32>, @location(3) vs_TEXCOORD2_param : vec3<f32>, @location(5) vs_TEXCOORD3_param : vec3<f32>, @location(6) vs_TEXCOORD4_param : vec3<f32>, @location(7) vs_TEXCOORD5_param : vec3<f32>, @location(4) vs_TEXCOORD9_param : f32, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

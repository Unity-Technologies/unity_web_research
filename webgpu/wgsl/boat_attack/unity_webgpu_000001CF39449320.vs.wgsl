...
fn main_1() {
  ...
  u_xlat0 = (vec3<f32>(x_627.y, x_627.y, x_627.y) * vec3<f32>(x_631.x, x_631.y, x_631.z));
  let x_635 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_637 : vec4<f32> = u_xlat1;
  let x_640 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_635.x, x_635.y, x_635.z) * vec3<f32>(x_637.x, x_637.x, x_637.x)) + x_640);
  let x_643 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_645 : vec4<f32> = u_xlat1;
  let x_648 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_643.x, x_643.y, x_643.z) * vec3<f32>(x_645.z, x_645.z, x_645.z)) + x_648);
  let x_652 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_654 : vec4<f32> = u_xlat1;
  let x_657 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec3<f32>(x_652.x, x_652.y, x_652.z) * vec3<f32>(x_654.w, x_654.w, x_654.w)) + x_657);
  return;
}

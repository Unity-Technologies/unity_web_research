...
fn main_1() {
  ...
  let x_208 : f32 = vs_PREV_POSITION_CS_NO_JITTER0.w;
  u_xlat0.x = (1.0f / x_208);
  let x_211 : vec2<f32> = u_xlat0;
  let x_213 : vec4<f32> = vs_PREV_POSITION_CS_NO_JITTER0;
  u_xlat0 = (vec2<f32>(x_211.x, x_211.x) * vec2<f32>(x_213.x, x_213.y));
  let x_218 : f32 = vs_POSITION_CS_NO_JITTER0.w;
  u_xlat2 = (1.0f / x_218);
  let x_220 : vec4<f32> = vs_POSITION_CS_NO_JITTER0;
  let x_222 : f32 = u_xlat2;
  let x_225 : vec2<f32> = u_xlat0;
  u_xlat0 = ((vec2<f32>(x_220.x, x_220.y) * vec2<f32>(x_222, x_222)) + -(x_225));
  let x_228 : vec2<f32> = u_xlat0;
  u_xlat0 = (x_228 * vec2<f32>(0.5f, -0.5f));
  let x_235 : f32 = x_171.unity_MotionVectorsParams.y;
  u_xlatb2 = !((x_235 == 0.0f));
  let x_239 : bool = u_xlatb2;
  if (x_239) {
    let x_244 : vec2<f32> = u_xlat0;
    x_241 = x_244;
  } else {
    x_241 = vec2<f32>(0.0f, 0.0f);
  }
  let x_247 : vec2<f32> = x_241;
  let x_248 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_247.x, x_247.y, x_248.z, x_248.w);
  let x_250 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_250.x, x_250.y, vec2<f32>(0.0f, 0.0f).x, vec2<f32>(0.0f, 0.0f).y);
  return;
}
...
@fragment
fn main(@builtin(position) gl_FragCoord_param : vec4<f32>, @location(2) vs_TEXCOORD0_param : vec2<f32>, @location(1) vs_PREV_POSITION_CS_NO_JITTER0_param : vec4<f32>, @location(0) vs_POSITION_CS_NO_JITTER0_param : vec4<f32>) -> main_out {

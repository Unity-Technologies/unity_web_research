...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_580.x, x_580.y, x_580.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_583 : vec4<f32> = u_xlat0;
  let x_586 : vec3<f32> = log2(abs(vec3<f32>(x_583.x, x_583.y, x_583.z)));
  let x_587 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_586.x, x_586.y, x_586.z, x_587.w);
  let x_589 : vec4<f32> = u_xlat2;
  let x_591 : vec3<f32> = (vec3<f32>(x_589.x, x_589.y, x_589.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_592 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_591.x, x_591.y, x_591.z, x_592.w);
  let x_594 : vec4<f32> = u_xlat2;
  let x_596 : vec3<f32> = exp2(vec3<f32>(x_594.x, x_594.y, x_594.z));
  let x_597 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_596.x, x_596.y, x_596.z, x_597.w);
  let x_599 : vec4<f32> = u_xlat2;
  let x_602 : vec3<f32> = ((vec3<f32>(x_599.x, x_599.y, x_599.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_603 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_602.x, x_602.y, x_602.z, x_603.w);
  let x_606 : vec4<f32> = u_xlat0;
  let x_608 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_606.x, x_606.y, x_606.z, x_606.x));
  ...
  let x_653 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_666 : f32 = x_13.x_GlobalMipBias.x;
  let x_667 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_663.x, x_663.y), x_666);
  u_xlat15 = x_667.w;
  let x_669 : f32 = u_xlat15;
  u_xlat15 = ((x_669 * 2.0f) + -1.0f);
  let x_674 : f32 = u_xlat15;
  u_xlatb1.x = (x_674 >= 0.0f);
  let x_678 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_678);
  let x_681 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_681)) + 1.0f);
  ...
  u_xlat15 = (x_690 * x_692);
  let x_694 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_694.x, x_694.y, x_694.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_721 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_719.x, x_719.y, x_719.z, x_719.x));
  ...
  let x_766 : vec3<f32> = ((vec3<f32>(x_759, x_759, x_759) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_764.x, x_764.y, x_764.z));
  let x_767 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_766.x, x_766.y, x_766.z, x_767.w);
  let x_769 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_769.x, x_769.y, x_769.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));

...
fn main_1() {
  ...
  let x_827 : vec2<f32> = vs_TEXCOORD0;
  let x_830 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_847 : f32 = x_38.x_GlobalMipBias.x;
  let x_848 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_844.x, x_844.y), x_847);
  u_xlat15 = x_848.w;
  let x_850 : f32 = u_xlat15;
  u_xlat15 = ((x_850 * 2.0f) + -1.0f);
  let x_853 : f32 = u_xlat15;
  u_xlatb1.x = (x_853 >= 0.0f);
  let x_857 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_857);
  let x_860 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_860)) + 1.0f);
  ...
  u_xlat15 = (x_869 * x_871);
  let x_873 : vec4<f32> = u_xlat0;
  let x_875 : vec3<f32> = (vec3<f32>(x_873.x, x_873.y, x_873.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_876 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_875.x, x_875.y, x_875.z, x_876.w);
  let x_878 : vec4<f32> = u_xlat0;
  let x_881 : vec3<f32> = log2(abs(vec3<f32>(x_878.x, x_878.y, x_878.z)));
  let x_882 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_881.x, x_881.y, x_881.z, x_882.w);
  let x_884 : vec4<f32> = u_xlat2;
  let x_886 : vec3<f32> = (vec3<f32>(x_884.x, x_884.y, x_884.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_897 : vec3<f32> = ((vec3<f32>(x_894.x, x_894.y, x_894.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_898 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_897.x, x_897.y, x_897.z, x_898.w);
  let x_901 : vec4<f32> = u_xlat0;
  let x_903 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_901.x, x_901.y, x_901.z, x_901.x));
  u_xlatb0 = vec3<bool>(x_903.x, x_903.y, x_903.z);
  let x_906 : bool = u_xlatb0.x;
  if (x_906) {
  ...
  let x_915 : f32 = x_907;
  u_xlat0.x = x_915;
  let x_918 : bool = u_xlatb0.y;
  if (x_918) {
    let x_923 : f32 = u_xlat1.y;
    x_919 = x_923;
  } else {
    let x_926 : f32 = u_xlat2.y;
    x_919 = x_926;
  }
  let x_927 : f32 = x_919;
  u_xlat0.y = x_927;
  let x_930 : bool = u_xlatb0.z;
  if (x_930) {
  ...
  let x_939 : f32 = x_931;
  u_xlat0.z = x_939;
  let x_941 : f32 = u_xlat15;
  let x_946 : vec4<f32> = u_xlat0;
  let x_948 : vec3<f32> = ((vec3<f32>(x_941, x_941, x_941) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_946.x, x_946.y, x_946.z));
  let x_949 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_948.x, x_948.y, x_948.z, x_949.w);
  let x_951 : vec4<f32> = u_xlat0;
  let x_953 : vec3<f32> = (vec3<f32>(x_951.x, x_951.y, x_951.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_954 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_953.x, x_953.y, x_953.z, x_954.w);
  let x_956 : vec4<f32> = u_xlat0;
  let x_958 : vec3<f32> = (vec3<f32>(x_956.x, x_956.y, x_956.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_959 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_958.x, x_958.y, x_958.z, x_959.w);
  let x_961 : vec4<f32> = u_xlat2;
  let x_963 : vec3<f32> = (vec3<f32>(x_961.x, x_961.y, x_961.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_964 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_963.x, x_963.y, x_963.z, x_964.w);
  let x_966 : vec4<f32> = u_xlat2;
  let x_969 : vec3<f32> = log2(abs(vec3<f32>(x_966.x, x_966.y, x_966.z)));
  let x_970 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_969.x, x_969.y, x_969.z, x_970.w);
  let x_972 : vec4<f32> = u_xlat2;
  let x_974 : vec3<f32> = (vec3<f32>(x_972.x, x_972.y, x_972.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_984 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_982.x, x_982.y, x_982.z, x_982.x));
  ...
  let x_1027 : vec3<f32> = max(vec3<f32>(x_1024.x, x_1024.y, x_1024.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1028 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1027.x, x_1027.y, x_1027.z, x_1028.w);
  SV_Target0.w = 1.0f;
  return;
}

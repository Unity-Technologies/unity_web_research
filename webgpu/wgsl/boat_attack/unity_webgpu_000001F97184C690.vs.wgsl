...
fn main_1() {
  ...
  vs_TEXCOORD6.w = 0.0f;
  let x_315 : vec2<f32> = in_TEXCOORD1;
  let x_318 : vec4<f32> = x_44.unity_LightmapST;
  let x_322 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_315 * vec2<f32>(x_318.x, x_318.y)) + vec2<f32>(x_322.z, x_322.w));
  let x_325 : vec4<f32> = u_xlat0;
  let x_328 : vec4<f32> = x_142.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_325.y, x_325.y, x_325.y, x_325.y) * x_328);
  let x_331 : vec4<f32> = x_142.unity_MatrixVP[0i];
  let x_332 : vec4<f32> = u_xlat0;
  let x_335 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_331 * vec4<f32>(x_332.x, x_332.x, x_332.x, x_332.x)) + x_335);
  let x_338 : vec4<f32> = x_142.unity_MatrixVP[2i];
  let x_339 : vec4<f32> = u_xlat0;
  let x_342 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_338 * vec4<f32>(x_339.z, x_339.z, x_339.z, x_339.z)) + x_342);
  let x_348 : vec4<f32> = u_xlat0;
  let x_350 : vec4<f32> = x_142.unity_MatrixVP[3i];

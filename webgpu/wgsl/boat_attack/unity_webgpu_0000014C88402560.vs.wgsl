...
@group(1) @binding(6) var<uniform> x_394 : VGlobals;

var<private> vs_INTERP0 : vec2<f32>;

var<private> in_TEXCOORD1 : vec4<f32>;

@group(1) @binding(3) var<uniform> x_445 : LightShadows;
...
fn main_1() {
  ...
  u_xlat8 = ((x_63 * 0.05000000074505805969f) + 1.0f);
  ...
  u_xlat12 = (x_282 * 0.10000000149011611938f);
  ...
  u_xlat13 = max(x_310, 1.17549435e-38f);
  ...
  vs_INTERP9 = (vec3<f32>(x_324, x_324, x_324) * vec3<f32>(x_326.x, x_326.y, x_326.z));
  ...
  u_xlat2.y = ((x_336 * -0.34999999403953552246f) + 0.34999999403953552246f);
  ...
  let x_418 : vec4<f32> = x_394.unity_MatrixVP[3i];
  gl_Position = (x_416 + x_418);
  let x_425 : vec4<f32> = in_TEXCOORD1;
  let x_429 : vec4<f32> = x_16.unity_LightmapST;
  let x_433 : vec4<f32> = x_16.unity_LightmapST;
  vs_INTERP0 = ((vec2<f32>(x_425.x, x_425.y) * vec2<f32>(x_429.x, x_429.y)) + vec2<f32>(x_433.z, x_433.w));
  let x_436 : vec4<f32> = u_xlat0;
  let x_447 : vec4<f32> = x_445.x_MainLightWorldToShadow[0i][1i];
  let x_449 : vec3<f32> = (vec3<f32>(x_436.y, x_436.y, x_436.y) * vec3<f32>(x_447.x, x_447.y, x_447.z));
  let x_450 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_449.x, x_449.y, x_449.z, x_450.w);
  let x_453 : vec4<f32> = x_445.x_MainLightWorldToShadow[0i][0i];
  let x_455 : vec4<f32> = u_xlat0;
  let x_458 : vec4<f32> = u_xlat1;
  let x_460 : vec3<f32> = ((vec3<f32>(x_453.x, x_453.y, x_453.z) * vec3<f32>(x_455.x, x_455.x, x_455.x)) + vec3<f32>(x_458.x, x_458.y, x_458.z));
  let x_461 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_460.x, x_460.y, x_460.z, x_461.w);
  let x_464 : vec4<f32> = x_445.x_MainLightWorldToShadow[0i][2i];
  ...
  vs_INTERP8 = vec3<f32>(x_475.x, x_475.y, x_475.z);
  let x_478 : vec4<f32> = u_xlat1;
  let x_481 : vec4<f32> = x_445.x_MainLightWorldToShadow[0i][3i];
  let x_483 : vec3<f32> = (vec3<f32>(x_478.x, x_478.y, x_478.z) + vec3<f32>(x_481.x, x_481.y, x_481.z));
  let x_484 : vec4<f32> = vs_INTERP3;
  vs_INTERP3 = vec4<f32>(x_483.x, x_483.y, x_483.z, x_484.w);
  vs_INTERP3.w = 0.0f;
  let x_489 : vec4<f32> = in_TANGENT0;
  let x_492 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_494 : vec3<f32> = (vec3<f32>(x_489.y, x_489.y, x_489.y) * vec3<f32>(x_492.x, x_492.y, x_492.z));
  let x_495 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_494.x, x_494.y, x_494.z, x_495.w);
  let x_498 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_500 : vec4<f32> = in_TANGENT0;
  ...
  let x_509 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_511 : vec4<f32> = in_TANGENT0;
  let x_514 : vec4<f32> = u_xlat0;
  let x_516 : vec3<f32> = ((vec3<f32>(x_509.x, x_509.y, x_509.z) * vec3<f32>(x_511.z, x_511.z, x_511.z)) + vec3<f32>(x_514.x, x_514.y, x_514.z));
  ...
  u_xlat12 = dot(vec3<f32>(x_519.x, x_519.y, x_519.z), vec3<f32>(x_521.x, x_521.y, x_521.z));
  let x_524 : f32 = u_xlat12;
  u_xlat12 = max(x_524, 1.17549435e-38f);
  let x_526 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_526);
  ...
  let x_534 : vec4<f32> = vs_INTERP4;
  vs_INTERP4 = vec4<f32>(x_533.x, x_533.y, x_533.z, x_534.w);
  let x_537 : f32 = in_TANGENT0.w;
  vs_INTERP4.w = x_537;
  let x_541 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP5 = x_541;
  let x_543 : vec4<f32> = in_COLOR0;
  vs_INTERP6 = x_543;

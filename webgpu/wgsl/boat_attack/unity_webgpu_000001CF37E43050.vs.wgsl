...
fn main_1() {
  ...
  vs_INTERP8 = vec3<f32>(x_413.x, x_413.y, x_413.z);
  let x_419 : vec4<f32> = u_xlat1;
  let x_421 : vec4<f32> = x_394.unity_MatrixVP[3i];
  gl_Position = (x_419 + x_421);
  vs_INTERP2 = vec3<f32>(0.0f, 0.0f, 0.0f);
  let x_429 : vec4<f32> = in_TANGENT0;
  let x_432 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_434 : vec3<f32> = (vec3<f32>(x_429.y, x_429.y, x_429.y) * vec3<f32>(x_432.x, x_432.y, x_432.z));
  let x_435 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_434.x, x_434.y, x_434.z, x_435.w);
  let x_438 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_440 : vec4<f32> = in_TANGENT0;
  let x_443 : vec4<f32> = u_xlat0;
  let x_445 : vec3<f32> = ((vec3<f32>(x_438.x, x_438.y, x_438.z) * vec3<f32>(x_440.x, x_440.x, x_440.x)) + vec3<f32>(x_443.x, x_443.y, x_443.z));
  let x_446 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_445.x, x_445.y, x_445.z, x_446.w);
  let x_449 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_451 : vec4<f32> = in_TANGENT0;
  let x_454 : vec4<f32> = u_xlat0;
  let x_456 : vec3<f32> = ((vec3<f32>(x_449.x, x_449.y, x_449.z) * vec3<f32>(x_451.z, x_451.z, x_451.z)) + vec3<f32>(x_454.x, x_454.y, x_454.z));
  ...
  u_xlat12 = dot(vec3<f32>(x_459.x, x_459.y, x_459.z), vec3<f32>(x_461.x, x_461.y, x_461.z));
  let x_464 : f32 = u_xlat12;
  u_xlat12 = max(x_464, 1.17549435e-38f);
  let x_466 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_466);
  ...
  let x_474 : vec4<f32> = vs_INTERP4;
  vs_INTERP4 = vec4<f32>(x_473.x, x_473.y, x_473.z, x_474.w);
  let x_477 : f32 = in_TANGENT0.w;
  vs_INTERP4.w = x_477;
  let x_482 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP5 = x_482;
  let x_484 : vec4<f32> = in_COLOR0;
  vs_INTERP6 = x_484;

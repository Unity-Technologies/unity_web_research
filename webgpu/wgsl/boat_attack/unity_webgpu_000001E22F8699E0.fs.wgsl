...
fn main_1() {
  ...
  let x_671 : vec4<f32> = x_40.x_Dithering_Params;
  let x_675 : vec4<f32> = x_40.x_Dithering_Params;
  ...
  let x_689 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_685.x, x_685.y), x_688);
  u_xlat15 = x_689.w;
  let x_691 : f32 = u_xlat15;
  u_xlat15 = ((x_691 * 2.0f) + -1.0f);
  let x_694 : f32 = u_xlat15;
  u_xlatb1.x = (x_694 >= 0.0f);
  let x_698 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_698);
  let x_701 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_701)) + 1.0f);
  ...
  u_xlat15 = (x_710 * x_712);
  let x_714 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_714.x, x_714.y, x_714.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  u_xlat1 = (vec3<f32>(x_790.x, x_790.y, x_790.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_793 : vec4<f32> = u_xlat0;
  let x_795 : vec3<f32> = (vec3<f32>(x_793.x, x_793.y, x_793.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_796 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_795.x, x_795.y, x_795.z, x_796.w);
  let x_798 : vec4<f32> = u_xlat2;
  let x_800 : vec3<f32> = (vec3<f32>(x_798.x, x_798.y, x_798.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_801 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_800.x, x_800.y, x_800.z, x_801.w);
  let x_803 : vec4<f32> = u_xlat2;
  let x_806 : vec3<f32> = log2(abs(vec3<f32>(x_803.x, x_803.y, x_803.z)));
  let x_807 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_806.x, x_806.y, x_806.z, x_807.w);
  let x_809 : vec4<f32> = u_xlat2;
  let x_811 : vec3<f32> = (vec3<f32>(x_809.x, x_809.y, x_809.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));

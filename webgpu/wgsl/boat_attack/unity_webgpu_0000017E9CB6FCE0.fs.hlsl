...
#line 6 "D:/src/unity_webgpu/Shaders/Includes/TerrainTool.cginc"
...
{
float targetHeight = ( _BrushParams [ 1 ] ) ;
...
float4 StampHeight ( v2f i ) : COLOR
{
...
float brushHeight = brushShape * ( _BrushParams [ 2 ] ) ;

float targetHeight ;
if ( ( _BrushParams [ 3 ] ) > 0.0f )
{
float brushIntersection = saturate ( 1.0f - ( _BrushParams [ 3 ] ) ) ;
...
height = lerp ( height , targetHeight , ( _BrushParams [ 0 ] ) ) ;

return PackHeightmap ( clamp ( height , 0.0f , ( 32766.0f / 65535.0f ) ) ) ;
}

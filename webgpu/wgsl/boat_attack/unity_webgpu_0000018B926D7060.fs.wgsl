...
fn main_1() {
  ...
  let x_572 : vec4<f32> = x_13.x_Dithering_Params;
  let x_576 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_590 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_586.x, x_586.y), x_589);
  u_xlat15 = x_590.w;
  let x_592 : f32 = u_xlat15;
  u_xlat15 = ((x_592 * 2.0f) + -1.0f);
  let x_597 : f32 = u_xlat15;
  u_xlatb1.x = (x_597 >= 0.0f);
  let x_601 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_601);
  let x_604 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_604)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_617.x, x_617.y, x_617.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_620 : vec4<f32> = u_xlat0;
  let x_623 : vec3<f32> = log2(abs(vec3<f32>(x_620.x, x_620.y, x_620.z)));
  let x_624 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_623.x, x_623.y, x_623.z, x_624.w);
  let x_626 : vec4<f32> = u_xlat2;
  let x_628 : vec3<f32> = (vec3<f32>(x_626.x, x_626.y, x_626.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_639 : vec3<f32> = ((vec3<f32>(x_636.x, x_636.y, x_636.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_640 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_639.x, x_639.y, x_639.z, x_640.w);
  let x_643 : vec4<f32> = u_xlat0;
  let x_645 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_643.x, x_643.y, x_643.z, x_643.x));
  ...
  let x_690 : vec3<f32> = ((vec3<f32>(x_683, x_683, x_683) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_688.x, x_688.y, x_688.z));
  let x_691 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_690.x, x_690.y, x_690.z, x_691.w);
  let x_693 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_693.x, x_693.y, x_693.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_696 : vec4<f32> = u_xlat0;
  let x_698 : vec3<f32> = (vec3<f32>(x_696.x, x_696.y, x_696.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_699 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_698.x, x_698.y, x_698.z, x_699.w);
  let x_701 : vec4<f32> = u_xlat2;
  let x_703 : vec3<f32> = (vec3<f32>(x_701.x, x_701.y, x_701.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_714 : vec3<f32> = (vec3<f32>(x_712.x, x_712.y, x_712.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_724 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_722.x, x_722.y, x_722.z, x_722.x));
  ...
  if (x_751) {
    let x_756 : f32 = u_xlat1.z;
    x_752 = x_756;
  } else {
    let x_759 : f32 = u_xlat2.z;
    x_752 = x_759;
  }
  let x_760 : f32 = x_752;
  u_xlat0.z = x_760;
  let x_764 : vec4<f32> = u_xlat0;
  let x_767 : vec3<f32> = max(vec3<f32>(x_764.x, x_764.y, x_764.z), vec3<f32>(0.0f, 0.0f, 0.0f));

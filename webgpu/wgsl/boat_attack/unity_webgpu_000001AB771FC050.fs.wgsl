...
fn main_1() {
  ...
  let x_631 : vec4<f32> = x_40.x_Grain_TilingParams;
  let x_635 : vec4<f32> = x_40.x_Grain_TilingParams;
  ...
  let x_649 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_645.x, x_645.y), x_648);
  u_xlat15 = x_649.w;
  let x_651 : f32 = u_xlat15;
  u_xlat15 = (x_651 + -0.5f);
  let x_654 : f32 = u_xlat15;
  let x_655 : f32 = u_xlat15;
  u_xlat15 = (x_654 + x_655);
  let x_657 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_657.x, x_657.y, x_657.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_666 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_666);
  let x_671 : f32 = x_40.x_Grain_Params.y;
  let x_673 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_671 * -(x_673)) + 1.0f);
  ...
  let x_687 : vec2<f32> = x_40.x_Grain_Params;
  u_xlat6 = (x_684 * vec3<f32>(x_687.x, x_687.x, x_687.x));
  let x_692 : vec3<f32> = u_xlat6;
  let x_693 : vec3<f32> = u_xlat1;
  let x_696 : vec4<f32> = u_xlat0;
  let x_698 : vec3<f32> = ((x_692 * vec3<f32>(x_693.x, x_693.x, x_693.x)) + vec3<f32>(x_696.x, x_696.y, x_696.z));

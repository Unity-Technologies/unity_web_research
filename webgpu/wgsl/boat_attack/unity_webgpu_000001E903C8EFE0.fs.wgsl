...
fn main_1() {
  ...
  let x_865 : vec3<f32> = sqrt(vec3<f32>(x_863.x, x_863.y, x_863.z));
  let x_866 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_865.x, x_865.y, x_865.z, x_866.w);
  let x_868 : vec2<f32> = vs_TEXCOORD0;
  let x_871 : vec4<f32> = x_25.x_Dithering_Params;
  let x_875 : vec4<f32> = x_25.x_Dithering_Params;
  let x_877 : vec2<f32> = ((x_868 * vec2<f32>(x_871.x, x_871.y)) + vec2<f32>(x_875.z, x_875.w));
  let x_878 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_877.x, x_877.y, x_878.z, x_878.w);
  let x_885 : vec4<f32> = u_xlat1;
  let x_888 : f32 = x_25.x_GlobalMipBias.x;
  let x_889 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_885.x, x_885.y), x_888);
  u_xlat15 = x_889.w;
  let x_891 : f32 = u_xlat15;
  u_xlat15 = ((x_891 * 2.0f) + -1.0f);
  let x_895 : f32 = u_xlat15;
  u_xlatb1.x = (x_895 >= 0.0f);
  let x_899 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_899);
  let x_902 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_902)) + 1.0f);
  ...
  u_xlat15 = (x_911 * x_913);
  let x_915 : vec4<f32> = u_xlat0;
  let x_917 : vec3<f32> = (vec3<f32>(x_915.x, x_915.y, x_915.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_927 : vec3<f32> = (vec3<f32>(x_925.x, x_925.y, x_925.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_928 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_927.x, x_927.y, x_927.z, x_928.w);
  let x_930 : vec4<f32> = u_xlat2;
  let x_932 : vec3<f32> = exp2(vec3<f32>(x_930.x, x_930.y, x_930.z));
  let x_933 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_932.x, x_932.y, x_932.z, x_933.w);
  let x_935 : vec4<f32> = u_xlat2;
  let x_938 : vec3<f32> = ((vec3<f32>(x_935.x, x_935.y, x_935.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_939 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_938.x, x_938.y, x_938.z, x_939.w);
  let x_942 : vec4<f32> = u_xlat0;
  let x_944 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_942.x, x_942.y, x_942.z, x_942.x));
  ...
  let x_989 : vec3<f32> = ((vec3<f32>(x_982, x_982, x_982) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_987.x, x_987.y, x_987.z));
  let x_990 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_989.x, x_989.y, x_989.z, x_990.w);
  let x_992 : vec4<f32> = u_xlat0;
  let x_994 : vec3<f32> = (vec3<f32>(x_992.x, x_992.y, x_992.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_995 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_994.x, x_994.y, x_994.z, x_995.w);
  let x_997 : vec4<f32> = u_xlat0;
  let x_999 : vec3<f32> = (vec3<f32>(x_997.x, x_997.y, x_997.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  ...
  let x_1068 : vec3<f32> = max(vec3<f32>(x_1065.x, x_1065.y, x_1065.z), vec3<f32>(0.0f, 0.0f, 0.0f));

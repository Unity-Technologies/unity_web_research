...
@group(1) @binding(5) var<uniform> x_158 : VGlobals;
...
fn main_1() {
  ...
  let x_144 : vec2<f32> = in_TEXCOORD1;
  let x_147 : vec4<f32> = x_44.unity_LightmapST;
  let x_151 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_144 * vec2<f32>(x_147.x, x_147.y)) + vec2<f32>(x_151.z, x_151.w));
  let x_154 : vec4<f32> = u_xlat0;
  let x_160 : vec4<f32> = x_158.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_154.y, x_154.y, x_154.y, x_154.y) * x_160);
  let x_163 : vec4<f32> = x_158.unity_MatrixVP[0i];
  let x_164 : vec4<f32> = u_xlat0;
  let x_167 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_163 * vec4<f32>(x_164.x, x_164.x, x_164.x, x_164.x)) + x_167);
  let x_170 : vec4<f32> = x_158.unity_MatrixVP[2i];
  let x_171 : vec4<f32> = u_xlat0;
  let x_174 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_170 * vec4<f32>(x_171.z, x_171.z, x_171.z, x_171.z)) + x_174);
  let x_180 : vec4<f32> = u_xlat0;
  let x_182 : vec4<f32> = x_158.unity_MatrixVP[3i];
  gl_Position = (x_180 + x_182);
  return;
}
...
fn main(@location(2) in_TEXCOORD0_param : vec2<f32>, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(3) in_TEXCOORD1_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_TEXCOORD7, gl_Position);
}

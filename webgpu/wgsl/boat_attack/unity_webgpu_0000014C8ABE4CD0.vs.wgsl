...
@group(1) @binding(5) var<uniform> x_209 : VGlobals;
...
fn main_1() {
  ...
  vs_TEXCOORD6.w = 0.0f;
  let x_195 : vec2<f32> = in_TEXCOORD1;
  let x_198 : vec4<f32> = x_44.unity_LightmapST;
  let x_202 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_195 * vec2<f32>(x_198.x, x_198.y)) + vec2<f32>(x_202.z, x_202.w));
  let x_205 : vec4<f32> = u_xlat0;
  let x_211 : vec4<f32> = x_209.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_205.y, x_205.y, x_205.y, x_205.y) * x_211);
  let x_214 : vec4<f32> = x_209.unity_MatrixVP[0i];
  let x_215 : vec4<f32> = u_xlat0;
  let x_218 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_214 * vec4<f32>(x_215.x, x_215.x, x_215.x, x_215.x)) + x_218);
  let x_221 : vec4<f32> = x_209.unity_MatrixVP[2i];
  let x_222 : vec4<f32> = u_xlat0;
  let x_225 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_221 * vec4<f32>(x_222.z, x_222.z, x_222.z, x_222.z)) + x_225);
  let x_231 : vec4<f32> = u_xlat0;
  let x_233 : vec4<f32> = x_209.unity_MatrixVP[3i];
  gl_Position = (x_231 + x_233);
  return;
}

...
@group(1) @binding(4) var<uniform> x_384 : LightShadows;
...
fn main_1() {
  ...
  let x_386 : vec4<f32> = x_384.x_MainLightWorldToShadow[0i][1i];
  let x_388 : vec3<f32> = (vec3<f32>(x_375.y, x_375.y, x_375.y) * vec3<f32>(x_386.x, x_386.y, x_386.z));
  let x_389 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_388.x, x_388.y, x_388.z, x_389.w);
  let x_392 : vec4<f32> = x_384.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_403 : vec4<f32> = x_384.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_417 : vec4<f32> = x_384.x_MainLightWorldToShadow[0i][3i];
  let x_419 : vec3<f32> = (vec3<f32>(x_414.x, x_414.y, x_414.z) + vec3<f32>(x_417.x, x_417.y, x_417.z));
  let x_420 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_419.x, x_419.y, x_419.z, x_420.w);
  vs_TEXCOORD6.w = 0.0f;
  let x_424 : vec4<f32> = x_40.x_WavingTint;
  let x_428 : vec3<f32> = (vec3<f32>(x_424.x, x_424.y, x_424.z) + vec3<f32>(-0.5f, -0.5f, -0.5f));
  let x_429 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_428.x, x_428.y, x_428.z, x_429.w);
  let x_431 : vec4<f32> = u_xlat0;
  let x_433 : vec4<f32> = u_xlat1;
  let x_438 : vec3<f32> = ((vec3<f32>(x_431.x, x_431.x, x_431.x) * vec3<f32>(x_433.x, x_433.y, x_433.z)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  ...
  let x_454 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_453.x, x_453.y, x_453.z, x_454.w);
  let x_456 : vec3<f32> = u_xlat5;
  let x_459 : vec4<f32> = x_351.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_456.y, x_456.y, x_456.y, x_456.y) * x_459);
  let x_462 : vec4<f32> = x_351.unity_MatrixVP[0i];
  let x_463 : vec3<f32> = u_xlat5;
  let x_466 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_462 * vec4<f32>(x_463.x, x_463.x, x_463.x, x_463.x)) + x_466);
  let x_469 : vec4<f32> = x_351.unity_MatrixVP[2i];
  let x_470 : vec3<f32> = u_xlat5;
  let x_473 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_469 * vec4<f32>(x_470.z, x_470.z, x_470.z, x_470.z)) + x_473);
  let x_479 : vec4<f32> = u_xlat0;
  let x_481 : vec4<f32> = x_351.unity_MatrixVP[3i];
  gl_Position = (x_479 + x_481);
  return;
}

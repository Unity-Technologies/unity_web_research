...
fn main_1() {
  ...
  let x_639 : vec3<f32> = (vec3<f32>(x_637.x, x_637.y, x_637.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_650 : vec3<f32> = (vec3<f32>(x_648.x, x_648.y, x_648.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_661 : vec3<f32> = ((vec3<f32>(x_658.x, x_658.y, x_658.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_662 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_661.x, x_661.y, x_661.z, x_662.w);
  let x_665 : vec4<f32> = u_xlat0;
  let x_667 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_665.x, x_665.y, x_665.z, x_665.x));
  u_xlatb0 = vec3<bool>(x_667.x, x_667.y, x_667.z);
  ...
  let x_691 : f32 = x_683;
  u_xlat0.y = x_691;
  let x_694 : bool = u_xlatb0.z;
  if (x_694) {
    let x_699 : f32 = u_xlat1.z;
    x_695 = x_699;
  } else {
  ...
  let x_703 : f32 = x_695;
  u_xlat0.z = x_703;
  let x_705 : vec2<f32> = vs_TEXCOORD0;
  let x_708 : vec4<f32> = x_13.x_Dithering_Params;
  let x_712 : vec4<f32> = x_13.x_Dithering_Params;
  let x_714 : vec2<f32> = ((x_705 * vec2<f32>(x_708.x, x_708.y)) + vec2<f32>(x_712.z, x_712.w));
  let x_715 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_714.x, x_714.y, x_715.z, x_715.w);
  let x_722 : vec4<f32> = u_xlat1;
  let x_725 : f32 = x_13.x_GlobalMipBias.x;
  let x_726 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_722.x, x_722.y), x_725);
  u_xlat15 = x_726.w;
  let x_728 : f32 = u_xlat15;
  u_xlat15 = ((x_728 * 2.0f) + -1.0f);
  let x_733 : f32 = u_xlat15;
  u_xlatb1.x = (x_733 >= 0.0f);
  let x_737 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_737);
  let x_740 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_740)) + 1.0f);
  ...
  let x_755 : vec3<f32> = (vec3<f32>(x_753.x, x_753.y, x_753.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_756 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_755.x, x_755.y, x_755.z, x_756.w);
  let x_758 : vec4<f32> = u_xlat0;
  let x_761 : vec3<f32> = log2(abs(vec3<f32>(x_758.x, x_758.y, x_758.z)));
  let x_762 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_761.x, x_761.y, x_761.z, x_762.w);
  let x_764 : vec4<f32> = u_xlat2;
  let x_766 : vec3<f32> = (vec3<f32>(x_764.x, x_764.y, x_764.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_767 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_766.x, x_766.y, x_766.z, x_767.w);
  let x_769 : vec4<f32> = u_xlat2;
  let x_771 : vec3<f32> = exp2(vec3<f32>(x_769.x, x_769.y, x_769.z));
  let x_772 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_771.x, x_771.y, x_771.z, x_772.w);
  let x_774 : vec4<f32> = u_xlat2;
  let x_777 : vec3<f32> = ((vec3<f32>(x_774.x, x_774.y, x_774.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_778 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_777.x, x_777.y, x_777.z, x_778.w);
  let x_780 : vec4<f32> = u_xlat0;
  let x_782 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_780.x, x_780.y, x_780.z, x_780.x));
  u_xlatb0 = vec3<bool>(x_782.x, x_782.y, x_782.z);
  let x_785 : bool = u_xlatb0.x;
  if (x_785) {
  ...
  let x_827 : vec3<f32> = ((vec3<f32>(x_820, x_820, x_820) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_825.x, x_825.y, x_825.z));
  let x_828 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_827.x, x_827.y, x_827.z, x_828.w);
  let x_830 : vec4<f32> = u_xlat0;
  let x_832 : vec3<f32> = (vec3<f32>(x_830.x, x_830.y, x_830.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_833 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_832.x, x_832.y, x_832.z, x_833.w);
  let x_835 : vec4<f32> = u_xlat0;
  let x_837 : vec3<f32> = (vec3<f32>(x_835.x, x_835.y, x_835.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_838 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_837.x, x_837.y, x_837.z, x_838.w);
  let x_840 : vec4<f32> = u_xlat2;
  let x_842 : vec3<f32> = (vec3<f32>(x_840.x, x_840.y, x_840.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_853 : vec3<f32> = (vec3<f32>(x_851.x, x_851.y, x_851.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_863 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_861.x, x_861.y, x_861.z, x_861.x));
  ...
  let x_906 : vec3<f32> = max(vec3<f32>(x_903.x, x_903.y, x_903.z), vec3<f32>(0.0f, 0.0f, 0.0f));

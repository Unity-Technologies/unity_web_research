...
fn main_1() {
  ...
  let x_339 : vec4<f32> = x_189.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_336.y, x_336.y, x_336.y, x_336.y) * x_339);
  let x_342 : vec4<f32> = x_189.unity_MatrixVP[0i];
  let x_343 : vec3<f32> = u_xlat1;
  let x_346 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_342 * vec4<f32>(x_343.x, x_343.x, x_343.x, x_343.x)) + x_346);
  let x_349 : vec4<f32> = x_189.unity_MatrixVP[2i];
  let x_350 : vec3<f32> = u_xlat1;
  let x_353 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_349 * vec4<f32>(x_350.z, x_350.z, x_350.z, x_350.z)) + x_353);
  let x_359 : vec4<f32> = u_xlat0;
  let x_361 : vec4<f32> = x_189.unity_MatrixVP[3i];
  gl_Position = (x_359 + x_361);
  return;
}

...
@group(0) @binding(21) var x_TerrainNormalmapTexture : texture_2d<f32>;

@group(0) @binding(20) var x_TerrainHeightmapTexture : texture_2d<f32>;
...
@group(1) @binding(4) var<uniform> x_406 : MainLightShadows;

var<private> vs_TEXCOORD8 : vec4<f32>;

@group(1) @binding(7) var<uniform> x_450 : VGlobals;
...
fn main_1() {
  ...
  let x_408 : vec4<f32> = x_406.x_MainLightWorldToShadow[0i][1i];
  let x_410 : vec3<f32> = (vec3<f32>(x_400.y, x_400.y, x_400.y) * vec3<f32>(x_408.x, x_408.y, x_408.z));
  let x_411 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_410.x, x_410.y, x_410.z, x_411.w);
  let x_414 : vec4<f32> = x_406.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_425 : vec4<f32> = x_406.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_439 : vec4<f32> = x_406.x_MainLightWorldToShadow[0i][3i];
  let x_441 : vec3<f32> = (vec3<f32>(x_436.x, x_436.y, x_436.z) + vec3<f32>(x_439.x, x_439.y, x_439.z));
  let x_442 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_441.x, x_441.y, x_441.z, x_442.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_446 : vec4<f32> = u_xlat0;
  let x_452 : vec4<f32> = x_450.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_446.y, x_446.y, x_446.y, x_446.y) * x_452);
  let x_455 : vec4<f32> = x_450.unity_MatrixVP[0i];
  let x_456 : vec4<f32> = u_xlat0;
  let x_459 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_455 * vec4<f32>(x_456.x, x_456.x, x_456.x, x_456.x)) + x_459);
  let x_462 : vec4<f32> = x_450.unity_MatrixVP[2i];
  let x_463 : vec4<f32> = u_xlat0;
  let x_466 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_462 * vec4<f32>(x_463.z, x_463.z, x_463.z, x_463.z)) + x_466);
  let x_472 : vec4<f32> = u_xlat0;
  let x_474 : vec4<f32> = x_450.unity_MatrixVP[3i];
  gl_Position = (x_472 + x_474);
  return;
}

...
layout(set = 0, binding = 8) uniform mediump  sampler sampler_EmissionMap;
...
vec4 hlslcc_FragCoord = vec4(gl_FragCoord.xyz, 1.0/gl_FragCoord.w);
    ...
    u_xlat36 = texture(sampler2D(_DitheringTexture, sampler_DitheringTexture), u_xlat3.xy, _GlobalMipBias.x).w;
    u_xlatb38 = unity_LODFade.x>=0.0;
    u_xlat36 = (u_xlatb38) ? abs(u_xlat36) : -abs(u_xlat36);

...
fn main_1() {
  ...
  let x_948 : vec4<f32> = x_25.x_Dithering_Params;
  let x_952 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_965 : f32 = x_25.x_GlobalMipBias.x;
  let x_966 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_962.x, x_962.y), x_965);
  u_xlat15 = x_966.w;
  let x_968 : f32 = u_xlat15;
  u_xlat15 = ((x_968 * 2.0f) + -1.0f);
  let x_972 : f32 = u_xlat15;
  u_xlatb1.x = (x_972 >= 0.0f);
  let x_976 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_976);
  let x_979 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_979)) + 1.0f);
  ...
  u_xlat15 = (x_988 * x_990);
  let x_992 : vec4<f32> = u_xlat0;
  let x_994 : vec3<f32> = (vec3<f32>(x_992.x, x_992.y, x_992.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_995 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_994.x, x_994.y, x_994.z, x_995.w);
  let x_997 : vec4<f32> = u_xlat0;
  let x_1000 : vec3<f32> = log2(abs(vec3<f32>(x_997.x, x_997.y, x_997.z)));
  let x_1001 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1000.x, x_1000.y, x_1000.z, x_1001.w);
  let x_1003 : vec4<f32> = u_xlat2;
  let x_1005 : vec3<f32> = (vec3<f32>(x_1003.x, x_1003.y, x_1003.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_1016 : vec3<f32> = ((vec3<f32>(x_1013.x, x_1013.y, x_1013.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_1017 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1016.x, x_1016.y, x_1016.z, x_1017.w);
  let x_1020 : vec4<f32> = u_xlat0;
  let x_1022 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_1020.x, x_1020.y, x_1020.z, x_1020.x));
  u_xlatb0 = vec3<bool>(x_1022.x, x_1022.y, x_1022.z);
  let x_1025 : bool = u_xlatb0.x;
  if (x_1025) {
  ...
  let x_1067 : vec3<f32> = ((vec3<f32>(x_1060, x_1060, x_1060) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1065.x, x_1065.y, x_1065.z));
  let x_1068 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1067.x, x_1067.y, x_1067.z, x_1068.w);
  let x_1070 : vec4<f32> = u_xlat0;
  let x_1072 : vec3<f32> = (vec3<f32>(x_1070.x, x_1070.y, x_1070.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1073 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1072.x, x_1072.y, x_1072.z, x_1073.w);
  let x_1075 : vec4<f32> = u_xlat0;
  let x_1077 : vec3<f32> = (vec3<f32>(x_1075.x, x_1075.y, x_1075.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1078 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1077.x, x_1077.y, x_1077.z, x_1078.w);
  let x_1080 : vec4<f32> = u_xlat2;
  let x_1082 : vec3<f32> = (vec3<f32>(x_1080.x, x_1080.y, x_1080.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1083 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1082.x, x_1082.y, x_1082.z, x_1083.w);
  let x_1085 : vec4<f32> = u_xlat2;
  let x_1088 : vec3<f32> = log2(abs(vec3<f32>(x_1085.x, x_1085.y, x_1085.z)));
  let x_1089 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1088.x, x_1088.y, x_1088.z, x_1089.w);
  let x_1091 : vec4<f32> = u_xlat2;
  let x_1093 : vec3<f32> = (vec3<f32>(x_1091.x, x_1091.y, x_1091.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_1094 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1093.x, x_1093.y, x_1093.z, x_1094.w);
  let x_1096 : vec4<f32> = u_xlat2;
  let x_1098 : vec3<f32> = exp2(vec3<f32>(x_1096.x, x_1096.y, x_1096.z));
  let x_1099 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1098.x, x_1098.y, x_1098.z, x_1099.w);
  let x_1101 : vec4<f32> = u_xlat0;
  let x_1103 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1101.x, x_1101.y, x_1101.z, x_1101.x));
  ...
  let x_1146 : vec3<f32> = max(vec3<f32>(x_1143.x, x_1143.y, x_1143.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1147 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1146.x, x_1146.y, x_1146.z, x_1147.w);
  SV_Target0.w = 1.0f;
  return;
}

...
@group(1) @binding(7) var<uniform> x_207 : VGlobals;
...
fn main_1() {
  ...
  vs_TEXCOORD6.w = 0.0f;
  let x_193 : vec2<f32> = in_TEXCOORD1;
  let x_196 : vec4<f32> = x_44.unity_LightmapST;
  let x_200 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_193 * vec2<f32>(x_196.x, x_196.y)) + vec2<f32>(x_200.z, x_200.w));
  let x_203 : vec4<f32> = u_xlat0;
  let x_209 : vec4<f32> = x_207.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_203.y, x_203.y, x_203.y, x_203.y) * x_209);
  let x_212 : vec4<f32> = x_207.unity_MatrixVP[0i];
  let x_213 : vec4<f32> = u_xlat0;
  let x_216 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_212 * vec4<f32>(x_213.x, x_213.x, x_213.x, x_213.x)) + x_216);
  let x_219 : vec4<f32> = x_207.unity_MatrixVP[2i];
  let x_220 : vec4<f32> = u_xlat0;
  let x_223 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_219 * vec4<f32>(x_220.z, x_220.z, x_220.z, x_220.z)) + x_223);
  let x_229 : vec4<f32> = u_xlat0;
  let x_231 : vec4<f32> = x_207.unity_MatrixVP[3i];
  gl_Position = (x_229 + x_231);
  return;
}
...
fn main(@location(2) in_TEXCOORD0_param : vec2<f32>, @location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(3) in_TEXCOORD1_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_TEXCOORD6, vs_TEXCOORD7, gl_Position);
}

...
@group(1) @binding(0) var<uniform> x_182 : VGlobals;
...
fn main_1() {
  ...
  let x_94 : vec3<f32> = in_NORMAL0;
  let x_98 : vec4<f32> = x_20.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_94, vec3<f32>(x_98.x, x_98.y, x_98.z));
  let x_104 : vec3<f32> = in_NORMAL0;
  let x_106 : vec4<f32> = x_20.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_104, vec3<f32>(x_106.x, x_106.y, x_106.z));
  let x_111 : vec3<f32> = in_NORMAL0;
  let x_114 : vec4<f32> = x_20.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_111, vec3<f32>(x_114.x, x_114.y, x_114.z));
  ...
  u_xlat6 = max(x_124, 1.17549435e-38f);
  let x_127 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_127);
  let x_131 : f32 = u_xlat6;
  let x_133 : vec4<f32> = u_xlat0;
  vs_TEXCOORD3 = (vec3<f32>(x_131, x_131, x_131) * vec3<f32>(x_133.x, x_133.y, x_133.z));
  let x_138 : vec4<f32> = in_POSITION0;
  let x_141 : vec4<f32> = x_20.unity_ObjectToWorld[1i];
  ...
  let x_147 : vec4<f32> = x_20.unity_ObjectToWorld[0i];
  ...
  let x_158 : vec4<f32> = x_20.unity_ObjectToWorld[2i];
  ...
  let x_172 : vec4<f32> = x_20.unity_ObjectToWorld[3i];
  let x_174 : vec3<f32> = (vec3<f32>(x_168.x, x_168.y, x_168.z) + vec3<f32>(x_172.x, x_172.y, x_172.z));
  let x_175 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_174.x, x_174.y, x_174.z, x_175.w);
  let x_178 : vec4<f32> = u_xlat0;
  let x_184 : vec4<f32> = x_182.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_178.y, x_178.y, x_178.y, x_178.y) * x_184);
  let x_187 : vec4<f32> = x_182.unity_MatrixVP[0i];
  let x_188 : vec4<f32> = u_xlat0;
  let x_191 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_187 * vec4<f32>(x_188.x, x_188.x, x_188.x, x_188.x)) + x_191);
  let x_194 : vec4<f32> = x_182.unity_MatrixVP[2i];
  let x_195 : vec4<f32> = u_xlat0;
  let x_198 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_194 * vec4<f32>(x_195.z, x_195.z, x_195.z, x_195.z)) + x_198);
  let x_204 : vec4<f32> = u_xlat0;
  let x_206 : vec4<f32> = x_182.unity_MatrixVP[3i];
  gl_Position = (x_204 + x_206);
  return;
}

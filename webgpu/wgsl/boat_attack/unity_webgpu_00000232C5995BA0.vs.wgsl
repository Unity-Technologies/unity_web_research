...
@group(1) @binding(5) var<uniform> x_260 : VGlobals;
...
fn main_1() {
  ...
  vs_TEXCOORD6.w = 0.0f;
  let x_246 : vec2<f32> = in_TEXCOORD1;
  let x_249 : vec4<f32> = x_44.unity_LightmapST;
  let x_253 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD8 = ((x_246 * vec2<f32>(x_249.x, x_249.y)) + vec2<f32>(x_253.z, x_253.w));
  let x_256 : vec4<f32> = u_xlat0;
  let x_262 : vec4<f32> = x_260.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_256.y, x_256.y, x_256.y, x_256.y) * x_262);
  let x_265 : vec4<f32> = x_260.unity_MatrixVP[0i];
  let x_266 : vec4<f32> = u_xlat0;
  let x_269 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_265 * vec4<f32>(x_266.x, x_266.x, x_266.x, x_266.x)) + x_269);
  let x_272 : vec4<f32> = x_260.unity_MatrixVP[2i];
  let x_273 : vec4<f32> = u_xlat0;
  let x_276 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_272 * vec4<f32>(x_273.z, x_273.z, x_273.z, x_273.z)) + x_276);
  let x_282 : vec4<f32> = u_xlat0;
  let x_284 : vec4<f32> = x_260.unity_MatrixVP[3i];
  gl_Position = (x_282 + x_284);
  return;
}

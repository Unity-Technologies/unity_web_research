...
fn main_1() {
  ...
  let x_731 : vec3<f32> = sqrt(vec3<f32>(x_729.x, x_729.y, x_729.z));
  let x_732 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_731.x, x_731.y, x_731.z, x_732.w);
  let x_734 : vec2<f32> = vs_TEXCOORD0;
  let x_737 : vec4<f32> = x_13.x_Dithering_Params;
  let x_741 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_754 : f32 = x_13.x_GlobalMipBias.x;
  let x_755 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_751.x, x_751.y), x_754);
  u_xlat15 = x_755.w;
  let x_757 : f32 = u_xlat15;
  u_xlat15 = ((x_757 * 2.0f) + -1.0f);
  let x_762 : f32 = u_xlat15;
  u_xlatb1.x = (x_762 >= 0.0f);
  let x_766 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_766);
  let x_769 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_769)) + 1.0f);
  let x_773 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_773);
  let x_775 : f32 = u_xlat15;
  u_xlat15 = (-(x_775) + 1.0f);
  let x_778 : f32 = u_xlat15;
  let x_780 : f32 = u_xlat1.x;
  u_xlat15 = (x_778 * x_780);
  let x_782 : vec4<f32> = u_xlat0;
  let x_784 : vec3<f32> = (vec3<f32>(x_782.x, x_782.y, x_782.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_785 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_784.x, x_784.y, x_784.z, x_785.w);
  let x_787 : vec4<f32> = u_xlat0;
  let x_789 : vec3<f32> = log2(vec3<f32>(x_787.x, x_787.y, x_787.z));
  ...
  let x_811 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_809.x, x_809.y, x_809.z, x_809.x));
  u_xlatb0 = vec3<bool>(x_811.x, x_811.y, x_811.z);
  let x_814 : bool = u_xlatb0.x;
  if (x_814) {
  ...
  let x_847 : f32 = x_839;
  u_xlat0.z = x_847;
  let x_849 : f32 = u_xlat15;
  let x_854 : vec4<f32> = u_xlat0;
  let x_856 : vec3<f32> = ((vec3<f32>(x_849, x_849, x_849) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_854.x, x_854.y, x_854.z));
  let x_857 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_856.x, x_856.y, x_856.z, x_857.w);
  let x_859 : vec4<f32> = u_xlat0;
  let x_861 : vec3<f32> = (vec3<f32>(x_859.x, x_859.y, x_859.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_862 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_861.x, x_861.y, x_861.z, x_862.w);
  let x_864 : vec4<f32> = u_xlat0;
  let x_866 : vec3<f32> = (vec3<f32>(x_864.x, x_864.y, x_864.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_867 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_866.x, x_866.y, x_866.z, x_867.w);
  let x_869 : vec4<f32> = u_xlat2;
  let x_871 : vec3<f32> = (vec3<f32>(x_869.x, x_869.y, x_869.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_872 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_871.x, x_871.y, x_871.z, x_872.w);
  let x_874 : vec4<f32> = u_xlat2;
  let x_877 : vec3<f32> = log2(abs(vec3<f32>(x_874.x, x_874.y, x_874.z)));
  let x_878 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_877.x, x_877.y, x_877.z, x_878.w);
  let x_880 : vec4<f32> = u_xlat2;
  let x_882 : vec3<f32> = (vec3<f32>(x_880.x, x_880.y, x_880.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_892 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_890.x, x_890.y, x_890.z, x_890.x));
  u_xlatb0 = vec3<bool>(x_892.x, x_892.y, x_892.z);
  ...
  let x_916 : f32 = x_908;
  u_xlat0.y = x_916;
  let x_919 : bool = u_xlatb0.z;
  if (x_919) {
    let x_924 : f32 = u_xlat1.z;
    x_920 = x_924;
  } else {
  ...
  let x_935 : vec3<f32> = max(vec3<f32>(x_932.x, x_932.y, x_932.z), vec3<f32>(0.0f, 0.0f, 0.0f));

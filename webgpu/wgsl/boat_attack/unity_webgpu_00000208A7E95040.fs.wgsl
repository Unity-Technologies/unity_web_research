...
fn main_1() {
  ...
  let x_377 : vec4<f32> = x_14.unity_WorldToLight[1i];
  u_xlat6 = (vec3<f32>(x_373.y, x_373.y, x_373.y) * vec3<f32>(x_377.x, x_377.y, x_377.z));
  let x_381 : vec4<f32> = x_14.unity_WorldToLight[0i];
  let x_383 : vec4<f32> = u_xlat2;
  let x_386 : vec3<f32> = u_xlat6;
  u_xlat6 = ((vec3<f32>(x_381.x, x_381.y, x_381.z) * vec3<f32>(x_383.x, x_383.x, x_383.x)) + x_386);
  let x_389 : vec4<f32> = x_14.unity_WorldToLight[2i];
  let x_391 : vec4<f32> = u_xlat2;
  let x_394 : vec3<f32> = u_xlat6;
  u_xlat6 = ((vec3<f32>(x_389.x, x_389.y, x_389.z) * vec3<f32>(x_391.w, x_391.w, x_391.w)) + x_394);
  let x_396 : vec3<f32> = u_xlat6;
  let x_398 : vec4<f32> = x_14.unity_WorldToLight[3i];
  u_xlat6 = (x_396 + vec3<f32>(x_398.x, x_398.y, x_398.z));
  let x_409 : vec3<f32> = u_xlat6;
  let x_411 : vec4<f32> = textureSampleBias(x_LightTexture0, sampler_LightTexture0, x_409, -8.0f);
  u_xlat6.x = x_411.w;
  ...
  u_xlat0 = (vec3<f32>(x_420.x, x_420.x, x_420.x) * vec3<f32>(x_424.x, x_424.y, x_424.z));
  let x_432 : vec3<f32> = u_xlat1;
  let x_434 : vec4<f32> = textureSample(x_CameraGBufferTexture0, sampler_CameraGBufferTexture0, vec2<f32>(x_432.x, x_432.y));
  ...
  u_xlat18 = inverseSqrt(x_465);
  let x_467 : f32 = u_xlat18;
  let x_469 : vec3<f32> = u_xlat1;
  u_xlat1 = (vec3<f32>(x_467, x_467, x_467) * x_469);
  let x_471 : vec4<f32> = u_xlat2;
  let x_475 : vec3<f32> = x_14.x_WorldSpaceCameraPos;
  let x_477 : vec3<f32> = (vec3<f32>(x_471.x, x_471.y, x_471.w) + -(x_475));
  ...
  u_xlat18 = dot(vec3<f32>(x_480.x, x_480.y, x_480.z), vec3<f32>(x_482.x, x_482.y, x_482.z));
  let x_485 : f32 = u_xlat18;
  u_xlat18 = inverseSqrt(x_485);
  ...
  u_xlat18 = dot(-(vec3<f32>(x_494.x, x_494.y, x_494.z)), x_497);
  let x_499 : f32 = u_xlat18;
  let x_500 : f32 = u_xlat18;
  u_xlat18 = (x_499 + x_500);
  let x_502 : vec3<f32> = u_xlat1;
  let x_503 : f32 = u_xlat18;
  let x_507 : vec4<f32> = u_xlat2;
  let x_510 : vec3<f32> = ((x_502 * -(vec3<f32>(x_503, x_503, x_503))) + -(vec3<f32>(x_507.x, x_507.y, x_507.z)));
  ...
  u_xlat18 = dot(x_513, -(x_514));
  let x_517 : f32 = u_xlat18;
  u_xlat18 = clamp(x_517, 0.0f, 1.0f);
  let x_519 : vec4<f32> = u_xlat2;
  let x_521 : vec3<f32> = u_xlat3;
  u_xlat1.x = dot(vec3<f32>(x_519.x, x_519.y, x_519.z), -(x_521));
  ...
  let x_538 : f32 = u_xlat5.w;
  u_xlat1.y = (-(x_538) + 1.0f);
  let x_547 : vec3<f32> = u_xlat1;
  let x_549 : vec4<f32> = textureSample(unity_NHxRoughness, samplerunity_NHxRoughness, vec2<f32>(x_547.x, x_547.y));
  u_xlat1.x = x_549.x;
  let x_553 : f32 = u_xlat1.x;
  u_xlat1.x = (x_553 * 16.0f);
  let x_557 : vec3<f32> = u_xlat1;
  let x_559 : vec4<f32> = u_xlat5;
  let x_562 : vec4<f32> = u_xlat4;
  u_xlat1 = ((vec3<f32>(x_557.x, x_557.x, x_557.x) * vec3<f32>(x_559.x, x_559.y, x_559.z)) + vec3<f32>(x_562.x, x_562.y, x_562.z));
  let x_565 : f32 = u_xlat18;
  let x_567 : vec3<f32> = u_xlat0;
  u_xlat0 = (vec3<f32>(x_565, x_565, x_565) * x_567);
  let x_571 : vec3<f32> = u_xlat0;
  let x_572 : vec3<f32> = u_xlat1;
  let x_573 : vec3<f32> = (x_571 * x_572);

...
fn main_1() {
  ...
  let x_852 : vec4<f32> = x_36.unity_ObjectToWorld[2i];
  let x_854 : vec4<f32> = u_xlat1;
  let x_857 : vec4<f32> = u_xlat1;
  let x_859 : vec3<f32> = ((vec3<f32>(x_852.x, x_852.y, x_852.z) * vec3<f32>(x_854.z, x_854.z, x_854.z)) + vec3<f32>(x_857.x, x_857.y, x_857.w));
  let x_860 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_859.x, x_859.y, x_859.z, x_860.w);
  let x_863 : vec3<f32> = vs_TEXCOORD6;
  let x_864 : vec4<f32> = u_xlat1;
  u_xlat18 = dot(x_863, vec3<f32>(x_864.x, x_864.y, x_864.z));
  let x_867 : f32 = u_xlat18;
  let x_868 : f32 = u_xlat18;
  u_xlat18 = (x_867 + x_868);
  let x_870 : vec4<f32> = u_xlat1;
  let x_872 : f32 = u_xlat18;
  let x_876 : vec3<f32> = vs_TEXCOORD6;
  let x_877 : vec3<f32> = ((vec3<f32>(x_870.x, x_870.y, x_870.z) * -(vec3<f32>(x_872, x_872, x_872))) + x_876);
  let x_878 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_877.x, x_877.y, x_877.z, x_878.w);
  let x_888 : vec4<f32> = u_xlat1;
  let x_890 : vec4<f32> = textureSample(x_Cube, sampler_Cube, vec3<f32>(x_888.x, x_888.y, x_888.z));
  ...
  let x_898 : vec3<f32> = (vec3<f32>(x_894.x, x_894.y, x_894.z) * vec3<f32>(x_896.x, x_896.y, x_896.z));
  ...
  let x_909 : vec3<f32> = ((vec3<f32>(x_903.x, x_903.y, x_903.z) * vec3<f32>(x_905.w, x_905.w, x_905.w)) + x_908);

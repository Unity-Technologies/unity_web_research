...
fn main_1() {
  ...
  let x_722 : vec3<f32> = sqrt(vec3<f32>(x_720.x, x_720.y, x_720.z));
  let x_723 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_722.x, x_722.y, x_722.z, x_723.w);
  let x_725 : vec2<f32> = vs_TEXCOORD0;
  let x_728 : vec4<f32> = x_38.x_Dithering_Params;
  let x_732 : vec4<f32> = x_38.x_Dithering_Params;
  let x_734 : vec2<f32> = ((x_725 * vec2<f32>(x_728.x, x_728.y)) + vec2<f32>(x_732.z, x_732.w));
  let x_735 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_734.x, x_734.y, x_735.z, x_735.w);
  let x_742 : vec4<f32> = u_xlat1;
  let x_745 : f32 = x_38.x_GlobalMipBias.x;
  let x_746 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_742.x, x_742.y), x_745);
  u_xlat15 = x_746.w;
  let x_748 : f32 = u_xlat15;
  u_xlat15 = ((x_748 * 2.0f) + -1.0f);
  let x_751 : f32 = u_xlat15;
  u_xlatb1.x = (x_751 >= 0.0f);
  let x_755 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_755);
  let x_758 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_758)) + 1.0f);
  ...
  u_xlat15 = (x_767 * x_769);
  let x_771 : vec4<f32> = u_xlat0;
  let x_773 : vec3<f32> = (vec3<f32>(x_771.x, x_771.y, x_771.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_850 : vec3<f32> = (vec3<f32>(x_848.x, x_848.y, x_848.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_851 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_850.x, x_850.y, x_850.z, x_851.w);
  let x_853 : vec4<f32> = u_xlat0;
  let x_855 : vec3<f32> = (vec3<f32>(x_853.x, x_853.y, x_853.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_856 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_855.x, x_855.y, x_855.z, x_856.w);
  let x_858 : vec4<f32> = u_xlat2;
  let x_860 : vec3<f32> = (vec3<f32>(x_858.x, x_858.y, x_858.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_871 : vec3<f32> = (vec3<f32>(x_869.x, x_869.y, x_869.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_881 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_879.x, x_879.y, x_879.z, x_879.x));
  ...
  let x_924 : vec3<f32> = max(vec3<f32>(x_921.x, x_921.y, x_921.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_925 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_924.x, x_924.y, x_924.z, x_925.w);
  SV_Target0.w = 1.0f;
  return;
}

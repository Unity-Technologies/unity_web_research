...
fn main_1() {
  ...
  let x_442 : vec4<f32> = in_TEXCOORD1;
  let x_446 : vec4<f32> = x_16.unity_LightmapST;
  let x_450 : vec4<f32> = x_16.unity_LightmapST;
  vs_INTERP0 = ((vec2<f32>(x_442.x, x_442.y) * vec2<f32>(x_446.x, x_446.y)) + vec2<f32>(x_450.z, x_450.w));
  let x_454 : vec4<f32> = in_TANGENT0;
  let x_457 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_459 : vec3<f32> = (vec3<f32>(x_454.y, x_454.y, x_454.y) * vec3<f32>(x_457.x, x_457.y, x_457.z));
  let x_460 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_459.x, x_459.y, x_459.z, x_460.w);
  let x_463 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_465 : vec4<f32> = in_TANGENT0;
  ...
  let x_474 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_476 : vec4<f32> = in_TANGENT0;
  ...
  u_xlat15 = dot(vec3<f32>(x_484.x, x_484.y, x_484.z), vec3<f32>(x_486.x, x_486.y, x_486.z));
  let x_489 : f32 = u_xlat15;
  u_xlat15 = max(x_489, 1.17549435e-38f);
  let x_491 : f32 = u_xlat15;
  u_xlat15 = inverseSqrt(x_491);
  ...
  let x_499 : vec4<f32> = vs_INTERP4;
  vs_INTERP4 = vec4<f32>(x_498.x, x_498.y, x_498.z, x_499.w);
  let x_502 : f32 = in_TANGENT0.w;
  vs_INTERP4.w = x_502;
  let x_507 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP5 = x_507;
  let x_509 : vec4<f32> = in_COLOR0;
  vs_INTERP6 = x_509;
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(5) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(4) in_TEXCOORD1_param : vec4<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP9, vs_INTERP8, gl_Position, vs_INTERP0, vs_INTERP4, vs_INTERP5, vs_INTERP6);
}

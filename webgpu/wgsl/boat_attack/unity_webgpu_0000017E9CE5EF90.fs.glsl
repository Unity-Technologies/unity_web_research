...
void main()
{
    ...
    u_xlat1 = (-u_xlat5.yyyy) * unity_MatrixInvVP[1];
    u_xlat1 = unity_MatrixInvVP[0] * u_xlat5.xxxx + u_xlat1;
    ...
    u_xlat15 = min(u_xlat5.x, _Clamp);
    u_xlatb5 = 0.0<u_xlat5.x;
    u_xlat0.xz = u_xlat0.xz * vec2(u_xlat15);
    u_xlat0.xy = bool(u_xlatb5) ? u_xlat0.xz : vec2(0.0, 0.0);
    ...
    u_xlat2.xyz = texture(sampler2D(_BlitTexture, sampler_PointClamp), u_xlat10.xy, _GlobalMipBias.x).xyz;
    ...
    u_xlat3.xyz = texture(sampler2D(_BlitTexture, sampler_PointClamp), u_xlat4.xy, _GlobalMipBias.x).xyz;

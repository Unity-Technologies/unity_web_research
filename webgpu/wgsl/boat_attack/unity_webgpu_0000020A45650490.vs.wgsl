...
@group(1) @binding(5) var<uniform> x_189 : VGlobals;
...
@group(1) @binding(4) var<uniform> x_349 : LightShadows;
...
fn main_1() {
  ...
  let x_351 : vec4<f32> = x_349.x_MainLightWorldToShadow[0i][1i];
  let x_353 : vec3<f32> = (vec3<f32>(x_340.y, x_340.y, x_340.y) * vec3<f32>(x_351.x, x_351.y, x_351.z));
  let x_354 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_353.x, x_353.y, x_353.z, x_354.w);
  let x_357 : vec4<f32> = x_349.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_368 : vec4<f32> = x_349.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_382 : vec4<f32> = x_349.x_MainLightWorldToShadow[0i][3i];
  let x_384 : vec3<f32> = (vec3<f32>(x_379.x, x_379.y, x_379.z) + vec3<f32>(x_382.x, x_382.y, x_382.z));
  let x_385 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_384.x, x_384.y, x_384.z, x_385.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_388 : vec3<f32> = u_xlat1;
  let x_391 : vec4<f32> = x_189.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_388.y, x_388.y, x_388.y, x_388.y) * x_391);
  let x_394 : vec4<f32> = x_189.unity_MatrixVP[0i];
  let x_395 : vec3<f32> = u_xlat1;
  let x_398 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_394 * vec4<f32>(x_395.x, x_395.x, x_395.x, x_395.x)) + x_398);
  let x_401 : vec4<f32> = x_189.unity_MatrixVP[2i];
  let x_402 : vec3<f32> = u_xlat1;
  let x_405 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_401 * vec4<f32>(x_402.z, x_402.z, x_402.z, x_402.z)) + x_405);
  let x_411 : vec4<f32> = u_xlat0;
  let x_413 : vec4<f32> = x_189.unity_MatrixVP[3i];
  gl_Position = (x_411 + x_413);
  return;
}

...
vec4 hlslcc_FragCoord = vec4(gl_FragCoord.xyz, 1.0/gl_FragCoord.w);
    ...
    u_xlat33 = max(u_xlat3.w, 9.99999975e-05);
    u_xlat3.xyz = u_xlat3.xyz / vec3(u_xlat33);
    u_xlat4.xy = vec2(1.0) / vec2(_ScaledScreenParams.xy);
    u_xlat4.xy = u_xlat4.xy * hlslcc_FragCoord.xy;
    u_xlat33 = u_xlat4.y * _ScaleBiasRt.x + _ScaleBiasRt.y;
    u_xlat4.z = (-u_xlat33) + 1.0;
    u_xlat33 = texture(sampler2D(_ScreenSpaceOcclusionTexture, sampler_ScreenSpaceOcclusionTexture), u_xlat4.xz, _GlobalMipBias.x).x;
    u_xlat1.x = u_xlat33 + -1.0;
    u_xlat1.x = _AmbientOcclusionParam.w * u_xlat1.x + 1.0;
    u_xlat33 = min(u_xlat33, 1.0);
    vec3 txVec0 = vec3(vs_TEXCOORD6.xy,vs_TEXCOORD6.z);
    u_xlat35 = textureLod(sampler2DShadow(_MainLightShadowmapTexture, sampler_MainLightShadowmapTexture), txVec0, 0.0);
    ...
    u_xlat35 = u_xlat36 * u_xlat4.x + u_xlat35;
    ...
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu33 ; u_xlatu_loop_1++)
    {
        ...
        u_xlat7.xyz = (-vs_TEXCOORD1.xyz) * _AdditionalLightsPosition[int(u_xlatu36)].www + _AdditionalLightsPosition[int(u_xlatu36)].xyz;
        ...
        u_xlat37 = u_xlat37 * _AdditionalLightsAttenuation[int(u_xlatu36)].x;
        u_xlat37 = (-u_xlat37) * u_xlat37 + 1.0;
        u_xlat37 = max(u_xlat37, 0.0);
        u_xlat37 = u_xlat37 * u_xlat37;
        u_xlat37 = u_xlat37 * u_xlat27.x;
        u_xlat27.x = dot(_AdditionalLightsSpotDir[int(u_xlatu36)].xyz, u_xlat7.xyz);
        u_xlat27.x = u_xlat27.x * _AdditionalLightsAttenuation[int(u_xlatu36)].z + _AdditionalLightsAttenuation[int(u_xlatu36)].w;
        u_xlat27.x = clamp(u_xlat27.x, 0.0, 1.0);
        u_xlat27.x = u_xlat27.x * u_xlat27.x;
        u_xlat37 = u_xlat37 * u_xlat27.x;
        u_xlatu27 = u_xlatu36 >> (5u & uint(0x1F));
        u_xlati38 = 1 << (int(u_xlatu36) & int(0x1F));
        u_xlati27 = int(uint(u_xlati38) & floatBitsToUint(_AdditionalLightsCookieEnableBits[int(u_xlatu27)]));
        if(u_xlati27 != 0) {
            u_xlati27 = int(_AdditionalLightsLightTypes[int(u_xlatu36)]);
            u_xlati38 = (u_xlati27 != 0) ? 0 : 1;
            u_xlati39 = int(u_xlatu36) << (2 & int(0x1F));
            if(u_xlati38 != 0) {
                u_xlat8.xyz = vs_TEXCOORD1.yyy * _AdditionalLightsWorldToLights[(u_xlati39 + 1) / 4][(u_xlati39 + 1) % 4].xyw;
                u_xlat8.xyz = _AdditionalLightsWorldToLights[u_xlati39 / 4][u_xlati39 % 4].xyw * vs_TEXCOORD1.xxx + u_xlat8.xyz;
                u_xlat8.xyz = _AdditionalLightsWorldToLights[(u_xlati39 + 2) / 4][(u_xlati39 + 2) % 4].xyw * vs_TEXCOORD1.zzz + u_xlat8.xyz;
                u_xlat8.xyz = u_xlat8.xyz + _AdditionalLightsWorldToLights[(u_xlati39 + 3) / 4][(u_xlati39 + 3) % 4].xyw;
                u_xlat8.xy = u_xlat8.xy / u_xlat8.zz;
                u_xlat8.xy = u_xlat8.xy * vec2(0.5, 0.5) + vec2(0.5, 0.5);
                u_xlat8.xy = clamp(u_xlat8.xy, 0.0, 1.0);
                u_xlat8.xy = _AdditionalLightsCookieAtlasUVRects[int(u_xlatu36)].xy * u_xlat8.xy + _AdditionalLightsCookieAtlasUVRects[int(u_xlatu36)].zw;
            } else {
                u_xlatb27 = u_xlati27==1;
                u_xlati27 = u_xlatb27 ? 1 : int(0);
                if(u_xlati27 != 0) {
                    u_xlat27.xy = vs_TEXCOORD1.yy * _AdditionalLightsWorldToLights[(u_xlati39 + 1) / 4][(u_xlati39 + 1) % 4].xy;
                    u_xlat27.xy = _AdditionalLightsWorldToLights[u_xlati39 / 4][u_xlati39 % 4].xy * vs_TEXCOORD1.xx + u_xlat27.xy;
                    u_xlat27.xy = _AdditionalLightsWorldToLights[(u_xlati39 + 2) / 4][(u_xlati39 + 2) % 4].xy * vs_TEXCOORD1.zz + u_xlat27.xy;
                    u_xlat27.xy = u_xlat27.xy + _AdditionalLightsWorldToLights[(u_xlati39 + 3) / 4][(u_xlati39 + 3) % 4].xy;
                    u_xlat27.xy = u_xlat27.xy * vec2(0.5, 0.5) + vec2(0.5, 0.5);
                    u_xlat27.xy = fract(u_xlat27.xy);
                    u_xlat8.xy = _AdditionalLightsCookieAtlasUVRects[int(u_xlatu36)].xy * u_xlat27.xy + _AdditionalLightsCookieAtlasUVRects[int(u_xlatu36)].zw;
                } else {
                    u_xlat9 = vs_TEXCOORD1.yyyy * _AdditionalLightsWorldToLights[(u_xlati39 + 1) / 4][(u_xlati39 + 1) % 4];
                    u_xlat9 = _AdditionalLightsWorldToLights[u_xlati39 / 4][u_xlati39 % 4] * vs_TEXCOORD1.xxxx + u_xlat9;
                    u_xlat9 = _AdditionalLightsWorldToLights[(u_xlati39 + 2) / 4][(u_xlati39 + 2) % 4] * vs_TEXCOORD1.zzzz + u_xlat9;
                    u_xlat9 = u_xlat9 + _AdditionalLightsWorldToLights[(u_xlati39 + 3) / 4][(u_xlati39 + 3) % 4];
        ...
        u_xlat8.xyz = u_xlat8.xyz * _AdditionalLightsColor[int(u_xlatu36)].xyz;

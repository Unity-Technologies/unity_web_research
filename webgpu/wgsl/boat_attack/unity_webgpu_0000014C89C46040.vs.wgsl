...
@group(1) @binding(3) var<uniform> x_434 : LightShadows;
...
fn main_1() {
  ...
  let x_436 : vec4<f32> = x_434.x_MainLightWorldToShadow[0i][1i];
  let x_438 : vec3<f32> = (vec3<f32>(x_425.y, x_425.y, x_425.y) * vec3<f32>(x_436.x, x_436.y, x_436.z));
  let x_439 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_438.x, x_438.y, x_438.z, x_439.w);
  let x_442 : vec4<f32> = x_434.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_453 : vec4<f32> = x_434.x_MainLightWorldToShadow[0i][2i];
  ...
  vs_INTERP8 = vec3<f32>(x_464.x, x_464.y, x_464.z);
  let x_467 : vec4<f32> = u_xlat1;
  let x_470 : vec4<f32> = x_434.x_MainLightWorldToShadow[0i][3i];
  let x_472 : vec3<f32> = (vec3<f32>(x_467.x, x_467.y, x_467.z) + vec3<f32>(x_470.x, x_470.y, x_470.z));
  let x_473 : vec4<f32> = vs_INTERP3;
  vs_INTERP3 = vec4<f32>(x_472.x, x_472.y, x_472.z, x_473.w);
  vs_INTERP3.w = 0.0f;
  let x_478 : vec4<f32> = in_TANGENT0;
  let x_481 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_483 : vec3<f32> = (vec3<f32>(x_478.y, x_478.y, x_478.y) * vec3<f32>(x_481.x, x_481.y, x_481.z));
  let x_484 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_483.x, x_483.y, x_483.z, x_484.w);
  let x_487 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_489 : vec4<f32> = in_TANGENT0;
  let x_492 : vec4<f32> = u_xlat0;
  let x_494 : vec3<f32> = ((vec3<f32>(x_487.x, x_487.y, x_487.z) * vec3<f32>(x_489.x, x_489.x, x_489.x)) + vec3<f32>(x_492.x, x_492.y, x_492.z));
  let x_495 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_494.x, x_494.y, x_494.z, x_495.w);
  let x_498 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  ...
  u_xlat12 = dot(vec3<f32>(x_508.x, x_508.y, x_508.z), vec3<f32>(x_510.x, x_510.y, x_510.z));
  let x_513 : f32 = u_xlat12;
  u_xlat12 = max(x_513, 1.17549435e-38f);
  let x_515 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_515);
  ...
  let x_523 : vec4<f32> = vs_INTERP4;
  vs_INTERP4 = vec4<f32>(x_522.x, x_522.y, x_522.z, x_523.w);
  let x_526 : f32 = in_TANGENT0.w;
  vs_INTERP4.w = x_526;
  let x_530 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP5 = x_530;
  let x_532 : vec4<f32> = in_COLOR0;
  vs_INTERP6 = x_532;
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(4) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP9, gl_Position, vs_INTERP8, vs_INTERP3, vs_INTERP4, vs_INTERP5, vs_INTERP6);
}

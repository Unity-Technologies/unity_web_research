...
fn main_1() {
  ...
  let x_767 : f32 = x_759;
  u_xlat0.z = x_767;
  let x_769 : vec2<f32> = vs_TEXCOORD0;
  let x_772 : vec4<f32> = x_13.x_Dithering_Params;
  let x_776 : vec4<f32> = x_13.x_Dithering_Params;
  let x_778 : vec2<f32> = ((x_769 * vec2<f32>(x_772.x, x_772.y)) + vec2<f32>(x_776.z, x_776.w));
  let x_779 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_778.x, x_778.y, x_779.z, x_779.w);
  let x_786 : vec4<f32> = u_xlat1;
  let x_789 : f32 = x_13.x_GlobalMipBias.x;
  let x_790 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_786.x, x_786.y), x_789);
  u_xlat15 = x_790.w;
  let x_792 : f32 = u_xlat15;
  u_xlat15 = ((x_792 * 2.0f) + -1.0f);
  let x_797 : f32 = u_xlat15;
  u_xlatb1.x = (x_797 >= 0.0f);
  let x_801 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_801);
  let x_804 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_804)) + 1.0f);
  ...
  u_xlat15 = (x_813 * x_815);
  let x_817 : vec4<f32> = u_xlat0;
  let x_819 : vec3<f32> = (vec3<f32>(x_817.x, x_817.y, x_817.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_820 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_819.x, x_819.y, x_819.z, x_820.w);
  let x_822 : vec4<f32> = u_xlat0;
  let x_825 : vec3<f32> = log2(abs(vec3<f32>(x_822.x, x_822.y, x_822.z)));
  let x_826 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_825.x, x_825.y, x_825.z, x_826.w);
  let x_828 : vec4<f32> = u_xlat2;
  let x_830 : vec3<f32> = (vec3<f32>(x_828.x, x_828.y, x_828.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_841 : vec3<f32> = ((vec3<f32>(x_838.x, x_838.y, x_838.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_842 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_841.x, x_841.y, x_841.z, x_842.w);
  let x_844 : vec4<f32> = u_xlat0;
  let x_846 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_844.x, x_844.y, x_844.z, x_844.x));
  u_xlatb0 = vec3<bool>(x_846.x, x_846.y, x_846.z);
  let x_849 : bool = u_xlatb0.x;
  if (x_849) {
  ...
  let x_858 : f32 = x_850;
  u_xlat0.x = x_858;
  let x_861 : bool = u_xlatb0.y;
  if (x_861) {
    let x_866 : f32 = u_xlat1.y;
    x_862 = x_866;
  } else {
    let x_869 : f32 = u_xlat2.y;
    x_862 = x_869;
  }
  let x_870 : f32 = x_862;
  u_xlat0.y = x_870;
  let x_873 : bool = u_xlatb0.z;
  if (x_873) {
  ...
  let x_891 : vec3<f32> = ((vec3<f32>(x_884, x_884, x_884) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_889.x, x_889.y, x_889.z));
  let x_892 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_891.x, x_891.y, x_891.z, x_892.w);
  let x_894 : vec4<f32> = u_xlat0;
  let x_896 : vec3<f32> = (vec3<f32>(x_894.x, x_894.y, x_894.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_897 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_896.x, x_896.y, x_896.z, x_897.w);
  let x_899 : vec4<f32> = u_xlat0;
  let x_901 : vec3<f32> = (vec3<f32>(x_899.x, x_899.y, x_899.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_902 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_901.x, x_901.y, x_901.z, x_902.w);
  let x_904 : vec4<f32> = u_xlat2;
  let x_906 : vec3<f32> = (vec3<f32>(x_904.x, x_904.y, x_904.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_907 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_906.x, x_906.y, x_906.z, x_907.w);
  let x_909 : vec4<f32> = u_xlat2;
  let x_912 : vec3<f32> = log2(abs(vec3<f32>(x_909.x, x_909.y, x_909.z)));
  let x_913 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_912.x, x_912.y, x_912.z, x_913.w);
  let x_915 : vec4<f32> = u_xlat2;
  let x_917 : vec3<f32> = (vec3<f32>(x_915.x, x_915.y, x_915.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_918 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_917.x, x_917.y, x_917.z, x_918.w);
  let x_920 : vec4<f32> = u_xlat2;
  let x_922 : vec3<f32> = exp2(vec3<f32>(x_920.x, x_920.y, x_920.z));
  let x_923 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_922.x, x_922.y, x_922.z, x_923.w);
  let x_925 : vec4<f32> = u_xlat0;
  let x_927 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_925.x, x_925.y, x_925.z, x_925.x));
  ...
  let x_970 : vec3<f32> = max(vec3<f32>(x_967.x, x_967.y, x_967.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_971 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_970.x, x_970.y, x_970.z, x_971.w);
  SV_Target0.w = 1.0f;
  return;
}

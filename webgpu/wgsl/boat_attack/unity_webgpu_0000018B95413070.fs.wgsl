...
fn main_1() {
  ...
  let x_633 : f32 = x_625;
  u_xlat0.z = x_633;
  let x_635 : vec2<f32> = vs_TEXCOORD0;
  let x_638 : vec4<f32> = x_13.x_Dithering_Params;
  let x_642 : vec4<f32> = x_13.x_Dithering_Params;
  let x_644 : vec2<f32> = ((x_635 * vec2<f32>(x_638.x, x_638.y)) + vec2<f32>(x_642.z, x_642.w));
  let x_645 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_644.x, x_644.y, x_645.z);
  let x_652 : vec3<f32> = u_xlat1;
  let x_655 : f32 = x_13.x_GlobalMipBias.x;
  let x_656 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_652.x, x_652.y), x_655);
  u_xlat15 = x_656.w;
  let x_658 : f32 = u_xlat15;
  u_xlat15 = ((x_658 * 2.0f) + -1.0f);
  let x_663 : f32 = u_xlat15;
  u_xlatb1.x = (x_663 >= 0.0f);
  let x_667 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_667);
  let x_670 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_670)) + 1.0f);
  ...
  u_xlat15 = (x_679 * x_681);
  let x_683 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_683.x, x_683.y, x_683.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_686 : vec4<f32> = u_xlat0;
  let x_689 : vec3<f32> = log2(abs(vec3<f32>(x_686.x, x_686.y, x_686.z)));
  let x_690 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_689.x, x_689.y, x_689.z, x_690.w);
  let x_692 : vec4<f32> = u_xlat2;
  let x_694 : vec3<f32> = (vec3<f32>(x_692.x, x_692.y, x_692.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_705 : vec3<f32> = ((vec3<f32>(x_702.x, x_702.y, x_702.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_706 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_705.x, x_705.y, x_705.z, x_706.w);
  let x_708 : vec4<f32> = u_xlat0;
  let x_710 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_708.x, x_708.y, x_708.z, x_708.x));
  u_xlatb0 = vec3<bool>(x_710.x, x_710.y, x_710.z);
  let x_713 : bool = u_xlatb0.x;
  if (x_713) {
  ...
  let x_755 : vec3<f32> = ((vec3<f32>(x_748, x_748, x_748) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_753.x, x_753.y, x_753.z));
  let x_756 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_755.x, x_755.y, x_755.z, x_756.w);
  let x_758 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_758.x, x_758.y, x_758.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_761 : vec4<f32> = u_xlat0;
  let x_763 : vec3<f32> = (vec3<f32>(x_761.x, x_761.y, x_761.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_764 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_763.x, x_763.y, x_763.z, x_764.w);
  let x_766 : vec4<f32> = u_xlat2;
  let x_768 : vec3<f32> = (vec3<f32>(x_766.x, x_766.y, x_766.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_779 : vec3<f32> = (vec3<f32>(x_777.x, x_777.y, x_777.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_789 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_787.x, x_787.y, x_787.z, x_787.x));
  ...
  let x_832 : vec3<f32> = max(vec3<f32>(x_829.x, x_829.y, x_829.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  vs_INTERP3 = (vec3<f32>(x_348, x_348, x_348) * vec3<f32>(x_350.x, x_350.y, x_350.z));
  ...
  vs_INTERP2 = vec3<f32>(x_427.x, x_427.y, x_427.z);
  ...
  u_xlat15 = max(x_475, 1.17549435e-38f);
  ...
  let x_485 : vec4<f32> = vs_INTERP0;
  vs_INTERP0 = vec4<f32>(x_484.x, x_484.y, x_484.z, x_485.w);
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(4) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP3, vs_INTERP2, gl_Position, vs_INTERP0, vs_INTERP1);
}

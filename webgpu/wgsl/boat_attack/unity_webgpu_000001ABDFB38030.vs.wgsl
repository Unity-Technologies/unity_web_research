...
@group(1) @binding(3) var<uniform> x_495 : MainLightShadows;
...
fn main_1() {
  ...
  let x_497 : vec4<f32> = x_495.x_MainLightWorldToShadow[0i][1i];
  let x_499 : vec3<f32> = (vec3<f32>(x_489.y, x_489.y, x_489.y) * vec3<f32>(x_497.x, x_497.y, x_497.z));
  let x_500 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_499.x, x_499.y, x_499.z, x_500.w);
  let x_503 : vec4<f32> = x_495.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_514 : vec4<f32> = x_495.x_MainLightWorldToShadow[0i][2i];
  let x_516 : vec4<f32> = u_xlat0;
  let x_519 : vec4<f32> = u_xlat0;
  let x_521 : vec3<f32> = ((vec3<f32>(x_514.x, x_514.y, x_514.z) * vec3<f32>(x_516.z, x_516.z, x_516.z)) + vec3<f32>(x_519.x, x_519.y, x_519.w));
  let x_522 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_521.x, x_521.y, x_521.z, x_522.w);
  let x_525 : vec4<f32> = u_xlat0;
  let x_528 : vec4<f32> = x_495.x_MainLightWorldToShadow[0i][3i];
  let x_530 : vec3<f32> = (vec3<f32>(x_525.x, x_525.y, x_525.z) + vec3<f32>(x_528.x, x_528.y, x_528.z));
  let x_531 : vec4<f32> = vs_INTERP9;
  vs_INTERP9 = vec4<f32>(x_530.x, x_530.y, x_530.z, x_531.w);
  vs_INTERP9.w = 0.0f;
  let x_536 : vec4<f32> = phase0_Output0_6;
  vs_INTERP5 = vec2<f32>(x_536.x, x_536.y);
  let x_539 : vec4<f32> = phase0_Output0_6;
  vs_INTERP6 = vec2<f32>(x_539.z, x_539.w);
  return;
}

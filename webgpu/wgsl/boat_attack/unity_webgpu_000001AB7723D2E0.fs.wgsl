...
fn main_1() {
  ...
  let x_606 : vec3<f32> = ((vec3<f32>(x_600.x, x_600.x, x_600.x) * x_602) + vec3<f32>(x_604.x, x_604.y, x_604.z));
  let x_607 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_606.x, x_606.y, x_606.z, x_607.w);
  let x_609 : vec2<f32> = vs_TEXCOORD0;
  let x_612 : vec4<f32> = x_40.x_Grain_TilingParams;
  let x_616 : vec4<f32> = x_40.x_Grain_TilingParams;
  let x_618 : vec2<f32> = ((x_609 * vec2<f32>(x_612.x, x_612.y)) + vec2<f32>(x_616.z, x_616.w));
  let x_619 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_618.x, x_618.y, x_619.z);
  let x_626 : vec3<f32> = u_xlat1;
  let x_629 : f32 = x_40.x_GlobalMipBias.x;
  let x_630 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_626.x, x_626.y), x_629);
  u_xlat15 = x_630.w;
  let x_632 : f32 = u_xlat15;
  u_xlat15 = (x_632 + -0.5f);
  let x_635 : f32 = u_xlat15;
  let x_636 : f32 = u_xlat15;
  u_xlat15 = (x_635 + x_636);
  let x_638 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_638.x, x_638.y, x_638.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_647 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_647);
  let x_652 : f32 = x_40.x_Grain_Params.y;
  let x_654 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_652 * -(x_654)) + 1.0f);
  let x_660 : f32 = u_xlat15;
  let x_662 : vec4<f32> = u_xlat0;
  u_xlat6 = (vec3<f32>(x_660, x_660, x_660) * vec3<f32>(x_662.x, x_662.y, x_662.z));
  let x_665 : vec3<f32> = u_xlat6;
  let x_668 : vec2<f32> = x_40.x_Grain_Params;
  u_xlat6 = (x_665 * vec3<f32>(x_668.x, x_668.x, x_668.x));
  let x_671 : vec3<f32> = u_xlat6;
  let x_672 : vec3<f32> = u_xlat1;
  let x_675 : vec4<f32> = u_xlat0;
  let x_677 : vec3<f32> = ((x_671 * vec3<f32>(x_672.x, x_672.x, x_672.x)) + vec3<f32>(x_675.x, x_675.y, x_675.z));

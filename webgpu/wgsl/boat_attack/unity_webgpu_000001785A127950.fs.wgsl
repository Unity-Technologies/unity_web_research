...
fn main_1() {
  ...
  let x_939 : vec3<f32> = ((x_933 * vec3<f32>(x_934.x, x_934.x, x_934.x)) + vec3<f32>(x_937.x, x_937.y, x_937.z));
  let x_940 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_939.x, x_939.y, x_939.z, x_940.w);
  let x_942 : vec4<f32> = u_xlat0;
  let x_944 : vec3<f32> = (vec3<f32>(x_942.x, x_942.y, x_942.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_945 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_944.x, x_944.y, x_944.z, x_945.w);
  let x_947 : vec4<f32> = u_xlat0;
  let x_950 : vec3<f32> = log2(abs(vec3<f32>(x_947.x, x_947.y, x_947.z)));
  let x_951 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_950.x, x_950.y, x_950.z, x_951.w);
  let x_953 : vec4<f32> = u_xlat2;
  let x_955 : vec3<f32> = (vec3<f32>(x_953.x, x_953.y, x_953.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_956 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_955.x, x_955.y, x_955.z, x_956.w);
  let x_958 : vec4<f32> = u_xlat2;
  let x_960 : vec3<f32> = exp2(vec3<f32>(x_958.x, x_958.y, x_958.z));
  let x_961 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_960.x, x_960.y, x_960.z, x_961.w);
  let x_963 : vec4<f32> = u_xlat2;
  let x_966 : vec3<f32> = ((vec3<f32>(x_963.x, x_963.y, x_963.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_967 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_966.x, x_966.y, x_966.z, x_967.w);
  let x_970 : vec4<f32> = u_xlat0;
  let x_972 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_970.x, x_970.y, x_970.z, x_970.x));
  ...
  u_xlat0.z = x_1008;
  let x_1010 : vec2<f32> = vs_TEXCOORD0;
  let x_1013 : vec4<f32> = x_25.x_Dithering_Params;
  let x_1017 : vec4<f32> = x_25.x_Dithering_Params;
  let x_1019 : vec2<f32> = ((x_1010 * vec2<f32>(x_1013.x, x_1013.y)) + vec2<f32>(x_1017.z, x_1017.w));
  let x_1020 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1019.x, x_1019.y, x_1020.z, x_1020.w);
  let x_1027 : vec4<f32> = u_xlat1;
  let x_1030 : f32 = x_25.x_GlobalMipBias.x;
  let x_1031 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_1027.x, x_1027.y), x_1030);
  u_xlat15 = x_1031.w;
  let x_1033 : f32 = u_xlat15;
  u_xlat15 = ((x_1033 * 2.0f) + -1.0f);
  let x_1037 : f32 = u_xlat15;
  u_xlatb1.x = (x_1037 >= 0.0f);
  let x_1041 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_1041);
  let x_1044 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_1044)) + 1.0f);
  let x_1048 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_1048);
  let x_1050 : f32 = u_xlat15;
  u_xlat15 = (-(x_1050) + 1.0f);
  let x_1053 : f32 = u_xlat15;
  let x_1055 : f32 = u_xlat1.x;
  u_xlat15 = (x_1053 * x_1055);
  let x_1057 : vec4<f32> = u_xlat0;
  let x_1059 : vec3<f32> = (vec3<f32>(x_1057.x, x_1057.y, x_1057.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_1060 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1059.x, x_1059.y, x_1059.z, x_1060.w);
  let x_1062 : vec4<f32> = u_xlat0;
  let x_1065 : vec3<f32> = log2(abs(vec3<f32>(x_1062.x, x_1062.y, x_1062.z)));
  let x_1066 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1065.x, x_1065.y, x_1065.z, x_1066.w);
  let x_1068 : vec4<f32> = u_xlat2;
  let x_1070 : vec3<f32> = (vec3<f32>(x_1068.x, x_1068.y, x_1068.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_1071 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1070.x, x_1070.y, x_1070.z, x_1071.w);
  let x_1073 : vec4<f32> = u_xlat2;
  let x_1075 : vec3<f32> = exp2(vec3<f32>(x_1073.x, x_1073.y, x_1073.z));
  let x_1076 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1075.x, x_1075.y, x_1075.z, x_1076.w);
  let x_1078 : vec4<f32> = u_xlat2;
  let x_1081 : vec3<f32> = ((vec3<f32>(x_1078.x, x_1078.y, x_1078.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_1082 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1081.x, x_1081.y, x_1081.z, x_1082.w);
  let x_1084 : vec4<f32> = u_xlat0;
  let x_1086 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_1084.x, x_1084.y, x_1084.z, x_1084.x));
  ...
  u_xlat0.y = x_1110;
  let x_1113 : bool = u_xlatb0.z;
  if (x_1113) {
    let x_1118 : f32 = u_xlat1.z;
    x_1114 = x_1118;
  } else {
    let x_1121 : f32 = u_xlat2.z;
    x_1114 = x_1121;
  }
  let x_1122 : f32 = x_1114;
  u_xlat0.z = x_1122;
  let x_1124 : f32 = u_xlat15;
  let x_1129 : vec4<f32> = u_xlat0;
  let x_1131 : vec3<f32> = ((vec3<f32>(x_1124, x_1124, x_1124) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_1129.x, x_1129.y, x_1129.z));
  let x_1132 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1131.x, x_1131.y, x_1131.z, x_1132.w);
  let x_1134 : vec4<f32> = u_xlat0;
  let x_1136 : vec3<f32> = (vec3<f32>(x_1134.x, x_1134.y, x_1134.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1137 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1136.x, x_1136.y, x_1136.z, x_1137.w);
  let x_1139 : vec4<f32> = u_xlat0;
  let x_1141 : vec3<f32> = (vec3<f32>(x_1139.x, x_1139.y, x_1139.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1142 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1141.x, x_1141.y, x_1141.z, x_1142.w);
  let x_1144 : vec4<f32> = u_xlat2;
  let x_1146 : vec3<f32> = (vec3<f32>(x_1144.x, x_1144.y, x_1144.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1147 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1146.x, x_1146.y, x_1146.z, x_1147.w);
  let x_1149 : vec4<f32> = u_xlat2;
  let x_1152 : vec3<f32> = log2(abs(vec3<f32>(x_1149.x, x_1149.y, x_1149.z)));
  let x_1153 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1152.x, x_1152.y, x_1152.z, x_1153.w);
  let x_1155 : vec4<f32> = u_xlat2;
  let x_1157 : vec3<f32> = (vec3<f32>(x_1155.x, x_1155.y, x_1155.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_1158 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1157.x, x_1157.y, x_1157.z, x_1158.w);
  let x_1160 : vec4<f32> = u_xlat2;
  let x_1162 : vec3<f32> = exp2(vec3<f32>(x_1160.x, x_1160.y, x_1160.z));
  ...
  let x_1210 : vec3<f32> = max(vec3<f32>(x_1207.x, x_1207.y, x_1207.z), vec3<f32>(0.0f, 0.0f, 0.0f));

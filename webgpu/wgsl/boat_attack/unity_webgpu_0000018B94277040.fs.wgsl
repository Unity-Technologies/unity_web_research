...
fn main_1() {
  ...
  let x_665 : vec2<f32> = vs_TEXCOORD0;
  let x_668 : vec4<f32> = x_13.x_Dithering_Params;
  let x_672 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_685 : f32 = x_13.x_GlobalMipBias.x;
  let x_686 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_682.x, x_682.y), x_685);
  u_xlat15 = x_686.w;
  let x_688 : f32 = u_xlat15;
  u_xlat15 = ((x_688 * 2.0f) + -1.0f);
  let x_693 : f32 = u_xlat15;
  u_xlatb1.x = (x_693 >= 0.0f);
  let x_697 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_697);
  let x_700 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_700)) + 1.0f);
  ...
  let x_715 : vec3<f32> = (vec3<f32>(x_713.x, x_713.y, x_713.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_726 : vec3<f32> = (vec3<f32>(x_724.x, x_724.y, x_724.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_737 : vec3<f32> = ((vec3<f32>(x_734.x, x_734.y, x_734.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_738 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_737.x, x_737.y, x_737.z, x_738.w);
  let x_741 : vec4<f32> = u_xlat0;
  let x_743 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_741.x, x_741.y, x_741.z, x_741.x));
  u_xlatb0 = vec3<bool>(x_743.x, x_743.y, x_743.z);
  let x_746 : bool = u_xlatb0.x;
  if (x_746) {
  ...
  let x_767 : f32 = x_759;
  u_xlat0.y = x_767;
  let x_770 : bool = u_xlatb0.z;
  if (x_770) {
    let x_775 : f32 = u_xlat1.z;
    x_771 = x_775;
  } else {
  ...
  let x_788 : vec3<f32> = ((vec3<f32>(x_781, x_781, x_781) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_786.x, x_786.y, x_786.z));
  let x_789 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_788.x, x_788.y, x_788.z, x_789.w);
  let x_791 : vec4<f32> = u_xlat0;
  let x_793 : vec3<f32> = (vec3<f32>(x_791.x, x_791.y, x_791.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_794 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_793.x, x_793.y, x_793.z, x_794.w);
  let x_796 : vec4<f32> = u_xlat0;
  let x_798 : vec3<f32> = (vec3<f32>(x_796.x, x_796.y, x_796.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_799 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_798.x, x_798.y, x_798.z, x_799.w);
  let x_801 : vec4<f32> = u_xlat2;
  let x_803 : vec3<f32> = (vec3<f32>(x_801.x, x_801.y, x_801.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_804 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_803.x, x_803.y, x_803.z, x_804.w);
  let x_806 : vec4<f32> = u_xlat2;
  let x_809 : vec3<f32> = log2(abs(vec3<f32>(x_806.x, x_806.y, x_806.z)));
  let x_810 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_809.x, x_809.y, x_809.z, x_810.w);
  let x_812 : vec4<f32> = u_xlat2;
  let x_814 : vec3<f32> = (vec3<f32>(x_812.x, x_812.y, x_812.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_824 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_822.x, x_822.y, x_822.z, x_822.x));
  u_xlatb0 = vec3<bool>(x_824.x, x_824.y, x_824.z);
  let x_827 : bool = u_xlatb0.x;
  if (x_827) {
  ...
  let x_836 : f32 = x_828;
  u_xlat0.x = x_836;
  let x_839 : bool = u_xlatb0.y;
  if (x_839) {
    let x_844 : f32 = u_xlat1.y;
    x_840 = x_844;
  } else {
  ...
  let x_848 : f32 = x_840;
  u_xlat0.y = x_848;
  let x_851 : bool = u_xlatb0.z;
  if (x_851) {
    let x_856 : f32 = u_xlat1.z;
    x_852 = x_856;
  } else {
  ...
  let x_867 : vec3<f32> = max(vec3<f32>(x_864.x, x_864.y, x_864.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  u_xlat0 = ((x_41 * vec3<f32>(64.06158447265625f, 64.06158447265625f, 64.06158447265625f)) + vec3<f32>(-0.0625f, -0.5f, -0.5f));
  let x_54 : vec3<f32> = u_xlat0;
  SV_Target0.x = dot(vec2<f32>(1.16439998149871826172f, 1.79270005226135253906f), vec2<f32>(x_54.x, x_54.z));
  let x_62 : vec3<f32> = u_xlat0;
  SV_Target0.y = dot(vec3<f32>(1.16439998149871826172f, -0.21330000460147857666f, -0.53289997577667236328f), x_62);
  let x_68 : vec3<f32> = u_xlat0;
  SV_Target0.z = dot(vec2<f32>(1.16439998149871826172f, 2.112400054931640625f), vec2<f32>(x_68.x, x_68.y));
  SV_Target0.w = 1.0f;
  return;
}

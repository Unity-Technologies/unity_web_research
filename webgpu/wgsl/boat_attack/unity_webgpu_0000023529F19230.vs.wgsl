...
fn main_1() {
  ...
  vs_INTERP4 = x_175;
  let x_180 : vec4<f32> = in_TEXCOORD1;
  let x_184 : vec4<f32> = x_21.unity_LightmapST;
  let x_188 : vec4<f32> = x_21.unity_LightmapST;
  vs_INTERP5 = ((vec2<f32>(x_180.x, x_180.y) * vec2<f32>(x_184.x, x_184.y)) + vec2<f32>(x_188.z, x_188.w));
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>, @location(5) in_COLOR0_param : vec4<f32>, @location(4) in_TEXCOORD1_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP0, gl_Position, vs_INTERP1, vs_INTERP2, vs_INTERP3, vs_INTERP4, vs_INTERP5);
}

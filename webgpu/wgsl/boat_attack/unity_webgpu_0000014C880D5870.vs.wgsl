...
@group(1) @binding(5) var<uniform> x_315 : VGlobals;
...
fn main_1() {
  ...
  let x_317 : vec4<f32> = x_315.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_311.y, x_311.y, x_311.y, x_311.y) * x_317);
  let x_320 : vec4<f32> = x_315.unity_MatrixVP[0i];
  let x_321 : vec4<f32> = u_xlat0;
  let x_324 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_320 * vec4<f32>(x_321.x, x_321.x, x_321.x, x_321.x)) + x_324);
  let x_327 : vec4<f32> = x_315.unity_MatrixVP[2i];
  let x_328 : vec4<f32> = u_xlat0;
  let x_331 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_327 * vec4<f32>(x_328.z, x_328.z, x_328.z, x_328.z)) + x_331);
  let x_337 : vec4<f32> = u_xlat0;
  let x_339 : vec4<f32> = x_315.unity_MatrixVP[3i];
  gl_Position = (x_337 + x_339);
  return;
}

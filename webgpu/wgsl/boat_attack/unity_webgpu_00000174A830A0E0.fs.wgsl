...
fn main_1() {
  ...
  let x_668 : vec4<f32> = x_26.x_Dithering_Params;
  let x_672 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_685 : f32 = x_26.x_GlobalMipBias.x;
  ...
  let x_715 : vec3<f32> = (vec3<f32>(x_713.x, x_713.y, x_713.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_742 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_740.x, x_740.y, x_740.z, x_740.x));
  ...
  let x_787 : vec3<f32> = ((vec3<f32>(x_780, x_780, x_780) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_785.x, x_785.y, x_785.z));
  let x_788 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_787.x, x_787.y, x_787.z, x_788.w);
  let x_790 : vec4<f32> = u_xlat0;
  let x_792 : vec3<f32> = (vec3<f32>(x_790.x, x_790.y, x_790.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_793 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_792.x, x_792.y, x_792.z, x_793.w);
  let x_795 : vec4<f32> = u_xlat0;
  let x_797 : vec3<f32> = (vec3<f32>(x_795.x, x_795.y, x_795.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_798 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_797.x, x_797.y, x_797.z, x_798.w);
  let x_800 : vec4<f32> = u_xlat2;
  let x_802 : vec3<f32> = (vec3<f32>(x_800.x, x_800.y, x_800.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_803 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_802.x, x_802.y, x_802.z, x_803.w);
  let x_805 : vec4<f32> = u_xlat2;
  let x_808 : vec3<f32> = log2(abs(vec3<f32>(x_805.x, x_805.y, x_805.z)));
  let x_809 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_808.x, x_808.y, x_808.z, x_809.w);
  let x_811 : vec4<f32> = u_xlat2;
  let x_813 : vec3<f32> = (vec3<f32>(x_811.x, x_811.y, x_811.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_823 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_821.x, x_821.y, x_821.z, x_821.x));
  ...
  let x_866 : vec3<f32> = max(vec3<f32>(x_863.x, x_863.y, x_863.z), vec3<f32>(0.0f, 0.0f, 0.0f));

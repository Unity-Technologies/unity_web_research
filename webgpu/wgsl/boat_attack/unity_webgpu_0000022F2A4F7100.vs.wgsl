...
@group(1) @binding(4) var<uniform> x_294 : MainLightShadows;

var<private> vs_TEXCOORD8 : vec4<f32>;

@group(1) @binding(7) var<uniform> x_338 : VGlobals;
...
fn main_1() {
  ...
  let x_296 : vec4<f32> = x_294.x_MainLightWorldToShadow[0i][1i];
  let x_298 : vec3<f32> = (vec3<f32>(x_288.y, x_288.y, x_288.y) * vec3<f32>(x_296.x, x_296.y, x_296.z));
  let x_299 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_298.x, x_298.y, x_298.z, x_299.w);
  let x_302 : vec4<f32> = x_294.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_313 : vec4<f32> = x_294.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_327 : vec4<f32> = x_294.x_MainLightWorldToShadow[0i][3i];
  let x_329 : vec3<f32> = (vec3<f32>(x_324.x, x_324.y, x_324.z) + vec3<f32>(x_327.x, x_327.y, x_327.z));
  let x_330 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_329.x, x_329.y, x_329.z, x_330.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_334 : vec4<f32> = u_xlat0;
  let x_340 : vec4<f32> = x_338.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_334.y, x_334.y, x_334.y, x_334.y) * x_340);
  let x_343 : vec4<f32> = x_338.unity_MatrixVP[0i];
  let x_344 : vec4<f32> = u_xlat0;
  let x_347 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_343 * vec4<f32>(x_344.x, x_344.x, x_344.x, x_344.x)) + x_347);
  let x_350 : vec4<f32> = x_338.unity_MatrixVP[2i];
  let x_351 : vec4<f32> = u_xlat0;
  let x_354 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_350 * vec4<f32>(x_351.z, x_351.z, x_351.z, x_351.z)) + x_354);
  let x_360 : vec4<f32> = u_xlat0;
  let x_362 : vec4<f32> = x_338.unity_MatrixVP[3i];
  gl_Position = (x_360 + x_362);
  return;
}

...
fn main_1() {
  ...
  let x_783 : vec3<f32> = sqrt(vec3<f32>(x_781.x, x_781.y, x_781.z));
  let x_784 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_783.x, x_783.y, x_783.z, x_784.w);
  let x_786 : vec2<f32> = vs_TEXCOORD0;
  let x_789 : vec4<f32> = x_25.x_Dithering_Params;
  let x_793 : vec4<f32> = x_25.x_Dithering_Params;
  let x_795 : vec2<f32> = ((x_786 * vec2<f32>(x_789.x, x_789.y)) + vec2<f32>(x_793.z, x_793.w));
  let x_796 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_795.x, x_795.y, x_796.z);
  let x_803 : vec3<f32> = u_xlat1;
  let x_806 : f32 = x_25.x_GlobalMipBias.x;
  let x_807 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_803.x, x_803.y), x_806);
  u_xlat15 = x_807.w;
  let x_809 : f32 = u_xlat15;
  u_xlat15 = ((x_809 * 2.0f) + -1.0f);
  let x_813 : f32 = u_xlat15;
  u_xlatb1.x = (x_813 >= 0.0f);
  let x_817 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_817);
  let x_820 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_820)) + 1.0f);
  ...
  u_xlat15 = (x_829 * x_831);
  let x_833 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_833.x, x_833.y, x_833.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_836 : vec4<f32> = u_xlat0;
  let x_838 : vec3<f32> = log2(vec3<f32>(x_836.x, x_836.y, x_836.z));
  let x_839 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_838.x, x_838.y, x_838.z, x_839.w);
  let x_841 : vec4<f32> = u_xlat2;
  let x_843 : vec3<f32> = (vec3<f32>(x_841.x, x_841.y, x_841.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_844 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_843.x, x_843.y, x_843.z, x_844.w);
  let x_846 : vec4<f32> = u_xlat2;
  let x_848 : vec3<f32> = exp2(vec3<f32>(x_846.x, x_846.y, x_846.z));
  let x_849 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_848.x, x_848.y, x_848.z, x_849.w);
  let x_851 : vec4<f32> = u_xlat2;
  let x_854 : vec3<f32> = ((vec3<f32>(x_851.x, x_851.y, x_851.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_855 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_854.x, x_854.y, x_854.z, x_855.w);
  let x_858 : vec4<f32> = u_xlat0;
  let x_860 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_858.x, x_858.y, x_858.z, x_858.x));
  ...
  let x_905 : vec3<f32> = ((vec3<f32>(x_898, x_898, x_898) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_903.x, x_903.y, x_903.z));
  let x_906 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_905.x, x_905.y, x_905.z, x_906.w);
  let x_908 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_908.x, x_908.y, x_908.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_911 : vec4<f32> = u_xlat0;
  let x_913 : vec3<f32> = (vec3<f32>(x_911.x, x_911.y, x_911.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_914 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_913.x, x_913.y, x_913.z, x_914.w);
  let x_916 : vec4<f32> = u_xlat2;
  let x_918 : vec3<f32> = (vec3<f32>(x_916.x, x_916.y, x_916.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_929 : vec3<f32> = (vec3<f32>(x_927.x, x_927.y, x_927.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_939 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_937.x, x_937.y, x_937.z, x_937.x));
  ...
  let x_982 : vec3<f32> = max(vec3<f32>(x_979.x, x_979.y, x_979.z), vec3<f32>(0.0f, 0.0f, 0.0f));

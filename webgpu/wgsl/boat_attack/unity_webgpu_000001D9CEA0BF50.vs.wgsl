...
fn main_1() {
  ...
  vs_TEXCOORD6 = ((vec3<f32>(x_616.x, x_616.y, x_616.z) * vec3<f32>(x_618.z, x_618.z, x_618.z)) + vec3<f32>(x_621.x, x_621.y, x_621.w));
  ...
  u_xlat0.x = ((x_631 * x_633) + -(x_636));
  let x_640 : vec4<f32> = u_xlat3;
  let x_642 : vec4<f32> = u_xlat3;
  u_xlat1 = (vec4<f32>(x_640.y, x_640.w, x_640.z, x_640.x) * x_642);
  let x_645 : vec4<f32> = x_19.unity_SHBr;
  let x_646 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_645, x_646);
  let x_650 : vec4<f32> = x_19.unity_SHBg;
  let x_651 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_650, x_651);
  let x_656 : vec4<f32> = x_19.unity_SHBb;
  let x_657 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_656, x_657);

...
fn main_1() {
  ...
  let x_395 : vec4<f32> = x_385.unity_FogColor;
  let x_398 : vec3<f32> = ((vec3<f32>(x_389.x, x_389.y, x_389.z) * vec3<f32>(x_391.x, x_391.y, x_391.z)) + -(vec3<f32>(x_395.x, x_395.y, x_395.z)));
  let x_399 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_398.x, x_398.y, x_398.z, x_399.w);
  let x_404 : f32 = u_xlat1.w;
  SV_Target0.w = x_404;
  let x_409 : f32 = vs_TEXCOORD3.w;
  ...
  let x_422 : vec4<f32> = x_385.unity_FogColor;
  let x_424 : vec3<f32> = ((vec3<f32>(x_416, x_416, x_416) * vec3<f32>(x_418.x, x_418.y, x_418.z)) + vec3<f32>(x_422.x, x_422.y, x_422.z));
  let x_425 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_424.x, x_424.y, x_424.z, x_425.w);
  return;
}

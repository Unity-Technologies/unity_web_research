...
layout(set = 0, binding = 5) uniform mediump texture2D _BlueNoise_Texture;
...
layout(set = 0, binding = 8) uniform mediump  sampler sampler_LinearRepeat;
void main()
{
    ...
    if(u_xlatb8){
        ...
        u_xlat16.x = u_xlat5.x / u_xlat6.x;
        ...
        u_xlat1.x = u_xlat1.x * _Distortion_Params2.y;
        u_xlat5.x = min(abs(u_xlat1.x), 1.0);
        ...
        u_xlat19 = u_xlat12 * u_xlat5.x;
        u_xlatb26 = 1.0<abs(u_xlat1.x);
        u_xlat19 = u_xlat19 * -2.0 + 1.57079637;
        u_xlat19 = u_xlatb26 ? u_xlat19 : float(0.0);
        u_xlat5.x = u_xlat5.x * u_xlat12 + u_xlat19;
        u_xlat1.x = min(u_xlat1.x, 1.0);
        u_xlatb1.x = u_xlat1.x<(-u_xlat1.x);
        u_xlat1.x = (u_xlatb1.x) ? (-u_xlat5.x) : u_xlat5.x;
    ...
    u_xlat16.xy = u_xlat1.xy + u_xlat2.zw;
    u_xlat2.xy = u_xlat0.xy + u_xlat2.xy;
    u_xlat4.xy = vec2(1.0) / vec2(u_xlat16.xy);
    u_xlat4.zw = u_xlat1.xy * u_xlat4.xy + vec2(-1.0, -1.0);
    u_xlat1.xy = vec2(1.0) / vec2(u_xlat2.xy);
    ...
    u_xlat5 = u_xlat16.xxxx * u_xlat6 + u_xlat5;
    u_xlat0 = u_xlat14.xyxy + u_xlat4.zyxy;
    u_xlat0 = u_xlat0 + vec4(-0.5, -0.5, -0.5, -0.5);
    u_xlat0 = u_xlat0 * _Bloom_Texture_TexelSize.xyxy;
    u_xlat0 = min(u_xlat0, vec4(1.0, 1.0, 1.0, 1.0));
    u_xlat4 = textureLod(sampler2D(_Bloom_Texture, sampler_LinearClamp), u_xlat0.xy, 0.0);
    u_xlat0 = textureLod(sampler2D(_Bloom_Texture, sampler_LinearClamp), u_xlat0.zw, 0.0);
    u_xlat0 = u_xlat0 * u_xlat2.xxxx;
    u_xlat0 = u_xlat16.xxxx * u_xlat4 + u_xlat0;
    u_xlat0 = u_xlat0 * u_xlat2.yyyy;
    u_xlat0 = u_xlat16.yyyy * u_xlat5 + u_xlat0;
    ...
    u_xlat0.xyz = u_xlat0.xyz * _Bloom_Params.yzw + u_xlat3.xyz;

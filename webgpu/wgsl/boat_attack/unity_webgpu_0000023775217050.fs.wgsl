...
fn main_1() {
  ...
  let x_931 : vec3<f32> = ((vec3<f32>(x_925.x, x_925.x, x_925.x) * x_927) + vec3<f32>(x_929.x, x_929.y, x_929.z));
  let x_932 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_931.x, x_931.y, x_931.z, x_932.w);
  let x_934 : vec2<f32> = vs_TEXCOORD0;
  let x_937 : vec4<f32> = x_38.x_Grain_TilingParams;
  let x_941 : vec4<f32> = x_38.x_Grain_TilingParams;
  ...
  let x_954 : f32 = x_38.x_GlobalMipBias.x;
  let x_955 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_951.x, x_951.y), x_954);
  u_xlat18 = x_955.w;
  let x_957 : f32 = u_xlat18;
  u_xlat18 = (x_957 + -0.5f);
  let x_959 : f32 = u_xlat18;
  let x_960 : f32 = u_xlat18;
  u_xlat18 = (x_959 + x_960);
  let x_962 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_962.x, x_962.y, x_962.z), vec3<f32>(0.21267290413379669189f, 0.71515220403671264648f, 0.07217500358819961548f));
  let x_971 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_971);
  let x_976 : f32 = x_38.x_Grain_Params.y;
  let x_978 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_976 * -(x_978)) + 1.0f);
  let x_984 : f32 = u_xlat18;
  let x_986 : vec4<f32> = u_xlat0;
  u_xlat7 = (vec3<f32>(x_984, x_984, x_984) * vec3<f32>(x_986.x, x_986.y, x_986.z));
  let x_989 : vec3<f32> = u_xlat7;
  let x_992 : vec2<f32> = x_38.x_Grain_Params;
  u_xlat7 = (x_989 * vec3<f32>(x_992.x, x_992.x, x_992.x));
  let x_995 : vec3<f32> = u_xlat7;
  let x_996 : vec4<f32> = u_xlat1;
  let x_999 : vec4<f32> = u_xlat0;
  let x_1001 : vec3<f32> = ((x_995 * vec3<f32>(x_996.x, x_996.x, x_996.x)) + vec3<f32>(x_999.x, x_999.y, x_999.z));
  let x_1002 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1001.x, x_1001.y, x_1001.z, x_1002.w);
  let x_1004 : vec4<f32> = u_xlat0;
  let x_1006 : vec3<f32> = (vec3<f32>(x_1004.x, x_1004.y, x_1004.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_1007 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1006.x, x_1006.y, x_1006.z, x_1007.w);
  let x_1009 : vec4<f32> = u_xlat0;
  let x_1012 : vec3<f32> = log2(abs(vec3<f32>(x_1009.x, x_1009.y, x_1009.z)));
  let x_1013 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1012.x, x_1012.y, x_1012.z, x_1013.w);
  let x_1015 : vec4<f32> = u_xlat2;
  let x_1017 : vec3<f32> = (vec3<f32>(x_1015.x, x_1015.y, x_1015.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_1028 : vec3<f32> = ((vec3<f32>(x_1025.x, x_1025.y, x_1025.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_1029 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1028.x, x_1028.y, x_1028.z, x_1029.w);
  let x_1032 : vec4<f32> = u_xlat0;
  let x_1034 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_1032.x, x_1032.y, x_1032.z, x_1032.x));
  u_xlatb0 = vec3<bool>(x_1034.x, x_1034.y, x_1034.z);
  let x_1037 : bool = u_xlatb0.x;
  if (x_1037) {
  ...
  u_xlat0.x = x_1046;
  let x_1049 : bool = u_xlatb0.y;
  if (x_1049) {
    let x_1054 : f32 = u_xlat1.y;
    x_1050 = x_1054;
  } else {
    let x_1057 : f32 = u_xlat2.y;
    x_1050 = x_1057;
  }
  let x_1058 : f32 = x_1050;
  u_xlat0.y = x_1058;
  let x_1061 : bool = u_xlatb0.z;
  if (x_1061) {
    let x_1066 : f32 = u_xlat1.z;
    x_1062 = x_1066;
  } else {
  ...
  u_xlat0.z = x_1070;
  let x_1072 : vec2<f32> = vs_TEXCOORD0;
  let x_1075 : vec4<f32> = x_38.x_Dithering_Params;
  let x_1079 : vec4<f32> = x_38.x_Dithering_Params;
  let x_1081 : vec2<f32> = ((x_1072 * vec2<f32>(x_1075.x, x_1075.y)) + vec2<f32>(x_1079.z, x_1079.w));
  let x_1082 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1081.x, x_1081.y, x_1082.z, x_1082.w);
  let x_1089 : vec4<f32> = u_xlat1;
  let x_1092 : f32 = x_38.x_GlobalMipBias.x;
  let x_1093 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_1089.x, x_1089.y), x_1092);
  u_xlat18 = x_1093.w;
  let x_1095 : f32 = u_xlat18;
  u_xlat18 = ((x_1095 * 2.0f) + -1.0f);
  let x_1098 : f32 = u_xlat18;
  u_xlatb1.x = (x_1098 >= 0.0f);
  let x_1102 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_1102);
  let x_1105 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_1105)) + 1.0f);
  let x_1109 : f32 = u_xlat18;
  u_xlat18 = sqrt(x_1109);
  let x_1111 : f32 = u_xlat18;
  u_xlat18 = (-(x_1111) + 1.0f);
  let x_1114 : f32 = u_xlat18;
  let x_1116 : f32 = u_xlat1.x;
  u_xlat18 = (x_1114 * x_1116);
  let x_1118 : vec4<f32> = u_xlat0;
  let x_1120 : vec3<f32> = (vec3<f32>(x_1118.x, x_1118.y, x_1118.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_1121 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1120.x, x_1120.y, x_1120.z, x_1121.w);
  let x_1123 : vec4<f32> = u_xlat0;
  let x_1126 : vec3<f32> = log2(abs(vec3<f32>(x_1123.x, x_1123.y, x_1123.z)));
  let x_1127 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1126.x, x_1126.y, x_1126.z, x_1127.w);
  let x_1129 : vec4<f32> = u_xlat2;
  let x_1131 : vec3<f32> = (vec3<f32>(x_1129.x, x_1129.y, x_1129.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_1142 : vec3<f32> = ((vec3<f32>(x_1139.x, x_1139.y, x_1139.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  ...
  let x_1192 : vec3<f32> = ((vec3<f32>(x_1185, x_1185, x_1185) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1190.x, x_1190.y, x_1190.z));
  let x_1193 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1192.x, x_1192.y, x_1192.z, x_1193.w);
  let x_1195 : vec4<f32> = u_xlat0;
  let x_1197 : vec3<f32> = (vec3<f32>(x_1195.x, x_1195.y, x_1195.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1198 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1197.x, x_1197.y, x_1197.z, x_1198.w);
  let x_1200 : vec4<f32> = u_xlat0;
  let x_1202 : vec3<f32> = (vec3<f32>(x_1200.x, x_1200.y, x_1200.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1203 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1202.x, x_1202.y, x_1202.z, x_1203.w);
  let x_1205 : vec4<f32> = u_xlat2;
  let x_1207 : vec3<f32> = (vec3<f32>(x_1205.x, x_1205.y, x_1205.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1208 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1207.x, x_1207.y, x_1207.z, x_1208.w);
  let x_1210 : vec4<f32> = u_xlat2;
  let x_1213 : vec3<f32> = log2(abs(vec3<f32>(x_1210.x, x_1210.y, x_1210.z)));
  let x_1214 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1213.x, x_1213.y, x_1213.z, x_1214.w);
  let x_1216 : vec4<f32> = u_xlat2;
  let x_1218 : vec3<f32> = (vec3<f32>(x_1216.x, x_1216.y, x_1216.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1228 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1226.x, x_1226.y, x_1226.z, x_1226.x));
  ...
  let x_1271 : vec3<f32> = max(vec3<f32>(x_1268.x, x_1268.y, x_1268.z), vec3<f32>(0.0f, 0.0f, 0.0f));

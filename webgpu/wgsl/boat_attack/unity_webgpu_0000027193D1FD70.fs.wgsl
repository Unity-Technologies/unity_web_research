...
fn main_1() {
  ...
  let x_694 : vec3<f32> = (vec3<f32>(x_692.x, x_692.y, x_692.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_695 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_694.x, x_694.y, x_694.z, x_695.w);
  let x_697 : vec4<f32> = u_xlat0;
  let x_700 : vec3<f32> = log2(abs(vec3<f32>(x_697.x, x_697.y, x_697.z)));
  let x_701 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_700.x, x_700.y, x_700.z, x_701.w);
  let x_703 : vec4<f32> = u_xlat2;
  let x_705 : vec3<f32> = (vec3<f32>(x_703.x, x_703.y, x_703.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_716 : vec3<f32> = ((vec3<f32>(x_713.x, x_713.y, x_713.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_717 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_716.x, x_716.y, x_716.z, x_717.w);
  let x_720 : vec4<f32> = u_xlat0;
  let x_722 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_720.x, x_720.y, x_720.z, x_720.x));
  ...
  let x_767 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_780 : f32 = x_38.x_GlobalMipBias.x;
  let x_781 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_777.x, x_777.y), x_780);
  u_xlat15 = x_781.w;
  let x_783 : f32 = u_xlat15;
  u_xlat15 = ((x_783 * 2.0f) + -1.0f);
  let x_786 : f32 = u_xlat15;
  u_xlatb1.x = (x_786 >= 0.0f);
  let x_790 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_790);
  let x_793 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_793)) + 1.0f);
  let x_797 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_797);
  let x_799 : f32 = u_xlat15;
  u_xlat15 = (-(x_799) + 1.0f);
  let x_802 : f32 = u_xlat15;
  let x_804 : f32 = u_xlat1.x;
  u_xlat15 = (x_802 * x_804);
  let x_806 : vec4<f32> = u_xlat0;
  let x_808 : vec3<f32> = (vec3<f32>(x_806.x, x_806.y, x_806.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_819 : vec3<f32> = (vec3<f32>(x_817.x, x_817.y, x_817.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_830 : vec3<f32> = ((vec3<f32>(x_827.x, x_827.y, x_827.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_831 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_830.x, x_830.y, x_830.z, x_831.w);
  let x_833 : vec4<f32> = u_xlat0;
  let x_835 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_833.x, x_833.y, x_833.z, x_833.x));
  ...
  let x_847 : f32 = x_839;
  u_xlat0.x = x_847;
  let x_850 : bool = u_xlatb0.y;
  if (x_850) {
    let x_855 : f32 = u_xlat1.y;
    x_851 = x_855;
  } else {
    let x_858 : f32 = u_xlat2.y;
    x_851 = x_858;
  }
  let x_859 : f32 = x_851;
  u_xlat0.y = x_859;
  let x_862 : bool = u_xlatb0.z;
  if (x_862) {
    let x_867 : f32 = u_xlat1.z;
    x_863 = x_867;
  } else {
  ...
  let x_880 : vec3<f32> = ((vec3<f32>(x_873, x_873, x_873) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_878.x, x_878.y, x_878.z));
  let x_881 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_880.x, x_880.y, x_880.z, x_881.w);
  let x_883 : vec4<f32> = u_xlat0;
  let x_885 : vec3<f32> = (vec3<f32>(x_883.x, x_883.y, x_883.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_886 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_885.x, x_885.y, x_885.z, x_886.w);
  let x_888 : vec4<f32> = u_xlat0;
  let x_890 : vec3<f32> = (vec3<f32>(x_888.x, x_888.y, x_888.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_891 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_890.x, x_890.y, x_890.z, x_891.w);
  let x_893 : vec4<f32> = u_xlat2;
  let x_895 : vec3<f32> = (vec3<f32>(x_893.x, x_893.y, x_893.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_906 : vec3<f32> = (vec3<f32>(x_904.x, x_904.y, x_904.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_916 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_914.x, x_914.y, x_914.z, x_914.x));

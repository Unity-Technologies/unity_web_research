...
fn main_1() {
  ...
  let x_919 : vec2<f32> = vs_TEXCOORD0;
  let x_922 : vec4<f32> = x_25.x_Dithering_Params;
  let x_926 : vec4<f32> = x_25.x_Dithering_Params;
  let x_928 : vec2<f32> = ((x_919 * vec2<f32>(x_922.x, x_922.y)) + vec2<f32>(x_926.z, x_926.w));
  let x_929 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_928.x, x_928.y, x_929.z, x_929.w);
  let x_936 : vec4<f32> = u_xlat1;
  let x_939 : f32 = x_25.x_GlobalMipBias.x;
  let x_940 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_936.x, x_936.y), x_939);
  u_xlat15 = x_940.w;
  let x_942 : f32 = u_xlat15;
  u_xlat15 = ((x_942 * 2.0f) + -1.0f);
  let x_946 : f32 = u_xlat15;
  u_xlatb1.x = (x_946 >= 0.0f);
  let x_950 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_950);
  let x_953 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_953)) + 1.0f);
  ...
  u_xlat15 = (x_962 * x_964);
  let x_966 : vec4<f32> = u_xlat0;
  let x_968 : vec3<f32> = (vec3<f32>(x_966.x, x_966.y, x_966.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_969 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_968.x, x_968.y, x_968.z, x_969.w);
  let x_971 : vec4<f32> = u_xlat0;
  let x_974 : vec3<f32> = log2(abs(vec3<f32>(x_971.x, x_971.y, x_971.z)));
  let x_975 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_974.x, x_974.y, x_974.z, x_975.w);
  let x_977 : vec4<f32> = u_xlat2;
  let x_979 : vec3<f32> = (vec3<f32>(x_977.x, x_977.y, x_977.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_980 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_979.x, x_979.y, x_979.z, x_980.w);
  let x_982 : vec4<f32> = u_xlat2;
  let x_984 : vec3<f32> = exp2(vec3<f32>(x_982.x, x_982.y, x_982.z));
  let x_985 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_984.x, x_984.y, x_984.z, x_985.w);
  let x_987 : vec4<f32> = u_xlat2;
  let x_990 : vec3<f32> = ((vec3<f32>(x_987.x, x_987.y, x_987.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_991 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_990.x, x_990.y, x_990.z, x_991.w);
  let x_994 : vec4<f32> = u_xlat0;
  let x_996 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_994.x, x_994.y, x_994.z, x_994.x));
  ...
  u_xlat0.y = x_1020;
  let x_1023 : bool = u_xlatb0.z;
  if (x_1023) {
    let x_1028 : f32 = u_xlat1.z;
    x_1024 = x_1028;
  } else {
  ...
  let x_1041 : vec3<f32> = ((vec3<f32>(x_1034, x_1034, x_1034) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1039.x, x_1039.y, x_1039.z));
  let x_1042 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1041.x, x_1041.y, x_1041.z, x_1042.w);
  let x_1044 : vec4<f32> = u_xlat0;
  let x_1046 : vec3<f32> = (vec3<f32>(x_1044.x, x_1044.y, x_1044.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));

...
fn main_1() {
  ...
  let x_624 : vec4<f32> = x_19.x_EnvMatrix[2i];
  let x_626 : vec4<f32> = u_xlat3;
  let x_629 : vec3<f32> = u_xlat0;
  vs_TEXCOORD7 = ((vec3<f32>(x_624.x, x_624.y, x_624.z) * vec3<f32>(x_626.z, x_626.z, x_626.z)) + x_629);
  let x_631 : vec4<f32> = u_xlat1;
  let x_635 : vec4<f32> = x_19.unity_WorldToLight[1i];
  u_xlat0 = (vec3<f32>(x_631.y, x_631.y, x_631.y) * vec3<f32>(x_635.x, x_635.y, x_635.z));
  let x_639 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_641 : vec4<f32> = u_xlat1;
  let x_644 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_639.x, x_639.y, x_639.z) * vec3<f32>(x_641.x, x_641.x, x_641.x)) + x_644);
  let x_647 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_649 : vec4<f32> = u_xlat1;
  let x_652 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_647.x, x_647.y, x_647.z) * vec3<f32>(x_649.z, x_649.z, x_649.z)) + x_652);
  let x_656 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_658 : vec4<f32> = u_xlat1;
  let x_661 : vec3<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec3<f32>(x_656.x, x_656.y, x_656.z) * vec3<f32>(x_658.w, x_658.w, x_658.w)) + x_661);
  return;
}

...
fn main_1() {
  ...
  let x_884 : u32 = x_163.x_RenderingLayerMaxInt;
  let x_886 : f32 = x_43.unity_RenderingLayer.x;
  u_xlatu0 = (x_884 & bitcast<u32>(x_886));
  let x_889 : u32 = u_xlatu0;
  u_xlat0.x = f32(x_889);
  let x_894 : f32 = u_xlat0.x;
  let x_897 : f32 = x_163.x_RenderingLayerRcpMaxInt;
  SV_Target1.x = (x_894 * x_897);
  let x_901 : f32 = SV_Target1.x;
  SV_Target1.x = clamp(x_901, 0.0f, 1.0f);
  let x_904 : vec4<f32> = SV_Target1;
  SV_Target1 = vec4<f32>(x_904.x, vec3<f32>(0.0f, 0.0f, 0.0f).x, vec3<f32>(0.0f, 0.0f, 0.0f).y, vec3<f32>(0.0f, 0.0f, 0.0f).z);
  return;
}

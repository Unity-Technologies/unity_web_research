...
;
    ...
    u_xlat11.x = textureLod(sampler2DShadow(_MainLightShadowmapTexture, sampler_MainLightShadowmapTexture), txVec0, 0.0);
    u_xlat33 = (-_MainLightShadowParams.x) + 1.0;
    u_xlat11.x = u_xlat11.x * _MainLightShadowParams.x + u_xlat33;
    u_xlatb33 = 0.0>=u_xlat3.z;
    u_xlatb1 = u_xlat3.z>=1.0;
    u_xlatb33 = u_xlatb33 || u_xlatb1;
    u_xlat11.x = (u_xlatb33) ? 1.0 : u_xlat11.x;
    u_xlat3.xyz = vs_TEXCOORD1.xyz + (-_WorldSpaceCameraPos.xyz);
    u_xlat33 = dot(u_xlat3.xyz, u_xlat3.xyz);
    u_xlat33 = u_xlat33 * _MainLightShadowParams.z + _MainLightShadowParams.w;
    u_xlat33 = clamp(u_xlat33, 0.0, 1.0);
    u_xlat1.x = (-u_xlat11.x) + 1.0;
    u_xlat11.x = u_xlat33 * u_xlat1.x + u_xlat11.x;
    ...
    if(u_xlatb33){
        ...
        u_xlat33 = (u_xlatb5.y) ? u_xlat3.w : u_xlat3.x;
        u_xlat3.xyz = (u_xlatb5.x) ? u_xlat3.xyz : vec3(u_xlat33);
    } else {
        u_xlat3.x = float(1.0);
        u_xlat3.y = float(1.0);
        u_xlat3.z = float(1.0);
    ...
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu11 ; u_xlatu_loop_1++)
    {
        ...
        u_xlat1.x = dot(unity_LightIndices[int(u_xlatu1)], uintBitsToFloat(ImmCB_0[u_xlati35]));
        ...
        u_xlat35 = u_xlat35 * _AdditionalLightsAttenuation[int(u_xlatu1)].x;
        u_xlat35 = (-u_xlat35) * u_xlat35 + 1.0;
        u_xlat35 = max(u_xlat35, 0.0);
        u_xlat35 = u_xlat35 * u_xlat35;
        u_xlat35 = u_xlat35 * u_xlat36;
        u_xlat36 = dot(_AdditionalLightsSpotDir[int(u_xlatu1)].xyz, u_xlat7.xyz);
        u_xlat36 = u_xlat36 * _AdditionalLightsAttenuation[int(u_xlatu1)].z + _AdditionalLightsAttenuation[int(u_xlatu1)].w;
        u_xlat36 = clamp(u_xlat36, 0.0, 1.0);
        u_xlat36 = u_xlat36 * u_xlat36;
        u_xlat35 = u_xlat35 * u_xlat36;
        u_xlatu36 = u_xlatu1 >> (5u & uint(0x1F));
        u_xlati37 = 1 << (int(u_xlatu1) & int(0x1F));
        u_xlati36 = int(uint(u_xlati37) & floatBitsToUint(_AdditionalLightsCookieEnableBits[int(u_xlatu36)]));
        if(u_xlati36 != 0) {
            u_xlati36 = int(_AdditionalLightsLightTypes[int(u_xlatu1)]);
            u_xlati37 = (u_xlati36 != 0) ? 0 : 1;
            u_xlati27 = int(u_xlatu1) << (2 & int(0x1F));
            if(u_xlati37 != 0) {
                ...
                if(u_xlati36 != 0) {
                    u_xlat30.xy = vs_TEXCOORD1.yy * _AdditionalLightsWorldToLights[(u_xlati27 + 1) / 4][(u_xlati27 + 1) % 4].xy;
                    u_xlat30.xy = _AdditionalLightsWorldToLights[u_xlati27 / 4][u_xlati27 % 4].xy * vs_TEXCOORD1.xx + u_xlat30.xy;
                    u_xlat30.xy = _AdditionalLightsWorldToLights[(u_xlati27 + 2) / 4][(u_xlati27 + 2) % 4].xy * vs_TEXCOORD1.zz + u_xlat30.xy;
                    u_xlat30.xy = u_xlat30.xy + _AdditionalLightsWorldToLights[(u_xlati27 + 3) / 4][(u_xlati27 + 3) % 4].xy;
                    u_xlat30.xy = u_xlat30.xy * vec2(0.5, 0.5) + vec2(0.5, 0.5);
                    u_xlat30.xy = fract(u_xlat30.xy);
                    u_xlat8.xy = _AdditionalLightsCookieAtlasUVRects[int(u_xlatu1)].xy * u_xlat30.xy + _AdditionalLightsCookieAtlasUVRects[int(u_xlatu1)].zw;
                } else {
                    u_xlat9 = vs_TEXCOORD1.yyyy * _AdditionalLightsWorldToLights[(u_xlati27 + 1) / 4][(u_xlati27 + 1) % 4];
                    u_xlat9 = _AdditionalLightsWorldToLights[u_xlati27 / 4][u_xlati27 % 4] * vs_TEXCOORD1.xxxx + u_xlat9;
                    u_xlat9 = _AdditionalLightsWorldToLights[(u_xlati27 + 2) / 4][(u_xlati27 + 2) % 4] * vs_TEXCOORD1.zzzz + u_xlat9;
                    u_xlat9 = u_xlat9 + _AdditionalLightsWorldToLights[(u_xlati27 + 3) / 4][(u_xlati27 + 3) % 4];
                    u_xlat9.xyz = u_xlat9.xyz / u_xlat9.www;
                    u_xlat36 = dot(u_xlat9.xyz, u_xlat9.xyz);
                    u_xlat36 = inversesqrt(u_xlat36);
                    u_xlat9.xyz = vec3(u_xlat36) * u_xlat9.xyz;
                    u_xlat36 = dot(abs(u_xlat9.xyz), vec3(1.0, 1.0, 1.0));
                    u_xlat36 = max(u_xlat36, 9.99999997e-07);
                    u_xlat36 = float(1.0) / float(u_xlat36);
                    u_xlat10.xyz = vec3(u_xlat36) * u_xlat9.zxy;
                    ...
                    u_xlat27.xy = u_xlat9.xy * vec2(u_xlat36) + u_xlat27.xy;
                    u_xlat27.xy = u_xlat27.xy * vec2(0.5, 0.5) + vec2(0.5, 0.5);
                    u_xlat27.xy = clamp(u_xlat27.xy, 0.0, 1.0);
                    u_xlat8.xy = _AdditionalLightsCookieAtlasUVRects[int(u_xlatu1)].xy * u_xlat27.xy + _AdditionalLightsCookieAtlasUVRects[int(u_xlatu1)].zw;
                }
            }
            ...
            u_xlat8.xyz = (u_xlatb5.x) ? u_xlat8.xyz : vec3(u_xlat36);
        } else {
            u_xlat8.x = float(1.0);
            u_xlat8.y = float(1.0);
            u_xlat8.z = float(1.0);

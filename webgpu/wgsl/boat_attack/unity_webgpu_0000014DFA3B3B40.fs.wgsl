...
fn main_1() {
  ...
  let x_881 : vec2<f32> = vs_TEXCOORD0;
  let x_884 : vec4<f32> = x_13.x_Grain_TilingParams;
  let x_888 : vec4<f32> = x_13.x_Grain_TilingParams;
  let x_890 : vec2<f32> = ((x_881 * vec2<f32>(x_884.x, x_884.y)) + vec2<f32>(x_888.z, x_888.w));
  let x_891 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_890.x, x_890.y, x_891.z, x_891.w);
  let x_898 : vec4<f32> = u_xlat1;
  let x_901 : f32 = x_13.x_GlobalMipBias.x;
  let x_902 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_898.x, x_898.y), x_901);
  u_xlat18 = x_902.w;
  let x_904 : f32 = u_xlat18;
  u_xlat18 = (x_904 + -0.5f);
  let x_906 : f32 = u_xlat18;
  let x_907 : f32 = u_xlat18;
  u_xlat18 = (x_906 + x_907);
  let x_909 : vec4<f32> = u_xlat0;
  u_xlat1.x = dot(vec3<f32>(x_909.x, x_909.y, x_909.z), vec3<f32>(0.21267290413379669189f, 0.71515220403671264648f, 0.07217500358819961548f));
  let x_918 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_918);
  let x_923 : f32 = x_13.x_Grain_Params.y;
  let x_925 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_923 * -(x_925)) + 1.0f);
  let x_931 : f32 = u_xlat18;
  let x_933 : vec4<f32> = u_xlat0;
  u_xlat7 = (vec3<f32>(x_931, x_931, x_931) * vec3<f32>(x_933.x, x_933.y, x_933.z));
  let x_936 : vec3<f32> = u_xlat7;
  let x_939 : vec2<f32> = x_13.x_Grain_Params;
  u_xlat7 = (x_936 * vec3<f32>(x_939.x, x_939.x, x_939.x));
  let x_942 : vec3<f32> = u_xlat7;
  let x_943 : vec4<f32> = u_xlat1;
  let x_946 : vec4<f32> = u_xlat0;
  let x_948 : vec3<f32> = ((x_942 * vec3<f32>(x_943.x, x_943.x, x_943.x)) + vec3<f32>(x_946.x, x_946.y, x_946.z));
  let x_949 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_948.x, x_948.y, x_948.z, x_949.w);
  let x_951 : vec2<f32> = vs_TEXCOORD0;
  let x_954 : vec4<f32> = x_13.x_Dithering_Params;
  let x_958 : vec4<f32> = x_13.x_Dithering_Params;
  let x_960 : vec2<f32> = ((x_951 * vec2<f32>(x_954.x, x_954.y)) + vec2<f32>(x_958.z, x_958.w));
  let x_961 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_960.x, x_960.y, x_961.z, x_961.w);
  let x_968 : vec4<f32> = u_xlat1;
  let x_971 : f32 = x_13.x_GlobalMipBias.x;
  let x_972 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_968.x, x_968.y), x_971);
  u_xlat18 = x_972.w;
  let x_974 : f32 = u_xlat18;
  u_xlat18 = ((x_974 * 2.0f) + -1.0f);
  let x_978 : f32 = u_xlat18;
  u_xlatb1.x = (x_978 >= 0.0f);
  let x_982 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_982);
  let x_985 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_985)) + 1.0f);
  ...
  u_xlat18 = (x_994 * x_996);
  let x_998 : vec4<f32> = u_xlat0;
  let x_1000 : vec3<f32> = (vec3<f32>(x_998.x, x_998.y, x_998.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_1001 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1000.x, x_1000.y, x_1000.z, x_1001.w);
  let x_1003 : vec4<f32> = u_xlat0;
  let x_1006 : vec3<f32> = log2(abs(vec3<f32>(x_1003.x, x_1003.y, x_1003.z)));
  let x_1007 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1006.x, x_1006.y, x_1006.z, x_1007.w);
  let x_1009 : vec4<f32> = u_xlat2;
  let x_1011 : vec3<f32> = (vec3<f32>(x_1009.x, x_1009.y, x_1009.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_1012 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1011.x, x_1011.y, x_1011.z, x_1012.w);
  let x_1014 : vec4<f32> = u_xlat2;
  let x_1016 : vec3<f32> = exp2(vec3<f32>(x_1014.x, x_1014.y, x_1014.z));
  let x_1017 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1016.x, x_1016.y, x_1016.z, x_1017.w);
  let x_1019 : vec4<f32> = u_xlat2;
  let x_1022 : vec3<f32> = ((vec3<f32>(x_1019.x, x_1019.y, x_1019.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_1023 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1022.x, x_1022.y, x_1022.z, x_1023.w);
  let x_1026 : vec4<f32> = u_xlat0;
  let x_1028 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_1026.x, x_1026.y, x_1026.z, x_1026.x));
  ...
  let x_1073 : vec3<f32> = ((vec3<f32>(x_1066, x_1066, x_1066) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1071.x, x_1071.y, x_1071.z));
  let x_1074 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1073.x, x_1073.y, x_1073.z, x_1074.w);
  let x_1076 : vec4<f32> = u_xlat0;
  let x_1078 : vec3<f32> = (vec3<f32>(x_1076.x, x_1076.y, x_1076.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1079 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1078.x, x_1078.y, x_1078.z, x_1079.w);
  let x_1081 : vec4<f32> = u_xlat0;
  let x_1083 : vec3<f32> = (vec3<f32>(x_1081.x, x_1081.y, x_1081.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1084 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1083.x, x_1083.y, x_1083.z, x_1084.w);
  let x_1086 : vec4<f32> = u_xlat2;
  let x_1088 : vec3<f32> = (vec3<f32>(x_1086.x, x_1086.y, x_1086.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1089 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1088.x, x_1088.y, x_1088.z, x_1089.w);
  let x_1091 : vec4<f32> = u_xlat2;
  let x_1094 : vec3<f32> = log2(abs(vec3<f32>(x_1091.x, x_1091.y, x_1091.z)));
  let x_1095 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1094.x, x_1094.y, x_1094.z, x_1095.w);
  let x_1097 : vec4<f32> = u_xlat2;
  let x_1099 : vec3<f32> = (vec3<f32>(x_1097.x, x_1097.y, x_1097.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_1100 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1099.x, x_1099.y, x_1099.z, x_1100.w);
  let x_1102 : vec4<f32> = u_xlat2;
  let x_1104 : vec3<f32> = exp2(vec3<f32>(x_1102.x, x_1102.y, x_1102.z));
  let x_1105 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1104.x, x_1104.y, x_1104.z, x_1105.w);
  let x_1107 : vec4<f32> = u_xlat0;
  let x_1109 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1107.x, x_1107.y, x_1107.z, x_1107.x));
  ...
  let x_1152 : vec3<f32> = max(vec3<f32>(x_1149.x, x_1149.y, x_1149.z), vec3<f32>(0.0f, 0.0f, 0.0f));

...
fn main_1() {
  ...
  let x_93 : vec4<f32> = in_TANGENT0;
  let x_96 : vec4<f32> = x_21.unity_ObjectToWorld[1i];
  u_xlat0 = (vec3<f32>(x_93.y, x_93.y, x_93.y) * vec3<f32>(x_96.x, x_96.y, x_96.z));
  let x_100 : vec4<f32> = x_21.unity_ObjectToWorld[0i];
  let x_102 : vec4<f32> = in_TANGENT0;
  let x_105 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_100.x, x_100.y, x_100.z) * vec3<f32>(x_102.x, x_102.x, x_102.x)) + x_105);
  let x_108 : vec4<f32> = x_21.unity_ObjectToWorld[2i];
  let x_110 : vec4<f32> = in_TANGENT0;
  let x_113 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_108.x, x_108.y, x_108.z) * vec3<f32>(x_110.z, x_110.z, x_110.z)) + x_113);
  ...
  let x_130 : vec4<f32> = vs_INTERP0;
  vs_INTERP0 = vec4<f32>(x_129.x, x_129.y, x_129.z, x_130.w);
  let x_135 : f32 = in_TANGENT0.w;
  vs_INTERP0.w = x_135;
  let x_140 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP1 = x_140;
  let x_142 : vec3<f32> = in_NORMAL0;
  let x_144 : vec4<f32> = x_21.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_142, vec3<f32>(x_144.x, x_144.y, x_144.z));
  let x_149 : vec3<f32> = in_NORMAL0;
  let x_151 : vec4<f32> = x_21.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_149, vec3<f32>(x_151.x, x_151.y, x_151.z));
  let x_155 : vec3<f32> = in_NORMAL0;
  let x_157 : vec4<f32> = x_21.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_155, vec3<f32>(x_157.x, x_157.y, x_157.z));
  let x_161 : vec3<f32> = u_xlat0;
  let x_162 : vec3<f32> = u_xlat0;
  u_xlat6 = dot(x_161, x_162);
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP2, gl_Position, vs_INTERP0, vs_INTERP1, vs_INTERP3);
}

...
fn main_1() {
  ...
  let x_394 : f32 = x_386;
  u_xlat0.z = x_394;
  let x_396 : vec2<f32> = vs_TEXCOORD0;
  let x_399 : vec4<f32> = x_27.x_Dithering_Params;
  let x_403 : vec4<f32> = x_27.x_Dithering_Params;
  let x_405 : vec2<f32> = ((x_396 * vec2<f32>(x_399.x, x_399.y)) + vec2<f32>(x_403.z, x_403.w));
  let x_406 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_405.x, x_405.y, x_406.z);
  let x_413 : vec3<f32> = u_xlat1;
  let x_416 : f32 = x_27.x_GlobalMipBias.x;
  let x_417 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_413.x, x_413.y), x_416);
  u_xlat27 = x_417.w;
  let x_419 : f32 = u_xlat27;
  u_xlat27 = ((x_419 * 2.0f) + -1.0f);
  let x_423 : f32 = u_xlat27;
  u_xlat1.x = (-(abs(x_423)) + 1.0f);
  let x_429 : f32 = u_xlat27;
  u_xlatb27 = (x_429 >= 0.0f);
  let x_431 : bool = u_xlatb27;
  u_xlat27 = select(-1.0f, 1.0f, x_431);
  ...
  u_xlat0 = ((vec3<f32>(x_446, x_446, x_446) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_451);
  let x_453 : vec3<f32> = u_xlat0;
  u_xlat1 = (x_453 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_457 : vec3<f32> = u_xlat1;
  u_xlat1 = (x_457 * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_461 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(abs(x_461));
  let x_464 : vec3<f32> = u_xlat1;
  u_xlat1 = (x_464 * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_468 : vec3<f32> = u_xlat1;
  u_xlat1 = exp2(x_468);
  let x_470 : vec3<f32> = u_xlat0;
  u_xlat2 = (x_470 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));

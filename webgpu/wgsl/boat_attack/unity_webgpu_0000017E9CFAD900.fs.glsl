...
layout(location = 0) in highp  vec3 vs_CLIP_POSITION_NO_JITTER0;
layout(location = 1) in highp  vec3 vs_PREVIOUS_CLIP_POSITION_NO_JITTER0;
layout(location = 2) in highp  vec4 vs_INTERP0;
layout(location = 3) in highp  vec3 vs_INTERP1;
...
void main()
{
    ...
    u_xlat2 = texture(sampler2D(Texture2D_B222E8F, samplerTexture2D_B222E8F), vs_INTERP0.xy, _GlobalMipBias.x).w;
    ...
    u_xlat4 = float(1.0) / float(vs_CLIP_POSITION_NO_JITTER0.z);
    u_xlat0.xy = vs_CLIP_POSITION_NO_JITTER0.xy * vec2(u_xlat4) + (-u_xlat0.xy);
    u_xlat0.xy = u_xlat0.xy * vec2(0.5, -0.5);
    u_xlatb4 = unity_MotionVectorsParams.y!=0.0;
    SV_Target0.xy = bool(u_xlatb4) ? u_xlat0.xy : vec2(0.0, 0.0);

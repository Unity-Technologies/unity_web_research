...
fn main_1() {
  ...
  let x_800 : vec3<f32> = sqrt(vec3<f32>(x_798.x, x_798.y, x_798.z));
  let x_801 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_800.x, x_800.y, x_800.z, x_801.w);
  let x_803 : vec2<f32> = vs_TEXCOORD0;
  let x_806 : vec4<f32> = x_25.x_Dithering_Params;
  let x_810 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_823 : f32 = x_25.x_GlobalMipBias.x;
  let x_824 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_820.x, x_820.y), x_823);
  u_xlat15 = x_824.w;
  let x_826 : f32 = u_xlat15;
  u_xlat15 = ((x_826 * 2.0f) + -1.0f);
  let x_830 : f32 = u_xlat15;
  u_xlatb1.x = (x_830 >= 0.0f);
  let x_834 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_834);
  let x_837 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_837)) + 1.0f);
  ...
  u_xlat15 = (x_846 * x_848);
  let x_850 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_850.x, x_850.y, x_850.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_860 : vec3<f32> = (vec3<f32>(x_858.x, x_858.y, x_858.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_871 : vec3<f32> = ((vec3<f32>(x_868.x, x_868.y, x_868.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_872 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_871.x, x_871.y, x_871.z, x_872.w);
  let x_875 : vec4<f32> = u_xlat0;
  let x_877 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_875.x, x_875.y, x_875.z, x_875.x));
  ...
  let x_913 : f32 = x_905;
  u_xlat0.z = x_913;
  let x_915 : f32 = u_xlat15;
  let x_920 : vec4<f32> = u_xlat0;
  let x_922 : vec3<f32> = ((vec3<f32>(x_915, x_915, x_915) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_920.x, x_920.y, x_920.z));
  let x_923 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_922.x, x_922.y, x_922.z, x_923.w);
  let x_925 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_925.x, x_925.y, x_925.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_928 : vec4<f32> = u_xlat0;
  let x_930 : vec3<f32> = (vec3<f32>(x_928.x, x_928.y, x_928.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_931 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_930.x, x_930.y, x_930.z, x_931.w);
  let x_933 : vec4<f32> = u_xlat2;
  let x_935 : vec3<f32> = (vec3<f32>(x_933.x, x_933.y, x_933.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_936 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_935.x, x_935.y, x_935.z, x_936.w);
  let x_938 : vec4<f32> = u_xlat2;
  let x_941 : vec3<f32> = log2(abs(vec3<f32>(x_938.x, x_938.y, x_938.z)));
  let x_942 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_941.x, x_941.y, x_941.z, x_942.w);
  let x_944 : vec4<f32> = u_xlat2;
  let x_946 : vec3<f32> = (vec3<f32>(x_944.x, x_944.y, x_944.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_956 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_954.x, x_954.y, x_954.z, x_954.x));
  u_xlatb0 = vec3<bool>(x_956.x, x_956.y, x_956.z);
  ...
  let x_980 : f32 = x_972;
  u_xlat0.y = x_980;
  let x_983 : bool = u_xlatb0.z;
  if (x_983) {
    let x_988 : f32 = u_xlat1.z;
    x_984 = x_988;
  } else {
  ...
  let x_999 : vec3<f32> = max(vec3<f32>(x_996.x, x_996.y, x_996.z), vec3<f32>(0.0f, 0.0f, 0.0f));

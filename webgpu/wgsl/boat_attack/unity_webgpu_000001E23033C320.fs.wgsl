...
fn main_1() {
  ...
  let x_690 : vec4<f32> = x_40.x_Grain_TilingParams;
  let x_694 : vec4<f32> = x_40.x_Grain_TilingParams;
  ...
  let x_708 : vec4<f32> = textureSampleBias(x_Grain_Texture, sampler_LinearRepeat, vec2<f32>(x_704.x, x_704.y), x_707);
  u_xlat15 = x_708.w;
  ...
  u_xlat1.x = dot(vec3<f32>(x_716.x, x_716.y, x_716.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_725 : f32 = u_xlat1.x;
  u_xlat1.x = sqrt(x_725);
  let x_730 : f32 = x_40.x_Grain_Params.y;
  let x_732 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_730 * -(x_732)) + 1.0f);
  let x_738 : f32 = u_xlat15;
  let x_740 : vec4<f32> = u_xlat0;
  u_xlat6 = (vec3<f32>(x_738, x_738, x_738) * vec3<f32>(x_740.x, x_740.y, x_740.z));
  let x_743 : vec3<f32> = u_xlat6;
  let x_746 : vec2<f32> = x_40.x_Grain_Params;
  u_xlat6 = (x_743 * vec3<f32>(x_746.x, x_746.x, x_746.x));
  let x_749 : vec3<f32> = u_xlat6;
  let x_750 : vec3<f32> = u_xlat1;
  let x_753 : vec4<f32> = u_xlat0;
  let x_755 : vec3<f32> = ((x_749 * vec3<f32>(x_750.x, x_750.x, x_750.x)) + vec3<f32>(x_753.x, x_753.y, x_753.z));
  let x_756 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_755.x, x_755.y, x_755.z, x_756.w);
  let x_758 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_758.x, x_758.y, x_758.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_769 : vec3<f32> = (vec3<f32>(x_767.x, x_767.y, x_767.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_780 : vec3<f32> = ((vec3<f32>(x_777.x, x_777.y, x_777.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_781 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_780.x, x_780.y, x_780.z, x_781.w);
  let x_784 : vec4<f32> = u_xlat0;
  let x_786 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_784.x, x_784.y, x_784.z, x_784.x));

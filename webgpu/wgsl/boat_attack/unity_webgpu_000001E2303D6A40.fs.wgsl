...
fn main_1() {
  ...
  let x_699 : vec2<f32> = vs_TEXCOORD0;
  let x_702 : vec4<f32> = x_25.x_Dithering_Params;
  let x_706 : vec4<f32> = x_25.x_Dithering_Params;
  let x_708 : vec2<f32> = ((x_699 * vec2<f32>(x_702.x, x_702.y)) + vec2<f32>(x_706.z, x_706.w));
  let x_709 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_708.x, x_708.y, x_709.z);
  let x_716 : vec3<f32> = u_xlat1;
  let x_719 : f32 = x_25.x_GlobalMipBias.x;
  let x_720 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_716.x, x_716.y), x_719);
  u_xlat15 = x_720.w;
  let x_722 : f32 = u_xlat15;
  u_xlat15 = ((x_722 * 2.0f) + -1.0f);
  let x_726 : f32 = u_xlat15;
  u_xlatb1.x = (x_726 >= 0.0f);
  let x_730 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_730);
  let x_733 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_733)) + 1.0f);
  ...
  u_xlat15 = (x_742 * x_744);
  let x_746 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_746.x, x_746.y, x_746.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_819 : vec3<f32> = ((vec3<f32>(x_812, x_812, x_812) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_817.x, x_817.y, x_817.z));
  let x_820 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_819.x, x_819.y, x_819.z, x_820.w);
  let x_822 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_822.x, x_822.y, x_822.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_825 : vec4<f32> = u_xlat0;
  let x_827 : vec3<f32> = (vec3<f32>(x_825.x, x_825.y, x_825.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_828 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_827.x, x_827.y, x_827.z, x_828.w);
  let x_830 : vec4<f32> = u_xlat2;
  let x_832 : vec3<f32> = (vec3<f32>(x_830.x, x_830.y, x_830.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_843 : vec3<f32> = (vec3<f32>(x_841.x, x_841.y, x_841.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_853 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_851.x, x_851.y, x_851.z, x_851.x));

...
fn main_1() {
  ...
  let x_630 : vec4<f32> = x_40.x_Dithering_Params;
  let x_634 : vec4<f32> = x_40.x_Dithering_Params;
  ...
  let x_648 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_644.x, x_644.y), x_647);
  u_xlat15 = x_648.w;
  let x_650 : f32 = u_xlat15;
  u_xlat15 = ((x_650 * 2.0f) + -1.0f);
  let x_653 : f32 = u_xlat15;
  u_xlatb1.x = (x_653 >= 0.0f);
  let x_657 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_657);
  let x_660 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_660)) + 1.0f);
  ...
  u_xlat15 = (x_669 * x_671);
  let x_673 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_673.x, x_673.y, x_673.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_676 : vec4<f32> = u_xlat0;
  let x_679 : vec3<f32> = log2(abs(vec3<f32>(x_676.x, x_676.y, x_676.z)));
  let x_680 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_679.x, x_679.y, x_679.z, x_680.w);
  let x_682 : vec4<f32> = u_xlat2;
  let x_684 : vec3<f32> = (vec3<f32>(x_682.x, x_682.y, x_682.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_685 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_684.x, x_684.y, x_684.z, x_685.w);
  let x_687 : vec4<f32> = u_xlat2;
  let x_689 : vec3<f32> = exp2(vec3<f32>(x_687.x, x_687.y, x_687.z));
  let x_690 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_689.x, x_689.y, x_689.z, x_690.w);
  let x_692 : vec4<f32> = u_xlat2;
  let x_695 : vec3<f32> = ((vec3<f32>(x_692.x, x_692.y, x_692.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_696 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_695.x, x_695.y, x_695.z, x_696.w);
  let x_699 : vec4<f32> = u_xlat0;
  let x_701 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_699.x, x_699.y, x_699.z, x_699.x));
  u_xlatb0 = vec3<bool>(x_701.x, x_701.y, x_701.z);
  let x_704 : bool = u_xlatb0.x;
  if (x_704) {
  ...
  let x_713 : f32 = x_705;
  u_xlat0.x = x_713;
  let x_716 : bool = u_xlatb0.y;
  if (x_716) {
    let x_721 : f32 = u_xlat1.y;
    x_717 = x_721;
  } else {
    let x_724 : f32 = u_xlat2.y;
    x_717 = x_724;
  }
  let x_725 : f32 = x_717;
  u_xlat0.y = x_725;
  let x_728 : bool = u_xlatb0.z;
  if (x_728) {
    let x_733 : f32 = u_xlat1.z;
    x_729 = x_733;
  } else {
    let x_736 : f32 = u_xlat2.z;
    x_729 = x_736;
  }
  let x_737 : f32 = x_729;
  u_xlat0.z = x_737;
  ...
  let x_746 : vec3<f32> = ((vec3<f32>(x_739, x_739, x_739) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_744.x, x_744.y, x_744.z));
  let x_747 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_746.x, x_746.y, x_746.z, x_747.w);
  let x_749 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_749.x, x_749.y, x_749.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));

...
fn main_1() {
  ...
  let x_662 : vec3<f32> = ((x_656 * vec3<f32>(x_657.x, x_657.x, x_657.x)) + vec3<f32>(x_660.x, x_660.y, x_660.z));
  let x_663 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_662.x, x_662.y, x_662.z, x_663.w);
  let x_665 : vec4<f32> = u_xlat0;
  let x_667 : vec3<f32> = (vec3<f32>(x_665.x, x_665.y, x_665.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_678 : vec3<f32> = (vec3<f32>(x_676.x, x_676.y, x_676.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_689 : vec3<f32> = ((vec3<f32>(x_686.x, x_686.y, x_686.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_690 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_689.x, x_689.y, x_689.z, x_690.w);
  let x_693 : vec4<f32> = u_xlat0;
  let x_695 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_693.x, x_693.y, x_693.z, x_693.x));
  ...
  let x_731 : f32 = x_723;
  u_xlat0.z = x_731;
  let x_733 : vec2<f32> = vs_TEXCOORD0;
  let x_736 : vec4<f32> = x_13.x_Dithering_Params;
  let x_740 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_753 : f32 = x_13.x_GlobalMipBias.x;
  let x_754 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_750.x, x_750.y), x_753);
  u_xlat15 = x_754.w;
  let x_756 : f32 = u_xlat15;
  u_xlat15 = ((x_756 * 2.0f) + -1.0f);
  let x_761 : f32 = u_xlat15;
  u_xlatb1.x = (x_761 >= 0.0f);
  let x_765 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_765);
  let x_768 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_768)) + 1.0f);
  ...
  u_xlat15 = (x_777 * x_779);
  let x_781 : vec4<f32> = u_xlat0;
  let x_783 : vec3<f32> = (vec3<f32>(x_781.x, x_781.y, x_781.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_784 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_783.x, x_783.y, x_783.z, x_784.w);
  let x_786 : vec4<f32> = u_xlat0;
  let x_789 : vec3<f32> = log2(abs(vec3<f32>(x_786.x, x_786.y, x_786.z)));
  let x_790 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_789.x, x_789.y, x_789.z, x_790.w);
  let x_792 : vec4<f32> = u_xlat2;
  let x_794 : vec3<f32> = (vec3<f32>(x_792.x, x_792.y, x_792.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_795 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_794.x, x_794.y, x_794.z, x_795.w);
  let x_797 : vec4<f32> = u_xlat2;
  let x_799 : vec3<f32> = exp2(vec3<f32>(x_797.x, x_797.y, x_797.z));
  let x_800 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_799.x, x_799.y, x_799.z, x_800.w);
  let x_802 : vec4<f32> = u_xlat2;
  let x_805 : vec3<f32> = ((vec3<f32>(x_802.x, x_802.y, x_802.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_806 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_805.x, x_805.y, x_805.z, x_806.w);
  let x_808 : vec4<f32> = u_xlat0;
  let x_810 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_808.x, x_808.y, x_808.z, x_808.x));
  ...
  let x_855 : vec3<f32> = ((vec3<f32>(x_848, x_848, x_848) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_853.x, x_853.y, x_853.z));
  let x_856 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_855.x, x_855.y, x_855.z, x_856.w);
  let x_858 : vec4<f32> = u_xlat0;
  let x_860 : vec3<f32> = (vec3<f32>(x_858.x, x_858.y, x_858.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_861 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_860.x, x_860.y, x_860.z, x_861.w);
  let x_863 : vec4<f32> = u_xlat0;
  let x_865 : vec3<f32> = (vec3<f32>(x_863.x, x_863.y, x_863.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_866 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_865.x, x_865.y, x_865.z, x_866.w);
  let x_868 : vec4<f32> = u_xlat2;
  let x_870 : vec3<f32> = (vec3<f32>(x_868.x, x_868.y, x_868.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_871 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_870.x, x_870.y, x_870.z, x_871.w);
  let x_873 : vec4<f32> = u_xlat2;
  let x_876 : vec3<f32> = log2(abs(vec3<f32>(x_873.x, x_873.y, x_873.z)));
  let x_877 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_876.x, x_876.y, x_876.z, x_877.w);
  let x_879 : vec4<f32> = u_xlat2;
  let x_881 : vec3<f32> = (vec3<f32>(x_879.x, x_879.y, x_879.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  let x_882 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_881.x, x_881.y, x_881.z, x_882.w);
  let x_884 : vec4<f32> = u_xlat2;
  let x_886 : vec3<f32> = exp2(vec3<f32>(x_884.x, x_884.y, x_884.z));
  let x_887 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_886.x, x_886.y, x_886.z, x_887.w);
  let x_889 : vec4<f32> = u_xlat0;
  let x_891 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_889.x, x_889.y, x_889.z, x_889.x));
  u_xlatb0 = vec3<bool>(x_891.x, x_891.y, x_891.z);
  let x_894 : bool = u_xlatb0.x;
  if (x_894) {
  ...
  let x_903 : f32 = x_895;
  u_xlat0.x = x_903;
  let x_906 : bool = u_xlatb0.y;
  if (x_906) {
    let x_911 : f32 = u_xlat1.y;
    x_907 = x_911;
  } else {
    let x_914 : f32 = u_xlat2.y;
    x_907 = x_914;
  }
  let x_915 : f32 = x_907;
  u_xlat0.y = x_915;
  let x_918 : bool = u_xlatb0.z;
  if (x_918) {
    let x_923 : f32 = u_xlat1.z;
    x_919 = x_923;
  } else {

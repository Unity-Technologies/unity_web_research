...
fn main_1() {
  ...
  vs_INTERP1 = (vec3<f32>(x_123, x_123, x_123) * x_125);
  let x_129 : vec4<f32> = in_TANGENT0;
  let x_132 : vec4<f32> = x_21.unity_ObjectToWorld[1i];
  u_xlat0 = (vec3<f32>(x_129.y, x_129.y, x_129.y) * vec3<f32>(x_132.x, x_132.y, x_132.z));
  let x_136 : vec4<f32> = x_21.unity_ObjectToWorld[0i];
  let x_138 : vec4<f32> = in_TANGENT0;
  let x_141 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_136.x, x_136.y, x_136.z) * vec3<f32>(x_138.x, x_138.x, x_138.x)) + x_141);
  let x_144 : vec4<f32> = x_21.unity_ObjectToWorld[2i];
  let x_146 : vec4<f32> = in_TANGENT0;
  let x_149 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_144.x, x_144.y, x_144.z) * vec3<f32>(x_146.z, x_146.z, x_146.z)) + x_149);
  let x_151 : vec3<f32> = u_xlat0;
  let x_152 : vec3<f32> = u_xlat0;
  u_xlat6 = dot(x_151, x_152);
  let x_154 : f32 = u_xlat6;
  u_xlat6 = max(x_154, 1.17549435e-37f);
  let x_156 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_156);
  let x_159 : f32 = u_xlat6;
  let x_161 : vec3<f32> = u_xlat0;
  let x_162 : vec3<f32> = (vec3<f32>(x_159, x_159, x_159) * x_161);
  let x_163 : vec4<f32> = vs_INTERP2;
  vs_INTERP2 = vec4<f32>(x_162.x, x_162.y, x_162.z, x_163.w);
  let x_168 : f32 = in_TANGENT0.w;
  vs_INTERP2.w = x_168;
  let x_173 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP3 = x_173;
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_INTERP1, vs_INTERP2, vs_INTERP3);
}

...
fn main_1() {
  ...
  let x_694 : vec3<f32> = sqrt(vec3<f32>(x_692.x, x_692.y, x_692.z));
  let x_695 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_694.x, x_694.y, x_694.z, x_695.w);
  let x_697 : vec2<f32> = vs_TEXCOORD0;
  let x_700 : vec4<f32> = x_38.x_Dithering_Params;
  let x_704 : vec4<f32> = x_38.x_Dithering_Params;
  ...
  let x_717 : f32 = x_38.x_GlobalMipBias.x;
  let x_718 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_714.x, x_714.y), x_717);
  u_xlat15 = x_718.w;
  let x_720 : f32 = u_xlat15;
  u_xlat15 = ((x_720 * 2.0f) + -1.0f);
  let x_723 : f32 = u_xlat15;
  u_xlatb1.x = (x_723 >= 0.0f);
  let x_727 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_727);
  let x_730 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_730)) + 1.0f);
  ...
  u_xlat15 = (x_739 * x_741);
  let x_743 : vec4<f32> = u_xlat0;
  let x_745 : vec3<f32> = (vec3<f32>(x_743.x, x_743.y, x_743.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_755 : vec3<f32> = (vec3<f32>(x_753.x, x_753.y, x_753.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_766 : vec3<f32> = ((vec3<f32>(x_763.x, x_763.y, x_763.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_767 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_766.x, x_766.y, x_766.z, x_767.w);
  let x_770 : vec4<f32> = u_xlat0;
  let x_772 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_770.x, x_770.y, x_770.z, x_770.x));
  ...
  let x_817 : vec3<f32> = ((vec3<f32>(x_810, x_810, x_810) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_815.x, x_815.y, x_815.z));
  let x_818 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_817.x, x_817.y, x_817.z, x_818.w);
  let x_820 : vec4<f32> = u_xlat0;
  let x_822 : vec3<f32> = (vec3<f32>(x_820.x, x_820.y, x_820.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_823 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_822.x, x_822.y, x_822.z, x_823.w);
  let x_825 : vec4<f32> = u_xlat0;
  let x_827 : vec3<f32> = (vec3<f32>(x_825.x, x_825.y, x_825.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_828 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_827.x, x_827.y, x_827.z, x_828.w);
  let x_830 : vec4<f32> = u_xlat2;
  let x_832 : vec3<f32> = (vec3<f32>(x_830.x, x_830.y, x_830.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_833 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_832.x, x_832.y, x_832.z, x_833.w);
  let x_835 : vec4<f32> = u_xlat2;
  let x_838 : vec3<f32> = log2(abs(vec3<f32>(x_835.x, x_835.y, x_835.z)));
  let x_839 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_838.x, x_838.y, x_838.z, x_839.w);
  let x_841 : vec4<f32> = u_xlat2;
  let x_843 : vec3<f32> = (vec3<f32>(x_841.x, x_841.y, x_841.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_853 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_851.x, x_851.y, x_851.z, x_851.x));
  ...
  let x_896 : vec3<f32> = max(vec3<f32>(x_893.x, x_893.y, x_893.z), vec3<f32>(0.0f, 0.0f, 0.0f));

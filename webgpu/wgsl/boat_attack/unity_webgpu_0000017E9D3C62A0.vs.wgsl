...
@group(1) @binding(0) var<uniform> x_196 : VGlobals;
...
fn main_1() {
  ...
  u_xlat8 = vec2<f32>(x_80.x, x_80.y);
  ...
  u_xlat8.x = ((x_109 * 256.0f) + x_113);
  let x_117 : f32 = u_xlat8.x;
  let x_124 : f32 = x_120.x_TerrainHeightmapScale.y;
  u_xlat8.x = (x_117 * x_124);
  let x_128 : f32 = u_xlat8.x;
  u_xlat8.x = (x_128 * 0.00389105058275163174f);
  let x_134 : vec2<f32> = u_xlat8;
  ...
  u_xlat8 = (vec2<f32>(x_146.x, x_146.y) * vec2<f32>(x_149.x, x_149.z));
  ...
  let x_199 : vec3<f32> = x_196.x_LightPosition;
  u_xlat2 = (-(vec3<f32>(x_191.x, x_191.y, x_191.z)) + x_199);
  let x_202 : vec3<f32> = u_xlat2;
  let x_203 : vec3<f32> = u_xlat2;
  u_xlat12 = dot(x_202, x_203);
  let x_205 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_205);
  let x_207 : f32 = u_xlat12;
  let x_209 : vec3<f32> = u_xlat2;
  u_xlat2 = (vec3<f32>(x_207, x_207, x_207) * x_209);
  let x_212 : vec4<f32> = u_xlat1;
  let x_215 : vec4<f32> = x_140.unity_ObjectToWorld[1i];
  u_xlat3 = (vec3<f32>(x_212.y, x_212.y, x_212.y) * vec3<f32>(x_215.x, x_215.y, x_215.z));
  let x_219 : vec4<f32> = x_140.unity_ObjectToWorld[0i];
  let x_221 : vec4<f32> = u_xlat1;
  let x_224 : vec3<f32> = u_xlat3;
  let x_225 : vec3<f32> = ((vec3<f32>(x_219.x, x_219.y, x_219.z) * vec3<f32>(x_221.x, x_221.x, x_221.x)) + x_224);
  let x_226 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_225.x, x_225.y, x_226.z, x_225.z);
  let x_229 : vec4<f32> = x_140.unity_ObjectToWorld[2i];
  let x_231 : vec4<f32> = u_xlat1;
  let x_234 : vec4<f32> = u_xlat1;
  let x_236 : vec3<f32> = ((vec3<f32>(x_229.x, x_229.y, x_229.z) * vec3<f32>(x_231.z, x_231.z, x_231.z)) + vec3<f32>(x_234.x, x_234.y, x_234.w));
  let x_237 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_236.x, x_236.y, x_236.z, x_237.w);
  let x_239 : vec4<f32> = u_xlat1;
  let x_241 : vec4<f32> = u_xlat1;
  u_xlat12 = dot(vec3<f32>(x_239.x, x_239.y, x_239.z), vec3<f32>(x_241.x, x_241.y, x_241.z));
  let x_244 : f32 = u_xlat12;
  u_xlat12 = max(x_244, 1.17549435e-38f);
  let x_247 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_247);
  ...
  u_xlat12 = dot(x_256, vec3<f32>(x_257.x, x_257.y, x_257.z));
  let x_260 : f32 = u_xlat12;
  u_xlat12 = clamp(x_260, 0.0f, 1.0f);
  let x_264 : vec3<f32> = u_xlat2;
  let x_266 : vec4<f32> = x_196.x_ShadowBias;
  let x_269 : vec4<f32> = u_xlat0;
  let x_271 : vec3<f32> = ((x_264 * vec3<f32>(x_266.x, x_266.x, x_266.x)) + vec3<f32>(x_269.x, x_269.y, x_269.z));
  let x_272 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_271.x, x_271.y, x_271.z, x_272.w);
  let x_274 : f32 = u_xlat12;
  u_xlat12 = (-(x_274) + 1.0f);
  let x_277 : f32 = u_xlat12;
  let x_279 : f32 = x_196.x_ShadowBias.y;
  u_xlat12 = (x_277 * x_279);
  let x_281 : vec4<f32> = u_xlat1;
  let x_283 : f32 = u_xlat12;
  let x_286 : vec4<f32> = u_xlat0;
  let x_288 : vec3<f32> = ((vec3<f32>(x_281.x, x_281.y, x_281.z) * vec3<f32>(x_283, x_283, x_283)) + vec3<f32>(x_286.x, x_286.y, x_286.z));
  let x_289 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_288.x, x_288.y, x_288.z, x_289.w);
  let x_291 : vec4<f32> = u_xlat0;
  let x_294 : vec4<f32> = x_196.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_291.y, x_291.y, x_291.y, x_291.y) * x_294);
  let x_297 : vec4<f32> = x_196.unity_MatrixVP[0i];
  let x_298 : vec4<f32> = u_xlat0;
  let x_301 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_297 * vec4<f32>(x_298.x, x_298.x, x_298.x, x_298.x)) + x_301);
  let x_304 : vec4<f32> = x_196.unity_MatrixVP[2i];
  ...
  let x_312 : vec4<f32> = x_196.unity_MatrixVP[3i];
  u_xlat0 = (x_310 + x_312);
  let x_319 : f32 = u_xlat0.z;
  gl_Position.z = min(x_319, 1.0f);
  let x_323 : vec4<f32> = u_xlat0;
  let x_324 : vec3<f32> = vec3<f32>(x_323.x, x_323.y, x_323.w);
  let x_327 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_324.x, x_324.y, x_327.z, x_324.z);
  return;
}

...
fn main_1() {
  ...
  let x_270 : vec4<f32> = x_142.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_267.y, x_267.y, x_267.y, x_267.y) * x_270);
  let x_273 : vec4<f32> = x_142.unity_MatrixVP[0i];
  let x_274 : vec4<f32> = u_xlat0;
  let x_277 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_273 * vec4<f32>(x_274.x, x_274.x, x_274.x, x_274.x)) + x_277);
  let x_280 : vec4<f32> = x_142.unity_MatrixVP[2i];
  let x_281 : vec4<f32> = u_xlat0;
  let x_284 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_280 * vec4<f32>(x_281.z, x_281.z, x_281.z, x_281.z)) + x_284);
  let x_290 : vec4<f32> = u_xlat0;
  let x_292 : vec4<f32> = x_142.unity_MatrixVP[3i];
  gl_Position = (x_290 + x_292);
  return;
}

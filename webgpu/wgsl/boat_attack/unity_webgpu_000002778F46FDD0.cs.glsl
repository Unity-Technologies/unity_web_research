...
void main()
{
    ...
    u_xlatu4 = floatBitsToUint(u_xlat4) & 31u;
    TGSM0[gl_LocalInvocationIndex].value[(0 >> 2)] = u_xlatu4;
    //memoryBarrierShared;
    barrier();
    u_xlatb1 = lessThan(u_xlatu0.xxxx, uvec4(16u, 8u, 4u, 2u));
    if(u_xlatb1.x){
        u_xlati6 = int(gl_LocalInvocationIndex) + 16;
        u_xlatu6 = TGSM0[u_xlati6].value[(0 >> 2) + 0];
        u_xlatu4 = max(u_xlatu6, u_xlatu4);
        TGSM0[gl_LocalInvocationIndex].value[(0 >> 2)] = u_xlatu4;
    }
    //memoryBarrierShared;
    barrier();
    if(u_xlatb1.y){
        u_xlatu4 = TGSM0[gl_LocalInvocationIndex].value[(0 >> 2) + 0];
    ...
    if(u_xlatb0){
        u_xlatu0.x = TGSM0[gl_LocalInvocationIndex].value[(0 >> 2) + 0];
        u_xlati4 = int(gl_LocalInvocationIndex) + 1;
        u_xlatu4 = TGSM0[u_xlati4].value[(0 >> 2) + 0];
        u_xlatu0.x = max(u_xlatu4, u_xlatu0.x);
        TGSM0[gl_LocalInvocationIndex].value[(0 >> 2)] = u_xlatu0.x;
    }
    //memoryBarrierShared;
    barrier();
    u_xlati0 = int(TGSM0[u_xlatu0.y].value[(0 >> 2) + 0]);
    u_xlatb0 = u_xlati0==31;

...
fn main_1() {
  ...
  let x_669 : vec3<f32> = (vec3<f32>(x_667.x, x_667.y, x_667.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_670 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_669.x, x_669.y, x_669.z, x_670.w);
  let x_672 : vec4<f32> = u_xlat0;
  let x_675 : vec3<f32> = log2(abs(vec3<f32>(x_672.x, x_672.y, x_672.z)));
  let x_676 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_675.x, x_675.y, x_675.z, x_676.w);
  let x_678 : vec4<f32> = u_xlat2;
  let x_680 : vec3<f32> = (vec3<f32>(x_678.x, x_678.y, x_678.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_681 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_680.x, x_680.y, x_680.z, x_681.w);
  let x_683 : vec4<f32> = u_xlat2;
  let x_685 : vec3<f32> = exp2(vec3<f32>(x_683.x, x_683.y, x_683.z));
  let x_686 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_685.x, x_685.y, x_685.z, x_686.w);
  let x_688 : vec4<f32> = u_xlat2;
  let x_691 : vec3<f32> = ((vec3<f32>(x_688.x, x_688.y, x_688.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_692 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_691.x, x_691.y, x_691.z, x_692.w);
  let x_695 : vec4<f32> = u_xlat0;
  let x_697 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_695.x, x_695.y, x_695.z, x_695.x));
  ...
  let x_733 : f32 = x_725;
  u_xlat0.z = x_733;
  let x_735 : vec2<f32> = vs_TEXCOORD0;
  let x_738 : vec4<f32> = x_26.x_Dithering_Params;
  let x_742 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_755 : f32 = x_26.x_GlobalMipBias.x;
  let x_756 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_752.x, x_752.y), x_755);
  u_xlat15 = x_756.w;
  let x_758 : f32 = u_xlat15;
  u_xlat15 = ((x_758 * 2.0f) + -1.0f);
  let x_763 : f32 = u_xlat15;
  u_xlatb1.x = (x_763 >= 0.0f);
  let x_767 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_767);
  let x_770 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_770)) + 1.0f);
  ...
  u_xlat15 = (x_779 * x_781);
  let x_783 : vec4<f32> = u_xlat0;
  let x_785 : vec3<f32> = (vec3<f32>(x_783.x, x_783.y, x_783.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_796 : vec3<f32> = (vec3<f32>(x_794.x, x_794.y, x_794.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_807 : vec3<f32> = ((vec3<f32>(x_804.x, x_804.y, x_804.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_808 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_807.x, x_807.y, x_807.z, x_808.w);
  let x_810 : vec4<f32> = u_xlat0;
  let x_812 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_810.x, x_810.y, x_810.z, x_810.x));
  ...
  let x_862 : vec3<f32> = (vec3<f32>(x_860.x, x_860.y, x_860.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_863 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_862.x, x_862.y, x_862.z, x_863.w);
  let x_865 : vec4<f32> = u_xlat0;
  let x_867 : vec3<f32> = (vec3<f32>(x_865.x, x_865.y, x_865.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_868 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_867.x, x_867.y, x_867.z, x_868.w);
  let x_870 : vec4<f32> = u_xlat2;
  let x_872 : vec3<f32> = (vec3<f32>(x_870.x, x_870.y, x_870.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_883 : vec3<f32> = (vec3<f32>(x_881.x, x_881.y, x_881.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_936 : vec3<f32> = max(vec3<f32>(x_933.x, x_933.y, x_933.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_937 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_936.x, x_936.y, x_936.z, x_937.w);
  SV_Target0.w = 1.0f;
  return;
}

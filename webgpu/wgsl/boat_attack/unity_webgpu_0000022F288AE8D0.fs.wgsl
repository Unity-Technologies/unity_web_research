...
fn main_1() {
  ...
  u_xlat1 = (x_561 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_563 : vec3<f32> = u_xlat0;
  let x_565 : vec3<f32> = log2(abs(x_563));
  let x_566 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_565.x, x_565.y, x_565.z, x_566.w);
  let x_568 : vec4<f32> = u_xlat2;
  let x_570 : vec3<f32> = (vec3<f32>(x_568.x, x_568.y, x_568.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_581 : vec3<f32> = ((vec3<f32>(x_578.x, x_578.y, x_578.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_582 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_581.x, x_581.y, x_581.z, x_582.w);
  let x_585 : vec3<f32> = u_xlat0;
  let x_587 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_585.x, x_585.y, x_585.z, x_585.x));

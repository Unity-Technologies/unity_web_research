...
fn main_1() {
  ...
  u_xlat9 = max(x_79, 1.17549435e-37f);
  ...
  let x_190 : vec4<f32> = x_171.x_CascadeShadowSplitSpheres1;
  ...
  u_xlat1.y = dot(x_194, x_195);
  let x_198 : vec4<f32> = u_xlat0;
  let x_201 : vec4<f32> = x_171.x_CascadeShadowSplitSpheres2;
  u_xlat2 = (vec3<f32>(x_198.x, x_198.y, x_198.z) + -(vec3<f32>(x_201.x, x_201.y, x_201.z)));
  let x_205 : vec3<f32> = u_xlat2;
  let x_206 : vec3<f32> = u_xlat2;
  u_xlat1.z = dot(x_205, x_206);
  let x_209 : vec4<f32> = u_xlat0;
  let x_213 : vec4<f32> = x_171.x_CascadeShadowSplitSpheres3;
  u_xlat2 = (vec3<f32>(x_209.x, x_209.y, x_209.z) + -(vec3<f32>(x_213.x, x_213.y, x_213.z)));
  let x_217 : vec3<f32> = u_xlat2;
  let x_218 : vec3<f32> = u_xlat2;
  u_xlat1.w = dot(x_217, x_218);
  let x_225 : vec4<f32> = u_xlat1;
  let x_227 : vec4<f32> = x_171.x_CascadeShadowSplitSphereRadii;
  u_xlatb1 = (x_225 < x_227);
  let x_231 : bool = u_xlatb1.x;
  u_xlat2.x = select(-0.0f, -1.0f, x_231);
  let x_237 : bool = u_xlatb1.y;
  u_xlat2.y = select(-0.0f, -1.0f, x_237);
  let x_241 : bool = u_xlatb1.z;
  u_xlat2.z = select(-0.0f, -1.0f, x_241);
  let x_245 : bool = u_xlatb1.x;
  u_xlat1.x = select(0.0f, 1.0f, x_245);
  let x_249 : bool = u_xlatb1.y;
  u_xlat1.y = select(0.0f, 1.0f, x_249);
  let x_253 : bool = u_xlatb1.z;
  u_xlat1.z = select(0.0f, 1.0f, x_253);
  let x_257 : bool = u_xlatb1.w;
  u_xlat1.w = select(0.0f, 1.0f, x_257);
  let x_260 : vec3<f32> = u_xlat2;
  let x_261 : vec4<f32> = u_xlat1;
  u_xlat2 = (x_260 + vec3<f32>(x_261.y, x_261.z, x_261.w));
  let x_264 : vec3<f32> = u_xlat2;
  let x_265 : vec3<f32> = max(x_264, vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_266 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_266.x, x_265.x, x_265.y, x_265.z);
  let x_268 : vec4<f32> = u_xlat1;
  u_xlat9 = dot(x_268, vec4<f32>(4.0f, 3.0f, 2.0f, 1.0f));
  let x_274 : f32 = u_xlat9;
  u_xlat9 = (-(x_274) + 4.0f);
  let x_279 : f32 = u_xlat9;
  u_xlatu9 = u32(x_279);
  let x_283 : u32 = u_xlatu9;
  u_xlati9 = (bitcast<i32>(x_283) << bitcast<u32>(2i));
  let x_286 : vec4<f32> = u_xlat0;
  let x_288 : i32 = u_xlati9;
  let x_291 : i32 = u_xlati9;
  let x_295 : vec4<f32> = x_171.x_MainLightWorldToShadow[((x_288 + 1i) / 4i)][((x_291 + 1i) % 4i)];
  ...
  let x_300 : i32 = u_xlati9;
  let x_302 : i32 = u_xlati9;
  let x_305 : vec4<f32> = x_171.x_MainLightWorldToShadow[(x_300 / 4i)][(x_302 % 4i)];
  ...
  let x_315 : i32 = u_xlati9;
  let x_318 : i32 = u_xlati9;
  let x_322 : vec4<f32> = x_171.x_MainLightWorldToShadow[((x_315 + 2i) / 4i)][((x_318 + 2i) % 4i)];
  ...
  let x_335 : i32 = u_xlati9;
  let x_338 : i32 = u_xlati9;
  let x_342 : vec4<f32> = x_171.x_MainLightWorldToShadow[((x_335 + 3i) / 4i)][((x_338 + 3i) % 4i)];
  let x_344 : vec3<f32> = (vec3<f32>(x_333.x, x_333.y, x_333.z) + vec3<f32>(x_342.x, x_342.y, x_342.z));
  let x_345 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_344.x, x_344.y, x_344.z, x_345.w);
  vs_TEXCOORD4.w = 0.0f;
  let x_348 : vec4<f32> = u_xlat0;
  let x_351 : vec4<f32> = x_17.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_348.y, x_348.y, x_348.y, x_348.y) * x_351);
  let x_354 : vec4<f32> = x_17.unity_MatrixVP[0i];
  let x_355 : vec4<f32> = u_xlat0;
  let x_358 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_354 * vec4<f32>(x_355.x, x_355.x, x_355.x, x_355.x)) + x_358);
  let x_361 : vec4<f32> = x_17.unity_MatrixVP[2i];
  let x_362 : vec4<f32> = u_xlat0;
  let x_365 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_361 * vec4<f32>(x_362.z, x_362.z, x_362.z, x_362.z)) + x_365);
  let x_371 : vec4<f32> = u_xlat0;
  let x_373 : vec4<f32> = x_17.unity_MatrixVP[3i];
  gl_Position = (x_371 + x_373);
  return;
}

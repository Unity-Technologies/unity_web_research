...
fn main_1() {
  ...
  let x_690 : vec3<f32> = sqrt(vec3<f32>(x_688.x, x_688.y, x_688.z));
  let x_691 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_690.x, x_690.y, x_690.z, x_691.w);
  let x_693 : vec2<f32> = vs_TEXCOORD0;
  let x_696 : vec4<f32> = x_26.x_Dithering_Params;
  let x_700 : vec4<f32> = x_26.x_Dithering_Params;
  let x_702 : vec2<f32> = ((x_693 * vec2<f32>(x_696.x, x_696.y)) + vec2<f32>(x_700.z, x_700.w));
  let x_703 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_702.x, x_702.y, x_703.z, x_703.w);
  let x_710 : vec4<f32> = u_xlat1;
  let x_713 : f32 = x_26.x_GlobalMipBias.x;
  let x_714 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_710.x, x_710.y), x_713);
  u_xlat15 = x_714.w;
  let x_716 : f32 = u_xlat15;
  u_xlat15 = ((x_716 * 2.0f) + -1.0f);
  let x_721 : f32 = u_xlat15;
  u_xlatb1.x = (x_721 >= 0.0f);
  let x_725 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_725);
  let x_728 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_728)) + 1.0f);
  ...
  u_xlat15 = (x_737 * x_739);
  let x_741 : vec4<f32> = u_xlat0;
  let x_743 : vec3<f32> = (vec3<f32>(x_741.x, x_741.y, x_741.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_753 : vec3<f32> = (vec3<f32>(x_751.x, x_751.y, x_751.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_764 : vec3<f32> = ((vec3<f32>(x_761.x, x_761.y, x_761.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_765 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_764.x, x_764.y, x_764.z, x_765.w);
  let x_768 : vec4<f32> = u_xlat0;
  let x_770 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_768.x, x_768.y, x_768.z, x_768.x));
  u_xlatb0 = vec3<bool>(x_770.x, x_770.y, x_770.z);
  let x_773 : bool = u_xlatb0.x;
  if (x_773) {
  ...
  let x_815 : vec3<f32> = ((vec3<f32>(x_808, x_808, x_808) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_813.x, x_813.y, x_813.z));
  let x_816 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_815.x, x_815.y, x_815.z, x_816.w);
  let x_818 : vec4<f32> = u_xlat0;
  let x_820 : vec3<f32> = (vec3<f32>(x_818.x, x_818.y, x_818.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_821 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_820.x, x_820.y, x_820.z, x_821.w);
  let x_823 : vec4<f32> = u_xlat0;
  let x_825 : vec3<f32> = (vec3<f32>(x_823.x, x_823.y, x_823.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_826 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_825.x, x_825.y, x_825.z, x_826.w);
  let x_828 : vec4<f32> = u_xlat2;
  let x_830 : vec3<f32> = (vec3<f32>(x_828.x, x_828.y, x_828.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_831 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_830.x, x_830.y, x_830.z, x_831.w);
  let x_833 : vec4<f32> = u_xlat2;
  let x_836 : vec3<f32> = log2(abs(vec3<f32>(x_833.x, x_833.y, x_833.z)));
  let x_837 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_836.x, x_836.y, x_836.z, x_837.w);
  let x_839 : vec4<f32> = u_xlat2;
  let x_841 : vec3<f32> = (vec3<f32>(x_839.x, x_839.y, x_839.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_842 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_841.x, x_841.y, x_841.z, x_842.w);
  let x_844 : vec4<f32> = u_xlat2;
  let x_846 : vec3<f32> = exp2(vec3<f32>(x_844.x, x_844.y, x_844.z));
  let x_847 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_846.x, x_846.y, x_846.z, x_847.w);
  let x_849 : vec4<f32> = u_xlat0;
  let x_851 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_849.x, x_849.y, x_849.z, x_849.x));
  ...
  let x_894 : vec3<f32> = max(vec3<f32>(x_891.x, x_891.y, x_891.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_895 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_894.x, x_894.y, x_894.z, x_895.w);
  SV_Target0.w = 1.0f;
  return;
}

...
fn main_1() {
  ...
  let x_619 : vec3<f32> = (vec3<f32>(x_617.x, x_617.y, x_617.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_620 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_619.x, x_619.y, x_619.z, x_620.w);
  let x_622 : vec4<f32> = u_xlat0;
  let x_625 : vec3<f32> = log2(abs(vec3<f32>(x_622.x, x_622.y, x_622.z)));
  let x_626 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_625.x, x_625.y, x_625.z, x_626.w);
  let x_628 : vec4<f32> = u_xlat2;
  let x_630 : vec3<f32> = (vec3<f32>(x_628.x, x_628.y, x_628.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_641 : vec3<f32> = ((vec3<f32>(x_638.x, x_638.y, x_638.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_642 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_641.x, x_641.y, x_641.z, x_642.w);
  let x_645 : vec4<f32> = u_xlat0;
  let x_647 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_645.x, x_645.y, x_645.z, x_645.x));

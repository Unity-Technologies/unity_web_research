...
fn main_1() {
  ...
  let x_667 : vec4<f32> = x_27.x_Dithering_Params;
  let x_671 : vec4<f32> = x_27.x_Dithering_Params;
  ...
  let x_684 : f32 = x_27.x_GlobalMipBias.x;
  ...
  let x_713 : vec3<f32> = (x_712 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_714 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_713.x, x_713.y, x_713.z, x_714.w);
  let x_716 : vec3<f32> = u_xlat0;
  let x_718 : vec3<f32> = log2(abs(x_716));
  let x_719 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_718.x, x_718.y, x_718.z, x_719.w);
  let x_721 : vec4<f32> = u_xlat2;
  let x_723 : vec3<f32> = (vec3<f32>(x_721.x, x_721.y, x_721.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_734 : vec3<f32> = ((vec3<f32>(x_731.x, x_731.y, x_731.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_735 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_734.x, x_734.y, x_734.z, x_735.w);
  let x_738 : vec3<f32> = u_xlat0;
  let x_740 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_738.x, x_738.y, x_738.z, x_738.x));
  ...
  u_xlat0 = ((vec3<f32>(x_778, x_778, x_778) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_783);
  let x_785 : vec3<f32> = u_xlat0;
  let x_786 : vec3<f32> = (x_785 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_787 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_786.x, x_786.y, x_786.z, x_787.w);
  let x_789 : vec3<f32> = u_xlat0;
  let x_790 : vec3<f32> = (x_789 + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_791 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_790.x, x_790.y, x_790.z, x_791.w);
  let x_793 : vec4<f32> = u_xlat2;
  let x_795 : vec3<f32> = (vec3<f32>(x_793.x, x_793.y, x_793.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_796 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_795.x, x_795.y, x_795.z, x_796.w);
  let x_798 : vec4<f32> = u_xlat2;
  let x_801 : vec3<f32> = log2(abs(vec3<f32>(x_798.x, x_798.y, x_798.z)));
  let x_802 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_801.x, x_801.y, x_801.z, x_802.w);
  let x_804 : vec4<f32> = u_xlat2;
  let x_806 : vec3<f32> = (vec3<f32>(x_804.x, x_804.y, x_804.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  if (x_843) {
    ...
    let x_851 : f32 = u_xlat2.z;
    x_844 = x_851;
  }
  let x_852 : f32 = x_844;
  u_xlat0.z = x_852;
  let x_856 : vec3<f32> = u_xlat0;
  let x_857 : vec3<f32> = max(x_856, vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_858 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_857.x, x_857.y, x_857.z, x_858.w);
  SV_Target0.w = 1.0f;
  return;
}

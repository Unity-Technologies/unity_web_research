...
@group(1) @binding(7) var<uniform> x_394 : VGlobals;
...
@group(1) @binding(3) var<uniform> x_511 : MainLightShadows;
...
fn main_1() {
  ...
  vs_INTERP0 = vec3<f32>(x_423.x, x_423.y, x_423.z);
  ...
  vs_INTERP4 = x_481;
  let x_485 : vec4<f32> = in_TEXCOORD1;
  let x_489 : vec4<f32> = x_16.unity_LightmapST;
  let x_493 : vec4<f32> = x_16.unity_LightmapST;
  vs_INTERP5 = ((vec2<f32>(x_485.x, x_485.y) * vec2<f32>(x_489.x, x_489.y)) + vec2<f32>(x_493.z, x_493.w));
  ...
  let x_513 : vec4<f32> = x_511.x_MainLightWorldToShadow[0i][1i];
  let x_515 : vec3<f32> = (vec3<f32>(x_505.y, x_505.y, x_505.y) * vec3<f32>(x_513.x, x_513.y, x_513.z));
  let x_516 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_515.x, x_515.y, x_515.z, x_516.w);
  let x_519 : vec4<f32> = x_511.x_MainLightWorldToShadow[0i][0i];
  ...
  u_xlat0 = vec4<f32>(x_526.x, x_526.y, x_527.z, x_526.z);
  let x_530 : vec4<f32> = x_511.x_MainLightWorldToShadow[0i][2i];
  let x_532 : vec4<f32> = u_xlat0;
  let x_535 : vec4<f32> = u_xlat0;
  let x_537 : vec3<f32> = ((vec3<f32>(x_530.x, x_530.y, x_530.z) * vec3<f32>(x_532.z, x_532.z, x_532.z)) + vec3<f32>(x_535.x, x_535.y, x_535.w));
  let x_538 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_537.x, x_537.y, x_537.z, x_538.w);
  let x_541 : vec4<f32> = u_xlat0;
  let x_544 : vec4<f32> = x_511.x_MainLightWorldToShadow[0i][3i];
  let x_546 : vec3<f32> = (vec3<f32>(x_541.x, x_541.y, x_541.z) + vec3<f32>(x_544.x, x_544.y, x_544.z));
  let x_547 : vec4<f32> = vs_INTERP9;
  vs_INTERP9 = vec4<f32>(x_546.x, x_546.y, x_546.z, x_547.w);
  vs_INTERP9.w = 0.0f;
  return;
}

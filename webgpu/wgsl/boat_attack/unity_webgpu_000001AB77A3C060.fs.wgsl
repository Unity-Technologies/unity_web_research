...
fn main_1() {
  ...
  let x_759 : vec2<f32> = vs_TEXCOORD0;
  let x_762 : vec4<f32> = x_25.x_Dithering_Params;
  let x_766 : vec4<f32> = x_25.x_Dithering_Params;
  let x_768 : vec2<f32> = ((x_759 * vec2<f32>(x_762.x, x_762.y)) + vec2<f32>(x_766.z, x_766.w));
  let x_769 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_768.x, x_768.y, x_769.z);
  let x_776 : vec3<f32> = u_xlat1;
  let x_779 : f32 = x_25.x_GlobalMipBias.x;
  let x_780 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_776.x, x_776.y), x_779);
  u_xlat15 = x_780.w;
  let x_782 : f32 = u_xlat15;
  u_xlat15 = ((x_782 * 2.0f) + -1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_806.x, x_806.y, x_806.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_809 : vec4<f32> = u_xlat0;
  let x_812 : vec3<f32> = log2(abs(vec3<f32>(x_809.x, x_809.y, x_809.z)));
  let x_813 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_812.x, x_812.y, x_812.z, x_813.w);
  let x_815 : vec4<f32> = u_xlat2;
  let x_817 : vec3<f32> = (vec3<f32>(x_815.x, x_815.y, x_815.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  let x_818 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_817.x, x_817.y, x_817.z, x_818.w);
  let x_820 : vec4<f32> = u_xlat2;
  let x_822 : vec3<f32> = exp2(vec3<f32>(x_820.x, x_820.y, x_820.z));
  let x_823 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_822.x, x_822.y, x_822.z, x_823.w);
  let x_825 : vec4<f32> = u_xlat2;
  let x_828 : vec3<f32> = ((vec3<f32>(x_825.x, x_825.y, x_825.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_829 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_828.x, x_828.y, x_828.z, x_829.w);
  let x_832 : vec4<f32> = u_xlat0;
  let x_834 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_832.x, x_832.y, x_832.z, x_832.x));
  u_xlatb0 = vec3<bool>(x_834.x, x_834.y, x_834.z);
  let x_837 : bool = u_xlatb0.x;
  if (x_837) {
  ...
  let x_846 : f32 = x_838;
  u_xlat0.x = x_846;
  let x_849 : bool = u_xlatb0.y;
  if (x_849) {
    let x_854 : f32 = u_xlat1.y;
    x_850 = x_854;
  } else {
  ...
  let x_858 : f32 = x_850;
  u_xlat0.y = x_858;
  let x_861 : bool = u_xlatb0.z;
  if (x_861) {
    let x_866 : f32 = u_xlat1.z;
    x_862 = x_866;
  } else {
  ...
  let x_879 : vec3<f32> = ((vec3<f32>(x_872, x_872, x_872) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_877.x, x_877.y, x_877.z));
  let x_880 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_879.x, x_879.y, x_879.z, x_880.w);
  let x_882 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_882.x, x_882.y, x_882.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_885 : vec4<f32> = u_xlat0;
  let x_887 : vec3<f32> = (vec3<f32>(x_885.x, x_885.y, x_885.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_888 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_887.x, x_887.y, x_887.z, x_888.w);
  let x_890 : vec4<f32> = u_xlat2;
  let x_892 : vec3<f32> = (vec3<f32>(x_890.x, x_890.y, x_890.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  ...
  let x_903 : vec3<f32> = (vec3<f32>(x_901.x, x_901.y, x_901.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_913 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_911.x, x_911.y, x_911.z, x_911.x));

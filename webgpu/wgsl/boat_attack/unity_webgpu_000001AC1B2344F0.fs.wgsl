...
fn main_1() {
  ...
  let x_110 : f32 = vs_PREV_POSITION_CS_NO_JITTER0.w;
  u_xlat0.x = (1.0f / x_110);
  let x_113 : vec2<f32> = u_xlat0;
  let x_115 : vec4<f32> = vs_PREV_POSITION_CS_NO_JITTER0;
  u_xlat0 = (vec2<f32>(x_113.x, x_113.x) * vec2<f32>(x_115.x, x_115.y));
  let x_121 : f32 = vs_POSITION_CS_NO_JITTER0.w;
  u_xlat2 = (1.0f / x_121);
  let x_123 : vec4<f32> = vs_POSITION_CS_NO_JITTER0;
  let x_125 : f32 = u_xlat2;
  let x_128 : vec2<f32> = u_xlat0;
  u_xlat0 = ((vec2<f32>(x_123.x, x_123.y) * vec2<f32>(x_125, x_125)) + -(x_128));
  let x_131 : vec2<f32> = u_xlat0;
  u_xlat0 = (x_131 * vec2<f32>(0.5f, -0.5f));
  let x_140 : f32 = x_68.unity_MotionVectorsParams.y;
  u_xlatb2 = !((x_140 == 0.0f));
  let x_144 : bool = u_xlatb2;
  if (x_144) {
    let x_149 : vec2<f32> = u_xlat0;
    x_146 = x_149;
  } else {
    x_146 = vec2<f32>(0.0f, 0.0f);
  }
  let x_152 : vec2<f32> = x_146;
  let x_153 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_152.x, x_152.y, x_153.z, x_153.w);
  let x_155 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_155.x, x_155.y, vec2<f32>(0.0f, 0.0f).x, vec2<f32>(0.0f, 0.0f).y);
  return;
}
...
@fragment
fn main(@builtin(position) gl_FragCoord_param : vec4<f32>, @location(1) vs_PREV_POSITION_CS_NO_JITTER0_param : vec4<f32>, @location(0) vs_POSITION_CS_NO_JITTER0_param : vec4<f32>) -> main_out {

...
fn main_1() {
  ...
  let x_884 : vec4<f32> = x_13.x_Dithering_Params;
  let x_888 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_902 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_898.x, x_898.y), x_901);
  u_xlat18 = x_902.w;
  let x_904 : f32 = u_xlat18;
  u_xlat18 = ((x_904 * 2.0f) + -1.0f);
  let x_908 : f32 = u_xlat18;
  u_xlatb1.x = (x_908 >= 0.0f);
  let x_912 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_912);
  let x_915 : f32 = u_xlat18;
  u_xlat18 = (-(abs(x_915)) + 1.0f);
  ...
  u_xlat18 = (x_924 * x_926);
  let x_928 : vec4<f32> = u_xlat0;
  let x_930 : vec3<f32> = (vec3<f32>(x_928.x, x_928.y, x_928.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_931 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_930.x, x_930.y, x_930.z, x_931.w);
  let x_933 : vec4<f32> = u_xlat0;
  let x_936 : vec3<f32> = log2(abs(vec3<f32>(x_933.x, x_933.y, x_933.z)));
  let x_937 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_936.x, x_936.y, x_936.z, x_937.w);
  let x_939 : vec4<f32> = u_xlat2;
  let x_941 : vec3<f32> = (vec3<f32>(x_939.x, x_939.y, x_939.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_952 : vec3<f32> = ((vec3<f32>(x_949.x, x_949.y, x_949.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_953 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_952.x, x_952.y, x_952.z, x_953.w);
  let x_956 : vec4<f32> = u_xlat0;
  let x_958 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_956.x, x_956.y, x_956.z, x_956.x));
  u_xlatb0 = vec3<bool>(x_958.x, x_958.y, x_958.z);
  let x_961 : bool = u_xlatb0.x;
  if (x_961) {
  ...
  let x_982 : f32 = x_974;
  u_xlat0.y = x_982;
  let x_985 : bool = u_xlatb0.z;
  if (x_985) {
    let x_990 : f32 = u_xlat1.z;
    x_986 = x_990;
  } else {
  ...
  let x_1003 : vec3<f32> = ((vec3<f32>(x_996, x_996, x_996) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1001.x, x_1001.y, x_1001.z));
  let x_1004 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1003.x, x_1003.y, x_1003.z, x_1004.w);
  let x_1006 : vec4<f32> = u_xlat0;
  let x_1008 : vec3<f32> = (vec3<f32>(x_1006.x, x_1006.y, x_1006.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1009 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1008.x, x_1008.y, x_1008.z, x_1009.w);
  let x_1011 : vec4<f32> = u_xlat0;
  let x_1013 : vec3<f32> = (vec3<f32>(x_1011.x, x_1011.y, x_1011.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1014 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1013.x, x_1013.y, x_1013.z, x_1014.w);
  let x_1016 : vec4<f32> = u_xlat2;
  let x_1018 : vec3<f32> = (vec3<f32>(x_1016.x, x_1016.y, x_1016.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1019 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1018.x, x_1018.y, x_1018.z, x_1019.w);
  let x_1021 : vec4<f32> = u_xlat2;
  let x_1024 : vec3<f32> = log2(abs(vec3<f32>(x_1021.x, x_1021.y, x_1021.z)));
  let x_1025 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1024.x, x_1024.y, x_1024.z, x_1025.w);
  let x_1027 : vec4<f32> = u_xlat2;
  let x_1029 : vec3<f32> = (vec3<f32>(x_1027.x, x_1027.y, x_1027.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1039 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1037.x, x_1037.y, x_1037.z, x_1037.x));
  u_xlatb0 = vec3<bool>(x_1039.x, x_1039.y, x_1039.z);
  let x_1042 : bool = u_xlatb0.x;
  if (x_1042) {
  ...
  u_xlat0.y = x_1063;
  let x_1066 : bool = u_xlatb0.z;
  if (x_1066) {
    let x_1071 : f32 = u_xlat1.z;
    x_1067 = x_1071;
  } else {
    let x_1074 : f32 = u_xlat2.z;
    x_1067 = x_1074;
  }
  let x_1075 : f32 = x_1067;
  u_xlat0.z = x_1075;
  let x_1079 : vec4<f32> = u_xlat0;
  let x_1082 : vec3<f32> = max(vec3<f32>(x_1079.x, x_1079.y, x_1079.z), vec3<f32>(0.0f, 0.0f, 0.0f));

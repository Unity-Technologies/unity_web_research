...
fn main_1() {
  ...
  let x_597 : vec4<f32> = x_13.x_Dithering_Params;
  let x_601 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  let x_615 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_611.x, x_611.y), x_614);
  u_xlat15 = x_615.w;
  let x_617 : f32 = u_xlat15;
  u_xlat15 = ((x_617 * 2.0f) + -1.0f);
  let x_622 : f32 = u_xlat15;
  u_xlatb1.x = (x_622 >= 0.0f);
  let x_626 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_626);
  let x_629 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_629)) + 1.0f);
  ...
  u_xlat15 = (x_638 * x_640);
  let x_642 : vec4<f32> = u_xlat0;
  let x_644 : vec3<f32> = (vec3<f32>(x_642.x, x_642.y, x_642.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_645 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_644.x, x_644.y, x_644.z, x_645.w);
  let x_647 : vec4<f32> = u_xlat0;
  let x_650 : vec3<f32> = log2(abs(vec3<f32>(x_647.x, x_647.y, x_647.z)));
  let x_651 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_650.x, x_650.y, x_650.z, x_651.w);
  let x_653 : vec4<f32> = u_xlat2;
  let x_655 : vec3<f32> = (vec3<f32>(x_653.x, x_653.y, x_653.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  let x_656 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_655.x, x_655.y, x_655.z, x_656.w);
  let x_658 : vec4<f32> = u_xlat2;
  let x_660 : vec3<f32> = exp2(vec3<f32>(x_658.x, x_658.y, x_658.z));
  let x_661 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_660.x, x_660.y, x_660.z, x_661.w);
  let x_663 : vec4<f32> = u_xlat2;
  let x_666 : vec3<f32> = ((vec3<f32>(x_663.x, x_663.y, x_663.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_667 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_666.x, x_666.y, x_666.z, x_667.w);
  let x_670 : vec4<f32> = u_xlat0;
  let x_672 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_670.x, x_670.y, x_670.z, x_670.x));
  ...
  let x_717 : vec3<f32> = ((vec3<f32>(x_710, x_710, x_710) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_715.x, x_715.y, x_715.z));
  let x_718 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_717.x, x_717.y, x_717.z, x_718.w);
  let x_720 : vec4<f32> = u_xlat0;
  let x_722 : vec3<f32> = (vec3<f32>(x_720.x, x_720.y, x_720.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_723 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_722.x, x_722.y, x_722.z, x_723.w);
  let x_725 : vec4<f32> = u_xlat0;
  let x_727 : vec3<f32> = (vec3<f32>(x_725.x, x_725.y, x_725.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_728 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_727.x, x_727.y, x_727.z, x_728.w);
  let x_730 : vec4<f32> = u_xlat2;
  let x_732 : vec3<f32> = (vec3<f32>(x_730.x, x_730.y, x_730.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_733 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_732.x, x_732.y, x_732.z, x_733.w);
  let x_735 : vec4<f32> = u_xlat2;
  let x_738 : vec3<f32> = log2(abs(vec3<f32>(x_735.x, x_735.y, x_735.z)));
  let x_739 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_738.x, x_738.y, x_738.z, x_739.w);
  let x_741 : vec4<f32> = u_xlat2;
  let x_743 : vec3<f32> = (vec3<f32>(x_741.x, x_741.y, x_741.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_753 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_751.x, x_751.y, x_751.z, x_751.x));
  u_xlatb0 = vec3<bool>(x_753.x, x_753.y, x_753.z);
  let x_756 : bool = u_xlatb0.x;
  if (x_756) {
  ...
  let x_796 : vec3<f32> = max(vec3<f32>(x_793.x, x_793.y, x_793.z), vec3<f32>(0.0f, 0.0f, 0.0f));

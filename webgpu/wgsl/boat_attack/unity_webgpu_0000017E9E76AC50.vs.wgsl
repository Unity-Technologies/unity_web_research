...
fn main_1() {
  ...
  vs_TEXCOORD1 = ((x_172 * vec4<f32>(x_173.w, x_173.w, x_173.w, x_173.w)) + x_176);
  let x_178 : vec4<f32> = u_xlat1;
  let x_181 : vec4<f32> = x_18.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_178.y, x_178.y, x_178.y, x_178.y) * x_181);
  let x_184 : vec4<f32> = x_18.unity_MatrixVP[0i];
  let x_185 : vec4<f32> = u_xlat1;
  let x_188 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_184 * vec4<f32>(x_185.x, x_185.x, x_185.x, x_185.x)) + x_188);
  let x_191 : vec4<f32> = x_18.unity_MatrixVP[2i];
  ...
  let x_203 : vec4<f32> = x_18.unity_MatrixVP[3i];
  gl_Position = (x_201 + x_203);
  let x_207 : f32 = in_TEXCOORD0.y;
  u_xlatb0 = (0.0f < x_207);
  let x_211 : bool = u_xlatb0;
  vs_TEXCOORD0.y = select(0.0f, 1.0f, x_211);
  let x_217 : f32 = in_TEXCOORD0.x;
  vs_TEXCOORD0.x = x_217;
  return;
}

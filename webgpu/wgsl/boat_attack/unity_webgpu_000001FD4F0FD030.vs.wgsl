...
@group(1) @binding(6) var<uniform> x_44 : VGlobals;
...
@group(1) @binding(3) var<uniform> x_447 : LightShadows;
...
fn main_1() {
  ...
  let x_449 : vec4<f32> = x_447.x_MainLightWorldToShadow[0i][1i];
  let x_451 : vec3<f32> = (vec3<f32>(x_438.y, x_438.y, x_438.y) * vec3<f32>(x_449.x, x_449.y, x_449.z));
  let x_452 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_451.x, x_451.y, x_451.z, x_452.w);
  let x_455 : vec4<f32> = x_447.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_466 : vec4<f32> = x_447.x_MainLightWorldToShadow[0i][2i];
  ...
  vs_INTERP8 = vec3<f32>(x_477.x, x_477.y, x_477.z);
  let x_480 : vec4<f32> = u_xlat1;
  let x_483 : vec4<f32> = x_447.x_MainLightWorldToShadow[0i][3i];
  let x_485 : vec3<f32> = (vec3<f32>(x_480.x, x_480.y, x_480.z) + vec3<f32>(x_483.x, x_483.y, x_483.z));
  let x_486 : vec4<f32> = vs_INTERP3;
  vs_INTERP3 = vec4<f32>(x_485.x, x_485.y, x_485.z, x_486.w);
  vs_INTERP3.w = 0.0f;
  let x_491 : vec4<f32> = in_TANGENT0;
  let x_494 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_496 : vec3<f32> = (vec3<f32>(x_491.y, x_491.y, x_491.y) * vec3<f32>(x_494.x, x_494.y, x_494.z));
  let x_497 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_496.x, x_496.y, x_496.z, x_497.w);
  let x_500 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_502 : vec4<f32> = in_TANGENT0;
  ...
  let x_511 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_513 : vec4<f32> = in_TANGENT0;
  let x_516 : vec4<f32> = u_xlat0;
  let x_518 : vec3<f32> = ((vec3<f32>(x_511.x, x_511.y, x_511.z) * vec3<f32>(x_513.z, x_513.z, x_513.z)) + vec3<f32>(x_516.x, x_516.y, x_516.z));
  ...
  u_xlat15 = dot(vec3<f32>(x_521.x, x_521.y, x_521.z), vec3<f32>(x_523.x, x_523.y, x_523.z));
  let x_526 : f32 = u_xlat15;
  u_xlat15 = max(x_526, 1.17549435e-38f);
  let x_528 : f32 = u_xlat15;
  u_xlat15 = inverseSqrt(x_528);
  ...
  let x_536 : vec4<f32> = vs_INTERP4;
  vs_INTERP4 = vec4<f32>(x_535.x, x_535.y, x_535.z, x_536.w);
  let x_539 : f32 = in_TANGENT0.w;
  vs_INTERP4.w = x_539;
  let x_543 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP5 = x_543;
  let x_545 : vec4<f32> = in_COLOR0;
  vs_INTERP6 = x_545;

...
@group(1) @binding(0) var<uniform> x_59 : VGlobals;
...
fn main_1() {
  ...
  let x_97 : vec4<f32> = x_21.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_95, vec3<f32>(x_97.x, x_97.y, x_97.z));
  let x_103 : vec3<f32> = in_NORMAL0;
  let x_105 : vec4<f32> = x_21.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_103, vec3<f32>(x_105.x, x_105.y, x_105.z));
  let x_109 : vec3<f32> = in_NORMAL0;
  let x_111 : vec4<f32> = x_21.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_109, vec3<f32>(x_111.x, x_111.y, x_111.z));

...
@group(1) @binding(2) var<uniform> x_397 : VGlobals;
...
fn main_1() {
  ...
  u_xlat3.y = ((x_339 * -0.34999999403953552246f) + 0.34999999403953552246f);
  ...
  let x_400 : vec3<f32> = x_397.x_LightPosition;
  let x_401 : vec3<f32> = (-(vec3<f32>(x_392.x, x_392.y, x_392.z)) + x_400);
  let x_402 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_401.x, x_401.y, x_401.z, x_402.w);
  let x_404 : vec4<f32> = u_xlat1;
  let x_406 : vec4<f32> = u_xlat1;
  u_xlat12 = dot(vec3<f32>(x_404.x, x_404.y, x_404.z), vec3<f32>(x_406.x, x_406.y, x_406.z));
  let x_409 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_409);
  let x_411 : f32 = u_xlat12;
  let x_413 : vec4<f32> = u_xlat1;
  let x_415 : vec3<f32> = (vec3<f32>(x_411, x_411, x_411) * vec3<f32>(x_413.x, x_413.y, x_413.z));
  let x_416 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_415.x, x_415.y, x_415.z, x_416.w);
  let x_418 : vec4<f32> = u_xlat1;
  let x_420 : vec4<f32> = u_xlat2;
  u_xlat12 = dot(vec3<f32>(x_418.x, x_418.y, x_418.z), vec3<f32>(x_420.x, x_420.y, x_420.z));
  let x_423 : f32 = u_xlat12;
  u_xlat12 = clamp(x_423, 0.0f, 1.0f);
  let x_425 : vec4<f32> = u_xlat1;
  let x_428 : vec4<f32> = x_397.x_ShadowBias;
  let x_431 : vec4<f32> = u_xlat0;
  let x_433 : vec3<f32> = ((vec3<f32>(x_425.x, x_425.y, x_425.z) * vec3<f32>(x_428.x, x_428.x, x_428.x)) + vec3<f32>(x_431.x, x_431.y, x_431.z));
  let x_434 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_433.x, x_433.y, x_433.z, x_434.w);
  let x_438 : vec4<f32> = u_xlat0;
  vs_INTERP1 = vec3<f32>(x_438.x, x_438.y, x_438.z);
  let x_440 : f32 = u_xlat12;
  u_xlat0.x = (-(x_440) + 1.0f);
  let x_445 : f32 = u_xlat0.x;
  let x_447 : f32 = x_397.x_ShadowBias.y;
  u_xlat0.x = (x_445 * x_447);
  let x_450 : vec4<f32> = u_xlat2;
  let x_452 : vec4<f32> = u_xlat0;
  let x_455 : vec4<f32> = u_xlat1;
  let x_457 : vec3<f32> = ((vec3<f32>(x_450.x, x_450.y, x_450.z) * vec3<f32>(x_452.x, x_452.x, x_452.x)) + vec3<f32>(x_455.x, x_455.y, x_455.z));
  let x_458 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_457.x, x_457.y, x_457.z, x_458.w);
  let x_461 : vec4<f32> = u_xlat2;
  vs_INTERP2 = vec3<f32>(x_461.x, x_461.y, x_461.z);
  let x_463 : vec4<f32> = u_xlat0;
  let x_466 : vec4<f32> = x_397.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_463.y, x_463.y, x_463.y, x_463.y) * x_466);
  let x_469 : vec4<f32> = x_397.unity_MatrixVP[0i];
  let x_470 : vec4<f32> = u_xlat0;
  let x_473 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_469 * vec4<f32>(x_470.x, x_470.x, x_470.x, x_470.x)) + x_473);
  let x_476 : vec4<f32> = x_397.unity_MatrixVP[2i];
  let x_477 : vec4<f32> = u_xlat0;
  let x_480 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_476 * vec4<f32>(x_477.z, x_477.z, x_477.z, x_477.z)) + x_480);
  let x_482 : vec4<f32> = u_xlat0;
  let x_484 : vec4<f32> = x_397.unity_MatrixVP[3i];
  u_xlat0 = (x_482 + x_484);
  let x_491 : f32 = u_xlat0.z;
  gl_Position.z = min(x_491, 1.0f);
  let x_495 : vec4<f32> = u_xlat0;
  let x_496 : vec3<f32> = vec3<f32>(x_495.x, x_495.y, x_495.w);
  let x_499 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_496.x, x_496.y, x_499.z, x_496.z);
  let x_503 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP0 = x_503;
  return;
}

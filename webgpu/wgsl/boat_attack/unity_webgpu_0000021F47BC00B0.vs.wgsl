...
@group(1) @binding(3) var<uniform> x_509 : MainLightShadows;
...
fn main_1() {
  ...
  let x_511 : vec4<f32> = x_509.x_MainLightWorldToShadow[0i][1i];
  let x_513 : vec3<f32> = (vec3<f32>(x_503.y, x_503.y, x_503.y) * vec3<f32>(x_511.x, x_511.y, x_511.z));
  let x_514 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_513.x, x_513.y, x_513.z, x_514.w);
  let x_517 : vec4<f32> = x_509.x_MainLightWorldToShadow[0i][0i];
  let x_519 : vec4<f32> = u_xlat0;
  let x_522 : vec4<f32> = u_xlat1;
  let x_524 : vec3<f32> = ((vec3<f32>(x_517.x, x_517.y, x_517.z) * vec3<f32>(x_519.x, x_519.x, x_519.x)) + vec3<f32>(x_522.x, x_522.y, x_522.z));
  let x_525 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_524.x, x_524.y, x_525.z, x_524.z);
  let x_528 : vec4<f32> = x_509.x_MainLightWorldToShadow[0i][2i];
  let x_530 : vec4<f32> = u_xlat0;
  let x_533 : vec4<f32> = u_xlat0;
  let x_535 : vec3<f32> = ((vec3<f32>(x_528.x, x_528.y, x_528.z) * vec3<f32>(x_530.z, x_530.z, x_530.z)) + vec3<f32>(x_533.x, x_533.y, x_533.w));
  let x_536 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_535.x, x_535.y, x_535.z, x_536.w);
  let x_539 : vec4<f32> = u_xlat0;
  let x_542 : vec4<f32> = x_509.x_MainLightWorldToShadow[0i][3i];
  let x_544 : vec3<f32> = (vec3<f32>(x_539.x, x_539.y, x_539.z) + vec3<f32>(x_542.x, x_542.y, x_542.z));
  let x_545 : vec4<f32> = vs_INTERP9;
  vs_INTERP9 = vec4<f32>(x_544.x, x_544.y, x_544.z, x_545.w);
  vs_INTERP9.w = 0.0f;
  let x_550 : vec4<f32> = phase0_Output0_6;
  vs_INTERP5 = vec2<f32>(x_550.x, x_550.y);
  let x_553 : vec4<f32> = phase0_Output0_6;
  vs_INTERP6 = vec2<f32>(x_553.z, x_553.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec3<f32>, @location(4) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_TANGENT0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_INTERP1, gl_Position, vs_INTERP0, vs_INTERP2, vs_INTERP3, vs_INTERP4, vs_INTERP9);
}

...
fn main_1() {
  ...
  u_xlat1 = (x_706 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_708 : vec3<f32> = u_xlat0;
  let x_710 : vec3<f32> = log2(abs(x_708));
  ...
  let x_732 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_730.x, x_730.y, x_730.z, x_730.x));
  u_xlatb0 = vec3<bool>(x_732.x, x_732.y, x_732.z);
  let x_735 : bool = u_xlatb0.x;
  if (x_735) {
  ...
  let x_756 : f32 = x_748;
  u_xlat0.y = x_756;
  let x_759 : bool = u_xlatb0.z;
  if (x_759) {
    let x_764 : f32 = u_xlat1.z;
    x_760 = x_764;
  } else {
    let x_767 : f32 = u_xlat2.z;
    x_760 = x_767;
  }
  let x_768 : f32 = x_760;
  u_xlat0.z = x_768;
  let x_770 : vec2<f32> = vs_TEXCOORD0;
  let x_773 : vec4<f32> = x_26.x_Dithering_Params;
  let x_777 : vec4<f32> = x_26.x_Dithering_Params;
  let x_779 : vec2<f32> = ((x_770 * vec2<f32>(x_773.x, x_773.y)) + vec2<f32>(x_777.z, x_777.w));
  let x_780 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_779.x, x_779.y, x_780.z);
  let x_787 : vec3<f32> = u_xlat1;
  let x_790 : f32 = x_26.x_GlobalMipBias.x;
  let x_791 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_787.x, x_787.y), x_790);
  u_xlat15 = x_791.w;
  let x_793 : f32 = u_xlat15;
  u_xlat15 = ((x_793 * 2.0f) + -1.0f);
  ...
  u_xlat1 = (x_817 * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_819 : vec3<f32> = u_xlat0;
  let x_821 : vec3<f32> = log2(abs(x_819));
  let x_822 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_821.x, x_821.y, x_821.z, x_822.w);
  let x_824 : vec4<f32> = u_xlat2;
  let x_826 : vec3<f32> = (vec3<f32>(x_824.x, x_824.y, x_824.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_837 : vec3<f32> = ((vec3<f32>(x_834.x, x_834.y, x_834.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_838 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_837.x, x_837.y, x_837.z, x_838.w);
  let x_840 : vec3<f32> = u_xlat0;
  let x_842 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_840.x, x_840.y, x_840.z, x_840.x));
  u_xlatb0 = vec3<bool>(x_842.x, x_842.y, x_842.z);
  ...
  let x_878 : f32 = x_870;
  u_xlat0.z = x_878;
  let x_880 : f32 = u_xlat15;
  let x_885 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_880, x_880, x_880) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + x_885);
  let x_887 : vec3<f32> = u_xlat0;
  u_xlat1 = (x_887 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_889 : vec3<f32> = u_xlat0;
  let x_890 : vec3<f32> = (x_889 + vec3<f32>(0.055f, 0.055f, 0.055f));
  ...
  let x_957 : vec3<f32> = max(x_956, vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_958 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_957.x, x_957.y, x_957.z, x_958.w);
  SV_Target0.w = 1.0f;
  return;
}

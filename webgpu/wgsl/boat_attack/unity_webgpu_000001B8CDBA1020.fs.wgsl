...
fn main_1() {
  ...
  let x_801 : vec3<f32> = clamp(vec3<f32>(x_797.x, x_797.y, x_797.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_802 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_801.x, x_801.y, x_801.z, x_802.w);
  let x_806 : f32 = x_25.x_UserLut_Params.w;
  u_xlatb18 = (0.0f < x_806);
  let x_808 : bool = u_xlatb18;
  if (x_808) {
    let x_811 : vec4<f32> = u_xlat0;
    u_xlat1 = (vec3<f32>(x_811.x, x_811.y, x_811.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
    ...
    let x_825 : vec3<f32> = (vec3<f32>(x_821.x, x_821.y, x_821.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
    ...
    let x_840 : vec3<f32> = ((vec3<f32>(x_833.x, x_833.y, x_833.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
    let x_841 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_840.x, x_840.y, x_840.z, x_841.w);
    let x_845 : vec4<f32> = u_xlat0;
    let x_848 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_845.x, x_845.y, x_845.z, x_845.x));
    ...
    let x_892 : vec4<f32> = x_25.x_UserLut_Params;
    let x_894 : vec3<f32> = (vec3<f32>(x_889.z, x_889.x, x_889.y) * vec3<f32>(x_892.z, x_892.z, x_892.z));
    let x_895 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_894.x, x_894.y, x_894.z, x_895.w);
    let x_898 : f32 = u_xlat2.x;
    u_xlat18 = floor(x_898);
    let x_901 : vec4<f32> = x_25.x_UserLut_Params;
    let x_903 : vec2<f32> = (vec2<f32>(x_901.x, x_901.y) * vec2<f32>(0.5f, 0.5f));
    let x_904 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_903.x, x_904.y, x_904.z, x_903.y);
    let x_906 : vec4<f32> = u_xlat2;
    let x_909 : vec4<f32> = x_25.x_UserLut_Params;
    let x_912 : vec4<f32> = u_xlat2;
    let x_914 : vec2<f32> = ((vec2<f32>(x_906.y, x_906.z) * vec2<f32>(x_909.x, x_909.y)) + vec2<f32>(x_912.x, x_912.w));
    let x_915 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_915.x, x_914.x, x_914.y, x_915.w);
    let x_917 : f32 = u_xlat18;
    let x_919 : f32 = x_25.x_UserLut_Params.y;
    let x_922 : f32 = u_xlat2.y;
    u_xlat2.x = ((x_917 * x_919) + x_922);
    let x_929 : vec4<f32> = u_xlat2;
    let x_931 : vec4<f32> = textureSampleLevel(x_UserLut, sampler_LinearClamp, vec2<f32>(x_929.x, x_929.z), 0.0f);
    ...
    let x_941 : vec2<f32> = u_xlat4;
    let x_942 : vec2<f32> = (vec2<f32>(x_939.x, x_939.z) + x_941);
    let x_943 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_942.x, x_942.y, x_943.z, x_943.w);
    let x_948 : vec4<f32> = u_xlat2;
    let x_950 : vec4<f32> = textureSampleLevel(x_UserLut, sampler_LinearClamp, vec2<f32>(x_948.x, x_948.y), 0.0f);
    let x_951 : vec3<f32> = vec3<f32>(x_950.x, x_950.y, x_950.z);
    let x_952 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_951.x, x_951.y, x_951.z, x_952.w);
    let x_955 : f32 = u_xlat1.z;
    let x_957 : f32 = x_25.x_UserLut_Params.z;
    let x_959 : f32 = u_xlat18;
    u_xlat18 = ((x_955 * x_957) + -(x_959));
    let x_962 : vec4<f32> = u_xlat3;
    let x_965 : vec4<f32> = u_xlat2;
    let x_967 : vec3<f32> = (-(vec3<f32>(x_962.x, x_962.y, x_962.z)) + vec3<f32>(x_965.x, x_965.y, x_965.z));
    ...
    let x_977 : vec3<f32> = ((vec3<f32>(x_970, x_970, x_970) * vec3<f32>(x_972.x, x_972.y, x_972.z)) + vec3<f32>(x_975.x, x_975.y, x_975.z));
    ...
    let x_984 : vec3<f32> = (-(x_980) + vec3<f32>(x_982.x, x_982.y, x_982.z));
    let x_985 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_984.x, x_984.y, x_984.z, x_985.w);
    let x_988 : vec4<f32> = x_25.x_UserLut_Params;
    let x_990 : vec4<f32> = u_xlat2;
    let x_993 : vec3<f32> = u_xlat1;
    u_xlat1 = ((vec3<f32>(x_988.w, x_988.w, x_988.w) * vec3<f32>(x_990.x, x_990.y, x_990.z)) + x_993);
    let x_995 : vec3<f32> = u_xlat1;
    let x_998 : vec3<f32> = (x_995 * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
    let x_999 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_998.x, x_998.y, x_998.z, x_999.w);
    let x_1001 : vec3<f32> = u_xlat1;
    let x_1004 : vec3<f32> = (x_1001 + vec3<f32>(0.055f, 0.055f, 0.055f));
    let x_1005 : vec4<f32> = u_xlat3;
    u_xlat3 = vec4<f32>(x_1004.x, x_1004.y, x_1004.z, x_1005.w);
    let x_1007 : vec4<f32> = u_xlat3;
    let x_1011 : vec3<f32> = (vec3<f32>(x_1007.x, x_1007.y, x_1007.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
    let x_1012 : vec4<f32> = u_xlat3;
    u_xlat3 = vec4<f32>(x_1011.x, x_1011.y, x_1011.z, x_1012.w);
    let x_1014 : vec4<f32> = u_xlat3;
    let x_1017 : vec3<f32> = log2(abs(vec3<f32>(x_1014.x, x_1014.y, x_1014.z)));
    let x_1018 : vec4<f32> = u_xlat3;
    u_xlat3 = vec4<f32>(x_1017.x, x_1017.y, x_1017.z, x_1018.w);
    let x_1020 : vec4<f32> = u_xlat3;
    let x_1024 : vec3<f32> = (vec3<f32>(x_1020.x, x_1020.y, x_1020.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
    let x_1025 : vec4<f32> = u_xlat3;
    u_xlat3 = vec4<f32>(x_1024.x, x_1024.y, x_1024.z, x_1025.w);
    let x_1027 : vec4<f32> = u_xlat3;
    let x_1029 : vec3<f32> = exp2(vec3<f32>(x_1027.x, x_1027.y, x_1027.z));
    let x_1030 : vec4<f32> = u_xlat3;
    u_xlat3 = vec4<f32>(x_1029.x, x_1029.y, x_1029.z, x_1030.w);
    let x_1034 : vec3<f32> = u_xlat1;
    let x_1036 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_1034.x, x_1034.y, x_1034.z, x_1034.x));
    u_xlatb1 = vec3<bool>(x_1036.x, x_1036.y, x_1036.z);
    let x_1039 : bool = u_xlatb1.x;
    if (x_1039) {
  ...
  let x_1079 : vec3<f32> = (vec3<f32>(x_1074.x, x_1074.y, x_1074.z) * vec3<f32>(x_1077.z, x_1077.z, x_1077.z));
  let x_1080 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1079.x, x_1079.y, x_1080.z, x_1079.z);
  let x_1083 : f32 = u_xlat0.w;
  u_xlat18 = floor(x_1083);
  let x_1086 : vec4<f32> = x_25.x_Lut_Params;
  let x_1088 : vec2<f32> = (vec2<f32>(x_1086.x, x_1086.y) * vec2<f32>(0.5f, 0.5f));
  let x_1089 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_1088.x, x_1088.y, x_1089.z);
  let x_1091 : vec4<f32> = u_xlat0;
  let x_1094 : vec4<f32> = x_25.x_Lut_Params;
  let x_1097 : vec3<f32> = u_xlat1;
  let x_1099 : vec2<f32> = ((vec2<f32>(x_1091.x, x_1091.y) * vec2<f32>(x_1094.x, x_1094.y)) + vec2<f32>(x_1097.x, x_1097.y));
  let x_1100 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_1100.x, x_1099.x, x_1099.y);
  let x_1102 : f32 = u_xlat18;
  let x_1104 : f32 = x_25.x_Lut_Params.y;
  let x_1107 : f32 = u_xlat1.y;
  u_xlat1.x = ((x_1102 * x_1104) + x_1107);
  let x_1114 : vec3<f32> = u_xlat1;
  let x_1116 : vec4<f32> = textureSampleLevel(x_InternalLut, sampler_LinearClamp, vec2<f32>(x_1114.x, x_1114.z), 0.0f);
  let x_1117 : vec3<f32> = vec3<f32>(x_1116.x, x_1116.y, x_1116.z);
  let x_1118 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1117.x, x_1117.y, x_1117.z, x_1118.w);
  let x_1121 : f32 = x_25.x_Lut_Params.y;
  u_xlat0.x = x_1121;
  u_xlat0.y = 0.0f;
  let x_1124 : vec4<f32> = u_xlat0;
  let x_1126 : vec3<f32> = u_xlat1;
  let x_1128 : vec2<f32> = (vec2<f32>(x_1124.x, x_1124.y) + vec2<f32>(x_1126.x, x_1126.z));
  let x_1129 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1128.x, x_1128.y, x_1129.z, x_1129.w);
  let x_1134 : vec4<f32> = u_xlat0;
  let x_1136 : vec4<f32> = textureSampleLevel(x_InternalLut, sampler_LinearClamp, vec2<f32>(x_1134.x, x_1134.y), 0.0f);
  u_xlat1 = vec3<f32>(x_1136.x, x_1136.y, x_1136.z);
  let x_1139 : f32 = u_xlat0.z;
  let x_1141 : f32 = x_25.x_Lut_Params.z;
  let x_1143 : f32 = u_xlat18;
  u_xlat0.x = ((x_1139 * x_1141) + -(x_1143));
  let x_1147 : vec4<f32> = u_xlat2;
  let x_1150 : vec3<f32> = u_xlat1;
  u_xlat6 = (-(vec3<f32>(x_1147.x, x_1147.y, x_1147.z)) + x_1150);
  let x_1152 : vec4<f32> = u_xlat0;
  let x_1154 : vec3<f32> = u_xlat6;
  let x_1156 : vec4<f32> = u_xlat2;
  let x_1158 : vec3<f32> = ((vec3<f32>(x_1152.x, x_1152.x, x_1152.x) * x_1154) + vec3<f32>(x_1156.x, x_1156.y, x_1156.z));
  let x_1159 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1158.x, x_1158.y, x_1158.z, x_1159.w);
  let x_1163 : vec4<f32> = u_xlat0;
  let x_1165 : vec3<f32> = sqrt(vec3<f32>(x_1163.x, x_1163.y, x_1163.z));

...
fn main_1() {
  ...
  let x_550 : vec4<f32> = x_26.x_Dithering_Params;
  let x_554 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_568 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_564.x, x_564.y), x_567);
  u_xlat15 = x_568.w;
  let x_570 : f32 = u_xlat15;
  u_xlat15 = ((x_570 * 2.0f) + -1.0f);
  let x_575 : f32 = u_xlat15;
  u_xlatb1.x = (x_575 >= 0.0f);
  let x_579 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_579);
  let x_582 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_582)) + 1.0f);
  ...
  u_xlat1 = (vec3<f32>(x_595.x, x_595.y, x_595.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_598 : vec4<f32> = u_xlat0;
  let x_601 : vec3<f32> = log2(abs(vec3<f32>(x_598.x, x_598.y, x_598.z)));
  let x_602 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_601.x, x_601.y, x_601.z, x_602.w);
  let x_604 : vec4<f32> = u_xlat2;
  let x_606 : vec3<f32> = (vec3<f32>(x_604.x, x_604.y, x_604.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_617 : vec3<f32> = ((vec3<f32>(x_614.x, x_614.y, x_614.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_618 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_617.x, x_617.y, x_617.z, x_618.w);
  let x_621 : vec4<f32> = u_xlat0;
  let x_623 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_621.x, x_621.y, x_621.z, x_621.x));
  u_xlatb0 = vec3<bool>(x_623.x, x_623.y, x_623.z);
  ...
  let x_668 : vec3<f32> = ((vec3<f32>(x_661, x_661, x_661) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_666.x, x_666.y, x_666.z));
  let x_669 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_668.x, x_668.y, x_668.z, x_669.w);
  let x_671 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_671.x, x_671.y, x_671.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_674 : vec4<f32> = u_xlat0;
  let x_676 : vec3<f32> = (vec3<f32>(x_674.x, x_674.y, x_674.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_677 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_676.x, x_676.y, x_676.z, x_677.w);
  let x_679 : vec4<f32> = u_xlat2;
  let x_681 : vec3<f32> = (vec3<f32>(x_679.x, x_679.y, x_679.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_682 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_681.x, x_681.y, x_681.z, x_682.w);
  let x_684 : vec4<f32> = u_xlat2;
  let x_687 : vec3<f32> = log2(abs(vec3<f32>(x_684.x, x_684.y, x_684.z)));
  let x_688 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_687.x, x_687.y, x_687.z, x_688.w);
  let x_690 : vec4<f32> = u_xlat2;
  let x_692 : vec3<f32> = (vec3<f32>(x_690.x, x_690.y, x_690.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  let x_693 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_692.x, x_692.y, x_692.z, x_693.w);
  let x_695 : vec4<f32> = u_xlat2;
  let x_697 : vec3<f32> = exp2(vec3<f32>(x_695.x, x_695.y, x_695.z));
  let x_698 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_697.x, x_697.y, x_697.z, x_698.w);
  let x_700 : vec4<f32> = u_xlat0;
  let x_702 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_700.x, x_700.y, x_700.z, x_700.x));
  ...
  let x_745 : vec3<f32> = max(vec3<f32>(x_742.x, x_742.y, x_742.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_746 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_745.x, x_745.y, x_745.z, x_746.w);
  SV_Target0.w = 1.0f;
  return;
}

...
float u_xlat46;
float u_xlat48;
    u_xlat0.x = texture(sampler2D(_CameraDepthTexture, sampler_CameraDepthTexture), vs_TEXCOORD0.xy, _GlobalMipBias.x).x;
...
;
    ...
    u_xlat1.xyz = u_xlat0.yyy * _MainLightWorldToShadow[(u_xlati45 + 1) / 4][(u_xlati45 + 1) % 4].xyz;
    u_xlat1.xyz = _MainLightWorldToShadow[u_xlati45 / 4][u_xlati45 % 4].xyz * u_xlat0.xxx + u_xlat1.xyz;
    u_xlat0.xyz = _MainLightWorldToShadow[(u_xlati45 + 2) / 4][(u_xlati45 + 2) % 4].xyz * u_xlat0.zzz + u_xlat1.xyz;
    u_xlat0.xyz = u_xlat0.xyz + _MainLightWorldToShadow[(u_xlati45 + 3) / 4][(u_xlati45 + 3) % 4].xyz;

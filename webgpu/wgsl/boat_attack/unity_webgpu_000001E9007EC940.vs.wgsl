...
@group(1) @binding(3) var<uniform> x_241 : MainLightShadows;

var<private> vs_TEXCOORD8 : vec4<f32>;

@group(1) @binding(6) var<uniform> x_285 : VGlobals;
...
fn main_1() {
  ...
  let x_245 : vec3<f32> = (vec3<f32>(x_235.y, x_235.y, x_235.y) * vec3<f32>(x_243.x, x_243.y, x_243.z));
  let x_246 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_245.x, x_245.y, x_245.z, x_246.w);
  let x_249 : vec4<f32> = x_241.x_MainLightWorldToShadow[0i][0i];
  ...
  let x_260 : vec4<f32> = x_241.x_MainLightWorldToShadow[0i][2i];
  ...
  let x_274 : vec4<f32> = x_241.x_MainLightWorldToShadow[0i][3i];
  let x_276 : vec3<f32> = (vec3<f32>(x_271.x, x_271.y, x_271.z) + vec3<f32>(x_274.x, x_274.y, x_274.z));
  let x_277 : vec4<f32> = vs_TEXCOORD8;
  vs_TEXCOORD8 = vec4<f32>(x_276.x, x_276.y, x_276.z, x_277.w);
  vs_TEXCOORD8.w = 0.0f;
  let x_281 : vec4<f32> = u_xlat0;
  let x_287 : vec4<f32> = x_285.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_281.y, x_281.y, x_281.y, x_281.y) * x_287);
  let x_290 : vec4<f32> = x_285.unity_MatrixVP[0i];
  let x_291 : vec4<f32> = u_xlat0;
  let x_294 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_290 * vec4<f32>(x_291.x, x_291.x, x_291.x, x_291.x)) + x_294);
  let x_297 : vec4<f32> = x_285.unity_MatrixVP[2i];
  let x_298 : vec4<f32> = u_xlat0;
  let x_301 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_297 * vec4<f32>(x_298.z, x_298.z, x_298.z, x_298.z)) + x_301);
  let x_307 : vec4<f32> = u_xlat0;
  let x_309 : vec4<f32> = x_285.unity_MatrixVP[3i];

...
@group(1) @binding(5) var<uniform> x_159 : VGlobals;
...
fn main_1() {
  ...
  let x_145 : vec2<f32> = in_TEXCOORD1;
  let x_148 : vec4<f32> = x_44.unity_LightmapST;
  let x_152 : vec4<f32> = x_44.unity_LightmapST;
  vs_TEXCOORD7 = ((x_145 * vec2<f32>(x_148.x, x_148.y)) + vec2<f32>(x_152.z, x_152.w));
  let x_155 : vec4<f32> = u_xlat0;
  let x_161 : vec4<f32> = x_159.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_155.y, x_155.y, x_155.y, x_155.y) * x_161);
  let x_164 : vec4<f32> = x_159.unity_MatrixVP[0i];
  let x_165 : vec4<f32> = u_xlat0;
  let x_168 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_164 * vec4<f32>(x_165.x, x_165.x, x_165.x, x_165.x)) + x_168);
  let x_171 : vec4<f32> = x_159.unity_MatrixVP[2i];
  let x_172 : vec4<f32> = u_xlat0;
  let x_175 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_171 * vec4<f32>(x_172.z, x_172.z, x_172.z, x_172.z)) + x_175);
  let x_181 : vec4<f32> = u_xlat0;
  let x_183 : vec4<f32> = x_159.unity_MatrixVP[3i];
  gl_Position = (x_181 + x_183);
  return;
}

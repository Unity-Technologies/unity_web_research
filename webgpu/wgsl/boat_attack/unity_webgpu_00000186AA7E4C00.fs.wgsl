...
fn main_1() {
  ...
  let x_878 : vec2<f32> = vs_TEXCOORD0;
  let x_881 : vec4<f32> = x_25.x_Dithering_Params;
  let x_885 : vec4<f32> = x_25.x_Dithering_Params;
  let x_887 : vec2<f32> = ((x_878 * vec2<f32>(x_881.x, x_881.y)) + vec2<f32>(x_885.z, x_885.w));
  let x_888 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_887.x, x_887.y, x_888.z, x_888.w);
  let x_895 : vec4<f32> = u_xlat1;
  let x_898 : f32 = x_25.x_GlobalMipBias.x;
  let x_899 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_895.x, x_895.y), x_898);
  u_xlat15 = x_899.w;
  let x_901 : f32 = u_xlat15;
  u_xlat15 = ((x_901 * 2.0f) + -1.0f);
  let x_905 : f32 = u_xlat15;
  u_xlatb1.x = (x_905 >= 0.0f);
  let x_909 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_909);
  let x_912 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_912)) + 1.0f);
  ...
  let x_927 : vec3<f32> = (vec3<f32>(x_925.x, x_925.y, x_925.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_938 : vec3<f32> = (vec3<f32>(x_936.x, x_936.y, x_936.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_949 : vec3<f32> = ((vec3<f32>(x_946.x, x_946.y, x_946.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_950 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_949.x, x_949.y, x_949.z, x_950.w);
  let x_953 : vec4<f32> = u_xlat0;
  let x_955 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_953.x, x_953.y, x_953.z, x_953.x));
  ...
  let x_1000 : vec3<f32> = ((vec3<f32>(x_993, x_993, x_993) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_998.x, x_998.y, x_998.z));
  let x_1001 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1000.x, x_1000.y, x_1000.z, x_1001.w);
  let x_1003 : vec4<f32> = u_xlat0;
  let x_1005 : vec3<f32> = (vec3<f32>(x_1003.x, x_1003.y, x_1003.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_1006 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1005.x, x_1005.y, x_1005.z, x_1006.w);
  let x_1008 : vec4<f32> = u_xlat0;
  let x_1010 : vec3<f32> = (vec3<f32>(x_1008.x, x_1008.y, x_1008.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_1011 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1010.x, x_1010.y, x_1010.z, x_1011.w);
  let x_1013 : vec4<f32> = u_xlat2;
  let x_1015 : vec3<f32> = (vec3<f32>(x_1013.x, x_1013.y, x_1013.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_1016 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1015.x, x_1015.y, x_1015.z, x_1016.w);
  let x_1018 : vec4<f32> = u_xlat2;
  let x_1021 : vec3<f32> = log2(abs(vec3<f32>(x_1018.x, x_1018.y, x_1018.z)));
  let x_1022 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1021.x, x_1021.y, x_1021.z, x_1022.w);
  let x_1024 : vec4<f32> = u_xlat2;
  let x_1026 : vec3<f32> = (vec3<f32>(x_1024.x, x_1024.y, x_1024.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  u_xlatb0 = vec3<bool>(x_1036.x, x_1036.y, x_1036.z);
  let x_1039 : bool = u_xlatb0.x;
  if (x_1039) {
  ...
  u_xlat0.x = x_1048;
  let x_1051 : bool = u_xlatb0.y;
  if (x_1051) {
    let x_1056 : f32 = u_xlat1.y;
    x_1052 = x_1056;
  } else {
  ...
  let x_1079 : vec3<f32> = max(vec3<f32>(x_1076.x, x_1076.y, x_1076.z), vec3<f32>(0.0f, 0.0f, 0.0f));

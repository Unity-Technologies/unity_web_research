...
fn main_1() {
  ...
  let x_732 : vec4<f32> = x_13.x_Dithering_Params;
  ...
  u_xlat1 = vec4<f32>(x_738.x, x_738.y, x_739.z, x_739.w);
  let x_746 : vec4<f32> = u_xlat1;
  let x_749 : f32 = x_13.x_GlobalMipBias.x;
  ...
  u_xlatb1.x = (x_757 >= 0.0f);
  let x_761 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_761);
  let x_764 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_764)) + 1.0f);
  ...
  u_xlat15 = (x_773 * x_775);
  let x_777 : vec4<f32> = u_xlat0;
  let x_779 : vec3<f32> = (vec3<f32>(x_777.x, x_777.y, x_777.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_790 : vec3<f32> = (vec3<f32>(x_788.x, x_788.y, x_788.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_801 : vec3<f32> = ((vec3<f32>(x_798.x, x_798.y, x_798.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_802 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_801.x, x_801.y, x_801.z, x_802.w);
  let x_805 : vec4<f32> = u_xlat0;
  let x_807 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_805.x, x_805.y, x_805.z, x_805.x));
  u_xlatb0 = vec3<bool>(x_807.x, x_807.y, x_807.z);
  let x_810 : bool = u_xlatb0.x;
  if (x_810) {
  ...
  let x_852 : vec3<f32> = ((vec3<f32>(x_845, x_845, x_845) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_850.x, x_850.y, x_850.z));
  let x_853 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_852.x, x_852.y, x_852.z, x_853.w);
  let x_855 : vec4<f32> = u_xlat0;
  let x_857 : vec3<f32> = (vec3<f32>(x_855.x, x_855.y, x_855.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_858 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_857.x, x_857.y, x_857.z, x_858.w);
  let x_860 : vec4<f32> = u_xlat0;
  let x_862 : vec3<f32> = (vec3<f32>(x_860.x, x_860.y, x_860.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_863 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_862.x, x_862.y, x_862.z, x_863.w);
  let x_865 : vec4<f32> = u_xlat2;
  let x_867 : vec3<f32> = (vec3<f32>(x_865.x, x_865.y, x_865.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_868 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_867.x, x_867.y, x_867.z, x_868.w);
  let x_870 : vec4<f32> = u_xlat2;
  let x_873 : vec3<f32> = log2(abs(vec3<f32>(x_870.x, x_870.y, x_870.z)));
  let x_874 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_873.x, x_873.y, x_873.z, x_874.w);
  let x_876 : vec4<f32> = u_xlat2;
  let x_878 : vec3<f32> = (vec3<f32>(x_876.x, x_876.y, x_876.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_888 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_886.x, x_886.y, x_886.z, x_886.x));
  ...
  let x_931 : vec3<f32> = max(vec3<f32>(x_928.x, x_928.y, x_928.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_932 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_931.x, x_931.y, x_931.z, x_932.w);
  SV_Target0.w = 1.0f;
  return;
}

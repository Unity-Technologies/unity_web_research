...
@group(1) @binding(2) var<uniform> x_394 : VGlobals;
...
fn main_1() {
  ...
  vs_INTERP0 = vec3<f32>(x_414.x, x_414.y, x_414.z);
  let x_420 : vec4<f32> = u_xlat1;
  let x_422 : vec4<f32> = x_394.unity_MatrixVP[3i];
  gl_Position = (x_420 + x_422);
  let x_427 : vec4<f32> = in_TANGENT0;
  let x_430 : vec4<f32> = x_16.unity_ObjectToWorld[1i];
  let x_432 : vec3<f32> = (vec3<f32>(x_427.y, x_427.y, x_427.y) * vec3<f32>(x_430.x, x_430.y, x_430.z));
  let x_433 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_432.x, x_432.y, x_432.z, x_433.w);
  let x_436 : vec4<f32> = x_16.unity_ObjectToWorld[0i];
  let x_438 : vec4<f32> = in_TANGENT0;
  let x_441 : vec4<f32> = u_xlat0;
  let x_443 : vec3<f32> = ((vec3<f32>(x_436.x, x_436.y, x_436.z) * vec3<f32>(x_438.x, x_438.x, x_438.x)) + vec3<f32>(x_441.x, x_441.y, x_441.z));
  let x_444 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_443.x, x_443.y, x_443.z, x_444.w);
  let x_447 : vec4<f32> = x_16.unity_ObjectToWorld[2i];
  let x_449 : vec4<f32> = in_TANGENT0;
  let x_452 : vec4<f32> = u_xlat0;
  let x_454 : vec3<f32> = ((vec3<f32>(x_447.x, x_447.y, x_447.z) * vec3<f32>(x_449.z, x_449.z, x_449.z)) + vec3<f32>(x_452.x, x_452.y, x_452.z));
  ...
  u_xlat12 = dot(vec3<f32>(x_457.x, x_457.y, x_457.z), vec3<f32>(x_459.x, x_459.y, x_459.z));
  let x_462 : f32 = u_xlat12;
  u_xlat12 = max(x_462, 1.17549435e-37f);
  let x_464 : f32 = u_xlat12;
  u_xlat12 = inverseSqrt(x_464);
  ...
  let x_472 : vec4<f32> = vs_INTERP2;
  vs_INTERP2 = vec4<f32>(x_471.x, x_471.y, x_471.z, x_472.w);
  let x_475 : f32 = in_TANGENT0.w;
  vs_INTERP2.w = x_475;
  let x_480 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP3 = x_480;
  return;
}

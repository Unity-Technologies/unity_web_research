...
fn main_1() {
  ...
  let x_765 : vec3<f32> = sqrt(vec3<f32>(x_763.x, x_763.y, x_763.z));
  let x_766 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_765.x, x_765.y, x_765.z, x_766.w);
  let x_768 : vec2<f32> = vs_TEXCOORD0;
  let x_771 : vec4<f32> = x_38.x_Dithering_Params;
  let x_775 : vec4<f32> = x_38.x_Dithering_Params;
  let x_777 : vec2<f32> = ((x_768 * vec2<f32>(x_771.x, x_771.y)) + vec2<f32>(x_775.z, x_775.w));
  let x_778 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_777.x, x_777.y, x_778.z, x_778.w);
  let x_785 : vec4<f32> = u_xlat1;
  let x_788 : f32 = x_38.x_GlobalMipBias.x;
  let x_789 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_785.x, x_785.y), x_788);
  u_xlat15 = x_789.w;
  let x_791 : f32 = u_xlat15;
  u_xlat15 = ((x_791 * 2.0f) + -1.0f);
  let x_794 : f32 = u_xlat15;
  u_xlatb1.x = (x_794 >= 0.0f);
  let x_798 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_798);
  let x_801 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_801)) + 1.0f);
  ...
  let x_816 : vec3<f32> = (vec3<f32>(x_814.x, x_814.y, x_814.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  ...
  let x_826 : vec3<f32> = (vec3<f32>(x_824.x, x_824.y, x_824.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_837 : vec3<f32> = ((vec3<f32>(x_834.x, x_834.y, x_834.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_838 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_837.x, x_837.y, x_837.z, x_838.w);
  let x_841 : vec4<f32> = u_xlat0;
  let x_843 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_841.x, x_841.y, x_841.z, x_841.x));
  ...
  let x_888 : vec3<f32> = ((vec3<f32>(x_881, x_881, x_881) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_886.x, x_886.y, x_886.z));
  let x_889 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_888.x, x_888.y, x_888.z, x_889.w);
  let x_891 : vec4<f32> = u_xlat0;
  let x_893 : vec3<f32> = (vec3<f32>(x_891.x, x_891.y, x_891.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_894 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_893.x, x_893.y, x_893.z, x_894.w);
  let x_896 : vec4<f32> = u_xlat0;
  let x_898 : vec3<f32> = (vec3<f32>(x_896.x, x_896.y, x_896.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_899 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_898.x, x_898.y, x_898.z, x_899.w);
  let x_901 : vec4<f32> = u_xlat2;
  let x_903 : vec3<f32> = (vec3<f32>(x_901.x, x_901.y, x_901.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_904 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_903.x, x_903.y, x_903.z, x_904.w);
  let x_906 : vec4<f32> = u_xlat2;
  let x_909 : vec3<f32> = log2(abs(vec3<f32>(x_906.x, x_906.y, x_906.z)));
  let x_910 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_909.x, x_909.y, x_909.z, x_910.w);
  let x_912 : vec4<f32> = u_xlat2;
  let x_914 : vec3<f32> = (vec3<f32>(x_912.x, x_912.y, x_912.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_924 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_922.x, x_922.y, x_922.z, x_922.x));
  u_xlatb0 = vec3<bool>(x_924.x, x_924.y, x_924.z);
  ...
  let x_936 : f32 = x_928;
  u_xlat0.x = x_936;
  let x_939 : bool = u_xlatb0.y;
  if (x_939) {
    let x_944 : f32 = u_xlat1.y;
    x_940 = x_944;
  } else {
  ...
  let x_948 : f32 = x_940;
  u_xlat0.y = x_948;
  let x_951 : bool = u_xlatb0.z;
  if (x_951) {
    let x_956 : f32 = u_xlat1.z;
    x_952 = x_956;
  } else {
    let x_959 : f32 = u_xlat2.z;
    x_952 = x_959;
  }
  let x_960 : f32 = x_952;
  u_xlat0.z = x_960;
  let x_964 : vec4<f32> = u_xlat0;
  let x_967 : vec3<f32> = max(vec3<f32>(x_964.x, x_964.y, x_964.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_968 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_967.x, x_967.y, x_967.z, x_968.w);
  SV_Target0.w = 1.0f;
  return;
}

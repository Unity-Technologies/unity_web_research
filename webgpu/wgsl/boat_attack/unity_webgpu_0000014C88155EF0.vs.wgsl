...
@group(1) @binding(6) var<uniform> x_186 : VGlobals;
...
fn main_1() {
  ...
  let x_188 : vec4<f32> = x_186.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_182.y, x_182.y, x_182.y, x_182.y) * x_188);
  let x_191 : vec4<f32> = x_186.unity_MatrixVP[0i];
  let x_192 : vec4<f32> = u_xlat0;
  let x_195 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_191 * vec4<f32>(x_192.x, x_192.x, x_192.x, x_192.x)) + x_195);
  let x_198 : vec4<f32> = x_186.unity_MatrixVP[2i];
  let x_199 : vec4<f32> = u_xlat0;
  let x_202 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_198 * vec4<f32>(x_199.z, x_199.z, x_199.z, x_199.z)) + x_202);
  let x_208 : vec4<f32> = u_xlat0;
  let x_210 : vec4<f32> = x_186.unity_MatrixVP[3i];
  gl_Position = (x_208 + x_210);
  return;
}

...
fn main_1() {
  ...
  vs_INTERP1 = vec3<f32>(x_455.x, x_455.y, x_455.z);
  let x_457 : vec4<f32> = u_xlat0;
  let x_460 : vec4<f32> = x_45.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_457.y, x_457.y, x_457.y, x_457.y) * x_460);
  let x_463 : vec4<f32> = x_45.unity_MatrixVP[0i];
  let x_464 : vec4<f32> = u_xlat0;
  let x_467 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_463 * vec4<f32>(x_464.x, x_464.x, x_464.x, x_464.x)) + x_467);
  let x_470 : vec4<f32> = x_45.unity_MatrixVP[2i];
  let x_471 : vec4<f32> = u_xlat0;
  let x_474 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_470 * vec4<f32>(x_471.z, x_471.z, x_471.z, x_471.z)) + x_474);
  let x_476 : vec4<f32> = u_xlat0;
  let x_478 : vec4<f32> = x_45.unity_MatrixVP[3i];
  u_xlat0 = (x_476 + x_478);
  let x_485 : f32 = u_xlat0.z;
  gl_Position.z = min(x_485, 1.0f);
  let x_489 : vec4<f32> = u_xlat0;
  let x_490 : vec3<f32> = vec3<f32>(x_489.x, x_489.y, x_489.w);
  let x_493 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_490.x, x_490.y, x_493.z, x_490.z);
  let x_497 : vec4<f32> = in_TEXCOORD0;
  vs_INTERP2 = x_497;
  return;
}

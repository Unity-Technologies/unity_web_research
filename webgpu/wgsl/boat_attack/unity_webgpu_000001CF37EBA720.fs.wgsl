...
fn main_1() {
  ...
  let x_860 : vec3<f32> = sqrt(vec3<f32>(x_858.x, x_858.y, x_858.z));
  let x_861 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_860.x, x_860.y, x_860.z, x_861.w);
  let x_863 : vec2<f32> = vs_TEXCOORD0;
  let x_866 : vec4<f32> = x_25.x_Dithering_Params;
  let x_870 : vec4<f32> = x_25.x_Dithering_Params;
  let x_872 : vec2<f32> = ((x_863 * vec2<f32>(x_866.x, x_866.y)) + vec2<f32>(x_870.z, x_870.w));
  let x_873 : vec3<f32> = u_xlat1;
  u_xlat1 = vec3<f32>(x_872.x, x_872.y, x_873.z);
  let x_880 : vec3<f32> = u_xlat1;
  let x_883 : f32 = x_25.x_GlobalMipBias.x;
  let x_884 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_880.x, x_880.y), x_883);
  u_xlat15 = x_884.w;
  let x_886 : f32 = u_xlat15;
  u_xlat15 = ((x_886 * 2.0f) + -1.0f);
  let x_890 : f32 = u_xlat15;
  u_xlatb1.x = (x_890 >= 0.0f);
  let x_894 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_894);
  let x_897 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_897)) + 1.0f);
  let x_901 : f32 = u_xlat15;
  u_xlat15 = sqrt(x_901);
  let x_903 : f32 = u_xlat15;
  u_xlat15 = (-(x_903) + 1.0f);
  let x_906 : f32 = u_xlat15;
  let x_908 : f32 = u_xlat1.x;
  u_xlat15 = (x_906 * x_908);
  ...
  let x_920 : vec3<f32> = (vec3<f32>(x_918.x, x_918.y, x_918.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_931 : vec3<f32> = ((vec3<f32>(x_928.x, x_928.y, x_928.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_932 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_931.x, x_931.y, x_931.z, x_932.w);
  let x_935 : vec4<f32> = u_xlat0;
  let x_937 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_935.x, x_935.y, x_935.z, x_935.x));
  u_xlatb0 = vec3<bool>(x_937.x, x_937.y, x_937.z);
  ...
  let x_949 : f32 = x_941;
  u_xlat0.x = x_949;
  let x_952 : bool = u_xlatb0.y;
  if (x_952) {
    let x_957 : f32 = u_xlat1.y;
    x_953 = x_957;
  } else {
    let x_960 : f32 = u_xlat2.y;
    x_953 = x_960;
  }
  let x_961 : f32 = x_953;
  u_xlat0.y = x_961;
  let x_964 : bool = u_xlatb0.z;
  if (x_964) {
    let x_969 : f32 = u_xlat1.z;
    x_965 = x_969;
  } else {
  ...
  let x_973 : f32 = x_965;
  u_xlat0.z = x_973;
  let x_975 : f32 = u_xlat15;
  let x_980 : vec4<f32> = u_xlat0;
  let x_982 : vec3<f32> = ((vec3<f32>(x_975, x_975, x_975) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_980.x, x_980.y, x_980.z));
  let x_983 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_982.x, x_982.y, x_982.z, x_983.w);
  let x_985 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec3<f32>(x_985.x, x_985.y, x_985.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_988 : vec4<f32> = u_xlat0;
  let x_990 : vec3<f32> = (vec3<f32>(x_988.x, x_988.y, x_988.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_991 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_990.x, x_990.y, x_990.z, x_991.w);
  let x_993 : vec4<f32> = u_xlat2;
  let x_995 : vec3<f32> = (vec3<f32>(x_993.x, x_993.y, x_993.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  ...
  let x_1006 : vec3<f32> = (vec3<f32>(x_1004.x, x_1004.y, x_1004.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1016 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1014.x, x_1014.y, x_1014.z, x_1014.x));
  ...
  let x_1059 : vec3<f32> = max(vec3<f32>(x_1056.x, x_1056.y, x_1056.z), vec3<f32>(0.0f, 0.0f, 0.0f));
  let x_1060 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1059.x, x_1059.y, x_1059.z, x_1060.w);
  SV_Target0.w = 1.0f;
  return;
}

...
;
    ...
    u_xlat5.xyz = unity_SHC.xyz * u_xlat0.xxx + u_xlat6.xyz;
    ...
    if(u_xlatb34){
        ...
        u_xlat34 = (u_xlatb5.y) ? u_xlat3.w : u_xlat3.x;
        u_xlat3.xyz = (u_xlatb5.x) ? u_xlat3.xyz : vec3(u_xlat34);
    } else {
        u_xlat3.x = float(1.0);
        u_xlat3.y = float(1.0);
        u_xlat3.z = float(1.0);
    ...
    for(uint u_xlatu_loop_1 = 0u ; u_xlatu_loop_1<u_xlatu0 ; u_xlatu_loop_1++)
    {
        ...
        if(u_xlatb27){
            ...
            if(u_xlatb27){
                ...
                u_xlat38 = trunc(_AdditionalShadowParams[u_xlati35].w);
                u_xlat27.x = u_xlat27.x + u_xlat38;
                u_xlati37 = int(u_xlat27.x);
            }
            u_xlati37 = u_xlati37 << (2 & int(0x1F));
            u_xlat8 = vs_TEXCOORD2.yyyy * _AdditionalLightsWorldToShadow[(u_xlati37 + 1) / 4][(u_xlati37 + 1) % 4];
            u_xlat8 = _AdditionalLightsWorldToShadow[u_xlati37 / 4][u_xlati37 % 4] * vs_TEXCOORD2.xxxx + u_xlat8;
            u_xlat8 = _AdditionalLightsWorldToShadow[(u_xlati37 + 2) / 4][(u_xlati37 + 2) % 4] * vs_TEXCOORD2.zzzz + u_xlat8;
        ...
        u_xlati27 = 1 << (u_xlati35 & int(0x1F));
        u_xlati27 = int(uint(u_xlati27) & floatBitsToUint(_AdditionalLightsCookieEnableBits));
        if(u_xlati27 != 0) {

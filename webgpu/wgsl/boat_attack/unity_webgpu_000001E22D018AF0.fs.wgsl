...
fn main_1() {
  ...
  u_xlat1 = (vec3<f32>(x_699.x, x_699.y, x_699.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  let x_702 : vec4<f32> = u_xlat0;
  let x_705 : vec3<f32> = log2(abs(vec3<f32>(x_702.x, x_702.y, x_702.z)));
  let x_706 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_705.x, x_705.y, x_705.z, x_706.w);
  let x_708 : vec4<f32> = u_xlat2;
  let x_710 : vec3<f32> = (vec3<f32>(x_708.x, x_708.y, x_708.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_721 : vec3<f32> = ((vec3<f32>(x_718.x, x_718.y, x_718.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_722 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_721.x, x_721.y, x_721.z, x_722.w);
  let x_725 : vec4<f32> = u_xlat0;
  let x_727 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_725.x, x_725.y, x_725.z, x_725.x));
  u_xlatb0 = vec3<bool>(x_727.x, x_727.y, x_727.z);
  ...
  let x_754 : f32 = x_746;
  SV_Target0.y = x_754;
  let x_757 : bool = u_xlatb0.z;
  if (x_757) {
    let x_762 : f32 = u_xlat1.z;
    x_758 = x_762;
  } else {
    let x_765 : f32 = u_xlat2.z;
    x_758 = x_765;
  }
  let x_766 : f32 = x_758;
  SV_Target0.z = x_766;

...
fn main_1() {
  ...
  let x_906 : vec3<f32> = sqrt(vec3<f32>(x_904.x, x_904.y, x_904.z));
  let x_907 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_906.x, x_906.y, x_906.z, x_907.w);
  let x_909 : vec2<f32> = vs_TEXCOORD0;
  let x_912 : vec4<f32> = x_25.x_Dithering_Params;
  let x_916 : vec4<f32> = x_25.x_Dithering_Params;
  ...
  let x_929 : f32 = x_25.x_GlobalMipBias.x;
  let x_930 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_926.x, x_926.y), x_929);
  u_xlat15 = x_930.w;
  let x_932 : f32 = u_xlat15;
  u_xlat15 = ((x_932 * 2.0f) + -1.0f);
  let x_936 : f32 = u_xlat15;
  u_xlatb1.x = (x_936 >= 0.0f);
  let x_940 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_940);
  let x_943 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_943)) + 1.0f);
  ...
  u_xlat15 = (x_952 * x_954);
  let x_956 : vec4<f32> = u_xlat0;
  let x_958 : vec3<f32> = (vec3<f32>(x_956.x, x_956.y, x_956.z) * vec3<f32>(12.9200000762939453125f, 12.9200000762939453125f, 12.9200000762939453125f));
  let x_959 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_958.x, x_958.y, x_958.z, x_959.w);
  let x_961 : vec4<f32> = u_xlat0;
  let x_963 : vec3<f32> = log2(vec3<f32>(x_961.x, x_961.y, x_961.z));
  let x_964 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_963.x, x_963.y, x_963.z, x_964.w);
  let x_966 : vec4<f32> = u_xlat2;
  let x_968 : vec3<f32> = (vec3<f32>(x_966.x, x_966.y, x_966.z) * vec3<f32>(0.4166666567325592041f, 0.4166666567325592041f, 0.4166666567325592041f));
  ...
  let x_979 : vec3<f32> = ((vec3<f32>(x_976.x, x_976.y, x_976.z) * vec3<f32>(1.05499994754791259766f, 1.05499994754791259766f, 1.05499994754791259766f)) + vec3<f32>(-0.05499999970197677612f, -0.05499999970197677612f, -0.05499999970197677612f));
  let x_980 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_979.x, x_979.y, x_979.z, x_980.w);
  let x_983 : vec4<f32> = u_xlat0;
  let x_985 : vec4<bool> = (vec4<f32>(0.00313080009073019028f, 0.00313080009073019028f, 0.00313080009073019028f, 0.0f) >= vec4<f32>(x_983.x, x_983.y, x_983.z, x_983.x));
  ...
  let x_1030 : vec3<f32> = ((vec3<f32>(x_1023, x_1023, x_1023) * vec3<f32>(0.0039215688593685627f, 0.0039215688593685627f, 0.0039215688593685627f)) + vec3<f32>(x_1028.x, x_1028.y, x_1028.z));
  let x_1031 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1030.x, x_1030.y, x_1030.z, x_1031.w);
  let x_1033 : vec4<f32> = u_xlat0;
  let x_1035 : vec3<f32> = (vec3<f32>(x_1033.x, x_1033.y, x_1033.z) * vec3<f32>(0.07739938050508499146f, 0.07739938050508499146f, 0.07739938050508499146f));
  let x_1036 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_1035.x, x_1035.y, x_1035.z, x_1036.w);
  let x_1038 : vec4<f32> = u_xlat0;
  let x_1040 : vec3<f32> = (vec3<f32>(x_1038.x, x_1038.y, x_1038.z) + vec3<f32>(0.05499999970197677612f, 0.05499999970197677612f, 0.05499999970197677612f));
  let x_1041 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1040.x, x_1040.y, x_1040.z, x_1041.w);
  let x_1043 : vec4<f32> = u_xlat2;
  let x_1045 : vec3<f32> = (vec3<f32>(x_1043.x, x_1043.y, x_1043.z) * vec3<f32>(0.94786733388900756836f, 0.94786733388900756836f, 0.94786733388900756836f));
  let x_1046 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1045.x, x_1045.y, x_1045.z, x_1046.w);
  let x_1048 : vec4<f32> = u_xlat2;
  let x_1051 : vec3<f32> = log2(abs(vec3<f32>(x_1048.x, x_1048.y, x_1048.z)));
  let x_1052 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1051.x, x_1051.y, x_1051.z, x_1052.w);
  let x_1054 : vec4<f32> = u_xlat2;
  let x_1056 : vec3<f32> = (vec3<f32>(x_1054.x, x_1054.y, x_1054.z) * vec3<f32>(2.40000009536743164062f, 2.40000009536743164062f, 2.40000009536743164062f));
  ...
  let x_1066 : vec4<bool> = (vec4<f32>(0.04044999927282333374f, 0.04044999927282333374f, 0.04044999927282333374f, 0.0f) >= vec4<f32>(x_1064.x, x_1064.y, x_1064.z, x_1064.x));
  u_xlatb0 = vec3<bool>(x_1066.x, x_1066.y, x_1066.z);
  let x_1069 : bool = u_xlatb0.x;
  if (x_1069) {
  ...
  u_xlat0.x = x_1078;
  let x_1081 : bool = u_xlatb0.y;
  if (x_1081) {
    let x_1086 : f32 = u_xlat1.y;
    x_1082 = x_1086;
  } else {
    let x_1089 : f32 = u_xlat2.y;
    x_1082 = x_1089;
  }
  let x_1090 : f32 = x_1082;
  u_xlat0.y = x_1090;
  let x_1093 : bool = u_xlatb0.z;
  if (x_1093) {
    let x_1098 : f32 = u_xlat1.z;
    x_1094 = x_1098;
  } else {
    let x_1101 : f32 = u_xlat2.z;
    x_1094 = x_1101;
  }
  let x_1102 : f32 = x_1094;
  u_xlat0.z = x_1102;
  let x_1106 : vec4<f32> = u_xlat0;
  let x_1109 : vec3<f32> = max(vec3<f32>(x_1106.x, x_1106.y, x_1106.z), vec3<f32>(0.0f, 0.0f, 0.0f));

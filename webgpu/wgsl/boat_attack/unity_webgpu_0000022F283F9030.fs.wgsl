...
fn main_1() {
  ...
  let x_555 : vec3<f32> = sqrt(vec3<f32>(x_553.x, x_553.y, x_553.z));
  let x_556 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_555.x, x_555.y, x_555.z, x_556.w);
  let x_558 : vec2<f32> = vs_TEXCOORD0;
  let x_561 : vec4<f32> = x_26.x_Dithering_Params;
  let x_565 : vec4<f32> = x_26.x_Dithering_Params;
  ...
  let x_578 : f32 = x_26.x_GlobalMipBias.x;
  let x_579 : vec4<f32> = textureSampleBias(x_BlueNoise_Texture, sampler_PointRepeat, vec2<f32>(x_575.x, x_575.y), x_578);
  u_xlat15 = x_579.w;
  let x_581 : f32 = u_xlat15;
  u_xlat15 = ((x_581 * 2.0f) + -1.0f);
  let x_586 : f32 = u_xlat15;
  u_xlatb1.x = (x_586 >= 0.0f);
  let x_590 : bool = u_xlatb1.x;
  u_xlat1.x = select(-1.0f, 1.0f, x_590);
  let x_593 : f32 = u_xlat15;
  u_xlat15 = (-(abs(x_593)) + 1.0f);
  ...
  u_xlat15 = (x_602 * x_604);
  let x_606 : vec4<f32> = u_xlat0;
  let x_608 : vec3<f32> = (vec3<f32>(x_606.x, x_606.y, x_606.z) * vec3<f32>(12.920000076f, 12.920000076f, 12.920000076f));
  ...
  let x_618 : vec3<f32> = (vec3<f32>(x_616.x, x_616.y, x_616.z) * vec3<f32>(0.416666657f, 0.416666657f, 0.416666657f));
  ...
  let x_629 : vec3<f32> = ((vec3<f32>(x_626.x, x_626.y, x_626.z) * vec3<f32>(1.054999948f, 1.054999948f, 1.054999948f)) + vec3<f32>(-0.055f, -0.055f, -0.055f));
  let x_630 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_629.x, x_629.y, x_629.z, x_630.w);
  let x_633 : vec4<f32> = u_xlat0;
  let x_635 : vec4<bool> = (vec4<f32>(0.0031308f, 0.0031308f, 0.0031308f, 0.0f) >= vec4<f32>(x_633.x, x_633.y, x_633.z, x_633.x));
  ...
  let x_680 : vec3<f32> = ((vec3<f32>(x_673, x_673, x_673) * vec3<f32>(0.003921569f, 0.003921569f, 0.003921569f)) + vec3<f32>(x_678.x, x_678.y, x_678.z));
  let x_681 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_680.x, x_680.y, x_680.z, x_681.w);
  let x_683 : vec4<f32> = u_xlat0;
  let x_685 : vec3<f32> = (vec3<f32>(x_683.x, x_683.y, x_683.z) * vec3<f32>(0.077399381f, 0.077399381f, 0.077399381f));
  let x_686 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_685.x, x_685.y, x_685.z, x_686.w);
  let x_688 : vec4<f32> = u_xlat0;
  let x_690 : vec3<f32> = (vec3<f32>(x_688.x, x_688.y, x_688.z) + vec3<f32>(0.055f, 0.055f, 0.055f));
  let x_691 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_690.x, x_690.y, x_690.z, x_691.w);
  let x_693 : vec4<f32> = u_xlat2;
  let x_695 : vec3<f32> = (vec3<f32>(x_693.x, x_693.y, x_693.z) * vec3<f32>(0.947867334f, 0.947867334f, 0.947867334f));
  let x_696 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_695.x, x_695.y, x_695.z, x_696.w);
  let x_698 : vec4<f32> = u_xlat2;
  let x_701 : vec3<f32> = log2(abs(vec3<f32>(x_698.x, x_698.y, x_698.z)));
  let x_702 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_701.x, x_701.y, x_701.z, x_702.w);
  let x_704 : vec4<f32> = u_xlat2;
  let x_706 : vec3<f32> = (vec3<f32>(x_704.x, x_704.y, x_704.z) * vec3<f32>(2.400000095f, 2.400000095f, 2.400000095f));
  ...
  let x_716 : vec4<bool> = (vec4<f32>(0.040449999f, 0.040449999f, 0.040449999f, 0.0f) >= vec4<f32>(x_714.x, x_714.y, x_714.z, x_714.x));
  ...
  let x_759 : vec3<f32> = max(vec3<f32>(x_756.x, x_756.y, x_756.z), vec3<f32>(0.0f, 0.0f, 0.0f));

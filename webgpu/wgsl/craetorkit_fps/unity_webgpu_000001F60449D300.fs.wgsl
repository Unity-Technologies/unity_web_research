...
fn main_1() {
  ...
  let x_378 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_377.x, x_377.y, x_377.z, x_378.w);
  let x_381 : vec4<f32> = u_xlat1;
  let x_385 : vec3<f32> = ((vec3<f32>(x_381.x, x_381.y, x_381.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_386 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_385.x, x_385.y, x_385.z, x_386.w);
  let x_389 : vec4<f32> = u_xlat0;
  SV_Target0 = x_389;
  let x_392 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_392;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

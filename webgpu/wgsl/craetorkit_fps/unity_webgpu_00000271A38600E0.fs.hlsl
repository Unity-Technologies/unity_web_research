...
c . a = 0.0 ;
float unityFogFactor = unity_FogParams . x * ( max ( ( ( 1.0 - ( ( _unity_fogCoord ) . x ) / _ProjectionParams . y ) * _ProjectionParams . z ) , 0 ) ) ; unityFogFactor = exp2 ( - unityFogFactor * unityFogFactor ) ; c . rgb = lerp ( ( float4 ( 0 , 0 , 0 , 0 ) ) . rgb , ( c ) . rgb , saturate ( unityFogFactor ) ) ;
c . a = 1.0 ;
return c ;
}

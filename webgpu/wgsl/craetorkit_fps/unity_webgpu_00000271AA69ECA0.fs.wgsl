...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_518.w, x_518.x, x_518.y, x_518.z) * vec4<f32>(x_529.x, x_529.y, x_529.z, x_529.w));
  let x_536 : vec4<f32> = u_xlat0;
  let x_543 : vec3<f32> = ((vec3<f32>(x_536.y, x_536.z, x_536.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_588 : vec3<f32> = ((vec3<f32>(x_580.x, x_580.y, x_580.z) * vec3<f32>(x_583.x, x_583.x, x_583.x)) + vec3<f32>(x_586, x_586, x_586));
  ...
  u_xlat0 = vec4<f32>(x_603.x, x_602.x, x_602.y, x_602.z);
  let x_607 : f32 = x_49.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_607);
  let x_609 : bool = u_xlatb1;
  if (x_609) {
    let x_612 : vec4<f32> = u_xlat0;
    let x_613 : vec3<f32> = vec3<f32>(x_612.y, x_612.z, x_612.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_623.x, x_623.y, x_623.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_629 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_629.y, x_629.z, x_629.w, x_629.x);
  return;
}

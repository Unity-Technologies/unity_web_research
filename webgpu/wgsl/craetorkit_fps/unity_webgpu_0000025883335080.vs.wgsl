...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_604.y, x_604.w, x_604.z, x_604.x) * x_606);
  let x_609 : vec4<f32> = x_186.unity_SHBr;
  let x_610 : vec4<f32> = u_xlat0;
  u_xlat2.x = dot(x_609, x_610);
  let x_614 : vec4<f32> = x_186.unity_SHBg;
  let x_615 : vec4<f32> = u_xlat0;
  u_xlat2.y = dot(x_614, x_615);
  let x_619 : vec4<f32> = x_186.unity_SHBb;
  let x_620 : vec4<f32> = u_xlat0;
  u_xlat2.z = dot(x_619, x_620);
  let x_626 : vec4<f32> = x_186.unity_SHC;
  let x_628 : vec2<f32> = u_xlat1;

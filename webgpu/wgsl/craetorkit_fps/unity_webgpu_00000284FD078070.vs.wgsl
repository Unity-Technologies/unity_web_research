...
@group(1) @binding(0) var<uniform> x_260 : VGlobals;
...
fn main_1() {
  ...
  if (x_321) {
    ...
    x_322 = vec3<f32>(x_328.x, x_328.y, x_328.z);
  }
  let x_330 : vec3<f32> = x_322;
  let x_331 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_330.x, x_330.y, x_330.z, x_331.w);
  let x_333 : vec4<f32> = u_xlat0;
  let x_336 : vec4<f32> = x_260.unity_MatrixVP[1i];
  u_xlat1 = (vec4<f32>(x_333.y, x_333.y, x_333.y, x_333.y) * x_336);
  let x_339 : vec4<f32> = x_260.unity_MatrixVP[0i];
  let x_340 : vec4<f32> = u_xlat0;
  let x_343 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_339 * vec4<f32>(x_340.x, x_340.x, x_340.x, x_340.x)) + x_343);
  let x_346 : vec4<f32> = x_260.unity_MatrixVP[2i];
  let x_347 : vec4<f32> = u_xlat0;
  let x_350 : vec4<f32> = u_xlat1;
  u_xlat0 = ((x_346 * vec4<f32>(x_347.z, x_347.z, x_347.z, x_347.z)) + x_350);
  let x_353 : vec4<f32> = x_260.unity_MatrixVP[3i];
  let x_354 : vec4<f32> = in_POSITION0;
  let x_357 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_353 * vec4<f32>(x_354.w, x_354.w, x_354.w, x_354.w)) + x_357);
  let x_360 : f32 = x_260.unity_LightShadowBias.x;
  let x_362 : f32 = u_xlat0.w;
  u_xlat1.x = (x_360 / x_362);
  let x_366 : f32 = u_xlat1.x;
  u_xlat1.x = min(x_366, 0.0f);
  let x_370 : f32 = u_xlat1.x;
  u_xlat1.x = max(x_370, -1.0f);
  let x_376 : f32 = u_xlat0.z;
  let x_378 : f32 = u_xlat1.x;
  u_xlat14 = (x_376 + x_378);
  let x_381 : f32 = u_xlat0.w;
  let x_382 : f32 = u_xlat14;
  u_xlat1.x = min(x_381, x_382);
  let x_389 : vec4<f32> = u_xlat0;
  let x_390 : vec3<f32> = vec3<f32>(x_389.x, x_389.y, x_389.w);
  let x_393 : vec4<f32> = gl_Position;
  gl_Position = vec4<f32>(x_390.x, x_390.y, x_393.z, x_390.z);
  let x_395 : f32 = u_xlat14;
  let x_398 : f32 = u_xlat1.x;
  u_xlat0.x = (-(x_395) + x_398);
  let x_402 : f32 = x_260.unity_LightShadowBias.y;
  let x_404 : f32 = u_xlat0.x;
  let x_406 : f32 = u_xlat14;
  gl_Position.z = ((x_402 * x_404) + x_406);
  return;
}

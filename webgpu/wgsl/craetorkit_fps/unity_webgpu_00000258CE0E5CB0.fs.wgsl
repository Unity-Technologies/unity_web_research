...
fn main_1() {
  ...
  u_xlat1 = ((vec3<f32>(x_1387.z, x_1387.x, x_1387.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  let x_1395 : vec3<f32> = u_xlat1;
  u_xlat1 = log2(x_1395);
  let x_1397 : vec3<f32> = u_xlat1;
  u_xlat1 = ((x_1397 * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_1404 : vec3<f32> = u_xlat1;
  u_xlat1 = clamp(x_1404, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_1408 : vec3<f32> = u_xlat1;
  let x_1411 : vec3<f32> = x_26.x_Lut2D_Params;
  u_xlat11 = (x_1408 * vec3<f32>(x_1411.z, x_1411.z, x_1411.z));
  let x_1415 : f32 = u_xlat11.x;
  u_xlat11.x = floor(x_1415);
  let x_1419 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1421 : vec2<f32> = (vec2<f32>(x_1419.x, x_1419.y) * vec2<f32>(0.5f, 0.5f));
  let x_1422 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1421.x, x_1421.y, x_1422.z, x_1422.w);
  let x_1424 : vec3<f32> = u_xlat11;
  let x_1427 : vec3<f32> = x_26.x_Lut2D_Params;
  let x_1430 : vec4<f32> = u_xlat2;
  let x_1432 : vec2<f32> = ((vec2<f32>(x_1424.y, x_1424.z) * vec2<f32>(x_1427.x, x_1427.y)) + vec2<f32>(x_1430.x, x_1430.y));
  let x_1433 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1433.x, x_1432.x, x_1432.y, x_1433.w);
  let x_1436 : f32 = u_xlat11.x;
  let x_1438 : f32 = x_26.x_Lut2D_Params.y;
  let x_1441 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_1436 * x_1438) + x_1441);
  let x_1449 : vec4<f32> = u_xlat2;
  let x_1451 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_1449.x, x_1449.z));
  let x_1452 : vec3<f32> = vec3<f32>(x_1451.x, x_1451.y, x_1451.z);
  let x_1453 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_1452.x, x_1452.y, x_1452.z, x_1453.w);
  let x_1456 : f32 = x_26.x_Lut2D_Params.y;
  u_xlat4.x = x_1456;
  u_xlat4.y = 0.0f;
  let x_1459 : vec4<f32> = u_xlat2;
  let x_1461 : vec4<f32> = u_xlat4;
  u_xlat21 = (vec2<f32>(x_1459.x, x_1459.z) + vec2<f32>(x_1461.x, x_1461.y));
  let x_1467 : vec2<f32> = u_xlat21;
  let x_1468 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_1467);
  let x_1469 : vec3<f32> = vec3<f32>(x_1468.x, x_1468.y, x_1468.z);
  let x_1470 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_1469.x, x_1469.y, x_1469.z, x_1470.w);
  let x_1473 : f32 = u_xlat1.x;
  let x_1475 : f32 = x_26.x_Lut2D_Params.z;
  let x_1478 : f32 = u_xlat11.x;
  u_xlat1.x = ((x_1473 * x_1475) + -(x_1478));
  let x_1482 : vec4<f32> = u_xlat3;
  let x_1485 : vec4<f32> = u_xlat2;
  u_xlat11 = (-(vec3<f32>(x_1482.x, x_1482.y, x_1482.z)) + vec3<f32>(x_1485.x, x_1485.y, x_1485.z));
  let x_1488 : vec3<f32> = u_xlat1;
  let x_1490 : vec3<f32> = u_xlat11;
  let x_1492 : vec4<f32> = u_xlat3;
  let x_1494 : vec3<f32> = ((vec3<f32>(x_1488.x, x_1488.x, x_1488.x) * x_1490) + vec3<f32>(x_1492.x, x_1492.y, x_1492.z));
  let x_1495 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1494.x, x_1494.y, x_1494.z, x_1495.w);
  let x_1499 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1499);
  let x_1501 : bool = u_xlatb1;
  if (x_1501) {
    let x_1504 : vec4<f32> = u_xlat0;
    u_xlat1 = vec3<f32>(x_1504.x, x_1504.y, x_1504.z);
    let x_1506 : vec3<f32> = u_xlat1;
    u_xlat1 = clamp(x_1506, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1510 : vec3<f32> = u_xlat1;
    u_xlat0.w = dot(x_1510, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

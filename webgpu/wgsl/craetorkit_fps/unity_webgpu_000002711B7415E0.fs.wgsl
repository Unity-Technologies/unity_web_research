...
fn main_1() {
  ...
  let x_344 : vec3<f32> = vs_TEXCOORD7;
  ...
  let x_389 : vec3<f32> = (vec3<f32>(x_386.x, x_386.y, x_386.z) * x_388);
  let x_390 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_389.x, x_389.y, x_389.z, x_390.w);
  let x_393 : vec4<f32> = u_xlat2;
  let x_397 : vec3<f32> = ((vec3<f32>(x_393.x, x_393.y, x_393.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_398 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_397.x, x_397.y, x_397.z, x_398.w);
  let x_401 : vec3<f32> = u_xlat0;
  let x_402 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_401.x, x_401.y, x_401.z, x_402.w);
  SV_Target0.w = 1.0f;
  let x_408 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_408;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
...
@fragment
fn main(@location(4) vs_TEXCOORD3_param : vec2<f32>, @location(3) vs_COLOR0_param : vec4<f32>, @location(0) vs_TEXCOORD0_param : vec4<f32>, @location(1) vs_TEXCOORD1_param : vec4<f32>, @location(2) vs_TEXCOORD2_param : vec4<f32>, @location(5) vs_TEXCOORD7_param : vec3<f32>, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

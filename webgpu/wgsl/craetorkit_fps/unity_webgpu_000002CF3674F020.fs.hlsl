...
#line 33 "D:/src/unity_webgpu/Shaders/Includes/Internal/UnityUIE.cginc"
...
float4 settingIndex : TEXCOORD5 ;
float4 circle : TEXCOORD6 ;
float textureId : TEXCOORD7 ;
#line 103
} ;
...
nointerpolation float4 typeTexSettings : TEXCOORD1 ;
nointerpolation float2 colorUVs : TEXCOORD3 ;
...
sd = ( sd - 0.5 ) * sdfScale + isoPerimeter ;
return saturate ( 0.5 + 2.0 * sd / ( stsr / ( sharpness + 1.0f ) + softness ) ) ;
}

float4 uie_textcore ( float textAlpha , float2 uv , float textureSlot , float2 textCoreUV , float4 vertexColor , float opacity , float sdfScale , bool isDynamicColor , float sharpness , float extraDilate )
{
...
{
color = uie_textcore ( textAlpha , uv , textureSlot , IN . colorUVs . xy , IN . color , textOpacity , info . sdfScale , isDynamicColor , info . sharpness , IN . circle . x ) ;
}
else
{
color = IN . color ;

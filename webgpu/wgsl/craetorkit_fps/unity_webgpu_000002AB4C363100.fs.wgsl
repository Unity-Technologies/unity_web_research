...
fn main_1() {
  ...
  let x_744 : f32 = x_32.x_PostExposure;
  let x_746 : f32 = x_32.x_PostExposure;
  let x_748 : f32 = x_32.x_PostExposure;
  let x_750 : f32 = x_32.x_PostExposure;
  let x_751 : vec4<f32> = vec4<f32>(x_744, x_746, x_748, x_750);
  u_xlat0 = (vec4<f32>(x_740.w, x_740.x, x_740.y, x_740.z) * vec4<f32>(x_751.x, x_751.y, x_751.z, x_751.w));
  let x_758 : vec4<f32> = u_xlat0;
  let x_765 : vec3<f32> = ((vec3<f32>(x_758.y, x_758.z, x_758.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_794 : vec2<f32> = x_32.x_Lut3D_Params;
  let x_796 : vec3<f32> = (vec3<f32>(x_790.x, x_790.y, x_790.z) * vec3<f32>(x_794.y, x_794.y, x_794.y));
  let x_797 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_796.x, x_796.y, x_796.z, x_797.w);
  let x_801 : f32 = x_32.x_Lut3D_Params.x;
  u_xlat16 = (x_801 * 0.5f);
  let x_803 : vec4<f32> = u_xlat1;
  let x_806 : vec2<f32> = x_32.x_Lut3D_Params;
  let x_809 : f32 = u_xlat16;
  let x_811 : vec3<f32> = ((vec3<f32>(x_803.x, x_803.y, x_803.z) * vec3<f32>(x_806.x, x_806.x, x_806.x)) + vec3<f32>(x_809, x_809, x_809));
  let x_812 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_811.x, x_811.y, x_811.z, x_812.w);
  let x_822 : vec4<f32> = u_xlat1;
  let x_824 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_822.x, x_822.y, x_822.z));
  let x_825 : vec3<f32> = vec3<f32>(x_824.x, x_824.y, x_824.z);
  let x_826 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_826.x, x_825.x, x_825.y, x_825.z);
  let x_831 : f32 = x_32.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_831);
  let x_833 : bool = u_xlatb1;
  if (x_833) {
    let x_836 : vec4<f32> = u_xlat0;
    let x_837 : vec3<f32> = vec3<f32>(x_836.y, x_836.z, x_836.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_847.x, x_847.y, x_847.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_853 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_853.y, x_853.z, x_853.w, x_853.x);
  return;
}

...
#line 6 "D:/unity/fps_webgpu/Library/PackageCache/com.unity.postprocessing@3.2.2/PostProcessing/Shaders/Builtins/SubpixelMorphologicalAntialiasingBridge.hlsl"
Texture2D _MainTex ; SamplerState sampler_MainTex ;
Texture2D _BlendTex ; SamplerState sampler_BlendTex ;
Texture2D _AreaTex ; SamplerState sampler_AreaTex ;
Texture2D _SearchTex ; SamplerState sampler_SearchTex ;
...
#line 617 "D:/unity/fps_webgpu/Library/PackageCache/com.unity.postprocessing@3.2.2/PostProcessing/Shaders/Builtins/SubpixelMorphologicalAntialiasing.hlsl"
float3 SMAAGatherNeighbours ( float2 texcoord ,
float4 offset [ 3 ] ,
Texture2D tex ) {
#line 623
float P = tex . Sample ( sampler_MainTex , texcoord ) . r ;
float Pleft = tex . Sample ( sampler_MainTex , offset [ 0 ] . xy ) . r ;
float Ptop = tex . Sample ( sampler_MainTex , offset [ 0 ] . zw ) . r ;
...
out float4 offset [ 3 ] ) {
offset [ 0 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 1.0 , 0.0 , 0.0 , - 1.0 ) , texcoord . xyxy ) ;
offset [ 1 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( 1.0 , 0.0 , 0.0 , 1.0 ) , texcoord . xyxy ) ;
offset [ 2 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 2.0 , 0.0 , 0.0 , - 2.0 ) , texcoord . xyxy ) ;
}
#line 673
void SMAABlendingWeightCalculationVS ( float2 texcoord ,
out float2 pixcoord ,
out float4 offset [ 3 ] ) {
pixcoord = texcoord * _MainTex_TexelSize . zw ;
#line 679
offset [ 0 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 0.25 , - 0.125 , 1.25 , - 0.125 ) , texcoord . xyxy ) ;
offset [ 1 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 0.125 , - 0.25 , - 0.125 , 1.25 ) , texcoord . xyxy ) ;
#line 683
offset [ 2 ] = mad ( _MainTex_TexelSize . xxyy ,
...
out float4 offset ) {
offset = mad ( _MainTex_TexelSize . xyxy , float4 ( 1.0 , 0.0 , 0.0 , 1.0 ) , texcoord . xyxy ) ;
}
#line 707
...
float L = dot ( colorTex . Sample ( sampler_MainTex , texcoord ) . rgb , weights ) ;

float Lleft = dot ( colorTex . Sample ( sampler_MainTex , offset [ 0 ] . xy ) . rgb , weights ) ;
float Ltop = dot ( colorTex . Sample ( sampler_MainTex , offset [ 0 ] . zw ) . rgb , weights ) ;
#line 729
float4 delta ;
...
discard ;
#line 738
float Lright = dot ( colorTex . Sample ( sampler_MainTex , offset [ 1 ] . xy ) . rgb , weights ) ;
float Lbottom = dot ( colorTex . Sample ( sampler_MainTex , offset [ 1 ] . zw ) . rgb , weights ) ;
delta . zw = abs ( L - float2 ( Lright , Lbottom ) ) ;
#line 743
float2 maxDelta = max ( delta . xy , delta . zw ) ;
#line 746
float Lleftleft = dot ( colorTex . Sample ( sampler_MainTex , offset [ 2 ] . xy ) . rgb , weights ) ;
float Ltoptop = dot ( colorTex . Sample ( sampler_MainTex , offset [ 2 ] . zw ) . rgb , weights ) ;
...
#line 783
float4 delta ;
float3 C = colorTex . Sample ( sampler_MainTex , texcoord ) . rgb ;

float3 Cleft = colorTex . Sample ( sampler_MainTex , offset [ 0 ] . xy ) . rgb ;
float3 t = abs ( C - Cleft ) ;
delta . x = max ( max ( t . r , t . g ) , t . b ) ;

float3 Ctop = colorTex . Sample ( sampler_MainTex , offset [ 0 ] . zw ) . rgb ;
t = abs ( C - Ctop ) ;
...
discard ;
#line 802
float3 Cright = colorTex . Sample ( sampler_MainTex , offset [ 1 ] . xy ) . rgb ;
t = abs ( C - Cright ) ;
delta . z = max ( max ( t . r , t . g ) , t . b ) ;

float3 Cbottom = colorTex . Sample ( sampler_MainTex , offset [ 1 ] . zw ) . rgb ;
t = abs ( C - Cbottom ) ;
delta . w = max ( max ( t . r , t . g ) , t . b ) ;
#line 811
float2 maxDelta = max ( delta . xy , delta . zw ) ;
#line 814
float3 Cleftleft = colorTex . Sample ( sampler_MainTex , offset [ 2 ] . xy ) . rgb ;
t = abs ( Cleft - Cleftleft ) ;
delta . z = max ( max ( t . r , t . g ) , t . b ) ;

float3 Ctoptop = colorTex . Sample ( sampler_MainTex , offset [ 2 ] . zw ) . rgb ;
t = abs ( Ctop - Ctoptop ) ;
delta . w = max ( max ( t . r , t . g ) , t . b ) ;
#line 823
maxDelta = max ( maxDelta . xy , delta . zw ) ;
...
e = edgesTex . SampleLevel ( sampler_MainTex , coord . xy , 0 ) . rg ;
...
#line 935
return areaTex . SampleLevel ( sampler_MainTex , texcoord , 0 ) . rg ;
}
#line 941
...
float4 c ;
c . xy = edgesTex . SampleLevel ( sampler_MainTex , coords . xy , 0 , int2 ( - 1 , 0 ) ) . rg ;
c . zw = edgesTex . SampleLevel ( sampler_MainTex , coords . zw , 0 , int2 ( 1 , 0 ) ) . rg ;
...
if ( edgesTex . SampleLevel ( sampler_MainTex , texcoord , 0 , int2 ( 1 , 0 ) ) . r > 0.0 ) {
...
float4 c ;
c . x = edgesTex . SampleLevel ( sampler_MainTex , coords . xy , 0 , int2 ( - 1 , 0 ) ) . g ;
c . y = edgesTex . SampleLevel ( sampler_MainTex , coords . xy , 0 , int2 ( 0 , - 1 ) ) . r ;
c . zw = edgesTex . SampleLevel ( sampler_MainTex , coords . zw , 0 , int2 ( 1 , 0 ) ) . gr ;
float2 cc = mad ( float2 ( 2.0 , 2.0 ) , c . xz , c . yw ) ;
#line 1000
SMAAMovc ( bool2 ( step ( float2 ( 0.9 , 0.9 ) , d . zw ) ) , cc , float2 ( 0.0 , 0.0 ) ) ;
#line 1003
...
#line 1035
return searchTex . SampleLevel ( sampler_MainTex , mad ( scale , e , bias ) , 0 ) . a ;
}
#line 1041
...
e . g > 0.8281 &&
e . r == 0.0 ) {
e = edgesTex . SampleLevel ( sampler_MainTex , texcoord , 0 ) . rg ;
texcoord = mad ( - float2 ( 2.0 , 0.0 ) , _MainTex_TexelSize . xy , texcoord ) ;
}
...
e . g == 0.0 ) {
e = edgesTex . SampleLevel ( sampler_MainTex , texcoord , 0 ) . rg ;
texcoord = mad ( - float2 ( 0.0 , 2.0 ) , _MainTex_TexelSize . xy , texcoord ) ;
}
...
factor . x -= rounding . x * edgesTex . SampleLevel ( sampler_MainTex , texcoord . xy , 0 , int2 ( 0 , 1 ) ) . r ;
factor . x -= rounding . y * edgesTex . SampleLevel ( sampler_MainTex , texcoord . zw , 0 , int2 ( 1 , 1 ) ) . r ;
factor . y -= rounding . x * edgesTex . SampleLevel ( sampler_MainTex , texcoord . xy , 0 , int2 ( 0 , - 2 ) ) . r ;
factor . y -= rounding . y * edgesTex . SampleLevel ( sampler_MainTex , texcoord . zw , 0 , int2 ( 1 , - 2 ) ) . r ;
...
float2 factor = float2 ( 1.0 , 1.0 ) ;
factor . x -= rounding . x * edgesTex . SampleLevel ( sampler_MainTex , texcoord . xy , 0 , int2 ( 1 , 0 ) ) . g ;
factor . x -= rounding . y * edgesTex . SampleLevel ( sampler_MainTex , texcoord . zw , 0 , int2 ( 1 , 1 ) ) . g ;
factor . y -= rounding . x * edgesTex . SampleLevel ( sampler_MainTex , texcoord . xy , 0 , int2 ( - 2 , 0 ) ) . g ;
factor . y -= rounding . y * edgesTex . SampleLevel ( sampler_MainTex , texcoord . zw , 0 , int2 ( - 2 , 1 ) ) . g ;
...
float2 e = edgesTex . Sample ( sampler_MainTex , texcoord ) . rg ;
...
d . x = coords . x ;
#line 1203
float e1 = edgesTex . SampleLevel ( sampler_MainTex , coords . xy , 0 ) . r ;
#line 1206
coords . z = SMAASearchXRight ( edgesTex , searchTex , offset [ 0 ] . zw , offset [ 2 ] . y ) ;
d . y = coords . z ;
#line 1211
d = abs ( round ( mad ( _MainTex_TexelSize . zz , d , - pixcoord . xx ) ) ) ;
#line 1215
float2 sqrt_d = sqrt ( d ) ;
#line 1218
float e2 = edgesTex . SampleLevel ( sampler_MainTex , coords . zy , 0 , int2 ( 1 , 0 ) ) . r ;
#line 1222
...
d . x = coords . y ;
#line 1245
float e1 = edgesTex . SampleLevel ( sampler_MainTex , coords . xy , 0 ) . g ;
#line 1248
coords . z = SMAASearchYDown ( edgesTex , searchTex , offset [ 1 ] . zw , offset [ 2 ] . w ) ;
d . y = coords . z ;
#line 1252
d = abs ( round ( mad ( _MainTex_TexelSize . ww , d , - pixcoord . yy ) ) ) ;
#line 1256
float2 sqrt_d = sqrt ( d ) ;
#line 1259
float e2 = edgesTex . SampleLevel ( sampler_MainTex , coords . xz , 0 , int2 ( 0 , 1 ) ) . g ;
#line 1262
...
float4 a ;
a . x = blendTex . Sample ( sampler_MainTex , offset . xy ) . a ;
a . y = blendTex . Sample ( sampler_MainTex , offset . zw ) . g ;
a . wz = blendTex . Sample ( sampler_MainTex , texcoord ) . xz ;
#line 1311
[ branch ]
if ( dot ( a , float4 ( 1.0 , 1.0 , 1.0 , 1.0 ) ) < 1e-5 ) {
float4 color = colorTex . SampleLevel ( sampler_MainTex , texcoord , 0 ) ;
#line 1326
return color ;
} else {
...
#line 1338
float4 blendingCoord = mad ( blendingOffset , float4 ( _MainTex_TexelSize . xy , - _MainTex_TexelSize . xy ) , texcoord . xyxy ) ;
#line 1342
float4 color = blendingWeight . x * colorTex . SampleLevel ( sampler_MainTex , blendingCoord . xy , 0 ) ;
color += blendingWeight . y * colorTex . SampleLevel ( sampler_MainTex , blendingCoord . zw , 0 ) ;
#line 1360
return color ;
}
}
...
#line 1415
float4 current = currentColorTex . Sample ( sampler_MainTex , texcoord ) ;
float4 previous = previousColorTex . Sample ( sampler_MainTex , texcoord ) ;
...
#line 23 "D:/unity/fps_webgpu/Library/PackageCache/com.unity.postprocessing@3.2.2/PostProcessing/Shaders/Builtins/SubpixelMorphologicalAntialiasingBridge.hlsl"
struct VaryingsEdge
{
float4 vertex : SV_POSITION ;
float2 texcoord : TEXCOORD0 ;
...
o . texcoord = o . texcoord * float2 ( 1.0 , - 1.0 ) + float2 ( 0.0 , 1.0 ) ;
#line 40
o . offsets [ 0 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 1.0 , 0.0 , 0.0 , - 1.0 ) , o . texcoord . xyxy ) ;
o . offsets [ 1 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( 1.0 , 0.0 , 0.0 , 1.0 ) , o . texcoord . xyxy ) ;
o . offsets [ 2 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 2.0 , 0.0 , 0.0 , - 2.0 ) , o . texcoord . xyxy ) ;
...
o . texcoord = o . texcoord * float2 ( 1.0 , - 1.0 ) + float2 ( 0.0 , 1.0 ) ;
#line 73
o . pixcoord = o . texcoord * _MainTex_TexelSize . zw ;
#line 76
o . offsets [ 0 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 0.250 , - 0.125 , 1.250 , - 0.125 ) , o . texcoord . xyxy ) ;
o . offsets [ 1 ] = mad ( _MainTex_TexelSize . xyxy , float4 ( - 0.125 , - 0.250 , - 0.125 , 1.250 ) , o . texcoord . xyxy ) ;
#line 80
o . offsets [ 2 ] = mad ( _MainTex_TexelSize . xxyy , float4 ( - 2.0 , 2.0 , - 2.0 , 2.0 ) * float ( 16 ) ,
...
o . texcoord = o . texcoord * float2 ( 1.0 , - 1.0 ) + float2 ( 0.0 , 1.0 ) ;
#line 111
o . offset = mad ( _MainTex_TexelSize . xyxy , float4 ( 1.0 , 0.0 , 0.0 , 1.0 ) , o . texcoord . xyxy ) ;
return o ;
}

...
fn main_1() {
  ...
  let x_448 : vec4<f32> = x_32.x_AmbientColor;
  let x_450 : vec3<f32> = ((vec3<f32>(x_441.x, x_441.y, x_441.z) * vec3<f32>(x_443.x, x_443.y, x_443.z)) + vec3<f32>(x_448.x, x_448.y, x_448.z));
  let x_451 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_450.x, x_450.y, x_450.z, x_451.w);
  let x_454 : vec4<f32> = u_xlat0;
  let x_455 : vec3<f32> = vec3<f32>(x_454.x, x_454.y, x_454.z);
  let x_456 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_455.x, x_455.y, x_455.z, x_456.w);
  SV_Target0.w = 1.0f;
  let x_462 : f32 = x_32.x_Glossiness;
  SV_Target1.w = x_462;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

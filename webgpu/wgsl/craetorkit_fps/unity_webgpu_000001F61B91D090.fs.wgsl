...
fn main_1() {
  ...
  if (x_151) {
    ...
    let x_198 : vec4<f32> = x_45.unity_ProbeVolumeWorldToObject[3i];
    let x_200 : vec3<f32> = (vec3<f32>(x_194.x, x_194.y, x_194.z) + vec3<f32>(x_198.x, x_198.y, x_198.z));
  ...
  let x_561 : vec4<f32> = x_45.x_EmissionColor;
  let x_564 : vec3<f32> = u_xlat0;
  u_xlat0 = ((x_558 * vec3<f32>(x_561.x, x_561.y, x_561.z)) + x_564);
  let x_567 : f32 = vs_TEXCOORD1.w;
  u_xlat27 = x_567;
  let x_568 : f32 = u_xlat27;
  u_xlat27 = clamp(x_568, 0.0f, 1.0f);
  let x_570 : vec3<f32> = u_xlat0;
  let x_572 : vec4<f32> = x_45.unity_FogColor;
  u_xlat0 = (x_570 + -(vec3<f32>(x_572.x, x_572.y, x_572.z)));
  let x_578 : f32 = u_xlat27;
  let x_580 : vec3<f32> = u_xlat0;
  let x_583 : vec4<f32> = x_45.unity_FogColor;
  let x_585 : vec3<f32> = ((vec3<f32>(x_578, x_578, x_578) * x_580) + vec3<f32>(x_583.x, x_583.y, x_583.z));

...
fn main_1() {
  ...
  if (x_128) {
    ...
    let x_161 : vec4<f32> = x_31.unity_ProbeVolumeWorldToObject[3i];
    u_xlat4 = (x_158 + vec3<f32>(x_161.x, x_161.y, x_161.z));
  ...
  let x_291 : f32 = vs_TEXCOORD4;
  let x_293 : f32 = x_31.x_ProjectionParams.y;
  u_xlat1.x = (x_291 / x_293);
  let x_297 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_297) + 1.0f);
  let x_302 : f32 = u_xlat1.x;
  let x_304 : f32 = x_31.x_ProjectionParams.z;
  u_xlat1.x = (x_302 * x_304);
  let x_308 : f32 = u_xlat1.x;
  u_xlat1.x = max(x_308, 0.0f);
  let x_312 : f32 = u_xlat1.x;
  let x_315 : f32 = x_31.unity_FogParams.x;
  u_xlat1.x = (x_312 * x_315);
  let x_319 : f32 = u_xlat1.x;
  let x_321 : f32 = u_xlat1.x;
  u_xlat1.x = (x_319 * -(x_321));
  let x_326 : f32 = u_xlat1.x;
  u_xlat1.x = exp2(x_326);
  let x_329 : vec3<f32> = u_xlat0;
  let x_330 : f32 = u_xlat9;
  let x_335 : vec4<f32> = x_31.unity_FogColor;
  u_xlat0 = ((x_329 * vec3<f32>(x_330, x_330, x_330)) + -(vec3<f32>(x_335.x, x_335.y, x_335.z)));
  let x_341 : vec4<f32> = u_xlat1;
  let x_343 : vec3<f32> = u_xlat0;
  let x_346 : vec4<f32> = x_31.unity_FogColor;
  let x_348 : vec3<f32> = ((vec3<f32>(x_341.x, x_341.x, x_341.x) * x_343) + vec3<f32>(x_346.x, x_346.y, x_346.z));
  let x_349 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_348.x, x_348.y, x_348.z, x_349.w);
  SV_Target0.w = 1.0f;
  return;
}
...
@fragment
fn main(@location(0) vs_TEXCOORD0_param : vec2<f32>, @location(3) vs_TEXCOORD2_param : vec3<f32>, @location(4) vs_TEXCOORD5_param : vec4<f32>, @location(2) vs_TEXCOORD1_param : vec3<f32>, @location(1) vs_TEXCOORD4_param : f32, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

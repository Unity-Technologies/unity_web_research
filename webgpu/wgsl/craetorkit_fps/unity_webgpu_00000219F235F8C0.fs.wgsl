...
fn main_1() {
  ...
  let x_787 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_787);
  let x_789 : bool = u_xlatb0;
  if (x_789) {
    let x_792 : vec4<f32> = u_xlat3;
    u_xlat0 = vec3<f32>(x_792.x, x_792.y, x_792.z);
    let x_794 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_794, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_798 : vec3<f32> = u_xlat0;
    u_xlat3.w = dot(x_798, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

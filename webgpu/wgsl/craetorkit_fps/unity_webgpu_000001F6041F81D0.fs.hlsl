...
float unityFogFactor = unity_FogParams . x * ( max ( ( ( 1.0 - ( ( o . vertex ) . z ) / _ProjectionParams . y ) * _ProjectionParams . z ) , 0 ) ) ; unityFogFactor = exp2 ( - unityFogFactor * unityFogFactor ) ; o . fogCoord . x = unityFogFactor ;
}
...
if ( _DstBlend == 1 )
{
result . rgb = lerp ( ( float4 ( 0 , 0 , 0 , 0 ) ) . rgb , ( result ) . rgb , saturate ( ( IN . fogCoord ) . x ) ) ;
}
else
{
result . rgb = lerp ( ( unity_FogColor ) . rgb , ( result ) . rgb , saturate ( ( IN . fogCoord ) . x ) ) ;
}
#line 422
return result ;
}

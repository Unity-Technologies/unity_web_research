...
fn main_1() {
  ...
  let x_356 : vec3<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec3<f32>(x_355.x, x_355.y, x_356.z);
  let x_358 : vec4<f32> = u_xlat0;
  let x_362 : vec4<f32> = x_18.unity_WorldToLight[1i];
  let x_364 : vec3<f32> = (vec3<f32>(x_358.y, x_358.y, x_358.y) * vec3<f32>(x_362.x, x_362.y, x_362.z));
  ...
  let x_375 : vec3<f32> = ((vec3<f32>(x_368.x, x_368.y, x_368.z) * vec3<f32>(x_370.x, x_370.x, x_370.x)) + vec3<f32>(x_373.x, x_373.y, x_373.z));
  let x_376 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_375.x, x_375.y, x_375.z, x_376.w);
  let x_379 : vec4<f32> = x_18.unity_WorldToLight[2i];
  let x_381 : vec4<f32> = u_xlat0;
  let x_384 : vec4<f32> = u_xlat1;
  let x_386 : vec3<f32> = ((vec3<f32>(x_379.x, x_379.y, x_379.z) * vec3<f32>(x_381.z, x_381.z, x_381.z)) + vec3<f32>(x_384.x, x_384.y, x_384.z));
  let x_387 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_386.x, x_386.y, x_386.z, x_387.w);
  let x_391 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_393 : vec4<f32> = u_xlat0;
  let x_396 : vec4<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec3<f32>(x_391.x, x_391.y, x_391.z) * vec3<f32>(x_393.w, x_393.w, x_393.w)) + vec3<f32>(x_396.x, x_396.y, x_396.z));
  return;
}

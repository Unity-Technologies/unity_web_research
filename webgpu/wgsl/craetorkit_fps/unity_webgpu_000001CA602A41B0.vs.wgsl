...
fn main_1() {
  ...
  vs_COLOR0 = x_139;
  let x_143 : f32 = u_xlat0.z;
  vs_TEXCOORD4 = x_143;
  let x_146 : vec4<f32> = u_xlat0;
  let x_147 : vec2<f32> = vec2<f32>(x_146.z, x_146.w);
  let x_148 : vec4<f32> = vs_TEXCOORD5;
  vs_TEXCOORD5 = vec4<f32>(x_148.x, x_148.y, x_147.x, x_147.y);
  let x_154 : vec2<f32> = in_TEXCOORD0;
  let x_157 : vec4<f32> = x_17.x_MainTex_ST;
  let x_161 : vec4<f32> = x_17.x_MainTex_ST;
  vs_TEXCOORD2 = ((x_154 * vec2<f32>(x_157.x, x_157.y)) + vec2<f32>(x_161.z, x_161.w));
  ...
  u_xlat6 = ((x_171 * x_173) + -(x_175));
  let x_178 : vec4<f32> = u_xlat1;
  let x_180 : vec4<f32> = u_xlat1;
  u_xlat1 = (vec4<f32>(x_178.y, x_178.z, x_178.z, x_178.x) * vec4<f32>(x_180.x, x_180.y, x_180.z, x_180.z));
  let x_186 : vec4<f32> = x_17.unity_SHBr;
  let x_187 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_186, x_187);
  let x_191 : vec4<f32> = x_17.unity_SHBg;
  let x_192 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_191, x_192);
  let x_196 : vec4<f32> = x_17.unity_SHBb;
  let x_197 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_196, x_197);
  let x_203 : vec4<f32> = x_17.unity_SHC;
  let x_205 : f32 = u_xlat6;
  let x_208 : vec3<f32> = u_xlat2;
  vs_TEXCOORD3 = ((vec3<f32>(x_203.x, x_203.y, x_203.z) * vec3<f32>(x_205, x_205, x_205)) + x_208);
  let x_212 : f32 = u_xlat0.y;
  let x_215 : f32 = x_17.x_ProjectionParams.x;
  u_xlat3 = (x_212 * x_215);
  let x_217 : vec4<f32> = u_xlat0;
  let x_221 : vec2<f32> = (vec2<f32>(x_217.x, x_217.w) * vec2<f32>(0.5f, 0.5f));
  let x_222 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_221.x, x_222.y, x_221.y, x_222.w);
  let x_224 : f32 = u_xlat3;
  u_xlat0.w = (x_224 * 0.5f);
  let x_228 : vec4<f32> = u_xlat0;
  let x_230 : vec4<f32> = u_xlat0;
  let x_232 : vec2<f32> = (vec2<f32>(x_228.z, x_228.z) + vec2<f32>(x_230.x, x_230.w));
  let x_233 : vec4<f32> = vs_TEXCOORD5;
  vs_TEXCOORD5 = vec4<f32>(x_232.x, x_232.y, x_233.z, x_233.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, gl_Position, vs_TEXCOORD0, vs_COLOR0, vs_TEXCOORD4, vs_TEXCOORD5, vs_TEXCOORD2, vs_TEXCOORD3);
}

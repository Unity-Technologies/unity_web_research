...
fn main_1() {
  ...
  u_xlat0.x = dot(x_226, vec3<f32>(x_229.x, x_229.y, x_229.z));
  let x_233 : vec3<f32> = in_NORMAL0;
  let x_235 : vec4<f32> = x_131.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_233, vec3<f32>(x_235.x, x_235.y, x_235.z));
  let x_239 : vec3<f32> = in_NORMAL0;
  let x_241 : vec4<f32> = x_131.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_239, vec3<f32>(x_241.x, x_241.y, x_241.z));
  let x_246 : vec4<f32> = u_xlat0;
  let x_248 : vec4<f32> = u_xlat0;
  u_xlat6 = dot(vec3<f32>(x_246.x, x_246.y, x_246.z), vec3<f32>(x_248.x, x_248.y, x_248.z));
  let x_251 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_251);
  ...
  u_xlat6 = ((x_269 * x_271) + -(x_273));
  let x_276 : vec4<f32> = u_xlat0;
  let x_278 : vec4<f32> = u_xlat0;
  u_xlat1 = (vec4<f32>(x_276.y, x_276.z, x_276.z, x_276.x) * vec4<f32>(x_278.x, x_278.y, x_278.z, x_278.z));
  let x_282 : vec4<f32> = x_131.unity_SHBr;
  let x_283 : vec4<f32> = u_xlat1;
  u_xlat0.x = dot(x_282, x_283);
  let x_287 : vec4<f32> = x_131.unity_SHBg;
  let x_288 : vec4<f32> = u_xlat1;
  u_xlat0.y = dot(x_287, x_288);
  let x_292 : vec4<f32> = x_131.unity_SHBb;
  let x_293 : vec4<f32> = u_xlat1;
  u_xlat0.z = dot(x_292, x_293);
  let x_298 : vec4<f32> = x_131.unity_SHC;
  let x_300 : f32 = u_xlat6;
  let x_303 : vec4<f32> = u_xlat0;
  vs_TEXCOORD4 = ((vec3<f32>(x_298.x, x_298.y, x_298.z) * vec3<f32>(x_300, x_300, x_300)) + vec3<f32>(x_303.x, x_303.y, x_303.z));
...
fn main(@location(1) in_TEXCOORD0_param : vec4<f32>, @location(2) in_NORMAL0_param : vec3<f32>, @location(0) in_POSITION0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD2, gl_Position, vs_TEXCOORD5, vs_TEXCOORD0, vs_TEXCOORD3, vs_TEXCOORD1, vs_TEXCOORD4);
}

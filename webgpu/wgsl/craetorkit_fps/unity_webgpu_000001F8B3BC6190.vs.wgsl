...
fn main_1() {
  ...
  let x_189 : vec4<f32> = x_19.unity_MatrixVP[1i];
  u_xlat0 = (vec4<f32>(x_186.y, x_186.y, x_186.y, x_186.y) * x_189);
  let x_192 : vec4<f32> = x_19.unity_MatrixVP[0i];
  let x_193 : vec4<f32> = u_xlat1;
  let x_196 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_192 * vec4<f32>(x_193.x, x_193.x, x_193.x, x_193.x)) + x_196);
  let x_199 : vec4<f32> = x_19.unity_MatrixVP[2i];
  let x_200 : vec4<f32> = u_xlat1;
  let x_203 : vec4<f32> = u_xlat0;
  u_xlat0 = ((x_199 * vec4<f32>(x_200.z, x_200.z, x_200.z, x_200.z)) + x_203);
  let x_210 : vec4<f32> = x_19.unity_MatrixVP[3i];
  let x_211 : vec4<f32> = u_xlat1;
  let x_214 : vec4<f32> = u_xlat0;
  gl_Position = ((x_210 * vec4<f32>(x_211.w, x_211.w, x_211.w, x_211.w)) + x_214);
  let x_218 : vec3<f32> = in_NORMAL0;
  let x_220 : vec4<f32> = x_19.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_218, vec3<f32>(x_220.x, x_220.y, x_220.z));
  let x_224 : vec3<f32> = in_NORMAL0;
  let x_226 : vec4<f32> = x_19.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_224, vec3<f32>(x_226.x, x_226.y, x_226.z));
  let x_230 : vec3<f32> = in_NORMAL0;
  let x_232 : vec4<f32> = x_19.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_230, vec3<f32>(x_232.x, x_232.y, x_232.z));
  let x_237 : vec4<f32> = u_xlat0;
  let x_239 : vec4<f32> = u_xlat0;
  u_xlat6 = dot(vec3<f32>(x_237.x, x_237.y, x_237.z), vec3<f32>(x_239.x, x_239.y, x_239.z));
  let x_242 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_242);
  let x_245 : f32 = u_xlat6;
  let x_247 : vec4<f32> = u_xlat0;
  vs_TEXCOORD0 = (vec3<f32>(x_245, x_245, x_245) * vec3<f32>(x_247.x, x_247.y, x_247.z));
  let x_251 : vec4<f32> = in_COLOR0;
  vs_COLOR0 = x_251;
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, gl_Position, vs_TEXCOORD0, vs_COLOR0);
}

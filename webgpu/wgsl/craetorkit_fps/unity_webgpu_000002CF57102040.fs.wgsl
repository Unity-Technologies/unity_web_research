...
fn main_1() {
  ...
  let x_342 : vec4<f32> = x_99.x_Color2;
  let x_344 : vec3<f32> = ((vec3<f32>(x_335.x, x_335.y, x_335.z) * vec3<f32>(x_337.x, x_337.y, x_337.z)) + vec3<f32>(x_342.x, x_342.y, x_342.z));
  let x_345 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_344.x, x_344.y, x_344.z, x_345.w);
  let x_348 : vec4<f32> = u_xlat1;
  let x_349 : vec3<f32> = vec3<f32>(x_348.x, x_348.y, x_348.z);
  let x_350 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_349.x, x_349.y, x_349.z, x_350.w);
  SV_Target0.w = 1.0f;
  let x_356 : f32 = x_99.x_Glossiness;
  SV_Target1.w = x_356;
  let x_359 : vec3<f32> = vs_TEXCOORD1;
  let x_362 : vec3<f32> = ((x_359 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));

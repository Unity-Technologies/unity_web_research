...
fn main_1() {
  ...
  let x_311 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_309, x_309));
  u_xlat21 = x_311.x;
  let x_321 : vec3<f32> = u_xlat4;
  let x_322 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, x_321);
  u_xlat22 = x_322.w;
  let x_324 : f32 = u_xlat21;
  let x_325 : f32 = u_xlat22;
  u_xlat21 = (x_324 * x_325);
  let x_328 : f32 = u_xlat14.x;
  let x_329 : f32 = u_xlat21;
  u_xlat14.x = (x_328 * x_329);
  let x_332 : vec2<f32> = u_xlat14;
  let x_336 : vec4<f32> = x_46.x_LightColor0;
  u_xlat4 = (vec3<f32>(x_332.x, x_332.x, x_332.x) * vec3<f32>(x_336.x, x_336.y, x_336.z));
  let x_340 : vec4<f32> = vs_TEXCOORD1;
  let x_343 : vec3<f32> = u_xlat3;
  u_xlat14.x = dot(-(vec3<f32>(x_340.x, x_340.y, x_340.z)), x_343);
  let x_347 : f32 = u_xlat14.x;
  let x_349 : f32 = u_xlat14.x;
  u_xlat14.x = (x_347 + x_349);
  let x_352 : vec3<f32> = u_xlat3;
  let x_353 : vec2<f32> = u_xlat14;
  let x_357 : vec4<f32> = vs_TEXCOORD1;
  let x_360 : vec3<f32> = ((x_352 * -(vec3<f32>(x_353.x, x_353.x, x_353.x))) + -(vec3<f32>(x_357.x, x_357.y, x_357.z)));
  let x_361 : vec4<f32> = u_xlat5;
  u_xlat5 = vec4<f32>(x_360.x, x_360.y, x_360.z, x_361.w);
  let x_366 : f32 = vs_TEXCOORD2.w;
  u_xlat6.x = x_366;
  let x_369 : f32 = vs_TEXCOORD3.w;
  u_xlat6.y = x_369;
  let x_372 : f32 = vs_TEXCOORD4.w;
  u_xlat6.z = x_372;
  let x_374 : vec3<f32> = u_xlat3;
  let x_375 : vec3<f32> = u_xlat6;
  u_xlat14.x = dot(x_374, x_375);
  let x_379 : f32 = u_xlat14.x;
  u_xlat14.x = clamp(x_379, 0.0f, 1.0f);
  let x_382 : vec4<f32> = u_xlat5;
  let x_384 : vec3<f32> = u_xlat6;
  u_xlat21 = dot(vec3<f32>(x_382.x, x_382.y, x_382.z), x_384);
  let x_386 : f32 = u_xlat21;
  let x_387 : f32 = u_xlat21;
  u_xlat21 = (x_386 * x_387);
  let x_389 : f32 = u_xlat21;
  let x_390 : f32 = u_xlat21;
  u_xlat3.x = (x_389 * x_390);
  let x_394 : f32 = u_xlat0.y;
  let x_398 : f32 = x_46.x_GlossMapScale;
  u_xlat3.y = ((-(x_394) * x_398) + 1.0f);
  let x_408 : vec3<f32> = u_xlat3;
  let x_410 : vec4<f32> = textureSample(unity_NHxRoughness, samplerunity_NHxRoughness, vec2<f32>(x_408.x, x_408.y));
  u_xlat7 = x_410.x;
  let x_412 : f32 = u_xlat7;
  u_xlat7 = (x_412 * 16.0f);
  let x_415 : vec3<f32> = u_xlat1;
  let x_416 : f32 = u_xlat7;
  u_xlat1 = (x_415 * vec3<f32>(x_416, x_416, x_416));
  let x_419 : vec3<f32> = u_xlat2;
  let x_420 : vec4<f32> = u_xlat0;
  let x_423 : vec3<f32> = u_xlat1;
  let x_424 : vec3<f32> = ((x_419 * vec3<f32>(x_420.x, x_420.x, x_420.x)) + x_423);
  let x_425 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_424.x, x_424.y, x_425.z, x_424.z);
  let x_427 : vec2<f32> = u_xlat14;
  let x_429 : vec3<f32> = u_xlat4;
  u_xlat1 = (vec3<f32>(x_427.x, x_427.x, x_427.x) * x_429);
  let x_431 : vec4<f32> = u_xlat0;
  let x_433 : vec3<f32> = u_xlat1;
  let x_434 : vec3<f32> = (vec3<f32>(x_431.x, x_431.y, x_431.w) * x_433);
  let x_435 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_434.x, x_434.y, x_434.z, x_435.w);
  let x_438 : f32 = vs_TEXCOORD1.w;
  u_xlat21 = x_438;
  let x_439 : f32 = u_xlat21;
  u_xlat21 = clamp(x_439, 0.0f, 1.0f);

...
fn main_1() {
  ...
  let x_367 : vec4<f32> = in_POSITION0;
  let x_368 : vec2<f32> = vec2<f32>(x_367.x, x_367.z);
  let x_369 : vec3<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec3<f32>(x_368.x, x_368.y, x_369.z);
  let x_372 : f32 = u_xlat4.y;
  let x_374 : f32 = u_xlat4.y;
  u_xlat0.x = (x_372 * x_374);
  let x_378 : f32 = u_xlat4.x;
  let x_380 : f32 = u_xlat4.x;
  ...
  let x_392 : vec4<f32> = x_18.unity_SHBr;
  let x_393 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_392, x_393);
  let x_397 : vec4<f32> = x_18.unity_SHBg;
  let x_398 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_397, x_398);
  let x_403 : vec4<f32> = x_18.unity_SHBb;
  let x_404 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_403, x_404);
  let x_411 : vec4<f32> = x_18.unity_SHC;
  let x_413 : vec4<f32> = u_xlat0;
  let x_416 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_411.x, x_411.y, x_411.z) * vec3<f32>(x_413.x, x_413.x, x_413.x)) + vec3<f32>(x_416.x, x_416.y, x_416.z));
  return;
}

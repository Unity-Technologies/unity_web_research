...
#line 82
float sceneZ = LinearEyeDepth ( ( tex2Dproj ( _CameraDepthTexture , IN . projPos ) . r ) ) ;
...
#line 109
float4 c = tex2D ( _Ramp , float2 ( blend , 0 ) ) ;
c = lerp ( c , _Color2 , edge ) ;
float FogEdge = smoothstep ( noiseTex , noiseTex + 1 , fog ) ;

float4 fogc = lerp ( _Color2 , _Color1 , fog * FogEdge ) ;
c += fogc ;
float4 emission = lerp ( 0 , _Color2 , pow ( blend , 2 ) ) ;
float roughness = lerp ( 1 , 0 , blend ) ;
...
float4 custompack1 : TEXCOORD6 ;
float3 custompack2 : TEXCOORD7 ;
float2 _LightCoord : TEXCOORD8 ;
float1 fogCoord : TEXCOORD9 ;
#line 148
} ;
float4 _MainTex_ST ;
float4 _Normal_ST ;
...
o . custompack0 . xyzw = customInputData . projPos ;
o . custompack1 . xyzw = customInputData . grabPos ;
o . custompack2 . xy = customInputData . localDir ;
o . custompack2 . z = customInputData . depth ;
o . pos = UnityObjectToClipPos ( v . vertex ) ;
...
surfIN . projPos . x = 1.0 ;
surfIN . grabPos . x = 1.0 ;
surfIN . worldPos . x = 1.0 ;
surfIN . localDir . x = 1.0 ;
...
surfIN . projPos = IN . custompack0 . xyzw ;
surfIN . grabPos = IN . custompack1 . xyzw ;
surfIN . localDir = IN . custompack2 . xy ;
surfIN . depth = IN . custompack2 . z ;

...
fn main_1() {
  ...
  let x_198 : f32 = vs_TEXCOORD4;
  u_xlat1.x = x_198;
  let x_201 : f32 = u_xlat1.x;
  u_xlat1.x = clamp(x_201, 0.0f, 1.0f);
  let x_204 : vec3<f32> = u_xlat0;
  let x_205 : f32 = u_xlat6;
  let x_209 : vec4<f32> = x_31.unity_FogColor;
  u_xlat0 = ((x_204 * vec3<f32>(x_205, x_205, x_205)) + -(vec3<f32>(x_209.x, x_209.y, x_209.z)));
  let x_215 : vec4<f32> = u_xlat1;
  let x_217 : vec3<f32> = u_xlat0;
  let x_220 : vec4<f32> = x_31.unity_FogColor;
  let x_222 : vec3<f32> = ((vec3<f32>(x_215.x, x_215.x, x_215.x) * x_217) + vec3<f32>(x_220.x, x_220.y, x_220.z));
  let x_223 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_222.x, x_222.y, x_222.z, x_223.w);
  SV_Target0.w = 1.0f;
  return;
}

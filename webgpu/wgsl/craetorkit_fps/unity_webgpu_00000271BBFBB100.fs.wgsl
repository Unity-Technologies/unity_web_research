...
fn main_1() {
  ...
  let x_529 : f32 = x_49.x_PostExposure;
  let x_531 : f32 = x_49.x_PostExposure;
  let x_533 : f32 = x_49.x_PostExposure;
  let x_535 : f32 = x_49.x_PostExposure;
  let x_536 : vec4<f32> = vec4<f32>(x_529, x_531, x_533, x_535);
  u_xlat0 = (x_526 * vec4<f32>(x_536.x, x_536.y, x_536.z, x_536.w));
  let x_543 : vec4<f32> = u_xlat0;
  let x_550 : vec3<f32> = ((vec3<f32>(x_543.z, x_543.x, x_543.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_565 : vec3<f32> = ((vec3<f32>(x_558.x, x_558.y, x_558.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  ...
  let x_579 : vec3<f32> = x_49.x_Lut2D_Params;
  u_xlat6 = (vec3<f32>(x_575.x, x_575.y, x_575.z) * vec3<f32>(x_579.z, x_579.z, x_579.z));
  let x_583 : f32 = u_xlat6.x;
  u_xlat6.x = floor(x_583);
  let x_587 : vec3<f32> = x_49.x_Lut2D_Params;
  let x_590 : vec2<f32> = (vec2<f32>(x_587.x, x_587.y) * vec2<f32>(0.5f, 0.5f));
  let x_591 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_590.x, x_590.y, x_591.z, x_591.w);
  let x_593 : vec3<f32> = u_xlat6;
  let x_596 : vec3<f32> = x_49.x_Lut2D_Params;
  let x_599 : vec4<f32> = u_xlat2;
  let x_601 : vec2<f32> = ((vec2<f32>(x_593.y, x_593.z) * vec2<f32>(x_596.x, x_596.y)) + vec2<f32>(x_599.x, x_599.y));
  let x_602 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_602.x, x_601.x, x_601.y, x_602.w);
  let x_605 : f32 = u_xlat6.x;
  let x_607 : f32 = x_49.x_Lut2D_Params.y;
  let x_610 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_605 * x_607) + x_610);
  let x_618 : vec4<f32> = u_xlat2;
  let x_620 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_618.x, x_618.z));
  ...
  let x_625 : f32 = x_49.x_Lut2D_Params.y;
  u_xlat4.x = x_625;
  u_xlat4.y = 0.0f;
  let x_630 : vec4<f32> = u_xlat2;
  let x_632 : vec4<f32> = u_xlat4;
  u_xlat11 = (vec2<f32>(x_630.x, x_630.z) + vec2<f32>(x_632.x, x_632.y));
  let x_638 : vec2<f32> = u_xlat11;
  let x_639 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_638);
  ...
  let x_646 : f32 = x_49.x_Lut2D_Params.z;
  let x_649 : f32 = u_xlat6.x;
  u_xlat1.x = ((x_644 * x_646) + -(x_649));
  let x_653 : vec4<f32> = u_xlat3;
  let x_656 : vec4<f32> = u_xlat2;
  u_xlat6 = (-(vec3<f32>(x_653.x, x_653.y, x_653.z)) + vec3<f32>(x_656.x, x_656.y, x_656.z));
  let x_659 : vec4<f32> = u_xlat1;
  let x_661 : vec3<f32> = u_xlat6;
  let x_663 : vec4<f32> = u_xlat3;
  let x_665 : vec3<f32> = ((vec3<f32>(x_659.x, x_659.x, x_659.x) * x_661) + vec3<f32>(x_663.x, x_663.y, x_663.z));
  let x_666 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_665.x, x_665.y, x_665.z, x_666.w);
  let x_670 : f32 = x_49.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_670);
  let x_672 : bool = u_xlatb1;
  if (x_672) {
    ...
    u_xlat0.w = dot(vec3<f32>(x_686.x, x_686.y, x_686.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_911 : vec3<f32> = ((vec3<f32>(x_904.z, x_904.x, x_904.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_941 : vec3<f32> = x_55.x_Lut2D_Params;
  u_xlat10 = (vec3<f32>(x_936.x, x_936.y, x_936.z) * vec3<f32>(x_941.z, x_941.z, x_941.z));
  let x_945 : f32 = u_xlat10.x;
  u_xlat10.x = floor(x_945);
  let x_949 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_951 : vec2<f32> = (vec2<f32>(x_949.x, x_949.y) * vec2<f32>(0.5f, 0.5f));
  let x_952 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_951.x, x_951.y, x_952.z, x_952.w);
  let x_954 : vec3<f32> = u_xlat10;
  let x_957 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_960 : vec4<f32> = u_xlat2;
  let x_962 : vec2<f32> = ((vec2<f32>(x_954.y, x_954.z) * vec2<f32>(x_957.x, x_957.y)) + vec2<f32>(x_960.x, x_960.y));
  let x_963 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_963.x, x_962.x, x_962.y, x_963.w);
  let x_966 : f32 = u_xlat10.x;
  let x_968 : f32 = x_55.x_Lut2D_Params.y;
  let x_971 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_966 * x_968) + x_971);
  let x_979 : vec4<f32> = u_xlat2;
  let x_981 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_979.x, x_979.z));
  let x_982 : vec3<f32> = vec3<f32>(x_981.x, x_981.y, x_981.z);
  let x_983 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_982.x, x_982.y, x_982.z, x_983.w);
  let x_986 : f32 = x_55.x_Lut2D_Params.y;
  u_xlat4.x = x_986;
  u_xlat4.y = 0.0f;
  let x_990 : vec4<f32> = u_xlat2;
  let x_992 : vec4<f32> = u_xlat4;
  u_xlat19 = (vec2<f32>(x_990.x, x_990.z) + vec2<f32>(x_992.x, x_992.y));
  let x_998 : vec2<f32> = u_xlat19;
  ...
  let x_1007 : f32 = x_55.x_Lut2D_Params.z;
  let x_1010 : f32 = u_xlat10.x;
  u_xlat1.x = ((x_1004 * x_1007) + -(x_1010));
  let x_1014 : vec4<f32> = u_xlat3;
  let x_1017 : vec4<f32> = u_xlat2;
  u_xlat10 = (-(vec3<f32>(x_1014.x, x_1014.y, x_1014.z)) + vec3<f32>(x_1017.x, x_1017.y, x_1017.z));
  let x_1020 : vec4<f32> = u_xlat1;
  let x_1022 : vec3<f32> = u_xlat10;
  let x_1024 : vec4<f32> = u_xlat3;
  let x_1026 : vec3<f32> = ((vec3<f32>(x_1020.x, x_1020.x, x_1020.x) * x_1022) + vec3<f32>(x_1024.x, x_1024.y, x_1024.z));
  let x_1027 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_1026.x, x_1026.y, x_1026.z, x_1027.w);
  let x_1032 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_1032);
  let x_1034 : bool = u_xlatb1;
  if (x_1034) {
    let x_1037 : vec4<f32> = u_xlat0;
    let x_1038 : vec3<f32> = vec3<f32>(x_1037.x, x_1037.y, x_1037.z);
    let x_1039 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_1038.x, x_1038.y, x_1038.z, x_1039.w);
    let x_1041 : vec4<f32> = u_xlat1;
    let x_1045 : vec3<f32> = clamp(vec3<f32>(x_1041.x, x_1041.y, x_1041.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1046 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_1045.x, x_1045.y, x_1045.z, x_1046.w);
    let x_1048 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_1048.x, x_1048.y, x_1048.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

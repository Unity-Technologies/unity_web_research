...
fn main_1() {
  ...
  let x_1448 : f32 = x_26.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_1448);
  let x_1450 : bool = u_xlatb0;
  if (x_1450) {
    ...
    let x_1461 : vec3<f32> = clamp(vec3<f32>(x_1457.x, x_1457.y, x_1457.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1462 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1461.x, x_1461.y, x_1461.z, x_1462.w);
    let x_1464 : vec4<f32> = u_xlat0;
    u_xlat3.w = dot(vec3<f32>(x_1464.x, x_1464.y, x_1464.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

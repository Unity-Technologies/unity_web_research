...
fn main_1() {
  ...
  let x_498 : vec2<f32> = vec2<f32>(x_497.z, x_497.w);
  let x_499 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_499.x, x_499.y, x_498.x, x_498.y);
  let x_501 : f32 = u_xlat15;
  let x_503 : f32 = x_184.unity_MatrixV[1i].z;
  u_xlat1.x = (x_501 * x_503);
  let x_506 : f32 = u_xlat15;
  let x_509 : vec4<f32> = x_184.unity_MatrixVP[1i];
  let x_511 : vec3<f32> = (vec3<f32>(x_506, x_506, x_506) * vec3<f32>(x_509.x, x_509.y, x_509.w));
  let x_512 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_511.x, x_511.y, x_511.z, x_512.w);
  let x_515 : vec4<f32> = x_184.unity_MatrixVP[0i];
  let x_517 : vec3<f32> = u_xlat8;
  let x_520 : vec4<f32> = u_xlat2;
  let x_522 : vec3<f32> = ((vec3<f32>(x_515.x, x_515.y, x_515.w) * vec3<f32>(x_517.x, x_517.x, x_517.x)) + vec3<f32>(x_520.x, x_520.y, x_520.z));
  let x_523 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_522.x, x_522.y, x_522.z, x_523.w);
  let x_526 : f32 = x_184.unity_MatrixV[0i].z;
  let x_528 : f32 = u_xlat8.x;
  let x_531 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_526 * x_528) + x_531);
  let x_535 : f32 = x_184.unity_MatrixV[2i].z;
  let x_537 : f32 = u_xlat0.x;
  let x_540 : f32 = u_xlat1.x;
  u_xlat1.x = ((x_535 * x_537) + x_540);
  let x_544 : vec4<f32> = x_184.unity_MatrixVP[2i];
  let x_546 : vec4<f32> = u_xlat0;
  let x_549 : vec4<f32> = u_xlat2;
  u_xlat8 = ((vec3<f32>(x_544.x, x_544.y, x_544.w) * vec3<f32>(x_546.x, x_546.x, x_546.x)) + vec3<f32>(x_549.x, x_549.y, x_549.z));
  let x_552 : vec3<f32> = u_xlat8;
  let x_554 : vec4<f32> = x_184.unity_MatrixVP[3i];
  u_xlat8 = (x_552 + vec3<f32>(x_554.x, x_554.y, x_554.w));
  let x_558 : f32 = u_xlat1.x;
  let x_560 : f32 = x_184.unity_MatrixV[3i].z;
  u_xlat0.x = (x_558 + x_560);
  let x_566 : f32 = u_xlat0.x;
  vs_TEXCOORD3.z = -(x_566);
  let x_571 : f32 = u_xlat8.z;
  vs_TEXCOORD3.w = x_571;
  u_xlat2.x = 0.5f;
  u_xlat2.z = 0.5f;
  let x_577 : f32 = x_184.x_ProjectionParams.x;
  u_xlat2.y = x_577;
  let x_579 : vec3<f32> = u_xlat8;
  let x_580 : vec4<f32> = u_xlat2;
  let x_582 : vec3<f32> = (x_579 * vec3<f32>(x_580.x, x_580.y, x_580.z));
  ...
  let x_589 : vec3<f32> = (vec3<f32>(x_585.x, x_585.y, x_585.z) * vec3<f32>(x_587.x, x_587.y, x_587.w));
  let x_590 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_589.x, x_589.y, x_589.z, x_590.w);
  let x_593 : f32 = u_xlat1.y;
  u_xlat1.w = (x_593 * 0.5f);
  let x_596 : vec4<f32> = u_xlat1;
  let x_598 : vec4<f32> = u_xlat1;
  let x_600 : vec2<f32> = (vec2<f32>(x_596.z, x_596.z) + vec2<f32>(x_598.x, x_598.w));
  let x_601 : vec4<f32> = vs_TEXCOORD3;
  vs_TEXCOORD3 = vec4<f32>(x_600.x, x_600.y, x_601.z, x_601.w);
  let x_604 : f32 = u_xlat7.y;
  let x_606 : f32 = u_xlat7.y;
  u_xlat0.x = (x_604 * x_606);
  let x_610 : f32 = u_xlat7.x;
  let x_612 : f32 = u_xlat7.x;
  let x_615 : f32 = u_xlat0.x;
  u_xlat0.x = ((x_610 * x_612) + -(x_615));
  let x_619 : vec3<f32> = u_xlat7;
  let x_621 : vec3<f32> = u_xlat7;
  u_xlat1 = (vec4<f32>(x_619.y, x_619.z, x_619.z, x_619.x) * vec4<f32>(x_621.x, x_621.y, x_621.z, x_621.z));
  let x_625 : vec4<f32> = x_184.unity_SHBr;
  let x_626 : vec4<f32> = u_xlat1;
  u_xlat3.x = dot(x_625, x_626);
  let x_630 : vec4<f32> = x_184.unity_SHBg;
  let x_631 : vec4<f32> = u_xlat1;
  u_xlat3.y = dot(x_630, x_631);
  let x_635 : vec4<f32> = x_184.unity_SHBb;
  let x_636 : vec4<f32> = u_xlat1;
  u_xlat3.z = dot(x_635, x_636);
  let x_641 : vec4<f32> = x_184.unity_SHC;
  let x_643 : vec4<f32> = u_xlat0;
  let x_646 : vec4<f32> = u_xlat3;
  vs_TEXCOORD4 = ((vec3<f32>(x_641.x, x_641.y, x_641.z) * vec3<f32>(x_643.x, x_643.x, x_643.x)) + vec3<f32>(x_646.x, x_646.y, x_646.z));
  let x_650 : f32 = u_xlat2.y;
  u_xlat2.w = (x_650 * 0.5f);
  let x_653 : vec4<f32> = u_xlat2;
  let x_655 : vec4<f32> = u_xlat2;
  let x_657 : vec2<f32> = (vec2<f32>(x_653.z, x_653.z) + vec2<f32>(x_655.x, x_655.w));
  let x_658 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_657.x, x_657.y, x_658.z, x_658.w);
  let x_662 : i32 = gl_InstanceIndex;
  vs_SV_InstanceID0 = bitcast<u32>(x_662);
  return;
}

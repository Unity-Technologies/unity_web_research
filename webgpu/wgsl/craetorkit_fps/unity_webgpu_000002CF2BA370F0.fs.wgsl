...
fn main_1() {
  ...
  let x_535 : f32 = vs_TEXCOORD1.w;
  u_xlat24 = x_535;
  let x_536 : f32 = u_xlat24;
  u_xlat24 = clamp(x_536, 0.0f, 1.0f);
  let x_538 : vec4<f32> = u_xlat0;
  let x_541 : vec4<f32> = x_46.unity_FogColor;
  let x_544 : vec3<f32> = (vec3<f32>(x_538.x, x_538.y, x_538.z) + -(vec3<f32>(x_541.x, x_541.y, x_541.z)));
  ...
  let x_555 : vec4<f32> = x_46.unity_FogColor;
  let x_557 : vec3<f32> = ((vec3<f32>(x_549, x_549, x_549) * vec3<f32>(x_551.x, x_551.y, x_551.z)) + vec3<f32>(x_555.x, x_555.y, x_555.z));

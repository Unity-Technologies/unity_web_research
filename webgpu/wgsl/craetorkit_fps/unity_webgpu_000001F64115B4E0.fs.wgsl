...
fn main_1() {
  ...
  let x_481 : vec4<f32> = vs_TEXCOORD0;
  let x_483 : vec4<f32> = textureSample(x_EmissionMap, sampler_EmissionMap, vec2<f32>(x_481.x, x_481.y));
  u_xlat1 = vec3<f32>(x_483.x, x_483.y, x_483.z);
  let x_485 : vec3<f32> = u_xlat1;
  let x_488 : vec4<f32> = x_33.x_EmissionColor;
  let x_491 : vec3<f32> = u_xlat0;
  u_xlat0 = ((x_485 * vec3<f32>(x_488.x, x_488.y, x_488.z)) + x_491);
  let x_494 : f32 = vs_TEXCOORD1.w;
  u_xlat24 = x_494;
  let x_495 : f32 = u_xlat24;
  u_xlat24 = clamp(x_495, 0.0f, 1.0f);
  let x_497 : vec3<f32> = u_xlat0;
  let x_499 : vec4<f32> = x_33.unity_FogColor;
  u_xlat0 = (x_497 + -(vec3<f32>(x_499.x, x_499.y, x_499.z)));
  let x_505 : f32 = u_xlat24;
  let x_507 : vec3<f32> = u_xlat0;
  let x_510 : vec4<f32> = x_33.unity_FogColor;
  let x_512 : vec3<f32> = ((vec3<f32>(x_505, x_505, x_505) * x_507) + vec3<f32>(x_510.x, x_510.y, x_510.z));
  let x_513 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_512.x, x_512.y, x_512.z, x_513.w);
  SV_Target0.w = 1.0f;
  return;
}

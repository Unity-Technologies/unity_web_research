...
fn main_1() {
  ...
  vs_TEXCOORD3.w = x_189;
  let x_191 : vec4<f32> = u_xlat0;
  let x_193 : vec4<f32> = u_xlat0;
  let x_195 : vec2<f32> = (vec2<f32>(x_191.z, x_191.z) + vec2<f32>(x_193.x, x_193.w));
  let x_196 : vec4<f32> = vs_TEXCOORD3;
  vs_TEXCOORD3 = vec4<f32>(x_195.x, x_195.y, x_196.z, x_196.w);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @location(1) in_COLOR0_param : vec4<f32>, @location(2) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_COLOR0, vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD3);
}

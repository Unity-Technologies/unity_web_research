...
fn main_1() {
  ...
  let x_372 : vec2<f32> = vec2<f32>(x_371.z, x_371.w);
  let x_373 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_373.x, x_373.y, x_372.x, x_372.y);
  let x_376 : f32 = u_xlat0.y;
  u_xlat0.w = (x_376 * 0.5f);
  let x_379 : vec4<f32> = u_xlat0;
  let x_381 : vec4<f32> = u_xlat0;
  let x_383 : vec2<f32> = (vec2<f32>(x_379.z, x_379.z) + vec2<f32>(x_381.x, x_381.w));
  let x_384 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_383.x, x_383.y, x_384.z, x_384.w);
  ...
  vs_TEXCOORD7 = vec4<f32>(x_394.x, x_394.y, x_395.z, x_395.w);

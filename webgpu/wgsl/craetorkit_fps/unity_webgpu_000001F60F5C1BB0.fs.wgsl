...
fn main_1() {
  ...
  let x_364 : vec3<f32> = (vec3<f32>(x_360.x, x_360.y, x_360.z) * vec3<f32>(x_362.x, x_362.y, x_362.z));
  let x_365 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_364.x, x_364.y, x_364.z, x_365.w);
  let x_368 : f32 = vs_TEXCOORD1.w;
  u_xlat9 = x_368;
  let x_369 : f32 = u_xlat9;
  u_xlat9 = clamp(x_369, 0.0f, 1.0f);
  let x_373 : vec4<f32> = u_xlat0;
  let x_375 : f32 = u_xlat9;
  let x_377 : vec3<f32> = (vec3<f32>(x_373.x, x_373.y, x_373.z) * vec3<f32>(x_375, x_375, x_375));
  let x_378 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_377.x, x_377.y, x_377.z, x_378.w);
  SV_Target0.w = 1.0f;
  return;
}

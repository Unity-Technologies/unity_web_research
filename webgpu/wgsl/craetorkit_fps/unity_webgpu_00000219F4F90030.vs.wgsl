...
fn main_1() {
  ...
  u_xlat8.x = (x_646 + x_648);
  let x_654 : f32 = u_xlat8.x;
  vs_TEXCOORD4.z = -(x_654);
  let x_659 : f32 = u_xlat0.w;
  vs_TEXCOORD4.w = x_659;
  u_xlat1.x = 0.5f;
  u_xlat1.z = 0.5f;
  ...
  let x_671 : vec3<f32> = (vec3<f32>(x_667.x, x_667.z, x_667.w) * vec3<f32>(x_669.x, x_669.y, x_669.z));
  ...
  let x_678 : vec3<f32> = (vec3<f32>(x_674.x, x_674.y, x_674.z) * vec3<f32>(x_676.x, x_676.y, x_676.w));
  ...
  let x_684 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_684.x, x_684.y, x_683.x, x_683.y);
  let x_687 : f32 = u_xlat0.y;
  u_xlat0.w = (x_687 * 0.5f);
  let x_690 : vec4<f32> = u_xlat0;
  let x_692 : vec4<f32> = u_xlat0;
  let x_694 : vec2<f32> = (vec2<f32>(x_690.z, x_690.z) + vec2<f32>(x_692.x, x_692.w));
  let x_695 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_694.x, x_694.y, x_695.z, x_695.w);
  let x_698 : f32 = u_xlat3.y;
  let x_700 : f32 = u_xlat3.y;
  u_xlat0.x = (x_698 * x_700);
  ...
  u_xlat2 = (vec4<f32>(x_713.y, x_713.w, x_713.z, x_713.x) * x_715);
  let x_718 : vec4<f32> = x_184.unity_SHBr;
  let x_719 : vec4<f32> = u_xlat2;
  u_xlat3.x = dot(x_718, x_719);
  let x_723 : vec4<f32> = x_184.unity_SHBg;
  let x_724 : vec4<f32> = u_xlat2;
  u_xlat3.y = dot(x_723, x_724);
  let x_728 : vec4<f32> = x_184.unity_SHBb;
  let x_729 : vec4<f32> = u_xlat2;
  u_xlat3.z = dot(x_728, x_729);
  let x_735 : vec4<f32> = x_184.unity_SHC;
  let x_737 : vec4<f32> = u_xlat0;
  let x_740 : vec4<f32> = u_xlat3;
  vs_TEXCOORD5 = ((vec3<f32>(x_735.x, x_735.y, x_735.z) * vec3<f32>(x_737.x, x_737.x, x_737.x)) + vec3<f32>(x_740.x, x_740.y, x_740.z));
  let x_744 : f32 = u_xlat1.y;
  u_xlat1.w = (x_744 * 0.5f);
  let x_747 : vec4<f32> = u_xlat1;
  let x_749 : vec4<f32> = u_xlat1;
  let x_751 : vec2<f32> = (vec2<f32>(x_747.z, x_747.z) + vec2<f32>(x_749.x, x_749.w));
  let x_752 : vec4<f32> = vs_TEXCOORD7;
  vs_TEXCOORD7 = vec4<f32>(x_751.x, x_751.y, x_752.z, x_752.w);
  let x_756 : i32 = gl_InstanceIndex;
  vs_SV_InstanceID0 = bitcast<u32>(x_756);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @builtin(instance_index) gl_InstanceIndex_param : u32, @location(4) in_TANGENT0_param : vec4<f32>, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, vs_TEXCOORD0, vs_TEXCOORD2, gl_Position, vs_COLOR0, vs_TEXCOORD3, vs_TEXCOORD7, vs_TEXCOORD5);
}

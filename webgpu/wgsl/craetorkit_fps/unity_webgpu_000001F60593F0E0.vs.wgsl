...
fn main_1() {
  ...
  u_xlat14 = (x_509 * x_511);
  let x_514 : f32 = u_xlat8.x;
  let x_516 : f32 = u_xlat8.x;
  let x_518 : f32 = u_xlat14;
  u_xlat14 = ((x_514 * x_516) + -(x_518));
  let x_521 : vec3<f32> = u_xlat8;
  let x_523 : vec3<f32> = u_xlat8;
  u_xlat1 = (vec4<f32>(x_521.y, x_521.z, x_521.z, x_521.x) * vec4<f32>(x_523.x, x_523.y, x_523.z, x_523.z));
  let x_527 : vec4<f32> = x_184.unity_SHBr;
  let x_528 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_527, x_528);
  let x_532 : vec4<f32> = x_184.unity_SHBg;
  let x_533 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_532, x_533);
  let x_537 : vec4<f32> = x_184.unity_SHBb;
  let x_538 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_537, x_538);
  let x_543 : vec4<f32> = x_184.unity_SHC;
  let x_545 : f32 = u_xlat14;
  let x_548 : vec4<f32> = u_xlat2;
  vs_TEXCOORD3 = ((vec3<f32>(x_543.x, x_543.y, x_543.z) * vec3<f32>(x_545, x_545, x_545)) + vec3<f32>(x_548.x, x_548.y, x_548.z));
  let x_553 : f32 = u_xlat0.y;
  let x_555 : f32 = x_184.x_ProjectionParams.x;
  u_xlat7 = (x_553 * x_555);
  let x_557 : vec4<f32> = u_xlat0;
  let x_561 : vec2<f32> = (vec2<f32>(x_557.x, x_557.w) * vec2<f32>(0.5f, 0.5f));
  let x_562 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_561.x, x_562.y, x_561.y, x_562.w);
  let x_564 : f32 = u_xlat7;
  u_xlat0.w = (x_564 * 0.5f);
  let x_567 : vec4<f32> = u_xlat0;
  let x_569 : vec4<f32> = u_xlat0;
  let x_571 : vec2<f32> = (vec2<f32>(x_567.z, x_567.z) + vec2<f32>(x_569.x, x_569.w));
  let x_572 : vec4<f32> = vs_TEXCOORD5;
  vs_TEXCOORD5 = vec4<f32>(x_571.x, x_571.y, x_572.z, x_572.w);
  let x_576 : i32 = gl_InstanceIndex;
  vs_SV_InstanceID0 = bitcast<u32>(x_576);
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @builtin(instance_index) gl_InstanceIndex_param : u32, @location(1) in_NORMAL0_param : vec3<f32>, @location(2) in_COLOR0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD1, gl_Position, vs_TEXCOORD0, vs_COLOR0, vs_TEXCOORD2, vs_TEXCOORD4, vs_TEXCOORD5, vs_TEXCOORD3);
}

...
OUT . texcoord = ( IN . texcoord . xy . xy * _MainTex_ST . xy + _MainTex_ST . zw ) ;
OUT . mask = float4 ( IN . vertex . xy * 2 - clampedRect . xy - clampedRect . zw , 0.25 / ( 0.25 * float2 ( _UIMaskSoftnessX , _UIMaskSoftnessY ) + abs ( pixelSize . xy ) ) ) ;
...
float4 color = IN . color * UnityGetUIDiffuseColor ( IN . texcoord , _MainTex , _AlphaTex , _TextureSampleAdd ) ;
#line 122

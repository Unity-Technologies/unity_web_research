...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_867.w, x_867.x, x_867.y, x_867.z) + vec4<f32>(x_869.w, x_869.x, x_869.y, x_869.z));
  let x_874 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_874);
  ...
  if (x_876) {
    ...
    let x_887 : vec3<f32> = clamp(vec3<f32>(x_883.x, x_883.y, x_883.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_888 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_887.x, x_887.y, x_887.z, x_888.w);
    let x_890 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_890.x, x_890.y, x_890.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_900 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_900.y, x_900.z, x_900.w, x_900.x);
  return;
}

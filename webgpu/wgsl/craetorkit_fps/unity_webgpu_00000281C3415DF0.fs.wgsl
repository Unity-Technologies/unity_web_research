...
fn main_1() {
  ...
  let x_453 : vec3<f32> = ((vec3<f32>(x_446.x, x_446.y, x_446.z) * vec3<f32>(x_448.x, x_448.y, x_448.z)) + vec3<f32>(x_451.x, x_451.y, x_451.z));
  let x_454 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_453.x, x_453.y, x_453.z, x_454.w);
  let x_457 : vec4<f32> = u_xlat2;
  let x_460 : vec3<f32> = exp2(-(vec3<f32>(x_457.x, x_457.y, x_457.z)));
  let x_461 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_460.x, x_460.y, x_460.z, x_461.w);
  let x_464 : vec4<f32> = u_xlat1;
  let x_468 : vec3<f32> = ((vec3<f32>(x_464.x, x_464.y, x_464.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_469 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_468.x, x_468.y, x_468.z, x_469.w);

...
fn main_1() {
  ...
  let x_266 : vec2<f32> = (vec2<f32>(x_260.y, x_260.y) * vec2<f32>(x_264.x, x_264.y));
  ...
  let x_277 : vec2<f32> = ((vec2<f32>(x_270.x, x_270.y) * vec2<f32>(x_272.x, x_272.x)) + vec2<f32>(x_275.x, x_275.y));
  let x_278 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_277.x, x_277.y, x_278.z, x_278.w);
  let x_281 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_283 : vec4<f32> = u_xlat0;
  let x_286 : vec4<f32> = u_xlat0;
  let x_288 : vec2<f32> = ((vec2<f32>(x_281.x, x_281.y) * vec2<f32>(x_283.z, x_283.z)) + vec2<f32>(x_286.x, x_286.y));
  let x_289 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_288.x, x_288.y, x_289.z, x_289.w);
  let x_294 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_296 : vec4<f32> = u_xlat0;
  let x_299 : vec4<f32> = u_xlat0;
  vs_TEXCOORD2 = ((vec2<f32>(x_294.x, x_294.y) * vec2<f32>(x_296.w, x_296.w)) + vec2<f32>(x_299.x, x_299.y));

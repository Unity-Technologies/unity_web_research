...
fn main_1() {
  ...
  let x_307 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_305, x_305));
  u_xlat20 = x_307.x;
  let x_317 : vec3<f32> = u_xlat3;
  let x_318 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, x_317);
  u_xlat3.x = x_318.w;
  let x_321 : f32 = u_xlat20;
  let x_323 : f32 = u_xlat3.x;
  u_xlat20 = (x_321 * x_323);
  ...
  u_xlat3 = (vec3<f32>(x_328, x_328, x_328) * vec3<f32>(x_332.x, x_332.y, x_332.z));
  let x_336 : vec4<f32> = vs_TEXCOORD1;
  let x_339 : vec3<f32> = u_xlat2;
  u_xlat19 = dot(-(vec3<f32>(x_336.x, x_336.y, x_336.z)), x_339);
  let x_341 : f32 = u_xlat19;
  let x_342 : f32 = u_xlat19;
  u_xlat19 = (x_341 + x_342);
  let x_344 : vec3<f32> = u_xlat2;
  let x_345 : f32 = u_xlat19;
  let x_349 : vec4<f32> = vs_TEXCOORD1;
  let x_352 : vec3<f32> = ((x_344 * -(vec3<f32>(x_345, x_345, x_345))) + -(vec3<f32>(x_349.x, x_349.y, x_349.z)));
  let x_353 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_352.x, x_352.y, x_352.z, x_353.w);
  let x_358 : f32 = vs_TEXCOORD2.w;
  u_xlat5.x = x_358;
  let x_361 : f32 = vs_TEXCOORD3.w;
  u_xlat5.y = x_361;
  let x_364 : f32 = vs_TEXCOORD4.w;
  u_xlat5.z = x_364;
  let x_366 : vec3<f32> = u_xlat2;
  let x_367 : vec3<f32> = u_xlat5;
  u_xlat19 = dot(x_366, x_367);
  let x_369 : f32 = u_xlat19;
  u_xlat19 = clamp(x_369, 0.0f, 1.0f);
  ...
  u_xlat2.x = (x_377 * x_379);
  let x_383 : f32 = u_xlat2.x;
  let x_385 : f32 = u_xlat2.x;
  u_xlat2.x = (x_383 * x_385);
  let x_390 : f32 = x_33.x_Glossiness;
  u_xlat2.y = (-(x_390) + 1.0f);
  let x_399 : vec3<f32> = u_xlat2;
  let x_401 : vec4<f32> = textureSample(unity_NHxRoughness, samplerunity_NHxRoughness, vec2<f32>(x_399.x, x_399.y));
  u_xlat2.x = x_401.x;
  let x_405 : f32 = u_xlat2.x;
  u_xlat2.x = (x_405 * 16.0f);
  let x_409 : vec3<f32> = u_xlat0;
  let x_410 : vec3<f32> = u_xlat2;
  u_xlat0 = (x_409 * vec3<f32>(x_410.x, x_410.x, x_410.x));
  let x_413 : vec3<f32> = u_xlat1;
  let x_414 : f32 = u_xlat18;
  let x_417 : vec3<f32> = u_xlat0;
  u_xlat0 = ((x_413 * vec3<f32>(x_414, x_414, x_414)) + x_417);
  let x_419 : f32 = u_xlat19;
  let x_421 : vec3<f32> = u_xlat3;
  u_xlat1 = (vec3<f32>(x_419, x_419, x_419) * x_421);
  let x_425 : vec3<f32> = u_xlat0;
  let x_426 : vec3<f32> = u_xlat1;
  let x_427 : vec3<f32> = (x_425 * x_426);
  let x_428 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_427.x, x_427.y, x_427.z, x_428.w);
  SV_Target0.w = 1.0f;
  return;
}

...
layout(set = 0, binding = 2) uniform mediump texture3D _DisplacementNoise;
...
layout(set = 0, binding = 5) uniform mediump  sampler sampler_DisplacementNoise;
void main()
{
vec4 u_xlat0;
vec4 u_xlat1;
...
float u_xlat7;
    ...
    u_xlat0.xyz = u_xlat0.xyz + _SinTime.zzz;
    u_xlat0.x = texture(sampler3D(_DisplacementNoise, sampler_DisplacementNoise), u_xlat0.xyz).x;
    u_xlat2.xyz = texture(sampler2D(_EmissionMap, sampler_EmissionMap), vs_TEXCOORD0.xy).xyz;

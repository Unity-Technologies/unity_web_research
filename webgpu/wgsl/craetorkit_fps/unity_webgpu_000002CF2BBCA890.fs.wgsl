...
@group(0) @binding(15) var sampler_VolumeNoise : sampler;

@group(0) @binding(1) var x_AlbedoRamp1 : texture_2d<f32>;

@group(0) @binding(13) var sampler_AlbedoRamp1 : sampler;

@group(0) @binding(2) var x_AlbedoRamp2 : texture_2d<f32>;

@group(0) @binding(14) var sampler_AlbedoRamp2 : sampler;
...
fn main_1() {
  ...
  let x_553 : vec4<f32> = textureSample(x_LightTextureB0, sampler_LightTextureB0, vec2<f32>(x_551.x, x_551.x));
  u_xlat1.x = x_553.x;
  let x_564 : vec4<f32> = u_xlat4;
  let x_566 : vec4<f32> = textureSample(x_LightTexture0, sampler_LightTexture0, vec3<f32>(x_564.x, x_564.y, x_564.z));
  u_xlat7.x = x_566.w;
  ...
  u_xlat18 = (x_575 * x_577);
  let x_579 : f32 = u_xlat18;
  let x_583 : vec4<f32> = x_20.x_LightColor0;
  let x_585 : vec3<f32> = (vec3<f32>(x_579, x_579, x_579) * vec3<f32>(x_583.x, x_583.y, x_583.z));
  let x_586 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_585.x, x_585.y, x_585.z, x_586.w);
  let x_589 : vec3<f32> = vs_TEXCOORD0;
  let x_590 : vec3<f32> = vs_TEXCOORD0;
  u_xlat18 = dot(x_589, x_590);
  let x_592 : f32 = u_xlat18;
  u_xlat18 = inverseSqrt(x_592);
  let x_594 : f32 = u_xlat18;
  let x_596 : vec3<f32> = vs_TEXCOORD0;
  let x_597 : vec3<f32> = (vec3<f32>(x_594, x_594, x_594) * x_596);
  let x_598 : vec4<f32> = u_xlat4;
  u_xlat4 = vec4<f32>(x_597.x, x_597.y, x_597.z, x_598.w);
  let x_600 : vec4<f32> = u_xlat3;
  let x_604 : vec3<f32> = (vec3<f32>(x_600.x, x_600.y, x_600.z) + vec3<f32>(-0.039999999f, -0.039999999f, -0.039999999f));
  let x_605 : vec4<f32> = u_xlat5;
  u_xlat5 = vec4<f32>(x_604.x, x_604.y, x_604.z, x_605.w);
  let x_609 : f32 = x_20.x_Metallic;
  let x_611 : f32 = x_20.x_Metallic;
  let x_613 : f32 = x_20.x_Metallic;
  let x_614 : vec3<f32> = vec3<f32>(x_609, x_611, x_613);
  let x_619 : vec4<f32> = u_xlat5;
  let x_624 : vec3<f32> = ((vec3<f32>(x_614.x, x_614.y, x_614.z) * vec3<f32>(x_619.x, x_619.y, x_619.z)) + vec3<f32>(0.039999999f, 0.039999999f, 0.039999999f));
  let x_625 : vec4<f32> = u_xlat5;
  u_xlat5 = vec4<f32>(x_624.x, x_624.y, x_624.z, x_625.w);
  let x_628 : f32 = x_20.x_Metallic;
  u_xlat18 = ((-(x_628) * 0.959999979f) + 0.959999979f);
  let x_633 : vec3<f32> = u_xlat2;
  let x_634 : vec4<f32> = u_xlat4;
  u_xlat19 = dot(x_633, vec3<f32>(x_634.x, x_634.y, x_634.z));
  let x_637 : f32 = u_xlat19;
  let x_638 : f32 = u_xlat19;
  u_xlat19 = (x_637 + x_638);
  let x_640 : vec4<f32> = u_xlat4;
  let x_642 : f32 = u_xlat19;
  let x_646 : vec3<f32> = u_xlat2;
  u_xlat2 = ((vec3<f32>(x_640.x, x_640.y, x_640.z) * -(vec3<f32>(x_642, x_642, x_642))) + x_646);
  let x_648 : vec4<f32> = u_xlat4;
  let x_650 : vec3<f32> = u_xlat0;
  u_xlat19 = dot(vec3<f32>(x_648.x, x_648.y, x_648.z), x_650);
  let x_652 : f32 = u_xlat19;
  u_xlat19 = clamp(x_652, 0.0f, 1.0f);
  let x_654 : vec3<f32> = u_xlat2;
  let x_655 : vec3<f32> = u_xlat0;
  u_xlat0.x = dot(x_654, x_655);
  let x_659 : f32 = u_xlat0.x;
  let x_661 : f32 = u_xlat0.x;
  u_xlat0.x = (x_659 * x_661);
  let x_665 : f32 = u_xlat0.x;
  let x_667 : f32 = u_xlat0.x;
  u_xlat0.x = (x_665 * x_667);
  let x_672 : f32 = x_20.x_Glossiness;
  let x_675 : f32 = u_xlat3.w;
  u_xlat0.y = ((-(x_672) * x_675) + 1.0f);
  let x_684 : vec3<f32> = u_xlat0;
  let x_686 : vec4<f32> = textureSample(unity_NHxRoughness, samplerunity_NHxRoughness, vec2<f32>(x_684.x, x_684.y));
  u_xlat0.x = x_686.x;
  let x_690 : f32 = u_xlat0.x;
  u_xlat0.x = (x_690 * 16.0f);
  let x_693 : vec4<f32> = u_xlat5;
  let x_695 : vec3<f32> = u_xlat0;
  u_xlat0 = (vec3<f32>(x_693.x, x_693.y, x_693.z) * vec3<f32>(x_695.x, x_695.x, x_695.x));
  let x_698 : vec4<f32> = u_xlat3;
  let x_700 : f32 = u_xlat18;
  let x_703 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_698.x, x_698.y, x_698.z) * vec3<f32>(x_700, x_700, x_700)) + x_703);
  ...
  u_xlat0 = (x_712 * vec3<f32>(x_713.x, x_713.y, x_713.z));
  let x_718 : f32 = vs_TEXCOORD4;
  u_xlat18 = x_718;
  let x_719 : f32 = u_xlat18;
  u_xlat18 = clamp(x_719, 0.0f, 1.0f);

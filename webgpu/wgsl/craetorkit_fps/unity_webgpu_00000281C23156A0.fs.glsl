...
layout(set = 0, binding = 1) uniform mediump texture2D _To;
layout(location = 0) in highp vec2 vs_TEXCOORD0;
layout(location = 0) out highp vec4 SV_Target0;
layout(set = 0, binding = 2) uniform mediump  sampler sampler_MainTex;
layout(set = 0, binding = 3) uniform mediump  sampler sampler_To;
void main()
{
vec4 u_xlat0;
vec4 u_xlat1;
    u_xlat0 = texture(sampler2D(_To, sampler_To), vs_TEXCOORD0.xy);

...
layout(set = 0, binding = 3) uniform highp texture2D HiResAO;
writeonly layout(set = 0, binding = 4, r32f) highp uniform image2D AoResult_origX0X;
shared struct {
	uint value[1];
} TGSM0[256];
shared struct {
...
layout(set = 0, binding = 8) uniform highp  sampler samplerHiResAO;
void main()
{
vec4 u_xlat0;
int u_xlati0;
...
float u_xlat28;
    ...
    if(u_xlatb0.x){
        ...
        u_xlat4.x = uintBitsToFloat(TGSM0[u_xlati1.x].value[(0 >> 2) + 0]);
        ...
        u_xlat6.x = (u_xlati11 != 0) ? u_xlat10 : u_xlat2.x;
        u_xlat5 = u_xlat7;
        u_xlat11 = (u_xlatb4) ? u_xlat18.x : u_xlat6.x;
        ...
        u_xlat6.x = (u_xlati24 != 0) ? u_xlat18.x : u_xlat10;
        u_xlat3.x = u_xlat7;
        u_xlat24 = (u_xlatb17) ? u_xlat26 : u_xlat6.x;
        u_xlat16.x = u_xlat24 + u_xlat16.x;
        u_xlat16.x = u_xlat16.x * 0.5 + u_xlat5;
    ...
    if(u_xlatb0.y){
        ...
        u_xlat4.x = u_xlat26 * u_xlat26 + AdditionalParams.y;
    ...
    u_xlatu2 =  uvec4(ivec4(gl_GlobalInvocationID.xyyy) << (ivec4(1, 1, 1, 1) & int(0x1F)));
    u_xlat16.xy = vec2(u_xlatu2.xw);
    u_xlat16.xy = u_xlat16.xy * InvHighResolution.xy;
    u_xlat3 = textureGather(sampler2D(HiResAO, samplerHiResAO), u_xlat16.xy);
    u_xlat4 = textureGather(sampler2D(LoResDB, samplerLoResDB), u_xlat0.xy);
    u_xlat0 = textureGather(sampler2D(HiResDB, samplerHiResDB), u_xlat16.xy);
    u_xlati5 = ivec4(u_xlatu2.xwxw) + ivec4(int(0xFFFFFFFFu), 0, 0, int(0xFFFFFFFFu));
    u_xlat6 = (-u_xlat4) + u_xlat0.xxxx;
    u_xlat6 = abs(u_xlat6) + AdditionalParams.wwww;
    u_xlat6 = vec4(9.0, 3.0, 1.0, 3.0) / u_xlat6;
    u_xlat0.x = dot(u_xlat6, vec4(1.0, 1.0, 1.0, 1.0));
    u_xlat0.x = u_xlat0.x + AdditionalParams.x;
    u_xlat6.x = dot(u_xlat1.yzwx, u_xlat6);
    u_xlat6.x = u_xlat6.x + AdditionalParams.x;
    u_xlat3.x = u_xlat3.x * u_xlat6.x;
    u_xlat0.x = u_xlat3.x / u_xlat0.x;
    imageStore(AoResult_origX0X, u_xlati5.xy, u_xlat0.xxxx);
    u_xlat6 = (-u_xlat4.yzwx) + u_xlat0.yyyy;
    u_xlat6 = abs(u_xlat6) + AdditionalParams.wwww;
    u_xlat6 = vec4(9.0, 3.0, 1.0, 3.0) / u_xlat6;
    u_xlat0.x = dot(u_xlat6, vec4(1.0, 1.0, 1.0, 1.0));
    u_xlat0.y = dot(u_xlat1.zwxy, u_xlat6);
    u_xlat0.xy = u_xlat0.xy + AdditionalParams.xx;
    u_xlat8.x = u_xlat0.y * u_xlat3.y;
    u_xlat0.x = u_xlat8.x / u_xlat0.x;
    imageStore(AoResult_origX0X, ivec2(u_xlatu2.xw), u_xlat0.xxxx);
    u_xlat6 = (-u_xlat4.zwxy) + u_xlat0.zzzz;
    u_xlat6 = abs(u_xlat6) + AdditionalParams.wwww;
    u_xlat6 = vec4(9.0, 3.0, 1.0, 3.0) / u_xlat6;
    u_xlat0.x = dot(u_xlat6, vec4(1.0, 1.0, 1.0, 1.0));
    u_xlat0.y = dot(u_xlat1.wxyz, u_xlat6);
    u_xlat0.xy = u_xlat0.xy + AdditionalParams.xx;
    u_xlat8.x = u_xlat0.y * u_xlat3.z;
    u_xlat0.x = u_xlat8.x / u_xlat0.x;
    imageStore(AoResult_origX0X, u_xlati5.zw, u_xlat0.xxxx);
    u_xlati2 = ivec4(u_xlatu2) + ivec4(int(0xFFFFFFFFu), int(0xFFFFFFFFu), int(0xFFFFFFFFu), int(0xFFFFFFFFu));
    u_xlat0 = (-u_xlat4.wxyz) + u_xlat0.wwww;

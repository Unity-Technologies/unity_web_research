...
fn main_1() {
  ...
  let x_380 : vec2<f32> = (vec2<f32>(x_374.y, x_374.y) * vec2<f32>(x_378.x, x_378.y));
  ...
  let x_391 : vec2<f32> = ((vec2<f32>(x_384.x, x_384.y) * vec2<f32>(x_386.x, x_386.x)) + vec2<f32>(x_389.x, x_389.y));
  let x_392 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_391.x, x_391.y, x_392.z, x_392.w);
  let x_395 : vec4<f32> = x_17.unity_WorldToLight[2i];
  let x_397 : vec4<f32> = u_xlat0;
  let x_400 : vec4<f32> = u_xlat0;
  let x_402 : vec2<f32> = ((vec2<f32>(x_395.x, x_395.y) * vec2<f32>(x_397.z, x_397.z)) + vec2<f32>(x_400.x, x_400.y));
  ...
  vs_TEXCOORD6 = ((vec2<f32>(x_408.x, x_408.y) * vec2<f32>(x_410.w, x_410.w)) + vec2<f32>(x_413.x, x_413.y));

...
fn main_1() {
  ...
  txVec0 = vec3<f32>(x_365.x, x_365.y, x_366);
  let x_378 : vec3<f32> = txVec0;
  let x_380 : f32 = textureSampleCompareLevel(x_ShadowMapTexture, sampler_ShadowMapTexture, x_378.xy, x_378.z);
  u_xlat0.x = x_380;
  let x_386 : f32 = x_61.x_LightShadowData.x;
  u_xlat4 = (-(x_386) + 1.0f);
  let x_391 : vec4<f32> = u_xlat0;
  let x_393 : f32 = u_xlat4;
  let x_397 : vec4<f32> = x_61.x_LightShadowData;
  SV_Target0 = ((vec4<f32>(x_391.x, x_391.x, x_391.x, x_391.x) * vec4<f32>(x_393, x_393, x_393, x_393)) + vec4<f32>(x_397.x, x_397.x, x_397.x, x_397.x));
  return;
}

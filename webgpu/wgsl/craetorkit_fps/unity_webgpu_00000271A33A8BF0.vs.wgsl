...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_286.y, x_286.w, x_286.z, x_286.x) * x_288);
  let x_291 : vec4<f32> = x_17.unity_SHBr;
  let x_292 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_291, x_292);
  let x_296 : vec4<f32> = x_17.unity_SHBg;
  let x_297 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_296, x_297);
  let x_301 : vec4<f32> = x_17.unity_SHBb;
  let x_302 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_301, x_302);
  let x_308 : vec4<f32> = x_17.unity_SHC;
  let x_310 : vec4<f32> = u_xlat0;
  let x_313 : vec4<f32> = u_xlat2;
  vs_TEXCOORD6 = ((vec3<f32>(x_308.x, x_308.y, x_308.z) * vec3<f32>(x_310.x, x_310.x, x_310.x)) + vec3<f32>(x_313.x, x_313.y, x_313.z));
  return;
}

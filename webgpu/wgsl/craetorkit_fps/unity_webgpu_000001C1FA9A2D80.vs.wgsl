...
fn main_1() {
  ...
  let x_338 : vec3<f32> = (vec3<f32>(x_332.y, x_332.y, x_332.y) * vec3<f32>(x_336.x, x_336.y, x_336.z));
  let x_339 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_338.x, x_338.y, x_338.z, x_339.w);
  let x_342 : vec4<f32> = x_18.unity_WorldToLight[0i];
  ...
  let x_353 : vec4<f32> = x_18.unity_WorldToLight[2i];
  ...
  let x_366 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_368 : vec4<f32> = u_xlat0;
  let x_371 : vec4<f32> = u_xlat0;
  vs_TEXCOORD6 = ((vec3<f32>(x_366.x, x_366.y, x_366.z) * vec3<f32>(x_368.w, x_368.w, x_368.w)) + vec3<f32>(x_371.x, x_371.y, x_371.z));

...
fn main_1() {
  ...
  vs_TEXCOORD1 = (vec3<f32>(x_251, x_251, x_251) * vec3<f32>(x_253.x, x_253.y, x_253.z));
  let x_257 : f32 = u_xlat0.y;
  let x_260 : f32 = x_131.x_ProjectionParams.x;
  u_xlat0.y = (x_257 * x_260);
  let x_263 : vec4<f32> = u_xlat0;
  let x_267 : vec3<f32> = (vec3<f32>(x_263.x, x_263.w, x_263.y) * vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_268 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_267.x, x_268.y, x_267.y, x_267.z);
  let x_271 : vec4<f32> = u_xlat0;
  let x_272 : vec2<f32> = vec2<f32>(x_271.z, x_271.w);
  let x_273 : vec4<f32> = vs_TEXCOORD5;
  vs_TEXCOORD5 = vec4<f32>(x_273.x, x_273.y, x_272.x, x_272.y);
  ...
  vs_TEXCOORD5 = vec4<f32>(x_279.x, x_279.y, x_280.z, x_280.w);
  return;
}
...
fn main(@location(1) in_TEXCOORD0_param : vec4<f32>, @location(2) in_NORMAL0_param : vec3<f32>, @location(0) in_POSITION0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD2, gl_Position, vs_TEXCOORD6, vs_TEXCOORD0, vs_TEXCOORD3, vs_TEXCOORD1, vs_TEXCOORD5);
}

...
fn main_1() {
  ...
  let x_687 : vec3<f32> = ((vec3<f32>(x_680.z, x_680.x, x_680.y) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_716 : vec3<f32> = x_55.x_Lut2D_Params;
  u_xlat8 = (vec3<f32>(x_712.x, x_712.y, x_712.z) * vec3<f32>(x_716.z, x_716.z, x_716.z));
  let x_720 : f32 = u_xlat8.x;
  u_xlat8.x = floor(x_720);
  let x_724 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_727 : vec2<f32> = (vec2<f32>(x_724.x, x_724.y) * vec2<f32>(0.5f, 0.5f));
  let x_728 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_727.x, x_727.y, x_728.z, x_728.w);
  let x_730 : vec3<f32> = u_xlat8;
  let x_733 : vec3<f32> = x_55.x_Lut2D_Params;
  let x_736 : vec4<f32> = u_xlat2;
  let x_738 : vec2<f32> = ((vec2<f32>(x_730.y, x_730.z) * vec2<f32>(x_733.x, x_733.y)) + vec2<f32>(x_736.x, x_736.y));
  let x_739 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_739.x, x_738.x, x_738.y, x_739.w);
  let x_742 : f32 = u_xlat8.x;
  let x_744 : f32 = x_55.x_Lut2D_Params.y;
  let x_747 : f32 = u_xlat2.y;
  u_xlat2.x = ((x_742 * x_744) + x_747);
  let x_755 : vec4<f32> = u_xlat2;
  let x_757 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, vec2<f32>(x_755.x, x_755.z));
  ...
  let x_762 : f32 = x_55.x_Lut2D_Params.y;
  u_xlat4.x = x_762;
  u_xlat4.y = 0.0f;
  let x_767 : vec4<f32> = u_xlat2;
  let x_769 : vec4<f32> = u_xlat4;
  u_xlat15 = (vec2<f32>(x_767.x, x_767.z) + vec2<f32>(x_769.x, x_769.y));
  let x_775 : vec2<f32> = u_xlat15;
  let x_776 : vec4<f32> = textureSample(x_Lut2D, sampler_Lut2D, x_775);
  ...
  let x_783 : f32 = x_55.x_Lut2D_Params.z;
  let x_786 : f32 = u_xlat8.x;
  u_xlat1.x = ((x_781 * x_783) + -(x_786));
  let x_790 : vec4<f32> = u_xlat3;
  let x_793 : vec4<f32> = u_xlat2;
  u_xlat8 = (-(vec3<f32>(x_790.x, x_790.y, x_790.z)) + vec3<f32>(x_793.x, x_793.y, x_793.z));
  ...
  let x_807 : f32 = x_55.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_807);
  ...
  if (x_809) {
    ...
    let x_820 : vec3<f32> = clamp(vec3<f32>(x_816.x, x_816.y, x_816.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_821 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_820.x, x_820.y, x_820.z, x_821.w);
    let x_823 : vec4<f32> = u_xlat1;
    u_xlat0.w = dot(vec3<f32>(x_823.x, x_823.y, x_823.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_360 : vec3<f32> = exp2(-(vec3<f32>(x_357.x, x_357.y, x_357.z)));
  let x_361 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_360.x, x_360.y, x_360.z, x_361.w);
  let x_364 : vec4<f32> = u_xlat1;
  let x_368 : vec3<f32> = ((vec3<f32>(x_364.x, x_364.y, x_364.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_369 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_368.x, x_368.y, x_368.z, x_369.w);
  let x_372 : vec4<f32> = u_xlat0;
  SV_Target0 = x_372;
  let x_375 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_375;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

...
fn main_1() {
  ...
  u_xlat0.x = ((x_403 * x_405) + -(x_408));
  let x_412 : vec4<f32> = u_xlat3;
  let x_414 : vec4<f32> = u_xlat3;
  u_xlat1 = (vec4<f32>(x_412.y, x_412.w, x_412.z, x_412.x) * x_414);
  let x_417 : vec4<f32> = x_17.unity_SHBr;
  let x_418 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_417, x_418);
  let x_422 : vec4<f32> = x_17.unity_SHBg;
  let x_423 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_422, x_423);
  let x_427 : vec4<f32> = x_17.unity_SHBb;
  let x_428 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_427, x_428);
  let x_435 : vec4<f32> = x_17.unity_SHC;
  let x_437 : vec4<f32> = u_xlat0;
  let x_440 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_435.x, x_435.y, x_435.z) * vec3<f32>(x_437.x, x_437.x, x_437.x)) + vec3<f32>(x_440.x, x_440.y, x_440.z));
  return;
}

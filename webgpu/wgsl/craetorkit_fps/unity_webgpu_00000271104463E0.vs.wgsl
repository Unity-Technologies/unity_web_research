...
fn main_1() {
  ...
  u_xlat2.x = dot(x_403, x_404);
  let x_408 : vec4<f32> = x_17.unity_SHBg;
  let x_409 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_408, x_409);
  let x_413 : vec4<f32> = x_17.unity_SHBb;
  let x_414 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_413, x_414);
  let x_421 : vec4<f32> = x_17.unity_SHC;
  let x_423 : vec4<f32> = u_xlat0;
  let x_426 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_421.x, x_421.y, x_421.z) * vec3<f32>(x_423.x, x_423.x, x_423.x)) + vec3<f32>(x_426.x, x_426.y, x_426.z));
  return;
}

...
fn main_1() {
  ...
  if (x_50) {
    ...
    let x_98 : vec4<f32> = x_31.unity_ProbeVolumeWorldToObject[3i];
  ...
  let x_195 : vec3<f32> = (x_192 * vec3<f32>(x_193.x, x_193.y, x_193.z));
  let x_196 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_195.x, x_195.y, x_195.z, x_196.w);
  let x_198 : vec3<f32> = u_xlat0;
  let x_200 : vec3<f32> = vs_TEXCOORD3;
  u_xlat0 = (x_198 * x_200);
  let x_202 : vec4<f32> = u_xlat1;
  let x_204 : f32 = u_xlat6;
  let x_207 : vec3<f32> = u_xlat0;
  u_xlat0 = ((vec3<f32>(x_202.x, x_202.y, x_202.z) * vec3<f32>(x_204, x_204, x_204)) + x_207);
  let x_211 : f32 = vs_TEXCOORD4;
  u_xlat6 = x_211;
  let x_212 : f32 = u_xlat6;
  u_xlat6 = clamp(x_212, 0.0f, 1.0f);
  let x_214 : vec3<f32> = u_xlat0;
  let x_216 : vec4<f32> = x_31.unity_FogColor;
  u_xlat0 = (x_214 + -(vec3<f32>(x_216.x, x_216.y, x_216.z)));
  let x_222 : f32 = u_xlat6;
  let x_224 : vec3<f32> = u_xlat0;
  let x_227 : vec4<f32> = x_31.unity_FogColor;
  let x_229 : vec3<f32> = ((vec3<f32>(x_222, x_222, x_222) * x_224) + vec3<f32>(x_227.x, x_227.y, x_227.z));
  let x_230 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_229.x, x_229.y, x_229.z, x_230.w);
  SV_Target0.w = 1.0f;
  return;
}
...
@fragment
fn main(@location(0) vs_TEXCOORD0_param : vec2<f32>, @location(3) vs_TEXCOORD2_param : vec3<f32>, @location(2) vs_TEXCOORD1_param : vec3<f32>, @location(4) vs_TEXCOORD3_param : vec3<f32>, @location(1) vs_TEXCOORD4_param : f32, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

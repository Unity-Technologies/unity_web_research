...
fn main_1() {
  ...
  if (x_354) {
    ...
    if (x_527) {
      ...
      let x_534 : f32 = u_xlat11;
      x_528 = x_534;
    }
    let x_535 : f32 = x_528;
    u_xlat1.x = x_535;
    let x_538 : vec3<f32> = x_49.x_Vignette_Color;
    u_xlat6 = (-(x_538) + vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_541 : vec4<f32> = u_xlat1;
    let x_543 : vec3<f32> = u_xlat6;
    let x_546 : vec3<f32> = x_49.x_Vignette_Color;
    u_xlat6 = ((vec3<f32>(x_541.x, x_541.x, x_541.x) * x_543) + x_546);
    let x_548 : vec4<f32> = u_xlat0;
    let x_550 : vec3<f32> = u_xlat6;
    let x_552 : vec4<f32> = u_xlat0;
    u_xlat6 = ((vec3<f32>(x_548.x, x_548.y, x_548.z) * x_550) + -(vec3<f32>(x_552.x, x_552.y, x_552.z)));
    let x_558 : f32 = x_49.x_Vignette_Opacity;
    let x_560 : vec3<f32> = u_xlat6;
    let x_562 : vec4<f32> = u_xlat0;
    let x_564 : vec3<f32> = ((vec3<f32>(x_558, x_558, x_558) * x_560) + vec3<f32>(x_562.x, x_562.y, x_562.z));
    let x_565 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_564.x, x_564.y, x_564.z, x_565.w);
    let x_568 : f32 = u_xlat0.w;
    u_xlat0.x = (x_568 + -1.0f);
    let x_572 : f32 = u_xlat1.x;
    let x_574 : f32 = u_xlat0.x;
    u_xlat2.w = ((x_572 * x_574) + 1.0f);
  }
  let x_578 : vec4<f32> = u_xlat2;
  let x_582 : f32 = x_49.x_PostExposure;
  let x_584 : f32 = x_49.x_PostExposure;
  let x_586 : f32 = x_49.x_PostExposure;
  let x_588 : f32 = x_49.x_PostExposure;
  let x_589 : vec4<f32> = vec4<f32>(x_582, x_584, x_586, x_588);
  u_xlat0 = (vec4<f32>(x_578.w, x_578.x, x_578.y, x_578.z) * vec4<f32>(x_589.x, x_589.y, x_589.z, x_589.w));
  let x_596 : vec4<f32> = u_xlat0;
  let x_603 : vec3<f32> = ((vec3<f32>(x_596.y, x_596.z, x_596.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_618 : vec3<f32> = ((vec3<f32>(x_611.x, x_611.y, x_611.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  ...
  let x_632 : vec2<f32> = x_49.x_Lut3D_Params;
  let x_634 : vec3<f32> = (vec3<f32>(x_628.x, x_628.y, x_628.z) * vec3<f32>(x_632.y, x_632.y, x_632.y));
  let x_635 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_634.x, x_634.y, x_634.z, x_635.w);
  let x_638 : f32 = x_49.x_Lut3D_Params.x;
  u_xlat16 = (x_638 * 0.5f);
  let x_640 : vec4<f32> = u_xlat1;
  let x_643 : vec2<f32> = x_49.x_Lut3D_Params;
  ...
  let x_667 : f32 = x_49.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_667);
  let x_669 : bool = u_xlatb1;
  if (x_669) {
    let x_672 : vec4<f32> = u_xlat0;
    let x_673 : vec3<f32> = vec3<f32>(x_672.y, x_672.z, x_672.w);
    let x_674 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_673.x, x_673.y, x_673.z, x_674.w);
    let x_676 : vec4<f32> = u_xlat1;
    let x_680 : vec3<f32> = clamp(vec3<f32>(x_676.x, x_676.y, x_676.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_681 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_680.x, x_680.y, x_680.z, x_681.w);
    let x_683 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_683.x, x_683.y, x_683.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_693 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_693.y, x_693.z, x_693.w, x_693.x);
  return;
}

...
layout(set = 0, binding = 10) uniform mediump  sampler sampler_Specular;
layout(set = 0, binding = 11) uniform mediump  sampler sampler_DissolveNoise;
void main()
{
vec3 u_xlat0;
vec3 u_xlat1;
...
float u_xlat24;
    ...
    u_xlat22 = texture(sampler2D(_DissolveNoise, sampler_DissolveNoise), vs_TEXCOORD0.xy).x;
    u_xlat2.x = _EdgeSize + _Cutoff;
    u_xlat3 = texture(sampler2D(_MainTex, sampler_MainTex), vs_TEXCOORD0.xy);
    u_xlat4 = u_xlat3 * _Color;
    u_xlat9.xy = texture(sampler2D(_Specular, sampler_Specular), vs_TEXCOORD0.xy).xw;
    u_xlat9.x = u_xlat9.x * _Metallic;
    ...
    u_xlat22 = (-_Cutoff) * u_xlat2.x + u_xlat22;
    u_xlatb22 = u_xlat22<0.0;
    if(((int(u_xlatb22) * int(0xffffffffu)))!=0){discard;}
    u_xlat2.xw = vs_TEXCOORD4.yy * unity_WorldToLight[1].xy;
    u_xlat2.xw = unity_WorldToLight[0].xy * vs_TEXCOORD4.xx + u_xlat2.xw;
    u_xlat2.xw = unity_WorldToLight[2].xy * vs_TEXCOORD4.zz + u_xlat2.xw;
    u_xlat2.xw = u_xlat2.xw + unity_WorldToLight[3].xy;
    ...
    u_xlat2.x = texture(sampler2D(_LightTexture0, sampler_LightTexture0), u_xlat2.xw).w;
    ...
    u_xlat3.xyz = u_xlat9.xxx * u_xlat3.xyz + vec3(0.0399999991, 0.0399999991, 0.0399999991);
    u_xlat22 = (-u_xlat9.x) * 0.959999979 + 0.959999979;
    u_xlat2.xyw = vec3(u_xlat22) * u_xlat4.xyz;
    u_xlat22 = (-u_xlat9.y) * _Glossiness + 1.0;
    ...
    SV_Target0.xyz = u_xlat2.xyw * u_xlat1.xyz + u_xlat0.xyz;

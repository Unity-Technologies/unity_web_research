...
fn main_1() {
  ...
  let x_217 : vec4<f32> = x_131.x_Gradient_ST;
  vs_TEXCOORD0 = ((vec2<f32>(x_210.x, x_210.y) * vec2<f32>(x_213.x, x_213.y)) + vec2<f32>(x_217.z, x_217.w));
  let x_222 : f32 = in_TEXCOORD0.z;
  vs_TEXCOORD3 = x_222;
  let x_223 : vec3<f32> = in_NORMAL0;
  let x_225 : vec4<f32> = x_131.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_223, vec3<f32>(x_225.x, x_225.y, x_225.z));
  ...
  let x_250 : f32 = u_xlat6;
  let x_252 : vec4<f32> = u_xlat0;
  vs_TEXCOORD1 = (vec3<f32>(x_250, x_250, x_250) * vec3<f32>(x_252.x, x_252.y, x_252.z));
...
fn main(@location(1) in_TEXCOORD0_param : vec4<f32>, @location(2) in_NORMAL0_param : vec3<f32>, @location(0) in_POSITION0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD2, gl_Position, vs_TEXCOORD6, vs_TEXCOORD0, vs_TEXCOORD3, vs_TEXCOORD1);
}

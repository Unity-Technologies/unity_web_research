...
fn main_1() {
  ...
  let x_437 : vec3<f32> = exp2(-(vec3<f32>(x_434.x, x_434.y, x_434.z)));
  let x_438 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_437.x, x_437.y, x_437.z, x_438.w);
  let x_441 : vec4<f32> = u_xlat1;
  let x_445 : vec3<f32> = ((vec3<f32>(x_441.x, x_441.y, x_441.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_446 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_445.x, x_445.y, x_445.z, x_446.w);

...
#line 52 "Packages/com.unity.postprocessing/PostProcessing/Shaders/Builtins/MultiScaleVORender.compute"
...
TestSamplePair ( frontDepth , invRange , centerIdx , x * 16 )
) ;
}
else if ( x == y )
{
...
return 0.5 * (
TestSamplePair ( frontDepth , invRange , centerIdx , x * 16 - x ) +
TestSamplePair ( frontDepth , invRange , centerIdx , x * 16 + x )
) ;
}
else
...
return 0.25 * (
TestSamplePair ( frontDepth , invRange , centerIdx , y * 16 + x ) +
TestSamplePair ( frontDepth , invRange , centerIdx , y * 16 - x ) +
TestSamplePair ( frontDepth , invRange , centerIdx , x * 16 + y ) +
TestSamplePair ( frontDepth , invRange , centerIdx , x * 16 - y )
) ;
}
}
#line 207
[ numthreads ( 8 , 8 , 1 ) ]
void MultiScaleVORender_MSAA_interleaved ( uint3 Gid : SV_GroupID , uint GI : SV_GroupIndex , uint3 GTid : SV_GroupThreadID , uint3 DTid : SV_DispatchThreadID )
{
#line 213
...
#line 225
int destIdx = GTid . x * 2 + GTid . y * 2 * 16 ;
DepthSamples [ destIdx ] = float2 ( depths0 . w , depths1 . w ) ;
DepthSamples [ destIdx + 1 ] = float2 ( depths0 . z , depths1 . z ) ;
DepthSamples [ destIdx + 16 ] = float2 ( depths0 . x , depths1 . x ) ;
DepthSamples [ destIdx + 16 + 1 ] = float2 ( depths0 . y , depths1 . y ) ;
#line 243
GroupMemoryBarrierWithGroupSync ( ) ;
#line 248
uint thisIdx = GTid . x + GTid . y * 16 + 4 * 16 + 4 ;
#line 252

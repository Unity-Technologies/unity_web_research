...
fn main_1() {
  ...
  let x_472 : vec4<f32> = x_32.x_AmbientColor;
  let x_474 : vec3<f32> = ((vec3<f32>(x_465.x, x_465.y, x_465.z) * vec3<f32>(x_467.x, x_467.y, x_467.z)) + vec3<f32>(x_472.x, x_472.y, x_472.z));
  let x_475 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_474.x, x_474.y, x_474.z, x_475.w);
  let x_478 : vec4<f32> = u_xlat0;
  let x_479 : vec3<f32> = vec3<f32>(x_478.x, x_478.y, x_478.z);
  let x_480 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_479.x, x_479.y, x_479.z, x_480.w);
  SV_Target0.w = 1.0f;
  let x_486 : f32 = x_32.x_Glossiness;
  SV_Target1.w = x_486;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

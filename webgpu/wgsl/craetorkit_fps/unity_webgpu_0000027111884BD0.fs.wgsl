...
fn main_1() {
  ...
  u_xlat0 = (x_493 + x_494);
  let x_497 : vec2<f32> = vs_TEXCOORD1;
  let x_500 : vec4<f32> = x_51.x_Grain_Params2;
  let x_504 : vec4<f32> = x_51.x_Grain_Params2;
  ...
  let x_516 : vec4<f32> = textureSample(x_GrainTex, sampler_GrainTex, vec2<f32>(x_514.x, x_514.y));
  ...
  let x_528 : vec3<f32> = clamp(vec3<f32>(x_524.x, x_524.y, x_524.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_529 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_528.x, x_528.y, x_528.z, x_529.w);
  let x_532 : vec4<f32> = u_xlat2;
  u_xlat22 = dot(vec3<f32>(x_532.x, x_532.y, x_532.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_539 : f32 = u_xlat22;
  u_xlat22 = sqrt(x_539);
  let x_543 : f32 = x_51.x_Grain_Params1.x;
  let x_544 : f32 = u_xlat22;
  u_xlat22 = ((x_543 * -(x_544)) + 1.0f);
  ...
  let x_558 : f32 = x_51.x_Grain_Params1.y;
  let x_560 : f32 = x_51.x_Grain_Params1.y;
  let x_562 : f32 = x_51.x_Grain_Params1.y;
  let x_564 : vec3<f32> = (vec3<f32>(x_555.x, x_555.y, x_555.z) * vec3<f32>(x_558, x_560, x_562));
  ...
  let x_574 : vec3<f32> = ((vec3<f32>(x_567.x, x_567.y, x_567.z) * vec3<f32>(x_569, x_569, x_569)) + vec3<f32>(x_572.x, x_572.y, x_572.z));
  let x_575 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_574.x, x_574.y, x_574.z, x_575.w);
  let x_577 : vec4<f32> = u_xlat0;
  let x_581 : f32 = x_51.x_PostExposure;
  let x_583 : f32 = x_51.x_PostExposure;
  let x_585 : f32 = x_51.x_PostExposure;
  let x_587 : f32 = x_51.x_PostExposure;
  let x_588 : vec4<f32> = vec4<f32>(x_581, x_583, x_585, x_587);
  u_xlat0 = (vec4<f32>(x_577.w, x_577.x, x_577.y, x_577.z) * vec4<f32>(x_588.x, x_588.y, x_588.z, x_588.w));
  let x_595 : vec4<f32> = u_xlat0;
  let x_602 : vec3<f32> = ((vec3<f32>(x_595.y, x_595.z, x_595.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_617 : vec3<f32> = ((vec3<f32>(x_610.x, x_610.y, x_610.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  ...
  let x_632 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_634 : vec3<f32> = (vec3<f32>(x_627.x, x_627.y, x_627.z) * vec3<f32>(x_632.y, x_632.y, x_632.y));
  let x_635 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_634.x, x_634.y, x_634.z, x_635.w);
  let x_638 : f32 = x_51.x_Lut3D_Params.x;
  u_xlat22 = (x_638 * 0.5f);
  let x_640 : vec4<f32> = u_xlat1;
  let x_643 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_646 : f32 = u_xlat22;
  let x_648 : vec3<f32> = ((vec3<f32>(x_640.x, x_640.y, x_640.z) * vec3<f32>(x_643.x, x_643.x, x_643.x)) + vec3<f32>(x_646, x_646, x_646));
  let x_649 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_648.x, x_648.y, x_648.z, x_649.w);
  let x_659 : vec4<f32> = u_xlat1;
  let x_661 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_659.x, x_659.y, x_659.z));
  let x_662 : vec3<f32> = vec3<f32>(x_661.x, x_661.y, x_661.z);
  let x_663 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_663.x, x_662.x, x_662.y, x_662.z);
  let x_669 : f32 = x_51.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_669);
  let x_671 : bool = u_xlatb1;
  if (x_671) {
    let x_674 : vec4<f32> = u_xlat0;
    let x_675 : vec3<f32> = vec3<f32>(x_674.y, x_674.z, x_674.w);
    let x_676 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_675.x, x_675.y, x_675.z, x_676.w);
    let x_678 : vec4<f32> = u_xlat1;
    let x_682 : vec3<f32> = clamp(vec3<f32>(x_678.x, x_678.y, x_678.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_683 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_682.x, x_682.y, x_682.z, x_683.w);
    let x_685 : vec4<f32> = u_xlat1;
    u_xlat0.x = dot(vec3<f32>(x_685.x, x_685.y, x_685.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_691 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_691.y, x_691.z, x_691.w, x_691.x);
  return;
}

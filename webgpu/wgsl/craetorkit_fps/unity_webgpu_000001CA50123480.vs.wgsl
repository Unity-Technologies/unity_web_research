...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_358.y, x_358.y, x_358.y, x_358.y) * x_362);
  let x_365 : vec4<f32> = x_18.unity_WorldToLight[0i];
  let x_366 : vec4<f32> = u_xlat0;
  let x_369 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_365 * vec4<f32>(x_366.x, x_366.x, x_366.x, x_366.x)) + x_369);
  let x_372 : vec4<f32> = x_18.unity_WorldToLight[2i];
  let x_373 : vec4<f32> = u_xlat0;
  let x_376 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_372 * vec4<f32>(x_373.z, x_373.z, x_373.z, x_373.z)) + x_376);
  let x_380 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_381 : vec4<f32> = u_xlat0;
  let x_384 : vec4<f32> = u_xlat1;
  vs_TEXCOORD8 = ((x_380 * vec4<f32>(x_381.w, x_381.w, x_381.w, x_381.w)) + x_384);
  return;
}

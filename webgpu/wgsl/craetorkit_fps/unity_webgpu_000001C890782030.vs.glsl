...
layout(set = 0, binding = 6, std430) readonly buffer unity_ParticleInstanceData {
...
float u_xlat22;
    ...
    u_xlatu7.xy = uvec2(uint_bitfieldExtract(floatBitsToUint(u_xlat2.w), int(8) & int(0x1F), int(8) & int(0x1F)), uint_bitfieldExtract(floatBitsToUint(u_xlat2.w), int(16) & int(0x1F), int(8) & int(0x1F)));
    u_xlatu7.z = floatBitsToUint(u_xlat2.w) >> (24u & uint(0x1F));
    u_xlat5.yzw = vec3(u_xlatu7.xyz);
    ...
    u_xlat14 = (-u_xlat7.x) * unity_ParticleUVShiftData.y + u_xlat1.x;
    u_xlat14 = floor(u_xlat14);
    u_xlat2.x = u_xlat14 * unity_ParticleUVShiftData.z;
    u_xlat14 = (-unity_ParticleUVShiftData.w) + 1.0;
    u_xlat2.y = (-u_xlat7.x) * unity_ParticleUVShiftData.w + u_xlat14;
    ...
    vs_TEXCOORD3.xy = u_xlat7.xy * _MainTex_ST.xy + _MainTex_ST.zw;
    u_xlat7.x = u_xlat15 * unity_MatrixV[1].z;
    u_xlat1.xzw = vec3(u_xlat15) * unity_MatrixVP[1].xyw;
    u_xlat1.xzw = unity_MatrixVP[0].xyw * u_xlat8.xxx + u_xlat1.xzw;
    u_xlat7.x = unity_MatrixV[0].z * u_xlat8.x + u_xlat7.x;
    u_xlat7.x = unity_MatrixV[2].z * u_xlat0.x + u_xlat7.x;
    u_xlat0.xzw = unity_MatrixVP[2].xyw * u_xlat0.xxx + u_xlat1.xzw;
    u_xlat0.xzw = u_xlat0.xzw + unity_MatrixVP[3].xyw;
    u_xlat7.x = u_xlat7.x + unity_MatrixV[3].z;

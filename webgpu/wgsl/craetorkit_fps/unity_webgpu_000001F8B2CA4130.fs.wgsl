...
fn main_1() {
  ...
  u_xlat0 = ((vec3<f32>(x_1203.x, x_1203.y, x_1203.z) * x_1205) + x_1207);
  let x_1214 : vec4<f32> = vs_TEXCOORD0;
  let x_1216 : vec4<f32> = textureSample(x_EmissionMap, sampler_EmissionMap, vec2<f32>(x_1214.x, x_1214.y));
  u_xlat1 = vec3<f32>(x_1216.x, x_1216.y, x_1216.z);
  let x_1220 : vec3<f32> = u_xlat1;
  let x_1223 : vec4<f32> = x_33.x_EmissionColor;
  let x_1226 : vec3<f32> = u_xlat0;
  let x_1227 : vec3<f32> = ((x_1220 * vec3<f32>(x_1223.x, x_1223.y, x_1223.z)) + x_1226);
  let x_1228 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_1227.x, x_1227.y, x_1227.z, x_1228.w);
  SV_Target0.w = 1.0f;
  return;
}

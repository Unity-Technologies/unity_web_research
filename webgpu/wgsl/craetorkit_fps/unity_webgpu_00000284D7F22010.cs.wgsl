...
fn main_1() {
  ...
  if (x_51) {
    ...
    u_xlat4 = (vec3<f32>(x_521.x, x_521.x, x_521.x) + vec3<f32>(1.0f, 0.666666687f, 0.333333343f));
    let x_526 : vec3<f32> = u_xlat4;
    u_xlat4 = fract(x_526);
    let x_528 : vec3<f32> = u_xlat4;
    u_xlat4 = ((x_528 * vec3<f32>(6.0f, 6.0f, 6.0f)) + vec3<f32>(-3.0f, -3.0f, -3.0f));
    let x_534 : vec3<f32> = u_xlat4;
    u_xlat4 = (abs(x_534) + vec3<f32>(-1.0f, -1.0f, -1.0f));
    let x_537 : vec3<f32> = u_xlat4;
    u_xlat4 = clamp(x_537, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_541 : vec3<f32> = u_xlat4;
    u_xlat4 = (x_541 + vec3<f32>(-1.0f, -1.0f, -1.0f));
    let x_543 : vec2<f32> = u_xlat10;
    let x_545 : vec3<f32> = u_xlat4;
    u_xlat4 = ((vec3<f32>(x_543.x, x_543.x, x_543.x) * x_545) + vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_549 : vec3<f32> = u_xlat4;
    let x_550 : vec4<f32> = u_xlat1;
    u_xlat5 = (x_549 * vec3<f32>(x_550.x, x_550.x, x_550.x));
    let x_554 : vec4<f32> = x_27.x_HueSatCon;
    let x_556 : vec4<f32> = u_xlat0;
    u_xlat0.x = dot(vec2<f32>(x_554.y, x_554.y), vec2<f32>(x_556.x, x_556.x));
    let x_560 : vec3<f32> = u_xlat5;
    u_xlat5.x = dot(x_560, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
    let x_563 : vec4<f32> = u_xlat1;
    let x_565 : vec3<f32> = u_xlat4;
    let x_567 : vec3<f32> = u_xlat5;
    u_xlat4 = ((vec3<f32>(x_563.x, x_563.x, x_563.x) * x_565) + -(vec3<f32>(x_567.x, x_567.x, x_567.x)));
    let x_571 : vec4<f32> = u_xlat0;
    let x_573 : vec3<f32> = u_xlat4;
    let x_575 : vec3<f32> = u_xlat5;
    let x_577 : vec3<f32> = ((vec3<f32>(x_571.x, x_571.x, x_571.x) * x_573) + vec3<f32>(x_575.x, x_575.x, x_575.x));
    let x_578 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_577.x, x_577.y, x_577.z, x_578.w);
    let x_580 : vec4<f32> = u_xlat0;
    let x_582 : vec3<f32> = max(vec3<f32>(x_580.x, x_580.y, x_580.z), vec3<f32>(0.0f, 0.0f, 0.0f));
    let x_583 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_582.x, x_582.y, x_582.z, x_583.w);
    let x_585 : vec4<f32> = u_xlat0;
    let x_589 : vec3<f32> = (vec3<f32>(x_585.x, x_585.y, x_585.z) * vec3<f32>(1.313386083f, 1.313386083f, 1.313386083f));
    let x_590 : vec4<f32> = u_xlat1;
    u_xlat1 = vec4<f32>(x_589.x, x_589.y, x_589.z, x_590.w);
    let x_592 : vec4<f32> = u_xlat0;
    let x_599 : vec3<f32> = ((vec3<f32>(x_592.x, x_592.y, x_592.z) * vec3<f32>(0.262677222f, 0.262677222f, 0.262677222f)) + vec3<f32>(0.069599994f, 0.069599994f, 0.069599994f));
    ...
    let x_609 : vec3<f32> = ((vec3<f32>(x_602.x, x_602.y, x_602.z) * vec3<f32>(x_604.x, x_604.y, x_604.z)) + vec3<f32>(0.00544f, 0.00544f, 0.00544f));
    let x_610 : vec4<f32> = u_xlat2;
    u_xlat2 = vec4<f32>(x_609.x, x_609.y, x_609.z, x_610.w);
    let x_612 : vec4<f32> = u_xlat0;
    let x_617 : vec3<f32> = ((vec3<f32>(x_612.x, x_612.y, x_612.z) * vec3<f32>(0.262677222f, 0.262677222f, 0.262677222f)) + vec3<f32>(0.289999992f, 0.289999992f, 0.289999992f));
    ...
    let x_627 : vec3<f32> = ((vec3<f32>(x_620.x, x_620.y, x_620.z) * vec3<f32>(x_622.x, x_622.y, x_622.z)) + vec3<f32>(0.08160001f, 0.08160001f, 0.08160001f));
    ...
    let x_634 : vec3<f32> = (vec3<f32>(x_630.x, x_630.y, x_630.z) / vec3<f32>(x_632.x, x_632.y, x_632.z));
    let x_635 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_634.x, x_634.y, x_634.z, x_635.w);
    let x_637 : vec4<f32> = u_xlat0;
    let x_641 : vec3<f32> = (vec3<f32>(x_637.x, x_637.y, x_637.z) + vec3<f32>(-0.066666663f, -0.066666663f, -0.066666663f));
    let x_642 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_641.x, x_641.y, x_641.z, x_642.w);
    let x_644 : vec4<f32> = u_xlat0;
    let x_646 : vec3<f32> = (vec3<f32>(x_644.x, x_644.y, x_644.z) * vec3<f32>(1.313386083f, 1.313386083f, 1.313386083f));
    let x_647 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_646.x, x_646.y, x_646.z, x_647.w);
    let x_649 : vec4<f32> = u_xlat0;
    let x_651 : vec3<f32> = max(vec3<f32>(x_649.x, x_649.y, x_649.z), vec3<f32>(0.0f, 0.0f, 0.0f));
    let x_652 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_651.x, x_651.y, x_651.z, x_652.w);
    u_xlat0.w = 1.0f;
    let x_659 : vec3<u32> = gl_GlobalInvocationID;
    let x_662 : vec4<f32> = u_xlat0;
    textureStore(x_Output_origX0X, bitcast<vec3<i32>>(x_659), x_662);
  }
  return;
}

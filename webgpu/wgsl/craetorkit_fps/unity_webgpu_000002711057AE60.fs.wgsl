...
fn main_1() {
  ...
  let x_244 : vec3<f32> = vs_TEXCOORD6;
  ...
  let x_289 : vec3<f32> = (vec3<f32>(x_286.x, x_286.y, x_286.z) * x_288);
  let x_290 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_289.x, x_289.y, x_289.z, x_290.w);
  let x_293 : vec3<f32> = u_xlat0;
  let x_294 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_293.x, x_293.y, x_293.z, x_294.w);
  SV_Target0.w = 1.0f;
  let x_300 : f32 = x_31.x_Glossiness;
  SV_Target1.w = x_300;
  let x_303 : vec3<f32> = vs_TEXCOORD0;
  let x_306 : vec3<f32> = ((x_303 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_307 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_306.x, x_306.y, x_306.z, x_307.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}

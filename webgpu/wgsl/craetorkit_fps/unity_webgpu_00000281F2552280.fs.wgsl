...
fn main_1() {
  ...
  u_xlatb3 = vec3<bool>(x_839.x, x_839.y, x_839.z);
  let x_842 : vec4<f32> = u_xlat0;
  hlslcc_movcTemp = x_842;
  let x_844 : bool = u_xlatb3.x;
  if (x_844) {
  ...
  if (x_1044) {
    ...
    let x_1052 : f32 = u_xlat3.z;
    x_1045 = x_1052;
  }
  let x_1053 : f32 = x_1045;
  u_xlat2.z = x_1053;
  let x_1057 : f32 = x_55.x_LumaInAlpha;
  u_xlatb0.x = (0.5f < x_1057);
  let x_1061 : bool = u_xlatb0.x;
  if (x_1061) {
    let x_1064 : vec4<f32> = u_xlat2;
    let x_1065 : vec3<f32> = vec3<f32>(x_1064.x, x_1064.y, x_1064.z);
    let x_1066 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1065.x, x_1065.y, x_1065.z, x_1066.w);
    let x_1068 : vec4<f32> = u_xlat0;
    let x_1072 : vec3<f32> = clamp(vec3<f32>(x_1068.x, x_1068.y, x_1068.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_1073 : vec4<f32> = u_xlat0;
    u_xlat0 = vec4<f32>(x_1072.x, x_1072.y, x_1072.z, x_1073.w);
    let x_1075 : vec4<f32> = u_xlat0;
    u_xlat2.w = dot(vec3<f32>(x_1075.x, x_1075.y, x_1075.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

...
fn main_1() {
  ...
  let x_468 : f32 = vs_TEXCOORD1.w;
  u_xlat21 = x_468;
  let x_469 : f32 = u_xlat21;
  u_xlat21 = clamp(x_469, 0.0f, 1.0f);
  let x_471 : vec3<f32> = u_xlat0;
  let x_473 : vec4<f32> = x_33.unity_FogColor;
  u_xlat0 = (x_471 + -(vec3<f32>(x_473.x, x_473.y, x_473.z)));
  let x_479 : f32 = u_xlat21;
  let x_481 : vec3<f32> = u_xlat0;
  let x_484 : vec4<f32> = x_33.unity_FogColor;
  let x_486 : vec3<f32> = ((vec3<f32>(x_479, x_479, x_479) * x_481) + vec3<f32>(x_484.x, x_484.y, x_484.z));
  let x_487 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_486.x, x_486.y, x_486.z, x_487.w);
  SV_Target0.w = 1.0f;
  return;
}

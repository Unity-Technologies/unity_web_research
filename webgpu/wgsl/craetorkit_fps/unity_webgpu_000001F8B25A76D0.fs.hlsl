...
float unityFogFactor = unity_FogParams . x * ( max ( ( ( 1.0 - ( ( _unity_fogCoord ) . x ) / _ProjectionParams . y ) * _ProjectionParams . z ) , 0 ) ) ; unityFogFactor = exp2 ( - unityFogFactor * unityFogFactor ) ; c . rgb . rgb = lerp ( ( unity_FogColor ) . rgb , ( c . rgb ) . rgb , saturate ( unityFogFactor ) ) ;

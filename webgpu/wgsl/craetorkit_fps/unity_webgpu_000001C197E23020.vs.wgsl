...
@group(0) @binding(8) var<storage, read> x_62 : unity_ParticleInstanceData;
...
fn main_1() {
  ...
  let x_358 : f32 = x_176.unity_ParticleUVShiftData.z;
  u_xlat8.x = (x_356 * x_358);
  let x_362 : f32 = x_176.unity_ParticleUVShiftData.w;
  u_xlat1.x = (-(x_362) + 1.0f);
  let x_367 : f32 = u_xlat7.x;
  let x_370 : f32 = x_176.unity_ParticleUVShiftData.w;
  let x_373 : f32 = u_xlat1.x;
  u_xlat8.y = ((-(x_367) * x_370) + x_373);
  let x_378 : vec2<f32> = in_TEXCOORD0;
  let x_380 : vec4<f32> = x_176.unity_ParticleUVShiftData;
  let x_383 : vec2<f32> = u_xlat8;
  let x_384 : vec2<f32> = ((x_378 * vec2<f32>(x_380.z, x_380.w)) + x_383);
  let x_385 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_384.x, x_384.y, x_385.z, x_385.w);
  let x_391 : f32 = x_176.unity_ParticleUVShiftData.x;
  ...
  vs_TEXCOORD4 = vec4<f32>(x_423.x, x_423.y, x_422.x, x_422.y);
  let x_426 : f32 = u_xlat0.y;
  let x_428 : f32 = x_176.x_ProjectionParams.x;
  u_xlat0.z = (x_426 * x_428);
  let x_431 : vec4<f32> = u_xlat0;
  let x_435 : vec3<f32> = (vec3<f32>(x_431.x, x_431.w, x_431.z) * vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_436 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_435.x, x_436.y, x_435.y, x_435.z);
  ...
  let x_452 : vec2<f32> = ((vec2<f32>(x_445.x, x_445.y) * vec2<f32>(0.5f, -0.5f)) + vec2<f32>(x_450.z, x_450.z));
  let x_453 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_452.x, x_452.y, x_453.z, x_453.w);
  let x_456 : f32 = u_xlat0.w;
  vs_TEXCOORD3.w = x_456;
  return;
}
...
fn main(@location(0) in_POSITION0_param : vec4<f32>, @builtin(instance_index) gl_InstanceIndex_param : u32, @location(1) in_COLOR0_param : vec4<f32>, @location(2) in_TEXCOORD0_param : vec2<f32>) -> main_out {
  ...
  main_1();
  return main_out(vs_TEXCOORD3, gl_Position, vs_COLOR0, vs_TEXCOORD1, vs_TEXCOORD0, vs_TEXCOORD4);
}

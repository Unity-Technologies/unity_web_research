...
fn main_1() {
  ...
  u_xlatb9 = (x_204 == 1i);
  let x_207 : f32 = vs_TEXCOORD0;
  let x_210 : f32 = x_30.x_ProjectionParams.y;
  u_xlat1.x = (x_207 / x_210);
  let x_214 : f32 = u_xlat1.x;
  u_xlat1.x = (-(x_214) + 1.0f);
  let x_219 : f32 = u_xlat1.x;
  let x_221 : f32 = x_30.x_ProjectionParams.z;
  u_xlat1.x = (x_219 * x_221);
  let x_225 : f32 = u_xlat1.x;
  u_xlat1.x = max(x_225, 0.0f);
  let x_229 : f32 = u_xlat1.x;
  let x_232 : f32 = x_30.unity_FogParams.x;
  u_xlat1.x = (x_229 * x_232);
  let x_236 : f32 = u_xlat1.x;
  let x_238 : f32 = u_xlat1.x;
  u_xlat1.x = (x_236 * -(x_238));
  let x_243 : f32 = u_xlat1.x;
  u_xlat1.x = exp2(x_243);
  let x_246 : vec4<f32> = u_xlat0;
  let x_248 : vec2<f32> = u_xlat1;
  u_xlat4 = (vec3<f32>(x_246.x, x_246.y, x_246.z) * vec3<f32>(x_248.x, x_248.x, x_248.x));
  let x_251 : vec4<f32> = u_xlat0;
  let x_255 : vec4<f32> = x_30.unity_FogColor;
  let x_258 : vec3<f32> = (vec3<f32>(x_251.x, x_251.y, x_251.z) + -(vec3<f32>(x_255.x, x_255.y, x_255.z)));
  let x_259 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_258.x, x_258.y, x_258.z, x_259.w);
  let x_261 : vec2<f32> = u_xlat1;
  let x_263 : vec4<f32> = u_xlat0;
  let x_267 : vec4<f32> = x_30.unity_FogColor;
  let x_269 : vec3<f32> = ((vec3<f32>(x_261.x, x_261.x, x_261.x) * vec3<f32>(x_263.x, x_263.y, x_263.z)) + vec3<f32>(x_267.x, x_267.y, x_267.z));
  let x_270 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_269.x, x_269.y, x_269.z, x_270.w);
  let x_272 : bool = u_xlatb9;
  if (x_272) {
    let x_276 : vec3<f32> = u_xlat4;
    x_273 = x_276;
  } else {
    let x_278 : vec4<f32> = u_xlat0;
    x_273 = vec3<f32>(x_278.x, x_278.y, x_278.z);
  }

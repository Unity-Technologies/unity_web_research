...
fn main_1() {
  ...
  let x_354 : vec2<f32> = (vec2<f32>(x_348.y, x_348.y) * vec2<f32>(x_352.x, x_352.y));
  let x_355 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_354.x, x_354.y, x_355.z, x_355.w);
  let x_358 : vec4<f32> = x_18.unity_WorldToLight[0i];
  let x_360 : vec4<f32> = u_xlat0;
  let x_363 : vec4<f32> = u_xlat1;
  let x_365 : vec2<f32> = ((vec2<f32>(x_358.x, x_358.y) * vec2<f32>(x_360.x, x_360.x)) + vec2<f32>(x_363.x, x_363.y));
  let x_366 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_365.x, x_365.y, x_366.z, x_366.w);
  let x_369 : vec4<f32> = x_18.unity_WorldToLight[2i];
  let x_371 : vec4<f32> = u_xlat0;
  let x_374 : vec4<f32> = u_xlat0;
  let x_376 : vec2<f32> = ((vec2<f32>(x_369.x, x_369.y) * vec2<f32>(x_371.z, x_371.z)) + vec2<f32>(x_374.x, x_374.y));
  let x_377 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_376.x, x_376.y, x_377.z, x_377.w);
  let x_382 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_384 : vec4<f32> = u_xlat0;
  let x_387 : vec4<f32> = u_xlat0;
  vs_TEXCOORD6 = ((vec2<f32>(x_382.x, x_382.y) * vec2<f32>(x_384.w, x_384.w)) + vec2<f32>(x_387.x, x_387.y));

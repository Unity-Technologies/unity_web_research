...
fn main_1() {
  ...
  let x_232 : vec3<f32> = (vec3<f32>(x_228.x, x_228.y, x_228.z) * x_231);
  let x_233 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_232.x, x_232.y, x_232.z, x_233.w);
  SV_Target0.w = 1.0f;
  let x_238 : vec4<f32> = u_xlat0;
  let x_242 : vec3<f32> = (vec3<f32>(x_238.x, x_238.y, x_238.z) + vec3<f32>(-0.039999999f, -0.039999999f, -0.039999999f));
  let x_243 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_242.x, x_242.y, x_242.z, x_243.w);
  let x_247 : f32 = u_xlat0.w;
  let x_250 : f32 = x_18.x_Glossiness;
  SV_Target1.w = (x_247 * x_250);
  let x_254 : f32 = x_18.x_Metallic;
  let x_256 : f32 = x_18.x_Metallic;
  let x_258 : f32 = x_18.x_Metallic;
  ...
  let x_270 : vec4<f32> = SV_Target1;
  SV_Target1 = vec4<f32>(x_269.x, x_269.y, x_269.z, x_270.w);
  let x_274 : vec3<f32> = vs_TEXCOORD0;
  let x_277 : vec3<f32> = ((x_274 * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_278 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_277.x, x_277.y, x_277.z, x_278.w);
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;
}

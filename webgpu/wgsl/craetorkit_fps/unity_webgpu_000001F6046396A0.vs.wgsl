...
fn main_1() {
  ...
  let x_309 : vec2<f32> = (vec2<f32>(x_303.y, x_303.y) * vec2<f32>(x_307.x, x_307.y));
  let x_310 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_309.x, x_309.y, x_310.z, x_310.w);
  let x_313 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_315 : vec4<f32> = u_xlat0;
  let x_318 : vec4<f32> = u_xlat1;
  let x_320 : vec2<f32> = ((vec2<f32>(x_313.x, x_313.y) * vec2<f32>(x_315.x, x_315.x)) + vec2<f32>(x_318.x, x_318.y));
  let x_321 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_320.x, x_320.y, x_321.z, x_321.w);
  let x_324 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_326 : vec4<f32> = u_xlat0;
  let x_329 : vec4<f32> = u_xlat0;
  let x_331 : vec2<f32> = ((vec2<f32>(x_324.x, x_324.y) * vec2<f32>(x_326.z, x_326.z)) + vec2<f32>(x_329.x, x_329.y));
  let x_332 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_331.x, x_331.y, x_332.z, x_332.w);
  let x_337 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_339 : vec4<f32> = u_xlat0;
  let x_342 : vec4<f32> = u_xlat0;
  vs_TEXCOORD2 = ((vec2<f32>(x_337.x, x_337.y) * vec2<f32>(x_339.w, x_339.w)) + vec2<f32>(x_342.x, x_342.y));

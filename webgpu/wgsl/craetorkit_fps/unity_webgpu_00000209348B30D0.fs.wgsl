...
fn main_1() {
  ...
  let x_434 : vec3<f32> = exp2(-(vec3<f32>(x_431.x, x_431.y, x_431.z)));
  let x_435 : vec4<f32> = SV_Target3;
  SV_Target3 = vec4<f32>(x_434.x, x_434.y, x_434.z, x_435.w);
  let x_438 : vec4<f32> = u_xlat1;
  let x_442 : vec3<f32> = ((vec3<f32>(x_438.x, x_438.y, x_438.z) * vec3<f32>(0.5f, 0.5f, 0.5f)) + vec3<f32>(0.5f, 0.5f, 0.5f));
  let x_443 : vec4<f32> = SV_Target2;
  SV_Target2 = vec4<f32>(x_442.x, x_442.y, x_442.z, x_443.w);
  let x_446 : vec4<f32> = u_xlat0;
  SV_Target0 = x_446;
  let x_449 : f32 = x_36.x_Glossiness;
  SV_Target1.w = x_449;
  SV_Target2.w = 1.0f;
  SV_Target3.w = 1.0f;
  return;

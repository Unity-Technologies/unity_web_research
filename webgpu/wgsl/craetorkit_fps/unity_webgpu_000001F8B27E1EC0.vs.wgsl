...
@group(1) @binding(2) var<uniform> x_449 : UnityInstancing_PerDraw2;
...
fn main_1() {
  ...
  let x_426 : f32 = u_xlat4.y;
  let x_428 : f32 = u_xlat4.y;
  u_xlat5.x = (x_426 * x_428);
  let x_432 : f32 = u_xlat4.x;
  let x_434 : f32 = u_xlat4.x;
  let x_437 : f32 = u_xlat5.x;
  u_xlat5.x = ((x_432 * x_434) + -(x_437));
  let x_441 : vec4<f32> = u_xlat4;
  let x_443 : vec4<f32> = u_xlat4;
  u_xlat1 = (vec4<f32>(x_441.y, x_441.w, x_441.z, x_441.x) * x_443);
  let x_450 : i32 = u_xlati0;
  let x_453 : vec4<f32> = x_449.unity_Builtins2Array[(x_450 / 7i)].unity_SHBrArray;
  let x_454 : vec4<f32> = u_xlat1;
  u_xlat2.x = dot(x_453, x_454);
  let x_457 : i32 = u_xlati0;
  let x_460 : vec4<f32> = x_449.unity_Builtins2Array[(x_457 / 7i)].unity_SHBgArray;
  let x_461 : vec4<f32> = u_xlat1;
  u_xlat2.y = dot(x_460, x_461);
  let x_464 : i32 = u_xlati0;
  let x_467 : vec4<f32> = x_449.unity_Builtins2Array[(x_464 / 7i)].unity_SHBbArray;
  let x_468 : vec4<f32> = u_xlat1;
  u_xlat2.z = dot(x_467, x_468);
  let x_473 : i32 = u_xlati0;
  let x_477 : vec4<f32> = x_449.unity_Builtins2Array[(x_473 / 7i)].unity_SHCArray;
  let x_479 : vec3<f32> = u_xlat5;
  let x_482 : vec4<f32> = u_xlat2;
  vs_TEXCOORD7 = ((vec3<f32>(x_477.x, x_477.y, x_477.z) * vec3<f32>(x_479.x, x_479.x, x_479.x)) + vec3<f32>(x_482.x, x_482.y, x_482.z));
  let x_487 : i32 = gl_InstanceIndex;
  vs_SV_InstanceID0 = bitcast<u32>(x_487);
  return;
}
...
fn main(@builtin(instance_index) gl_InstanceIndex_param : u32, @location(0) in_POSITION0_param : vec4<f32>, @location(3) in_TEXCOORD0_param : vec4<f32>, @location(1) in_TANGENT0_param : vec4<f32>, @location(2) in_NORMAL0_param : vec3<f32>, @location(4) in_COLOR0_param : vec4<f32>) -> main_out {
  ...
  main_1();
  return main_out(gl_Position, vs_TEXCOORD0, vs_TEXCOORD1, vs_TEXCOORD2, vs_TEXCOORD3, vs_COLOR0, vs_TEXCOORD7, vs_SV_InstanceID0);
}

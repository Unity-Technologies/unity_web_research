...
fn main_1() {
  ...
  let x_500 : f32 = x_51.x_PostExposure;
  let x_502 : f32 = x_51.x_PostExposure;
  let x_504 : f32 = x_51.x_PostExposure;
  let x_506 : f32 = x_51.x_PostExposure;
  let x_507 : vec4<f32> = vec4<f32>(x_500, x_502, x_504, x_506);
  u_xlat0 = (vec4<f32>(x_496.w, x_496.x, x_496.y, x_496.z) * vec4<f32>(x_507.x, x_507.y, x_507.z, x_507.w));
  let x_514 : vec4<f32> = u_xlat0;
  let x_521 : vec3<f32> = ((vec3<f32>(x_514.y, x_514.z, x_514.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_536 : vec3<f32> = ((vec3<f32>(x_529.x, x_529.y, x_529.z) * vec3<f32>(0.073499784f, 0.073499784f, 0.073499784f)) + vec3<f32>(0.386036009f, 0.386036009f, 0.386036009f));
  let x_537 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_536.x, x_536.y, x_536.z, x_537.w);
  let x_539 : vec4<f32> = u_xlat1;
  let x_543 : vec3<f32> = clamp(vec3<f32>(x_539.x, x_539.y, x_539.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_544 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_543.x, x_543.y, x_543.z, x_544.w);
  let x_546 : vec4<f32> = u_xlat1;
  let x_551 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_553 : vec3<f32> = (vec3<f32>(x_546.x, x_546.y, x_546.z) * vec3<f32>(x_551.y, x_551.y, x_551.y));
  let x_554 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_553.x, x_553.y, x_553.z, x_554.w);
  let x_558 : f32 = x_51.x_Lut3D_Params.x;
  u_xlat22 = (x_558 * 0.5f);
  let x_560 : vec4<f32> = u_xlat1;
  let x_563 : vec2<f32> = x_51.x_Lut3D_Params;
  let x_566 : f32 = u_xlat22;
  let x_568 : vec3<f32> = ((vec3<f32>(x_560.x, x_560.y, x_560.z) * vec3<f32>(x_563.x, x_563.x, x_563.x)) + vec3<f32>(x_566, x_566, x_566));
  let x_569 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_568.x, x_568.y, x_568.z, x_569.w);
  let x_579 : vec4<f32> = u_xlat1;
  let x_581 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_579.x, x_579.y, x_579.z));
  let x_582 : vec3<f32> = vec3<f32>(x_581.x, x_581.y, x_581.z);
  let x_583 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_583.x, x_582.x, x_582.y, x_582.z);
  let x_589 : f32 = x_51.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_589);
  ...
  if (x_591) {
    ...
    u_xlat0.x = dot(vec3<f32>(x_605.x, x_605.y, x_605.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_615 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_615.y, x_615.z, x_615.w, x_615.x);
  return;
}

...
fn main_1() {
  ...
  u_xlat0 = (vec4<f32>(x_870.w, x_870.x, x_870.y, x_870.z) * vec4<f32>(x_881.x, x_881.y, x_881.z, x_881.w));
  let x_888 : vec4<f32> = u_xlat0;
  let x_895 : vec3<f32> = ((vec3<f32>(x_888.y, x_888.z, x_888.w) * vec3<f32>(5.55555582f, 5.55555582f, 5.55555582f)) + vec3<f32>(0.047995999f, 0.047995999f, 0.047995999f));
  ...
  let x_936 : vec2<f32> = x_26.x_Lut3D_Params;
  let x_939 : f32 = u_xlat31;
  let x_941 : vec3<f32> = ((vec3<f32>(x_933.x, x_933.y, x_933.z) * vec3<f32>(x_936.x, x_936.x, x_936.x)) + vec3<f32>(x_939, x_939, x_939));
  let x_942 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_941.x, x_941.y, x_941.z, x_942.w);
  let x_952 : vec4<f32> = u_xlat1;
  let x_954 : vec4<f32> = textureSample(x_Lut3D, sampler_Lut3D, vec3<f32>(x_952.x, x_952.y, x_952.z));
  let x_955 : vec3<f32> = vec3<f32>(x_954.x, x_954.y, x_954.z);
  let x_956 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_956.x, x_955.x, x_955.y, x_955.z);
  let x_960 : f32 = x_26.x_LumaInAlpha;
  u_xlatb1 = (0.5f < x_960);
  let x_962 : bool = u_xlatb1;
  if (x_962) {
    let x_965 : vec4<f32> = u_xlat0;
    let x_966 : vec3<f32> = vec3<f32>(x_965.y, x_965.z, x_965.w);
    ...
    u_xlat0.x = dot(vec3<f32>(x_976.x, x_976.y, x_976.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }
  let x_986 : vec4<f32> = u_xlat0;
  SV_Target0 = vec4<f32>(x_986.y, x_986.z, x_986.w, x_986.x);
  return;
}

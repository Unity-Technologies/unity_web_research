...
fn main_1() {
  ...
  let x_297 : vec3<f32> = (vec3<f32>(x_291.y, x_291.y, x_291.y) * vec3<f32>(x_295.x, x_295.y, x_295.z));
  let x_298 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_297.x, x_297.y, x_297.z, x_298.w);
  let x_301 : vec4<f32> = x_18.unity_WorldToLight[0i];
  ...
  let x_312 : vec4<f32> = x_18.unity_WorldToLight[2i];
  ...
  let x_325 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_327 : vec4<f32> = u_xlat0;
  let x_330 : vec4<f32> = u_xlat0;
  vs_TEXCOORD6 = ((vec3<f32>(x_325.x, x_325.y, x_325.z) * vec3<f32>(x_327.w, x_327.w, x_327.w)) + vec3<f32>(x_330.x, x_330.y, x_330.z));

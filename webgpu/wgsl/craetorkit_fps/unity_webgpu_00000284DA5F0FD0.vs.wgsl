...
fn main_1() {
  ...
  let x_367 : vec3<f32> = (vec3<f32>(x_361.y, x_361.y, x_361.y) * vec3<f32>(x_365.x, x_365.y, x_365.z));
  ...
  let x_378 : vec3<f32> = ((vec3<f32>(x_371.x, x_371.y, x_371.z) * vec3<f32>(x_373.x, x_373.x, x_373.x)) + vec3<f32>(x_376.x, x_376.y, x_376.z));
  ...
  let x_394 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_396 : vec4<f32> = u_xlat0;
  let x_399 : vec4<f32> = u_xlat0;
  vs_TEXCOORD8 = ((vec3<f32>(x_394.x, x_394.y, x_394.z) * vec3<f32>(x_396.w, x_396.w, x_396.w)) + vec3<f32>(x_399.x, x_399.y, x_399.z));
  return;
}

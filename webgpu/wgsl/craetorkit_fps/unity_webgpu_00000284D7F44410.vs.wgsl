...
fn main_1() {
  ...
  u_xlat1 = (vec4<f32>(x_362.y, x_362.y, x_362.y, x_362.y) * x_366);
  let x_369 : vec4<f32> = x_19.unity_WorldToLight[0i];
  let x_370 : vec4<f32> = u_xlat0;
  let x_373 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_369 * vec4<f32>(x_370.x, x_370.x, x_370.x, x_370.x)) + x_373);
  let x_376 : vec4<f32> = x_19.unity_WorldToLight[2i];
  let x_377 : vec4<f32> = u_xlat0;
  let x_380 : vec4<f32> = u_xlat1;
  u_xlat1 = ((x_376 * vec4<f32>(x_377.z, x_377.z, x_377.z, x_377.z)) + x_380);
  let x_385 : vec4<f32> = x_19.unity_WorldToLight[3i];
  let x_386 : vec4<f32> = u_xlat0;
  let x_389 : vec4<f32> = u_xlat1;
  vs_TEXCOORD5 = ((x_385 * vec4<f32>(x_386.w, x_386.w, x_386.w, x_386.w)) + x_389);
  return;
}

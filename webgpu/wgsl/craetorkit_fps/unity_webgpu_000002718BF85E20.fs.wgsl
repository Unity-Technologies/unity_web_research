...
fn main_1() {
  ...
  let x_233 : vec3<f32> = clamp(vec3<f32>(x_229.x, x_229.y, x_229.z), vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_234 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_233.x, x_233.y, x_233.z, x_234.w);
  let x_236 : vec4<f32> = u_xlat3;
  u_xlat21 = dot(vec3<f32>(x_236.x, x_236.y, x_236.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_243 : f32 = u_xlat21;
  u_xlat21 = sqrt(x_243);
  let x_246 : f32 = x_51.x_Grain_Params1.x;
  let x_247 : f32 = u_xlat21;
  u_xlat21 = ((x_246 * -(x_247)) + 1.0f);
  let x_251 : vec3<f32> = u_xlat0;
  let x_252 : vec3<f32> = u_xlat2;
  u_xlat2 = (x_251 * x_252);
  let x_254 : vec3<f32> = u_xlat2;
  let x_256 : f32 = x_51.x_Grain_Params1.y;
  let x_258 : f32 = x_51.x_Grain_Params1.y;
  let x_260 : f32 = x_51.x_Grain_Params1.y;
  u_xlat2 = (x_254 * vec3<f32>(x_256, x_258, x_260));
  let x_263 : vec3<f32> = u_xlat2;
  let x_264 : f32 = u_xlat21;
  let x_267 : vec3<f32> = u_xlat0;
  let x_268 : vec3<f32> = ((x_263 * vec3<f32>(x_264, x_264, x_264)) + x_267);
  let x_269 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_268.x, x_268.y, x_268.z, x_269.w);
  let x_275 : f32 = x_51.x_LumaInAlpha;
  u_xlatb0 = (0.5f < x_275);
  let x_277 : bool = u_xlatb0;
  if (x_277) {
    let x_280 : vec4<f32> = u_xlat1;
    u_xlat0 = vec3<f32>(x_280.x, x_280.y, x_280.z);
    let x_282 : vec3<f32> = u_xlat0;
    u_xlat0 = clamp(x_282, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
    let x_286 : vec3<f32> = u_xlat0;
    u_xlat1.w = dot(x_286, vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  }

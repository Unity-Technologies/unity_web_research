...
layout(set = 0, binding = 1) uniform mediump texture2D _BlendTex;
...
vec4 u_xlat2;
float u_xlat6;
    u_xlat0.x = texture(sampler2D(_BlendTex, sampler_MainTex), vs_TEXCOORD1.xy).w;
    u_xlat0.y = texture(sampler2D(_BlendTex, sampler_MainTex), vs_TEXCOORD1.zw).y;
    u_xlat0.zw = texture(sampler2D(_BlendTex, sampler_MainTex), vs_TEXCOORD0.xy).zx;
    u_xlat1.x = dot(u_xlat0, vec4(1.0, 1.0, 1.0, 1.0));
    u_xlatb1 = u_xlat1.x<9.99999975e-06;
    if(u_xlatb1){
        SV_Target0 = textureLod(sampler2D(_MainTex, sampler_MainTex), vs_TEXCOORD0.xy, 0.0);
    } else {

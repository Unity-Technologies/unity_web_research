...
fn main_1() {
  ...
  u_xlat23 = dot(x_477, x_478);
  let x_481 : f32 = u_xlat23;
  let x_482 : f32 = u_xlat23;
  u_xlat24 = (x_481 + x_482);
  let x_484 : vec3<f32> = u_xlat5;
  let x_485 : f32 = u_xlat24;
  let x_489 : vec3<f32> = u_xlat0;
  u_xlat0 = ((x_484 * -(vec3<f32>(x_485, x_485, x_485))) + x_489);
  let x_491 : vec3<f32> = u_xlat5;
  let x_493 : vec4<f32> = x_20.x_WorldSpaceLightPos0;
  u_xlat24 = dot(x_491, vec3<f32>(x_493.x, x_493.y, x_493.z));
  let x_496 : f32 = u_xlat24;
  u_xlat24 = clamp(x_496, 0.0f, 1.0f);
  let x_498 : f32 = u_xlat23;
  u_xlat23 = x_498;
  let x_499 : f32 = u_xlat23;
  u_xlat23 = clamp(x_499, 0.0f, 1.0f);
  let x_501 : vec3<f32> = u_xlat0;
  let x_503 : vec4<f32> = x_20.x_WorldSpaceLightPos0;
  u_xlat0.x = dot(x_501, vec3<f32>(x_503.x, x_503.y, x_503.z));
  let x_507 : f32 = u_xlat23;
  u_xlat0.y = (-(x_507) + 1.0f);
  ...
  u_xlat0 = vec3<f32>(x_515.x, x_515.y, x_516.z);
  let x_518 : vec3<f32> = u_xlat0;
  let x_520 : vec3<f32> = u_xlat0;
  let x_522 : vec2<f32> = (vec2<f32>(x_518.x, x_518.y) * vec2<f32>(x_520.x, x_520.y));
  let x_523 : vec4<f32> = u_xlat2;
  u_xlat2 = vec4<f32>(x_522.x, x_522.y, x_523.z, x_523.w);
  let x_526 : f32 = x_20.x_Glossiness;
  let x_528 : f32 = u_xlat1.w;
  let x_530 : f32 = u_xlat21;
  u_xlat0.x = ((x_526 * x_528) + -(x_530));
  let x_535 : f32 = u_xlat0.x;
  u_xlat0.x = (x_535 + 1.0f);
  let x_539 : f32 = u_xlat0.x;
  u_xlat0.x = clamp(x_539, 0.0f, 1.0f);
  let x_548 : vec4<f32> = u_xlat2;
  let x_550 : vec4<f32> = textureSample(unity_NHxRoughness, samplerunity_NHxRoughness, vec2<f32>(x_548.x, x_548.z));
  u_xlat7.x = x_550.x;
  let x_554 : f32 = u_xlat7.x;
  u_xlat7.x = (x_554 * 16.0f);
  let x_557 : vec3<f32> = u_xlat7;
  let x_559 : vec3<f32> = u_xlat6;
  let x_561 : vec4<f32> = u_xlat1;
  u_xlat7 = ((vec3<f32>(x_557.x, x_557.x, x_557.x) * x_559) + vec3<f32>(x_561.x, x_561.y, x_561.z));
  ...
  u_xlat2 = vec4<f32>(x_567.x, x_568.y, x_567.y, x_567.z);
  let x_570 : vec3<f32> = u_xlat6;
  let x_572 : vec3<f32> = u_xlat0;
  u_xlat4 = (-(x_570) + vec3<f32>(x_572.x, x_572.x, x_572.x));
  let x_575 : vec4<f32> = u_xlat2;
  let x_577 : vec3<f32> = u_xlat4;
  let x_579 : vec3<f32> = u_xlat6;
  u_xlat4 = ((vec3<f32>(x_575.y, x_575.y, x_575.y) * x_577) + x_579);
  let x_581 : vec4<f32> = u_xlat3;
  let x_583 : vec3<f32> = u_xlat4;
  let x_584 : vec3<f32> = (vec3<f32>(x_581.x, x_581.y, x_581.z) * x_583);
  let x_585 : vec4<f32> = u_xlat3;
  u_xlat3 = vec4<f32>(x_584.x, x_584.y, x_584.z, x_585.w);
  let x_588 : vec3<f32> = vs_TEXCOORD2;
  let x_589 : vec4<f32> = u_xlat1;
  let x_592 : vec4<f32> = u_xlat3;
  let x_594 : vec3<f32> = ((x_588 * vec3<f32>(x_589.x, x_589.y, x_589.z)) + vec3<f32>(x_592.x, x_592.y, x_592.z));
  ...
  u_xlat0 = ((x_597 * vec3<f32>(x_598.x, x_598.z, x_598.w)) + vec3<f32>(x_601.x, x_601.y, x_601.z));
  let x_606 : f32 = vs_TEXCOORD3;
  u_xlat21 = x_606;
  let x_607 : f32 = u_xlat21;
  u_xlat21 = clamp(x_607, 0.0f, 1.0f);
  let x_609 : vec3<f32> = u_xlat0;
  let x_611 : vec4<f32> = x_20.unity_FogColor;
  u_xlat0 = (x_609 + -(vec3<f32>(x_611.x, x_611.y, x_611.z)));
  let x_617 : f32 = u_xlat21;
  let x_619 : vec3<f32> = u_xlat0;
  let x_622 : vec4<f32> = x_20.unity_FogColor;
  let x_624 : vec3<f32> = ((vec3<f32>(x_617, x_617, x_617) * x_619) + vec3<f32>(x_622.x, x_622.y, x_622.z));
...
@fragment
fn main(@location(2) vs_TEXCOORD1_param : vec3<f32>, @location(3) vs_COLOR0_param : vec4<f32>, @location(0) vs_TEXCOORD0_param : vec3<f32>, @location(4) vs_TEXCOORD2_param : vec3<f32>, @location(1) vs_TEXCOORD3_param : f32, @builtin(position) gl_FragCoord_param : vec4<f32>) -> main_out {

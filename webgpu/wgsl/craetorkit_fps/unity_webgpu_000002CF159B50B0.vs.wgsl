...
fn main_1() {
  ...
  let x_425 : vec3<f32> = (vec3<f32>(x_419.y, x_419.y, x_419.y) * vec3<f32>(x_423.x, x_423.y, x_423.z));
  let x_426 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_425.x, x_425.y, x_425.z, x_426.w);
  let x_429 : vec4<f32> = x_18.unity_WorldToLight[0i];
  let x_431 : vec4<f32> = u_xlat0;
  let x_434 : vec4<f32> = u_xlat1;
  let x_436 : vec3<f32> = ((vec3<f32>(x_429.x, x_429.y, x_429.z) * vec3<f32>(x_431.x, x_431.x, x_431.x)) + vec3<f32>(x_434.x, x_434.y, x_434.z));
  let x_437 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_436.x, x_436.y, x_436.z, x_437.w);
  let x_440 : vec4<f32> = x_18.unity_WorldToLight[2i];
  ...
  let x_453 : vec4<f32> = x_18.unity_WorldToLight[3i];
  let x_455 : vec4<f32> = u_xlat0;
  let x_458 : vec4<f32> = u_xlat0;
  vs_TEXCOORD6 = ((vec3<f32>(x_453.x, x_453.y, x_453.z) * vec3<f32>(x_455.w, x_455.w, x_455.w)) + vec3<f32>(x_458.x, x_458.y, x_458.z));

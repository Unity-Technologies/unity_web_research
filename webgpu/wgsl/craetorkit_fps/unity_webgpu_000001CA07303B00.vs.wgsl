...
fn main_1() {
  ...
  u_xlat2 = (vec4<f32>(x_606.y, x_606.w, x_606.z, x_606.x) * x_608);
  let x_611 : vec4<f32> = x_184.unity_SHBr;
  let x_612 : vec4<f32> = u_xlat2;
  u_xlat3.x = dot(x_611, x_612);
  let x_616 : vec4<f32> = x_184.unity_SHBg;
  let x_617 : vec4<f32> = u_xlat2;
  u_xlat3.y = dot(x_616, x_617);
  let x_621 : vec4<f32> = x_184.unity_SHBb;
  let x_622 : vec4<f32> = u_xlat2;
  u_xlat3.z = dot(x_621, x_622);
  let x_628 : vec4<f32> = x_184.unity_SHC;
  let x_630 : vec4<f32> = u_xlat1;
  let x_633 : vec4<f32> = u_xlat3;
  vs_TEXCOORD4 = ((vec3<f32>(x_628.x, x_628.y, x_628.z) * vec3<f32>(x_630.x, x_630.x, x_630.x)) + vec3<f32>(x_633.x, x_633.y, x_633.z));
  let x_637 : f32 = u_xlat0.y;
  let x_639 : f32 = x_184.x_ProjectionParams.x;
  u_xlat0.y = (x_637 * x_639);
  let x_642 : vec4<f32> = u_xlat0;
  let x_646 : vec3<f32> = (vec3<f32>(x_642.x, x_642.w, x_642.y) * vec3<f32>(0.5f, 0.5f, 0.5f));
  ...
  let x_652 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_652.x, x_652.y, x_651.x, x_651.y);
  let x_654 : vec4<f32> = u_xlat1;
  let x_656 : vec4<f32> = u_xlat1;
  let x_658 : vec2<f32> = (vec2<f32>(x_654.z, x_654.z) + vec2<f32>(x_656.x, x_656.w));
  let x_659 : vec4<f32> = vs_TEXCOORD6;
  vs_TEXCOORD6 = vec4<f32>(x_658.x, x_658.y, x_659.z, x_659.w);
  let x_663 : i32 = gl_InstanceIndex;
  vs_SV_InstanceID0 = bitcast<u32>(x_663);
  return;
}

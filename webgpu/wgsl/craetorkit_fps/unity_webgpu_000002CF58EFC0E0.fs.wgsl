...
fn main_1() {
  ...
  u_xlatb5 = (x_634 < 0.0f);
  let x_636 : bool = u_xlatb5;
  if (x_636) {
    let x_641 : f32 = u_xlat5.y;
    x_637 = x_641;
  } else {
  ...
  u_xlat5 = (vec3<f32>(x_646.x, x_646.x, x_646.x) + vec3<f32>(1.0f, 0.666666687f, 0.333333343f));
  let x_651 : vec3<f32> = u_xlat5;
  u_xlat5 = fract(x_651);
  let x_653 : vec3<f32> = u_xlat5;
  u_xlat5 = ((x_653 * vec3<f32>(6.0f, 6.0f, 6.0f)) + vec3<f32>(-3.0f, -3.0f, -3.0f));
  let x_659 : vec3<f32> = u_xlat5;
  u_xlat5 = (abs(x_659) + vec3<f32>(-1.0f, -1.0f, -1.0f));
  let x_662 : vec3<f32> = u_xlat5;
  u_xlat5 = clamp(x_662, vec3<f32>(0.0f, 0.0f, 0.0f), vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_666 : vec3<f32> = u_xlat5;
  u_xlat5 = (x_666 + vec3<f32>(-1.0f, -1.0f, -1.0f));
  let x_668 : vec4<f32> = u_xlat1;
  let x_670 : vec3<f32> = u_xlat5;
  u_xlat5 = ((vec3<f32>(x_668.x, x_668.x, x_668.x) * x_670) + vec3<f32>(1.0f, 1.0f, 1.0f));
  let x_674 : vec3<f32> = u_xlat5;
  let x_675 : vec4<f32> = u_xlat2;
  let x_677 : vec3<f32> = (x_674 * vec3<f32>(x_675.x, x_675.x, x_675.x));
  let x_678 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_677.x, x_677.y, x_677.z, x_678.w);
  let x_680 : vec4<f32> = u_xlat1;
  u_xlat1.x = dot(vec3<f32>(x_680.x, x_680.y, x_680.z), vec3<f32>(0.212672904f, 0.715152204f, 0.072175004f));
  let x_684 : vec4<f32> = u_xlat2;
  let x_686 : vec3<f32> = u_xlat5;
  let x_688 : vec4<f32> = u_xlat1;
  u_xlat5 = ((vec3<f32>(x_684.x, x_684.x, x_684.x) * x_686) + -(vec3<f32>(x_688.x, x_688.x, x_688.x)));
  let x_692 : vec4<f32> = u_xlat0;
  let x_694 : vec3<f32> = u_xlat5;
  let x_696 : vec4<f32> = u_xlat1;
  let x_698 : vec3<f32> = ((vec3<f32>(x_692.x, x_692.x, x_692.x) * x_694) + vec3<f32>(x_696.x, x_696.x, x_696.x));
  ...
  let x_712 : vec3<f32> = (vec3<f32>(x_708.x, x_708.y, x_708.z) + vec3<f32>(0.00390625f, 0.00390625f, 0.00390625f));
  let x_713 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_712.x, x_712.y, x_712.z, x_713.w);
  u_xlat0.w = 0.75f;
  let x_720 : vec4<f32> = u_xlat0;
  let x_722 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_720.x, x_720.w));
  u_xlat1.x = x_722.w;
  ...
  let x_737 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_735.y, x_735.w));
  u_xlat6.x = x_737.w;
  let x_743 : vec4<f32> = u_xlat0;
  let x_745 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_743.z, x_743.w));
  u_xlat6.y = x_745.w;
  let x_748 : vec2<f32> = u_xlat6;
  let x_749 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_749.x, x_748.x, x_748.y, x_749.w);
  let x_751 : vec4<f32> = u_xlat1;
  let x_755 : vec2<f32> = clamp(vec2<f32>(x_751.y, x_751.z), vec2<f32>(0.0f, 0.0f), vec2<f32>(1.0f, 1.0f));
  let x_756 : vec4<f32> = u_xlat1;
  u_xlat1 = vec4<f32>(x_756.x, x_755.x, x_755.y, x_756.w);
  let x_758 : vec4<f32> = u_xlat1;
  let x_760 : vec3<f32> = (vec3<f32>(x_758.x, x_758.y, x_758.z) + vec3<f32>(0.00390625f, 0.00390625f, 0.00390625f));
  let x_761 : vec4<f32> = u_xlat0;
  u_xlat0 = vec4<f32>(x_760.x, x_760.y, x_760.z, x_761.w);
  u_xlat0.w = 0.75f;
  let x_767 : vec4<f32> = u_xlat0;
  let x_769 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_767.x, x_767.w));
  u_xlat0.x = x_769.x;
  let x_775 : f32 = u_xlat0.x;
  SV_Target0.x = x_775;
  let x_779 : f32 = SV_Target0.x;
  SV_Target0.x = clamp(x_779, 0.0f, 1.0f);
  let x_785 : vec4<f32> = u_xlat0;
  let x_787 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_785.y, x_785.w));
  u_xlat0.x = x_787.y;
  let x_793 : vec4<f32> = u_xlat0;
  let x_795 : vec4<f32> = textureSample(x_Curves, sampler_Curves, vec2<f32>(x_793.z, x_793.w));
  u_xlat0.y = x_795.z;
  let x_798 : vec4<f32> = u_xlat0;
  let x_799 : vec2<f32> = vec2<f32>(x_798.x, x_798.y);
  let x_800 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_800.x, x_799.x, x_799.y, x_800.w);
  let x_802 : vec4<f32> = SV_Target0;
  let x_806 : vec2<f32> = clamp(vec2<f32>(x_802.y, x_802.z), vec2<f32>(0.0f, 0.0f), vec2<f32>(1.0f, 1.0f));
  let x_807 : vec4<f32> = SV_Target0;
  SV_Target0 = vec4<f32>(x_807.x, x_806.x, x_806.y, x_807.w);
  SV_Target0.w = 1.0f;
  return;
}

...
layout(set = 0, binding = 1) uniform mediump texture2D _BloomTex;
layout(location = 0) in highp vec2 vs_TEXCOORD0;
layout(location = 1) in highp vec2 vs_TEXCOORD1;
layout(location = 0) out highp vec4 SV_Target0;
layout(set = 0, binding = 2) uniform mediump  sampler sampler_MainTex;
layout(set = 0, binding = 3) uniform mediump  sampler sampler_BloomTex;
void main()
{
vec4 u_xlat0;
vec4 u_xlat1;
...
vec4 u_xlat5;
    ...
    u_xlat0 = texture(sampler2D(_MainTex, sampler_MainTex), u_xlat0.xy);
    u_xlat1.x = 1.0;
    ...
    u_xlat4 = texture(sampler2D(_MainTex, sampler_MainTex), u_xlat3.xy);
    u_xlat3 = texture(sampler2D(_MainTex, sampler_MainTex), u_xlat3.zw);
    ...
    u_xlat1 = texture(sampler2D(_BloomTex, sampler_BloomTex), vs_TEXCOORD1.xy);

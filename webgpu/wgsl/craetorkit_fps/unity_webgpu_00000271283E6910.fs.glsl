...
bool u_xlatb2;
    ...
    u_xlat1.x = texture(sampler2D(_AlphaTex, sampler_AlphaTex), vs_TEXCOORD0.xy).x;
    u_xlat1.w = u_xlat1.x + _TextureSampleAdd.w;
    u_xlat2.x = u_xlat0.w * u_xlat1.w + -0.00100000005;
    u_xlatb2 = u_xlat2.x<0.0;
    if(((int(u_xlatb2) * int(0xffffffffu)))!=0){discard;}
    u_xlat2.xyz = texture(sampler2D(_MainTex, sampler_MainTex), vs_TEXCOORD0.xy).xyz;
    u_xlat1.xyz = u_xlat2.xyz + _TextureSampleAdd.xyz;

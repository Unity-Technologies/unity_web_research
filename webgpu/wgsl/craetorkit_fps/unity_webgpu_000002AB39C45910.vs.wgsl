...
fn main_1() {
  ...
  vs_TEXCOORD2 = vec4<f32>(x_186.w, x_186.w, x_186.w, x_186.x);
  let x_189 : vec4<f32> = u_xlat0;
  vs_TEXCOORD3 = vec4<f32>(x_189.w, x_189.w, x_189.w, x_189.y);
  let x_194 : f32 = u_xlat0.z;
  vs_TEXCOORD4.w = x_194;
  let x_199 : vec3<f32> = in_NORMAL0;
  let x_201 : vec4<f32> = x_18.unity_WorldToObject[0i];
  u_xlat0.x = dot(x_199, vec3<f32>(x_201.x, x_201.y, x_201.z));
  let x_206 : vec3<f32> = in_NORMAL0;
  let x_208 : vec4<f32> = x_18.unity_WorldToObject[1i];
  u_xlat0.y = dot(x_206, vec3<f32>(x_208.x, x_208.y, x_208.z));
  let x_212 : vec3<f32> = in_NORMAL0;
  let x_214 : vec4<f32> = x_18.unity_WorldToObject[2i];
  u_xlat0.z = dot(x_212, vec3<f32>(x_214.x, x_214.y, x_214.z));
  let x_219 : vec4<f32> = u_xlat0;
  let x_221 : vec4<f32> = u_xlat0;
  u_xlat6 = dot(vec3<f32>(x_219.x, x_219.y, x_219.z), vec3<f32>(x_221.x, x_221.y, x_221.z));
  let x_224 : f32 = u_xlat6;
  u_xlat6 = inverseSqrt(x_224);
  ...
  let x_231 : vec4<f32> = vs_TEXCOORD4;
  vs_TEXCOORD4 = vec4<f32>(x_230.x, x_230.y, x_230.z, x_231.w);
